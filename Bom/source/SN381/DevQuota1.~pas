  ...
  begin
   ...
   begin
      ...
      begin
         ...
         SQL.Add('      OUTER APPLY (SELECT TOP 1 PriceFormula FROM MaterialCBD_His M');
         SQL.Add('          WHERE M.ZSBH=XXZLS.CSBH AND M.CLBH=XXZLS.CLBH AND M.YN=0');
         SQL.Add('          AND M.PriceFormula<>'''' AND M.PriceFormula IS NOT NULL');
         if (CB_FTY.ItemIndex=1) and (main.Edit2.Text='CDC') then
           SQL.Add('          AND M.GSBH=''VB1''')
         else
           SQL.Add('          AND M.GSBH='''+main.Edit2.Text+'''');
         if (main.Edit2.Text='CDC') or (main.Edit2.Text='VB1') then
         begin
           SQL.Add('          AND M.Season=(case when SUBSTRING(CostingPriceList.PricingSeason,3,1)=''F'' then ''FH''+ SUBSTRING(CostingPriceList.PricingSeason,1,2)');
           SQL.Add('          when SUBSTRING(CostingPriceList.PricingSeason,3,1)=''H'' then ''FH''+ SUBSTRING(CostingPriceList.PricingSeason,1,2)');
           SQL.Add('          when SUBSTRING(CostingPriceList.PricingSeason,3,1)=''S'' then ''SS''+ SUBSTRING(CostingPriceList.PricingSeason,1,2)');
         ...
         begin
           SQL.Add('          AND M.Season=(case when SUBSTRING(CostingPriceList.PricingSeason,1,2)=''FA'' then ''FW''+ Right(CostingPriceList.PricingSeason,2)');
           SQL.Add('          when SUBSTRING(CostingPriceList.PricingSeason,1,2)=''WI'' then ''FW''+ Right(CostingPriceList.PricingSeason,2)');
           SQL.Add('          when SUBSTRING(CostingPriceList.PricingSeason,1,2)=''SP'' then ''SS''+ Right(CostingPriceList.PricingSeason,2)');
           SQL.Add('          when SUBSTRING(CostingPriceList.PricingSeason,1,2)=''SU'' then ''SS''+ Right(CostingPriceList.PricingSeason,2) end)');
         end
         else
           SQL.Add('          AND M.Season=CostingPriceList.PricingSeason');
         SQL.Add('      ) MaterialCBD_His');
...
begin
  ...
  begin
    ...
    //20251216 checked with Kelly different KHDH same price    09/06/2026 open it
    sql.add('and MaterialCBD_His.KHDH='''+KHDH+'''');
    if Edit19.Text<>'' then
...
begin               
  ...
  begin
    ...
    SQL.Add('where MaterialCBD_His.season like ''%'+edit4.text+'%'' ');
    sql.add('and MaterialCBD_His.KFCQ='''+KFCQ+''' ');
    sql.add('and MaterialCBD_His.KHDH='''+KHDH+'''');
...
var 
  eclApp, WorkBook, Sheet1, Sheet2: olevariant;
  i, j: integer;
begin
  ...
  begin
    try
      ...
      Application.MessageBox('Khong the mo Microsoft Excel', 'Loi', MB_OK + MB_ICONWarning);
      Exit;
    end;
    ...
    try
      ...
      // --- SHEET 1: MaterialCBD_His ---
      Sheet1 := WorkBook.WorkSheets[1];
      Sheet1.Name := 'MaterialCBD_His';
      
      Sheet1.Cells(1, 1) := 'No';
      for i := 0 to UploadQuery.fieldcount - 1 do
      begin
        Sheet1.Cells(1, i + 2) := UploadQuery.fields[i].FieldName;
      end;
      ...
      begin
        ...
        for i := 0 to UploadQuery.fieldcount - 1 do
        begin
          // Them dau nhay don de tranh Excel tu chuyen dinh dang
          Sheet1.Cells(j, i + 2) := '''' + UploadQuery.Fields[i].AsString;
        end;
        UploadQuery.Next;
        inc(j);
      end;
      ...
      // --- LAY DU LIEU CHO SHEET 2 ---
      with TemQry1 do
      begin
        Active := false;
        SQL.Clear;
        SQL.Add('select Season,GSBH,KHBH,CWHL,CWHL_ID,USERID,USERDATE,CustomerCFMDate from MaterialCBDEx');
        // if CheckBox9.Checked = true then showmessage(SQL.Text); // Tuy ban dung
        Active := true;
      end;

      // --- SHEET 2: MaterialCBDEX ---
      // Them mot sheet moi vao sau sheet 1
      Sheet2 := WorkBook.WorkSheets.Add(After := Sheet1);
      Sheet2.Name := 'MaterialCBDEX';

      Sheet2.Cells(1, 1) := 'No';
      for i := 0 to TemQry1.fieldcount - 1 do
      begin
        Sheet2.Cells(1, i + 2) := TemQry1.fields[i].FieldName;
      end;
      ...
      begin
        ...
        for i := 0 to TemQry1.fieldcount - 1 do
        begin
          Sheet2.Cells(j, i + 2) := '''' + TemQry1.Fields[i].AsString;
        end;
        TemQry1.Next;
        inc(j);
      end;
      ...
      // Quay lai sheet 1 truoc khi hien thi
      Sheet1.Activate;
      
      showmessage('Export Excel Thanh Cong!');
      eclApp.Visible := True;
      ...
      begin
        ...
        if not VarIsEmpty(eclApp) then eclApp.Quit;
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      requestlive:=true;
      Edit;
      FieldByName('yn').value := '1';  // d嫕h d?u xo?
      Query1.Post;
    end;
    Cancel2.Enabled:=true;
    Save2.Enabled:=true;
