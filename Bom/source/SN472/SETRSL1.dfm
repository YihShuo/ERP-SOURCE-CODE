object SETRSL: TSETRSL
  Left = 405
  Top = 130
  Width = 1386
  Height = 781
  Caption = 'SETRSL'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1370
    Height = 65
    Align = alTop
    TabOrder = 0
    object BB2: TBitBtn
      Left = 56
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Insert one New Record'
      Caption = 'Insert'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      OnClick = BB2Click
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB3: TBitBtn
      Left = 106
      Top = 8
      Width = 48
      Height = 49
      Hint = 'Delete one Record'
      Caption = 'Delete'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB4: TBitBtn
      Left = 154
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Modify'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      OnClick = BB4Click
      Glyph.Data = {
        
        50BB555555555555575F555555555555550B5555555555555575}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB5: TBitBtn
      Left = 203
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Save Current'
      Caption = 'Save'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 3
      OnClick = BB5Click
      Glyph.Data = {
        
        5990555555555555577755555555555555555555555555555555}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB6: TBitBtn
      Left = 252
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Cancel'
      Caption = 'Cancel'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 4
      OnClick = BB6Click
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB7: TBitBtn
      Left = 301
      Top = 8
      Width = 50
      Height = 49
      Hint = 'Exit Current Form'
      Caption = 'Exit'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 5
      OnClick = BB7Click
      Glyph.Data = {
        
        EE33337FFFFFFFFFFF7F33EEEEEEEEEEEE333377777777777773}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BBT1: TBitBtn
      Left = 368
      Top = 8
      Width = 49
      Height = 49
      Caption = 'First'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 6
      Glyph.Data = {
        
        CCC333333333333777FF33333333333333CC3333333333333773}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BBT2: TBitBtn
      Left = 418
      Top = 8
      Width = 48
      Height = 49
      Hint = 'Insert one New Record'
      Caption = 'Prior'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 7
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BBT3: TBitBtn
      Left = 467
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Delete one Record'
      Caption = 'Next'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 8
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BBT4: TBitBtn
      Left = 516
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Last'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 9
      Glyph.Data = {
        
        33333777333333333333CC333333333333337733333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object bbt5: TBitBtn
      Left = 584
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Print'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 10
      Glyph.Data = {
        
        33333337FFFF7733333333300000033333333337777773333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object bbt6: TBitBtn
      Left = 634
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Excel'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 11
      OnClick = bbt6Click
      Glyph.Data = {
        
        3BB33773333773333773B333333B3333333B7333333733333337}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
  end
  object PC1: TPageControl
    Left = 0
    Top = 177
    Width = 1370
    Height = 565
    ActivePage = TS3
    Align = alClient
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    OnChange = PC1Change
    OnChanging = PC1Changing
    object TS1: TTabSheet
      Caption = 'OK CFM '
      object DBGrid1: TDBGridEh
        Left = 0
        Top = 0
        Width = 1362
        Height = 534
        Align = alClient
        DataSource = DS1
        Flat = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -12
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        FooterRowCount = 1
        FrozenCols = 1
        ParentFont = False
        SumList.Active = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -15
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        UseMultiTitle = True
        OnCellClick = DBGrid1CellClick
        OnGetCellParams = DBGrid1GetCellParams
        Columns = <
          item
            EditButtons = <>
            FieldName = 'cldh'
            Footers = <
              item
                ValueType = fvtCount
              end>
            ReadOnly = True
            Title.Caption = #26009#34399'|MatNo'
            Width = 91
          end
          item
            EditButtons = <>
            FieldName = 'cllb'
            Footers = <>
            ReadOnly = True
            Title.Caption = #39006#21029'|cllb'
            Width = 49
          end
          item
            EditButtons = <>
            FieldName = 'zsdh'
            Footers = <>
            ReadOnly = True
            Title.Caption = #24288#21830'ID|SupID'
            Width = 58
          end
          item
            EditButtons = <>
            FieldName = 'zsywjc'
            Footers = <>
            ReadOnly = True
            Title.Caption = #24288#21830#21517#31281'|SuppName'
            Width = 200
          end
          item
            EditButtons = <>
            FieldName = 'ywpm'
            Footers = <>
            ReadOnly = True
            Title.Caption = #26448#26009#33521#25991'|ywpm'
            Width = 192
          end
          item
            EditButtons = <>
            FieldName = 'zwpm'
            Footers = <>
            ReadOnly = True
            Title.Caption = #26448#26009#20013#25991'|zwpm'
            Width = 214
          end
          item
            EditButtons = <>
            FieldName = 'dwbh'
            Footers = <>
            ReadOnly = True
            Title.Caption = #21934#20301'|dwbh'
            Width = 63
          end
          item
            EditButtons = <>
            FieldName = 'cqdh'
            Footers = <>
            ReadOnly = True
            Title.Caption = #25505#21312'|cqdh'
            Width = 40
          end
          item
            EditButtons = <>
            FieldName = 'DCYN'
            Footers = <>
            ReadOnly = True
            Title.Caption = #38283#30332'RSL|DCYN'
            Width = 48
          end
          item
            EditButtons = <>
            FieldName = 'DCCFM'
            Footers = <>
            ReadOnly = True
            Title.Caption = #38283#30332'RSL|DCCFM'
            Width = 51
          end
          item
            DisplayFormat = 'yyyy/mm/dd'
            EditButtons = <>
            FieldName = 'DCDate'
            Footers = <>
            ReadOnly = True
            Title.Caption = #38283#30332'RSL|DCDate'
            Width = 92
          end
          item
            Color = clWhite
            EditButtons = <>
            FieldName = 'DCFilePage'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlue
            Font.Height = -16
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsUnderline]
            Footers = <>
            ReadOnly = True
            Title.Caption = #38283#30332'RSL|DCFilePage'
            Width = 130
          end
          item
            EditButtons = <>
            FieldName = 'DCUpdateID'
            Footers = <>
            ReadOnly = True
            Title.Caption = #38283#30332'RSL|DCUpdateID'
          end
          item
            DisplayFormat = 'yyyy/mm/dd'
            EditButtons = <>
            FieldName = 'DCUpdateDate'
            Footers = <>
            ReadOnly = True
            Title.Caption = #38283#30332'RSL|DCUpdateDate'
            Width = 106
          end
          item
            EditButtons = <>
            FieldName = 'PDYN'
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|PDYN'
            Width = 56
          end
          item
            EditButtons = <>
            FieldName = 'PDCFM'
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|PDCFM'
            Width = 50
          end
          item
            EditButtons = <>
            FieldName = 'PDDate'
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|PDDate'
            Width = 92
          end
          item
            EditButtons = <>
            FieldName = 'PDFilePage'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlue
            Font.Height = -16
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsUnderline]
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|RSL TP1'
            Width = 136
          end
          item
            EditButtons = <>
            FieldName = 'PDFilePageT2'
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|RSL TP2'
            Width = 113
          end
          item
            EditButtons = <>
            FieldName = 'PDUpdateID'
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|PDUpdateID'
            Width = 75
          end
          item
            DisplayFormat = 'yyyy/mm/dd'
            EditButtons = <>
            FieldName = 'PDUpdateDate'
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|PDUpdateDate'
            Width = 118
          end
          item
            EditButtons = <>
            FieldName = 'Memo'
            Footers = <>
            ReadOnly = True
            Title.Caption = #20633#35387'|Memo'
            Width = 118
          end
          item
            EditButtons = <>
            FieldName = 'Gender'
            Footers = <>
            ReadOnly = True
            Title.Caption = #20633#35387'|GenderMemo'
            Width = 118
          end>
      end
    end
    object TS2: TTabSheet
      Caption = 'DC'
      ImageIndex = 1
      object DBGrid2: TDBGridEh
        Left = 0
        Top = 0
        Width = 1362
        Height = 534
        Align = alClient
        DataSource = DS1
        Flat = False
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -15
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        FooterRowCount = 1
        FrozenCols = 1
        PopupMenu = PopupMenu1
        SumList.Active = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -15
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        UseMultiTitle = True
        OnCellClick = DBGrid2CellClick
        OnEditButtonClick = DBGrid2EditButtonClick
        OnGetCellParams = DBGrid2GetCellParams
        Columns = <
          item
            EditButtons = <>
            FieldName = 'cldh'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <
              item
                ValueType = fvtCount
              end>
            ReadOnly = True
            Title.Caption = #26009#34399'|MatNo'
            Width = 87
          end
          item
            EditButtons = <>
            FieldName = 'cllb'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #39006#21029'|cllb'
            Width = 23
          end
          item
            Color = 12713983
            EditButtons = <>
            FieldName = 'zsdh'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #24288#21830'ID|SuppID'
            Width = 57
          end
          item
            EditButtons = <>
            FieldName = 'zsywjc'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #24288#21830#21517#31281'|SuppName'
            Width = 123
          end
          item
            EditButtons = <>
            FieldName = 'ywpm'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #26448#26009#33521#25991'|ywpm'
            Width = 192
          end
          item
            EditButtons = <>
            FieldName = 'zwpm'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #26448#26009#20013#25991'|zwpm'
            Width = 214
          end
          item
            EditButtons = <>
            FieldName = 'dwbh'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #21934#20301'|dwbh'
            Width = 63
          end
          item
            EditButtons = <>
            FieldName = 'cqdh'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #25505#21312'|cqdh'
            Width = 40
          end
          item
            EditButtons = <>
            FieldName = 'DCYN'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #38283#30332'RSL|DCYN'
            Width = 48
          end
          item
            EditButtons = <>
            FieldName = 'DCCFM'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #38283#30332'RSL|DCCFM'
            Width = 48
          end
          item
            Color = 12713983
            EditButtons = <>
            FieldName = 'DCDate'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            Title.Caption = #38283#30332'RSL|DCDate'
            Width = 82
          end
          item
            Color = 12713983
            EditButtons = <>
            FieldName = 'DCFilePage'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #38283#30332'RSL|DCFilePage'
            Width = 130
          end
          item
            EditButtons = <>
            FieldName = 'DCUpdateID'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #38283#30332'RSL|DCUpdateID'
            Width = 73
          end
          item
            DisplayFormat = 'yyyy/mm/dd'
            EditButtons = <>
            FieldName = 'DCUpdateDate'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #38283#30332'RSL|DCUpdateDate'
            Width = 88
          end
          item
            EditButtons = <>
            FieldName = 'Memo'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            Title.Caption = #20633#35387'|Memo'
            Width = 199
          end>
      end
    end
    object TS3: TTabSheet
      Caption = 'PD'
      ImageIndex = 2
      object DBGrid3: TDBGridEh
        Left = 0
        Top = 0
        Width = 1362
        Height = 534
        Align = alClient
        DataSource = DS1
        Flat = False
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -15
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        FooterRowCount = 1
        FrozenCols = 1
        Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgMultiSelect]
        PopupMenu = PopupMenu1
        SumList.Active = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -15
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        UseMultiTitle = True
        OnCellClick = DBGrid3CellClick
        OnEditButtonClick = DBGrid3EditButtonClick
        OnGetCellParams = DBGrid3GetCellParams
        Columns = <
          item
            EditButtons = <>
            FieldName = 'cldh'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <
              item
                ValueType = fvtCount
              end>
            ReadOnly = True
            Title.Caption = #26009#34399'|MatNo'
            Width = 93
          end
          item
            EditButtons = <>
            FieldName = 'cllb'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #39006#21029'|cllb'
            Width = 49
          end
          item
            Color = 12713983
            EditButtons = <>
            FieldName = 'zsdh'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #24288#21830'ID|SupID'
            Width = 58
          end
          item
            EditButtons = <>
            FieldName = 'zsywjc'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #24288#21830#21517#31281'|SuppName'
            Width = 119
          end
          item
            EditButtons = <>
            FieldName = 'ywpm'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #26448#26009#33521#25991'|ywpm'
            Width = 180
          end
          item
            EditButtons = <>
            FieldName = 'zwpm'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #26448#26009#20013#25991'|zwpm'
            Width = 196
          end
          item
            EditButtons = <>
            FieldName = 'dwbh'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #21934#20301'|dwbh'
            Width = 63
          end
          item
            EditButtons = <>
            FieldName = 'cqdh'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #25505#21312'|cqdh'
            Width = 40
          end
          item
            EditButtons = <>
            FieldName = 'PDYN'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|PDYN'
            Width = 45
          end
          item
            EditButtons = <>
            FieldName = 'PDCFM'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|PDCFM'
            Width = 48
          end
          item
            Color = 12713983
            EditButtons = <>
            FieldName = 'PDDate'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            Title.Caption = #37327#29986'RSL|PDDate'
            Width = 88
          end
          item
            Color = 12713983
            EditButtons = <>
            FieldName = 'PDFilePage'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|RSL TP1'
            Width = 117
          end
          item
            Color = 12713968
            EditButtons = <>
            FieldName = 'PDFilePageT2'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            Title.Caption = #37327#29986'RSL|RSL TP2'
            Width = 111
          end
          item
            EditButtons = <>
            FieldName = 'PDUpdateID'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|PDUpdateID'
            Width = 66
          end
          item
            DisplayFormat = 'yyyy/mm/dd'
            EditButtons = <>
            FieldName = 'PDUpdateDate'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|PDUpdateDate'
            Width = 99
          end
          item
            Color = 12713983
            EditButtons = <>
            FieldName = 'Memo'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            Title.Caption = #20633#35387'|Memo'
            Width = 200
          end
          item
            EditButtons = <>
            FieldName = 'Gender'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #20633#35387'|GenderMemo'
            Width = 85
          end>
      end
    end
    object TS4: TTabSheet
      Hint = 'Have order No part type'
      Caption = 'Expire CFM'
      ImageIndex = 3
      object DBGrid4: TDBGridEh
        Left = 0
        Top = 0
        Width = 1362
        Height = 534
        Align = alClient
        DataSource = DS1
        Flat = False
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -15
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        FooterRowCount = 1
        FrozenCols = 1
        SumList.Active = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -15
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        UseMultiTitle = True
        OnCellClick = DBGrid1CellClick
        OnGetCellParams = DBGrid1GetCellParams
        Columns = <
          item
            EditButtons = <>
            FieldName = 'cldh'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <
              item
                ValueType = fvtCount
              end>
            ReadOnly = True
            Title.Caption = #26009#34399'|MatNo'
            Width = 91
          end
          item
            EditButtons = <>
            FieldName = 'cllb'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #39006#21029'|cllb'
            Width = 49
          end
          item
            EditButtons = <>
            FieldName = 'zsdh'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #24288#21830'ID|SupID'
            Width = 58
          end
          item
            EditButtons = <>
            FieldName = 'zsywjc'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #24288#21830#21517#31281'|SuppName'
            Width = 200
          end
          item
            EditButtons = <>
            FieldName = 'ywpm'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #26448#26009#33521#25991'|ywpm'
            Width = 192
          end
          item
            EditButtons = <>
            FieldName = 'zwpm'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #26448#26009#20013#25991'|zwpm'
            Width = 214
          end
          item
            EditButtons = <>
            FieldName = 'dwbh'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #21934#20301'|dwbh'
            Width = 63
          end
          item
            EditButtons = <>
            FieldName = 'cqdh'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #25505#21312'|cqdh'
            Width = 40
          end
          item
            EditButtons = <>
            FieldName = 'DCYN'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #38283#30332'RSL|DCYN'
            Width = 48
          end
          item
            EditButtons = <>
            FieldName = 'DCCFM'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #38283#30332'RSL|DCCFM'
            Width = 51
          end
          item
            DisplayFormat = 'yyyy/mm/dd'
            EditButtons = <>
            FieldName = 'DCDate'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #38283#30332'RSL|DCDate'
            Width = 92
          end
          item
            Color = clWhite
            EditButtons = <>
            FieldName = 'DCFilePage'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #38283#30332'RSL|DCFilePage'
            Width = 130
          end
          item
            EditButtons = <>
            FieldName = 'DCUpdateID'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #38283#30332'RSL|DCUpdateID'
            Width = 74
          end
          item
            DisplayFormat = 'yyyy/mm/dd'
            EditButtons = <>
            FieldName = 'DCUpdateDate'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #38283#30332'RSL|DCUpdateDate'
            Width = 106
          end
          item
            EditButtons = <>
            FieldName = 'PDYN'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|PDYN'
            Width = 56
          end
          item
            EditButtons = <>
            FieldName = 'PDCFM'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|PDCFM'
            Width = 50
          end
          item
            EditButtons = <>
            FieldName = 'PDDate'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|PDDate'
            Width = 92
          end
          item
            EditButtons = <>
            FieldName = 'PDFilePage'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|PDFilePage'
            Width = 136
          end
          item
            EditButtons = <>
            FieldName = 'PDFilePageT2'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|PDFilePage T2'
            Width = 121
          end
          item
            EditButtons = <>
            FieldName = 'PDUpdateID'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|PDUpdateID'
            Width = 75
          end
          item
            DisplayFormat = 'yyyy/mm/dd'
            EditButtons = <>
            FieldName = 'PDUpdateDate'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #37327#29986'RSL|PDUpdateDate'
            Width = 118
          end
          item
            EditButtons = <>
            FieldName = 'Memo'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #20633#35387'|Memo'
            Width = 200
          end
          item
            EditButtons = <>
            FieldName = 'Gender'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Footers = <>
            ReadOnly = True
            Title.Caption = #20633#35387'|GenderMemo'
            Width = 119
          end>
      end
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 65
    Width = 1370
    Height = 112
    Align = alTop
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 2
    object clbh: TLabel
      Left = 7
      Top = 24
      Width = 49
      Height = 17
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'MatNo'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label3: TLabel
      Left = 168
      Top = 24
      Width = 90
      Height = 17
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'SuppName'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label1: TLabel
      Left = 487
      Top = 64
      Width = 40
      Height = 16
      Caption = 'Mode'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object Label2: TLabel
      Left = 8
      Top = 64
      Width = 64
      Height = 17
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'MatName'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object MonthLabel: TLabel
      Left = 532
      Top = 64
      Width = 70
      Height = 16
      Caption = 'Valid Month'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object CBX1: TCheckBox
      Left = 455
      Top = 23
      Width = 223
      Height = 17
      Caption = 'Nearly expired <=1 Month'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 4
    end
    object CLDHEdit: TEdit
      Left = 71
      Top = 24
      Width = 106
      Height = 28
      CharCase = ecUpperCase
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
    object Button1: TButton
      Left = 930
      Top = 23
      Width = 81
      Height = 32
      Caption = 'QUERY'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      OnClick = Button1Click
    end
    object ZSYWJCEdit: TEdit
      Left = 268
      Top = 24
      Width = 170
      Height = 28
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
    end
    object CB1: TComboBox
      Left = 626
      Top = 23
      Width = 185
      Height = 24
      Style = csDropDownList
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      TabOrder = 3
      Visible = False
      OnChange = CB1Change
      Items.Strings = (
        'In Processing'
        'No Audit'
        'Blank Memo'
        'All list')
    end
    object CheckBox1: TCheckBox
      Left = 822
      Top = 28
      Width = 40
      Height = 17
      Caption = 'TW'
      Checked = True
      State = cbChecked
      TabOrder = 5
    end
    object CheckBox2: TCheckBox
      Left = 822
      Top = 64
      Width = 40
      Height = 17
      Caption = 'VN'
      Checked = True
      State = cbChecked
      TabOrder = 6
    end
    object CheckBox3: TCheckBox
      Left = 865
      Top = 28
      Width = 49
      Height = 17
      Caption = 'Mine'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 7
      OnClick = CheckBox3Click
    end
    object YWPMEdit: TEdit
      Left = 87
      Top = 64
      Width = 170
      Height = 28
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 8
    end
    object MonthEdit: TEdit
      Left = 454
      Top = 64
      Width = 28
      Height = 24
      TabOrder = 9
      Text = '12'
    end
    object KidsCKbox: TCheckBox
      Left = 866
      Top = 64
      Width = 52
      Height = 17
      Caption = 'Kids'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 10
    end
    object YWPMEdit1: TEdit
      Left = 268
      Top = 64
      Width = 170
      Height = 28
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 11
    end
  end
  object CLZL: TQuery
    DatabaseName = 'DB'
    SQL.Strings = (
      
        'select clzl.cldh, clzl.cllb, rstlist.zsdh, zszl.zsywjc, clzl.ywp' +
        'm, clzl.zwpm,    '
      
        'clzl.dwbh, clzl.cqdh, rstlist.DCYN, rstlist.PDYN,rstlist.ID, rst' +
        'list.DCCFMDate, rstlist.DCCFMID,  '
      
        'rstlist.PDCFMDate, rstlist.PDCFMID, rstlist.DCDate, rstlist.PDDa' +
        'te,              '
      
        'rstlist.DCFilePage, rstlist.PDFilePage, rstlist.createID, rstlis' +
        't.createDate,    '
      
        'rstlist.DCUpdateID, rstlist.DCUpdateDate,                       ' +
        '                 '
      
        'rstlist.PDUpdateID, rstlist.PDUpdateDate, rstlist.DCCFM,  rstlis' +
        't.PDCFM,         '
      
        'rstlist.UpdateID, rstlist.UpdateDate,rstlist.GSBH, rstlist.Memo,' +
        'rstlist.Gender,rstlist.PDFilePageT2                  '
      'from clzl                                       '
      'left join clzl_dev on clzl_dev.cldh=clzl.cldh   '
      'left join rstlist on clzl.cldh = rstlist.cldh   '
      'left join zszl on rstlist.zsdh = zszl.zsdh      '
      
        'left join zszl_dev on zszl_dev.zsdh = zszl.zsdh and zszl_dev.gsb' +
        'h = '#39'VA12'#39
      'where clzl.cldh like '#39'%'#39' and clzl.ywpm like '#39'%%'#39
      '  and isnull(zszl.zsywjc,'#39#39') like '#39'%%'#39' '
      ' and rstlist.gsbh='#39'VA12'#39
      
        '  and ((rstlist.DCYN = '#39'Y'#39' and rstlist.DCYN = rstlist.DCCFM and ' +
        'left(convert(varchar, DateAdd(year,1,isnull(DCDate, '#39'2050-10-10'#39 +
        ')),112), 6) > left(convert(varchar, getdate(),112), 6)) '
      
        '   or  (rstlist.PDYN = '#39'Y'#39' and rstlist.PDYN = rstlist.PDCFM and ' +
        'left(convert(varchar, DateAdd(year,1,isnull(PDDate, '#39'2050-10-10'#39 +
        ')),112), 6) > left(convert(varchar, getdate(),112), 6)) )'
      'order by clzl.cldh')
    UpdateObject = UpRSTlist
    Left = 116
    Top = 308
    object CLZLID: TFloatField
      FieldName = 'ID'
    end
    object CLZLcldh: TStringField
      FieldName = 'cldh'
      Size = 10
    end
    object CLZLcllb: TStringField
      FieldName = 'cllb'
      Size = 4
    end
    object CLZLzsdh: TStringField
      FieldName = 'zsdh'
      Size = 6
    end
    object CLZLzsywjc: TStringField
      FieldName = 'zsywjc'
    end
    object CLZLzwpm: TStringField
      FieldName = 'zwpm'
      Size = 200
    end
    object CLZLywpm: TStringField
      FieldName = 'ywpm'
      Size = 200
    end
    object CLZLdwbh: TStringField
      FieldName = 'dwbh'
      Size = 4
    end
    object CLZLcqdh: TStringField
      FieldName = 'cqdh'
      Size = 4
    end
    object CLZLDCYN: TStringField
      FieldName = 'DCYN'
      Size = 1
    end
    object CLZLPDYN: TStringField
      FieldName = 'PDYN'
      Size = 1
    end
    object CLZLDCCFM: TStringField
      FieldName = 'DCCFM'
      Size = 1
    end
    object CLZLDCCFMDate: TDateTimeField
      FieldName = 'DCCFMDate'
    end
    object CLZLDCCFMID: TStringField
      FieldName = 'DCCFMID'
      Size = 10
    end
    object CLZLPDCFM: TStringField
      FieldName = 'PDCFM'
      Size = 1
    end
    object CLZLPDCFMDate: TDateTimeField
      FieldName = 'PDCFMDate'
    end
    object CLZLPDCFMID: TStringField
      FieldName = 'PDCFMID'
      Size = 10
    end
    object CLZLDCDate: TDateTimeField
      FieldName = 'DCDate'
    end
    object CLZLDCFilePage: TStringField
      FieldName = 'DCFilePage'
      Size = 150
    end
    object CLZLPDDate: TDateTimeField
      FieldName = 'PDDate'
    end
    object CLZLPDFilePage: TStringField
      FieldName = 'PDFilePage'
      Size = 150
    end
    object CLZLCreateID: TStringField
      FieldName = 'CreateID'
      Size = 10
    end
    object CLZLCreateDate: TDateTimeField
      FieldName = 'CreateDate'
    end
    object CLZLUpdateID: TStringField
      FieldName = 'DCUpdateID'
      Size = 10
    end
    object CLZLUpdateDate: TDateTimeField
      FieldName = 'DCUpdateDate'
    end
    object CLZLPDUpdateID: TStringField
      FieldName = 'PDUpdateID'
      Size = 10
    end
    object CLZLPDUpdateDate: TDateTimeField
      FieldName = 'PDUpdateDate'
    end
    object CLZLUpdateID2: TStringField
      FieldName = 'UpdateID'
      Size = 10
    end
    object CLZLUpdateDate2: TDateTimeField
      FieldName = 'UpdateDate'
    end
    object CLZLGSBH: TStringField
      FieldName = 'GSBH'
      FixedChar = True
      Size = 4
    end
    object CLZLMemo: TStringField
      FieldName = 'Memo'
      Size = 200
    end
    object CLZLGender: TStringField
      FieldName = 'Gender'
      FixedChar = True
      Size = 50
    end
    object CLZLPDFilePageT2: TStringField
      FieldName = 'PDFilePageT2'
      FixedChar = True
      Size = 150
    end
  end
  object DS1: TDataSource
    DataSet = CLZL
    Left = 148
    Top = 308
  end
  object UpRSTlist: TUpdateSQL
    ModifySQL.Strings = (
      'update rstlist'
      'set'
      '  ZSDH = :ZSDH,'
      '  DCYN = :DCYN,'
      '  PDYN = :PDYN,'
      '  DCDate = :DCDate,'
      '  PDDate = :PDDate,'
      '  DCFilePage = :DCFilePage,'
      '  PDFilePage = :PDFilePage,'
      '  PDFilePageT2 = :PDFilePageT2,'
      '  DCCFM = :DCCFM,'
      '  PDCFM = :PDCFM,'
      '  DCUpdateID = :DCUpdateID,'
      '  DCUpdateDate = :DCUpdateDate,'
      '  PDUpdateID = :PDUpdateID,'
      '  PDUpdateDate = :PDUpdateDate,'
      '  UpdateID = :UpdateID,'
      '  UpdateDate = :UpdateDate,'
      '  GSBH=:GSBH,'
      '  Memo=:Memo,'
      '  Gender=:Gender'
      'where'
      '  CLDH = :OLD_CLDH and'
      '  ZSDH = :OLD_ZSDH and'
      '  ID =:OLD_ID')
    InsertSQL.Strings = (
      'insert into rstlist'
      
        '  (CLDH, ZSDH, DCYN, PDYN, DCDate, PDDate, DCCFM, PDCFM, DCFileP' +
        'age, PDFilePage, CreateID, CreateDate, DCUpdateID, DCUpdateDate,' +
        ' PDUpdateID, PDUpdateDate, UpdateID, UpdateDate,GSBH)'
      'values'
      
        '  (:CLDH, :ZSDH, :DCYN, :PDYN, :DCDate, :PDDate, :DCCFM, :PDCFM ' +
        ', :DCFilePage, :PDFilePage, :CreateID, :CreateDate, :DCUpdateID,' +
        ' :DCUpdateDate , :PDUpdateID, :PDUpdateDate, :UpdateID, :UpdateD' +
        'ate,:GSBH)'
      ''
      ''
      '')
    Left = 116
    Top = 340
  end
  object OpenDialog1: TOpenDialog
    Left = 116
    Top = 276
  end
  object PopupMenu1: TPopupMenu
    Left = 148
    Top = 340
    object mun1: TMenuItem
      Caption = 'Delete Report RSL'
      Enabled = False
      OnClick = mun1Click
    end
    object mun2: TMenuItem
      Caption = 'Delete Report RSL KIDS'
      Enabled = False
      OnClick = mun2Click
    end
    object N1: TMenuItem
      Caption = '-'
    end
    object mun3: TMenuItem
      Caption = 'SET [Memo] TO ...'
      Enabled = False
      OnClick = SetClick
    end
    object N2: TMenuItem
      Caption = '-'
    end
    object mun4: TMenuItem
      Caption = 'CLEAR [Memo]'
      Enabled = False
      OnClick = SetClick
    end
  end
end
