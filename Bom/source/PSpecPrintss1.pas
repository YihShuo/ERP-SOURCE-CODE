...
with TempSQL do
  begin
    ...
    //訂單號碼明細
    sql.add('select 5 as NO,'+''''+'Quantity  '+''''+' as DDType,'+''''+Qty+' '+''''+' as SizeType,#XXCC.BOMCC,');
...
//QMatList.Active:=true;
  ...
  begin
    ...
    sql.add('where BomExport.ZLBH=:DDBH');
...
with MemoOth do
 ...
 with MemoOth do
  begin
    ...
    sql.Add('where  zlzls2.CLBH=CLBZZL.CLDH and  zlzls2.ZLBH=:DDBH)')   ;
    sql.add('and ZYBB='+''''+'E'+'''');
