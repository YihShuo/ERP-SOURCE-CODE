...
var
  D_PSpecPrintss: TD_PSpecPrintss;
...
begin
  ...
  begin
    try
      ...
      BomN53_ISO:=MyIni.ReadString('ERP','BomN53_ISO','');
    finally
      MyIni.Free;
    end;
  end;
...
procedure TD_PSpecPrintss.FormClose(Sender: TObject;var Action: TCloseAction);
begin
  Titlememo.close;
  MemoTemp.Active:=false;
  Gjtable.Active:=false;
  QMatList.Active:=false;
  MemoOth.Active:=false;
  Action:=CaFree;
end;

procedure TD_PSpecPrintss.TitlememoAfterOpen(DataSet: TDataSet);
...
begin
 ...
 //做交叉表
  ...
  //
  with QMatList do  //查詢Spec是否已被修改過
  begin
    Active:=false;
    sql.clear;
    sql.add('select BomExport.ZLBH ,BomExport.xh,BomExport.BWBH,BomExport.CLBH,');
    sql.add('       BomExport.MJBH,BomExport.CLSL,BomExport.USAGE,BomExport.YWSM as BWMCY,max(BomExport.CLMCY) AS CLMCY,');
  ...
  if QMatList.RecordCount <1 then //如果沒被修改過,則使用原始spec資料
  begin
    with QMatList do
    begin
      Active:=false;
  ...
  //材料主表的准備及整理**
  //orderlist.Memo1.Text:=qmatlist.SQL.Text;
  //子母材料的准備及標准寫入
  MemoTemp.active:=true;
end;

procedure TD_PSpecPrintss.ColumnHeaderBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
 Quickrep4.NewPage;
end;

procedure TD_PSpecPrintss.MemoTempAfterOpen(DataSet: TDataSet);
var a,b,c:string;
i,j:integer;
begin                   //加MEO說明文檔
...
with MemoOth do
 ...
 with MemoOth do
  begin
    ...
    sql.add('where exists(select  zlzls2.CLBH from DDZL_zlzls2 as ZLZLS2');
...
procedure TD_PSpecPrintss.FormDestroy(Sender: TObject);
begin
  QMatList.Free;
  D_PSpecPrintss:=nil;
end;

procedure TD_PSpecPrintss.QRCompositeReport1AddReports(Sender: TObject);
begin
...
procedure TD_PSpecPrintss.FormCreate(Sender: TObject);
begin
  tmpQMatList:=TStringlist.Create;
  tmpQMatList.Text:=QMatList.SQL.Text;
  ReadIni();
end;

procedure TD_PSpecPrintss.QRCompositeReport1Finished(Sender: TObject);
begin
     D_PSpecPrintss.QRLabel60.Caption:=ComName;
     D_PSpecPrintss.QRLabel18.Caption:=ComName;
     D_PSpecPrintss.QRLabel23.Caption:=ComName;
     D_PSpecPrintss.QRLabel54.Caption:=ComFullName;
     D_PSpecPrintss.QRLabel46.Caption:=ComFullName;
     D_PSpecPrintss.QRLabel4.Caption:=ComFullName;
     D_PSpecPrintss.QRLabel22.Caption:=BomN53_ISO;
end;
