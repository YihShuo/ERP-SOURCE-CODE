...
type
  TMaterialNew = class(TForm)
    ...
    procedure DBGrid1GetBtnParams(Sender: TObject; Column: TColumnEh;
  ...
  //   顯示 B1 內盒資料
{
  ...
  begin
    active:=false;
    sql.clear;
    sql.add('select  distinct xxzl.xiexing , xxzl.shehao, xxzl.article, bwzl.bwdh, bwzl.ywsm , zlzls2.clbh, clzl.ywpm + '' ('' + isnull(xxzls.ccqq,'''') + ''-'' + isnull(xxzls.ccqz,'''') + '')''  from zlzls2');
    sql.add('left join ddzl on zlzls2.zlbh = ddzl.zlbh ');
    sql.add('left join xxzl on ddzl.xiexing = xxzl.xiexing and ddzl.shehao = xxzl.shehao ');
    sql.add('left join xxzls on xxzl.xiexing = xxzls.xiexing and xxzl.shehao = xxzls.shehao and zlzls2.bwbh = xxzls.bwbh and zlzls2.clbh = xxzls.clbh ');
    sql.add('left join bwzl on zlzls2.bwbh = bwdh      ');
    sql.add('left join clzl on zlzls2.clbh = clzl.cldh ');
    sql.add('where ddzl.gsbh = ''vb1''     ');
    sql.add('and zlzls2.clbh like ''h14%'' ');
    sql.add('order by xxzl.xiexing , xxzl.shehao, bwzl.ywsm  ');
    active:=true;
  end;
 }
          ...
          usinserted:
            begin
              ...
              begin
                active:=false;
                sql.Clear;
                sql.Add('select * from clzl where cldh like ''V%''');
                 ...
                 begin
                   ...
                   begin
                     if ((query1.FieldByName('USERDATE').value>=(date-2)) or (formatdatetime('yyyy/MM/dd',date)<'2008/11/01')) then
                       begin
                       // check material name exist
                        if ((query1.FieldByName('YWPM').Value<> query1.FieldByName('YWPM').OldValue) or (query1.FieldByName('YWPM').Value<> query1.FieldByName('YWPM').OldValue)) then
                          begin
                            with tempQ do
                              begin
                                active:=false;
...
procedure TMaterialNew.DBGrid1GetBtnParams(Sender: TObject;
...
procedure TMaterialNew.FormShow(Sender: TObject);
begin
