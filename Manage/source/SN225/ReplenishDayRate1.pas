...
uses ExcelSheetDlg1,ReplenishDayRateDet1,FunUnit, main1;
...
begin
  ...
  begin
    ...
    sql.add('  if object_id('+''''+'tempdb..#DailyLL'+''''+') is not null  ');
    sql.add('begin   drop table #DailyLL end   ');
    sql.add('select Convert(varchar(10),BDepartment.GSBH) as GSBH ');
...
begin
  ...
  begin
    ...
    sql.Add('where IsNull(tybh,'''')<>''Y'') BGSZL');
    sql.Add('where 1=1 and SFL=''TM'' and CKBH is not null');
    ...
    begin
      if i =  recordcount-1 then
        CKBH:= CKBH + '''' + fieldbyname('CKBH').AsString + ''''
      else
        CKBH:= CKBH + '''' + fieldbyname('CKBH').AsString + '''' + ',' ;
      Qtemp.next;
    end;
...
begin
   ...
   CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Additional\Manage_SN225.xls'),Pchar(AppDir+'Additional\Manage_SN225.xls'),false);
...
procedure TReplenishDayRate.DBGrideh1CellClick(Column: TColumnEh);
var iYear,iMonth,iDay :word;
    SDate,EDate:string;
begin

  if  ((ReplenishDayRate.Query1.FieldByName('SB').AsString=' DelACC') or (ReplenishDayRate.Query1.FieldByName('SB').AsString=' RepACC')) then
  begin
    if  ((Column.Field.FieldName<>'Factory') and (Column.Field.FieldName<>'SB') and (Column.Field.AsString<>''))   then
    begin
      if (Column.Field.FieldName<>'Total') then
      begin
        iYear:=strtoint(CBX1.text);
        iMonth:=strtoint(CBX2.text);
        iDay:=strtoint(Column.Field.FieldName);
        SDate:=datetostr(EncodeDate(iYear,iMonth,iDay));
        EDate:=datetostr(EncodeDate(iYear,iMonth,iDay));
      end else
      begin
        iYear:=strtoint(CBX1.text);
        iMonth:=strtoint(CBX2.text);
        iDay:=1;
        SDate:=datetostr(EncodeDate(iYear,iMonth,iDay));
        EDate:=datetostr(endofthemonth(EncodeDate(iYear,iMonth,iDay)));
      end;
      ReplenishDayRateDet:=TReplenishDayRateDet.Create(self);
      //
      ReplenishDayRateDet.SelSDate:=SDate;
      ReplenishDayRateDet.SelEDate:=EDate;
      ReplenishDayRateDet.showData();
      ReplenishDayRateDet.ShowModal();
    end;
  end;
  //
