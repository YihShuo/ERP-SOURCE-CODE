...
with query2 do
  begin
    ...
    {sql.add('  if object_id('+''''+'tempdb..#DUSPrice'+''''+') is not null  ');
    sql.add('begin   drop table #DUSPrice end   '); }
    execsql;
  end;
action:=cafree;
end;
...
with query1 do
  begin
   ...
   // sql.add('                      and JGZL.CFMID1 <>'+''''+'NO'+'''');
    ...
    sql.add('       sum(isnull(CWDJ.USPrice,0)) USPrice,sum(isnull(CWDJ.USPrice,0))*KC.Qty USACC ');
    ...
    sql.add('from (select #CLZLKC.CLDH,KCZLS.KCBH,KCZLS.CKBH,isnull(#CLZLKC.LastRem,0)+isnull(#CLZLKC.RKQty,0)-isnull(#CLZLKC.LLQty,0)+isnull(#CLZLKC.JGRK,0)-isnull(#CLZLKC.JGCK,0) as Qty');
    sql.add('      from #CLZLKC ');
    sql.add('      left join (select * from KCZLS where CKBH='+''''+CBX1.text+'''');
    sql.add('                 ) KCZLS on KCZLS.CLBH=#CLZLKC.CLDH');
    //sql.add('      left join CLZHZL on CLZHZL.CLDH=#CLZLKC.CLDH ')  ;
    ...
    sql.add('       ) KC ');
    sql.add('left join CLZL on CLZL.CLDH=KC.CLDH ');    
    sql.add('left join CWDJ on CWDJ.CLBH=LL.CLBH ');
    sql.add('                  and CWDJ.DJYear=convert(varchar,datepart(yyyy,getdate())) ');
    sql.add('                  and CWDJ.DJMonth=convert(varchar,datepart(MM,getdate())) ');
    sql.add('where CLZL.YWPM like '+''''+'%'+edit2.text+'%'+'''');
    sql.add('      and CLZL.YWPM like '+''''+'%'+edit2.text+'%'+'''');
    sql.add('      and KC.Qty>0.05');
    if radiogroup1.itemindex=0 then
      begin
        sql.add('     and exists(select ZLZLS2.CLBH from ZLZLS2 ');
        sql.add('                left join DDZL on DDZL.ZLBH=ZLZLS2.ZLBH ');
        sql.add('                left join KCCK on KCCK.GSBH=DDZL.GSBH and KCCK.CKBH='+''''+CBX1.Text+'''');
    ...
    if radiogroup1.itemindex=1 then
      begin
        sql.add('     and not exists(select ZLZLS2.CLBH from ZLZLS2 ');
        sql.add('                    left join DDZL on DDZL.ZLBH=ZLZLS2.ZLBH ');
        sql.add('                    left join KCCK on KCCK.GSBH=DDZL.GSBH and KCCK.CKBH='+''''+CBX1.Text+'''');
        sql.add('                    where ZLZLS2.CLBH=KC.CLDH ');
        sql.add('                          and KCCK.CKBH is not null');
...
with query2 do
  begin
   ...
   { sql.Clear;
    ...
    sql.add('into #DUSPrice from (select CLZL.CLDH,');
