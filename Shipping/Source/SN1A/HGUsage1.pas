...
procedure THGUsage.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure THGUsage.FormDestroy(Sender: TObject);
begin
  HGUsage:=nil;
end;

procedure THGUsage.Button2Click(Sender: TObject);
begin
  ExportFormatExcel_XuatXu();
end;
procedure THGUsage.ExportFormatExcel_XuatXu();
var AppDir:String;
    i,j,datai:integer;
...
//
procedure THGUsage.ExportExcel(MasQry:TQuery;DetQry:TQuery);
...
begin
  ...
  try
    ...
    begin
      ...
      if (MasQry.Fields[4].Value <> MasQry.Fields[5].Value) then begin
        Range:= eclApp.Range['F'+inttostr(j)+':G'+inttostr(j)];
...
procedure THGUsage.Button5Click(Sender: TObject);
...
procedure THGUsage.RYQueryCalcFields(DataSet: TDataSet);
begin
  with RYQuery do
  begin
...
procedure THGUsage.DBGridEh4DrawColumnCell(Sender: TObject;
...
procedure THGUsage.DBGridEh4GetCellParams(Sender: TObject;
...
procedure THGUsage.MenuItem1Click(Sender: TObject);
begin
  ExportExcel(RYQuery,nil);
end;

procedure THGUsage.MenuItem3Click(Sender: TObject);
begin
  DetailInVNO_Det:=TDetailInVNO_Det.Create(self);
  DetailInVNO_Det.ZLBH:=HGUsage.RYQuery.FieldByName('ZLBH').AsString;
  DetailInVNO_Det.CLBH:=HGUsage.RYQuery.FieldByName('CLBH').AsString;
  DetailInVNO_Det.MJBH:=HGUsage.RYQuery.FieldByName('MJBH').AsString;
  DetailInVNO_Det.DType:=HGUsage.RYQuery.FieldByName('DType').AsString;
  DetailInVNO_Det.GSBH:=HGUsage.RYQuery.FieldByName('GSBH').AsString;
  if IsHide_Warehouse_XT='Y' then
  begin
...
procedure THGUsage.Button4Click(Sender: TObject);
begin
   ExportFormatExcel();
end;

procedure THGUsage.ExportFormatExcel();
var AppDir:String;
    i,j,datai:integer;
    eclApp,WorkBook:olevariant;
    DataRow,ExcelRow:integer;
...
//VGC加工檔
procedure THGUsage.Show_Xuong_VGC(Query:TQuery);
begin
  with Query do
  begin
    SQL.Add('Union all ');
...
procedure THGUsage.Show_Xuong_VGC_Sep(Query:TQuery;DDBH_M:String;SepCount:String;DDBH_C_SQLStr:String);
begin
  with Query do
  begin
    SQL.Add('Union all ');
...
procedure THGUsage.Show_NotIn_Xuong_VGC(Query:TQuery);
begin
  with Query do
  begin
    //不在現場VA12
...
procedure THGUsage.Show_NotIn_Xuong_VGC_Sep(Query:TQuery;DDBH_M:String;SepCount:String;DDBH_C_SQLStr:String);
begin
  if RB1.Checked=true then
  begin
...
function THGUsage.GetMutiRYSubSQL(Str:String):String;
...
//
procedure THGUsage.Button6Click(Sender: TObject);
var DDBH_M,DDBH_C_SQLStr,SepCount:String;
    i:integer;
begin
  //
...
procedure THGUsage.MAHG1Click(Sender: TObject);
...
procedure THGUsage.DBGridEh5GetCellParams(Sender: TObject;
  Column: TColumnEh; AFont: TFont; var Background: TColor;
  State: TGridDrawState);
begin
...
procedure THGUsage.ButtonMaVatTu_V2();
var ZLZLS2:string;
    i:integer;
begin
  if RB1.Checked=true then
...
procedure THGUsage.ButtonMaHG_V2(QueryObj:TQuery);
var ZLZLS2:string;
    i:integer;
begin
  if RB1.Checked=true then
  ...
  begin
    ...
    SQL.Add('select OrdCL.*,Round(KCLLS.TempQty,2) as TempQty ,case when KCLLS.CNO_NK='''' then null else CNO_NK end CNO_NK,case when KCLLS.CNO_TC='''' then null else CNO_TC end CNO_TC,case when KCLLS.CNO_HD='''' then null else CNO_HD end CNO_HD,');
    SQL.Add('       case when KCLLS.CNO_KD='''' then null else CNO_KD end CNO_KD,KCLLS.RKQty,IsNull(KCLLS.NK,0) as NK,IsNull(KCLLS.TC,0) as TC,IsNull(KCLLS.HD,0) as HD,IsNull(KCLLS.KD,0) as KD,IsNull(KCLLS.XT,0) as XT,IsNull(KCLLS.GC,0) as GC,');
    SQL.Add('       IsNull(Round(KCLLS.Qty-IsNull(KCLLS.NK,0)-IsNull(KCLLS.TC,0)-IsNull(KCLLS.HD,0)-IsNull(KCLLS.KD,0)-IsNull(KCLLS.XT,0)-IsNull(KCLLS.GC,0),2),0)  as ZZ,KCLLS.Qty,KCLLS.PKNO ');
...
//
//拆單版本平均分攤領料用量版本
procedure THGUsage.ButtonMaVatTu_Sep(DDBH_M:String;SepCount:String;DDBH_C_SQLStr:String);
var ZLZLS2:string;
    i:integer;
begin
...
procedure THGUsage.ButtonMaHG_Sep(DDBH_M:String;SepCount:String;DDBH_C_SQLStr:String;QueryObj:TQuery);
var ZLZLS2:string;
    i:integer;
    //
begin
...
procedure THGUsage.FormCreate(Sender: TObject);
var i:integer;
begin
  //底廠
  with Qtemp do
...
procedure THGUsage.RYEditPaste();
var
  CbBuf: PChar;
  BufSize: Integer;
  Handle: THandle;
...
procedure THGUsage.RYEditKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure THGUsage.Button1Click(Sender: TObject);
...
procedure THGUsage.PC1Changing(Sender: TObject;
  var AllowChange: Boolean);
begin
  Edit1.Text := RYQuery.fieldbyname('CLBH').AsString;
end;

procedure THGUsage.HGQueryCalcFields(DataSet: TDataSet);
begin
  with HGQuery do
  begin
...
procedure THGUsage.Excel1Click(Sender: TObject);
begin
  ExportExcel(HGQuery,nil);
end;

procedure THGUsage.B_ImportP3Click(Sender: TObject);
var
  i,j,n:integer;
  flag:boolean;
  MsExcel:olevariant;
...
procedure THGUsage.DBGridEh5DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumnEh;
  State: TGridDrawState);
begin
