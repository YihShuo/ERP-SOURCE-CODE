...
begin
   ...
   begin
    ...
    SQL.Add('       (KCLLS.Qty-IsNull(KCRKCX.NK,0)-IsNull(KCRKCX.TC,0)-IsNull(KCRKCX.HD,0)-IsNull(KCRKCX.KD,0)-IsNull(KCRKCX.XT,0)-IsNull(KCRKCX.GC,0)) as ZZ,KCLL.CFMID,KCLL.CFMDate,RateC');
    SQL.Add('from KCLLS');
    ...
    SQL.Add('                  Sum(IsNull(KCRKCX.NK,0)) as NK,Sum(IsNull(KCRKCX.TC,0)) as TC,Sum(IsNull(KCRKCX.HD,0)) as HD,Sum(IsNull(KCRKCX.KD,0)) as KD,Sum(IsNull(KCRKCX.XT,0)) as XT,Sum(IsNull(KCRKCX.GC,0)) as GC,Sum(IsNull(KCRKCX.ZZ,0)) as ZZ,RateC  from (');
    ...
    sql.Add('                   case when KCLLS.HGLB in (''NK'',''TC'')  then CLHG.RateC when KCLLS.HGLB=''HD'' then CLHD.RateC else NULL end as RateC');
    SQL.Add('           from KCLLS ');
    SQL.Add('           left join KCLL on KCLL.LLNO=KCLLS.LLNO');
    ...
    SQL.Add('           group by KCLLS.LLNO,KCLLS.SCBH,KCLLS.DFL, KCLLS.CLBH,KCLLS.HGLB,CLHG.RateC,CLHD.RateC');
    SQL.Add('           ) KCRKCX ');
