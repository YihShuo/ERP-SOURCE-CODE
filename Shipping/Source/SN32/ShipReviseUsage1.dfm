object ShipReviseUsage: TShipReviseUsage
  Left = 209
  Top = 172
  Width = 1361
  Height = 684
  Caption = 'ShipReviseUsage'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1345
    Height = 65
    Align = alTop
    TabOrder = 1
    object Label18: TLabel
      Left = 704
      Top = 32
      Width = 18
      Height = 24
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -19
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
    object BB2: TBitBtn
      Left = 56
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Insert one New Record'
      Caption = 'Insert'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      OnClick = BB2Click
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB3: TBitBtn
      Left = 104
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Delete one Record'
      Caption = 'Delete'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      OnClick = BB3Click
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB4: TBitBtn
      Left = 152
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Modify'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 3
      OnClick = BB4Click
      Glyph.Data = {
        
        50BB555555555555575F555555555555550B5555555555555575}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB5: TBitBtn
      Left = 200
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Save Current'
      Caption = 'Save'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 4
      OnClick = BB5Click
      Glyph.Data = {
        
        5990555555555555577755555555555555555555555555555555}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB6: TBitBtn
      Left = 248
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Cancel'
      Caption = 'Cancel'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 5
      OnClick = BB6Click
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB7: TBitBtn
      Left = 296
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Exit Current Form'
      Caption = 'Exit'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 6
      OnClick = BB7Click
      Glyph.Data = {
        
        EE33337FFFFFFFFFFF7F33EEEEEEEEEEEE333377777777777773}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB1: TBitBtn
      Left = 8
      Top = 8
      Width = 49
      Height = 49
      Caption = 'Query'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      Glyph.Data = {
        
        3000333333333377377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BBT1: TBitBtn
      Left = 368
      Top = 8
      Width = 49
      Height = 49
      Caption = 'First'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 7
      Glyph.Data = {
        
        CCC333333333333777FF33333333333333CC3333333333333773}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BBT2: TBitBtn
      Left = 416
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Insert one New Record'
      Caption = 'Prior'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 8
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BBT3: TBitBtn
      Left = 464
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Delete one Record'
      Caption = 'Next'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 9
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BBT4: TBitBtn
      Left = 512
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Last'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 10
      Glyph.Data = {
        
        33333777333333333333CC333333333333337733333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object bbt6: TBitBtn
      Left = 608
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Excel'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 11
      OnClick = bbt6Click
      Glyph.Data = {
        
        3BB33773333773333773B333333B3333333B7333333733333337}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 65
    Width = 1345
    Height = 65
    Align = alTop
    TabOrder = 0
    Visible = False
    object Label9: TLabel
      Left = 704
      Top = 32
      Width = 18
      Height = 24
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -19
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
    object BC2: TBitBtn
      Left = 56
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Insert one New Record'
      Caption = 'Insert'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BC3: TBitBtn
      Left = 104
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Delete one Record'
      Caption = 'Delete'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      OnClick = BC3Click
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BC4: TBitBtn
      Left = 152
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Modify'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      OnClick = BC4Click
      Glyph.Data = {
        
        50BB555555555555575F555555555555550B5555555555555575}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BC5: TBitBtn
      Left = 200
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Save Current'
      Caption = 'Save'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 3
      OnClick = BC5Click
      Glyph.Data = {
        
        5990555555555555577755555555555555555555555555555555}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BC6: TBitBtn
      Left = 248
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Cancel'
      Caption = 'Cancel'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 4
      OnClick = BC6Click
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BC7: TBitBtn
      Left = 296
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Exit Current Form'
      Caption = 'Exit'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 5
      OnClick = BB7Click
      Glyph.Data = {
        
        EE33337FFFFFFFFFFF7F33EEEEEEEEEEEE333377777777777773}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BCT1: TBitBtn
      Left = 368
      Top = 8
      Width = 49
      Height = 49
      Caption = 'First'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 6
      Glyph.Data = {
        
        CCC333333333333777FF33333333333333CC3333333333333773}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BCT2: TBitBtn
      Left = 416
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Insert one New Record'
      Caption = 'Prior'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 7
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BCT3: TBitBtn
      Left = 464
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Delete one Record'
      Caption = 'Next'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 8
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BCT4: TBitBtn
      Left = 512
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Last'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 9
      Glyph.Data = {
        
        33333777333333333333CC333333333333337733333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BCT6: TBitBtn
      Left = 608
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Excel'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 10
      OnClick = BCT6Click
      Glyph.Data = {
        
        3BB33773333773333773B333333B3333333B7333333733333337}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BCT7: TBitBtn
      Left = 656
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Spec'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 11
      OnClick = BCT7Click
      Glyph.Data = {
        
        33333337FFFF7733333333300000033333333337777773333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 130
    Width = 1345
    Height = 515
    Align = alClient
    Caption = 'Panel3'
    TabOrder = 2
    object ToolPanel: TPanel
      Left = 1
      Top = 1
      Width = 1343
      Height = 71
      Align = alTop
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      object Label2: TLabel
        Left = 4
        Top = 14
        Width = 70
        Height = 16
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'MatNo:'
      end
      object Label3: TLabel
        Left = 195
        Top = 14
        Width = 70
        Height = 16
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'MatName:'
      end
      object Label5: TLabel
        Left = 4
        Top = 44
        Width = 70
        Height = 16
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'OrderNo:'
      end
      object Label6: TLabel
        Left = 195
        Top = 44
        Width = 70
        Height = 16
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'Article:'
      end
      object Label1: TLabel
        Left = 396
        Top = 45
        Width = 70
        Height = 16
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'Part:'
      end
      object Label7: TLabel
        Left = 503
        Top = 14
        Width = 70
        Height = 16
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'MJBH:'
      end
      object Label4: TLabel
        Left = 676
        Top = 14
        Width = 70
        Height = 16
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'Material:'
      end
      object Label8: TLabel
        Left = 869
        Top = 14
        Width = 70
        Height = 16
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'Inv No:'
      end
      object Button1: TButton
        Left = 833
        Top = 35
        Width = 81
        Height = 33
        Caption = 'QUERY'
        TabOrder = 5
        OnClick = Button1Click
      end
      object Edit1: TEdit
        Left = 83
        Top = 10
        Width = 105
        Height = 24
        CharCase = ecUpperCase
        TabOrder = 0
      end
      object Edit2: TEdit
        Left = 275
        Top = 10
        Width = 110
        Height = 24
        CharCase = ecUpperCase
        TabOrder = 1
      end
      object Edit3: TEdit
        Left = 387
        Top = 10
        Width = 120
        Height = 24
        CharCase = ecUpperCase
        TabOrder = 2
      end
      object Edit4: TEdit
        Left = 83
        Top = 42
        Width = 105
        Height = 24
        CharCase = ecUpperCase
        Color = clYellow
        TabOrder = 4
      end
      object Edit5: TEdit
        Left = 275
        Top = 42
        Width = 110
        Height = 24
        CharCase = ecUpperCase
        TabOrder = 3
      end
      object Edit6: TEdit
        Left = 387
        Top = 42
        Width = 121
        Height = 24
        CharCase = ecUpperCase
        TabOrder = 6
      end
      object CheckBox2: TCheckBox
        Left = 637
        Top = 45
        Width = 140
        Height = 17
        Caption = 'Qty<>CLSL'
        TabOrder = 7
        OnClick = CheckBox2Click
      end
      object VTCombo: TComboBox
        Left = 757
        Top = 10
        Width = 108
        Height = 24
        Style = csDropDownList
        ItemHeight = 16
        ItemIndex = 0
        TabOrder = 8
        Text = 'ALL'
        Items.Strings = (
          'ALL'
          'Xuat GC'
          'Xuat Hien Truong')
      end
      object MotherEdit: TEdit
        Left = 579
        Top = 10
        Width = 94
        Height = 24
        CharCase = ecUpperCase
        TabOrder = 9
      end
      object CheckBox3: TCheckBox
        Left = 530
        Top = 44
        Width = 95
        Height = 17
        Caption = '<>W%'
        Checked = True
        State = cbChecked
        TabOrder = 10
        OnClick = CheckBox2Click
      end
      object Edit7: TEdit
        Left = 947
        Top = 10
        Width = 110
        Height = 24
        CharCase = ecUpperCase
        TabOrder = 11
      end
    end
    object PC1: TPageControl
      Left = 1
      Top = 72
      Width = 1343
      Height = 442
      ActivePage = TabSheet1
      Align = alClient
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      OnChange = PC1Change
      object TabSheet1: TTabSheet
        Caption = 'Dinh muc'
        object Splitter1: TSplitter
          Left = 1134
          Top = 0
          Width = 5
          Height = 411
          Align = alRight
        end
        object DBGridEh1: TDBGridEh
          Left = 0
          Top = 0
          Width = 1134
          Height = 411
          Align = alClient
          DataSource = DS1
          Flat = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          FooterColor = clWindow
          FooterFont.Charset = DEFAULT_CHARSET
          FooterFont.Color = clWindowText
          FooterFont.Height = -11
          FooterFont.Name = 'MS Sans Serif'
          FooterFont.Style = []
          FooterRowCount = 1
          ParentFont = False
          PopupMenu = PopupMenu1
          SumList.Active = True
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -13
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          UseMultiTitle = True
          OnDrawColumnCell = DBGridEh1DrawColumnCell
          OnEditButtonClick = DBGridEh1EditButtonClick
          Columns = <
            item
              EditButtons = <>
              FieldName = 'ZLBH'
              Footer.ValueType = fvtCount
              Footers = <>
              ReadOnly = True
              Title.Caption = #35330#21934'|Order No'
              Width = 81
            end
            item
              EditButtons = <>
              FieldName = 'BWBH'
              Footers = <>
              ReadOnly = True
              Title.Caption = #37096#20301'|Part'
              Width = 39
            end
            item
              EditButtons = <>
              FieldName = 'BWMC'
              Footers = <>
              ReadOnly = True
              Title.Caption = #37096#20301#21517#31281'|Part Name'
              Width = 121
            end
            item
              EditButtons = <>
              FieldName = 'CLBH'
              Footers = <>
              Title.Caption = #26009#34399'|Mat No'
              Width = 84
            end
            item
              EditButtons = <>
              FieldName = 'CLMC'
              Footers = <>
              ReadOnly = True
              Title.Caption = #21697#21517'|Material Name'
              Width = 300
            end
            item
              EditButtons = <>
              FieldName = 'SIZE'
              Footers = <>
              Title.Caption = #23610#30908'|SIZE'
            end
            item
              EditButtons = <>
              FieldName = 'DWBH'
              Footers = <>
              ReadOnly = True
              Title.Caption = #21934#20301'|Unit'
              Width = 37
            end
            item
              Color = clYellow
              EditButtons = <>
              FieldName = 'CLSL'
              Footers = <
                item
                  FieldName = 'CLSL'
                  ValueType = fvtSum
                end>
              Width = 70
            end
            item
              EditButtons = <>
              FieldName = 'USAGE'
              Footers = <>
              ReadOnly = True
              Width = 60
            end
            item
              EditButtons = <>
              FieldName = 'ZSMC'
              Footers = <>
              ReadOnly = True
              Title.Caption = 'Supplier'
              Width = 100
            end
            item
              EditButtons = <>
              FieldName = 'ARTICLE'
              Footers = <>
              ReadOnly = True
              Title.Caption = 'Article'
              Width = 63
            end
            item
              EditButtons = <>
              FieldName = 'MJBH'
              Footers = <>
              Title.Caption = #27597#26009#34399'|MJBH'
            end
            item
              EditButtons = <>
              FieldName = 'YWPM'
              Footers = <>
              Title.Caption = #27597#26448#26009#21697#21517'|YWPM'
              Width = 268
            end
            item
              EditButtons = <>
              FieldName = 'USERID'
              Footers = <>
            end>
        end
        object DBGridEh2: TDBGridEh
          Left = 1139
          Top = 0
          Width = 196
          Height = 411
          Align = alRight
          DataSource = DS2
          Flat = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          FooterColor = clWindow
          FooterFont.Charset = DEFAULT_CHARSET
          FooterFont.Color = clWindowText
          FooterFont.Height = -11
          FooterFont.Name = 'MS Sans Serif'
          FooterFont.Style = []
          FooterRowCount = 1
          ParentFont = False
          SumList.Active = True
          TabOrder = 1
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -13
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          UseMultiTitle = True
          Columns = <
            item
              EditButtons = <>
              FieldName = 'Qty'
              Footers = <>
              Title.Caption = #38936#26009'|KCLLS Qty'
            end>
        end
      end
      object TabSheet2: TTabSheet
        Caption = 'Sepc'
        ImageIndex = 1
        object DBGridEh3: TDBGridEh
          Left = 0
          Top = 0
          Width = 1076
          Height = 282
          Align = alClient
          DataSource = DS3
          Flat = False
          FooterColor = clWindow
          FooterFont.Charset = DEFAULT_CHARSET
          FooterFont.Color = clWindowText
          FooterFont.Height = -13
          FooterFont.Name = 'MS Sans Serif'
          FooterFont.Style = []
          FooterRowCount = 1
          SumList.Active = True
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -13
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          UseMultiTitle = True
          Columns = <
            item
              Color = clBtnFace
              EditButtons = <>
              FieldName = 'ZLBH'
              Footer.FieldName = 'ZLBH'
              Footer.ValueType = fvtCount
              Footers = <>
              ReadOnly = True
              Title.Caption = #35330#21934'|ZLBH'
              Width = 73
            end
            item
              EditButtons = <>
              FieldName = 'XH'
              Footers = <>
            end
            item
              EditButtons = <>
              FieldName = 'bwbh'
              Footers = <>
              Title.Caption = #37096#20301'|BWBH'
              Width = 48
            end
            item
              EditButtons = <>
              FieldName = 'clbh'
              Footers = <>
              Title.Caption = #26009#34399'|CLBH'
              Width = 110
            end
            item
              EditButtons = <>
              FieldName = 'mjbh'
              Footers = <>
              Title.Caption = #27597#26009'|MJBH'
              Width = 108
            end
            item
              EditButtons = <>
              FieldName = 'clsl'
              Footers = <>
              Title.Caption = #29992#37327'|CLSL'
              Width = 46
            end
            item
              DisplayFormat = '0.0000'
              EditButtons = <>
              FieldName = 'USAGE'
              Footers = <>
            end
            item
              EditButtons = <>
              FieldName = 'BWMCY'
              Footers = <>
              Title.Caption = #37096#20301#21517#31281'|BWMCY'
              Width = 110
            end
            item
              EditButtons = <>
              FieldName = 'CLMCY'
              Footers = <>
              Title.Caption = #26009#34399'|CLMCY'
              Width = 411
            end
            item
              EditButtons = <>
              FieldName = 'dwbh'
              Footers = <>
              Title.Caption = #21934#20301'|DWBH'
              Width = 53
            end
            item
              EditButtons = <>
              FieldName = 'LOSS'
              Footers = <>
              Width = 49
            end
            item
              EditButtons = <>
              FieldName = 'CQDH'
              Footers = <>
              Width = 48
            end
            item
              EditButtons = <>
              FieldName = 'JGYWSM'
              Footers = <>
              Width = 77
            end
            item
              EditButtons = <>
              FieldName = 'XieXing'
              Footers = <>
              Title.Caption = 'XIEXING'
              Width = 88
            end
            item
              EditButtons = <>
              FieldName = 'SheHao'
              Footers = <>
              Title.Caption = 'SHEHAO'
              Width = 72
            end>
        end
      end
    end
  end
  object Query1: TQuery
    AfterOpen = Query1AfterOpen
    DatabaseName = 'DB'
    SQL.Strings = (
      
        'select ZLZLS2.*,BWZL.ywsm BWMC,CLZL.YWPM as CLMC,CLZL.DWBH,ZSZL.' +
        'ZSYWJC AS ZSMC,DDZL.ARTICLE,DDZL.YN,CLZL1.YWPM,ZLZLS2All.CLSLQty' +
        ' ,IsNull(Round(KCLLS.Qty,2),0) as Qty,DDZL.YN as LockYN'
      'FROM DDZL_ZLZLS2 ZLZLS2'
      '            LEFT JOIN BWZL ON BWZL.BWDH=ZLZLS2.BWBH'
      '            LEFT JOIN CLZL ON ZLZLS2.CLBH=CLZL.CLDH'
      '            LEFT JOIN ZSZL ON ZSZL.ZSDH=ZLZLS2.CSBH'
      '            LEFT JOIN DDZL ON DDZL.DDBH=ZLZLS2.ZLBH'
      '            LEFT JOIN CLZL CLZL1 ON ZLZLS2.MJBH=CLZL1.CLDH '
      
        '            LEFT JOIN SHIP_XXZLS on SHIP_XXZLS.XieXing=DDZL.XieX' +
        'ing and SHIP_XXZLS.SheHao=DDZL.SheHao and SHIP_XXZLS.BWBH=ZLZLS2' +
        '.BWBH '
      'left join (Select ZLBH,CLBH,MJBH,IsNull(Sum(CLSL),0) as CLSLQty'
      '           FROM DDZL_ZLZLS2 ZLZLS2'
      
        '           where ZLZLS2.ZLBH in ('#39'Y1801-0967'#39') and ZLZLS2.CLBH l' +
        'ike '#39'%'#39' '
      
        '           Group by ZLBH,CLBH,MJBH) ZLZLS2All on ZLZLS2.ZLBH=ZLZ' +
        'LS2All.ZLBH and ZLZLS2.CLBH=ZLZLS2All.CLBH and ZLZLS2.MJBH=ZLZLS' +
        '2All.MJBH '
      
        'left join (select SCBH,CLBH,MJBH,IsNull(Sum(Qty),0) as Qty from ' +
        '('
      
        '            select   KCLLS.SCBH, KCLLS.CLBH,'#39'ZZZZZZZZZZ'#39' as MJBH' +
        ', sum(KCLLS.Qty) as Qty'
      '            from KCLLS'
      '            Left Join KCLL on KCLL.LLNO=KCLLS.LLNO '
      
        '            where  KCLLS.SCBH like '#39'Y1801-0967%'#39'  and KCLLS.CLBH' +
        ' like '#39'%'#39'  '
      '                 and KCLL.GSBH='#39'VA12'#39' '
      '                 and KCLL.SCBH<>'#39'JJJJJJJJJJ'#39' '
      '            Group by KCLLS.SCBH, KCLLS.CLBH'
      '            Union all'
      
        '            select JGZLSS.ZLBH as SCBH,JGZLS.ZMLB as CLBH,JGZLS.' +
        'CLBH as MJBH,Sum(JGZLS.Qty*JGZLSS.Qty) as Qty from JGZLS'
      
        '            Left Join JGZLSS on JGZLS.JGNO=JGZLSS.JGNO and JGZLS' +
        '.CLBH=JGZLSS.CLBH '
      '            Left Join JGZL  on JGZL.JGNO=JGZLS.JGNO '
      
        '            where JGZLSS.ZLBH like '#39'Y1801-0967%'#39' and JGZLS.ZMLB ' +
        'like '#39'%'#39' and JGZLS.CLBH like '#39'%'#39'  '
      '                  and JGZL.GSBH='#39'VA12'#39' '
      
        '                  and convert(smalldatetime,convert(varchar,JGZL' +
        '.CFMDate1,111))<'#39'2017/09/01'#39'  '
      '            Group by JGZLSS.ZLBH,JGZLS.ZMLB,JGZLS.CLBH'
      '            Union all'
      
        '            select   KCLLS.SCBH, KCLLS.CLBH,KCLLS.DFL as MJBH, s' +
        'um(KCLLS.Qty) as Qty'
      '            from KCLLS'
      '            Left Join KCLL on KCLL.LLNO=KCLLS.LLNO '
      
        '            where  KCLLS.SCBH like '#39'Y1801-0967%'#39'  and KCLLS.CLBH' +
        ' like '#39'%'#39'  '
      '                 and KCLL.GSBH='#39'VA12'#39' '
      '                 and KCLL.SCBH='#39'JJJJJJJJJJ'#39' '
      '            Group by KCLLS.SCBH, KCLLS.CLBH,KCLLS.DFL'
      
        '           ) KCLLS group by SCBH,CLBH,MJBH) KCLLS on KCLLS.CLBH=' +
        'ZLZLS2.CLBH and KCLLS.SCBH=ZLZLS2.ZLBH  and KCLLS.MJBH=ZLZLS2.MJ' +
        'BH '
      'WHERE ZLZLS2.CLBH like '#39'%'#39' '
      '            and CLZL.YWPM like '#39'%%'#39' '
      '            and CLZL.YWPM like '#39'%%'#39' '
      '            and ZLZLS2.ZLBH in ('#39'Y1801-0967'#39') '
      '            and DDZL.ARTICLE like '#39'%'#39' '
      '            and ZLZLS2.BWBH like '#39'%'#39' '
      '          and ZLZLS2.CLBH not like '#39'W%'#39' '
      
        '            and ((Ship_XXZLS.TYJH<>'#39'Y'#39')or (Ship_XXZLS.TYJH is nu' +
        'll))'
      
        'order by ZLZLS2.ZLBH,ZLZLS2.CLBH asc,ZLZLS2.MJBH,ZLZLS2.BWBH des' +
        'c,ZLZLS2.CLSL desc'
      '')
    UpdateObject = UpdateSQL1
    Left = 136
    Top = 216
    object Query1ZLBH: TStringField
      FieldName = 'ZLBH'
      FixedChar = True
      Size = 15
    end
    object Query1xh: TStringField
      FieldName = 'xh'
      FixedChar = True
      Size = 3
    end
    object Query1BWBH: TStringField
      FieldName = 'BWBH'
      FixedChar = True
      Size = 4
    end
    object Query1CSBH: TStringField
      FieldName = 'CSBH'
      FixedChar = True
      Size = 6
    end
    object Query1MJBH: TStringField
      FieldName = 'MJBH'
      FixedChar = True
      Size = 10
    end
    object Query1CLBH: TStringField
      FieldName = 'CLBH'
      FixedChar = True
      Size = 10
    end
    object Query1ZMLB: TStringField
      FieldName = 'ZMLB'
      FixedChar = True
      Size = 1
    end
    object Query1SIZE: TStringField
      FieldName = 'SIZE'
      FixedChar = True
      Size = 6
    end
    object Query1CLSL: TFloatField
      FieldName = 'CLSL'
      DisplayFormat = '0.00'
    end
    object Query1USAGE: TFloatField
      FieldName = 'USAGE'
    end
    object Query1USERID: TStringField
      FieldName = 'USERID'
      FixedChar = True
    end
    object Query1USERDATE: TDateTimeField
      FieldName = 'USERDATE'
    end
    object Query1YN: TStringField
      FieldName = 'YN'
      FixedChar = True
      Size = 1
    end
    object Query1BWMC: TStringField
      FieldName = 'BWMC'
      FixedChar = True
      Size = 40
    end
    object Query1CLMC: TStringField
      FieldName = 'CLMC'
      FixedChar = True
      Size = 200
    end
    object Query1DWBH: TStringField
      FieldName = 'DWBH'
      FixedChar = True
      Size = 4
    end
    object Query1ZSMC: TStringField
      FieldName = 'ZSMC'
      FixedChar = True
    end
    object Query1ARTICLE: TStringField
      FieldName = 'ARTICLE'
      FixedChar = True
    end
    object Query1YWPM: TStringField
      FieldName = 'YWPM'
      FixedChar = True
      Size = 200
    end
    object Query1CLSLQty: TFloatField
      FieldName = 'CLSLQty'
    end
    object Query1Qty: TCurrencyField
      FieldName = 'Qty'
    end
    object Query1LockYN: TStringField
      FieldName = 'LockYN'
      FixedChar = True
      Size = 1
    end
  end
  object UpdateSQL1: TUpdateSQL
    ModifySQL.Strings = (
      'update DDZL_ZLZLS2'
      'set'
      '        CLSL=:CLSL,'
      '        MJBH=:MJBH,'
      '        USERID=:USERID,'
      '        USERDATE=:USERDATE'
      'WHERE'
      '   ZLBH=:OLD_ZLBH AND'
      '   BWBH=:OLD_BWBH AND'
      '   CLBH=:OLD_CLBH AND'
      '   SIZE=:OLD_SIZE AND'
      '   YN=:OLD_YN'
      ''
      '')
    InsertSQL.Strings = (
      'Insert into DDZL_ZLZLS2'
      
        '   (ZLBH, XH ,BWBH, CSBH, MJBH, CLBH, ZMLB, SIZE, CLSL, USAGE, U' +
        'SERID, USERDATE, YN)'
      'Values'
      
        '   (:ZLBH, :XH ,:BWBH, :CSBH, :MJBH, :CLBH, :ZMLB, :SIZE, :CLSL,' +
        ' :USAGE, :USERID, :USERDATE, :YN)')
    DeleteSQL.Strings = (
      'Delete From DDZL_ZLZLS2'
      'WHERE'
      '   ZLBH=:OLD_ZLBH AND'
      '   BWBH=:OLD_BWBH AND'
      '   CLBH=:OLD_CLBH AND'
      '   SIZE=:OLD_SIZE AND'
      '   YN=:OLD_YN')
    Left = 136
    Top = 288
  end
  object DS1: TDataSource
    DataSet = Query1
    Left = 136
    Top = 248
  end
  object UpdateQry: TQuery
    DatabaseName = 'DB'
    SQL.Strings = (
      'select ZLZLS2.*, BWZL.ywsm BWMC,CLZL.YWPM as CLMC,'
      '       CLZL.DWBH,ZSZL.ZSYWJC AS ZSMC,DDZL.ARTICLE'
      'FROM ZLZLS2 LEFT JOIN BWZL ON BWZL.BWDH=ZLZLS2.BWBH'
      'LEFT JOIN CLZL ON ZLZLS2.CLBH=CLZL.CLDH'
      'LEFT JOIN ZSZL ON ZSZL.ZSDH=ZLZLS2.CSBH'
      'LEFT JOIN DDZL ON DDZL.DDBH=ZLZLS2.ZLBH'
      'WHERE ZLZLS2.ZLBH='#39'SD1511-108'#39'AND ZLZLS2.CLBH='#39'L090000993'#39
      'order by ZLZLS2.ZLBH, ZLZLS2.BWBH')
    Left = 176
    Top = 216
  end
  object KCLLSQry: TQuery
    DatabaseName = 'DB'
    DataSource = DS1
    SQL.Strings = (
      'select SCBH,CLBH,Sum(Qty) as Qty from ('
      'select   KCLLS.SCBH, KCLLS.CLBH, sum(KCLLS.Qty) as Qty'
      'from KCLLS'
      'where  CLBH=:CLBH and KCLLS.SCBH=:ZLBH'
      'group by KCLLS.SCBH, KCLLS.CLBH'
      'Union all'
      
        '        select JGZLSS.ZLBH as SCBH,JGZLS.ZMLB as CLBH,Sum(JGZLS.' +
        'Qty*JGZLSS.Qty) as Qty from JGZLS'
      
        'Left Join JGZLSS on JGZLS.JGNO=JGZLSS.JGNO and JGZLS.CLBH=JGZLSS' +
        '.CLBH '
      'where JGZLSS.ZLBH=:ZLBH and JGZLS.ZMLB=:CLBH'
      'Group by JGZLSS.ZLBH,JGZLS.ZMLB'
      ') KCLLS group by SCBH,CLBH')
    Left = 232
    Top = 216
    ParamData = <
      item
        DataType = ftFixedChar
        Name = 'CLBH'
        ParamType = ptUnknown
        Size = 11
      end
      item
        DataType = ftFixedChar
        Name = 'ZLBH'
        ParamType = ptUnknown
        Size = 16
      end
      item
        DataType = ftFixedChar
        Name = 'ZLBH'
        ParamType = ptUnknown
      end
      item
        DataType = ftFixedChar
        Name = 'CLBH'
        ParamType = ptUnknown
      end>
    object KCLLSQrySCBH: TStringField
      FieldName = 'SCBH'
      FixedChar = True
      Size = 15
    end
    object KCLLSQryCLBH: TStringField
      FieldName = 'CLBH'
      FixedChar = True
      Size = 15
    end
    object KCLLSQryQty: TCurrencyField
      FieldName = 'Qty'
      DisplayFormat = '#0.00'
    end
  end
  object DS2: TDataSource
    DataSet = KCLLSQry
    Left = 232
    Top = 248
  end
  object PopupMenu1: TPopupMenu
    Left = 136
    Top = 320
    object QtyCLSL1: TMenuItem
      Caption = 'Set Qty=CLSL'
      OnClick = QtyCLSL1Click
    end
  end
  object UpdateSQL2: TUpdateSQL
    Left = 352
    Top = 281
  end
  object Query2: TQuery
    DatabaseName = 'DB'
    SQL.Strings = (
      'if object_id('#39'tempdb..#Material'#39') is not null '
      'begin   '
      'drop table #Material'
      'end'
      ''
      
        'select zlzls2.ZLBH,zlzls2.xh,zlzls2.BWBH,zlzls2.clbh,zlzls2.mjbh' +
        ',zlzls2.clsl,zlzls2.USAGE,zlzls2.BWMCY,'
      
        #9#9'zlzls2.CLMCY,zlzls2.dwbh,zlzls2.LOSS,zlzls2.CQDH,zlzls2.JGYWSM' +
        ',zlzls2.XieXing,zlzls2.SheHao,'
      #9#9'zlzls2.CCQQ,zlzls2.CCQZ'
      'into #Material'
      'from ('
      
        'SELECT ZLZLS2.ZLBH , ZLZLS2.XH,ZLZLS2.bwbh ,zlzls2.clbh AS clbh ' +
        ',max(zlzls2.mjbh) AS mjbh ,'
      
        '              sum(zlzls2.clsl) AS clsl,(sum(zlzls2.clsl)/ddzl.pa' +
        'irs) AS USAGE,max(BWZL.ywsm) AS BWMCY ,'
      
        '              max(CLZL.ywpm) AS CLMCY ,max(CLZL.dwbh) AS dwbh,Is' +
        'Null(XXZLS.LOSS,0) as LOSS  ,CLZL.CQDH,XXZLS.JGYWSM ,DDZL.XieXin' +
        'g ,DDZL.SheHao'
      '              ,XXZLS.CCQQ,XXZLS.CCQZ'
      'FROM DDZL_ZLZLS2 AS ZLZLS2'
      'LEFT JOIN BWZL AS BWZL ON ZLZLS2.BWBH = BWZL.bwdh'
      'LEFT JOIN CLZL AS CLZL ON ZLZLS2.CLBH = CLZL.cldh'
      'LEFT JOIN DDZL AS DDZL ON ZLZLS2.ZLBH = DDZL.ZLBH'
      'LEFT JOIN XXZLS AS XXZLS ON DDZL.XIEXING = XXZLS.XIEXING'
      
        '         AND DDZL.SHEHAO = XXZLS.SHEHAO AND ZLZLS2.BWBH = XXZLS.' +
        'BWBH --AND ZLZLS2.CLBH = XXZLS.CLBH'
      'where ZLZLS2.ZLBH=:ZLBH and ZLZLS2.MJBH='#39'ZZZZZZZZZZ'#39
      
        'GROUP BY ZLZLS2.XH,zlzls2.zlbh,zlzls2.bwbh,zlzls2.clbh,XXZLS.CLS' +
        'L,CLZL.CQDH,XXZLS.LOSS,XXZLS.JGYWSM ,ddzl.pairs ,DDZL.XieXing ,D' +
        'DZL.SheHao,XXZLS.CCQQ,XXZLS.CCQZ'
      'union all'
      
        'Select ZLZLS2.ZLBH,zlzls2.xh,ZLZLS2.bwbh+'#39'-'#39' as bwbh,zlzls2.clbh' +
        ',max(zlzls2.mjbh) AS mjbh ,'
      
        '              sum(zlzls2.clsl) AS clsl,(sum(zlzls2.clsl)/ddzl.pa' +
        'irs) AS USAGE,'#39'-----'#39' AS BWMCY ,'
      
        '              max(CLZL.ywpm) AS CLMCY ,max(CLZL.dwbh) AS dwbh,XX' +
        'ZLS.LOSS  ,CLZL.CQDH,XXZLS.JGYWSM,DDZL.XieXing ,DDZL.SheHao'
      '              ,XXZLS.CCQQ,XXZLS.CCQZ'
      'from DDZL_zlzls2 AS ZLZLS2 '
      'LEFT JOIN BWZL AS BWZL ON ZLZLS2.BWBH = BWZL.bwdh '
      'LEFT JOIN CLZL AS CLZL ON ZLZLS2.CLBH = CLZL.cldh '
      'LEFT JOIN DDZL AS DDZL ON ZLZLS2.ZLBH = DDZL.ZLBH '
      'LEFT JOIN XXZLS AS XXZLS ON DDZL.XIEXING = XXZLS.XIEXING '
      
        '         AND DDZL.SHEHAO = XXZLS.SHEHAO AND ZLZLS2.BWBH = XXZLS.' +
        'BWBH --AND ZLZLS2.CLBH = XXZLS.CLBH'
      'where ZLZLS2.ZLBH=:ZLBH and ZLZLS2.MJBH<>'#39'ZZZZZZZZZZ'#39
      
        'GROUP BY ZLZLS2.ZLBH,ZLZLS2.XH,zlzls2.zlbh,zlzls2.bwbh,zlzls2.CL' +
        'BH,XXZLS.CLSL,CLZL.CQDH,XXZLS.LOSS,XXZLS.JGYWSM ,XXZLS.CCQQ,XXZL' +
        'S.CCQZ,ddzl.pairs,DDZL.XieXing ,DDZL.SheHao'
      ') zlzls2'
      
        'GROUP BY ZLZLS2.ZLBH,zlzls2.XH,zlzls2.bwbh , zlzls2.clbh, zlzls2' +
        '.mjbh,zlzls2.clsl,'
      
        #9#9'zlzls2.usage, zlzls2.BWMCY, zlzls2.CLMCY, zlzls2.dwbh, zlzls2.' +
        'LOSS,zlzls2.CQDH, zlzls2.JGYWSM,zlzls2.XieXing,zlzls2.SheHao,zlz' +
        'ls2.CCQQ, zlzls2.CCQZ'
      'ORDER BY  zlzls2.bwbh asc'
      ''
      ''
      'update #Material'
      'set CLMCY=CLMCY+JGYWSM'
      'where JGYWSM is not Null'
      ''
      ' select * from #Material'
      'order by BWBH')
    UpdateObject = UpdateSQL2
    Left = 352
    Top = 217
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'ZLBH'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'ZLBH'
        ParamType = ptUnknown
      end>
    object Query1ddbh: TStringField
      FieldName = 'ZLBH'
      FixedChar = True
      Size = 15
    end
    object StringField12: TStringField
      FieldName = 'clbh'
      FixedChar = True
      Size = 10
    end
    object StringField13: TStringField
      FieldName = 'bwbh'
      FixedChar = True
      Size = 4
    end
    object StringField14: TStringField
      FieldName = 'dwbh'
      FixedChar = True
      Size = 4
    end
    object Query1BWMCY: TStringField
      FieldName = 'BWMCY'
      FixedChar = True
      Size = 40
    end
    object Query1CLMCY: TStringField
      FieldName = 'CLMCY'
      FixedChar = True
      Size = 200
    end
    object Query1LOSS: TFloatField
      FieldName = 'LOSS'
    end
    object FloatField3: TFloatField
      FieldName = 'USAGE'
    end
    object FloatField4: TFloatField
      FieldName = 'clsl'
    end
    object StringField15: TStringField
      FieldName = 'XH'
      FixedChar = True
      Size = 3
    end
    object Query1CQDH: TStringField
      FieldName = 'CQDH'
      FixedChar = True
      Size = 4
    end
    object Query1JGYWSM: TStringField
      FieldName = 'JGYWSM'
      FixedChar = True
      Size = 100
    end
    object StringField16: TStringField
      FieldName = 'mjbh'
      FixedChar = True
      Size = 10
    end
    object Query1XieXing: TStringField
      FieldName = 'XieXing'
      FixedChar = True
      Size = 15
    end
    object Query1SheHao: TStringField
      FieldName = 'SheHao'
      FixedChar = True
      Size = 5
    end
    object Query2CCQQ: TStringField
      FieldName = 'CCQQ'
      FixedChar = True
      Size = 6
    end
    object Query2CCQZ: TStringField
      FieldName = 'CCQZ'
      FixedChar = True
      Size = 6
    end
  end
  object DS3: TDataSource
    DataSet = Query2
    Left = 352
    Top = 249
  end
end
