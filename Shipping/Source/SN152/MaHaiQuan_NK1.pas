...
var
  MaHaiQuan_NK: TMaHaiQuan_NK;

implementation

uses main1, TimMaHQ1,FunUnit, TimMHQ_NK1;

{$R *.dfm}

procedure TMaHaiQuan_NK.GetSubSQL(var SQL:string;BtnObj:TSpeedButton;EditObj:TEdit;FieldNM:string;Flag:string);
...
procedure TMaHaiQuan_NK.Button1Click(Sender: TObject);
var i:integer;
    tmpList:TStringlist;
    MatNoSQL:string;
begin
  ...
  begin
    ...
    begin
      sql.add('select top 1000 CLZL.CLDH,CLHG.clbh,clzl.ywpm,clzl.dwbh,clhg.HGBH,clhg.HGPM,clhg.UNitC,clhg.Ratec,clhg.userid,clhg.userdate,clhg.YN');
    end else
    begin
      sql.add('select CLZL.CLDH,CLHG.clbh,clzl.ywpm,clzl.dwbh,clhg.HGBH,clhg.HGPM,clhg.UNitC,clhg.Ratec,clhg.userid,clhg.userdate,clhg.YN');
    end;
    sql.add('from CLZL ');
    sql.add('left join CLHG1 CLHG on CLHG.clbh = CLZL.cldh  ');
    sql.add('Where 1=1 ');
    SQL.Add('       '+StringReplace(MatNoSQL, ':CLBH', 'CLZL.CLDH',[rfReplaceAll, rfIgnoreCase]));
    sql.add('and clzl.ywpm like '+'''%'+edit2.Text+'%'+'''');
    if CheckBox1.Checked = true then
    begin
      SQL.Add('and CLHG.CLBH is not null ');
    end;
...
procedure TMaHaiQuan_NK.BB1Click(Sender: TObject);
begin
  Panel1.Visible:=true;
end;

procedure TMaHaiQuan_NK.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TMaHaiQuan_NK.FormDestroy(Sender: TObject);
begin
  MaHaiQuan_NK:=nil;
end;

procedure TMaHaiQuan_NK.BB2Click(Sender: TObject);
begin
  with Query1 do
  begin
    requestlive:=true;
...
procedure TMaHaiQuan_NK.BB3Click(Sender: TObject);
begin
  BB2.enabled:=false;
  BB4.enabled:=false;
  BB5.enabled:=true;
...
procedure TMaHaiQuan_NK.BB4Click(Sender: TObject);
begin
   BB2.Enabled:= false;
   BB3.Enabled:= false;
   BB5.Enabled:=true;
...
procedure TMaHaiQuan_NK.BB5Click(Sender: TObject);
var i:integer;
begin
  try
    Query1.first;
...
procedure TMaHaiQuan_NK.BB6Click(Sender: TObject);
begin
with Query1 do
  begin
  Active:=false;
...
procedure TMaHaiQuan_NK.bbt6Click(Sender: TObject);
var i,j,k:integer;
...
procedure TMaHaiQuan_NK.DBGrid1EditButtonClick(Sender: TObject);
begin
  TimMHQ_NK:=TTimMHQ_NK.create(self);
  TimMHQ_NK.show;
end;

procedure TMaHaiQuan_NK.DBGrid1GetCellParams(Sender: TObject;
...
procedure TMaHaiQuan_NK.btImportClick(Sender: TObject);
var
  ExcelApp: Variant;
...
begin
  ...
  begin
    ...
    begin
      ...
      Qtemp.active:=false;
      Qtemp.SQL.Clear;
      Qtemp.SQL.Add('select * from CLHG1');
      ...
      begin
          Qtemp.active:=false;
          Qtemp.SQL.Clear;
          Qtemp.SQL.Add('update CLHG1 set ');
      ...
      begin
         Qtemp.active:=false;
         Qtemp.SQL.Clear;
         Qtemp.SQL.Add(' insert into CLHG1(CLBH,HGBH,HGPM,UnitC,RateC,UserID,UserDate,YN)');
...
procedure TMaHaiQuan_NK.FormShow(Sender: TObject);
begin
