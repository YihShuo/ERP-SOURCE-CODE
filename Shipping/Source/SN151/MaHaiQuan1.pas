...
var
  MaHaiQuan: TMaHaiQuan;

implementation

uses main1, TimMaHQ1,FunUnit;

{$R *.dfm}
procedure TMaHaiQuan.GetSubSQL(var SQL:string;BtnObj:TSpeedButton;EditObj:TEdit;FieldNM:string;Flag:string);
...
procedure TMaHaiQuan.Button1Click(Sender: TObject);
var i:integer;
    tmpList:TStringlist;
    MatNoSQL:string;
begin
...
procedure TMaHaiQuan.BB1Click(Sender: TObject);
begin
  Panel1.Visible:=true;
end;

procedure TMaHaiQuan.FormDestroy(Sender: TObject);
begin
  MaHaiQuan:=nil;
end;

procedure TMaHaiQuan.BB4Click(Sender: TObject);
begin
   BB2.Enabled:= false;
   BB3.Enabled:= false;
   BB5.Enabled:=true;
...
procedure TMaHaiQuan.BB6Click(Sender: TObject);
begin
with Query1 do
  begin
  Active:=false;
...
procedure TMaHaiQuan.BB3Click(Sender: TObject);
begin
  BB2.enabled:=false;
  BB4.enabled:=false;
  BB5.enabled:=true;
...
procedure TMaHaiQuan.DBGrid1GetCellParams(Sender: TObject; Column: TColumnEh;
      AFont: TFont; var Background: TColor; State: TGridDrawState);
begin
...
procedure TMaHaiQuan.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TMaHaiQuan.BB5Click(Sender: TObject);
var i:integer;
begin
  try
    Query1.first;
...
procedure TMaHaiQuan.BB2Click(Sender: TObject);
begin
  with Query1 do
  begin
    requestlive:=true;
...
procedure TMaHaiQuan.bbt6Click(Sender: TObject);
var i,j,k:integer;
...
procedure TMaHaiQuan.DBGrid1EditButtonClick(Sender: TObject);
begin
  TimMaHQ:=TTimMaHQ.create(self);
  TimMaHQ.show;
end;

procedure TMaHaiQuan.FormShow(Sender: TObject);
begin
  main.FormLanguage(Pointer(self),self.Name);
end;

procedure TMaHaiQuan.btImportClick(Sender: TObject);
var
  ExcelApp: Variant;
...
begin
  ...
  begin
    ...
    begin
      ...
      Qtemp.active:=false;
      Qtemp.SQL.Clear;
      Qtemp.SQL.Add('select * from CLHG');
      ...
      begin
         Qtemp.active:=false;
         Qtemp.SQL.Clear;
         Qtemp.SQL.Add(' insert into CLHG(CLBH,HGBH,HGPM,UnitC,RateC,UserID,UserDate,YN)');
