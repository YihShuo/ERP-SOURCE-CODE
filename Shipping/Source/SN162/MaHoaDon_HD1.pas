...
var
  MaHoaDon_HD: TMaHoaDon_HD;

implementation

uses main1, TimMaHoaDon1;

{$R *.dfm}

procedure TMaHoaDon_HD.GetSubSQL(var SQL:string;BtnObj:TSpeedButton;EditObj:TEdit;FieldNM:string;Flag:string);
...
procedure TMaHoaDon_HD.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TMaHoaDon_HD.FormDestroy(Sender: TObject);
begin
  MaHoaDon_HD:=nil;
end;

procedure TMaHoaDon_HD.BB1Click(Sender: TObject);
begin
  Panel1.Visible:=true;
end;

procedure TMaHoaDon_HD.Button1Click(Sender: TObject);
var i:integer;
    tmpList:TStringlist;
    MatNoSQL:string;
begin
  ...
  begin
    ...
    begin
      sql.add('select top 1000 CLZL.CLDH,CLHD.clbh,clzl.ywpm,clzl.dwbh,CLHD.HGBH,CLHD.HGPM,CLHD.UNitC,CLHD.Ratec,CLHD.userid,CLHD.userdate,CLHD.YN');
    end else
    begin
      sql.add('select CLZL.CLDH,CLHD.clbh,clzl.ywpm,clzl.dwbh,CLHD.HGBH,CLHD.HGPM,CLHD.UNitC,CLHD.Ratec,CLHD.userid,CLHD.userdate,CLHD.YN');
    end;
    sql.add('from CLZL ');
    sql.add('left join CLHD1 CLHD on CLHD.clbh = CLZL.cldh  ');
    sql.add('Where 1=1 ');
    SQL.Add('       '+StringReplace(MatNoSQL, ':CLBH', 'CLZL.CLDH',[rfReplaceAll, rfIgnoreCase]));
    sql.add('and clzl.ywpm like '+'''%'+edit2.Text+'%'+'''');
    if CheckBox1.Checked = true then
    begin
      SQL.Add('and CLHD.CLBH is not null ');
    end;
...
procedure TMaHoaDon_HD.BB2Click(Sender: TObject);
begin
  with Query1 do
  begin
    requestlive:=true;
...
procedure TMaHoaDon_HD.BB3Click(Sender: TObject);
begin
  BB2.enabled:=false;
  BB4.enabled:=false;
  BB5.enabled:=true;
...
procedure TMaHoaDon_HD.BB4Click(Sender: TObject);
begin
  BB2.Enabled:= false;
  BB3.Enabled:= false;
  BB5.Enabled:=true;
...
procedure TMaHoaDon_HD.BB5Click(Sender: TObject);
var i:integer;
begin
  try
    Query1.first;
...
procedure TMaHoaDon_HD.BB6Click(Sender: TObject);
begin
  with Query1 do
  begin
  Active:=false;
...
procedure TMaHoaDon_HD.DBGrid1EditButtonClick(Sender: TObject);
begin
  TimMaHoaDon:=TTimMaHoaDon.create(self);
  TimMaHoaDon.show;
end;

procedure TMaHoaDon_HD.bbt6Click(Sender: TObject);
var i,j,k:integer;
...
procedure TMaHoaDon_HD.DBGrid1GetCellParams(Sender: TObject;
...
procedure TMaHoaDon_HD.btImportClick(Sender: TObject);
var
  ExcelApp: Variant;
...
begin
  ...
  begin
    ...
    begin
      ...
      Qtemp.active:=false;
      Qtemp.SQL.Clear;
      Qtemp.SQL.Add('select * from CLHD1');
      ...
      begin
          Qtemp.active:=false;
          Qtemp.SQL.Clear;
          Qtemp.SQL.Add('update CLHD1 set ');
      ...
      begin
         Qtemp.active:=false;
         Qtemp.SQL.Clear;
         Qtemp.SQL.Add(' insert into CLHD1(CLBH,HGBH,HGPM,UnitC,RateC,UserID,UserDate,YN)');
...
procedure TMaHoaDon_HD.FormShow(Sender: TObject);
begin
