...
var
  MaHaiQuanNKNQ: TMaHaiQuanNKNQ;

implementation
uses main1,FunUnit,TimMaHQNKNQ1;
{$R *.dfm}

procedure TMaHaiQuanNKNQ.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
   action:=cafree;
end;

procedure TMaHaiQuanNKNQ.FormDestroy(Sender: TObject);
begin
   MaHaiQuanNKNQ:=nil;
end;

procedure TMaHaiQuanNKNQ.BB1Click(Sender: TObject);
begin
  Panel1.Visible:=true;
end;

procedure TMaHaiQuanNKNQ.GetSubSQL(var SQL:string;BtnObj:TSpeedButton;EditObj:TEdit;FieldNM:string;Flag:string);
...
procedure TMaHaiQuanNKNQ.Button1Click(Sender: TObject);
var i:integer;
    tmpList:TStringlist;
    MatNoSQL:string;
begin
   ...
   with Query1 do
    ...
    if ShowCK.Checked=true then
      begin
        sql.add('select top 1000 CLZL.CLDH,CLNKNQ.clbh,clzl.ywpm,clzl.dwbh,CLNKNQ.HGBH,CLNKNQ.HGPM,CLNKNQ.UNitC,CLNKNQ.Ratec,CLNKNQ.userid,CLNKNQ.userdate,CLNKNQ.YN');
      end else
      begin
        sql.add('select CLZL.CLDH,CLNKNQ.clbh,clzl.ywpm,clzl.dwbh,CLNKNQ.HGBH,CLNKNQ.HGPM,CLNKNQ.UNitC,CLNKNQ.Ratec,CLNKNQ.userid,CLNKNQ.userdate,CLNKNQ.YN');
      end;
      sql.add('from CLZL ');
      sql.add('left join CLNKNQ on CLNKNQ.clbh = CLZL.cldh  ');
      sql.add('Where 1=1 ');
      SQL.Add('       '+StringReplace(MatNoSQL, ':CLBH', 'CLZL.CLDH',[rfReplaceAll, rfIgnoreCase]));
      sql.add('and clzl.ywpm like '+'''%'+edit2.Text+'%'+'''');
    if CheckBox1.Checked = true then
      begin
        SQL.Add('and CLNKNQ.CLBH is not null ');
      end;
    sql.add('order by CLNKNQ.clbh  ');
...
procedure TMaHaiQuanNKNQ.BB2Click(Sender: TObject);
begin
  with Query1 do
  begin
    requestlive:=true;
...
procedure TMaHaiQuanNKNQ.BB3Click(Sender: TObject);
begin
  BB2.enabled:=false;
  BB4.enabled:=false;
  BB5.enabled:=true;
...
procedure TMaHaiQuanNKNQ.BB4Click(Sender: TObject);
begin
    BB2.Enabled:= false;
    BB3.Enabled:= false;
    BB5.Enabled:=true;
...
procedure TMaHaiQuanNKNQ.BB5Click(Sender: TObject);
var i:integer;
begin
  try
    Query1.first;
...
procedure TMaHaiQuanNKNQ.BB6Click(Sender: TObject);
begin
  with Query1 do
  begin
    Active:=false;
...
procedure TMaHaiQuanNKNQ.bbt6Click(Sender: TObject);
var i,j,k:integer;
...
procedure TMaHaiQuanNKNQ.btImportClick(Sender: TObject);
var
  ExcelApp: Variant;
...
begin
  ...
  begin
    ...
    begin
      ...
      Qtemp.active:=false;
      Qtemp.SQL.Clear;
      Qtemp.SQL.Add('select * from CLNKNQ');
      ...
      begin
          Qtemp.active:=false;
          Qtemp.SQL.Clear;
          Qtemp.SQL.Add('update CLNKNQ set ');
      ...
      begin
         Qtemp.active:=false;
         Qtemp.SQL.Clear;
         Qtemp.SQL.Add(' insert into CLNKNQ(CLBH,HGBH,HGPM,UnitC,RateC,UserID,UserDate,YN)');
...
procedure TMaHaiQuanNKNQ.DBGrid1GetCellParams(Sender: TObject;
...
procedure TMaHaiQuanNKNQ.DBGrid1EditButtonClick(Sender: TObject);
begin
  TimMaHQNKNQ:=TTimMaHQNKNQ.create(self);
  TimMaHQNKNQ.show;
end;
