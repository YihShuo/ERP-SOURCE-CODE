...
var
  MaHaiQuan_TC: TMaHaiQuan_TC;

implementation

uses main1, TimMa_TC1;

{$R *.dfm}

procedure TMaHaiQuan_TC.GetSubSQL(var SQL:string;BtnObj:TSpeedButton;EditObj:TEdit;FieldNM:string;Flag:string);
...
procedure TMaHaiQuan_TC.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TMaHaiQuan_TC.FormDestroy(Sender: TObject);
begin
  MaHaiQuan_TC:=nil;
end;

procedure TMaHaiQuan_TC.BB1Click(Sender: TObject);
begin
   Panel1.Visible:=true;
end;

procedure TMaHaiQuan_TC.Button1Click(Sender: TObject);
var i:integer;
    tmpList:TStringlist;
    MatNoSQL:string;
begin
  ...
  begin
    ...
    begin
      sql.add('select top 1000 CLZL.CLDH,CLTC.clbh,clzl.ywpm,clzl.dwbh,CLTC.HGBH,CLTC.HGPM,CLTC.UNitC,CLTC.Ratec,CLTC.userid,CLTC.userdate,CLTC.YN');
    end else
    begin
      sql.add('select CLZL.CLDH,CLTC.clbh,clzl.ywpm,clzl.dwbh,CLTC.HGBH,CLTC.HGPM,CLTC.UNitC,CLTC.Ratec,CLTC.userid,CLTC.userdate,CLTC.YN');
    end;
    sql.add('from CLZL ');
    sql.add('left join CLTC1 CLTC on CLTC.clbh = CLZL.cldh  ');
    sql.add('Where 1=1 ');
    SQL.Add('       '+StringReplace(MatNoSQL, ':CLBH', 'CLZL.CLDH',[rfReplaceAll, rfIgnoreCase]));
    sql.add('and clzl.ywpm like '+'''%'+edit2.Text+'%'+'''');
    if CheckBox1.Checked = true then
    begin
      SQL.Add('and CLTC.CLBH is not null ');
    end;
    sql.add('order by CLTC.clbh  ');
...
procedure TMaHaiQuan_TC.BB2Click(Sender: TObject);
begin
  with Query1 do
  begin
    requestlive:=true;
...
procedure TMaHaiQuan_TC.BB3Click(Sender: TObject);
begin
  BB2.enabled:=false;
  BB4.enabled:=false;
  BB5.enabled:=true;
...
procedure TMaHaiQuan_TC.BB4Click(Sender: TObject);
begin
  BB2.Enabled:= false;
  BB3.Enabled:= false;
  BB5.Enabled:=true;
...
procedure TMaHaiQuan_TC.BB5Click(Sender: TObject);
var i:integer;
begin
  try
    Query1.first;
...
procedure TMaHaiQuan_TC.BB6Click(Sender: TObject);
begin
  with Query1 do
  begin
  Active:=false;
...
procedure TMaHaiQuan_TC.bbt6Click(Sender: TObject);
var i,j,k:integer;
...
procedure TMaHaiQuan_TC.DBGrid1EditButtonClick(Sender: TObject);
begin
  TimMa_TC:=TTimMa_TC.create(self);
  TimMa_TC.show;
end;

procedure TMaHaiQuan_TC.DBGrid1GetCellParams(Sender: TObject;
...
procedure TMaHaiQuan_TC.btImportClick(Sender: TObject);
var
  ExcelApp: Variant;
...
begin
  ...
  begin
    ...
    begin
      ...
      Qtemp.active:=false;
      Qtemp.SQL.Clear;
      Qtemp.SQL.Add('select * from CLTC1');
      ...
      begin
          Qtemp.active:=false;
          Qtemp.SQL.Clear;
          Qtemp.SQL.Add('update CLTC1 set ');
      ...
      begin
         Qtemp.active:=false;
         Qtemp.SQL.Clear;
         Qtemp.SQL.Add(' insert into CLTC1(CLBH,HGBH,HGPM,UnitC,RateC,UserID,UserDate,YN)');
...
procedure TMaHaiQuan_TC.FormShow(Sender: TObject);
begin
