...
procedure TMaHaiQuanTC.GetSubSQL(var SQL:string;BtnObj:TSpeedButton;EditObj:TEdit;FieldNM:string;Flag:string);
...
procedure TMaHaiQuanTC.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TMaHaiQuanTC.FormDestroy(Sender: TObject);
begin
   MaHaiQuanTC:=nil;
end;

procedure TMaHaiQuanTC.BB1Click(Sender: TObject);
begin
   Panel1.Visible:=true;
end;

procedure TMaHaiQuanTC.Button1Click(Sender: TObject);
var i:integer;
    tmpList:TStringlist;
    MatNoSQL:string;
begin
  ...
  begin
    ...
    sql.add('from CLZL ');
    sql.add('left join CLTC on CLTC.clbh = CLZL.cldh  ');
    sql.add('Where 1=1 ');
...
procedure TMaHaiQuanTC.FormShow(Sender: TObject);
begin
  main.FormLanguage(Pointer(self),self.Name);
end;

procedure TMaHaiQuanTC.BB2Click(Sender: TObject);
begin
  with Query1 do
  begin
    requestlive:=true;
...
procedure TMaHaiQuanTC.BB3Click(Sender: TObject);
begin
  BB2.enabled:=false;
  BB4.enabled:=false;
  BB5.enabled:=true;
...
procedure TMaHaiQuanTC.BB4Click(Sender: TObject);
begin
  BB2.Enabled:= false;
  BB3.Enabled:= false;
  BB5.Enabled:=true;
...
procedure TMaHaiQuanTC.BB5Click(Sender: TObject);
var i:integer;
begin
  try
    Query1.first;
...
procedure TMaHaiQuanTC.BB6Click(Sender: TObject);
begin
  with Query1 do
  begin
  Active:=false;
...
procedure TMaHaiQuanTC.bbt6Click(Sender: TObject);
var i,j,k:integer;
...
procedure TMaHaiQuanTC.DBGrid1GetCellParams(Sender: TObject;
...
procedure TMaHaiQuanTC.DBGrid1EditButtonClick(Sender: TObject);
begin
  TimMaTC:=TTimMaTC.create(self);
  TimMaTC.show;
end;

procedure TMaHaiQuanTC.btImportClick(Sender: TObject);
var
  ExcelApp: Variant;
...
begin
  ...
  begin
    ...
    begin
      ...
      Qtemp.active:=false;
      Qtemp.SQL.Clear;
      Qtemp.SQL.Add('select * from CLTC');
      ...
      begin
         Qtemp.active:=false;
         Qtemp.SQL.Clear;
         Qtemp.SQL.Add(' insert into CLTC(CLBH,HGBH,HGPM,UnitC,RateC,UserID,UserDate,YN)');
