...
begin
  ...
  begin
    Active:=true;
    SQL.Clear;
    SQL.Add('select top 20 Cldh,zwpm,ywpm,dwbh,CQDH,CLZL.TYJH,zszl.zsdh,zszl.ZSYWJC  from CLZL ');
    sql.Add('left join zszl on zszl.zsdh=clzl.zsdh ');
    SQL.Add('where CLDH like ''%'+CLDHEdit.Text+'%'' and ((Substring(CLDH,1,3)=''V90'') or CLZL.YN=1 ) ');//越南臨時料號或台灣建立材料
...
begin
  ...
  begin
    ...
    begin
      SampleMaterial_Combinated.CLZHZLVN.FieldByName('ZSDH').AsString:=MatQry.FieldByName('ZSDH').AsString;
      SampleMaterial_Combinated.CLZHZLVN.FieldByName('ZSJC').AsString:=MatQry.FieldByName('ZSYWJC').AsString;
    end;
    //
