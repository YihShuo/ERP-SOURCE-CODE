...
type
  TAnyStock = class(TForm)
    ...
    procedure ShowSRClick(Sender: TObject);
  private
...
begin
if (showSR.Checked) and ((editseason.text='') or (Editstage.text='')) then
   begin
      showmessage('Plz type Season and Stage');
      abort;
   end;
if CBX1.text='' then
  begin
...
if length(amonth)=1 then
 ...
 begin
  ...
  begin
    ...
    //畐计秖
    if CBX1.text<>'ALL' then
       sql.add('left join (select JGZL.CKBH,JGZLS.ZMLB,sum(round(convert(float,JGZLS.Qty)*convert(float,JGZLM.Qty),2)) as JGCK from JGZLS with (nolock) ');
    if CBX1.text='ALL' then
       sql.add('left join (select ''ALL'' as CKBH,JGZLS.ZMLB,sum(round(convert(float,JGZLS.Qty)*convert(float,JGZLM.Qty),2)) as JGCK from JGZLS with (nolock) ');
   ...
   // sql.add('                      and JGZL.CFMID1 <>'+''''+'NO'+'''');
    ...
    if CBX1.text<>'ALL' then
    begin
       if not showSR.Checked then
          sql.add('select CLHG.HGBH,clzl.zsdh,zszl.zsqm,#CLZLKC.CLDH,'''' as devcode,IsNull(#CLZLKC.LBBH,CLLBFLS.LBBH) as LBBH,CLZL.YWPM,CLZL.ZWPM,#CLZLKC.DWBH,isnull(#CLZLKC.LastRem,0) as LastRem,');
       if showSR.Checked then
           sql.add('select CLHG.HGBH,clzl.zsdh,zszl.zsqm,#CLZLKC.CLDH,a.devcode,IsNull(#CLZLKC.LBBH,CLLBFLS.LBBH) as LBBH,CLZL.YWPM,CLZL.ZWPM,#CLZLKC.DWBH,isnull(#CLZLKC.LastRem,0) as LastRem,');
    ...
    begin
       if not showSR.Checked then
          sql.add('select ''ALL'' as HGBH,clzl.zsdh,zszl.zsqm,#CLZLKC.CLDH,'''' as devcode,''ALL'' LBBH,CLZL.YWPM,CLZL.ZWPM,#CLZLKC.DWBH,');
       if showSR.Checked then
          sql.add('select ''ALL'' as HGBH,clzl.zsdh,zszl.zsqm,#CLZLKC.CLDH,a.devcode,''ALL'' LBBH,CLZL.YWPM,CLZL.ZWPM,#CLZLKC.DWBH,');
       sql.add('       sum(isnull(#CLZLKC.LastRem,0)) as LastRem,');
       sql.add('       sum(isnull(#CLZLKC.RKQty,0)) as RKQty,');
       sql.add('       sum(isnull(#CLZLKC.LLQty,0)) as LLQty,');
       sql.add('       sum(isnull(round(#CLZLKC.JGRK,2),0)) as JGRK,');
       sql.add('       sum(isnull(round(#CLZLKC.JGCK,2),0)) as JGCK,');
       sql.add('       sum(isnull(round(#CLZLKC.JGCKTemp,2),0)) as JGCKTemp,');
       sql.add('       ''ALL'' as KCBH,''ALL'' as CKBH,');
       sql.add('       sum(isnull(Round(#CLZLKC.LastRem,2),0)+isnull(Round(#CLZLKC.RKQty,2),0)-isnull(Round(#CLZLKC.LLQty,2),0)+isnull(Round(#CLZLKC.JGRK,2),0)-isnull(Round(#CLZLKC.JGCK,2),0)) as Qty'); //20141027 э计材
    end;
    sql.add('from #CLZLKC ');
    ...
    begin
       sql.add('left join ( ');
       sql.add('select * from ( ');
       sql.add('select kfxxzl.devcode,ypzl.article,kfxxzl.xieming,clzl.cldh,clzl.ywpm,kfxxzl.khdh from ypzls ');
       sql.add('left join ypzl on ypzl.ypdh=ypzls.ypdh ');
       sql.add('left join kfxxzl on kfxxzl.xiexing=ypzl.xiexing and kfxxzl.shehao=ypzl.shehao ');
       sql.add('left join clzl on ypzls.clbh=clzl.cldh ');
       sql.add('where kfxxzl.khdh=''036'' and ypzl.kfjd = '''+EditStage.text+''' and kfxxzl.jijie='''+EditSeason.text+''' ');
       sql.add('union ');
       sql.add('select kfxxzl.devcode,ypzl.article,kfxxzl.xieming,clzhzl.cldh1,clzl.ywpm,kfxxzl.khdh from ypzls ');
       ...
       sql.add('where kfxxzl.khdh=''036'' and ypzl.kfjd = '''+EditStage.text+''' and kfxxzl.jijie='''+EditSeason.text+''' ');
       sql.add('union ');
       sql.add('select devcode,article,xieming,clzhzl.cldh1,clzl.ywpm,khdh from ( ');
       sql.add('select kfxxzl.devcode,ypzl.article,kfxxzl.xieming,clzl.cldh,clzl.ywpm,kfxxzl.khdh from ypzls ');
       sql.add('inner join clzhzl on clzhzl.cldh=ypzls.clbh ');
       sql.add('left join ypzl on ypzl.ypdh=ypzls.ypdh ');
       sql.add('left join kfxxzl on kfxxzl.xiexing=ypzl.xiexing and kfxxzl.shehao=ypzl.shehao ');
       sql.add('left join clzl on clzhzl.cldh1=clzl.cldh ');
       sql.add('where kfxxzl.khdh=''036'' and ypzl.kfjd = '''+EditStage.text+''' and kfxxzl.jijie='''+EditSeason.text+''')clzhzl2 ');
       sql.add('inner join clzhzl on clzhzl.cldh=clzhzl2.cldh ');
       sql.add('left join clzl on clzhzl.cldh1=clzl.cldh ');
       sql.add(')a ');
       sql.add(')a on a.cldh=#CLZLKC.CLDH ');
    end;
    if CBX1.text<>'ALL' then
    begin
    ...
    if showSR.checked then
       sql.add('and a.cldh is not null');
    if CBX1.text<>'ALL' then
...
{    sql.add('where  isnull(#CLZLKC.LastRem,0)+isnull(#CLZLKC.RKQty,0)-isnull(#CLZLKC.LLQty,0)+isnull(#CLZLKC.JGRK,0)-isnull(#CLZLKC.JGCK,0)>=0.05');
...
procedure TAnyStock.ShowSRClick(Sender: TObject);
begin
  if showsr.Checked then
  begin
     panel2.visible:=true;
