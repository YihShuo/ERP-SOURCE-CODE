...
type
  TSampleTracking_Pat = class(TForm)
    MasPanel: TPanel;
    BB2: TBitBtn;
    BB3: TBitBtn;
    BB4: TBitBtn;
    ...
    SPSCMasZBDate: TDateTimeField;
    SPSCMasZBExDate: TDateTimeField;
    SPSCMasSJDate: TDateTimeField;
    SPSCMasSJExDate: TDateTimeField;
    SPSCMasJDDate: TDateTimeField;
    SPSCMasJDExDate: TDateTimeField;
    SPSCMasWBDate: TDateTimeField;
    SPSCMasWBExDate: TDateTimeField;
    Splitter1: TSplitter;
...
var
  SampleTracking_Pat: TSampleTracking_Pat;
  NDate:TDate;
implementation
   uses main1,SampleTrackReason1,SampleTrackingRms1,  FunUnit;
{$R *.dfm}

procedure TSampleTracking_Pat.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TSampleTracking_Pat.FormDestroy(Sender: TObject);
begin
   SampleTracking_Pat:=nil;
end;
//
procedure TSampleTracking_Pat.Button1Click(Sender: TObject);
var i:integer;
begin
   //
   with SPSCMas do
   begin
     ...
     SQL.Add('  SPSCList.ZBDate,(Select MAX(ExeDate) from SPSCRm A where A.SPNO=SPSCList.SPNO and ItemNM=''ZBExDate'') as ZBExDate, ');
     SQL.Add('  SPSCList.SJDate,(Select MAX(ExeDate) from SPSCRm A where A.SPNO=SPSCList.SPNO and ItemNM=''SJExDate'') as SJExDate, ');
     SQL.Add('  SPSCList.JDDate,(Select MAX(ExeDate) from SPSCRm A where A.SPNO=SPSCList.SPNO and ItemNM=''JDExDate'') as JDExDate, ');
     SQL.Add('  SPSCList.WBDate,(Select MAX(ExeDate) from SPSCRm A where A.SPNO=SPSCList.SPNO and ItemNM=''WBExDate'') as WBExDate, ');
...
//
procedure TSampleTracking_Pat.BB2Click(Sender: TObject);
begin

end;
//
procedure TSampleTracking_Pat.BB3Click(Sender: TObject);
begin

end;
//
procedure TSampleTracking_Pat.BB4Click(Sender: TObject);
begin

end;
//
procedure TSampleTracking_Pat.BB6Click(Sender: TObject);
begin

end;

//儲存
procedure TSampleTracking_Pat.FormCreate(Sender: TObject);
begin
  //
  with tmpQuery do
  begin
  ...
  begin
   //單位執行項目
    ...
    DBGridEh2.Columns[0].PickList.add('ZBExDate');
    DBGridEh2.Columns[0].PickList.add('SJExDate');
    DBGridEh2.Columns[0].PickList.add('JDExDate');
    DBGridEh2.Columns[0].PickList.add('WBExDate');
    Label9.Visible:=true;
   //
   With SPSCDet do
   begin
     ...
     SQL.Add('where SPNO=:SPNO and RENO>0  and ((SPSCRm.ItemNM=''ZBExDate'') or (SPSCRm.ItemNM=''SJExDate'') or (SPSCRm.ItemNM=''JDExDate'') or (SPSCRm.ItemNM=''WBExDate'')) ');
  ...
  begin
   ...
   DBGridEh1.Columns[8].Visible:=false;  //SJExDate
   DBGridEh1.Columns[9].Visible:=false; //SJDate
   //
   With SPSCDet do
   begin
     Active:=false;
     ...
     SQL.Add('from SPSCRm ');
     SQL.Add('Left join SPSCRmS on SPSCRmS.LYBH=SPSCRm.LYBH ');
     SQL.Add('where SPNO=:SPNO and RENO>0  and ((SPSCRm.ItemNM=''ZBExDate'')  or (SPSCRm.ItemNM=''JDExDate'') or (SPSCRm.ItemNM=''WBExDate'')) ');
     SQL.Add('order by SPSCRm.ItemNM,SPSCRm.ExeDate ');
   end;
  end;
end;

procedure TSampleTracking_Pat.DBGridEh1DrawColumnCell(Sender: TObject;
...
begin
  ...
  if ((SPSCMas.FieldByName('ZBExDate').IsNull=false) and (SPSCMas.FieldByName('SJExDate').IsNull=false) and (SPSCMas.FieldByName('JDExDate').IsNull=false) and (SPSCMas.FieldByName('WBExDate').IsNull=false) )  then
  begin
...
procedure TSampleTracking_Pat.SPSCMasAfterOpen(DataSet: TDataSet);
begin
...
procedure TSampleTracking_Pat.RB1Click(Sender: TObject);
begin
...
procedure TSampleTracking_Pat.RB2Click(Sender: TObject);
begin
  //
  with SPSCDet do
  begin
...
procedure TSampleTracking_Pat.DBGridEh2DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumnEh;
  State: TGridDrawState);
begin
  //
...
procedure TSampleTracking_Pat.RB3Click(Sender: TObject);
begin
  //
  with SPSCDet do
  begin
...
procedure TSampleTracking_Pat.RB5Click(Sender: TObject);
begin
  //
  with SPSCDet do
    begin
...
//
procedure TSampleTracking_Pat.RB4Click(Sender: TObject);
var i,RENO:integer;
begin
  //
  SPSCDet.First;
...
procedure TSampleTracking_Pat.DBGridEh2EditButtonClick(Sender: TObject);
begin
  //
  with DBGridEh2 do
  begin
...
//
procedure TSampleTracking_Pat.BB1Click(Sender: TObject);
begin
  ToolPanel.Visible:=true;
end;
//
procedure TSampleTracking_Pat.BitBtn1Click(Sender: TObject);
begin
  //
  if SampleTrackingRms=nil then
    SampleTrackingRms:=TSampleTrackingRms.Create(self);
  SampleTrackingRms.SetDepartment('Pattern');
  SampleTrackingRms.Show;
  //
end;

procedure TSampleTracking_Pat.SPSCDetItemNMSetText(Sender: TField;
  const Text: String);
begin
  if Text='ZBExDate' then  SPSCDet.FieldByName('ExeDate').Value:=SPSCMas.FieldByName('ZBDate').Value;
  if Text='SJExDate' then  SPSCDet.FieldByName('ExeDate').Value:=SPSCMas.FieldByName('SJDate').Value;
  if Text='JDExDate' then  SPSCDet.FieldByName('ExeDate').Value:=SPSCMas.FieldByName('JDDate').Value;
  if Text='WBExDate' then  SPSCDet.FieldByName('ExeDate').Value:=SPSCMas.FieldByName('WBDate').Value;
