unit AccountDelRea_Det1;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, Menus, DB, DBTables, StdCtrls, ExtCtrls, Grids, DBGrids,
  comobj, GridsEh, DBGridEh;

type
  TAccountDelRea_Det = class(TForm)
    Panel1: TPanel;
    Label1: TLabel;
    Query1: TQuery;
...
var
  AccountDelRea_Det: TAccountDelRea_Det;

implementation

uses AccountDelReason1;

{$R *.dfm}

procedure TAccountDelRea_Det.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
action:=cafree;
end;

procedure TAccountDelRea_Det.FormCreate(Sender: TObject);
begin
with query1 do
  begin
    active:=false;
    ...
    sql.add('and KCLLS.YYBH=:YYBH');
    sql.add('and convert(smalldatetime,convert(varchar,KCLL.CFMdate,111)) between ');
    sql.add(''''+formatdatetime('yyyy/MM/dd',AccountDelreason.DTP1.Date)+'''');
    sql.add(' and '+''''+ formatdatetime('yyyy/MM/dd',AccountDelreason.DTP2.Date)+'''');
...
procedure TAccountDelRea_Det.Excel1Click(Sender: TObject);
var
...
procedure TAccountDelRea_Det.Query1CalcFields(DataSet: TDataSet);
begin
with query1 do
  begin
...
procedure TAccountDelRea_Det.FormDestroy(Sender: TObject);
begin
AccountDelRea_Det:=nil;
end;
