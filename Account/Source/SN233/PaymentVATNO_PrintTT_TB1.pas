...
type
  TPaymentVATNO_PrintTT_TB = class(TForm)
...
var
  PaymentVATNO_PrintTT_TB: TPaymentVATNO_PrintTT_TB;
...
function TPaymentVATNO_PrintTT_TB.RMBFloatToChinese(ARMBCash: Real): string;
const
...
function TPaymentVATNO_PrintTT_TB.RMBFloatToVietnam(inputNumber: Real): string;
const
...
procedure TPaymentVATNO_PrintTT_TB.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TPaymentVATNO_PrintTT_TB.FormDestroy(Sender: TObject);
begin
  PaymentVATNO_PrintTT_TB:=nil;
end;

procedure TPaymentVATNO_PrintTT_TB.VN_WORDPrint(sender: TObject;
  var Value: String);
  var acc, str:string;
begin
...
procedure TPaymentVATNO_PrintTT_TB.TW_WORDPrint(sender: TObject;
  var Value: String);
  var acc:string;
begin
  acc:=StringReplace(QRExpr12.Value.strResult,',','',[rfReplaceAll, rfIgnoreCase]);
...
procedure TPaymentVATNO_PrintTT_TB.DatePrint(sender: TObject;
  var Value: String);
begin
  date2.Caption:=PaymentVATNO.GetChineseDate(PaymentVATNO.qry_CGPayment.FieldByName('USERDATE').value);
end;

procedure TPaymentVATNO_PrintTT_TB.FormCreate(Sender: TObject);
begin
...
procedure TPaymentVATNO_PrintTT_TB.QRExpr12Print(sender: TObject;
  var Value: String);
begin
  if value='0.00' then
...
procedure TPaymentVATNO_PrintTT_TB.QRSubDetail3BeforePrint(
