...
type
  TPaymentVATNO_PrintTTTU = class(TForm)
    qry_TTT: TQuery;
...
var
  PaymentVATNO_PrintTTTU: TPaymentVATNO_PrintTTTU;
...
function TPaymentVATNO_PrintTTTU.RMBFloatToChinese(ARMBCash: Real): string;
const
...
function TPaymentVATNO_PrintTTTU.RMBFloatToVietnam(inputNumber: Real): string;
const
...
procedure TPaymentVATNO_PrintTTTU.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TPaymentVATNO_PrintTTTU.FormDestroy(Sender: TObject);
begin
  PaymentVATNO_PrintTTTU:=nil;
end;

procedure TPaymentVATNO_PrintTTTU.VN_WORD5Print(sender: TObject;
  var Value: String);
  var acc, str:string;
begin
...
procedure TPaymentVATNO_PrintTTTU.TW_WORD5Print(sender: TObject;
  var Value: String);
  var acc:string;
begin
  if PaymentVATNO.qry_CGPaymentTAM.FieldByName('SOTIEN_HOANLAI').value>0 then
...
procedure TPaymentVATNO_PrintTTTU.date4Print(sender: TObject;
  var Value: String);
begin
  date4.Caption:=PaymentVATNO.GetChineseDate(PaymentVATNO.qry_CGPayment.FieldByName('USERDATE').value);
end;

procedure TPaymentVATNO_PrintTTTU.FormCreate(Sender: TObject);
begin
