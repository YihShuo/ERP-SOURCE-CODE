unit PaymentVATNO_PrintPTU_TB1;
...
type
  TPaymentVATNO_PrintPTU_TB = class(TForm)
    qry_temp: TQuery;
    QuickRep6: TQuickRep;
    QRBand24: TQRBand;
    L61: TQRLabel;
...
var
  PaymentVATNO_PrintPTU_TB: TPaymentVATNO_PrintPTU_TB;
...
function TPaymentVATNO_PrintPTU_TB.RMBFloatToChinese(ARMBCash: Real): string;
const
...
function TPaymentVATNO_PrintPTU_TB.RMBFloatToVietnam(inputNumber: Real): string;
const
...
procedure TPaymentVATNO_PrintPTU_TB.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TPaymentVATNO_PrintPTU_TB.FormDestroy(Sender: TObject);
begin
  PaymentVATNO_PrintPTU_TB:=nil;
end;

procedure TPaymentVATNO_PrintPTU_TB.date5Print(sender: TObject;
  var Value: String);
begin
  date5.Caption:=PaymentVATNO.GetChineseDate(PaymentVATNO.qry_CGPayment.FieldByName('NGAY_TAMUNG').value);
end;

procedure TPaymentVATNO_PrintPTU_TB.VN_WORD5Print(sender: TObject;
  var Value: String);
  var acc, str:string;
begin
...
procedure TPaymentVATNO_PrintPTU_TB.TW_WORD5Print(sender: TObject;
  var Value: String);
  var acc:string;
begin
  acc:=StringReplace(QRExpr26.Value.strResult,',','',[rfReplaceAll, rfIgnoreCase]);
