...
procedure TOrderMark_Mt.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TOrderMark_Mt.Button1Click(Sender: TObject);
begin
  with query1 do
  begin
    active:=false;
...
procedure TOrderMark_Mt.DBGrideh1DblClick(Sender: TObject);
var a:string;
begin
  ...
  begin
    with OrderMark.YWDDMT do
      begin
        ...
        edit;
        fieldbyname('MTBH1').value:=query1.fieldbyname('MTBH').value;
        fieldbyname('MTLR1').value:=query1.fieldbyname('MTQZ').value+a;
        //post;
      end;
  end;
  Close;
...
procedure TOrderMark_Mt.DBGrideh1KeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
     Dbgrideh1dblclick(nil);
end;

procedure TOrderMark_Mt.FormDestroy(Sender: TObject);
begin
  ordermark_mt:=nil;
end;

procedure TOrderMark_Mt.FormShow(Sender: TObject);
begin
