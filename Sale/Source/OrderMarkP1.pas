...
procedure TOrderMarkP.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TOrderMarkP.FormCreate(Sender: TObject);
begin
DTP1.Date:=date;
end;

procedure TOrderMarkP.BB4Click(Sender: TObject);
begin
close;
end;

procedure TOrderMarkP.BB1Click(Sender: TObject);
begin
edit1.Text:='1';
with QTW do
  begin
    active:=false;
    SQL.Clear;
    sql.Add('select * from LY_SHOE.dbo.DDMarkp l');
...
procedure TOrderMarkP.QVNAfterOpen(DataSet: TDataSet);
begin
...
procedure TOrderMarkP.BD1Click(Sender: TObject);
var i:integer;
begin
...
procedure TOrderMarkP.BD4Click(Sender: TObject);
begin
QVN.first;
...
procedure TOrderMarkP.BD3Click(Sender: TObject);
begin
...
procedure TOrderMarkP.BD5Click(Sender: TObject);
begin
QVN.Active:=false;
QVn.CachedUpdates:=false;
QVN.RequestLive:=false;
...
procedure TOrderMarkP.BD2Click(Sender: TObject);
var i:integer;
begin
QVN.CachedUpdates:=true;
QVN.RequestLive:=true;
...
procedure TOrderMarkP.BB2Click(Sender: TObject);
begin
edit1.Text:='0';
with QTW do
  begin
    active:=false;
    SQL.Clear;
    sql.Add('select * from LY_ERP.dbo.DDMarkp l');
    sql.add('where not exists(select DDBH from LY_SHOE.dbo.DDmarkp d ');
