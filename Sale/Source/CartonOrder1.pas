...
uses
  ...
  ,Math,dateutils, Menus, Mask, DBCtrls, ComCtrls, comobj,DBGridEhImpExp, ShellAPI, Zlib;
...
type
  ...
  public
    procedure setCarton(vCLBH:Variant;bBelow:boolean;vYWPM:String);
  end;
...
procedure TCartonOrder.setCarton(vCLBH:Variant;bBelow:boolean;vYWPM:String);
...
procedure TCartonOrder.N1Click(Sender: TObject);
begin
  countCTS();
end;
...
begin
  ...
  begin
      first;
      next;
      if CGZL.fieldbyname('ZSBH').isnull then
      begin
          showmessage('Pls select supplier first, then save again.');
          abort;
      end;
      while not eof do
      begin
      ...
      begin
        ...
        begin
            last;
            sCGNO:=fieldbyname('CGNO').AsString;
            sCGNO:=copy(sCGNO,7,5);
            sCGNO:=inttostr(strtoint(sCGNO)+1);
            while length(sCGNO)<5 do
              sCGNO:='0'+sCGNO;
        end else
          sCGNO:='00001';
        sCGNO :=y+m+sCGNO;
      ...
      // bug: CGZL 只在 INSERT 時需代入 CGNO
      try
        case CGZL.updatestatus of
        usinserted:
          begin
            // 存入 CGZL
            CGZL.edit;
            CGZL.fieldbyname('CGNO').Value:=sCGNO;
            CGZL.fieldbyname('GSBH').Value:=main.Edit2.text;
            CGZL.fieldbyname('CGLX').Value:='5';
            CGZL.FieldByName('userID').Value:=main.edit1.text;
            CGZL.FieldByName('userdate').Value:=formatdatetime('yyyy/MM/dd',Ndate);
            CGZL.FieldByName('CGDate').Value:=formatdatetime('yyyy/MM/dd HH:NN:SS',Now());
            CGZL.FieldByName('YN').Value:='1';
            upCGZL.apply(ukinsert);
        ...
        usmodified:
          begin
            if (NDate-CGZL.FieldByName('USERDATE').value)<90  then
            begin
                CGZL.edit;
                CGZL.FieldByName('userID').Value:=main.edit1.text;
                CGZL.FieldByName('userdate').Value:=formatdatetime('yyyy/MM/dd',Ndate);
                upCGZL.apply(ukmodify);
            end else
            begin
        ...
        sCGNO:=CGZL.fieldbyname('CGNO').Value;
        CGZL.active:=false;
...
begin
  ...
  begin
    ...
    begin
      ...
      SQL.Add('where YWBZPOS.DDBH in (select DDBH from YWDD where YSBH='''+YWDD.fieldbyname('DDBH').value+''') and XXZLS.CLBH in (select CLBH from ZLZLS2 where ZLBH='''+YWDD.fieldbyname('DDBH').value+''' and CLBH like ''H14%'')');
      active:=true;
    end;
          ...
          case YWBZPOE.updatestatus of
            ...
            begin
                ...
                begin
                  ...
                  begin
                    if YWBZPOE.fieldbyname('CLBH').isnull then
                    begin
                ...
                begin
                  if YWBZPOE.fieldbyname('CLBH').isnull=false then
                  begin
                     YWBZPOE.edit;
                     YWBZPOE.FieldByName('userID').Value:=main.edit1.text;
                     YWBZPOE.FieldByName('userdate').Value:=NDate;
...
begin
   ...
   begin
     ...
     begin
       Active:=false;
       SQL.Clear;
       SQL.Add('select CLBH from ZLZLS2 where ZLBH='''+YWDD.FieldByName('DDBH').AsString+''' and CLBH like ''H14%'' ');
       Active:=true;
       if RecordCount>0 then
       begin
         InfoBox.Caption:='';
       ...
       begin
         InfoBox.Caption:='No inner box, khong co H14% ma vat tu';
       end;
       Active:=false;
     end;
   end;
...
procedure TCartonOrder.BitBtn1Click(Sender: TObject);
var bm:Tbookmark;
    i,index:integer;
      ...
      try
        ...
        begin
          ...
          begin
            ...
            sql.add('select  YWDD.YSBH as DDBH,''VN'' as XH,BWZL.BWDH as BWBH,CGZL.ZSBH,');
            ...
            sql.add('          select '''+YWDD.fieldbyname('DDBH').Value+''' as ddbh,CLBH,Sum(CTS) as CLSL from ywbzpo where DDBH in (select DDBH from YWDD where YSBH='''+YWDD.fieldbyname('DDBH').Value+''') Group by CLBH ');
            sql.add('          union all');
            sql.add('          select ddbh,CLBH,Sum(CTS) as CLSL from ywbzpoe where DDBH='''+YWDD.fieldbyname('DDBH').Value+''' Group by DDBH,CLBH)YWBZPO)YWBZPO');
