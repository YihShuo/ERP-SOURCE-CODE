...
procedure TOrderMemo.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TOrderMemo.FormCreate(Sender: TObject);
begin
DTP1.Date:=date;
end;

procedure TOrderMemo.BB4Click(Sender: TObject);
begin
close;
end;

procedure TOrderMemo.BB1Click(Sender: TObject);
begin
edit1.Text:='1';
with QTW do
  begin
    active:=false;
    SQL.Clear;
    sql.Add('select * from LY_SHOE.dbo.DDBZZL l');
...
procedure TOrderMemo.QVNAfterOpen(DataSet: TDataSet);
begin
...
procedure TOrderMemo.BD1Click(Sender: TObject);
var i:integer;
begin
...
procedure TOrderMemo.BD4Click(Sender: TObject);
begin
QVN.first;
...
procedure TOrderMemo.BD3Click(Sender: TObject);
begin
...
procedure TOrderMemo.BD5Click(Sender: TObject);
begin
QVN.Active:=false;
QVn.CachedUpdates:=false;
QVN.RequestLive:=false;
...
procedure TOrderMemo.BD2Click(Sender: TObject);
var i:integer;
begin
QVN.CachedUpdates:=true;
QVN.RequestLive:=true;
...
procedure TOrderMemo.BB2Click(Sender: TObject);
begin
edit1.Text:='0';
with QTW do
  begin
    active:=false;
    SQL.Clear;
    sql.Add('select * from LY_ERP.dbo.DDBZZL l');
    sql.add('where not exists(select DDBH from LY_SHOE.dbo.DDBZZL d ');
    sql.add('where l.DDBH=d.DDBH collate Chinese_PRC_CI_AI_WS  ');
    sql.add('AND l.ZYLB=d.ZYLB collate Chinese_PRC_CI_AI_WS ');
