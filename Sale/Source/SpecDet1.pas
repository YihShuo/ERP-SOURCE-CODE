...
procedure TSpecDet.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TSpecDet.FormCreate(Sender: TObject);
begin
DTP1.Date:=date;
end;

procedure TSpecDet.BB4Click(Sender: TObject);
begin
close;
end;

procedure TSpecDet.BB1Click(Sender: TObject);
begin
edit1.Text:='1';
with QTW do
  begin
    active:=false;
    SQL.Clear;
    sql.Add('select * from LY_SHOE.dbo.zlzls l');
...
procedure TSpecDet.QVNAfterOpen(DataSet: TDataSet);
begin
...
procedure TSpecDet.BD1Click(Sender: TObject);
var i:integer;
begin
...
procedure TSpecDet.BD4Click(Sender: TObject);
begin
QVN.first;
...
procedure TSpecDet.BD3Click(Sender: TObject);
begin
...
procedure TSpecDet.BD5Click(Sender: TObject);
begin
QVN.Active:=false;
QVn.CachedUpdates:=false;
QVN.RequestLive:=false;
...
procedure TSpecDet.BD2Click(Sender: TObject);
var i:integer;
begin
QVN.CachedUpdates:=true;
QVN.RequestLive:=true;
...
procedure TSpecDet.BB2Click(Sender: TObject);
begin
edit1.Text:='0';
with QTW do
  begin
    active:=false;
    SQL.Clear;
    sql.Add('select * from LY_ERP.dbo.zlzls l');
    sql.add('where not exists(select zlbh from LY_SHOE.dbo.zlzls d ');
    sql.add('where l.zlbh=d.zlbh collate Chinese_PRC_CI_AI_WS  ');
    sql.add('and l.zlcc=d.zlcc collate Chinese_PRC_CI_AI_WS)  ');
    sql.add('order by l.userdate')  ;
    active:=true;
  end;
QVN.Active:=true;
