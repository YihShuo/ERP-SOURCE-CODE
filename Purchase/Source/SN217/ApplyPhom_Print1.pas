...
var
  ApplyPhom_Print: TApplyPhom_Print;

implementation
  uses ApplyPhom1, FunUnit, main1;
{$R *.dfm}
procedure TApplyPhom_Print.Readini();
var MyIni :Tinifile;
...
procedure TApplyPhom_Print.FormCreate(Sender: TObject);
begin
  AppDir:=ExtractFilePath(Application.ExeName);
  Readini();
  //簽核系統審核
  ...
  begin
    ...
    SQL.Add('      WHERE FORM_TABLE = ''SGDH'' and FLOW_DESC=''模具請購單''  AND STATUS IN (''N'', ''Z'') AND FORM_PRESENTATION LIKE ''%'+ApplyPhom.SGMas.FieldByName('SGNO').AsString+'%''  ');
    ...
    begin
      ...
      SQL.Add('      WHERE ACTUAL_SIGNER IS NOT NULL AND LYN_FormSigner.TNO=''''' + ApplyPhom.SGMas.FieldByName('SGNO').AsString + ''''''') AS OP  ');
    ...
    begin        
      ...
      CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Images\'+ApplyPhom.SGMas.FieldByName('CFMID1').AsString+'.bmp'),Pchar(AppDir+'Images\'+ApplyPhom.SGMas.FieldByName('CFMID1').AsString+'.bmp'),false);
      if FileExists(AppDir+'Images\'+ApplyPhom.SGMas.FieldByName('CFMID1').AsString+'.bmp') then
      begin
         TopManger.Picture.LoadFromFile(AppDir+'Images\'+ApplyPhom.SGMas.FieldByName('CFMID1').AsString+'.bmp');
      end;
...
procedure TApplyPhom_Print.FormDestroy(Sender: TObject);
begin
  ApplyPhom_Print:=nil;
end;

procedure TApplyPhom_Print.FormClose(Sender: TObject;
