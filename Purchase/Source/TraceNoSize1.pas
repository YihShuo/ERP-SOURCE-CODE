...
procedure TTraceNoSize.FormClose(Sender: TObject;
...
procedure TTraceNoSize.BB1Click(Sender: TObject);
begin
panel2.Visible:=true;
end;

procedure TTraceNoSize.Button1Click(Sender: TObject);
begin
  with CGMas do
  begin
    active:=false;
    ...
    sql.add('and (CGZL.CGLX=''1'' or CGZL.CGLX=''3'' or CGZL.CGLX=''4'') ');
...
procedure TTraceNoSize.FormCreate(Sender: TObject);
begin
  with tmpQuery do
  begin
    active:=false;
...
procedure TTraceNoSize.BB4Click(Sender: TObject);
begin
PC1.ActivePage:=TS4;
with CGDet do
  begin
...
procedure TTraceNoSize.BB5Click(Sender: TObject);
var i:integer;
    isUpdate:boolean;
begin
  try
...
procedure TTraceNoSize.BB6Click(Sender: TObject);
begin
with CGDet do
  begin
    active:=false;
...
procedure TTraceNoSize.BBT1Click(Sender: TObject);
begin
CGDet.first;
end;

procedure TTraceNoSize.BBT2Click(Sender: TObject);
begin
CGDet.prior;
end;

procedure TTraceNoSize.BBT3Click(Sender: TObject);
begin
CGDet.next;
end;

procedure TTraceNoSize.BBT4Click(Sender: TObject);
begin
CGDet.last;
end;

procedure TTraceNoSize.BB7Click(Sender: TObject);
begin
close;
end;

procedure TTraceNoSize.CGDetAfterOpen(DataSet: TDataSet);
begin
  BB4.Enabled:=true;
  BBT1.Enabled:=true;
  BBT2.Enabled:=true;
...
procedure TTraceNoSize.Edit1KeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
    edit2.SetFocus;
end;

procedure TTraceNoSize.Edit2KeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
    edit3.SetFocus;
end;

procedure TTraceNoSize.Edit3KeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
    button1click(nil);
end;

procedure TTraceNoSize.BO3Click(Sender: TObject);
var i:integer;
    MDate:TDate;
begin
  try
...
procedure TTraceNoSize.BO4Click(Sender: TObject);
begin
  with CGs do
  begin
    active:=false;
...
procedure TTraceNoSize.BO5Click(Sender: TObject);
begin
...
procedure TTraceNoSize.FormDestroy(Sender: TObject);
begin
  TraceNoSize:=nil;
end;

procedure TTraceNoSize.DBGridEh1DblClick(Sender: TObject);
begin
...
procedure TTraceNoSize.DBGridEh1KeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
    DBGridEh1DblClick(nil);
end;

procedure TTraceNoSize.ImportETDBtnClick(Sender: TObject);
...
procedure TTraceNoSize.BO6Click(Sender: TObject);
var   a:string;
...
procedure TTraceNoSize.ImportETCBtnClick(Sender: TObject);
var OrderListExcel:Variant;
...
begin
    ...
    try
      ...
      begin
        ...
        CGNO_index:=-1;
        //開啟Excel
        OrderExcFN:=OpenDialog1.FileName;//Excel檔案名稱 備份複製用
