...
type
  TPurPham_Replenish = class(TForm)
    Panel5: TPanel;
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
...
var
  PurPham_Replenish: TPurPham_Replenish;
  NDate:TDate;
  FY,FM:string;
...
procedure TPurPham_Replenish.FormCreate(Sender: TObject);
var i: integer;
begin
  with Query1 do
  begin
...
procedure TPurPham_Replenish.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TPurPham_Replenish.FormDestroy(Sender: TObject);
begin
  PurPham_Replenish:=nil;
end;

procedure TPurPham_Replenish.Button5Click(Sender: TObject);
begin
  with SGDH do
  begin
    Active:=false;
  ...
  begin
     Active:=false;
     SQL.Clear;
     SQL.Add('select SGDHSS.SGNO,SGDHSS.CLBH,SGDHSS.XXCC,SGDHSS.Qty,SGDHSS.okQty,SGDHSS.Diff,');
     SQL.Add('       CLZL.YWPM,CLZL.ZWPM,SGDHSS.DepName,ZSZL.ZSDH,ZSZL.zsywjc,SGDHSS.LB,SGDHSS.CFMID,SGDHSS.CFMDate,SGDHSS.CFMID1,SGDHSS.CFMDate1,SGDHSS.GSBH,SGDHSS.USERID');
     SQL.Add('from (');
     SQL.Add('select SGDHSS.SGNO,SGDHSS.CLBH,SGDHSS.XXCC,SGDHSS.Qty,IsNull(CGZLSS.okQty,0) as okQty,isnull(SGDHSS.Qty,0)-isnull(CGZLSS.okQty,0) as Diff,');
     SQL.Add('       BDepartment.DepName,SGDH.LB,SGDH.CFMID,SGDH.CFMDate,SGDH.CFMID1,SGDH.CFMDate1,SGDH.GSBH,SGDH.USERID,(select distinct BJNO from SGDHS where SGNO = '''+SGNOEdit.Text+''') as BJNO');
     SQL.Add('from SGDHSS ');
     SQL.Add('left join SGDH on SGDH.SGNO=SGDHSS.SGNO  ');
     SQL.Add('left join (select ZLBH,CLBH,Sum(Qty) as okQty   ');
     SQL.Add('  		     from CGZLSS where 1=1 and CGZLSS.ZLBH like '''+SGNOEdit.Text+'%'+'''  Group by ZLBH,CLBH)   ');
     SQL.Add('           CGZLSS on CGZLSS.CLBH=SGDHSS.CLBH and CGZLSS.ZLBH=SGDHSS.SGNO  ');
     SQL.Add('left join BDepartment on BDepartment.ID=SGDH.DepID  ');
     SQL.add('where 1=1 and  SGDHSS.CLBH=:CLBH');
     SQL.Add('and convert(smalldatetime,convert(varchar,SGDH.SGDate,111)) between ');
     SQL.add(''''+Formatdatetime('yyyy/MM/dd',DTP1.Date) +'''');
     SQL.add(' and ');
     SQL.add(''''+Formatdatetime('yyyy/MM/dd',DTP2.Date) +'''');
     ...
     //funcobj.WriteErrorLog(sql.Text);
     SQL.Add(') SGDHSS');
     SQL.Add('left join CLZL on CLZL.CLDH = SGDHSS.CLBH  ');
     SQL.Add('Left join CGBJ on CGBJ.BJNO=SGDHSS.BJNO  ');
     SQL.Add('left join ZSZL on ZSZL.ZSDH = CGBJ.ZSBH  ');
     //showmessage(SQL.Text);
     Active:=true;
  end;
...
procedure TPurPham_Replenish.PO1Click(Sender: TObject);
var bm:Tbookmark;
                 ...
                 begin
                   ...
                   begin
                     ...
                     begin
                       ...
                       sql.Add(','''+SGDHS.fieldbyname('XXCC').AsString+'''');
                       sql.Add(',''Pham'' ');
                       sql.Add(',GetDate()');
...
procedure TPurPham_Replenish.M1Click(Sender: TObject);
begin
  E1.Enabled:=false;
  M1.Enabled:=false;
  PO1.Enabled:=true;
...
function TPurPham_Replenish.CreateCGBJ(ZSBH:string;CLBH:String;CQDH:String):string;
var BJNO:string;
begin
  //20151022先檢查是否已存在廠商
  BJNO:='';
...
procedure TPurPham_Replenish.E3Click(Sender: TObject);
var   a:string;
...
procedure TPurPham_Replenish.Excel1Click(Sender: TObject);
var   a:string;
      eclApp,WorkBook:olevariant;
 //     xlsFileName:string;
      i,j:integer;
...
procedure TPurPham_Replenish.DBGridEh1GetCellParams(Sender: TObject;
...
procedure TPurPham_Replenish.Reject1Click(Sender: TObject);
var bm:Tbookmark;
    CGNO,BJNO,Supp_ID:string;
    USPrice,VNPrice:string;
    QUSPrice,QVNPrice:string;
...
procedure TPurPham_Replenish.Reject2Click(Sender: TObject);
begin
//整張退單
