...
type
  TPurOtherSizeTW_Print = class(TForm)
    QuickRep1: TQuickRep;
    TempSize: TQuery;
    MasSize: TQuery;
...
var
  PurOtherSizeTW_Print: TPurOtherSizeTW_Print;

implementation

uses PurOther1,FunUnit, main1;

{$R *.dfm}

procedure TPurOtherSizeTW_Print.ReadIni();
var MyIni :Tinifile;
...
begin
  ...
  if FileExists(AppDir+'\Images\'+main.Edit1.Text+'.bmp')=false then
      CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Images\'+main.Edit1.Text+'.bmp'),Pchar(AppDir+'Images\'+main.Edit1.Text+'.bmp'),false);
  if FileExists(AppDir+'\Images\'+SuperVisorID)=false then
      CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Images\'+SuperVisorID),Pchar(AppDir+'Images\'+SuperVisorID),false);
...
procedure TPurOtherSizeTW_Print.FormCreate(Sender: TObject);
begin
  SeasonQ.Active:=true;
  L9.Caption:='BuyNo:'+SeasonQ.FieldByName('BUYNO').AsString;
  L10.Caption:='Brand:'+SeasonQ.FieldByName('KFJC').AsString;
  SeasonQ.Active:=false;
  ...
  begin
    ...
    sql.add('select CGZLSS.CLBH,DDZL.DDBH as ZLBH,CGZLSS.XXCC,DDZL.Shipdate,');
    sql.add('       convert(decimal(18,1),CGZLSS.Qty) as Qty ');
    sql.add('       ,DDZL.Article,XXZL.XieMing,XXZL.YSSM');
    sql.add('       into #CGQty');
    sql.add('from CGZLSS ');
    sql.add('left join SCBL on SCBL.BLNO=CGZLSS.ZLBH ');
    sql.add('left join DDZL on DDZL.DDBH=SCBL.ZLBH ');
    sql.add('left join XXZL on DDZL.XieXing=XXZL.XieXing and DDZL.SheHao=XXZL.SheHao ');
    sql.add('where CGZLSS.CGNO='+''''+PurOther.CGMas.fieldbyname('CGNO').Value+'''');
    sql.add('order by CGZLSS.CLBH,DDZL.DDBH,CGZLSS.XXCC ');

    sql.add('select #CGQty.CLBH,#CGQty.ZLBH,substring(convert(nvarchar,#TotQty.YQDate,111),6,10) as YQDate,');
    sql.add('#TotQty.TotQty,substring(convert(nvarchar,#CGQty.ShipDate,111),6,10) as ShipDate,#CGQty.Article,#CGQty.XieMing,#CGQty.YSSM');
    ...
    sql.add('group by #CGQty.CLBH,#CGQty.ZLBH,#TotQty.YQDate,#TotQty.TotQty,#CGQty.ShipDate,#CGQty.Article,#CGQty.XieMing,#CGQty.YSSM');
...
procedure TPurOtherSizeTW_Print.FormDestroy(Sender: TObject);
begin
PurOtherSizeTW_Print:=nil;
end;

procedure TPurOtherSizeTW_Print.FormClose(Sender: TObject;
