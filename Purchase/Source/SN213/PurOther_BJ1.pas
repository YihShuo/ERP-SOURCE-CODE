...
uses PurOther1, main1,FunUnit, Exchange_PurSize1;

{$R *.dfm}

procedure TPurOther_BJ.ReadIni();
var MyIni :Tinifile;
    AppDir:string;
begin
  //
...
begin
  ...
  begin
    ...
    sql.Add('where CGBJS.CLBH='+''''+PurOther.CGDet.fieldbyname('CLBH').Value+''''+'');
...
procedure TPurOther_BJ.DBGrid1DblClick(Sender: TObject);
var i,CWHL:integer;
begin
...
procedure TPurOther_BJ.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
if key=#13 then
  DBGrid1dblclick(nil);
end;
...
procedure TPurOther_BJ.DBGrid1EditButtonClick(Sender: TObject);
begin
  if (DBGrid1.SelectedField.FieldName='CWHL') then
  begin
    Exchange_PurSize:=TExchange_PurSize.Create(self);
    Exchange_PurSize.Button1.Click;
    Exchange_PurSize.ShowModal();
    Exchange_PurSize.Free;
  end;
end;

procedure TPurOther_BJ.Query1BeforeScroll(DataSet: TDataSet);
begin
  if IsAllowStroll=false then abort;
end;

procedure TPurOther_BJ.FormCreate(Sender: TObject);
begin
  ReadIni();
end;

procedure TPurOther_BJ.FormShow(Sender: TObject);
begin
