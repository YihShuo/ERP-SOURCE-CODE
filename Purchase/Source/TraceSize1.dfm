object TraceSize: TTraceSize
  ...
  object CGS: TQuery
    ...
    DataSource = DS4
    SQL.Strings = (
      'select CGNO,CLBH,CGZLSS.ZLBH,max(YQDate) as YQDate,'
      
        '     max(CFMDate) as CFMDate,sum(qty) as Qty,DDZL.Article,max (s' +
        'czldate.Adate) as Adate,bdepartment.depname,max(MaterialETC) as ' +
        'MaterialETC'
      'from CGZLSS inner join DDZL on CGZLSS.ZLBH=DDZL.DDBH'
      'left join sczldate on sczldate.zlbh = CGZLSS.ZLBH'
      'left join bdepartment on bdepartment.id = sczldate.assemble'
      'where CGNO=:CGNO'
      'and CLBH=:CLBH'
      'group by CGNO,CLBH,CGZLSS.ZLBH,DDZL.Article,bdepartment.depname'
    ...
    object CGSAdate: TDateTimeField
      FieldName = 'Adate'
    end
    object CGSdepname: TStringField
      FieldName = 'depname'
      FixedChar = True
      Size = 50
    end
    object CGSMaterialETC: TDateTimeField
      FieldName = 'MaterialETC'
    end
  end
  object CGDet: TQuery
  ...
  object UPDet: TUpdateSQL
    ...
    InsertSQL.Strings = (
      'insert into CGZLS'
      '  (YQDate,CFMDate, USERDate, USERID)'
      'values'
      '  (:YQDate,:CFMDate, :USERDate, :USERID)')
    DeleteSQL.Strings = (
      'delete from CGZLS'
