unit SupplierReviewDevelop_MonthlyReport1;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, DB, DBTables, QRCtrls, StdCtrls, QuickRpt, ExtCtrls;

type
  TSupplierReviewDevelop_MonthlyReport = class(TForm)
    QSignature: TQuery;
    QuickRep1: TQuickRep;
    QRBand1: TQRBand;
    QRLabel1: TQRLabel;
...
var
  SupplierReviewDevelop_MonthlyReport: TSupplierReviewDevelop_MonthlyReport;
...
procedure TSupplierReviewDevelop_MonthlyReport.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TSupplierReviewDevelop_MonthlyReport.FormDestroy(
  Sender: TObject);
begin
  SupplierReviewDevelop_MonthlyReport:=nil;
end;

procedure TSupplierReviewDevelop_MonthlyReport.LoadData;
var
  QRShape: TQRShape;
  QRLabel: TQRLabel;
...
begin
  QRMonth.Caption := 'Thang ' + IntToStr(SupplierReviewDevelop.QSupplier.FieldByName('Month').AsInteger) + ' 月 / nam ' + IntToStr(SupplierReviewDevelop.QSupplier.FieldByName('Year').AsInteger) + ' 年';
  QRListNo.Caption := SupplierReviewDevelop.QSupplier.FieldByName('LNO').AsString;
  ...
  begin
    ...
    SQL.Add('    WHERE FORM_TABLE = ''WF_SupplierReview_Dev'' AND FLOW_DESC = ''供應商品質與交期統計表'' AND FORM_PRESENTATION LIKE ''%' + SupplierReviewDevelop.QSupplier.FieldByName('LNO').AsString + '%''');
    ...
    SQL.Add('  WHERE LNO = ''' + SupplierReviewDevelop.QSupplier.FieldByName('LNO').AsString + '''');
    ...
    SQL.Add('LEFT JOIN EEP.LingYi.dbo.GROUPS AS GROUPS ON GROUPS.GROUPID = SYS_TODOHIS.S_ROLE_ID');
    SQL.Add('WHERE SYS_TODOHIS.FORM_TABLE = ''WF_SupplierReview_Dev'' AND FLOW_DESC = ''供應商品質與交期統計表'' AND SYS_TODOHIS.FORM_PRESENTATION LIKE ''%' + SupplierReviewDevelop.QSupplier.FieldByName('LNO').AsString + '%''');
    ...
    begin
      ...
      SQL.Add('       LEFT JOIN [UOF].[dbo].[LYN_FormSigner] LYN_FormSigner on LYN_FormSigner.TableName=''''WF_SupplierReview_Dev'''' and LYN_FormSigner.Type=''''Monthly'''' and LYN_FormSigner.FNO=TB_WKF_TASK.DOC_NBR ');
      ...
      SQL.Add('       WHERE ACTUAL_SIGNER IS NOT NULL AND LYN_FormSigner.TNO = ''''' + SupplierReviewDevelop.QSupplier.FieldByName('LNO').AsString + ''''' ');
