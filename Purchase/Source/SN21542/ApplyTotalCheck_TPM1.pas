...
type
  TApplyTotalCheck_TPM = class(TForm)
    DBGrid3: TDBGridEh;
    SGMas: TQuery;
...
var
  ApplyTotalCheck_TPM: TApplyTotalCheck_TPM;
  NDate:TDate;
...
procedure TApplyTotalCheck_TPM.ReadIni();
  var MyIni :Tinifile;
      AppDir:string;
begin
  //
...
procedure TApplyTotalCheck_TPM.Button1Click(Sender: TObject);
begin
  with SGMas do
  begin
    Active:=false;
    ...
    SQL.Add('where LB  in (''04'')  and flowflag=''Z'' and convert(smalldatetime,convert(varchar,SGDH.USERDATE,111)) between ');
...
procedure TApplyTotalCheck_TPM.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=CaFree;
end;

procedure TApplyTotalCheck_TPM.FormDestroy(Sender: TObject);
begin
  ApplyTotalCheck_TPM:=nil;
end;

procedure TApplyTotalCheck_TPM.FormCreate(Sender: TObject);
begin
  AppDir:=ExtractFilePath(Application.ExeName);
  with query1 do
  begin
...
procedure TApplyTotalCheck_TPM.Button3Click(Sender: TObject);
begin
...
procedure TApplyTotalCheck_TPM.Button2Click(Sender: TObject);
var SGNO:String;
begin
  if SGMas.Active=false then Exit;
  if SGMas.RecordCount=0 then Exit;
...
procedure TApplyTotalCheck_TPM.Button4Click(Sender: TObject);
begin
...
procedure TApplyTotalCheck_TPM.SGDetCalcFields(DataSet: TDataSet);
begin
  with SGDet do
  begin
...
procedure TApplyTotalCheck_TPM.Button5Click(Sender: TObject);
begin
  if SGMas.Active=false then Exit;
  if SGMas.RecordCount=0 then Exit;
  if SGDet.Active=false then Exit;
