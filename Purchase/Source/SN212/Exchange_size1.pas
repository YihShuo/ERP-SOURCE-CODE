...
{$R *.dfm}
procedure TExchange_Size.ReadIni();
var MyIni :Tinifile;
    AppDir:string;
begin
  //
...
procedure TExchange_Size.FormCreate(Sender: TObject);
...
procedure TExchange_Size.FormDestroy(Sender: TObject);
begin
   Exchange_Size:=nil;
end;

procedure TExchange_Size.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TExchange_Size.Button1Click(Sender: TObject);
begin
 with query1 do
  begin
    active:=false;
...
procedure TExchange_Size.DBGridEh1DblClick(Sender: TObject);
begin
if query1.RecordCount>0 then
  begin
    with PurSize_BJ.Query1  do
    begin
       edit;
