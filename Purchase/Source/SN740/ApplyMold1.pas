...
var
  ApplyMold: TApplyMold;
  NDate:TDate;

implementation

uses main1, ApplyMoldDepName1, ApplyMoldMatNo1, ApplyMold_Print1, ApplyPurMold_BJ1,
  ApplyTotalCapex1;

{$R *.dfm}

procedure TApplyMold.ReadIni();
var MyIni :Tinifile;
    AppDir:string;
begin
  //
...
procedure TApplyMold.Button1Click(Sender: TObject);
begin
  with SGMas do
  begin
    Active:=false;
    ...
    if main.Edit2.Text = 'VA12' then
      SQL.Add('and SGDH.LB in (''05'',''09'')')
    else
      SQL.Add('and SGDH.LB=''09''');
...
procedure TApplyMold.SGMasAfterOpen(DataSet: TDataSet);
begin
  BB2.Enabled:=true;
  BB3.Enabled:=true;
  BB4.Enabled:=true;
...
procedure TApplyMold.SGDetAfterOpen(DataSet: TDataSet);
begin
  BD2.Enabled:=true;
  BD3.Enabled:=true;
  BD4.Enabled:=true;
...
procedure TApplyMold.FormCreate(Sender: TObject);
var i:integer;
begin
  AppDir:=ExtractFilePath(Application.ExeName);
  with Query1 do
...
procedure TApplyMold.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=CaFree;
end;

procedure TApplyMold.FormDestroy(Sender: TObject);
begin
  ApplyMold:=nil;
end;

procedure TApplyMold.BB1Click(Sender: TObject);
begin
  panel2.Visible:=true;
end;

procedure TApplyMold.BBTT1Click(Sender: TObject);
begin
  ApplyMoldDepName:=TApplyMoldDepName.Create(self);
  ApplyMoldDepName.Show();
end;

procedure TApplyMold.BB2Click(Sender: TObject);
begin
  with SGMas do
  begin
    requestlive:=true;
...
procedure TApplyMold.BB3Click(Sender: TObject);
begin
...
procedure TApplyMold.BB4Click(Sender: TObject);
begin
...
procedure TApplyMold.BB5Click(Sender: TObject);
var y,m,SGNO:string;
    i:integer;
begin
...
procedure TApplyMold.BB6Click(Sender: TObject);
begin
  with SGMas do
  begin
    active:=false;
...
procedure TApplyMold.BD2Click(Sender: TObject);
begin
  if (SGMas.FieldByName('Flowflag').AsString<>'X') and (trim(SGMas.FieldByName('Flowflag').AsString)<>'') then
  begin
    Showmessage('Da gui trinh ky, khong the sua !');
...
procedure TApplyMold.BD3Click(Sender: TObject);
begin
  if (SGMas.FieldByName('Flowflag').AsString<>'X') and (trim(SGMas.FieldByName('Flowflag').AsString)<>'') then
  begin
    Showmessage('Da gui trinh ky, khong the sua !');
...
procedure TApplyMold.BD4Click(Sender: TObject);
begin
  if SGMas.FieldByName('flowflag').Value = 'X' then begin
    showmessage('This document has been canceled');
    exit;
...
procedure TApplyMold.BD5Click(Sender: TObject);
var i:integer;
begin
  if (SGMas.FieldByName('CFMID').value<>'NO')   then
  begin
...
procedure TApplyMold.PC1Change(Sender: TObject);
...
procedure TApplyMold.BBTT2Click(Sender: TObject);
begin
  if SGDet.State in [DsInsert,DsEdit] then begin
    ApplyMoldMatNo:=TApplyMoldMatNo.Create(self);
    ApplyMoldMatNo.Show();
  end;
end;

procedure TApplyMold.BD6Click(Sender: TObject);
begin
  with SGDet do
  begin
    active:=false;
...
procedure TApplyMold.bbt6Click(Sender: TObject);
...
procedure TApplyMold.BTD7Click(Sender: TObject);
var eclApp,WorkBook:olevariant;
    i,j:integer;
begin
...
procedure TApplyMold.DBGrid3DrawColumnCell(Sender: TObject;
...
procedure TApplyMold.dbgrid4DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumnEh;
  State: TGridDrawState);
begin
  if SGDet.FieldByName('YN').value='0' then
...
procedure TApplyMold.BDT5Click(Sender: TObject);
begin
  ...
  ApplyMold_Print:=TApplyMold_Print.Create(self);
  ApplyMold_Print.quickrep2.prepare;
  ApplyMold_Print.quickrep2.preview;
  ApplyMold_Print.free;
end;


procedure TApplyMold.SGDetCalcFields(DataSet: TDataSet);
begin
  with SGDet do
  begin
...
procedure TApplyMold.DBGrid3DblClick(Sender: TObject);
begin
  if  SGMas.Active then
  begin
...
procedure TApplyMold.dbgrid4EditButtonClick(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      ApplyPurMold_BJ:=TApplyPurMold_BJ.create(self);
      ApplyPurMold_BJ.Edit1.Text:=SGDet.fieldbyname('CLBH').Value;
      ApplyPurMold_BJ.show;
    end;
  end;
end;

procedure TApplyMold.BO7Click(Sender: TObject);
begin
...
procedure TApplyMold.BO2Click(Sender: TObject);
begin
  if SGDet.RequestLive then
  begin
    showmessage('Pls save material data first.');
...
procedure TApplyMold.BO3Click(Sender: TObject);
begin
  if SGDet.RequestLive then
  begin
    showmessage('Pls save material data first.');
...
procedure TApplyMold.BO5Click(Sender: TObject);
begin
  with SGDetS do
  begin
    active:=false;
...
procedure TApplyMold.BO4Click(Sender: TObject);
var i:integer;
    Qty:real;
    Memo:String;
begin
...
procedure TApplyMold.BO1Click(Sender: TObject);
begin
  if SGDet.RequestLive then
  begin
    showmessage('Pls save material data first.');
...
procedure TApplyMold.DBGrid5DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumnEh;
  State: TGridDrawState);
begin
  if SGDetS.FieldByName('YN').value='0' then
...
procedure TApplyMold.SGMasAfterScroll(DataSet: TDataSet);
begin
...
procedure TApplyMold.Button3Click(Sender: TObject);
begin
  if SGDet.State in [DsInsert,DsEdit] then begin
    ApplyTotalCapex:=TApplyTotalCapex.Create(self);
    ApplyTotalCapex1.Formname := 'ApplyMold';
