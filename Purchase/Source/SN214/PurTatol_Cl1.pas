...
var
  PurTatol_CL: TPurTatol_CL;

implementation

uses PurTotal1, MaterialNew1, main1;

{$R *.dfm}

procedure TPurTatol_CL.EDIT1KeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TPurTatol_CL.Edit2KeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TPurTatol_CL.Edit3KeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TPurTatol_CL.Edit4KeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TPurTatol_CL.Button1Click(Sender: TObject);
begin
...
with query1 do
  begin
    active:=false;
    sql.Clear;
    sql.add('select CLDH,CLLB,YWPM,ZWPM,DWBH from CLZL where LEFT(CLDH,1) IN (''X'',''Y'',''Z'',''W'')');
    SQL.Add('AND CLDH LIKE');
    sql.add(''''+edit1.Text+'%'+'''');
...
procedure TPurTatol_CL.Button2Click(Sender: TObject);
begin
...
procedure TPurTatol_CL.FormDestroy(Sender: TObject);
begin
  PurTatol_CL:=nil;
end;

procedure TPurTatol_CL.FormClose(Sender: TObject;
...
procedure TPurTatol_CL.DBGridEh1DblClick(Sender: TObject);
begin
...
procedure TPurTatol_CL.DBGridEh1KeyPress(Sender: TObject; var Key: Char);
begin
   if key=#13 then
   DBGridEh1DblClick(nil);
end;

procedure TPurTatol_CL.FormShow(Sender: TObject);
begin
