...
type
  TPurSampleInvoice_ZS = class(TForm)
    Panel1: TPanel;
    Label1: TLabel;
    Label2: TLabel;
    Button1: TButton;
...
var
  PurSampleInvoice_ZS: TPurSampleInvoice_ZS;

implementation

uses  PurSampleInvoice1, main1, FunUnit;

{$R *.dfm}

procedure TPurSampleInvoice_ZS.Button1Click(Sender: TObject);
begin
  ...
  begin
    active:=false;
    sql.Clear;
    sql.add('select ZSZL.ZSDH,ZSYWJC,ZSQM,BUSERS.USERNAME as SamplePurchaser_Name from ZSZL ');
...
procedure TPurSampleInvoice_ZS.Edit1KeyPress(Sender: TObject; var Key: Char);
begin
if key=#13 then
  edit2.setfocus
...
procedure TPurSampleInvoice_ZS.Edit2KeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
  button1click(nil);
end;

procedure TPurSampleInvoice_ZS.FormDestroy(Sender: TObject);
begin
  PurSampleInvoice_ZS:=nil;
end;

procedure TPurSampleInvoice_ZS.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TPurSampleInvoice_ZS.DBGridEh1DblClick(Sender: TObject);
begin
  if query1.recordcount>0 then
  begin
    with PurSampleInvoice.CGMas do
    begin
      edit;
...
procedure TPurSampleInvoice_ZS.DBGridEh1KeyPress(Sender: TObject;
  var Key: Char);
begin
  if key=#13 then
