unit Apply_Purchase_Printing_Mold_Print1;
...
type
  TApply_Purchase_Printing_Mold_Print = class(TForm)
    QuickRep2: TQuickRep;
...
var
  Apply_Purchase_Printing_Mold_Print: TApply_Purchase_Printing_Mold_Print;

implementation

uses  main1, Apply_Purchase_Printing_Mold1, ApplyMold1;

{$R *.dfm}
procedure TApply_Purchase_Printing_Mold_Print.Readini();
var MyIni :Tinifile;
...
procedure TApply_Purchase_Printing_Mold_Print.FormCreate(Sender: TObject);
begin
  AppDir:=ExtractFilePath(Application.ExeName);
  Readini();
  //簽核系統審核
  ...
  begin
    ...
    SQL.Add('      WHERE FORM_TABLE = ''SGDH'' and FLOW_DESC=''模具請購單''  AND STATUS IN (''N'', ''Z'') AND FORM_PRESENTATION LIKE ''%'+Apply_Purchase_Printing_Mold.SGMas.FieldByName('SGNO').AsString+'%''  ');
    ...
    begin
      ...
      SQL.Add('      WHERE ACTUAL_SIGNER IS NOT NULL AND LYN_FormSigner.TNO=''''' + Apply_Purchase_Printing_Mold.SGMas.FieldByName('SGNO').AsString + ''''''') AS OP  ');
    ...
    begin           
      ...
      CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Images\'+Apply_Purchase_Printing_Mold.SGMas.FieldByName('CFMID1').AsString+'.bmp'),Pchar(AppDir+'Images\'+Apply_Purchase_Printing_Mold.SGMas.FieldByName('CFMID1').AsString+'.bmp'),false);
      if FileExists(AppDir+'Images\'+Apply_Purchase_Printing_Mold.SGMas.FieldByName('CFMID1').AsString+'.bmp') then
      begin
         TopManger.Picture.LoadFromFile(AppDir+'Images\'+Apply_Purchase_Printing_Mold.SGMas.FieldByName('CFMID1').AsString+'.bmp');
      end;
  ...
  begin
    ...
    begin
      ...
      SQL.Add('Left join LBZLS on LBZLS.LBDH=CLZL.dwbh and LBZLS.LB=''06'' ');
