...
uses SupplierSample1 , TraceListSample1, Payment1,
  TraceList1;
...
procedure TTraceListSample_Style.DBGrid1DblClick(Sender: TObject);
begin
...
if query1.recordcount>0 then
  begin
    ...
    {
    //20141030 ¼Ö»õ¥Îµù¸Ñ
    if TraceListSample_Style.Label1.Caption='TraceList' then
       TraceList.Edit9.text:= query1.fieldbyname('Style').value ;
    if TraceListSample_Style.Label1.Caption='SampleMatTrack' then
       SampleMatTrack.Edit6.text:= query1.fieldbyname('Style').value ;
    if TraceListSample_Style.Label1.Caption='Payment' then
       Payment.Edit4.text:= query1.fieldbyname('Style').value ;
    }   
    query1.Active:=false;
    close;
  end;
...
procedure TTraceListSample_Style.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
if key=#13 then
  DBGrid1Dblclick(nil);
end;
