...
type
  TApplyTotal_Print = class(TForm)
    ...
    ManagerID: TQRImage;
    HardwareID: TQRImage;
    HardwareMID: TQRImage;
    SupvervisorID: TQRImage;
    QRLabel30: TQRLabel;
    QRLabel31: TQRLabel;
    TopDirector: TQRImage;
    ...
    QR_HardwareDate: TQRDBText;
    QR_HardwareMDate: TQRDBText;
    Query2: TQuery;
...
var
  ApplyTotal_Print: TApplyTotal_Print;

implementation

uses ApplyTotal1, FunUnit, main1;

{$R *.dfm}
procedure TApplyTotal_Print.Readini();
var MyIni :Tinifile;
...
procedure TApplyTotal_Print.FormCreate(Sender: TObject);
begin
  AppDir:=ExtractFilePath(Application.ExeName);
  Readini();
  ...
  begin
    ...
    SQL.Add('  MAX(CASE WHEN S_STEP_DESC = ''硬體審核'' THEN S_USER_ID END) AS HardwareID,  ');
    SQL.Add('  MAX(CASE WHEN S_STEP_DESC = ''硬體審核'' THEN USERNAME END) AS Hardware,  ');
    SQL.Add('  MAX(CASE WHEN S_STEP_DESC = ''硬體審核'' THEN ApproveDate END) AS HardwareDate,  ');
    SQL.Add('  MAX(CASE WHEN S_STEP_DESC = ''IT硬體主管'' THEN S_USER_ID END) AS HardwareMID,  ');
    SQL.Add('  MAX(CASE WHEN S_STEP_DESC = ''IT硬體主管'' THEN USERNAME END) AS HardwareM,  ');
    SQL.Add('  MAX(CASE WHEN S_STEP_DESC = ''IT硬體主管'' THEN ApproveDate END) AS HardwareMDate  ');
    SQL.Add('       FROM (  ');
    SQL.Add('      ');
    ...
    SQL.Add('      WHERE FORM_TABLE = ''SGDH'' AND STATUS IN (''N'', ''Z'') AND FORM_PRESENTATION LIKE ''%'+ApplyTotal.SGMas.FieldByName('SGNO').AsString+'%''  ');
    ...
    begin
      ...
      SQL.Add('  MAX(CASE WHEN SITE_CODE = ''IT'' THEN ACCOUNT END) AS HardwareID,  ');
      SQL.Add('  MAX(CASE WHEN SITE_CODE = ''IT'' THEN NAME END) AS Hardware,  ');
      SQL.Add('  MAX(CASE WHEN SITE_CODE = ''IT'' THEN FINISH_TIME END) AS HardwareDate,  ');
      SQL.Add('  MAX(CASE WHEN SITE_CODE = ''ITM'' THEN ACCOUNT END) AS HardwareMID,  ');
      SQL.Add('  MAX(CASE WHEN SITE_CODE = ''ITM'' THEN NAME END) AS HardwareM,  ');
      SQL.Add('  MAX(CASE WHEN SITE_CODE = ''ITM'' THEN FINISH_TIME END) AS HardwareMDate  ');
      ...
      SQL.Add('      LEFT JOIN [UOF].[dbo].[LYN_FormSigner] LYN_FormSigner on LYN_FormSigner.TableName=''''SGDH'''' and LYN_FormSigner.Type=''''Total'''' and LYN_FormSigner.FNO=TB_WKF_TASK.DOC_NBR ');
      ...
      SQL.Add('      WHERE ACTUAL_SIGNER IS NOT NULL AND LYN_FormSigner.TNO=''''' + ApplyTotal.SGMas.FieldByName('SGNO').AsString + ''''''') AS OP  ');
    ...
    begin
      ...
      if FieldByName('Hardware').AsString = '1' then lbIT.Caption:='代' else lbIT.Caption:='';
      if FieldByName('HardwareM').AsString = '1' then lbITM.Caption:='代' else lbITM.Caption:='';
      ...
      //
      CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Images\'+FieldByName('HardwareID').AsString+'.bmp'),Pchar(AppDir+'Images\'+FieldByName('HardwareID').AsString+'.bmp'),false);
      if FileExists(AppDir+'Images\'+FieldByName('HardwareID').AsString+'.bmp') then
      begin
        HardwareID.Picture.LoadFromFile(AppDir+'Images\'+FieldByName('HardwareID').AsString+'.bmp');
      end;
      //
      CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Images\'+FieldByName('HardwareMID').AsString+'.bmp'),Pchar(AppDir+'Images\'+FieldByName('HardwareMID').AsString+'.bmp'),false);
      if FileExists(AppDir+'Images\'+FieldByName('HardwareMID').AsString+'.bmp') then
      begin
        HardwareMID.Picture.LoadFromFile(AppDir+'Images\'+FieldByName('HardwareMID').AsString+'.bmp');
      end;
      if ApplyTotal.SGMas.FieldByName('CFMID1').AsString <> '' then
      begin
        CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Images\'+ApplyTotal.SGMas.FieldByName('CFMID1').AsString+'.bmp'),Pchar(AppDir+'Images\'+ApplyTotal.SGMas.FieldByName('CFMID1').AsString+'.bmp'),false);
        TopDirector.Picture.LoadFromFile(AppDir+'Images\'+ApplyTotal.SGMas.FieldByName('CFMID1').AsString+'.bmp');
      end
    end;
    //Active:=false;
  end;
  ...
  begin
    ...
    begin
      ...
      SQL.Add('where SGDHS.SGNO=:SGNO and SGDHS.Qty>0 ');
