...
procedure TCGNOConfirm.ReadIni();
  var MyIni :Tinifile;
      AppDir:string;
begin
  //
...
procedure TCGNOConfirm.Button1Click(Sender: TObject);
begin
  ...
  begin
    ...
    if CheckBox1.Checked = true then
      SQL.Add('      and IsNull(CGZL_DK_CFM.CFMID,''NO'')<>''NO'' ')
    else
      SQL.Add('      and IsNull(CGZL_DK_CFM.CFMID,''NO'')=''NO'' ');
...
procedure TCGNOConfirm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=CaFree;
end;

procedure TCGNOConfirm.FormDestroy(Sender: TObject);
begin
  CGNOConfirm:=nil;
end;

procedure TCGNOConfirm.FormCreate(Sender: TObject);
begin
  AppDir:=ExtractFilePath(Application.ExeName);
  with TmpQry do
  begin
...
procedure TCGNOConfirm.Button3Click(Sender: TObject);
begin
      ...
      begin
        ...
        begin
           ...
           begin
             ...
             SQL.Add('Begin');
             SQL.Add('  Insert into CGZL_DK_CFM (CGNO, CFMID, CFMDATE) Values ('''+CGMas.FieldByName('CGNO').AsString+''','''+main.Edit1.Text+''',GetDate())');
             SQL.Add('End Else');
             SQL.Add('Begin');
             SQL.Add('  Update CGZL_DK_CFM Set CFMID='''+main.Edit1.Text+''' ,CFMDate=GetDate() where CGNO='''+CGMas.FieldByName('CGNO').AsString+''' ');
             SQL.Add('End');
             ExecSQL();
           end;
        end;
...
procedure TCGNOConfirm.Button4Click(Sender: TObject);
begin

  if CGMas.fieldbyname('CFMID1').Value<>'NO' then
  begin
  ...
  try
    ...
    begin
      ...
      begin
         ...
         SQL.Add('IF NOT EXISTS (Select CGNO from CGZL_DK_CFM where CGNO='''+CGMas.FieldByName('CGNO').AsString+''')');
         SQL.Add('Begin');
         SQL.Add('  Insert into CGZL_DK_CFM (CGNO, CFMID, CFMDATE) Values ('''+CGMas.FieldByName('CGNO').AsString+''',''NO'',GetDate())');
         SQL.Add('End Else');
         SQL.Add('Begin');
         SQL.Add('  Update CGZL_DK_CFM Set CFMID=''NO'' ,CFMDate=GetDate() where CGNO='''+CGMas.FieldByName('CGNO').AsString+''' ');
         SQL.Add('End');
         ExecSQL();
      end;
      active:=false;
...
procedure TCGNOConfirm.Button5Click(Sender: TObject);
begin
  if CGMas.Active=false then Exit;
  if CGMas.RecordCount=0 then Exit;
  if CGDet.Active=false then Exit;
...
procedure TCGNOConfirm.CGDetCalcFields(DataSet: TDataSet);
begin
