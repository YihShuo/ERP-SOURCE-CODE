...
begin
  ...
  begin
    ...
    sql.add(''''+'%'+edit2.Text+'%'+'''');
    sql.add('and YN='+''''+'2'+'''');
    if cbStop.Text = 'Lock' then
      SQL.Add('and TYJH = ''Y'' ')
    else if cbStop.Text = 'NotLock' then
...
begin
  ...
  begin
    ...
    begin
      if MessageDlg('Ban co muon khoa NCU nay khong?',
        mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      begin
        CachedUpdates := true;
        Edit;
        FieldByName('TYJH').AsString := 'Y';
        Post;
        UpSQL1.Apply(ukModify);
        CachedUpdates := false;
        Active:=false;
...
begin
  ...
  begin
    ...
    if RecordCount > 0 then
    begin
      if MessageDlg('Ban co muon mo khoa NCU nay khong?',
        mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      begin
        CachedUpdates := true;
        Edit;
        FieldByName('TYJH').AsString := 'N';
        Post;
        UpSQL1.Apply(ukModify);
        CachedUpdates := false;
        Active:=false;
...
procedure TSuppNew.DBGrid1GetCellParams(Sender: TObject; Column: TColumnEh;
  AFont: TFont; var Background: TColor; State: TGridDrawState);
begin
