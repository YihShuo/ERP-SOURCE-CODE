...
procedure TCLBH_Weight.BB1Click(Sender: TObject);
begin
with Query1 do
  begin
    cachedupdates:=true;
...
procedure TCLBH_Weight.BB3Click(Sender: TObject);
begin
with Query1 do
  begin
    cachedupdates:=true;
...
procedure TCLBH_Weight.ImportExcelClick(Sender: TObject);
var
        ...
        try
          ...
          begin
            ...
            //Chua ton tai thi Insert moi
              ...
              SQL.Add('VALUES (''' + CLBHValue + ''', ' + WeightValue + ', ''' + main.Edit1.Text + ''', GETDATE(), 1)');
              ExecSQL;
            end;
          ...
          begin
            ...
            begin
              Close;
              SQL.Clear;
              SQL.Add('UPDATE CLBH_Weight SET Weight = ' + WeightValue + ', USERID = ''' + main.Edit1.Text + ''', USERDATE = GETDATE()');
...
procedure TCLBH_Weight.Button1Click(Sender: TObject);
begin
  ...
  with Query1 do
    begin
      active:=false;
      sql.Clear;
      SQL.Add('select * from CLBH_Weight where CLBH like ''%'+edCLBH.Text+'%''');
      SQL.Add('AND USERDATE BETWEEN ''' + FormatDateTime('yyyy-mm-dd', dtpFrom.Date) + ''' ');
      SQL.Add('AND ''' + FormatDateTime('yyyy-mm-dd', dtpTo.Date) + ''' ');
      if ckbMine.Checked = true then
      begin
...
procedure TCLBH_Weight.Query1AfterOpen(DataSet: TDataSet);
begin
  BB1.Enabled:=true;
  ///BB2.Enabled:=true;
  BB3.Enabled:=true;
...
procedure TCLBH_Weight.BB5Click(Sender: TObject);
begin
with Query1 do
  begin
    active:=false;
...
procedure TCLBH_Weight.BB4Click(Sender: TObject);
var i:integer;
begin
  try
    Query1.first;
...
procedure TCLBH_Weight.BB2Click(Sender: TObject);
begin
with Query1 do
  begin
    cachedupdates:=true;
...
procedure TCLBH_Weight.bbt6Click(Sender: TObject);
var   a:string;
...
procedure TCLBH_Weight.BB6Click(Sender: TObject);
begin
  Close;
end;

procedure TCLBH_Weight.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TCLBH_Weight.FormDestroy(Sender: TObject);
begin
  CLBH_Weight := nil;
end;

procedure TCLBH_Weight.DBGrid1GetCellParams(Sender: TObject; Column: TColumnEh;
  AFont: TFont; var Background: TColor; State: TGridDrawState);
begin
if Query1.FieldByName('YN').value='0' then
