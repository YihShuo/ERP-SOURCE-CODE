...
procedure TRYCheck_Det.Button1Click(Sender: TObject);
begin
  Close;
end;

procedure TRYCheck_Det.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  free;
end;

procedure TRYCheck_Det.Query1AfterOpen(DataSet: TDataSet);
var i:integer;
begin
...
procedure TRYCheck_Det.FormDestroy(Sender: TObject);
begin
RYCheck_Det:=nil;
end;

procedure TRYCheck_Det.DBGridEh1DrawColumnCell(Sender: TObject;
...
procedure TRYCheck_Det.FormCreate(Sender: TObject);
begin
  with query1 do
  begin
    active:=false;
    ...
    if QCDailyReport.label2.Caption = 'A' then
    ...
    sql.add(' and convert(smalldatetime,convert(varchar,SCBB.SCDATE)) between ');
    sql.add(''''+formatdatetime('yyyy/MM/dd',QCDailyReport.DTP1.Date)+''''+' and '+''''+formatdatetime('yyyy/MM/dd',QCDailyReport.DTP2.Date)+'''');
    sql.Add(' and SCBB.DepNo = '+''''+QCDailyReport.edit1.Text+'''');
