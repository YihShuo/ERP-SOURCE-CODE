...
begin
  ...
  //20140308舊版異動SMZL
  if (CK1.Checked or CK2.Checked) then
    SubSQL := SubSQL + ' scbbs.GXLB = ''A'' or ';
  if CK3.Checked then
    SubSQL := SubSQL + ' scbbs.GXLB = ''C'' or ';
  if CK4.Checked then
    SubSQL := SubSQL + ' scbbs.GXLB = ''S'' or ';
  if SubSQL <> '' then
...
begin
  ...
  //20140308舊版異動SMZL
  if (CK1.Checked or CK2.Checked) then
    SubSQL := SubSQL + ' SMDDSS.GXLB = ''A'' or ';
  if CK3.Checked then
    SubSQL := SubSQL + ' SMDDSS.GXLB = ''C'' or ';
  if CK4.Checked then
    SubSQL := SubSQL + ' SMDDSS.GXLB = ''S'' or ';
  if SubSQL <> '' then
    SubSQL := ' and ( ' + Copy(SubSQL, 1, length(SubSQL) - 4) + ')';
  result := SubSQL;
end;
...
begin
  if CK1.Checked then
    SubSQL := SubSQL + ' WOPR.GXLB = ''A'' or ';
  if CK2.Checked then
    SubSQL := SubSQL + ' WOPR.GXLB = ''AR'' or ';
  if CK3.Checked then
    SubSQL := SubSQL + ' WOPR.GXLB = ''C'' or ';
  if CK4.Checked then
    SubSQL := SubSQL + ' WOPR.GXLB = ''C+S'' or ';
  if SubSQL <> '' then
    SubSQL := ' and ( ' + Copy(SubSQL, 1, length(SubSQL) - 4) + ')';
  result := SubSQL;
end;
...
begin
  ...
  with Qtemp do
    begin
      ...
      //sql.add('             and SCBB.GSBH ='+''''+main.Edit2.Textmain.Edit2.Text+''''+'');
    ...
    begin
      ...
      if IsThisMonth = true then
        begin
          ...
          sql.add('     from (select scbb.DepNo,BDepartment.DepName,scbbs.GXLB,' + '''' + '1.Output' + '''' +
            ' as SB,SUM(scbbs.Qty) as qty,DATEPART(DAY,SCBB.SCDate) as NDay,''0''as Total');
          sql.add('	     from scbbs');
          sql.add('	     left join scbb on scbb.ProNo = scbbs.ProNo');
          sql.add('	     left join BDepartment on scbb.DepNo = BDepartment.ID');
          sql.add('      where  convert(smalldatetime,convert(varchar,SCBB.SCDate,111)) between ');
          sql.add('             ''' + formatdatetime('yyyy/MM/dd', sDate) + '''' + 'and ' + '''' +
            formatdatetime('yyyy/MM/dd', eDate - 1) + '''');
          ...
          sql.add('             ''' + formatdatetime('yyyy/MM/dd', eDate) + '''' + 'and ' + '''' +
            formatdatetime('yyyy/MM/dd', eDate) + '''');
          sql.Add('             and BDepartment.DepName like ' + '''' + '%' + edit1.Text + '%' + '''');
      ...
      sql.add('	from(     select WOPR.DepNo,BDepartment.DepName,Replace(WOPR.GXLB,''C+S'',''S'')as GXLB,' + '''' +
        '2.Total inspection' + '''' + ' as SB,SUM(WOPR.Qty)+SUM(WOPR.ReQty)+sum(WOPR.NGQty) as Qty');
      ...
      //
      sql.add('	     group by WOPR.DepNo,BDepartment.DepName,WOPR.GXLB,DATEPART(DAY,WOPR.USERDATE))T1');
      //20150908
      sql.Add('left join(select WOPR.DepNo,BDepartment.DepName,Replace(WOPR.GXLB,''C+S'',''S'')as GXLB,' + '''' +
        '2.Total inspection' + '''' + ' as SB,SUM(WOPR.Qty)+SUM(WOPR.ReQty)+sum(WOPR.NGQty)as Total');
      sql.add('	     from WOPR');
      sql.add('	     left join BDepartment on WOPR.DepNo = BDepartment.ID');
      sql.add('      where  convert(smalldatetime,convert(varchar,WOPR.USERDATE,111)) between ');
      sql.add('             ''' + formatdatetime('yyyy/MM/dd', sDate) + '''' + 'and ' + '''' +
      ...
      sql.Add('Select T1.DepNo,T1.DepName, T1.GXLB,T1.SB, T1.qty, T1.NDay,T2.ToTal');
      sql.add('from( select WOPR.DepNo,BDepartment.DepName,Replace(WOPR.GXLB,''C+S'',''S'')as GXLB,' + '''' +
        '3.Total defect' + '''' + ' as SB,SUM(WOPR.NGQty) as Qty,DATEPART(DAY,WOPR.USERDATE) as NDay');
      sql.add('	     from WOPR');
      sql.add('	     left join BDepartment on WOPR.DepNo = BDepartment.ID');
      sql.add('      where  convert(smalldatetime,convert(varchar,WOPR.USERDATE,111)) between ');
      sql.add('             ''' + formatdatetime('yyyy/MM/dd', sDate) + '''' + 'and ' + '''' +
      ...
      //20150908
      sql.Add('left join(select WOPR.DepNo,BDepartment.DepName,Replace(WOPR.GXLB,''C+S'',''S'')as GXLB,' + '''' +
        '3.Total defect' + '''' + ' as SB,SUM(WOPR.NGQty) as Total');
      sql.add('	         from WOPR');
      sql.add('	         left join BDepartment on WOPR.DepNo = BDepartment.ID');
      sql.add('          where  convert(smalldatetime,convert(varchar,WOPR.USERDATE,111)) between ');
      sql.add('             ''' + formatdatetime('yyyy/MM/dd', sDate) + '''' + 'and ' + '''' +
      ...
      sql.add('	     union all');
      sql.Add('Select T1.DepNo,T1.DepName, T1.GXLB,T1.SB, T1.qty, T1.NDay,T2.ToTal');
      sql.add('from( select WOPRB.DepNo,WOPRB.DepName,Replace(WOPRB.GXLB,''C+S'',''S'')as GXLB,' + '''' + '4.Pass%' +
        '''' + ' as SB,');
      sql.add('			 Round(100-((CAST(QCRDB.Qty AS Float)/NULLIF(WOPRB.Qty,0))*100),2) as Qty,WOPRB.NDay');
      sql.add('	     from(select WOPR.DepNo,BDepartment.DepName,WOPR.GXLB,SUM(WOPR.Qty)+SUM(WOPR.ReQty)+sum(WOPR.NGQty) as Qty');
      sql.add('           ,DATEPART(DAY,WOPR.USERDATE) as NDay');
      sql.add('		        from WOPR');
      sql.add('		        left join BDepartment on WOPR.DepNo = BDepartment.ID');
      sql.add('           where  convert(smalldatetime,convert(varchar,WOPR.USERDATE,111)) between ');
      ...
      sql.Add(Get_WOPR_SubSQL());
      //
      sql.add('				          group by WOPR.DepNo,WOPR.GXLB,DATEPART(DAY,WOPR.USERDATE)) as QCRDB on WOPRB.DepNo = QCRDB.DepNo and WOPRB.GXLB = QCRDB.GXLB and WOPRB.NDay = QCRDB.NDay)T1');
      //20150908
      sql.Add('left join(select WOPRB.DepNo,WOPRB.DepName,Replace(WOPRB.GXLB,''C+S'',''S'')as GXLB,' + '''' + '4.Pass%'
        + '''' + ' as SB,');
      ...
      sql.add('	     union all');
      sql.Add('Select T1.DepNo,T1.DepName, T1.GXLB,T1.SB, T1.qty, T1.NDay,T2.ToTal');
      sql.add('	from(  select ' + '''' + 'Z' + '''' + ' as DepNo,' + '''' + 'Z' + '''' + ' as DepName,'' '' as GXLB,' +
        '''' + '5.Daily inspection' + '''' + ' as SB');
      ...
      //20150908
      sql.Add('left join(select ' + '''' + 'Z' + '''' + ' as DepNo,' + '''' + 'Z' + '''' + ' as DepName,'' '' as GXLB,'
        + '''' + '5.Daily inspection' + '''' + ' as SB');
      sql.add('             ,SUM(WOPR.Qty)+SUM(WOPR.ReQty)+sum(WOPR.NGQty) as Total');
      sql.add('		   from WOPR');
      sql.add('			 left join BDepartment on WOPR.DepNo = BDepartment.ID');
      sql.add('      where  convert(smalldatetime,convert(varchar,WOPR.USERDATE,111)) between ');
      sql.add('             ''' + formatdatetime('yyyy/MM/dd', sDate) + '''' + 'and ' + '''' +
      ...
      sql.Add('Select T1.DepNo,T1.DepName, T1.GXLB,T1.SB, T1.qty, T1.NDay,T2.ToTal');
      sql.add('from( select ' + '''' + 'Z' + '''' + ' as DepNo,' + '''' + 'Z' + '''' + ' as DepName,'' '' as GXLB,' +
        '''' + '7.Daily defect' + '''' + ' as SB,SUM(WOPR.NGQty) as Qty');
      sql.add('             ,DATEPART(DAY,WOPR.USERDATE) as NDay');
      sql.add('	     from WOPR');
      sql.add('	     left join BDepartment on WOPR.DepNo = BDepartment.ID');
      sql.add('      where  convert(smalldatetime,convert(varchar,WOPR.USERDATE,111)) between ');
      ...
      sql.add('	     group by DATEPART(DAY,WOPR.USERDATE))T1');
      //20150908
      sql.Add('    left join(select ' + '''' + 'Z' + '''' + ' as DepNo,' + '''' + 'Z' + '''' +
        ' as DepName,'' '' as GXLB,' + '''' + '7.Daily defect' + '''' + ' as SB,SUM(WOPR.NGQty) as Total');
      sql.add('	     from WOPR');
      sql.add('	     left join BDepartment on WOPR.DepNo = BDepartment.ID');
      sql.add('      where  convert(smalldatetime,convert(varchar,WOPR.USERDATE,111)) between ');
      sql.add('             ''' + formatdatetime('yyyy/MM/dd', sDate) + '''' + 'and ' + '''' +
      ...
      sql.Add(Get_WOPR_SubSQL());
      //
      sql.add('			            group by DATEPART(DAY,WOPR.USERDATE)) as Depcount on DayInsp.NDay = Depcount.NDay)T1');
      //20150908
      sql.Add('left join(select ' + '''' + 'Z' + '''' + ' as DepNo,' + '''' + 'Z' + '''' + ' as DepName,'' '' as GXLB,'
        + '''' + '8.AVG inspection' + '''' + ' as SB');
      sql.add('             ,round(CAST(DayInsp.Qty AS Float)/Depcount.Depcount,2) as Total,DayInsp.NDay ');
      sql.add('	     from (select SUM(WOPR.Qty)+SUM(WOPR.ReQty)+sum(WOPR.NGQty) as Qty,''0'' as NDay');
      sql.add('	    		   from WOPR');
      sql.add('			       left join BDepartment on WOPR.DepNo = BDepartment.ID');
      sql.add('            where  convert(smalldatetime,convert(varchar,WOPR.USERDATE,111)) between ');
      sql.add('                   ''' + formatdatetime('yyyy/MM/dd', sDate) + '''' + 'and ' + '''' +
      ...
      sql.add('	     union all');
      sql.Add('Select T1.DepNo,T1.DepName, T1.GXLB,T1.SB, T1.qty, T1.NDay,T2.ToTal');
      sql.add('from( select ' + '''' + 'Z1' + '''' + ' as DepNo,' + '''' + 'Z1' + '''' + ' as DepName,'' '' as GXLB,' +
        '''' + '10.AVG defect%' + '''' + ' as SB');
      ...
      sql.add('			            group by DATEPART(DAY,WOPR.USERDATE)) as Depcount on DayInsp.NDay = Depcount.NDay)T1');
      //20150908
      sql.Add('left join ( select ' + '''' + 'Z1' + '''' + ' as DepNo,' + '''' + 'Z1' + '''' +
        ' as DepName,'' '' as GXLB,' + '''' + '10.AVG defect%' + '''' + ' as SB');
      ...
      sql.add('			        ,DayInsp.NDay ');
      sql.add('	     from (select SUM(WOPR.Qty)+SUM(WOPR.ReQty)+sum(WOPR.NGQty) as InsQty,SUM(WOPR.NGQty) as NGQty,''0'' as NDay');
      sql.add('		    	   from WOPR');
      sql.add('		         left join BDepartment on WOPR.DepNo = BDepartment.ID');
      sql.add('            where  convert(smalldatetime,convert(varchar,WOPR.USERDATE,111)) between ');
      sql.add('                   ''' + formatdatetime('yyyy/MM/dd', sDate) + '''' + 'and ' + '''' +
      ...
      //20150908
      sql.Add('left join (select ' + '''' + 'Z1' + '''' + ' as DepNo,' + '''' + 'Z1' + '''' +
        ' as DepName,'' '' as GXLB,' + '''' + '11.Daily RFT%' + '''' + ' as SB');
...
begin
  ...
  if trim(query1.FieldByName('SB').value) = '11.Daily RFT%' then
    begin
      if (column.field.fieldname = '1') or (column.field.fieldname = '2') or (column.field.fieldname = '3') or
        (column.field.fieldname = '4')
        or (column.field.fieldname = '5') or (column.field.fieldname = '6') or (column.field.fieldname = '7') or
          (column.field.fieldname = '8')
        or (column.field.fieldname = '9') or (column.field.fieldname = '10') or (column.field.fieldname = '11') or
          (column.field.fieldname = '12')
        or (column.field.fieldname = '13') or (column.field.fieldname = '14') or (column.field.fieldname = '15') or
          (column.field.fieldname = '16')
        or (column.field.fieldname = '17') or (column.field.fieldname = '18') or (column.field.fieldname = '19') or
          (column.field.fieldname = '20')
        or (column.field.fieldname = '21') or (column.field.fieldname = '22') or (column.field.fieldname = '23') or
          (column.field.fieldname = '24')
        or (column.field.fieldname = '25') or (column.field.fieldname = '26') or (column.field.fieldname = '27') or
          (column.field.fieldname = '28')
        or (column.field.fieldname = '29') or (column.field.fieldname = '30') or (column.field.fieldname = '31') then
        begin
...
procedure TMQRBD.imgCloseClick(Sender: TObject);
begin
Close;
end;
