...
type
  TLeatherMatInspection = class(TForm)
    ...
    QDetailGrade: TStringField;
    QDetailTotalSF: TCurrencyField;
    QDetailPercentGrade: TIntegerField;
    QDetailUSERID: TStringField;
    QDetailUSERDate: TDateTimeField;
    QDetailYN: TIntegerField;
...
procedure TLeatherMatInspection.PrintSign(
  AWorksheet: OleVariant;
  AQuery: TQuery;
  AInsertRow: Integer;
  const AIDField, ADateField: string;
...
  Worksheet.Cells[19, 5].Value := VarAsType(DBGridEh1.GetFooterValue(0, DBGridEh1.Columns[2]), varDouble);
...
  Worksheet.Cells[19, 16].Value :=
  FormatFloat('0.00',
    VarAsType(DBGridEh1.GetFooterValue(0, DBGridEh1.Columns[6]), varDouble) /
    VarAsType(DBGridEh1.GetFooterValue(0, DBGridEh1.Columns[2]), varDouble) * 100
  ) + '%';
...
procedure TLeatherMatInspection.BitBtn1Click(Sender: TObject);
begin
  with QDetail do
  begin
  RequestLive :=  true;
...
procedure TLeatherMatInspection.BitBtn2Click(Sender: TObject);
begin
if messagedlg('Are you sure you want to delete?',mtconfirmation,[mbYes,mbNo],0)<>mrYes then
  begin
    abort;
...
procedure TLeatherMatInspection.BitBtn3Click(Sender: TObject);
begin
with QDetail do
  begin
    cachedupdates:=true;
        ...
        case QDetail.updatestatus of
          usinserted:
            begin
              ...
              begin
                QDetail.Edit;
                QDetail.FieldByName('ReportID').Value := Query1.FieldByName('ReportID').Value;
                QDetail.FieldByName('USERID').Value := main.Edit1.Text;
                QDetail.FieldByName('YN').Value := 1;
                QDetail.FieldByName('USERDate').Value := FormatDateTime('yyyy-mm-dd', Now);
                UpDetail.apply(ukinsert);
              end;
            end;
          usmodified:
             begin
               if QDetail.FieldByName('YN').value='0' then
                 begin
                   if messagedlg('Are you sure you want to delete?',mtconfirmation,[mbYes,mbNo],0)=mrYes then
                   begin
                    QDetail.Edit;
                    QDetail.FieldByName('USERID').Value := main.Edit1.Text;
                    UpDetail.Apply(ukdelete)
                   end;
                 end else
                 begin
                  QDetail.Edit;
                  UpDetail.apply(ukmodify);
                 end;
              end;
        end;
        QDetail.next;
...
procedure TLeatherMatInspection.BitBtn5Click(Sender: TObject);
begin
  with QDetail do
  begin
    CachedUpdates := false;
