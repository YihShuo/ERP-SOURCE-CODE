...
procedure TIncomeMatMidInSole.PrintSign(
  AWorksheet: OleVariant;
  AQuery: TQuery;
  AInsertRow: Integer;
  const AIDField, ADateField: string;
...
begin
  ...
  PrintSign(Worksheet, Query1, InsertRow, 'SCFID',  'SCFDate',  8, True);
  PrintSign(Worksheet, Query1, InsertRow, 'LCFID',  'LCFDate',  14, True);
  PrintSign(Worksheet, Query1, InsertRow, 'PreparedID', 'PreparedDate', 18, False);
...
begin
  ...
  BasePath := '\\192.168.71.11\FileServer\WebData\upload-QC\' + main.Edit2.Text + '\N94\';
...
begin
  ...
  SourceFile := '\\192.168.71.11\FileServer\WebData\upload-QC\' + main.Edit2.Text + '\N94\' + FileName;
