...
begin
  ...
  begin
    active:=false;
    sql.Clear;
    sql.add('select QCC.DepNo,QCC.DepName,QCC.YYBH,QCC.YWSM');
    ...
    //defect每日各部門發生次數
    sql.add('from(select QCR.DepNo,BDepartment.DepName,QCBLYY.YYBH');
    ...
    sql.add('	    group by QCR.DepNo,BDepartment.DepName,QCBLYY.YYBH');
    ...
    //部門合計Defect數
    sql.add('	    union all');
    sql.add('	    select QCR.DepNo,BDepartment.DepName,'+''''+'Z'+''''+' as YYBH,'+''''+'2.Total Defect'+''''+' as YWSM,sum(QCRD.Qty) as Qty,DATEPART(DAY,QCRD.USERDATE) as NDay');
    sql.add('	    from QCR');
    sql.add('	    left join QCRD on QCR.ProNo = QCRD.ProNo');
    sql.add('	    left join BDepartment on QCR.DepNo = BDepartment.ID');
    sql.add('	    left join QCBLYY on QCR.GSBH = QCBLYY.GSBH and QCRD.YYBH = QCBLYY.YYBH');
    ...
    if CBX3.text = 'A+S' then
      sql.add('          and (QCR.GXLB = '+''''+'A'+''''+' or QCR.GXLB = '+''''+'C+S'+''''+')');
    sql.add('	    group by QCR.DepNo,BDepartment.DepName,DATEPART(DAY,QCRD.USERDATE)');
    sql.add('	    ) as QCC');
    sql.add('group by QCC.DepNo,QCC.DepName,QCC.YYBH,QCC.YWSM');
