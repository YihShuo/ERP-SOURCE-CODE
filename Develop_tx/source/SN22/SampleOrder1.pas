...
type
  TSampleOrder = class(TForm)
    ...
    Label72: TLabel;
    Edit6: TEdit;
    YPZLSFlexID: TStringField;
    YPZLstuffingpaper: TStringField;
    CB_Lebel_E245: TCheckBox;
    btn1: TButton;
    opendialog: TOpenDialog;
    YPZLKT_Check: TBooleanField;
    dtmfldYPZLKT_CheckDate: TDateTimeField;
    ...
    procedure CopyPartUsage1Click(Sender: TObject);
    procedure SpeedButton1Click(Sender: TObject);
    procedure Button3Click(Sender: TObject);
    procedure btn1Click(Sender: TObject);
    function TrimStrNB(i,j:integer;MsExcel:olevariant):Double;
  private
...
implementation
  ...
  SampleOrder_Developer1;
{$R *.dfm}

procedure TSampleOrder.ReadIni();
var MyIni :Tinifile;
    AppDir,UserIDSign: string;
    tmpUserIDSign:TStringlist;
    index:integer;
begin
  //
  ...
  begin
    try
      ...
      UserIDSign:=MyIni.ReadString('ERP','Develop_N22_UserIDSign','');
  ...
  if  UserIDSign <> '' then
  begin
    tmpUserIDSign:=FuncObj.SplitString(UserIDSign,',');
    index:=tmpUserIDSign.IndexOf(main.Edit1.Text);
...
function TSampleOrder.TrimStrNB(i,j:integer;MsExcel:olevariant):Double;
var ReturnD:Double;
    EditText2,ReplaceStr: String;
    k :integer;
    L:integer;
begin
    ...
    begin
      ...
      //ditText2:=msExcel.WorkBooks[1].WorkSheets[1].Cells[i,j].Text;
      L:=Length(EditText2);
      if L>=11 then
        L:=11;
...
procedure TSampleOrder.ERPQueryGetConnection();
  var ini:TiniFile;
      DBIP,DBName,User,Pass,spassword,sline,tempstr,YPDHCodeList,YPDHCodeList_SX:string;
      i,j,ad:integer;
      tmpList,tmpList_SX:TStringlist;
begin
  try
    ...
    tmpList.Free; }  //2026.06.06
    //2026.06.06 Them don hang mau ket thuc bang S,X cho 3 nhan hang Uniqlo, Vessi, Veja khi chon Copy Sample Order
    YPDHCodeList:=ini.ReadString('ERP','DevelopN22_YPDH','LAI,BYI');
    YPDHCodeList_SX:=ini.ReadString('ERP','DevelopN22_YPDH_SX','UQI,VSI,VJI,LAI');     //2026.06.16 Them LAIS, LAIX khi chon Copy Sample Order
    tmpList:=funcObj.SplitString(YPDHCodeList,',');
    tmpList_SX:=funcObj.SplitString(YPDHCodeList_SX,',');
    //SetLength(YPZL_YPDH, tmpList.Count*3+6);
    SetLength(YPZL_YPDH, tmpList.Count*3+8);      //2026.06.16 Them LAIS, LAIX khi chon Copy Sample Order
    j:=0;
    for i := 0 to tmpList.Count - 1 do
    begin
      YPZL_YPDH[j] := tmpList.Strings[i] + 'V'; Inc(j);
      YPZL_YPDH[j] := tmpList.Strings[i] + 'K'; Inc(j);
      YPZL_YPDH[j] := tmpList.Strings[i] + 'T'; Inc(j);
      if tmpList_SX.IndexOf(tmpList[i]) >= 0 then
      begin
        YPZL_YPDH[j] := tmpList.Strings[i] + 'S'; Inc(j);
        YPZL_YPDH[j] := tmpList.Strings[i] + 'X'; Inc(j);
      end;
    end;
    tmpList.Free;
    tmpList_SX.Free;
...
begin
  ...
  begin
    ...
    SQL.Add(' (case when  ypzl_w.ypdh is not null then ''Y'' ELSE ''N'' end) as chemical,ypzl.YPCCI,ypzl.YPCCIL,ypzl.YPCCOL,ypzl.YPCCSS');
    SQL.Add(' ,(case when  ypzl_stuffingpaper.ypdh is not null then ''Y'' ELSE ''N'' end) as stuffingpaper, YPZL.KT_Check, YPZL.KT_CheckDate ');
    ...
    SQL.Add(' LEFT JOIN (SELECT distinct ypdh from YPZLS WHERE BWBH=''E103'') ypzl_stuffingpaper ON ypzl_stuffingpaper.YPDH=YPZL.YPDH');
    ...
    if CB_Lebel_E245.Checked=True then
      begin
        SQL.Add('LEFT JOIN YPZLS on YPZLS.YPDH=ypzl.YPDH and YPZLS.BWBH=''E007'' ');
        SQL.Add('where YPZLS.BWBH is null ');
      end
    else
      SQL.Add('where 1=1 ');
...
var s1,s2,s3,s4,s5,s6,s7,s8,y,m,YPDH,OldShehao,OldXieXing,OldArticle,oldypcc,oldypcco,oldypccm,oldypccl,CopyYID:string;
    OldQty:real;
    i:integer;
    s9,s10,s11,s12,s13,s14,s15,s16,oldypcci,oldypccil,oldypccol,oldypccss:string;
begin
   //20250327 Sean 開發主管要求系統按條件阻擋超過特定雙數
  if YPZL.FieldByName('KFJD').value='CR1' then
  begin
    if YPZL.FieldByName('DevType').value='Inline' then
      begin
        if (pos('LAIK',YPZL.FieldByName('YPDH').asstring)>0) and ((pos('TN',YPZL.FieldByName('KFLX2').asstring)>0) or (pos('NU',YPZL.FieldByName('KFLX2').asstring)>0))then
          begin
            if YPZL.FieldByName('Quantity').value>25 then
            begin
              showmessage('Inline R1 TN/NU MAX=25 prs' );
              abort;
            end;
          end
        else if  (pos('LAIK',YPZL.FieldByName('YPDH').asstring)>0)  then
          begin
            if YPZL.FieldByName('Quantity').value>15 then
            begin
              showmessage('Inline R1 MAX=15 prs' );
              abort;
            end;
          end;
      end
    else if YPZL.FieldByName('DevType').value='Incubate' then
      begin
        if (pos('LAIK',YPZL.FieldByName('YPDH').asstring)>0)
          and ((pos('NG',YPZL.FieldByName('KFLX2').asstring)>0) or (pos('MU+',YPZL.FieldByName('KFLX2').asstring)>0)
                or (pos('NU',YPZL.FieldByName('KFLX2').asstring)>0) or (pos('TN',YPZL.FieldByName('KFLX2').asstring)>0))then
          begin
            if YPZL.FieldByName('Quantity').value>35 then
            begin
              showmessage('Incubate R1 TN/NU/NG/NM(MU)+ MAX=35 prs' );
              abort;
            end;
          end
        else if  (pos('LAIK',YPZL.FieldByName('YPDH').asstring)>0)  then
          begin
            if YPZL.FieldByName('Quantity').value>20 then
            begin
              showmessage('Incubate R1 MAX=20 prs' );
              abort;
            end;
          end;
      end
    else if YPZL.FieldByName('DevType').value='CTM' then
      begin
        if (pos('LAIK',YPZL.FieldByName('YPDH').asstring)>0)
          and ((pos('NG',YPZL.FieldByName('KFLX2').asstring)>0) or (pos('MU+',YPZL.FieldByName('KFLX2').asstring)>0)
                or (pos('NU',YPZL.FieldByName('KFLX2').asstring)>0) or (pos('TN',YPZL.FieldByName('KFLX2').asstring)>0))then
          begin
            if YPZL.FieldByName('Quantity').value>35 then
            begin
              showmessage('CTM R1 TN/NU/NG/NM(MU)+ MAX=35 prs' );
              abort;
            end;
          end
        else if  (pos('LAIK',YPZL.FieldByName('YPDH').asstring)>0)  then
          begin
            if YPZL.FieldByName('Quantity').value>20 then
            begin
              showmessage('CTM R1 MAX=20 prs' );
              abort;
            end;
          end;
      end;
  end
  else if YPZL.FieldByName('KFJD').value='CR2' then
  begin
    if YPZL.FieldByName('DevType').value='Inline' then
      begin
        if (pos('LAIK',YPZL.FieldByName('YPDH').asstring)>0) and ((pos('TN',YPZL.FieldByName('KFLX2').asstring)>0) or (pos('NU',YPZL.FieldByName('KFLX2').asstring)>0))then
          begin
            if YPZL.FieldByName('Quantity').value>22 then
            begin
              showmessage('Inline R2 LAIK TN/NU MAX=22 prs' );
              abort;
            end;
          end
        else if  (pos('LAIK',YPZL.FieldByName('YPDH').asstring)>0)  then
          begin
            if YPZL.FieldByName('Quantity').value>17 then
            begin
              showmessage('Inline R2 LAIK MAX=17 prs' );
              abort;
            end;
          end
        else if (pos('LAIV',YPZL.FieldByName('YPDH').asstring)>0) and ((pos('TN',YPZL.FieldByName('KFLX2').asstring)>0) or (pos('NU',YPZL.FieldByName('KFLX2').asstring)>0))then
          begin
            if YPZL.FieldByName('Quantity').value>14 then
            begin
              showmessage('Inline R2 LAIV TN/NU MAX=14 prs' );
              abort;
            end;
          end
        else if  (pos('LAIV',YPZL.FieldByName('YPDH').asstring)>0)  then
          begin
            if YPZL.FieldByName('Quantity').value>9 then
            begin
              showmessage('Inline R2 LAIV MAX=9 prs' );
              abort;
            end;
          end;
      end
      ...
      begin
        if (pos('LAIK',YPZL.FieldByName('YPDH').asstring)>0)
          and ((pos('NG',YPZL.FieldByName('KFLX2').asstring)>0) or (pos('MU+',YPZL.FieldByName('KFLX2').asstring)>0)
                or (pos('NU',YPZL.FieldByName('KFLX2').asstring)>0) or (pos('TN',YPZL.FieldByName('KFLX2').asstring)>0))then
          begin
            if YPZL.FieldByName('Quantity').value>31 then
            begin
              showmessage('Incubate R2 LAIK TN/NU/NG/NM(MU)+ MAX=31 prs' );
              abort;
            end;
          end
        else if  (pos('LAIK',YPZL.FieldByName('YPDH').asstring)>0)  then
          begin
            if YPZL.FieldByName('Quantity').value>16 then
            begin
              showmessage('Incubate R2 LAIK MAX=16 prs' );
              abort;
            end;
          end;
      end;
  end
  else if YPZL.FieldByName('KFJD').value='SMS' then
  begin
    if YPZL.FieldByName('DevType').value='Inline' then
      begin
        if  (pos('LAIK',YPZL.FieldByName('YPDH').asstring)>0)  then
          begin
            if YPZL.FieldByName('Quantity').value>25 then
            begin
              showmessage('Inline R3 LAIK MAX=25 prs' );
              abort;
            end;
       {   end
        else if  (pos('LAIV',YPZL.FieldByName('YPDH').asstring)>0)  then
          begin
            if YPZL.FieldByName('Quantity').value>45 then
            begin
              showmessage('Inline R3 LAIV MAX=45 prs' );
              abort;
            end; }
          end;
      end
      ...
      begin
        if (pos('LAIK',YPZL.FieldByName('YPDH').asstring)>0)
          ...
          begin
            if YPZL.FieldByName('Quantity').value>20 then
            begin
              showmessage('CTM R3 LAIK TN/NU/NG/NM(MU)+ MAX=20 prs' );
              abort;
            end;
          end
        else if  (pos('LAIK',YPZL.FieldByName('YPDH').asstring)>0)  then
          begin
            if YPZL.FieldByName('Quantity').value>15 then
            begin
              showmessage('CTM R3 LAIK MAX=15 prs' );
              abort;
            end;
          end;
      end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if  YPZL.FieldByName('YPCCI').OldValue=NULL then OldYPCCI:='' else OldYPCCI:=YPZL.FieldByName('YPCCI').OldValue;
              if  YPZL.FieldByName('YPCCIL').OldValue=NULL then OldYPCCIL:='' else OldYPCCIL:=YPZL.FieldByName('YPCCIL').OldValue;
              if  YPZL.FieldByName('YPCCOL').OldValue=NULL then OldYPCCOL:='' else OldYPCCOL:=YPZL.FieldByName('YPCCOL').OldValue;
              if  YPZL.FieldByName('YPCCSS').OldValue=NULL then OldYPCCSS:='' else OldYPCCSS:=YPZL.FieldByName('YPCCSS').OldValue;
              ...
              if ((YPZL.FieldByName('Article').AsString<>YPZL.FieldByName('Article').OldValue)or(YPZL.FieldByName('XieXing').AsString<>OldXieXing)or(YPZL.FieldByName('Shehao').AsString<>OldShehao)or(YPZL.FieldByName('Quantity').Value<>OldQty)
                  or(YPZL.FieldByName('YPCC').AsString<>OldYPCC)or(YPZL.FieldByName('YPCCO').AsString<>OldYPCCO)or(YPZL.FieldByName('YPCCM').AsString<>OldYPCCM)or(YPZL.FieldByName('YPCCL').AsString<>OldYPCCL)
                  or(YPZL.FieldByName('YPCCI').AsString<>OldYPCCI)or(YPZL.FieldByName('YPCCIL').AsString<>OldYPCCIL)or(YPZL.FieldByName('YPCCOL').AsString<>OldYPCCOL)or(YPZL.FieldByName('YPCCSS').AsString<>OldYPCCSS)
                   )then
              begin
                with TempQry do
                begin
                ...
                s9 := StringReplace(OLDYPCCI, '''', '''''', [rfReplaceAll]);
                s10 := StringReplace(YPZL.fieldbyname('YPCCI').AsString, '''', '''''', [rfReplaceAll]);
                s11 := StringReplace(OLDYPCCIL, '''', '''''', [rfReplaceAll]);
                s12 := StringReplace(YPZL.fieldbyname('YPCCIL').AsString, '''', '''''', [rfReplaceAll]);
                s13 := StringReplace(OLDYPCCOL, '''', '''''', [rfReplaceAll]);
                s14 := StringReplace(YPZL.fieldbyname('YPCCOL').AsString, '''', '''''', [rfReplaceAll]);
                s15 := StringReplace(OLDYPCCSS, '''', '''''', [rfReplaceAll]);
                s16 := StringReplace(YPZL.fieldbyname('YPCCSS').AsString, '''', '''''', [rfReplaceAll]);
                with TempQry do
                begin
                  active:=false;
                  sql.Clear;
                  sql.Add('insert into YPZL_log');
                  sql.Add('(YPDH,oldQuantity,Quantity,OLDYPCC,YPCC,OLDYPCCO,YPCCO,OLDYPCCL,YPCCL,OldYPCCM,YPCCM,OldYPCCI,YPCCI,OldYPCCIL,YPCCIL,OldYPCCOL,YPCCOL,OldYPCCSS,YPCCSS,UserID,UserDate)');
...
begin
  ...
  //2025.08.18
  if main.Edit2.Text='VC1' then  YPZLGrid.Columns[12].Visible:=true;
...
begin
  ...
  begin
    show1.Enabled:=false;
    CopyPartUsage1.Enabled:=false;
  end else
  begin
    show1.Enabled:=true;
    CopyPartUsage1.Enabled:=true;
  end;
end;
...
begin
  ...
  begin
    ...
    YPZLSGrid.columns[13].ButtonStyle:=cbsEllipsis;
  end; {else
  begin
...
begin
  ...
  if ((YPZL.fieldbyname('USERID').AsString=main.edit1.Text) or (main.edit1.Text='40895') or (main.edit1.Text='51024') or (main.edit1.Text='41190') or (main.edit1.Text='51267')) then
  begin
    with YPZLS do
    begin
      requestlive:=true;
...
begin
  YPZLSGrid.Columns[15].PickList.Clear;
  YPZLSGrid.Columns[15].PickList.Add('USD');
  YPZLSGrid.Columns[15].PickList.Add('VND');
  YPZLSGrid.Columns[15].PickList.Add('NTD');
  YPZLSGrid.Columns[15].PickList.Add('EUR');
  YPZLSGrid.Columns[15].PickList.Add('GBP');
  YPZLSGrid.Columns[15].PickList.Add('HKD');
  YPZLSGrid.Columns[15].PickList.Add('JPD');
  YPZLSGrid.Columns[15].PickList.Add('RMB');
end;
...
begin
            ...
            begin
              ...
              with TempQry do
               begin
                 active:=false;
                 sql.Clear;
                 sql.add('update ypzl set fd_check=''0'',pur_recieve=''0'' ,re_check=''0'',kt_check=''0''');
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              YPZL.FieldByName('re_check').Value:=0;
              YPZL.FieldByName('Pur_recieve').Value:=0;
              YPZL.FieldByName('KT_Check').Value:=0;
            end;
            upmas.apply(ukmodify);
          end else
          begin
  ...
  YPZLSGrid.columns[13].ButtonStyle:=cbsnone;
  newsize.text:='';
...
begin
  ...
  if(not DirectoryExists(AppDir+'Additional\'))  then ForceDirectories(AppDir+'Additional\');
    CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Additional\SampleOrder20150328_New.xls'),Pchar(AppDir+'Additional\SampleOrder20150328_New.xls'),false);
  if FileExists(AppDir+'Additional\SampleOrder20150328_New.xls') then
  begin
    if  YPZL.active  then
    begin
      try
        ...
        //
      try
        eclApp.WorkBooks.Open(AppDir+'Additional\SampleOrder20150328_New.xls');
...
//                eclApp.ActiveSheet.Rows[Offset+i].RowHeight := 60;
        ...
        begin
          ...
          begin
            ...
            SQL.Add('      ,ypzls.Remark ,bwzl.ywsm+''-''+bwzl.zwsm AS BWMCNM,bwzl.zwsm ,bwzl.ywsm ,clzl.zwpm AS CLMC,clzl.Ywpm AS CLYWMC ,clzl.dwbh ,zszl.zsjc ,clzl.clzmlb ,clzl.tyjh ,clzl.tyjh ,ypzls.JGZWSM ,ypzls.JGYWSM ,zszl.zsywjc, clzl_flex.cldhflex as FlexID ');
          ...
          begin
            SQL.Add('select * from ( ');
            SQL.Add('SELECT ypzls.YPDH ,ypzls.xh ,ypzls.xh1 ,ypzls.BWBH ,ypzls.CLBH ,ypzls.BWLB ,ypzls.CSBH ,ypzls.loss ,ypzls.CLSL ,ypzls.CLDJ ,ypzls.Currency ,ypzls.Rate,clzl.CQDH ');
            SQL.Add(',ypzls.Remark ,bwzl.ywsm+''-''+bwzl.zwsm AS BWMCNM,bwzl.zwsm ,bwzl.ywsm  ,clzl.zwpm AS CLMC,clzl.Ywpm AS CLYWMC ,clzl.dwbh ,zszl.zsjc ,clzl.clzmlb ,ypzls.JGZWSM ,ypzls.JGYWSM ,zszl.zsywjc, clzl_flex.cldhflex as FlexID ');
            SQL.Add('FROM ypzls ypzls ');
            SQL.Add('LEFT JOIN bwzl bwzl ON ypzls.BWBH = bwzl.bwdh ');
            SQL.Add('LEFT JOIN clzl clzl ON ypzls.CLBH = clzl.cldh ');
            SQL.Add('LEFT JOIN zszl zszl ON ypzls.CSBH = zszl.zsdh ');
            ...
            SQL.Add(' case when  ((clzhzl_dev.zsdh is not null) and (clzhzl_dev.zsdh<>'''')) then zszl_dev.zsywjc else  zszl.zsywjc  end as zsywjc, clzl_flex.cldhflex as FlexID ');
            SQL.Add('FROM ypzls ypzls ');
            ...
            SQL.Add('LEFT JOIN clzl_flex ON clzl_flex.cldh = clzhzl.CLDH1 ');
...
//                eclApp.ActiveSheet.Rows[Offset+i].Font.Size := 16;
              ...
              if TempQry.FieldByName('CLBH').AsString<>'' then
                ...
                eclApp.Cells(Offset+i,13):=TempQry.FieldByName('FlexID').AsString;
                eclApp.Cells(Offset+i,14):=TempQry.FieldByName('CLSL').AsString;
                eclApp.Cells(Offset+i,15):=YPZL.FieldByName('Quantity').AsString;
                eclApp.Cells(Offset+i,16):=TempQry.FieldByName('loss').AsString;
              if YPZL.FieldByName('Quantity').AsString<>'' then
                eclApp.Cells(Offset+i,17):=YPZL.FieldByName('Quantity').Value * TempQry.FieldByName('CLSL').Value ;
                eclApp.Cells(Offset+i,18):=TempQry.FieldByName('zsywjc').AsString;
                TempQry.Next;
            end;
          end else
          begin
        ...
        begin
          ...
          begin
            {if(not DirectoryExists(AppDir+'Images\'))  then ForceDirectories(AppDir+'Images\');
              CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Images\60633.bmp'),Pchar(AppDir+'Images\60633.bmp'),false);
            if FileExists(AppDir+'Images\60633.bmp') then
              signature:=AppDir+'Images\60633.bmp';}
            //2026.01.05 Thay doi chu ky chu quan theo don hang mau
            if (copy(YPZL.FieldByName('YPDH').AsString,1,3)='CUI') then
            begin
              if(not DirectoryExists(AppDir+'Images\'))  then ForceDirectories(AppDir+'Images\');
                CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Images\60633.bmp'),Pchar(AppDir+'Images\60633.bmp'),false);
              if FileExists(AppDir+'Images\60633.bmp') then
                signature:=AppDir+'Images\60633.bmp';
            end else if (copy(YPZL.FieldByName('YPDH').AsString,1,3)='UQI') then
            begin
              if(not DirectoryExists(AppDir+'Images\'))  then ForceDirectories(AppDir+'Images\');
                CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Images\51338.bmp'),Pchar(AppDir+'Images\51338.bmp'),false);
              if FileExists(AppDir+'Images\51338.bmp') then
                signature:=AppDir+'Images\51338.bmp';
            end else if ((copy(YPZL.FieldByName('YPDH').AsString,1,3)='VSI') or (copy(YPZL.FieldByName('YPDH').AsString,1,3)='VJI')) then
            begin
              if(not DirectoryExists(AppDir+'Images\'))  then ForceDirectories(AppDir+'Images\');
                CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Images\93204.bmp'),Pchar(AppDir+'Images\93204.bmp'),false);
              if FileExists(AppDir+'Images\93204.bmp') then
                signature:=AppDir+'Images\93204.bmp';
            end;
          end
          else
          begin
            if ((copy(YPZL.FieldByName('YPDH').AsString,4,1)='K') or (copy(YPZL.FieldByName('YPDH').AsString,4,1)='V')) and ((DBEDIT15.text = 'INCUBATE') or (DBEDIT15.text = 'CTM') or (DBEDIT15.text = 'CBY')) then
            begin
              if(not DirectoryExists(AppDir+'Images\'))  then ForceDirectories(AppDir+'Images\');
                ...
                //CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Images\51309.bmp'),Pchar(AppDir+'Images\51309.bmp'),false);
                 CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Images\71442.bmp'), Pchar(AppDir+'Images\71442.bmp'), false);
              {if FileExists(AppDir+'Images\51309.bmp') then
                signature:=AppDir+'Images\51309.bmp';}
                if FileExists(AppDir+'Images\71442.bmp') then
                  signature:=AppDir+'Images\71442.bmp';
            ...
            begin
              if(not DirectoryExists(AppDir+'Images\'))  then ForceDirectories(AppDir+'Images\');
                CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Images\45077.bmp'), Pchar(AppDir+'Images\45077.bmp'), false);
              if FileExists(AppDir+'Images\45077.bmp') then
                  signature:=AppDir+'Images\45077.bmp';

            end else if ((copy(YPZL.FieldByName('YPDH').AsString,4,1)='K') or (copy(YPZL.FieldByName('YPDH').AsString,4,1)='V')) and (DBEDIT15.text = 'INNOVATION') then
            begin
              if(not DirectoryExists(AppDir+'Images\'))  then ForceDirectories(AppDir+'Images\');
                CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Images\48654.bmp'), Pchar(AppDir+'Images\48654.bmp'), false);
              if FileExists(AppDir+'Images\48654.bmp') then
                  signature:=AppDir+'Images\48654.bmp';
          ...
          begin
            ...
            workSheet.Shapes.AddPicture(signature,false,true, workRange.left, workRange.top +5, workRange.width +20, workRange.height+25 );
            picture := Unassigned;
          end;
          with TempQry do
          begin
          ...
          eclApp.Range[eclApp.Cells.Item[sign,17],eclApp.Cells.Item[sign,17]].HorizontalAlignment := -4131;
          eclApp.Cells(sign,17):=TempQry.FieldByName('UserName').AsString+'['+YPZL.FieldByName('USERID').AsString+']';
        end;
...
begin
  ...
  //check and add hand tag & Stuffing paper
  if PC1.ActivePage.PageIndex=1 then
  begin
    ...
    with TempQry do
      begin
        //ShowMessage('CHeck');
        //check and add hand tag
        Active:=false;
        SQL.Clear;
        SQL.Add('  SELECT * FROM ypzls  ');
        SQL.Add('  WHERE YPDH = '''+YPDH_DB.text+'''');
        if  DBEdit14.Text='CFM' then
          SQL.Add('  and CLBH=''V90M002551''  ')
        else if  (DBEdit14.Text='WE1') or (DBEdit14.Text='WE2')or (DBEdit14.Text='WE3')  then
          SQL.Add('  and CLBH=''V90A031055''  ')
        else if  (DBEdit14.Text='FIT') or (DBEdit14.Text='WEA')  then
          SQL.Add('  and CLBH=''V90A031055''  ')
        else if  (DBEdit14.Text='CR1') or (DBEdit14.Text='SOS') or (DBEdit14.Text='SEE') or (DBEdit14.Text='SE1') or (DBEdit14.Text='SE2') or (DBEdit14.Text='SE3') or (DBEdit14.Text='SE4')
              or (DBEdit14.Text='POS') or (DBEdit14.Text='PU') or (DBEdit14.Text='PU1') or (DBEdit14.Text='PU2') or (DBEdit14.Text='PU3')  or (DBEdit14.Text='PU4')  then
          SQL.Add('  and CLBH=''V90M002550''  ')
        else if  (DBEdit14.Text='CR2')  then
          SQL.Add('  and CLBH=''V90M000406''  ')
        else if  (DBEdit14.Text='PDT')  then          
          SQL.Add('  and CLBH=''V90A031056''  ')
        else
          SQL.Add('  and CLBH=''V90M000404''  ');
        //showmessage(SQL.text);
        Active:=true;
        if Eof then
          begin
            BD2.Click;
            YPZLS.FieldByName('BWBH').Value:='E007';
            if  DBEdit14.Text='CFM' then
              YPZLS.FieldByName('CLBH').Value:='V90M002551'    
            else if  (DBEdit14.Text='WE1') or (DBEdit14.Text='WE2')or (DBEdit14.Text='WE3')  then
              YPZLS.FieldByName('CLBH').Value:='V90A031055'
            else if  (DBEdit14.Text='FIT') or (DBEdit14.Text='WEA')  then
              YPZLS.FieldByName('CLBH').Value:='V90A031055'
            else if  (DBEdit14.Text='CR1') or (DBEdit14.Text='SOS') or (DBEdit14.Text='SEE') or (DBEdit14.Text='SE1') or (DBEdit14.Text='SE2') or (DBEdit14.Text='SE3') or (DBEdit14.Text='SE4')
                  or (DBEdit14.Text='POS') or (DBEdit14.Text='PU') or (DBEdit14.Text='PU1') or (DBEdit14.Text='PU2') or (DBEdit14.Text='PU3')  or (DBEdit14.Text='PU4')  then
              YPZLS.FieldByName('CLBH').Value:='V90M002550'
            else if  (DBEdit14.Text='CR2')  then
              YPZLS.FieldByName('CLBH').Value:='V90M000406'
            else if  (DBEdit14.Text='PDT')  then
              YPZLS.FieldByName('CLBH').Value:='V90A031056'
            else
              YPZLS.FieldByName('CLBH').Value:='V90M000404';
            YPZLS.FieldByName('CSBH').Value:='P123';
            YPZLS.FieldByName('CLSL').Value:=2;
            YPZLSBWBHSetText(YPZLS.FieldByName('BWBH'), 'E007');
            YPZLS.Edit;
            YPZLSCLBHSetText(YPZLS.FieldByName('CLBH'),  YPZLS.FieldByName('CLBH').Value);
            YPZLS.Edit;
            YPZLSCSBHSetText(YPZLS.FieldByName('CSBH'), 'V142');
          end;

                                                            
        //check and add  Stuffing paper  
        if (Pos('KEY CHAIN',XieMing_DB.text)=0) or (Pos('SANDAL',XieMing_DB.text)=0)then
        begin    
          Active:=false;
          SQL.Clear;
          SQL.Add('  SELECT * FROM ypzls  ');  
          SQL.Add('  WHERE YPDH = '''+YPDH_DB.text+'''');
          SQL.Add('  and BWBH = ''E103''');
          Active:=true;
          if Eof then
            begin
              BD2.Click;
              YPZLS.FieldByName('BWBH').Value:='E103';    
              YPZLS.FieldByName('CLBH').Value:='H100001473';
              if  DBEdit5.Text='U' then
                YPZLS.FieldByName('CLSL').Value:=10
              else if  (DBEdit5.Text='W')   then
                YPZLS.FieldByName('CLSL').Value:=10
              else if  (DBEdit5.Text='K')   then
                YPZLS.FieldByName('CLSL').Value:=6
              else if  (DBEdit5.Text='Y')  then
                YPZLS.FieldByName('CLSL').Value:=6
              else if  (DBEdit5.Text='I')  then
                YPZLS.FieldByName('CLSL').Value:=6
              else
                YPZLS.FieldByName('CLSL').Value:=6;
              YPZLS.FieldByName('CSBH').Value:='V211';
              YPZLSBWBHSetText(YPZLS.FieldByName('BWBH'), 'E103');
              YPZLS.Edit;
              YPZLSCLBHSetText(YPZLS.FieldByName('CLBH'),  YPZLS.FieldByName('CLBH').Value);
              YPZLS.Edit;
              YPZLSCSBHSetText(YPZLS.FieldByName('CSBH'), 'V211');
            end;
        end;
        Active:=false;
        SQL.Clear;
...
var
  ...
  i, j: Integer;
  Field: TField;
  FieldNames: array[0..37] of string;
  DisplayNames: array[0..37] of string;
begin
  if YPZL.Active then
  begin
    FieldNames[0] := 'YPDH';         DisplayNames[0] := 'SampleNo';
    FieldNames[1] := 'XieXing';        DisplayNames[1] := 'StyleNo';
    FieldNames[2] := 'SheHao';         DisplayNames[2] := 'ColorNo';
    FieldNames[3] := 'JiJie';          DisplayNames[3] := 'Season';
    FieldNames[4] := 'DevType';        DisplayNames[4] := 'DevType1';
    FieldNames[5] := 'SubType';        DisplayNames[5] := 'SubType';
    FieldNames[6] := 'XieMing';        DisplayNames[6] := 'ArtName';
    FieldNames[7] := 'DevCode';        DisplayNames[7] := 'DevCode';
    FieldNames[8] := 'Article';        DisplayNames[8] := 'ArtNo';
    FieldNames[9] := 'YSSM';           DisplayNames[9] := 'ColorDesc';
    FieldNames[10] := 'EngName';       DisplayNames[10] := 'EngName';
    FieldNames[11] := 'FD';            DisplayNames[11] := 'Developer';
    FieldNames[12] := 'KFJD';          DisplayNames[12] := 'Stage';
    FieldNames[13] := 'Quantity';      DisplayNames[13] := 'PairQty';
    FieldNames[14] := 'XTMH';          DisplayNames[14] := 'LastNo';
    FieldNames[15] := 'YPCC';          DisplayNames[15] := 'Size';
    FieldNames[16] := 'YPCCO';         DisplayNames[16] := 'OutsoleSize';
    FieldNames[17] := 'YPCCL';         DisplayNames[17] := 'LastSize';
    FieldNames[18] := 'ypccm';         DisplayNames[18] := 'MidsoleSize';
    FieldNames[19] := 'YPCCI';         DisplayNames[19] := 'InsoleSize';
    FieldNames[20] := 'YPCCIL';        DisplayNames[20] := 'InSoleFillerSize';
    FieldNames[21] := 'YPCCOL';        DisplayNames[21] := 'OutSoleFillerSize';
    FieldNames[22] := 'YPCCSS';        DisplayNames[22] := 'StrobelSockSize';
    FieldNames[23] := 'BuildDate';     DisplayNames[23] := 'BuildDate';
    FieldNames[24] := 'MH';            DisplayNames[24] := 'MedialHeight';
    FieldNames[25] := 'LH';            DisplayNames[25] := 'LateralHeight';
    FieldNames[26] := 'BH';            DisplayNames[26] := 'BackHeight';
    FieldNames[27] := 'KFLX2';         DisplayNames[27] := 'TechLevel';
    FieldNames[28] := 'ywsm_1';        DisplayNames[28] := 'Pro.Type';
    FieldNames[29] := 'logo';          DisplayNames[29] := 'SizeRange';
    FieldNames[30] := 'FD_CHECK';      DisplayNames[30] := 'FD_CHECK_BOM_CHECK';
    FieldNames[31] := 're_check';      DisplayNames[31] := 're_check_Dev_Check';
    FieldNames[32] := 'PUR_RECIEVE';   DisplayNames[32] := 'PUR_RECIEVE_Pur_check';
    FieldNames[33] := 'ProductionLocation'; DisplayNames[33] := 'ProductionLocation';
    FieldNames[34] := 'Chemical';      DisplayNames[34] := 'Chemical';
    FieldNames[35] := 'Stuffingpaper'; DisplayNames[35] := 'Stuffingpaper';
    FieldNames[36] := 'DROPPED';       DisplayNames[36] := 'DROPPED';
    FieldNames[37] := 'PFC';           DisplayNames[37] := 'Factory';
    {FieldNames[36] := 'CFM';           DisplayNames[34] := 'CFM';
    FieldNames[37] := 'FINDATE';       DisplayNames[35] := 'FinishDate';
    FieldNames[38] := 'GSDH';          DisplayNames[36] := 'GSDH';     }
    ...
    try
      ...
      for i := 0 to 37 do
      begin
        eclApp.Cells(1, i + 1) := DisplayNames[i];
      end;
      ...
      begin
        for i := 0 to 37 do
        begin
          eclApp.Cells(j, i + 1) := YPZL.FieldByName(FieldNames[i]).Value;
        end;
        YPZL.Next;
        Inc(j);
      end;
...
begin
  ...
  //2026.06.06 To mau cho don hang mau ket thuc bang S,X
  if copy(YPZL.FieldByName('ypdh').AsString,4,1)='S' then
  begin
     YPZLGrid.canvas.Brush.Color:=RGB(60,179,113);
     YPZLGrid.DefaultDrawColumnCell(Rect,DataCol,Column,State);
  end;
  if copy(YPZL.FieldByName('ypdh').AsString,4,1)='X' then
  begin
     YPZLGrid.canvas.Brush.Color:=RGB(252,224,166);
     YPZLGrid.DefaultDrawColumnCell(Rect,DataCol,Column,State);
  end;
end;
...
procedure TSampleOrder.CopyPartUsage1Click(Sender: TObject);
begin
  UsageCopy:=TUsageCopy.Create(self);
  UsageCopy.Show;
end;

procedure TSampleOrder.SpeedButton1Click(Sender: TObject);
begin
  SampleOrder_Developer:=TSampleOrder_Developer.Create(self);
  SampleOrder_Developer.ShowModal();
  SampleOrder_Developer.Free;
end;
...
begin
  ...
  begin 
    Active:=false;
    SQL.Clear; 
    SQL.Add('  SELECT DENSE_RANK() OVER(ORDER BY YPZLS.XH) AS ROWID,ypzls.YPDH ,ypzls.xh ,ypzls.BWBH ,ypzls.CLBH ,ypzls.BWLB ,ypzls.CSBH ,ypzls.loss ,ypzls.CLSL ,ypzls.CLDJ ,ypzls.Currency ,CLZL_LS.JHDH,ypzls.Rate ,ypzls.Remark ,  ');
    SQL.Add('         bwzl.ywsm+''-''+bwzl.zwsm AS BWMCNM,       bwzl.zwsm,bwzl.ywsm,clzl.zwpm AS CLMC ,clzl.dwbh ,zszl.zsjc ,clzl.clzmlb  ,clzl.tyjh ,ypzls.JGZWSM ,ypzls.JGYWSM ,zszl.zsywjc ,clzl.ywpm ,clzl.cqdh,zszl_dev.SamplePurchaser,''1'' as YN,  ');
    SQL.Add('         CLZL.UserID as MatUser,ypzls.UserID,ypzls.UserDate,XH1,YPZLS.Costing_Proportion,YPZLS.Costing_CLSL, clzl_flex.cldhflex as FlexID FROM ypzls ypzls  ');
    SQL.Add('  LEFT JOIN bwzl bwzl ON ypzls.BWBH = bwzl.bwdh   ');
    SQL.Add('  LEFT JOIN clzl clzl ON ypzls.CLBH = clzl.cldh   ');
    SQL.Add('  left join CLZL_LS clzl_ls on CLZL.CLDH=CLZL_LS.CLDH    ');
    SQL.Add('  LEFT JOIN zszl zszl ON ypzls.CSBH = zszl.zsdh  ');
    ...
    if Edit2.Text<>'' then
      SQL.Add(' AND YPZLS.BWBH like '''+Edit2.Text+'%'' ');
    if Edit3.Text<>'' then
      SQL.Add(' AND bwzl.ywsm like ''%'+Edit3.Text+'%'' ');
    if Edit4.Text<>'' then
      SQL.Add(' AND clzl.ywpm like ''%'+Edit4.Text+'%'' ');
    if Edit5.Text<>'' then
      SQL.Add(' AND ypzls.csbh like '''+Edit5.Text+'%'' ');
    if Edit6.Text<>'' then
    ...
    if OpenDialog.Execute=true then
     begin
      ...
      with YPZLS do
       ...
       while (flag=true) do
          begin
            Text2:=MsExcel.WorkBooks[1].WorkSheets[1].Cells[j,2].text;
            if Text2='' then
              break;
              BD2.Click;                                    

              FieldByName('BWBH').value:=MsExcel.WorkBooks[1].WorkSheets[1].Cells[j,2].text;
              FieldByName('CLBH').value:=MsExcel.WorkBooks[1].WorkSheets[1].Cells[j,4].text;
              FieldByName('CSBH').value:=MsExcel.WorkBooks[1].WorkSheets[1].Cells[j,7].text;
              FieldByName('loss').value:=TrimStrNB(j,9,msExcel);
              FieldByName('CLSL').value:=TrimStrNB(j,10,msExcel);
              YPZLSBWBHSetText(YPZLS.FieldByName('BWBH'), YPZLS.FieldByName('BWBH').Value);
              YPZLS.Edit;
              YPZLSCLBHSetText(YPZLS.FieldByName('CLBH'),  YPZLS.FieldByName('CLBH').Value);
              YPZLS.Edit;
              YPZLSCSBHSetText(YPZLS.FieldByName('CSBH'), YPZLS.FieldByName('CSBH').Value);
