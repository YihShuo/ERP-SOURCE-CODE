...
type
  TSampleOrder_Mat = class(TForm)
    ...
    Excel: TMenuItem;
    ArticleQryEngname: TStringField;
    PopupMenu2: TPopupMenu;
    S1: TMenuItem;
    MatQryMemo: TStringField;
...
implementation
  uses main1,SampleOrder1,FunUnit, Material_Memo1;
{$R *.dfm}
...
begin
  ...
  begin
    Active:=false;
    SQL.Clear;
    SQL.Add('select top 20 Cldh,zwpm,ywpm,dwbh,CQDH,CLZL.TYJH,YPZLS.UseCount,zszl.zsdh,zszl.ZSYWJC,CLZL_Memo.Memo from CLZL ');
    sql.add('left join ( ');
  ...
  begin
    if MatQry.RecordCount>0 then
    begin
      with ArticleQry do
      begin
        Active:=false;
        sql.Clear;
        sql.Add('  select kfxxzl.devcode,kfxxzl.ARTICLE,ypzlS.CLBH,ypzlS.YPDH,ypzl.KFJD,kfxxzl.JiJie,kfxxzl.FD,ypzlS.CSBH,Busers.Engname  ');
        sql.Add('  from  ypzlS ');
...
procedure TSampleOrder_Mat.S1Click(Sender: TObject);
begin
  if (Material_Memo <> Nil) then
    Material_Memo.Close;
  Material_Memo := TMaterial_Memo.Create(Self);
  Material_Memo.CLDHEdit.Text:=MatQry.FieldByName('CLDH').AsString;
  Material_Memo.Show;
end;
