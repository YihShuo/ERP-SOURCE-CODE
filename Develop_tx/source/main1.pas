...
type
  Tmain = class(TForm)
    ...
    procedure SN633Click(Sender: TObject);
    procedure SNA23Click(Sender: TObject);
    procedure SN47Click(Sender: TObject);
    procedure SNA9Click(Sender: TObject);
  private
     MKID:String;
...
uses DM3,tempmaterialp, MaterialChengp, smapleorderlistp, PurOther1, DelOther1,
  ...
  scandata1, NC2BP1, TechShoeStock1, ClearGiaCong1, scandata_Import1,
  CFactoryScan1;
...
begin
  ...
  begin
    ...
    SNA23.enabled := Locate('FORMID', 'SNA23', []);
    ...
    SN47.enabled := Locate('FORMID', 'SN47', []);
    SNA9.enabled := Locate('FORMID', 'SNA9', []);
    active:=false;
  end;
  end;
end;
...
begin
  showFm(Pointer(SNC2), TSNC2, TmenuItem(Sender).Name);
end;

procedure Tmain.SN633Click(Sender: TObject);
begin
  showFm(Pointer(TechShoeStock), TTechShoeStock, TmenuItem(Sender).Name);
end;

procedure Tmain.SNA23Click(Sender: TObject);
begin
  showFm(Pointer(ClearGiaCong), TClearGiaCong, TmenuItem(Sender).Name);
end;

procedure Tmain.SN47Click(Sender: TObject);
begin
  showFm(Pointer(scandata_Import), Tscandata_Import, TmenuItem(Sender).Name);
end;

procedure Tmain.SNA9Click(Sender: TObject);
begin
  showFm(Pointer(CFactoryScan), TCFactoryScan, TmenuItem(Sender).Name);
end;
