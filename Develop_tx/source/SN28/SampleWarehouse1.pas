...
type
  TSampleWarehouse = class(TForm)
   // main: TPanel;
    ...
    Button7: TButton;
    QryTmpMemo2: TQuery;
    updateQryTmpMemo2: TUpdateSQL;
    BitBtn1: TBitBtn;
    Query1FD: TStringField;
    shoetestC9: TBooleanField;
    dtmfldC9_Send: TDateTimeField;
    dtmfldC9_Get: TDateTimeField;
 ...
 // showmessage(main.edit1.text);
  ...
  begin
    ...
    if checkbox3.checked then
       SQL.Add('top 10');
    SQL.Add('shoetest.FD,shoetest.testno,shoetest.ypdh,isnull(Busers.engname,''NoName'')+''-''+Busers.USERID as Developer,');
...
begin
 ...
 begin
   ...
   SQL.Add('  shoetest.lamination,shoetest.lamination_CRMTO,shoetest.Amico,shoetest.Sublimation,shoetest.kietphong,shoetest.E_embroidery,shoetest.printing,shoetest.LiMao,shoetest.Laser,shoetest.DAJU,shoetest.C9,shoetest.C9_Send,shoetest.C9_Get,');
   //===2026.05.14
   SQL.Add('  ypzl.YPDH,ypzl.YPCC,isnull(bw01.qty,0) FinishedShoe');
   //=============
   SQL.add('from shoetest');
   ...
   if Edit15.Text<> '' then
      SQL.Add('and shoetestplan.Serino like '''+Edit15.Text+'%'' ');
   //======2026.05.30
   ...
   begin
    ...
    //===2026.05.14
    else if Combobox2.text = 'C9 Send' then begin
       SQL.Add(' and shoetest.C9_Send between');
       sql.add(' convert(smalldatetime,'''+formatdatetime('yyyy/MM/dd',DTP3.datetime)+' '+formatdatetime('hh:mm:ss',DTP11.datetime)+''') ');
       sql.add(' and  ');
       sql.add(' convert(smalldatetime,'''+formatdatetime('yyyy/MM/dd',DTP4.datetime)+' '+formatdatetime('hh:mm:ss',DTP22.datetime)+''')  ');
    end
    else if Combobox2.text = 'C9 Get' then begin
       SQL.Add(' and shoetest.C9_Get between');
       sql.add(' convert(smalldatetime,'''+formatdatetime('yyyy/MM/dd',DTP3.datetime)+' '+formatdatetime('hh:mm:ss',DTP11.datetime)+''') ');
       sql.add(' and  ');
       sql.add(' convert(smalldatetime,'''+formatdatetime('yyyy/MM/dd',DTP4.datetime)+' '+formatdatetime('hh:mm:ss',DTP22.datetime)+''')  ');
    end
...
begin
  if  shoetest.active  then
    ...
    try
       ...
       eclApp.Cells(1,51):='C9_Send';
       eclApp.Cells(1,52):='C9_Get';
       shoetest.First;
        j:=2;
      ...
      eclapp.columns.autofit;
      eclApp.Columns['AV'].ColumnWidth := 50;
      eclApp.Columns['AV'].WrapText := True;
      eclapp.Rows.AutoFit;
      showmessage('Succeed');
      eclApp.Visible:=True;
      except
...
procedure TSampleWarehouse.Button7Click(Sender: TObject);
begin
    with QryTmpMemo2 do
    begin
      Active := False;
      SQL.Clear;
      SQL.Add('UPDATE shoetest SET memo2 = NULL ');
      SQL.Add('WHERE memo2 IS NOT NULL ');
      SQL.Add('AND LEN(REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(memo2)), CHAR(9), ''''), CHAR(10), ''''), CHAR(13), '''')) > 0');
      SQL.Add('AND PreparationDate IS NOT NULL ');
      SQL.Add('AND S_material IS NOT NULL ');
      SQL.Add('AND insole_send IS NOT NULL ');
      SQL.Add('AND lace_send IS NOT NULL ');
      SQL.Add('AND shoetest.TBreceiveDate BETWEEN ');
      SQL.Add('CONVERT(smalldatetime, ''2024/09/01 00:00:00'') ');
      SQL.Add('AND CONVERT(smalldatetime, GETDATE()) ');
      ExecSQL;
      showmessage('Succeed');
    end;
end;
...
procedure TSampleWarehouse.BitBtn1Click(Sender: TObject);
var   a:string;
      eclApp,WorkBook:olevariant;
      i,j:integer;
 begin
  if  QStatus.active  then
    ...
    try
        WorkBook:=eclApp.workbooks.Add;
        for   i:=0   to   QStatus.fieldcount-1   do
          begin
              eclApp.Cells(1,i+1):=QStatus.fields[i].FieldName;
          end;
        ...
        while   not   QStatus.Eof   do
          begin
            for   i:=0   to  QStatus.fieldcount-1  do
            begin
              eclApp.Cells(j,i+1):=QStatus.Fields[i].Value;
            end;
          QStatus.Next;
          inc(j);
          end;
