...
type
   ...
   // procedure R4modifydate();
    ...
    procedure BtnExcelFormatClick(Sender: TObject);
  private
   AppDir:string;
  ...
  begin
    ...
    sql.Clear;
     ...
     with Qryname do
       ...
       //=====02.01.2025
       SQL.add('order by '+TBname+'.tpdate');
       //==
       //showmessage(sql.text);
       active:=true;
       Ebn.Enabled:=false;
...
BEGIN
    ...
    begin
      ...
      //=====02.01.2025====
      SQL.add('select DENSE_RANK() over (order by '+TBname+'.tpdate, kfxxzl.devcode) as Type,');
      //=====
...
//      sql.add('left join  (select * from kfxxzl where khdh=''036'')kfxxzl on '+TBname+'.xiexing=kfxxzl.xiexing and '+TBname+'.shehao=kfxxzl.shehao');
      ...
      SQL.Add('left join (');
      SQL.Add('  SELECT Busers.Engname, Fdgroup.teamleader FROM Fdgroups');
      SQL.Add('  LEFT JOIN Busers ON Busers.USERID = Fdgroups.FDID');
      SQL.Add('  left join (');
      SQL.Add('    select fdteam, busers.engname teamleader from Fdgroup');
      SQL.Add('    left join busers on busers.userid=Fdgroup.teamleader');
      SQL.Add('  ) AS Fdgroup on Fdgroups.FDTeam=Fdgroup.FDTeam');
      SQL.Add('  WHERE Fdgroups.fdname = ''' + Trim(ComboBox7.Items.Text) + '''');
      SQL.Add(') AS Fdgroup ON Fdgroup.Engname = ypzl.FD');
      ...
      //SQL.add('order by kfxxzl.devcode');
      //=======02.01.2025
      SQL.add('order by '+TBname+'.tpdate, KFXXZL.DEVCODE');
      //=====
      //showmessage(sql.text);
      Active:=true;
    end;
  ...
  begin
   ...
   begin
    ...
    SQL.Add('SELECT SRDet_ML.* FROM SRDet_ML');
    if (main.Edit2.Text = 'VC1') then
    begin
      SQL.Add('LEFT JOIN (');
      SQL.Add('  SELECT DISTINCT YPDH, CLBH FROM (');
      SQL.Add('    SELECT DISTINCT SRDet_ML.YPDH, YPZLS.CLBH,');
      SQL.Add('    CASE WHEN PATINDEX(''%-[0-9]%'', BWZL.YWSM) > 0 THEN LEFT(BWZL.YWSM, PATINDEX(''%-[0-9]%'', BWZL.YWSM + ''-'') - 1) ELSE BWZL.YWSM END AS YWSM FROM SRDet_ML');
      SQL.Add('    LEFT JOIN YPZLS ON YPZLS.YPDH = SRDet_ML.YPDH AND YPZLS.CLBH = SRDet_ML.CLBH');
      SQL.Add('    LEFT JOIN BWZL ON BWZL.BWDH = YPZLS.BWBH');
      SQL.Add('    WHERE LEFT(SRDet_ML.YPDH, 4) = ''' + LAI.Text + ''' AND SRDet_ML.Season = ''' + season.Text + ''' AND SRDet_ML.Stage IN (''' + Stage + ''')');
      SQL.Add('  ) AS YPZL');
      SQL.Add('  WHERE YWSM NOT LIKE ''%REINF%'' AND YWSM NOT LIKE ''%PADDING'' AND YWSM NOT LIKE ''%TAPE'' AND YWSM NOT IN (''SIZE LABEL'', ''UPC LABEL'')');
      SQL.Add(') AS YPZL ON YPZL.YPDH = SRDet_ML.YPDH AND YPZL.CLBH = SRDet_ML.CLBH');
    end;
    SQL.Add('WHERE LEFT(SRDet_ML.YPDH, 4) = ''' + LAI.Text + ''' AND SRDet_ML.Season = ''' + season.Text + ''' AND SRDet_ML.Stage IN (''' + Stage + ''')');
    if (main.Edit2.Text = 'VC1') then
  ...
  begin
    Active := false;
    SQL.Clear;
    SQL.Add('SELECT SRDet_FML.* FROM SRDet_FML');
    if (main.Edit2.Text = 'VC1') then
    begin
      SQL.Add('LEFT JOIN (');
      SQL.Add('  SELECT DISTINCT YPDH, CLBH FROM (');
      SQL.Add('    SELECT DISTINCT SRDet_FML.YPDH, YPZLS.CLBH,');
      SQL.Add('    CASE WHEN PATINDEX(''%-[0-9]%'', BWZL.YWSM) > 0 THEN LEFT(BWZL.YWSM, PATINDEX(''%-[0-9]%'', BWZL.YWSM + ''-'') - 1) ELSE BWZL.YWSM END AS YWSM FROM SRDet_FML');
      SQL.Add('    LEFT JOIN YPZLS ON YPZLS.YPDH = SRDet_FML.YPDH AND YPZLS.CLBH = SRDet_FML.CLBH');
      SQL.Add('    LEFT JOIN BWZL ON BWZL.BWDH = YPZLS.BWBH');
      SQL.Add('    WHERE LEFT(SRDet_FML.YPDH, 4) = ''' + LAI.Text + ''' AND SRDet_FML.Season = ''' + season.Text + ''' AND SRDet_FML.Stage IN (''' + Stage + ''')');
      SQL.Add('  ) AS YPZL');
      SQL.Add('  WHERE YWSM NOT LIKE ''%REINF%'' AND YWSM NOT LIKE ''%PADDING'' AND YWSM NOT LIKE ''%TAPE'' AND YWSM NOT IN (''SIZE LABEL'', ''UPC LABEL'')');
      SQL.Add(') AS YPZL ON YPZL.YPDH = SRDet_FML.YPDH AND YPZL.CLBH = SRDet_FML.CLBH');
    end;
    SQL.Add('WHERE LEFT(SRDet_FML.YPDH, 4) = ''' + LAI.Text + ''' AND SRDet_FML.Season = ''' + season.Text + ''' AND SRDet_FML.Stage IN (''' + Stage + ''')');
    if (main.Edit2.Text = 'VC1') then
      SQL.Add('AND YPZL.YPDH IS NOT NULL');
  ...
  if combobox3.text='Uppers'  then
     begin
      ...
      begin
        ...
        SQL.Add('left join (select season,CLBH,Location from MaterialMOQ  Group by CLBH,Location ) MaterialMOQ on MaterialMOQ.CLBH=CGZLS.CLBH');
...
procedure TSampleDevelopInformation.BtnExcelFormatClick(Sender: TObject);
var eclApp,WorkBook:olevariant;
    i,j,k:integer;
    checkshoePicture: string;
    pictureExcel : OleVariant;
    workRange : OleVariant;
    workSheet : OleVariant;
begin
  with ExcelFormat do
    begin
      ...
      sql.Clear;
      SQL.add('select xxzlkf.imgnameCFM,kfxxzl.XieMing,');
      SQL.add('kfxxzl.devcode,xxzl.article');
      sql.Add('from cfm');
      sql.add('left join  kfxxzl on cfm.xiexing=kfxxzl.xiexing and cfm.shehao=kfxxzl.shehao');
      sql.add('left join  xxzl on CFM.xiexing=xxzl.xiexing and CFM.shehao=xxzl.shehao');
      sql.add('left join  xxzlkf on cfm.xiexing=xxzlkf.xiexing and cfm.shehao=xxzlkf.shehao');
    ...
    if ExcelFormat.Active then
    begin
      if ExcelFormat.recordcount=0 then
      begin
    ...
    if(not DirectoryExists(AppDir+'Additional\'))  then ForceDirectories(AppDir+'Additional\');
      CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Additional\Develop_N72_CFMStatus.xlsx'),Pchar(AppDir+'Additional\Develop_N72_CFMStatus.xlsx'),false);
      if FileExists(AppDir+'Additional\Develop_N72_CFMStatus.xlsx') then
      begin
        try
          eclApp := CreateOleObject('Excel.Application');
          eclApp.DisplayAlerts := False;
          eclApp.Workbooks.Open(AppDir + 'Additional\Develop_N72_CFMStatus.xlsx');
          ...
          for i := 1 to ExcelFormat.RecordCount do
          begin
            checkshoePicture := StringReplace(ExcelFormat.FieldByName('imgnameCFM').AsString, 'H:', '\\192.168.23.11\A_DataCenter\CDC\Sales\BOM', [rfReplaceAll, rfIgnoreCase]);
            if FileExists(checkshoePicture) then
            begin
              workSheet.Activate;
              workRange := workSheet.Cells.Item[j, 1];
              pictureExcel := workSheet.Pictures.Insert(checkshoePicture);
              pictureExcel.Top := workRange.Top;
              pictureExcel.Left := workRange.Left;
              pictureExcel.Width := 150;
              pictureExcel.Height := 150;
            end;
          eclApp.Cells(j, 2) := ExcelFormat.FieldByName('XieMing').Value;
          eclApp.Cells(j, 3) := ExcelFormat.FieldByName('devcode').Value;
          eclApp.Cells(j, 4) := ExcelFormat.FieldByName('article').Value;
          Inc(j);
          ExcelFormat.Next;
      end;
