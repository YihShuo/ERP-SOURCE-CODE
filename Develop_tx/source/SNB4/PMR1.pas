...
type
  TPMR = class(TForm)
    ...
    Button3: TButton;
    Q_PMRListPMRDONE: TBooleanField;
    Q_PMRListSSDEVCheck: TBooleanField;
...
begin
  ...
  if not Q_PMRList.FieldByName('PMRDONE').IsNull then
  begin
    if Q_PMRList.FieldByName('PMRDONE').AsBoolean = False then
...
begin
  ...
  if not Q_PMRListSS.FieldByName('DEVCheck').IsNull then
  begin
    if Q_PMRListSS.FieldByName('DEVCheck').AsBoolean = False then
      AFont.Color := clBlue;
  end;
end;
