...
procedure TTBClear1.PWD();
var
  WorkOrder, FilePath, PWD: string;
begin
  if (MessageDlg('Please enter your password to continue', mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
  begin
    PostMessage(Handle, InputBoxMessage, 0, 0);
    PWD := InputBox('Unlock Password', 'Password', '');
    if (PWD = FormatDateTime('yyyyMMdd', Now)) then
    begin
      PWDTrue:='True'
    end else
...
begin
  ...
  begin
    ...
    begin
      Active:=false;
      SQL.Clear;
      SQL.Add('Select YPDH,complete From shoetest  where Testno='''+edit8.text+''' ');
      Active:=true;
    end;
    if  copy(QTemp.FieldByName('YPDH').AsString,4,1)='V' then
    begin
       if Date > Trunc(QTemp.FieldByName('complete').AsDateTime) then
       begin
          showmessage('Already scan complete last month!');
          abort;
       end;
    end;
    //
