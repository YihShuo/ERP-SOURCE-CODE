...
var
  CFactoryScan: TCFactoryScan;

implementation
  uses main1, FunUnit;
{$R *.dfm}

procedure TCFactoryScan.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
    Action:=caFree;
end;

procedure TCFactoryScan.FormDestroy(Sender: TObject);
begin
  CFactoryScan:=nil;
end;

procedure TCFactoryScan.Edit1KeyPress(Sender: TObject; var Key: Char);
begin
  if (Key = #13)  then
  begin
    with query1 do
    begin
      Active:=false;
      SQL.Clear;
      SQL.Add('select shoetest.testno,shoetest.Article,TBReceive,Finish,Complete,shoetest.YPDH,Pending_C,CutDate,Pending_S,StitchingDate,Assemblecomplete  ');
      SQL.Add('from shoetest ');
      SQL.Add('left join ypzl on shoetest.ypdh=ypzl.ypdh');
      SQL.Add('where Testno='''+edit1.text+''' and ypzl.gsdh=''VC1''  ');
      Active:=true;
...
//檢查是否領料完畢
Function TCFactoryScan.CheckKCLLS_Finish():boolean;
...
begin
  ...
  begin
    ...
    begin
       ...
       begin
         IsYPZL_ZLZLS2:=false;
         showmessage(''+YPDH+' chua co du lieu Shipping. Vui long lien he kho thao tac Shipping N44');
         abort;
       end;
       Active:=false;
    end;
    //
    // kiem tra R2/R3 da tinh dinh muc chua
    ...
    begin
      ...
      begin
         Active:=false;
         SQL.Clear;
         SQL.Add(' Select ypzls.GSBH,zlzls3.GSBH as GSBH_Check  ');
         SQL.Add('  From ( Select case when csbh=''JNGR2'' then ''VR2'' when csbh=''JNGR3'' then ''VR3'' end GSBH,ypzls.YPDH');
         if IsYPZL_ZLZLS2=false then
         begin
            SQL.Add('  From ypzls ypzls  ');
         end else
         begin
            SQL.Add('  From YPZL_ZLZLS2 ypzls ');
         end;
         SQL.Add('  Where YPDH='''+YPDH+''' ');
         SQL.Add('  and CSBH in (''JNGR3'',''JNGR2'') ');
         SQL.Add('  Group by case when csbh=''JNGR2'' then ''VR2'' when csbh=''JNGR3'' then ''VR3'' end,ypzls.YPDH) YPZLS ');
         SQL.Add(' Left join ( Select DISTINCT CQDH as GSBH,zlbh1 as ZLBH ');
         SQL.Add('              From '+main.LIY_DD+'.dbo.ZLZLS3  ZLZLS3 ');
         SQL.Add('        	    Where ZLBH1='''+YPDH+''' and ZLZLS3.TCLYL>0  ) ZLZLS3 on ypzls.GSBH=zlzls3.GSBH and ypzls.YPDH=ZLZLS3.ZLBH ');
         SQL.Add('  Where zlzls3.GSBH is null ');
...
procedure TCFactoryScan.SacnTBComplete();
begin
    ...
    begin
      ...
      begin
        Active:=false;
        SQL.Clear;
        SQL.Add('SET DATEFIRST 1;');
        SQL.Add('update shoetest set finish =''1'',complete=convert(smalldatetime,convert(varchar,GETDATE(),111)),Pending_C=1,Pending_S=1 ');
        SQL.Add('      ,CutID = ''' + main.Edit1.Text + ''',');
        SQL.Add('      StitchingID = ''' + main.Edit1.Text + ''',');
        SQL.Add('      AssemblyID = ''' + main.Edit1.Text + ''',');
        SQL.Add('      PackingID = ''' + main.Edit1.Text + ''',');
        {SQL.Add('      ,Cutdate=case when Cutdate is not null then CutDate else convert(smalldatetime,convert(varchar,GETDATE(),111))-5 end ');
        SQL.Add('      ,StitchingDate=case when StitchingDate is not null then StitchingDate else convert(smalldatetime,convert(varchar,GETDATE(),111))-3 end ');
        SQL.Add('      ,Assemblecomplete=case when Assemblecomplete is not null then Assemblecomplete else convert(smalldatetime,convert(varchar,GETDATE(),111))-1 end ');  }
        SQL.Add('  CutDate = CASE');
        SQL.Add('    WHEN CutDate IS NOT NULL THEN CutDate');
        SQL.Add('    WHEN DATEPART(WEEKDAY, GETDATE() - 5) = 7 THEN GETDATE() - 6');
        SQL.Add('    ELSE GETDATE() - 5');
        SQL.Add('  END,');
        SQL.Add('  StitchingDate = CASE');
        SQL.Add('    WHEN StitchingDate IS NOT NULL THEN StitchingDate');
        SQL.Add('    WHEN DATEPART(WEEKDAY, GETDATE() - 3) = 7 THEN GETDATE() - 4');
        SQL.Add('    ELSE GETDATE() - 3');
        SQL.Add('  END,');
        SQL.Add('  Assemblecomplete = CASE');
        SQL.Add('    WHEN Assemblecomplete IS NOT NULL THEN Assemblecomplete');
        SQL.Add('    WHEN DATEPART(WEEKDAY, GETDATE() - 1) = 7 THEN GETDATE() - 2');
        SQL.Add('    ELSE GETDATE() - 1');
        SQL.Add('  END');
        SQL.Add('where Testno='''+edit1.text+'''');
        SQL.Add('select testno,Article,TBReceive,finish,complete,YPDH,Pending_C,CutDate,Pending_S,StitchingDate,Assemblecomplete from shoetest where Testno='''+edit1.text+'''');
        //showmessage(sql.Text);
        Active:=true;
      end;
    end else
    begin
       Showmessage(YPDH+':chua mo phieu het vat tu khong scan nhap kho thanh pham, xin bao ton kho kiem tra lai');
    end;
    Edit1.Clear;
end;

procedure TCFactoryScan.FormShow(Sender: TObject);
begin
  PageControl1.ActivePageIndex := 0;
end;

procedure TCFactoryScan.FormCreate(Sender: TObject);
var i:integer;
begin
  PageControl1.ActivePageIndex := 0;
  with query1 do
  begin
    ...
    SQL.Add('from shoetest ');
    SQL.Add('left join ypzl on shoetest.ypdh=ypzl.ypdh');
    SQL.Add('where ypzl.gsdh=''VC1'' and CONVERT(varchar(100), Complete, 112) = CONVERT(varchar(100), GETDATE(), 112)');
    Active:=true;
  end;
  //底廠
  with Qtemp do
