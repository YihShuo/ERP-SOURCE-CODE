...
type
  TBasicInformation = class(TForm)
    ...
    Query7XieXing: TStringField;
    Query7SheHao: TStringField;
    Query7XieMing: TStringField;
    Query7DEVCODE: TStringField;
    Query7ARTICLE: TStringField;
    Query7JiJie: TStringField;
    Query7Subtype: TStringField;
    Query7Techlevel: TStringField;
    Query7InlineNonInline: TStringField;
    Query7ProductionType: TStringField;
    Query7category: TStringField;
    Query7CTS: TStringField;
    Query7Planning_Level_2: TStringField;
    Query7Segment: TStringField;
    Query7LYN_Order: TIntegerField;
    Query7LYI_Order: TIntegerField;
    Query7UserID: TStringField;
    Query7UserDate: TDateTimeField;
    Query7YN: TStringField;
    Button1: TButton;
    OpenDialog1: TOpenDialog;
    Query7SS: TStringField;
...
begin
  ...
  begin
    ...
    SQL.Add('  WHERE KFXXZL.khdh in (select KHBH from kfzl_GS where GSBH = '''+ComboBox1.Text+''') AND kfjd <> ''SEE'' AND KFXXZL.JiJie LIKE ''%' + CB_SEASON.Text + '%''');
...
//    SQL.Add('  AND LEN(KFXXZL.SheHao) <= 2   ');
  ...
  begin
    Flag := false;
  end  else if (PageControl1.ActivePageIndex = 6) then
  begin
    Flag := false;
  end;
...
{      if (CheckBox5.Checked = true) and (CheckBox6.Checked = true) then
  ...
  //20240921 add CFM Sample tab
  with Query7 do
  begin
    Active:=false;
    SQL.Clear;
    SQL.Add('  Select CFM.XieXing,CFM.SheHao,kfxxzl.XieMing,kfxxzl.DEVCODE,xxzl.ARTICLE,kfxxzl.JiJie,ypzl.Subtype  ');
    SQL.Add('  	,case kfxxzl.KFLX when ''CU'' then ''NC'' when ''MU'' then ''NM'' when ''MU+'' then ''NM+'' else kfxxzl.KFLX end  as ''Techlevel'' ,xxzlkf.SS ');
    SQL.Add('  	, case when isnull(ypzl.Subtype,'''') <> '''' and (ypzl.Subtype=''Inline'' or ypzl.Subtype=''CBY'') then ''Inline'' else ''Non-Inline'' end  as ''Inline/Non-Inline''  ');
    //SQL.Add('  	,Isnull(CFM_Sample.ProductionType,xxzlkf.ProductionType) as ProductionType,Isnull(CFM_Sample.Category,xxzlkf.category) as Category,Isnull(CFM_Sample.CTS,xxzlkf.CTS) as CTS  ');
    SQL.Add('   ,case when Isnull(CFM_Sample.ProductionType,'''')<> '''' then CFM_Sample.ProductionType else xxzlkf.ProductionType end ProductionType ');
    SQL.Add('	  ,case when Isnull(CFM_Sample.Category,'''')<> '''' then CFM_Sample.Category else xxzlkf.Category end Category ');
    SQL.Add(' 	,case when Isnull(CFM_Sample.CTS,'''')<> '''' then CFM_Sample.CTS else xxzlkf.CTS end CTS ');
    SQL.Add('   ,xxzlkf.Planning_Level_2,XXZLKF.Segment  ');
    SQL.Add('   ,case when isnull(CFM_Sample.LYN_Order,'''') <> '''' then CFM_Sample.LYN_Order else DDZL.pairs end LYN_Order ');
    SQL.Add('   ,CFM_Sample.LYI_Order,xxzlkf.SS,CFM_Sample.UserID,CFM_Sample.UserDate,CFM_Sample.YN  ');
    SQL.Add('  From CFM  ');
    ...
    SQL.Add('  Left join Fdgroups on Fdgroups.fdname=kfxxzl.FD  ');
    SQL.Add('  Left join kfzl_GS on kfzl_GS.KHBH=kfxxzl.KHDH  ');
    SQL.Add('  Left join CFM_Sample on CFM.XieXing=CFM_Sample.XieXing and CFM.SheHao=CFM_Sample.Shehao  ');
    SQL.Add('  Left join (Select ddzl.XieXing,left(ddzl.SheHao,2) as SheHao,Sum(isnull(pairs,0)) pairs  From ddzl ');
    SQL.Add('       			Left join kfxxzl on ddzl.XieXing=kfxxzl.XieXing and kfxxzl.SheHao=left(ddzl.SheHao,2) ');
    SQL.Add('             Where 1=1  and KFXXZL.JiJie LIKE ''%' + CB_SEASON.Text + '%'' and CONVERT(varchar,DDRQ,11)< case when RIGHT(kfxxzl.JiJie,1) =''S'' then convert(varchar(6),LEFT(kfxxzl.JiJie,2)-1)+''/09/30'' ');
    SQL.Add('                                                            when RIGHT(kfxxzl.JiJie,1) =''U'' then convert(varchar(6),LEFT(kfxxzl.JiJie,2)-1)+''/12/31'' ');
    SQL.Add('                                     											 when RIGHT(kfxxzl.JiJie,1) =''F'' then convert(varchar(6),LEFT(kfxxzl.JiJie,2))+''/03/31'' ');
    SQL.Add('                                                            when RIGHT(kfxxzl.JiJie,1) =''H'' then convert(varchar(6),LEFT(kfxxzl.JiJie,2))+''/06/30'' end ');
    SQL.Add('       			Group by ddzl.XieXing,left(ddzl.SheHao,2)) DDZL on DDZL.XieXing=kfxxzl.XieXing and DDZL.SheHao=kfxxzl.SheHao  ');
    SQL.Add('  Where ypzl.kfjd =''CFM'' ');
    SQL.Add('   and kfxxzl.devcode <>''''  ');
    SQL.Add('   and kfxxzl.xieming not like ''%NULLIFY%''   ');
    ...
    SQL.Add('   and kfzl_GS.GSBH= ''' + ComboBox1.Text + ''' ');
    if  CB_SEASON.Text<> '' then
      SQL.Add(' and KFXXZL.JiJie LIKE ''%' + CB_SEASON.Text + '%'' ');
    if (CB_NNULL.Checked) then
    begin
      SQL.Add('AND KFXXZL.Xieming NOT LIKE ''%NULLIFY%''');
      SQL.Add('AND KFXXZL.Xieming NOT LIKE ''%NULIFY%''');
    ...
    SQL.Add(' Group By CFM.XieXing,CFM.SheHao,kfxxzl.XieMing,kfxxzl.DEVCODE,xxzl.ARTICLE,xxzlkf.SS,kfxxzl.JiJie,ypzl.Subtype ');
    SQL.Add('	          ,case kfxxzl.KFLX when ''CU'' then ''NC'' when ''MU'' then ''NM'' when ''MU+'' then ''NM+'' else kfxxzl.KFLX end ');
    SQL.Add('		        ,case when isnull(ypzl.Subtype,'''') <> '''' and (ypzl.Subtype=''Inline'' or ypzl.Subtype=''CBY'') then ''Inline'' else ''Non-Inline'' end  ');
    SQL.Add('       		,case when Isnull(CFM_Sample.ProductionType,'''')<> '''' then CFM_Sample.ProductionType else xxzlkf.ProductionType end  ');
    SQL.Add('       		,case when Isnull(CFM_Sample.Category,'''')<> '''' then CFM_Sample.Category else xxzlkf.Category end ');
    SQL.Add('       		,case when Isnull(CFM_Sample.CTS,'''')<> '''' then CFM_Sample.CTS else xxzlkf.CTS end ');
    SQL.Add('       		,xxzlkf.Planning_Level_2,XXZLKF.Segment  ');
    SQL.Add('       		,case when isnull(CFM_Sample.LYN_Order,'''') <> '''' then CFM_Sample.LYN_Order else DDZL.pairs end  ');
    SQL.Add('       		,CFM_Sample.LYI_Order,CFM_Sample.UserID,CFM_Sample.UserDate,CFM_Sample.YN  ');
    SQL.Add(' Order by  CFM.XieXing,CFM.SheHao ');
    //funcobj.WriteErrorLog(sql.Text);
    Active:=true;
  end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        usModified:
          begin
            if (TargetQuery.FieldByName('YN').AsString = '1') then
            begin
              TargetQuery.Edit;
              TargetQuery.FieldByName('UserDate').Value := Ndate;
              TargetQuery.FieldByName('UserID').Value := main.Edit1.text;
              ...
              begin
                 ...
                 SQL.Add('Set SS='''+TargetQuery.FieldByName('SS').AsString+''',ProductionType='''+TargetQuery.FieldByName('ProductionType').AsString+''',category='''+TargetQuery.FieldByName('Category').AsString+''', CTS='''+TargetQuery.FieldByName('CTS').AsString+''' ');
                 SQL.Add('Where XieXing='''+TargetQuery.FieldByName('XieXing').AsString+'''  and SheHao='''+TargetQuery.FieldByName('SheHao').AsString+''' ');
                 //funcobj.WriteErrorLog(sql.Text);
                 execsql;
                 active:=false;
              end;
            ...
            begin
              ...
              TargetQuery.FieldByName('UserDate').Value := Ndate;
              TargetQuery.FieldByName('UserID').Value := main.Edit1.text;
              TargetQuery.FieldByName('YN').Value := 1;
              TargetUpdateSQL.apply(ukInsert);
              with QSearch do
              begin
                 active:=false;
                 sql.Clear;
    ...
    try
      ...
      begin
        ...
        if not VarIsClear(DesRange)  then
          ...
          begin
            showmessage('Target Sample Price not found in excel');
            Exit;
        end;
        ...
        if not VarIsClear(DesRange)  then
          ...
          begin
            showmessage('Development TSP not found in excel');
            Exit;
        end;
        ...
        begin
          ...
          until IsBreak=true;
          //關閉excel
          OrderListExcel.ActiveWorkBook.Saved := True;
          OrderListExcel.ActiveWorkBook.Close;
          OrderListExcel.Quit;
          VarClear(OrderListExcel);
...
procedure TBasicInformation.Button1Click(Sender: TObject);
var
  ExcelApp: Variant;
  S_XieXing,S_SheHao: string;
  S_SS,S_ProductionType,S_Category,S_CTS,S_LYIOrder:string;
  i,j:integer;
begin
  ...
  begin
    ...
    i:=2;
    S_XieXing:=ExcelApp.Cells[i,1].Value;
    S_SheHao:=ExcelApp.Cells[i,2].Value;
    S_SS:=ExcelApp.Cells[i,10].Value;
    S_ProductionType:=ExcelApp.Cells[i,11].Value;
    S_Category:=ExcelApp.Cells[i,12].Value;
    S_CTS:=ExcelApp.Cells[i,13].Value;
    S_LYIOrder:=ExcelApp.Cells[i,17].Value;
    while ((trim(S_XieXing)<>'') and (trim(S_SheHao)<>''))  do
    begin
      Query7.First;
      for j:=0 to Query7.RecordCount-1 do
      begin
        if ((Query7.FieldByName('XieXing').Value= S_XieXing) and (Query7.FieldByName('SheHao').Value= S_SheHao)) then
        begin
          Query7.Edit;
          Query7.FieldByName('SS').AsString:=S_SS;
          Query7.FieldByName('ProductionType').AsString:=S_ProductionType;
          Query7.FieldByName('Category').AsString:=S_Category;
          Query7.FieldByName('CTS').AsString:=S_CTS;
          Query7.FieldByName('LYI_Order').AsString:=S_LYIOrder;
          Query7.Post;
        end;
        Query7.Next;
      end;
