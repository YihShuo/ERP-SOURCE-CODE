...
uses
  ...
  ExtCtrls, DB, DBTables, GridsEh, DBGridEh, Menus, comobj, ADODB, DBGridEhImpExp, ShellAPI,IniFiles,Clipbrd;
type
  TSamplePlan = class(TForm)
    ...
    shoetestCutting_Localtion: TStringField;
    shoetestStitching_Localtion: TStringField;
    shoetestAssembly_Localtion: TStringField;
    shoetestPacking_Localtion: TStringField;
    shoetestArticleKFXXZL: TStringField;
    shoetestCancel_C: TBooleanField;
    shoetestCancel_S: TBooleanField;
    shoetestCancel_A: TBooleanField;
    shoetestCutID: TStringField;
    shoetestStitchingID: TStringField;
    shoetestAssemblyID: TStringField;
    shoetestPackingID: TStringField;
    shoetestYPCC: TStringField;
    PopupMenu_Noti: TPopupMenu;
    CreateNoti: TMenuItem;
    shoetestShoetest_NotiChange_TestNo: TIntegerField;
    Label17: TLabel;
    editYPDH: TEdit;
    mmo1: TMemo;
    btn1: TBitBtn;
    shoetestPlanningMemo: TMemoField;
    shoetestCutting_Priority: TBooleanField;
    shoetestStiching_Priority: TBooleanField;
    shoetestAssemble_Priority: TBooleanField;
    qryShoeTestPriority: TQuery;
    updtsqlShoeTestPriority: TUpdateSQL;
    qryShoeTestPriorityTestNo: TStringField;
    qryShoeTestPriorityCutting_Priority: TBooleanField;
    qryShoeTestPriorityStiching_Priority: TBooleanField;
    qryShoeTestPriorityAssemble_Priority: TBooleanField;
    dtmfldShoeTestPriorityCutting_Date: TDateTimeField;
    dtmfldShoeTestPriorityStiching_Date: TDateTimeField;
    dtmfldShoeTestPriorityAssemble_Date: TDateTimeField;
    qryShoeTestPriorityUSERID: TStringField;
    dtmfldShoeTestPriorityUSERDATE: TDateTimeField;
    ...
    procedure CreateNotiClick(Sender: TObject);
...
uses main1, ChooseDPDR, FunUnit, NotiChange1;
...
procedure TSamplePlan.TestNoEditPaste();
var
  CbBuf: PChar;
  BufSize: Integer;
  Handle: THandle;
...
begin
 try
   ...
   begin
    ...
    begin
      TotalTestNo:=TotalTestNo+rowList.Strings[i]+',';
    end;
    rowList.Free;
    if length(TotalTestNo)>0 then
    begin
      Edit2.Text:=Copy(TotalTestNo,1,length(TotalTestNo)-1);
    end;
   end;
 except
   on E:Exception do
...
var
  ListTestNo: TStringList;
  i: Integer;
begin
 ListTestNo := TStringList.Create;
 with shoetest do
 begin
   //01.11.2025 tim theo nhieu TestNo
   ListTestNo.Delimiter := ',';
   ListTestNo.DelimitedText := Edit2.Text;

   if Copy(Edit2.Text, Length(Edit2.Text), 1) = ',' then
   Edit2.Text := Copy(Edit2.Text, 1, Length(Edit2.Text)-1);
   ListTestNo.DelimitedText := Edit2.Text;

   if ListTestNo.Count > 20 then
   begin
     ShowMessage('TestNo cannot be more than 20');
     ListTestNo.Free;
     Exit;
   end;
   close;
   ...
   requestlive:=true;
   SQL.add('select');
   SQL.add('  shoetestplan.Serino,ypzl.YPDH,ypzl.YPCC,shoetest.Season,shoetest.stage,groups,groups2,shoetest.testno,shoetest.Article,TBReceiveDate,');
   ...
   sql.Add('  shoetest.Cutting_Localtion,shoetest.Stitching_Localtion,shoetest.Assembly_Localtion,shoetest.Packing_Localtion,Cancel_C,Cancel_S,Cancel_A,kfxxzl.ARTICLE as ArticleKFXXZL,CutID,StitchingID,AssemblyID,PackingID');
   SQL.add('  ,CASE WHEN Shoetest_NotiChange.TestNo IS NOT NULL THEN 1 ELSE 0 END AS Shoetest_NotiChange_TestNo,');
   SQL.Add('  shoetest_priority.Cutting_Priority, shoetest_priority.Stiching_Priority, shoetest_priority.Assemble_Priority');
   SQL.add('from shoetest');
   sql.add('left join shoetest_priority on shoetest_priority.TestNo = shoetest.TestNo');
   SQL.add('left join shoetestplan on shoetest.testno = shoetestplan.testno');
   ...
   sql.Add('  )AS Wdate');
   sql.Add('  from shoetest )warehouse on warehouse.testno=shoetest.testno');
   sql.Add('left join Shoetest_NotiChange on Shoetest_NotiChange.TestNo = shoetest.TestNo');
   SQL.add('where  1=1');
   if edit8.Text<>'' then
   ...
   //01.11.2025 tim kiem theo nhieu TestNo
   {if edit2.Text<>'' then
      SQL.add('and shoetest.testno like '''+edit2.Text+'%'+''''); }
   if edit2.Text<>'' then
     begin
        sql.Add('       and (');
        for i := 0 to ListTestNo.Count - 1 do
        begin
        if i > 0 then
          sql.Add('           or');
          sql.Add('           shoetest.testno like ''' + ListTestNo[i] + '%''');
        end;
        sql.Add('       )');
     end;
   ...
   begin
    ...
    //25-12-2024
    else if Combobox2.text = 'TB_notOK' then begin
       SQL.Add(' and shoetest.TBreceiveDate between');
       sql.add(' convert(smalldatetime,'''+formatdatetime('yyyy/MM/dd',DTP1.datetime)+' '+formatdatetime('hh:mm:ss',DTP11.datetime)+''') ');
       sql.add(' and  ');
       sql.add(' convert(smalldatetime,'''+formatdatetime('yyyy/MM/dd',DTP2.datetime)+' '+formatdatetime('hh:mm:ss',DTP22.datetime)+''')  ');
       sql.add(' and (SQTBOK is null or SQTBOK=''0'')');
    end
   end;
   ...
   if editYPDH.Text<>'' then
    SQL.add('and YPZL.YPDH like ''%'+editYPDH.Text+'%'+'''');
...
begin
  ...
  try
    qryShoeTestPriority.Close;
    qryShoeTestPriority.RequestLive := True;
    qryShoeTestPriority.CachedUpdates := True;
    qryShoeTestPriority.Open;
      ...
      usmodified:
        begin
         ...
         begin
           ...
           //======2025/05/07
           if (shoetest.FieldByName('CutDate').Value <> shoetest.FieldByName('CutDate').OldValue) then
           begin
             SQL.Add('INSERT INTO BDelRec (TableName, TNO, CLBH, OldID, DelID, DelDate)');
             SQL.Add('SELECT ''ShoeTest'',''' + shoetest.FieldByName('testno').AsString+'_C' + ''','''+DatetoStr(shoetest.fieldbyname('CutDate').OldValue)+'***'+DatetoStr(shoetest.fieldbyname('CutDate').Value)+''',''' + main.Edit1.Text +''','''+ main.Edit1.Text + ''', GetDate() FROM ShoeTest');
             SQL.Add('WHERE TestNo = ''' + shoetest.FieldByName('testno').AsString + ''' ');
           end;

           if(shoetest.FieldByName('StitchingDate').Value <> shoetest.FieldByName('StitchingDate').OldValue) then
           begin
             SQL.Add('INSERT INTO BDelRec (TableName, TNO, CLBH, OldID, DelID, DelDate)');
             SQL.Add('SELECT ''ShoeTest'',''' + shoetest.FieldByName('testno').AsString+'_S' + ''','''+DatetoStr(shoetest.fieldbyname('StitchingDate').OldValue)+'***'+DatetoStr(shoetest.fieldbyname('StitchingDate').Value)+''',''' + main.Edit1.Text +''','''+ main.Edit1.Text + ''', GetDate() FROM ShoeTest');
             SQL.Add('WHERE TestNo = ''' + shoetest.FieldByName('testno').AsString + ''' ');
           end;

           if (shoetest.FieldByName('Assemblecomplete').Value <> shoetest.FieldByName('Assemblecomplete').OldValue) then
           begin
             SQL.Add('INSERT INTO BDelRec (TableName, TNO, CLBH, OldID, DelID, DelDate)');
             SQL.Add('SELECT ''ShoeTest'',''' + shoetest.FieldByName('testno').AsString+'_A' + ''','''+DatetoStr(shoetest.fieldbyname('Assemblecomplete').OldValue)+'***'+DatetoStr(shoetest.fieldbyname('Assemblecomplete').Value)+''',''' + main.Edit1.Text +''','''+ main.Edit1.Text + ''', GetDate() FROM ShoeTest');
             SQL.Add('WHERE TestNo = ''' + shoetest.FieldByName('testno').AsString + ''' ');
           end;

           if (shoetest.FieldByName('Complete').Value <> shoetest.FieldByName('Complete').OldValue) then
           begin
             SQL.Add('INSERT INTO BDelRec (TableName, TNO, CLBH, OldID, DelID, DelDate)');
             SQL.Add('SELECT ''ShoeTest'',''' + shoetest.FieldByName('testno').AsString+'_P' + ''','''+DatetoStr(shoetest.fieldbyname('Complete').OldValue)+'***'+DatetoStr(shoetest.fieldbyname('Complete').Value)+''',''' + main.Edit1.Text +''','''+ main.Edit1.Text + ''', GetDate() FROM ShoeTest');
             SQL.Add('WHERE TestNo = ''' + shoetest.FieldByName('testno').AsString + ''' ');
           end;
          //==================
         ...
         begin
          ...
          if qryShoeTestPriority.RecordCount > 0 then
          begin
            qryShoeTestPriority.First;
            qryShoeTestPriority.Edit;
          end
          else
          begin
            qryShoeTestPriority.Insert;
          end;

          qryShoeTestPriority.FieldByName('TestNo').Value := shoetest.FieldByName('TestNo').Value;
          qryShoeTestPriority.FieldByName('UserID').Value := main.Edit1.Text;
          qryShoeTestPriority.FieldByName('UserDate').Value := NDate;

          if shoetest.FieldByName('Cutting_Priority').Value <> shoetest.FieldByName('Cutting_Priority').OldValue then
          begin
            qryShoeTestPriority.FieldByName('Cutting_Priority').Value := (shoetest.FieldByName('Cutting_Priority').Value <> null) and (shoetest.FieldByName('Cutting_Priority').Value);
            qryShoeTestPriority.FieldByName('Cutting_Date').Value := NDate;
          end;

          if shoetest.FieldByName('Stiching_Priority').Value <> shoetest.FieldByName('Stiching_Priority').OldValue then
          begin
            qryShoeTestPriority.FieldByName('Stiching_Priority').Value := (shoetest.FieldByName('Stiching_Priority').Value <> null) and (shoetest.FieldByName('Stiching_Priority').Value);
            qryShoeTestPriority.FieldByName('Stiching_Date').Value := NDate;
          end;

          if shoetest.FieldByName('Assemble_Priority').Value <> shoetest.FieldByName('Assemble_Priority').OldValue then
          begin
            qryShoeTestPriority.FieldByName('Assemble_Priority').Value := (shoetest.FieldByName('Assemble_Priority').Value <> null) and (shoetest.FieldByName('Assemble_Priority').Value);
            qryShoeTestPriority.FieldByName('Assemble_Date').Value := NDate;
          end;

          if qryShoeTestPriority.RecordCount > 0 then
          begin
            updtsqlShoeTestPriority.Apply(ukModify);
          end
          else
          begin
            updtsqlShoeTestPriority.Apply(ukInsert);
          end;
         end;
      end;
      shoetest.next;
    ...
    qryShoeTestPriority.Close;
    qryShoeTestPriority.RequestLive := False;
    qryShoeTestPriority.CachedUpdates := False;
...
begin
   ...
   DBGridEh1.Columns[15].PickList.Clear;
   DBGridEh1.Columns[16].PickList.Clear;
   DBGridEh1.Columns[17].PickList.Clear;
   if main.Edit2.Text='CDC' then
   begin
      DBGridEh1.Columns[14].PickList.Add('CDC');
      DBGridEh1.Columns[15].PickList.Add('CDC');
      DBGridEh1.Columns[16].PickList.Add('CDC');
      DBGridEh1.Columns[17].PickList.Add('CDC');
   end else
   if main.Edit2.Text='VC1' then
   begin
      DBGridEh1.Columns[14].PickList.Add('VC1');
      DBGridEh1.Columns[15].PickList.Add('VC1');
      DBGridEh1.Columns[16].PickList.Add('VC1');
      DBGridEh1.Columns[17].PickList.Add('VC1');
   end;
...
begin
  ...
  if (shoetest.FieldByName('Cancel_C').value='1') or (shoetest.FieldByName('Cancel_S').value='1') or (shoetest.FieldByName('Cancel_A').value='1')  then
  begin
    DBGridEh1.canvas.font.color:=clBlue;
  end;
  if shoetest.FieldByName('Shoetest_NotiChange_TestNo').value = '1' then
  begin
...
begin
   ...
   begin
     if (main.edit1.text='51022') or (main.edit1.text='51270') or (main.edit1.text='51152') then  //�D�ޥi�H��w��
     begin
...
procedure TSamplePlan.CreateNotiClick(Sender: TObject);
begin
  NotiChange:=TNotiChange.create(self);
  NotiChange.ShowModal();
end;

procedure TSamplePlan.btn1Click(Sender: TObject);
var
...
begin
  try
      ...
      Sheet.Range['A1'].Value := 'TIEN DO GIA CONG DE (C9)';
      Sheet.Range['A1'].Font.Bold := True;
      Sheet.Range['A1'].Font.Size := 16;
      Sheet.Range['A1'].HorizontalAlignment := -4108;

      Sheet.Range['A2', 'F2'].Merge;
      Sheet.Range['A2'].Value := 'PROCESSING OUTSOLE SCHEDULE (C9)';
      Sheet.Range['A2'].HorizontalAlignment := -4108;

      // --- Ngay be phai ---
      Sheet.Range['G1', 'G2'].Merge;
      Sheet.Range['G1'].Value := 'NGAY ' + FormatDateTime('dd/mm/yyyy', Now);
      Sheet.Range['G1'].HorizontalAlignment := -4152; // right

      // --- Header ---
      Sheet.Range['A4'].Value := 'Type';
      Sheet.Range['B4'].Value := 'TestNo';
      Sheet.Range['C4'].Value := 'SR#';
      Sheet.Range['D4'].Value := 'SIZE';
      Sheet.Range['E4'].Value := 'So luong';
      Sheet.Range['F4'].Value := 'Ngay giao de den C9';
      Sheet.Range['G4'].Value := 'Ngay phong mau nhan de';

      Sheet.Range['A4:G4'].Font.Bold := True;
      Sheet.Range['A4:G4'].Interior.Color := $00FFFF; // nen mau vag nhat
      ...
      while not shoetest.Eof do
      begin
        Sheet.Cells[Row, 1] := shoetest.FieldByName('Serino').AsString;
        Sheet.Cells[Row, 2] := shoetest.FieldByName('TestNo').AsString;
        Sheet.Cells[Row, 3] := shoetest.FieldByName('Article').AsString;
        Sheet.Cells[Row, 4] := shoetest.FieldByName('YPCC').AsString;
        Sheet.Cells[Row, 5] := shoetest.FieldByName('FinishedShoe').AsFloat;
        Sheet.Cells[Row, 6].NumberFormat := '#,##0.00'; // hien so thap phan
        Sheet.Cells[Row, 7] := ''; // Ngay giao de den C9
        Sheet.Cells[Row, 8] := ''; // Ngay phong mau nhan de
        Inc(Row);
        shoetest.Next;
      end;

      // --- Ke vien ---
      Sheet.Range['A4', 'G' + IntToStr(Row - 1)].Borders.LineStyle := 1;
      Sheet.Columns.AutoFit;

      // --- Footer ---
      Row := Row + 2;
      Sheet.Cells[Row, 2] := 'CHU QUAN';
      Sheet.Cells[Row, 4] := 'CAN BO';
      Sheet.Cells[Row, 6] := 'LAP BIEU';
      Excel.Visible := True; 
      ShowMessage('Success');
    except
      on E: Exception do
...
procedure TSamplePlan.Edit2KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
