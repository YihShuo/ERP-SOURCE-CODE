...
type
  TScanTBReceive1 = class(TForm)
    ...
    chkC9PT: TCheckBox;
    CheckBoxC9: TDBCheckBox;
    Query4C9: TBooleanField;
    Query1C9: TBooleanField;
...
begin
   ...
   if CheckBoxC9.Checked then
      CheckBoxC9.color:=cllime;
   ...
   if not CheckBoxC9.Checked then
      CheckBoxC9.color:=clMenu;


end;

procedure TScanTBReceive1.Edit1KeyPress(Sender: TObject; var Key: Char);
var chk1,chk2,chk3,chk4,chk5,chk6,chk7,chk8,chk9,chk10,chk11: String;
begin
    chk1 := '0';
    chk2 := '0';
    chk3 := '0';
  ...
  begin
    ...
    begin
      Active:=false;
      SQL.Clear;
      SQL.Add('select stage,shoetest.testno,ETC,Article,TBReceive,TBReceiveDate,lamination,Sublimation,E_embroidery,Printing,lamination_CRMTO,kietphong,Amico,Limao,Laser,DAJU,C9,GCuserid,GCuserdate,shipDate,finish from shoetest');
    ...
    begin
      active:=false;
      SQL.Clear;
      SQL.Add(' Select ypdh,ETC From Shoetest  Where Testno='''+edit1.text+''' ');
      active:=true;
    end;
    button1.Click;
    if (query1.FieldByName('finish').Value = '1') then
    begin
      ShowMessage('TB Receive is Finish.');
      Exit;
    end
    else if (query1.FieldByName('TBReceiveDate').AsDateTime > query1.FieldByName('ETC').AsDateTime) then
    begin
      ShowMessage('TB Receive Date is later than ETC, excution denied.');
      Exit;
    end
    else if (query1.FieldByName('ShipDate').IsNull = false) then
    begin
      if (query1.FieldByName('TBReceiveDate').AsDateTime > query1.FieldByName('ShipDate').AsDateTime) then
      begin
        ShowMessage('TB Receive Date is later than ShipDate, excution denied.');
        Exit;
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          SQL.Add('update shoetest set lamination='+chk1+',Sublimation='+chk2+',E_embroidery='+chk3+',Printing='+chk4+',lamination_CRMTO='+chk5+',kietphong='+chk6+',Amico='+chk7+',Limao='+chk8+',Laser='+chk9+',C9='+chk11+',gcuserid='''+main.edit1.text+''',gcuserdate=getdate(),DAJU='+chk10+'');
          if edit4.text<>'' then
            SQL.Add(',ETC='''+edit4.text+'''');
          if CheckBox11.Checked = true then
            //SQL.Add(',CutDate='''+formatdatetime('YYYY/MM/DD',now)+'''')
            //2025.05.07
            SQL.Add(',CutDate='''+formatdatetime('YYYY/MM/DD',Query2.fieldbyname('ETC').Value)+'''')
          else
            SQL.Add(',CutDate=null');
          if CheckBox12.Checked = true then
            //SQL.Add(',StitchingDate='''+formatdatetime('YYYY/MM/DD',now)+'''')
            //2025.05.07
            SQL.Add(',StitchingDate='''+formatdatetime('YYYY/MM/DD',Query2.fieldbyname('ETC').Value)+'''')
          else
            SQL.Add(',StitchingDate=null');
          if CheckBox13.Checked = true then
            //SQL.Add(',Assemblecomplete='''+formatdatetime('YYYY/MM/DD',now)+'''')
            //2025.05.07
            SQL.Add(',Assemblecomplete='''+formatdatetime('YYYY/MM/DD',Query2.fieldbyname('ETC').Value)+'''')
          else
            SQL.Add(',Assemblecomplete=null');
          if CheckBox14.Checked = true then
            SQL.Add(',PreparationDate='''+formatdatetime('YYYY/MM/DD',now)+'''')
          else
            SQL.Add(',PreparationDate=null');
          if CheckBox15.Checked = true then
            SQL.Add(',S_material='''+formatdatetime('YYYY/MM/DD',now)+'''')
          else
            SQL.Add(',S_material=null');
          if CheckBox16.Checked = true then
            SQL.Add(',insole_send='''+formatdatetime('YYYY/MM/DD',now)+'''')
          else
            SQL.Add(',insole_send=null');
          if CheckBox17.Checked = true then
            SQL.Add(',lace_send='''+formatdatetime('YYYY/MM/DD',now)+'''')
          else
            SQL.Add(',lace_send=null');
          if CheckBox18.Checked = true then
          begin
            if copy(Query2.FieldByName('ypdh').AsString,4,1) <>'V' then
            begin
              //SQL.Add(',complete='''+formatdatetime('YYYY/MM/DD',now)+'''');
              //2025.05.07
              SQL.Add(',complete='''+formatdatetime('YYYY/MM/DD',Query2.fieldbyname('ETC').Value)+'''');
            end;
          end else
          begin
            if copy(Query2.FieldByName('ypdh').AsString,4,1) <>'V' then
            begin
              SQL.Add(',complete=null');
            end;
          end;
          SQL.Add('where Testno='''+edit1.text+'''');
        end;
        ...
        SQL.Add('select stage,testno,Article,ETC,TBReceive,TBReceiveDate,lamination,Sublimation,E_embroidery,Printing,lamination_CRMTO,kietphong,Amico,Limao,Laser,DAJU,C9,ShipDate,finish from shoetest where Testno='''+edit1.text+'''');
        Active:=true;
      end;
      //showmessage('Already');
      Edit1.Clear;
...
begin
    ...
    begin
      Active:=false;
      SQL.Clear;
      SQL.Add('update shoetest set TBReceive =''1'',TBReceiveDate=getdate(),lamination='+chk1+',Sublimation='+chk2+',E_embroidery='+chk3+',Printing='+chk4+',lamination_CRMTO='+chk5+',kietphong='+chk6+',Amico='+chk7+',Limao='+chk8+',Laser='+chk9+',C9='+chk11+',DAJU='+chk10+'');
      if edit4.text<>'' then
         SQL.Add(',ETC='''+edit4.text+'''');
      if CheckBox11.Checked = true then
        //SQL.Add(',CutDate='''+formatdatetime('YYYY/MM/DD',now)+'''')
...
begin
    ...
    begin
      Active:=false;
      SQL.Clear;
      SQL.Add('select stage,shoetest.testno,Article,shoetest.ETC,TBReceive,TBReceiveDate,lamination,Sublimation,E_embroidery,Printing,lamination_CRMTO,kietphong,Amico,Limao,Laser,GCuserid,GCuserdate,DAJU,C9 from shoetest ');
...
begin
    ...
    SQL.Add('     when zsywjc=''JNG R3''  then ''2''');
    ...
    SQL.Add('LEFT JOIN zszl zszl ON clzhzl.zsdh = zszl.zsdh ');
    SQL.Add('LEFT JOIN zszl zszl_dev on clzhzl_dev.zsdh = zszl_dev.zsdh ');
    SQL.Add(')  YPZLS where zsywjc in (''BAO HE'',''LiMao'',''kiet phong'',''Amico'',''CRMTO'',''JNG'',''TIENHUNG'',''DA JU'',''Jia Hua'',''Thang Kim'',''JNG R3'') ');
  ...
  first;
      ...
      begin
        ...
        if (query3.fieldbyname('zsywjc').value='THANG KIM') then
           checkbox9.Checked := true;
        if (query3.fieldbyname('zsywjc').value='DA JU') then
           checkbox10.Checked := true;
        if (query3.fieldbyname('zsywjc').value='JNG R3') then
           chkC9PT.Checked := true;
      next;
      end;
