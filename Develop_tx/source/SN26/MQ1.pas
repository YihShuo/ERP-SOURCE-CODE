...
type
  TMQ = class(TForm)
    ...
    Label13: TLabel;
    Edit10: TEdit;
    MaterialQryCLBH_Child: TStringField;
    strngfldMaterialQryNCU_CLBH_Child: TStringField;
    lbl1: TLabel;
    edt1: TEdit;
...
begin
  ...
  begin
    Active:=false;
    SQL.Clear;
    SQL.Add('select kfxxzl.devtype,kfxxzl.fd,ypzl.quantity,kfxxzl.devcode,kfxxzl.jijie,ypzl.KFJD,ypzls.CLBH');
    SQL.Add(',case when clzhzl.cldh1 is not null then clzhzl.cldh1 else ypzls.clbh end as CLBH_Child');
    //26.05.2026 add column zszl.zsywjc
    SQL.Add(',clzl.ywpm,clzl.zwpm,clzl_printing.Printing_Status,clzl_printing.PrintingUserid,clzl_printing.PrintingUserdate,clzl_printing.Printing_Memo,bwzl.ywsm, zszl.zsywjc NCU_CLBH_Child from YPZL');
    ...
    SQL.Add('left join zszl on zszl.zsdh = case when clzhzl_dev.zsdh is not null and clzhzl_dev.zsdh <> '''' then clzhzl_dev.zsdh else clzhzl.zsdh end');
    ...
    if Edit10.text<>'' then
       SQL.Add('and isnull(clzhzl.cldh1,ypzls.clbh) like '''+Edit10.Text+'%'' ');
    //26.05.2026 add column zszl.zsywjc
    if edt1.text<>'' then
       SQL.Add('and zszl.zsywjc like  ''%'+edt1.Text+'%'' ');
    SQL.Add('group by kfxxzl.devtype,kfxxzl.fd,ypzl.quantity,kfxxzl.devcode,kfxxzl.jijie,ypzl.KFJD,ypzls.CLBH,clzhzl.cldh1');
    //26.05.2026 add column zszl.zsywjc
    SQL.Add(',clzl.ywpm,clzl.zwpm,clzl_printing.Printing_Status,clzl_printing.PrintingUserid,clzl_printing.PrintingUserdate,clzl_printing.Printing_Memo,bwzl.ywsm,zszl.zsywjc');
