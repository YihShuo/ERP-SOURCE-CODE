...
begin
  ...
  begin
      ...
      if query2.FieldByName('Pending_S').value<>'1' then
      begin
        if query2.FieldByName('StitchingDate').Value = null then
        begin
          if query2.FieldByName('StitchingDate_First').Value <= formatdatetime('YYYY/MM/DD',now) then
          begin
            fill := 0;
            SacnTBStitching();
          end else
...
begin
    ...
    begin
      ...
      if fill = 0 then
        ...
        SQL.Add('update shoetest set Pending_S =''1'',StitchingDate=getdate(), StitchingID = ''' + main.Edit1.Text + '''  where Testno='''+edit2.text+'''')
      else
        //SQL.Add('update shoetest set Pending_S =''1'',StitchingDate=getdate(),StitchingDate_First = getdate()  where Testno='''+edit2.text+'''');
        SQL.Add('update shoetest set Pending_S =''1'',StitchingDate=getdate(),StitchingDate_First = getdate(), StitchingID = ''' + main.Edit1.Text + '''  where Testno='''+edit2.text+'''');
      ExecSQL;   //2025.10.21
      SQL.Clear;
      SQL.Add('select testno,Article,TBReceive,Pending_S,StitchingDate,convert(varchar, StitchingDate_first, 111) as StitchingDate_first from shoetest where Testno='''+edit2.text+'''');
      Active:=true;
