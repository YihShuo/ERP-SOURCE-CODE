...
type
  TSampleTracking_Pro = class(TForm)
    MasPanel: TPanel;
    BB2: TBitBtn;
    BB3: TBitBtn;
    BB4: TBitBtn;
    ...
    Label17: TLabel;
    Label18: TLabel;
    SPSCMasCutDate: TDateTimeField;
    SPSCMasCutExDate: TDateTimeField;
    SPSCMasSteDate: TDateTimeField;
    SPSCMasSteExDate: TDateTimeField;
    SPSCMasAseDate: TDateTimeField;
    SPSCMasAseExDate: TDateTimeField;
...
var
  SampleTracking_Pro: TSampleTracking_Pro;
  NDate:TDate;
implementation
   uses main1,SampleTrackReason1,SampleTrackingRms1,  FunUnit;
{$R *.dfm}

procedure TSampleTracking_Pro.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TSampleTracking_Pro.FormDestroy(Sender: TObject);
begin
   SampleTracking_Pro:=nil;
end;
//
procedure TSampleTracking_Pro.Button1Click(Sender: TObject);
var i:integer;
begin
   //
   with SPSCMas do
   begin
     ...
     SQL.Add('  SPSCList.CutDate,(Select MAX(ExeDate) from SPSCRm A where A.SPNO=SPSCList.SPNO and ItemNM=''CutExDate'') as CutExDate, ');
     SQL.Add('  SPSCList.SteDate,(Select MAX(ExeDate) from SPSCRm A where A.SPNO=SPSCList.SPNO and ItemNM=''SteExDate'') as SteExDate, ');
     SQL.Add('  SPSCList.AseDate,(Select MAX(ExeDate) from SPSCRm A where A.SPNO=SPSCList.SPNO and ItemNM=''AseExDate'') as AseExDate, ');
   ...
   DBGridEh2.Columns[0].PickList.add('CutExDate');
   DBGridEh2.Columns[0].PickList.add('SteExDate');
   DBGridEh2.Columns[0].PickList.add('AseExDate');
   //
   ToolPanel.Visible:=false;
end;
//
procedure TSampleTracking_Pro.BB2Click(Sender: TObject);
begin

end;
//
procedure TSampleTracking_Pro.BB3Click(Sender: TObject);
begin

end;
//
procedure TSampleTracking_Pro.BB4Click(Sender: TObject);
begin

end;
//
procedure TSampleTracking_Pro.BB6Click(Sender: TObject);
begin

end;

//儲存
procedure TSampleTracking_Pro.FormCreate(Sender: TObject);
begin
  //
  with tmpQuery do
  begin
...
procedure TSampleTracking_Pro.DBGridEh1DrawColumnCell(Sender: TObject;
...
begin
  ...
  //
  if ((SPSCMas.FieldByName('CutExDate').IsNull=false) and (SPSCMas.FieldByName('SteExDate').IsNull=false) and (SPSCMas.FieldByName('AseExDate').IsNull=false)  )  then
  begin
...
procedure TSampleTracking_Pro.SPSCMasAfterOpen(DataSet: TDataSet);
begin
...
procedure TSampleTracking_Pro.RB1Click(Sender: TObject);
begin
...
procedure TSampleTracking_Pro.RB2Click(Sender: TObject);
begin
  //
  with SPSCDet do
  begin
...
procedure TSampleTracking_Pro.DBGridEh2DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumnEh;
  State: TGridDrawState);
begin
  //
...
procedure TSampleTracking_Pro.RB3Click(Sender: TObject);
begin
  //
  with SPSCDet do
  begin
...
procedure TSampleTracking_Pro.RB5Click(Sender: TObject);
begin
  //
  with SPSCDet do
    begin
...
//
procedure TSampleTracking_Pro.RB4Click(Sender: TObject);
var i,RENO:integer;
begin
  //
  SPSCDet.First;
...
procedure TSampleTracking_Pro.DBGridEh2EditButtonClick(Sender: TObject);
begin
  //
  with DBGridEh2 do
  begin
...
//
procedure TSampleTracking_Pro.BB1Click(Sender: TObject);
begin
  ToolPanel.Visible:=true;
end;
//
procedure TSampleTracking_Pro.BitBtn1Click(Sender: TObject);
begin
  //
  if SampleTrackingRms=nil then
    SampleTrackingRms:=TSampleTrackingRms.Create(self);
  SampleTrackingRms.SetDepartment('Production');
  SampleTrackingRms.Show;
  //
end;

procedure TSampleTracking_Pro.SPSCDetItemNMSetText(Sender: TField;
  const Text: String);
begin
  if Text='CutExDate' then  SPSCDet.FieldByName('ExeDate').Value:=SPSCMas.FieldByName('CutDate').Value;
  if Text='SteExDate' then  SPSCDet.FieldByName('ExeDate').Value:=SPSCMas.FieldByName('SteDate').Value;
  if Text='AseExDate' then  SPSCDet.FieldByName('ExeDate').Value:=SPSCMas.FieldByName('AseDate').Value;
