...
uses
  ...
  Menus, colorbutton, C_Excel, StrUtils, IdBaseComponent, IdCoder,
...
type
  TPaperRoom = class(TForm)
    ...
    masqryLocationTrial: TStringField;  procedure FormClose(Sender: TObject; var Action: TCloseAction);
    ...
    procedure PrintRreview1Click(Sender: TObject);
    procedure pmMaspmPopup(Sender: TObject);
...
begin
  ...
  begin
    ...
    if main.Edit2.Text = 'CDC' then
      ...
      begin
        SQL.Add(',Article=(case when charindex('' / '''+'+ kfxxzl.Article +'+''' / '''+'+ kfxxzl.YSSM'+',shoetest.Article ) > 0 ');
        SQL.Add(' then shoetest.Article ');
        SQL.Add('else shoetest.Article +'+''' / '''+'+ kfxxzl.Article +'+''' / '''+'+ kfxxzl.YSSM end)');
      end;
    ...
    SQL.Add(','''' as YN');
    SQL.Add(',xxzlkf.CTS');
    SQL.Add(',shoetest.SeeType,shoetest.Print_Request,shoetest.Print_Check,shoetest.Rubber_Check,shoetest.Rubber_Request,shoetest.LocationTrial');
    ...
    sql.Add(',shoetest.LocationTrial');
          ...
          begin
            ...
            begin
               ...
               begin
                 ...
                 sql.Add(' and ISNUMERIC(RIGHT(TestNo,LEN(TestNo)-4) )=1');                 
                 sql.add('order by LEN(TestNo), TestNo');
               //  memo1.text:=sql.text;
                 active:=true;
...
begin
  ...
  or (masqry.FieldByName('stage').AsString='SE3') or (masqry.FieldByName('stage').AsString='SE4') or (masqry.FieldByName('stage').AsString='PU1')
  or (masqry.FieldByName('stage').AsString='PU2') or (masqry.FieldByName('stage').AsString='PU3') or (masqry.FieldByName('stage').AsString='PU4')  then
  begin
     SeeExcel();
  end else
  begin
...
begin
  ...
  begin
    SBookmark := TBookmark(dbgrdh1.SelectedRows[i]);
    masqry.GotoBookmark(SBookmark);
    if (TestNo = '') then
      TestNo := '''' + masqry.FieldByName('TestNo').AsString + ''''
    else
      TestNo := TestNo + ',''' + masqry.FieldByName('TestNo').AsString + '''';
  end;

  PapaerRoom_print := TPapaerRoom_print.Create(Self);
  with PapaerRoom_print.PreviewQry do
  begin
    Active := false;
    SQL.Clear;
    SQL.Add('IF OBJECT_ID(''tempdb..#SS'') IS NOT NULL');
    SQL.Add('BEGIN DROP TABLE #SS END;');

    SQL.Add('SELECT ShoeTest2.TestNo, ''Status:'' + purpose.YWSM + ''('' + purpose.VWSM + '')___Parts:'' + part.YWSM + ''('' + part.VWSM + '')___Qty:'' + CAST(CAST(ShoeTest2.Qty AS Float) AS VARCHAR) AS Status INTO #SS FROM ShoeTest2');
    SQL.Add('LEFT JOIN shoetestDetail AS purpose on purpose.YYBH = shoetest2.DepName');
    SQL.Add('LEFT JOIN shoetestDetail AS part on part.YYBH = shoetest2.parts');
    SQL.Add('WHERE ShoeTest2.TestNo IN (' + TestNo + ')');
    SQL.Add('GROUP BY ShoeTest2.TestNo, purpose.YWSM, purpose.VWSM, part.YWSM, part.VWSM, ShoeTest2.Qty');

    SQL.Add('SELECT ''LYNDC Sample Commercialization Sheet (Sample No: '' + ShoeTest.YPDH + '')'' AS YPDH, ShoeTest.TestNo, CAST(NULL AS VARBINARY(MAX)) AS Barcode,');
    SQL.Add('KFZL.KFJC AS Brand, KFXXZL.DevType AS DevTp, YPZL.SubType AS SubTp, YPZL.KFJD AS Stage, ShoeTest.SeeType,');
    SQL.Add('ShoeTest.TestTime, YPZL.FD, KFXXZL.JiJie AS Season, ShoeTest.Article + '' / '' + KFXXZL.Article + '' / '' + KFXXZL.YSSM AS Article, KFXXZL.XieMing,');
    SQL.Add('ShoeTest.Construction, ShoeTest.Purpose, YPZL.YPCC AS SIZ, ISNULL(shoe.ShoeQTY,0) AS Qty, ISNULL(upper.UpperQty, 0) AS UpperQty,');
    SQL.Add('ISNULL(Bottom.BottomQty, 0) AS BottomQty, ShoeTest.ETC, ShoeTest.UserDate, ShoeTest.ShipDate, KFXXZL.XTMH AS LastNo, KFXXZL.DAOMH AS CutDieNo,');
    SQL.Add('KFXXZL.DDMH, ShoeTest.Print_Request, ShoeTest.Rubber_Request, CAST(SS.Status AS VARCHAR(MAX)) AS Status,');
    SQL.Add('CAST(''{\rtf1\ansi\deff0{\fonttbl{\f0 Arial;}}\fs20 '' + REPLACE(ShoeTest.Memo1, CHAR(13) + CHAR(10), ''\par '') + ''}'' AS VARCHAR(MAX)) AS Memo1 FROM ShoeTest');
    SQL.Add('LEFT JOIN YPZL ON YPZL.YPDH = ShoeTest.YPDH');
    SQL.Add('LEFT JOIN KFXXZL ON YPZL.XieXing = KFXXZL.XieXing AND YPZL.SheHao = KFXXZL.SheHao');
    SQL.Add('LEFT JOIN KFZL ON KFZL.KFDH = KFXXZL.KHDH');
    SQL.Add('LEFT JOIN (');
    SQL.Add('  SELECT shoetest2.TestNo, sum(shoetest2.Qty) AS ShoeQty FROM shoetest2');
    SQL.Add('  LEFT JOIN shoetestdetail ON shoetest2.parts = shoetestdetail.YYBH');
    SQL.Add('  WHERE shoetest2.TestNo IN (' + TestNo + ')');
    SQL.Add('  AND shoetestdetail.class = ''shoe''');
    SQL.Add('  GROUP BY shoetest2.TestNo');
    SQL.Add(') AS shoe ON shoe.TestNo = ShoeTest.TestNo');
    SQL.Add('LEFT JOIN (');
    SQL.Add('  SELECT shoetest2.TestNo, sum(shoetest2.Qty) AS UpperQty FROM shoetest2');
    SQL.Add('  LEFT JOIN shoetestdetail ON shoetest2.parts = shoetestdetail.YYBH');
    SQL.Add('  WHERE shoetest2.TestNo IN (' + TestNo + ')');
    SQL.Add('  AND shoetestdetail.class = ''upper''');
    SQL.Add('  GROUP BY shoetest2.TestNo');
    SQL.Add(') AS upper ON upper.TestNo = ShoeTest.TestNo');
    SQL.Add('LEFT JOIN (');
    SQL.Add('  SELECT shoetest2.TestNo, sum(shoetest2.Qty) AS BottomQty FROM shoetest2');
    SQL.Add('  LEFT JOIN shoetestdetail ON shoetest2.parts = shoetestdetail.YYBH');
    SQL.Add('  WHERE shoetest2.TestNo IN (' + TestNo + ')');
    SQL.Add('  AND shoetestdetail.class = ''Bottom''');
    SQL.Add('  GROUP BY shoetest2.TestNo');
    SQL.Add(') AS Bottom ON Bottom.TestNo = ShoeTest.TestNo');
    SQL.Add('LEFT JOIN (');
    SQL.Add('  SELECT DISTINCT TestNo, ''{\rtf1\ansi\deff0{\fonttbl{\f0 Arial;}}\fs20 '' + (');
    SQL.Add('    SELECT SS2.Status + ''\par '' FROM #SS SS2');
    SQL.Add('    WHERE SS2.TestNo = SS1.TestNo');
    SQL.Add('    FOR XML PATH('''')');
    SQL.Add('  ) + ''}'' AS Status FROM #SS AS SS1');
    SQL.Add(') AS SS ON SS.TestNo = ShoeTest.TestNo');
    SQL.Add('WHERE ShoeTest.TestNo IN (' + TestNo + ')');  
    SQL.Add('ORDER BY ShoeTest.TestNo DESC');
    Active := true;
    ...
    while not Eof do
    begin
      PapaerRoom_print.GenerateBarcode(FieldByName('TestNo').AsString);
      Next;
    end;
    First;
  end;
    ...
    begin
      ...
      try
        ...
        //eclApp.Cells(1,1):='LYNDC Sample Commercialization Sheet(SampleNO: '+Masqry.FieldByName('ypdh').AsString+')';
         eclApp.Cells(3,4) := 'NO:'+ Masqry.FieldByName('TESTNO').AsString;
         eclApp.Cells[3,4].Font.Size := 25;
         eclApp.Cells(4,4) := 'Article:'+ Masqry.FieldByName('Article').AsString;
         eclApp.Cells[4,4].Font.Size := 25;
  ...
  begin
   ...
   //   Picture := eclApp.WorkSheets[2].Pictures.Insert(GetCurrentDir + '\Barcode.jpg');
     ...
     Picture.Height := 35;
      ...
      eclApp.Cells[7,1].WrapText := True;
     ...
     // eclApp.Cells(13,1) := 'Print_Request'+ Masqry.FieldByName('Print_Request').AsString;
      ...
      eclApp.Cells(12,1) := 'Location Trial:' + Masqry.FieldByName('LocationTrial').AsString;
...
procedure TPaperRoom.PrintRreview1Click(Sender: TObject);
begin
  {IDCard_Picture := TIDCard_Picture.Create(Self);
  IDCard_Picture.Init;
  IDCard_Picture.QuickRep1.Preview;
  IDCard_Picture.Free;
  SampleSheet_print   }                         
end;

procedure TPaperRoom.pmMaspmPopup(Sender: TObject);
begin
  Print.Visible := (dbgrdh1.SelectedRows.Count > 0);
end;
