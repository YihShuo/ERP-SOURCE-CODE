...
uses
  ...
  GanttCh, Buttons, comobj, IniFiles;
...
type
  TDFC = class(TForm)
    ...
    DSHN: TDataSource;
    QryHN: TQuery;
    QryHNGROUPS: TStringField;
    QryHNgroups2: TStringField;
    Button14: TButton;
    DBGridEh16: TDBGridEh;
    cbkC2: TCheckBox;
    DTP8: TDateTimePicker;
    DTP9: TDateTimePicker;
    cbk_excluding: TCheckBox;
    QryHNSRQtyCuttingPlan: TIntegerField;
    QryHNQtyCuttingPlan: TFloatField;
    QryHNSRQtyCuttingFinished: TIntegerField;
    QryHNQtyCuttingFinished: TFloatField;
    QryHNSRQtyStitchingPlan: TIntegerField;
    QryHNQtyStitchingPlan: TFloatField;
    QryHNSRQtyStitchingFinished: TIntegerField;
    QryHNQtyStitchingFinished: TFloatField;
    QryHNSRQtyAssemblyPlan: TIntegerField;
    QryHNQtyAssemblyPlan: TFloatField;
    QryHNSRQtyAssemblyFinished: TIntegerField;
    QryHNQtyAssemblyFinished: TFloatField;
    QryHNSRQtyPackingFinished: TIntegerField;
    QryHNQtyPackingFinished: TFloatField;
    cboProcess: TComboBox;
    Label18: TLabel;
    edtUser: TEdit;
    CheckBox2: TCheckBox;
    CheckBox3: TCheckBox;
    CheckBox4: TCheckBox;
    R1_R4_PC_SKU: TStringField;
    R2_R3_ADD_WearFitTest_SKU: TStringField;
    MST_EXT_FSR_SKU: TStringField;
    SEE_Pullover_SKU: TStringField;
    QUV_OtherTest_SKU: TStringField;
    Shoe_SKU: TStringField;
    half_SKU: TStringField;
    gsbhCb: TComboBox;
    gsbhCbLabel: TLabel;
    ...
    procedure cbkC2Click(Sender: TObject);
    procedure cbk_excludingClick(Sender: TObject);
...
uses
  MQ1, FunUnit, main1, Math, StrUtils;
...
with SampleQry do
      ...
      sql.add('isnull(SUM(DISTINCT ReceiveSR),0) ReceiveSR,isnull(SUM(DISTINCT CutMaterialOK),0) CutMaterialOK,isnull(SUM(DISTINCT CutMaterialGet),0) CutMaterialGet,isnull(SUM(DISTINCT cutdate),0) cutdate,');
      sql.add('isnull(SUM(DISTINCT StitchingMaterialOK),0) StitchingMaterialOK,isnull(sum(DISTINCT StitchingMaterialGet),0) StitchingMaterialGet,isnull(SUM(DISTINCT StitchingDate),0) StitchingDate,');
      sql.add('isnull(SUM(DISTINCT AssemblyMaterialOK),0) AssemblyMaterialOK,isnull(sum(DISTINCT AssemblyMaterialGet),0) AssemblyMaterialGet,isnull(SUM(DISTINCT AssemblyDate),0) AssemblyDate,');
      sql.add('isnull(SUM(DISTINCT PackingMaterialOK),0) PackingMaterialOK,isnull(SUM(DISTINCT PackingMaterialGet),0) PackingMaterialGet,isnull(SUM(DISTINCT PackingDate),0) PackingDate from(');
      sql.add('select DISTINCT shoetestplanshow.Groups,');
      //sql.add(',shoetestplanshow.type');
      sql.add('COUNT(DISTINCT shoetestplan.Serino) ReceiveSR,CutMaterialOK.CutMaterialOK,CutMaterialGet.CutMaterialGet,cutdate.Cutdate,');
      ...
      sql.add('left join (');
      sql.add('	select shoetestplanshow.Groups,COUNT(DISTINCT shoetestplan.Serino) CutMaterialOK from shoetestplanshow');
      sql.add('	left join shoetestplan on shoetestplan.groups=shoetestplanshow.groups and shoetestplanshow.type=left(shoetestplan.ypdh,4)');
      sql.add('	left join shoetest on shoetest.testno=shoetestplan.testno');
      sql.add('	where preparationDate is not null');
      sql.add('	group by shoetestplanshow.Groups) CutMaterialOK on CutMaterialOK.Groups=shoetestplanshow.Groups ');
      sql.add('left join (');
      sql.add('	select shoetestplanshow.Groups,COUNT(DISTINCT shoetestplan.Serino) CutMaterialGet from shoetestplanshow');
      sql.add('	left join shoetestplan on shoetestplan.groups=shoetestplanshow.groups and shoetestplanshow.type=left(shoetestplan.ypdh,4)');
      sql.add('	left join shoetest on shoetest.testno=shoetestplan.testno');
      sql.add('	where FirstMatOutDate is not null');
      sql.add('	group by shoetestplanshow.Groups) CutMaterialGet on CutMaterialGet.Groups=shoetestplanshow.Groups ');
      sql.add('left join (');
      sql.add('	select shoetestplanshow.Groups,COUNT(DISTINCT shoetestplan.Serino) cutdate from shoetestplanshow');
      sql.add('	left join shoetestplan on shoetestplan.groups=shoetestplanshow.groups and shoetestplanshow.type=left(shoetestplan.ypdh,4)');
      sql.add('	left join shoetest on shoetest.testno=shoetestplan.testno');
      sql.add('	where cutdate is not null');
      sql.add('	group by shoetestplanshow.Groups) cutdate on cutdate.Groups=shoetestplanshow.Groups ');
      sql.add('left join (');
      sql.add('	select shoetestplanshow.Groups,COUNT(DISTINCT shoetestplan.Serino) StitchingMaterialOK from shoetestplanshow');
      sql.add('	left join shoetestplan on shoetestplan.groups=shoetestplanshow.groups and shoetestplanshow.type=left(shoetestplan.ypdh,4)');
      sql.add('	left join shoetest on shoetest.testno=shoetestplan.testno');
      sql.add('	where S_material is not null');
      sql.add('	group by shoetestplanshow.Groups) StitchingMaterialOK on StitchingMaterialOK.Groups=shoetestplanshow.Groups ');
      sql.add('left join (');
      sql.add('	select shoetestplanshow.Groups,COUNT(DISTINCT shoetestplan.Serino) StitchingMaterialGet from shoetestplanshow');
      sql.add('	left join shoetestplan on shoetestplan.groups=shoetestplanshow.groups and shoetestplanshow.type=left(shoetestplan.ypdh,4)');
      sql.add('	left join shoetest on shoetest.testno=shoetestplan.testno');
      sql.add('	where StitchingPhuFinishDate is not null');
      sql.add('	group by shoetestplanshow.Groups) StitchingMaterialGet on StitchingMaterialGet.Groups=shoetestplanshow.Groups ');
      sql.add('left join (');
      sql.add('	select shoetestplanshow.Groups,COUNT(DISTINCT shoetestplan.Serino) stitchingDate from shoetestplanshow');
      sql.add('	left join shoetestplan on shoetestplan.groups=shoetestplanshow.groups and shoetestplanshow.type=left(shoetestplan.ypdh,4)');
      sql.add('	left join shoetest on shoetest.testno=shoetestplan.testno');
      sql.add('	where stitchingDate is not null');
      sql.add('	group by shoetestplanshow.Groups) stitchingDate on stitchingDate.Groups=shoetestplanshow.Groups ');

      sql.add('left join (');
      sql.add('	select shoetestplanshow.Groups,COUNT(DISTINCT shoetestplan.Serino) AssemblyMaterialOK from shoetestplanshow');
      sql.add('	left join shoetestplan on shoetestplan.groups=shoetestplanshow.groups and shoetestplanshow.type=left(shoetestplan.ypdh,4)');
      sql.add('	left join shoetest on shoetest.testno=shoetestplan.testno');
      sql.add('	where insole_send is not null');
      sql.add('	group by shoetestplanshow.Groups) AssemblyMaterialOK on AssemblyMaterialOK.Groups=shoetestplanshow.Groups ');
      sql.add('left join (');
      sql.add('	select shoetestplanshow.Groups,COUNT(DISTINCT shoetestplan.Serino) AssemblyMaterialGet from shoetestplanshow');
      sql.add('	left join shoetestplan on shoetestplan.groups=shoetestplanshow.groups and shoetestplanshow.type=left(shoetestplan.ypdh,4)');
      sql.add('	left join shoetest on shoetest.testno=shoetestplan.testno');
      sql.add('	where insole_get is not null');
      sql.add('	group by shoetestplanshow.Groups) AssemblyMaterialGet on AssemblyMaterialGet.Groups=shoetestplanshow.Groups ');
      sql.add('left join (');
      sql.add('	select shoetestplanshow.Groups,COUNT(DISTINCT shoetestplan.Serino) AssemblyDate from shoetestplanshow');
      sql.add('	left join shoetestplan on shoetestplan.groups=shoetestplanshow.groups and shoetestplanshow.type=left(shoetestplan.ypdh,4)');
      sql.add('	left join shoetest on shoetest.testno=shoetestplan.testno');
      sql.add('	where Assemblecomplete is not null');
      sql.add('	group by shoetestplanshow.Groups) AssemblyDate on AssemblyDate.Groups=shoetestplanshow.Groups ');

      sql.add('left join (');
      sql.add('	select shoetestplanshow.Groups,COUNT(DISTINCT shoetestplan.Serino) PackingMaterialOK from shoetestplanshow');
      sql.add('	left join shoetestplan on shoetestplan.groups=shoetestplanshow.groups and shoetestplanshow.type=left(shoetestplan.ypdh,4)');
      sql.add('	left join shoetest on shoetest.testno=shoetestplan.testno');
      sql.add('	where lace_send is not null');
      sql.add('	group by shoetestplanshow.Groups) PackingMaterialOK on PackingMaterialOK.Groups=shoetestplanshow.Groups ');
      sql.add('left join (');
      sql.add('	select shoetestplanshow.Groups,COUNT(DISTINCT shoetestplan.Serino) PackingMaterialGet from shoetestplanshow');
      sql.add('	left join shoetestplan on shoetestplan.groups=shoetestplanshow.groups and shoetestplanshow.type=left(shoetestplan.ypdh,4)');
      sql.add('	left join shoetest on shoetest.testno=shoetestplan.testno');
      sql.add('	where lace_get is not null');
      sql.add('	group by shoetestplanshow.groups) PackingMaterialGet on PackingMaterialGet.Groups=shoetestplanshow.Groups');
      sql.add('left join (');
      sql.add('	select shoetestplanshow.Groups,COUNT(DISTINCT shoetestplan.Serino) PackingDate from shoetestplanshow');
      sql.add('	left join shoetestplan on shoetestplan.groups=shoetestplanshow.groups and shoetestplanshow.type=left(shoetestplan.ypdh,4)');
      sql.add('	left join shoetest on shoetest.testno=shoetestplan.testno');
      sql.add('	where  Complete is not null');
      sql.add('	group by shoetestplanshow.groups) PackingDate on PackingDate.Groups=shoetestplanshow.Groups ');
      sql.add('group by shoetestplanshow.groups,CutMaterialOK,CutMaterialGet,cutdate,');
      sql.add('AssemblyMaterialOK.AssemblyMaterialOK,AssemblyMaterialGet.AssemblyMaterialGet,AssemblyDate.AssemblyDate,');
...
begin
  ReadIni();
  if gsbhCb.Items.IndexOf(main.Edit2.Text) < 0 then
  begin
     gsbhCb.Items.Insert(0, main.Edit2.Text);
     gsbhCb.ItemIndex := 0;
  end
  else
  begin
    gsbhCb.ItemIndex := gsbhCb.Items.IndexOf(main.Edit2.Text);
  end;

  PageControl1.ActivePageIndex := 1;
  if (main.Edit2.Text <> 'CDC') and (main.Edit2.Text <> 'SKX') and (main.Edit2.Text <> 'TBDC') and (main.Edit2.Text <> 'VC1') then begin
    //PageControl1.ActivePageIndex := 1;
    exit;
  end;
  with query1 do
  ...
  DTP2.Date := Date;
  DTP8.Date := Date;
  DTP9.Date := IncMonth(DTP8.Date, 1) - 1;
  DTP111.Date := Date-1;
end;
...
begin
    with CutQry do
      ...
      //13.12.2024
      sql.add('and ypzl.GSDH= '''+gsbhCb.Text+'''');
...
begin
    with StitchingQry do
      ...
      sql.add('and ypzl.GSDH= '''+gsbhCb.Text+'''');
      //sql.add('and YWSM in (''Finished Shoe'',''Half Stitching Upper'',''Stitching Upper'',''Lasted upper'',''Upper Component'',''CFM sample'',''Upper And Bottom(CE)'')');
      sql.add('and YWSM in (''Finished Shoe'',''Half Stitching Upper'',''Stitching Upper'',''Lasted upper'',''Upper Component'',''CFM sample'')');
      sql.add(')main');
      sql.add('group by ywsm)planning');
      sql.add('full outer join (');
      sql.add('select ywsm,SUM(Qty) SampleRoomQty from (');
...
begin
    with AssemblyQry do
      ...
      sql.add('and YWSM in (''Finished Shoe'',''Lasted upper'',''CFM sample'')');
      sql.add(')main');
      sql.add('group by ywsm)planning');
      sql.add('full outer join (');
      sql.add('select ywsm,SUM(Qty) SampleRoomQty from (');
...
begin
    with PackingQry do
      ...
      sql.add('and YWSM in (''Finished Shoe'',''CFM sample'')');
      sql.add(')main');
...
begin//生產樣品出貨雙數(XT GIAY):R1,R4,PC
    with R1_R4_PC_ do
      ...
      active:=false;
      sql.Clear;
      sql.add('select shoetest.testno,shoetest.SQTBOKTime,sum(shoetest2.qty) R1_R4_PC,parts.ywsm,kfxxzl.Article as SKU from shoetest');
      sql.add('left join shoetest2 on shoetest2.TestNo=shoetest.TestNo');
      ...
      //====27.06.2025
      //====05.09.2025 them xet dieu kien don hang VC1 LYN-IT04250900007
      if main.Edit2.Text ='VC1'then
        begin
          //sql.add('      and shoetestdetail.YYBH in (''MD01'',''MD06'',''MD08'') and left(shoetest.YPDH,4)  in (''CUIK'',''UQIK'')');
          sql.Add('and shoetestdetail.YYBH in (''MD01'',''MD06'',''MD08'') and left(shoetest.YPDH,4) in (''CUIK'',''UQIK'',''AZIK'',''VSIK'')');
        end else
        begin
          sql.add('      and shoetestdetail.YYBH in (''MD01'',''MD06'',''MD08'') and left(shoetest.YPDH,4) in (''LAIK'',''BYIK'',''SKXK'',''CBYK'')');
        end;
      ...
      sql.add('group by shoetest.testno,shoetest.SQTBOKTime,parts.ywsm,kfxxzl.Article');
...
begin//生產樣品出貨雙數(XT GIAY):R2,R3,ADD,wear/fit test
    with R2_R3_ADD_WearFitTest_ do
      ...
      active:=false;
      sql.Clear;
      sql.add('select shoetest.testno,shoetest.SQTBOKTime,sum(shoetest2.qty) R2_R3_ADD_WearFitTest,parts.ywsm,kfxxzl.Article as SKU from shoetest');
      sql.add('left join shoetest2 on shoetest2.TestNo=shoetest.TestNo');
      sql.add('left join shoetestdetail on shoetestdetail.YYBH=shoetest2.DepName');
      sql.add('left join shoetestdetail parts on parts.YYBH=shoetest2.parts');
      SQL.add('left join ypzl on ypzl.YPDH=shoetest.YPDH');
      ...
      if main.Edit2.Text ='VC1'then
        begin
          //sql.add('      and shoetestdetail.YYBH in (''MD01'',''MD06'',''MD08'') and left(shoetest.YPDH,4)  in (''CUIV'',''UQIV'')');
          sql.Add('and shoetestdetail.YYBH in (''MD01'',''MD03'',''MD04'') and left(shoetest.YPDH,4) in (''CUIK'',''UQIK'',''AZIK'',''VSIK'')');
        end else
        begin
          sql.add('            and shoetestdetail.YYBH in (''MD01'',''MD03'',''MD04'') and left(shoetest.YPDH,4) in (''LAIV'',''BYIV'')');
        end;
...
begin//技術試作鞋(GIAY PKT):MST, EXT, FSR
    with MST_EXT_FSR_ do
      ...
      active:=false;
      sql.Clear;
      sql.add('select shoetest.testno,shoetest.SQTBOKTime,sum(shoetest2.qty) MST_EXT_FSR,parts.ywsm,kfxxzl.Article as SKU from shoetest');
      sql.add('left join shoetest2 on shoetest2.TestNo=shoetest.TestNo');
      sql.add('left join shoetestdetail on shoetestdetail.YYBH=shoetest2.DepName');
      sql.add('left join shoetestdetail parts on parts.YYBH=shoetest2.parts');
      SQL.add('left join ypzl on ypzl.YPDH=shoetest.YPDH');
      ...
      if main.Edit2.Text ='VC1'then
        begin
          //sql.add('      and shoetestdetail.YYBH in (''MD01'',''MD06'',''MD08'') and left(shoetest.YPDH,4)  in (''CUIT'',''UQIT'')');
          sql.Add('and shoetestdetail.YYBH in (''MD07'') and left(shoetest.YPDH,4) in (''CUIT'',''UQIT'',''AZIT'',''VSIT'')');
        end else
        begin
          sql.add('            and shoetestdetail.YYBH in (''MD07'') and left(shoetest.YPDH,4) in (''LAIT'',''BYIT'')');
        end;
...
begin//試版鞋Giay Lam Th? R?p: SEE-Pullover
    with SEE_Pullover_ do
      ...
      active:=false;
      sql.Clear;
      sql.add('select shoetest.testno,shoetest.SQTBOKTime,sum(shoetest2.qty) SEE_Pullover,parts.ywsm,kfxxzl.Article as SKU from shoetest');
      sql.add('left join shoetest2 on shoetest2.TestNo=shoetest.TestNo');
      sql.add('left join shoetestdetail on shoetestdetail.YYBH=shoetest2.DepName');
      sql.add('left join shoetestdetail parts on parts.YYBH=shoetest2.parts');
      SQL.add('left join ypzl on ypzl.YPDH=shoetest.YPDH');
      ...
      if main.Edit2.Text ='VC1'then
        begin
          //sql.add('      and shoetestdetail.YYBH in (''MD01'',''MD06'',''MD08'') and left(shoetest.YPDH,4)  in (''CUIK'',''UQIK'')');
          sql.Add('and shoetestdetail.YYBH in (''MD15'') and left(shoetest.YPDH,4) in (''CUIK'',''UQIK'',''AZIK'',''VSIK'')');
        end else
        begin
          sql.add('            and shoetestdetail.YYBH in (''MD15'') and left(shoetest.YPDH,4) in (''LAIK'',''BYIK'',''SKXK'',''CBYK'')');
        end;
...
begin//其他測試鞋(Another test) Cac lo?i giay th? : 彎曲/拉力/老化/QUV.. khac QUV,Aging,Giay th? keo + th? nh?p
    with QUV_OtherTest_ do
      ...
      active:=false;
      sql.Clear;
      sql.add('select shoetest.testno,shoetest.SQTBOKTime,sum(shoetest2.qty) QUV_OtherTest,parts.ywsm,kfxxzl.Article as SKU from shoetest');
      sql.add('left join shoetest2 on shoetest2.TestNo=shoetest.TestNo');
      sql.add('left join shoetestdetail on shoetestdetail.YYBH=shoetest2.DepName');
      sql.add('left join shoetestdetail parts on parts.YYBH=shoetest2.parts');
      SQL.add('left join ypzl on ypzl.YPDH=shoetest.YPDH');
      ...
      if main.Edit2.Text ='VC1'then
        begin
          //sql.add('      and shoetestdetail.YYBH in (''MD01'',''MD06'',''MD08'') and left(shoetest.YPDH,4)  in (''CUIK'',''UQIK'')');
          sql.Add('and shoetestdetail.YYBH in (''MD02'',''MD13'',''MD14'') and left(shoetest.YPDH,4) in (''CUIK'',''UQIK'',''AZIK'',''VSIK'')');
        end else
        begin
          sql.add('            and shoetestdetail.YYBH in (''MD02'',''MD13'',''MD14'') and left(shoetest.YPDH,4) in (''LAIK'',''BYIK'',''SKXK'',''CBYK'')');
        end;
...
begin//生產樣品留底雙數(GIAY):R1;R2,R3,R4,PC
    with Shoe_ do
      ...
      active:=false;
      sql.Clear;
      sql.add('select shoetest.testno,shoetest.SQTBOKTime,sum(shoetest2.qty) Shoe,parts.ywsm,kfxxzl.Article as SKU from shoetest');
      sql.add('left join shoetest2 on shoetest2.TestNo=shoetest.TestNo');
      sql.add('left join shoetestdetail on shoetestdetail.YYBH=shoetest2.DepName');
      sql.add('left join shoetestdetail parts on parts.YYBH=shoetest2.parts');
      SQL.add('left join ypzl on ypzl.YPDH=shoetest.YPDH');
      ...
      if main.Edit2.Text ='VC1'then
        begin
          //sql.add('      and shoetestdetail.YYBH in (''MD01'',''MD06'',''MD08'') and left(shoetest.YPDH,4)  in (''CUIK'',''UQIK'') and shoetest2.parts in (''BW01'',''BW10'')');
          sql.Add('and shoetestdetail.YYBH in (''MD09'',''MD05'',''MD11'',''MD16'') and left(shoetest.YPDH,4) in (''CUIK'',''UQIK'',''AZIK'',''VSIK'') and shoetest2.parts in (''BW01'',''BW10'')');
        end else
        begin
          sql.add('            and shoetestdetail.YYBH in (''MD09'',''MD05'',''MD11'',''MD16'') and left(shoetest.YPDH,4)  in (''LAIK'',''BYIK'',''SKXK'',''CBYK'') and shoetest2.parts in (''BW01'',''BW10'')');
        end;
...
begin//生產樣品留底半成品鞋雙數:R1;R2,R3,R4,PC
    with half_ do
      ...
      active:=false;
      sql.Clear;
      sql.add('select shoetest.testno,shoetest.SQTBOKTime,sum(shoetest2.qty) half,parts.ywsm,kfxxzl.Article as SKU from shoetest');
      sql.add('left join shoetest2 on shoetest2.TestNo=shoetest.TestNo');
      sql.add('left join shoetestdetail on shoetestdetail.YYBH=shoetest2.DepName');
      sql.add('left join shoetestdetail parts on parts.YYBH=shoetest2.parts');
      SQL.add('left join ypzl on ypzl.YPDH=shoetest.YPDH');
      ...
      if main.Edit2.Text ='VC1'then
        begin
          //sql.add('      and shoetestdetail.YYBH in (''MD01'',''MD06'',''MD08'') and left(shoetest.YPDH,4)  in (''CUIK'',''UQIK'') and shoetest2.parts not in (''BW01'',''BW10'')');
          sql.Add('and shoetestdetail.YYBH in (''MD09'',''MD05'',''MD11'',''MD16'') and left(shoetest.YPDH,4) in (''CUIK'',''UQIK'',''AZIK'',''VSIK'') and shoetest2.parts not in (''BW01'',''BW10'')');
        end else
        begin
          sql.add('            and shoetestdetail.YYBH in (''MD09'',''MD05'',''MD11'',''MD16'') and left(shoetest.YPDH,4)  in (''LAIK'',''BYIK'',''SKXK'',''CBYK'') and shoetest2.parts not in (''BW01'',''BW10'')');
        end;
...
begin
  ...
  begin
    ...
    SQL.Add(', SUM(CASE WHEN Hour <= 24 THEN Qty else 0 END) AS Total');
    SQL.Add('FROM (');
    ...
    if (CB1.ItemIndex = 5) or (CB1.ItemIndex = 6) then
    begin
      GSBH := IfThen(CB1.ItemIndex = 5 , 'CDC' ,'VC1');
      
      SQL.Add('  SELECT Date, Hour, GXLB, Qty FROM (');
      SQL.Add('    SELECT CONVERT(VARCHAR, ShoeTest.CutDate, 111) AS Date, DATEPART(HH, DATEADD(MI, -30, ShoeTest.CutDate)) AS Hour, ''Cutting'' AS GXLB, shoetest2.Qty FROM ShoeTest');
      SQL.Add('    LEFT JOIN ShoeTest2 ON ShoeTest2.TestNo = ShoeTest.TestNo');
      SQL.Add('    LEFT JOIN ShoeTestDetail ON ShoeTestDetail.YYBH = ShoeTest2.Parts');
      SQL.Add('    LEFT JOIN dbo.YPZL ON YPZL.YPDH = ShoeTest.YPDH');
      SQL.Add('    WHERE CONVERT(SmallDateTime, CONVERT(VARCHAR, ShoeTest.CutDate, 111)) BETWEEN ''' + FormatDateTime('yyyy/MM/dd', DTP1.Date) + ''' AND ''' + FormatDateTime('yyyy/MM/dd', DTP2.Date) + '''');
      SQL.Add('    AND YWSM IN (''Finished Shoe'', ''Half Stitching Upper'', ''Stitching Upper'', ''Lasted upper'', ''Upper Component'', ''CFM sample'', ''Upper And Bottom(CE)'')');
      SQL.Add('    AND GSDH='''+GSBH+'''');
      SQL.Add('    UNION ALL');
      SQL.Add('    SELECT CONVERT(VARCHAR, ShoeTest.StitchingDate, 111) AS Date, DATEPART(HH, DATEADD(MI, -30, ShoeTest.StitchingDate)) AS Hour, ''Stitching'' AS GXLB, shoetest2.Qty FROM ShoeTest');
      SQL.Add('    LEFT JOIN ShoeTest2 ON ShoeTest2.TestNo = ShoeTest.TestNo');
      SQL.Add('    LEFT JOIN ShoeTestDetail ON ShoeTestDetail.YYBH = ShoeTest2.Parts');
    ...
    begin
      ...
      SQL.Add('    and SMZL.UserID= '+''''+edtUser.Text+'''');
      //======
      SQL.Add('    UNION ALL');
      ...
      SQL.Add('    LEFT JOIN SMDDSS ON SMDDSS.CODEBAR = SMZL.CODEBAR');
      SQL.Add('    WHERE CONVERT(SmallDateTime, CONVERT(VARCHAR, ScanDate, 111)) BETWEEN ''' + FormatDateTime('yyyy/MM/dd', DTP1.Date) + ''' AND ''' + FormatDateTime('yyyy/MM/dd', DTP2.Date) + ''' AND DepNO = ''A12050301'' AND GXLB = ''B''');
      //======03.01.2025
      if edtUser.Text <>'' then
      SQL.Add('    and SMZL.UserID= '+''''+edtUser.Text+'''');
      //======
      ...
      if DTP1.Date < Qtmp.FieldByName('Userdate').Value then begin
        ...
        //======03.01.2025
      if edtUser.Text <>'' then
        SQL.Add('    and SMZLold.UserID= '+''''+edtUser.Text+'''');
      //======
        SQL.Add('    UNION ALL');
...
begin
  ...
  begin
    Active := false;
    SQL.Clear;
    if (cboProcess.Text = 'Cutting') or (cboProcess.Text = '') then
      begin
    ...
    if (cboProcess.Text = 'Stitching') or (cboProcess.Text = '') then
    begin
      if SQL.Count > 0 then
        SQL.Add('union');
    ...
    if (cboProcess.Text = 'Assembly') or (cboProcess.Text = '') then
    begin
      if SQL.Count > 0 then
        SQL.Add('union');
    ...
    //SQL.Add('order by Plantime, OKtime ,serino');
    //showmessage(sql.Text);
    Active := True;
  end;
end;
...
begin
  ...
  begin
    Active := false;
    SQL.Clear;
    SQL.Add('select ''1'' serino,Date,''Cutting'' Process,Sample');
    ...
    SQL.Add(',MassProduction,Total, convert(varchar,cast(ROUND(Total/'''+edit1.text+'''*100,2) as numeric(5,2)))+''%'' Complete from (');
    SQL.Add('select top 2 Date,SUM(Sample) Sample');
    if not CheckBox4.Checked then
      begin
      SQL.Add(',SUM(CBY) CBY');
      end;
    SQL.Add(',SUM(MassProduction) MassProduction,SUM(TOTal) Total from (');
    ...
    SQL.Add('isnull(SampleQty,0) Sample');
    if not CheckBox4.Checked then
      begin
    SQL.Add(',isnull(CBYQty,0) CBY');
    end;
    SQL.Add(',isnull(MassProductionQty,0) MassProduction,');
    if not CheckBox4.Checked then
    begin
      SQL.Add('isnull(SampleQty,0) + isnull(CBYQty,0) + isnull(MassProductionQty,0) AS Total FROM (');
    end else
    begin
      SQL.Add('isnull(SampleQty,0) + isnull(MassProductionQty,0) AS Total FROM (');
    end;
    ...
    SQL.Add('union');
    SQL.Add('select ''2'' Serino,Date,''Stitching'' Process,Sample');
    if not CheckBox4.Checked then
      begin
      SQL.Add(',CBY');
      end else
    ...
    begin
      SQL.Add('isnull(SampleQty,0) +isnull(MassProductionQty,0) Total   from (');
    end;
    ...
    SQL.Add('    left join shoetestDetail on shoetestDetail.yybh=shoetest2.parts');
    //SQL.Add('    WHERE YWSM in (''Finished Shoe'',''Half Stitching Upper'',''Stitching Upper'',''Lasted upper'',''Upper Component'',''CFM sample'',''Upper And Bottom(CE)'')');
    SQL.Add('    WHERE YWSM in (''Finished Shoe'',''Half Stitching Upper'',''Stitching Upper'',''Lasted upper'',''Upper Component'',''CFM sample'')');
    SQL.Add('    group by shoetest2.testno');
    SQL.Add('  ) AS sample on sample.TestNo=main.testno');
    SQL.Add(') AS main2');
    ...
    SQL.Add('    where date =''' + FormatDateTime('yyyy/MM/dd', DTP111.Date) + '''');
    SQL.Add('union');
    SQL.Add('select ''3'' Serino,Date,''Assembling'' Process,Sample');
    if not CheckBox4.Checked then
    begin
      SQL.Add(',CBY');
    end else
    ...
    SQL.Add('    left join shoetestDetail on shoetestDetail.yybh=shoetest2.parts');
    //SQL.Add('    where  YWSM in (''Finished Shoe'',''Lasted upper'',''CFM sample'',''Upper And Bottom(CE)'')');
    SQL.Add('    where  YWSM in (''Finished Shoe'',''Lasted upper'',''CFM sample'')');
    SQL.Add('    group by shoetest2.testno');
    SQL.Add('  ) AS sample on sample.TestNo=main.testno');
    SQL.Add(') AS main2');
    ...
    SQL.Add('    SELECT CONVERT(VARCHAR, ScanDate, 111) AS Date, sum(SMDDSS.Qty*SMZL.CTS) AS MassProductionQty FROM SMZL');
    SQL.Add('    LEFT JOIN SMDDSS ON SMDDSS.CODEBAR = SMZL.CODEBAR');
    SQL.Add('    WHERE CONVERT(SmallDateTime, CONVERT(VARCHAR, ScanDate, 111)) BETWEEN convert(varchar, getdate()-7, 111) and convert(varchar, getdate(), 111) AND DepNO = ''A12050301'' AND GXLB = ''B''');
    SQL.Add('    group by CONVERT(VARCHAR, ScanDate, 111))MassProductionQty on MassProductionQty.Date=sample.date');
    SQL.Add('    )main  group by Date order by date desc)Assembling');
    SQL.Add('    where date =''' + FormatDateTime('yyyy/MM/dd', DTP111.Date) + '''');
    SQL.Add('union');

    SQL.Add('select ''4'' Serino,Date,''Packing'' Process,Sample');
    if not CheckBox4.Checked then
    begin
      SQL.Add(',CBY');
    end else
    ...
    SQL.Add('    left join shoetestDetail on shoetestDetail.yybh=shoetest2.parts');
    //SQL.Add('    where  YWSM in (''Finished Shoe'',''Lasted upper'',''CFM sample'',''Upper And Bottom(CE)'')');
    SQL.Add('    where  YWSM in (''Finished Shoe'',''CFM sample'')');
    SQL.Add('    group by shoetest2.testno');
    SQL.Add('  ) AS sample on sample.TestNo=main.testno');
    SQL.Add(') AS main2');
    SQL.Add('group by AssembleOK)sample');
...
procedure TDFC.Button14Click(Sender: TObject);
begin
  with QryHN do
  begin
    Active := false;
    SQL.Clear;
    sql.Add('SELECT GROUPS,groups2,');
    sql.Add('SUM(CASE WHEN PlanType = ''CuttingPlan'' THEN SRQty ELSE 0 END) AS SRQtyCuttingPlan,');
    sql.Add('SUM(CASE WHEN PlanType = ''CuttingPlan'' THEN Qty ELSE 0 END) AS QtyCuttingPlan,');
    sql.Add('SUM(CASE WHEN PlanType = ''CuttingFinished'' THEN SRQty ELSE 0 END) AS SRQtyCuttingFinished,');
    sql.Add('SUM(CASE WHEN PlanType = ''CuttingFinished'' THEN Qty ELSE 0 END) AS QtyCuttingFinished,');
    sql.Add('SUM(CASE WHEN PlanType = ''StitchingPlan'' THEN SRQty ELSE 0 END) AS SRQtyStitchingPlan,');
    sql.Add('SUM(CASE WHEN PlanType = ''StitchingPlan'' THEN Qty ELSE 0 END) AS QtyStitchingPlan,');
    sql.Add('SUM(CASE WHEN PlanType = ''StitchingFinished'' THEN SRQty ELSE 0 END) AS SRQtyStitchingFinished,');
    sql.Add('SUM(CASE WHEN PlanType = ''StitchingFinished'' THEN Qty ELSE 0 END) AS QtyStitchingFinished,');
    sql.Add('SUM(CASE WHEN PlanType = ''AssemblyPlan'' THEN SRQty ELSE 0 END) AS SRQtyAssemblyPlan,');
    sql.Add('SUM(CASE WHEN PlanType = ''AssemblyPlan'' THEN Qty ELSE 0 END) AS QtyAssemblyPlan,');
    sql.Add('SUM(CASE WHEN PlanType = ''AssemblyFinished'' THEN SRQty ELSE 0 END) AS SRQtyAssemblyFinished,');
    sql.Add('SUM(CASE WHEN PlanType = ''AssemblyFinished'' THEN Qty ELSE 0 END) AS QtyAssemblyFinished,');
    sql.Add('SUM(CASE WHEN PlanType = ''PackingFinished'' THEN SRQty ELSE 0 END) AS SRQtyPackingFinished,');
    sql.Add('SUM(CASE WHEN PlanType = ''PackingFinished'' THEN Qty ELSE 0 END) AS QtyPackingFinished');
    sql.Add('FROM (');
    //=============================================================================================================
    sql.Add('SELECT groups,groups2,COUNT(Distinct Serino) AS SRQty,');
    sql.Add('(SUM(FinishedShoe) + SUM(HalfStitchingUpper) + SUM(StitchingUpper) + SUM(LastedUpper) + SUM(UpperComponent) + SUM(CFMSample) + SUM(UpperAndBottomCE)) AS Qty,');
    sql.Add('''CuttingPlan'' AS PlanType');
    sql.Add('FROM (');
    sql.Add('SELECT shoetestplan.Serino,groups,groups2,CutDate_first,CONVERT(DATETIME, CONVERT(VARCHAR, Complete, 23)) AS Complete,ISNULL(bw01.qty, 0) AS FinishedShoe,');
    sql.Add('ISNULL(bw02.qty, 0) AS HalfStitchingUpper,ISNULL(bw03.qty,0) StitchingUpper,ISNULL(bw04.qty, 0) AS LastedUpper,ISNULL(bw05.qty, 0) AS UpperComponent,ISNULL(bw10.qty, 0) AS CFMSample,');
    sql.Add('ISNULL(bw11.qty,0) UpperAndBottomCE');
    sql.Add('FROM ShoeTest');
    sql.Add('left join ypzl on ypzl.YPDH = shoetest.YPDH');
    sql.Add('LEFT JOIN shoetestplan ON shoetest.testno = shoetestplan.testno');
    sql.Add('LEFT JOIN (SELECT SUM(qty) AS qty, parts, testno FROM shoetest2 WHERE parts = ''BW01'' GROUP BY parts, testno) bw01 ON bw01.testno = shoetest.testno');
    sql.Add('LEFT JOIN (SELECT SUM(qty) qty, parts, testno FROM shoetest2 WHERE parts = ''BW02'' GROUP BY parts, testno) bw02 ON bw02.testno = shoetest.testno');
    sql.Add('LEFT JOIN (SELECT SUM(qty) AS qty, parts, testno FROM shoetest2 WHERE parts = ''BW03'' GROUP BY parts, testno) bw03 ON bw03.testno = shoetest.testno');
    sql.Add('LEFT JOIN (SELECT SUM(qty) qty, parts, testno FROM shoetest2 WHERE parts = ''BW04'' GROUP BY parts, testno) bw04 ON bw04.testno = shoetest.testno');
    sql.Add('LEFT JOIN (SELECT SUM(qty) qty, parts, testno FROM shoetest2 WHERE parts = ''BW05'' GROUP BY parts, testno) bw05 ON bw05.testno = shoetest.testno');
    sql.Add('LEFT JOIN (SELECT SUM(qty) qty, parts, testno FROM shoetest2 WHERE parts = ''BW10'' GROUP BY parts, testno) bw10 ON bw10.testno = shoetest.testno');
    sql.Add('LEFT JOIN (SELECT SUM(qty) qty,parts,testno from shoetest2 where parts=''BW11'' GROUP BY parts,testno)bw11 on bw11.testno=shoetest.testno');
    sql.Add('WHERE 1=1 and shoetest.CutDate_first BETWEEN CONVERT(smalldatetime, ''' + FormatDateTime('yyyy/MM/dd 00:00:00', DTP8.DateTime) + ''')');
    sql.Add('AND CONVERT(smalldatetime, ''' + FormatDateTime('yyyy/MM/dd 23:59:00', DTP9.DateTime) + ''')');
    SQL.Add('and ypzl.GSDH='''+gsbhCb.Text+''' ');
    sql.Add(' ) AS CuttingPlan');
    sql.Add('GROUP BY GROUPS, groups2');
    sql.Add('UNION ALL');
     //=============================================================================================================
    sql.Add('SELECT groups,groups2,COUNT(Distinct Serino) AS SRQty,');
    sql.Add('(SUM(FinishedShoe) + SUM(HalfStitchingUpper) + SUM(StitchingUpper) + SUM(LastedUpper) + SUM(UpperComponent) + SUM(CFMSample) + SUM(UpperAndBottomCE)) AS Qty,');
    sql.Add(' ''CuttingFinished'' AS PlanType');
    sql.Add('FROM (');
    sql.Add('select shoetestplan.Serino,groups,groups2,shoetest.testno,shoetest.Article,CutDate, convert (datetime,convert(varchar,Complete, 23)) as Complete, isnull(bw01.qty,0) FinishedShoe,');
    sql.Add('ISNULL(bw02.qty, 0) AS HalfStitchingUpper,ISNULL(bw03.qty,0) StitchingUpper,ISNULL(bw04.qty, 0) AS LastedUpper,ISNULL(bw05.qty, 0) AS UpperComponent,ISNULL(bw10.qty, 0) AS CFMSample,');
    sql.Add('ISNULL(bw11.qty,0) UpperAndBottomCE');
    sql.Add('FROM ShoeTest');
    sql.Add('left join ypzl on ypzl.YPDH = shoetest.YPDH');
    ...
    sql.Add('LEFT JOIN (SELECT SUM(qty) qty,parts,testno from shoetest2 where parts=''BW11'' GROUP BY parts,testno)bw11 on bw11.testno=shoetest.testno');
    sql.Add('where 1=1');
    sql.Add('and shoetest.CutDate BETWEEN CONVERT(smalldatetime, ''' + FormatDateTime('yyyy/MM/dd 00:00:00', DTP8.DateTime) + ''')');
    sql.Add('AND CONVERT(smalldatetime, ''' + FormatDateTime('yyyy/MM/dd 23:59:00', DTP9.DateTime) + ''')');
    SQL.Add('and ypzl.GSDH='''+gsbhCb.Text+''' ');
    sql.Add(' ) AS CuttingFinished');
    sql.Add('GROUP BY GROUPS, groups2');
    sql.Add('UNION ALL');
     //=============================================================================================================
    sql.Add('SELECT groups,groups2,COUNT(Distinct Serino) AS SRQty, ');
    sql.Add('(SUM(FinishedShoe) + SUM(HalfStitchingUpper) + SUM(StitchingUpper) + SUM(LastedUpper) + SUM(UpperComponent) + SUM(CFMSample)) AS Qty,');
    sql.Add(' ''StitchingPlan'' AS PlanType ');
    sql.Add('FROM (');
    sql.Add('select shoetestplan.Serino,groups,groups2,shoetest.testno,shoetest.Article,StitchingDate_first, convert (datetime,convert(varchar,Complete, 23)) as Complete, isnull(bw01.qty,0) FinishedShoe,');
    sql.Add('ISNULL(bw02.qty, 0) AS HalfStitchingUpper,ISNULL(bw03.qty,0) StitchingUpper,ISNULL(bw04.qty, 0) AS LastedUpper,ISNULL(bw05.qty, 0) AS UpperComponent,ISNULL(bw10.qty, 0) AS CFMSample');
    sql.Add('FROM ShoeTest');
    sql.Add('left join shoetestplan on shoetest.testno = shoetestplan.testno');
    sql.Add('left join ypzl on ypzl.YPDH = shoetest.YPDH');
    ...
    sql.Add('LEFT JOIN (SELECT SUM(qty) qty, parts, testno FROM shoetest2 WHERE parts = ''BW10'' GROUP BY parts, testno) bw10 ON bw10.testno = shoetest.testno');
    sql.Add('where  1=1');
    sql.Add('and shoetest.StitchingDate_first BETWEEN CONVERT(smalldatetime, ''' + FormatDateTime('yyyy/MM/dd 00:00:00', DTP8.DateTime) + ''')');
    sql.Add('AND CONVERT(smalldatetime, ''' + FormatDateTime('yyyy/MM/dd 23:59:00', DTP9.DateTime) + ''')');
    SQL.Add('and ypzl.GSDH='''+gsbhCb.Text+''' ');
    sql.Add(' ) AS StitchingPlan');
    sql.Add('GROUP BY GROUPS, groups2');
    sql.Add('UNION ALL');
     //=============================================================================================================
    sql.Add('SELECT groups,groups2,COUNT(Distinct Serino) AS SRQty,');
    sql.Add('(SUM(FinishedShoe) + SUM(HalfStitchingUpper) + SUM(StitchingUpper) + SUM(LastedUpper) + SUM(UpperComponent) + SUM(CFMSample)) AS Qty,');
    sql.Add(' ''StitchingFinished'' AS PlanType ');
    sql.Add('FROM (');
    sql.Add('select shoetestplan.Serino,groups,groups2,shoetest.testno,shoetest.Article,StitchingDate,convert (datetime,convert(varchar,Complete, 23)) as Complete,isnull(bw01.qty,0) FinishedShoe,');
    sql.Add('ISNULL(bw02.qty, 0) AS HalfStitchingUpper,ISNULL(bw03.qty,0) StitchingUpper,ISNULL(bw04.qty, 0) AS LastedUpper,ISNULL(bw05.qty, 0) AS UpperComponent,ISNULL(bw10.qty, 0) AS CFMSample');
    sql.Add('FROM ShoeTest');
    sql.Add('left join shoetestplan on shoetest.testno = shoetestplan.testno');
    sql.Add('left join ypzl on ypzl.YPDH = shoetest.YPDH');
    ...
    sql.Add('LEFT JOIN (SELECT SUM(qty) qty, parts, testno FROM shoetest2 WHERE parts = ''BW10'' GROUP BY parts, testno) bw10 ON bw10.testno = shoetest.testno');
    sql.Add('where  1=1');
    sql.Add('and shoetest.StitchingDate BETWEEN CONVERT(smalldatetime, ''' + FormatDateTime('yyyy/MM/dd 00:00:00', DTP8.DateTime) + ''')');
    sql.Add('AND CONVERT(smalldatetime, ''' + FormatDateTime('yyyy/MM/dd 23:59:00', DTP9.DateTime) + ''')');
    SQL.Add('and ypzl.GSDH='''+gsbhCb.Text+''' ');
    sql.Add(' ) AS StitchingFinished');
    sql.Add('GROUP BY GROUPS, groups2');
    sql.Add('UNION ALL');
     //=============================================================================================================
    sql.Add('SELECT groups,groups2,COUNT(Distinct Serino) AS SRQty,');
    sql.Add('(SUM(FinishedShoe) + SUM(LastedUpper) + SUM(CFMSample)) AS Qty,');
    sql.Add(' ''AssemblyPlan'' AS PlanType ');
    sql.Add('FROM (');
    sql.Add('select shoetestplan.Serino,groups,groups2,shoetest.testno,shoetest.Article,AssembiyDate_first,convert (datetime,convert(varchar,Complete, 23)) as Complete,isnull(bw01.qty,0) FinishedShoe,');
    sql.Add('ISNULL(bw04.qty, 0) AS LastedUpper,ISNULL(bw10.qty, 0) AS CFMSample');
    sql.Add('FROM ShoeTest');
    sql.Add('left join ypzl on ypzl.YPDH = shoetest.YPDH');
    sql.Add('left join shoetestplan on shoetest.testno = shoetestplan.testno');
    sql.Add('LEFT JOIN (SELECT SUM(qty) AS qty, parts, testno FROM shoetest2 WHERE parts = ''BW01'' GROUP BY parts, testno) bw01 ON bw01.testno = shoetest.testno');
    ...
    sql.Add('LEFT JOIN (SELECT SUM(qty) qty, parts, testno FROM shoetest2 WHERE parts = ''BW10'' GROUP BY parts, testno) bw10 ON bw10.testno = shoetest.testno');
    sql.Add('where  1=1');
    sql.Add('and shoetest.AssembiyDate_first BETWEEN CONVERT(smalldatetime, ''' + FormatDateTime('yyyy/MM/dd 00:00:00', DTP8.DateTime) + ''')');
    sql.Add('AND CONVERT(smalldatetime, ''' + FormatDateTime('yyyy/MM/dd 23:59:00', DTP9.DateTime) + ''')');
    SQL.Add('and ypzl.GSDH='''+gsbhCb.Text+''' ');
    sql.Add(' ) AS AssemblyPlan');
    sql.Add('GROUP BY GROUPS, groups2');
    sql.Add('UNION ALL');
     //=============================================================================================================
    sql.Add('SELECT groups,groups2,COUNT(Distinct Serino) AS SRQty,');
    sql.Add('(SUM(FinishedShoe) + SUM(LastedUpper) + SUM(CFMSample)) AS Qty,');
    sql.Add(' ''AssemblyFinished'' AS PlanType ');
    sql.Add('FROM (');
    sql.Add('select shoetestplan.Serino,groups,groups2,shoetest.testno,shoetest.Article,Assemblecomplete,convert (datetime,convert(varchar,Complete, 23)) as Complete,isnull(bw01.qty,0) FinishedShoe,');
    sql.Add('ISNULL(bw04.qty, 0) AS LastedUpper,ISNULL(bw10.qty, 0) AS CFMSample');
    sql.Add('FROM ShoeTest');
    sql.Add('left join ypzl on ypzl.YPDH = shoetest.YPDH');
    sql.Add('left join shoetestplan on shoetest.testno = shoetestplan.testno');
    ...
    sql.Add('LEFT JOIN (SELECT SUM(qty) qty, parts, testno FROM shoetest2 WHERE parts = ''BW10'' GROUP BY parts, testno) bw10 ON bw10.testno = shoetest.testno');
    sql.Add('where  1=1');
    sql.Add('and shoetest.Assemblecomplete BETWEEN CONVERT(smalldatetime, ''' + FormatDateTime('yyyy/MM/dd 00:00:00', DTP8.DateTime) + ''')');
    sql.Add('AND CONVERT(smalldatetime, ''' + FormatDateTime('yyyy/MM/dd 23:59:00', DTP9.DateTime) + ''')');
    SQL.Add('and ypzl.GSDH='''+gsbhCb.Text+''' ');
    sql.Add(' ) AS AssemblyFinished');
    sql.Add('GROUP BY GROUPS, groups2');
    sql.Add('UNION ALL');
     //=============================================================================================================
    sql.Add('SELECT groups,groups2,COUNT(Distinct Serino) AS SRQty,');
    sql.Add(' (SUM(FinishedShoe) + SUM(CFMSample)) AS Qty,');
    sql.Add(' ''PackingFinished'' AS PlanType ');
    sql.Add('FROM (');
    sql.Add('select shoetestplan.Serino,groups,groups2,shoetest.testno,shoetest.Article,complete as packingok,convert (datetime,convert(varchar,Complete, 23)) as Complete,isnull(bw01.qty,0) FinishedShoe,');
    sql.Add('ISNULL(bw10.qty, 0) AS CFMSample');
    sql.Add('FROM ShoeTest');
    sql.Add('left join ypzl on ypzl.YPDH = shoetest.YPDH');
    sql.Add('left join shoetestplan on shoetest.testno = shoetestplan.testno');
    sql.Add('LEFT JOIN (SELECT SUM(qty) AS qty, parts, testno FROM shoetest2 WHERE parts = ''BW01'' GROUP BY parts, testno) bw01 ON bw01.testno = shoetest.testno');
    sql.Add('LEFT JOIN (SELECT SUM(qty) qty, parts, testno FROM shoetest2 WHERE parts = ''BW10'' GROUP BY parts, testno) bw10 ON bw10.testno = shoetest.testno');
    sql.Add('where  1=1');
    sql.Add('and shoetest.complete BETWEEN CONVERT(smalldatetime, ''' + FormatDateTime('yyyy/MM/dd 00:00:00', DTP8.DateTime) + ''')');
    sql.Add('AND CONVERT(smalldatetime, ''' + FormatDateTime('yyyy/MM/dd 23:59:00', DTP9.DateTime) + ''')');
    SQL.Add('and ypzl.GSDH='''+gsbhCb.Text+''' ');
    sql.Add(' ) AS PackingFinished');
    sql.Add('GROUP BY GROUPS, groups2');
     //=============================================================================================================  
    sql.Add(') AS TotalQuery');
    if cbkC2.Checked then begin
       SQL.Add('WHERE  groups2 = ''C2'' ');
    end
    else if cbk_excluding.Checked then begin
       SQL.Add('WHERE  groups2 <> ''C2'' and groups2<>''MP'' ');
    end;
    sql.Add('GROUP BY GROUPS, groups2');
    sql.Add('ORDER BY GROUPS, groups2');
    //showmessage(sql.text);
    Active := true;
  end;
end;

procedure TDFC.cbkC2Click(Sender: TObject);
begin
  if cbkC2.Checked then
    cbk_excluding.Checked := False;
end;

procedure TDFC.cbk_excludingClick(Sender: TObject);
begin
  if cbk_excluding.Checked then
    cbkC2.Checked := False;
end;

procedure TDFC.DBGridEh6TitleClick(Column: TColumnEh);
var
  SortOrder: string;
begin
  if Column.FieldName = 'Plantime' then
  begin
    ...
    QDelay.SQL.Text := StringReplace(QDelay.SQL.Text, 'ORDER BY Plantime ASC', '', [rfIgnoreCase]);
    QDelay.SQL.Text := StringReplace(QDelay.SQL.Text, 'ORDER BY Plantime DESC', '', [rfIgnoreCase]);

    QDelay.SQL.Add('ORDER BY Plantime' + SortOrder);
...
procedure TDFC.CB1Change(Sender: TObject);
begin
  if CB1.ItemIndex = 2 then
    CheckBox3.Checked := False;
end;

procedure TDFC.ReadIni();
var MyIni :Tinifile;
...
begin
  ...
  begin
    try
      ...
      UserIDList:=MyIni.ReadString('ERP','Dev_N64_ShowGSBHCbUserIds','');
    finally
      MyIni.Free;
    end;
  end;
