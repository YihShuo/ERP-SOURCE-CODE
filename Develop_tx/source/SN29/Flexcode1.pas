...
type
  TFlexCode = class(TForm)
    ...
    Label12: TLabel;
    Label13: TLabel;
    cbCLZMLB: TDBComboBox;
    SplitCK: TCheckBox;
    PopupMenu_His: TPopupMenu;
    MenuItem1: TMenuItem;
...
uses main1,FunUnit, FlexCodeHistory1;
...
var tmpList:TStringlist;
    i:integer;
begin
    if (not checkbox4.Checked) and ((edit3.Text='') and (edit4.Text='') and (edit5.Text='') ) then
    begin
    ...
    begin
      ...
      if cbCLZMLB.Text = 'N' then
        sql.Add('and clzmlb=''N'' and (tyjh is NULL or tyjh=''N'')')
      else if cbCLZMLB.Text = 'Y' then
        sql.Add('and clzmlb=''Y'' and (tyjh is NULL or tyjh=''N'')')
      else
      ...
      begin
        ...
        begin
          tmpList:=FuncObj.SplitString(edit4.Text,'*');
          for i:=0 to tmpList.Count-1 do
          begin
            if trim(tmpList.Strings[i])<>'' then
              SQL.Add('and a.ywpm like ''%'+StringReplace(trim(tmpList.Strings[i]),':','',[rfReplaceAll, rfIgnoreCase])+'%'' ');
          end;
          tmpList.Free;
        end
      else
      ...
      if checkbox3.Checked then
        begin
          sql.add('and a.UserID=''' + main.Edit1.Text + ''' ');
          //CheckMine thi khong check Text UserID
        end else
        begin
          if edit11.Text <> '' then
            sql.Add('and a.UserID like ''%' + edit11.text + '%''');
        end;
        
      if checkbox4.Checked then
        begin
        sql.add('and convert(smalldatetime,convert(varchar,a.userdate,111))  between ');
      ...
      if checkbox3.Checked then
        begin
          sql.add('and clzl.UserID=''' + main.Edit1.Text + ''' ');
          //CheckMine thi khong check Text UserID
        end else
        begin
          if edit11.Text <> '' then
            sql.Add('and clzl.UserID like ''%' + edit11.text + '%''');
        end;
    ...
    begin
      ...
      begin
        ...
        with tmpqry do
            begin
              active := false;
              sql.Clear;
              sql.Add('SELECT MAX(ver) AS MaxVer FROM clzl_flex_his WHERE cldh = :cldh');
              ParamByName('cldh').AsString := query2.FieldByName('cldh').AsString;
              active := true;
              if FieldByName('MaxVer').IsNull then
                currentVer := -1
              else
                currentVer := FieldByName('MaxVer').AsInteger;
              active := false;
        end;
        //====================================
      ...
      with tmpqry do
        begin
          active := false;
          sql.Clear;
          sql.Add('INSERT INTO clzl_flex_his (cldh, ver, cldhflex, userID, userdate)');
          sql.Add('VALUES (:cldh, :ver, :cldhflex, :userID, :userdate)');
          ParamByName('cldh').AsString := query2.FieldByName('cldh').AsString;
          ParamByName('ver').AsInteger := currentVer + 1;
          ParamByName('cldhflex').AsString := query2.FieldByName('cldhflex').AsString;
          ParamByName('userID').AsString := main.edit1.text;
          ParamByName('userdate').AsDateTime := NDate;
          ExecSQL;
      end;
      query2.next;
    end;
...
var
  ...
  Flexcode,FlexcodeDes,Memo:TStringlist;
begin
    ...
    n:=0;
    //j:=1;
    j:=2; //Lay tu dong thu 2, bo tieu de
    flag:=true;
      while(flag=true)  do
       begin
         Flexcode.Add(Trim(msExcel.WorkBooks[1].WorkSheets[1].Cells[j, 1]));
         FlexcodeDes.Add(msExcel.WorkBooks[1].WorkSheets[1].Cells[j,2]);
         Memo.Add(msExcel.WorkBooks[1].WorkSheets[1].Cells[j,3]);
   ...
   with FC do
    ...
    for j:=0 to n-2 do
     begin
         ...
         FC.FieldByName('Memo').value:=Memo[j];
         fc.FieldByName('YN').AsString := '1';
         fc.FieldByName('USERID').AsString :=main.edit1.text;
...
begin
    ...
    begin
         active:=false;
         sql.Clear;
         sql.add('select cldhflex,cldhflexdes,Memo,USERID,USERDATE,YN from clzl_flexs where 1=1');
         if edit8.text<>'' then
...
begin
  ...
  try
    ...
    begin
      if fc.UpdateStatus = usInserted then
      begin
        fc.Edit;
        fc.FieldByName('YN').AsString := '1';
        fc.FieldByName('USERID').AsString :=main.edit1.text;
      ...
      else if fc.UpdateStatus = usModified then
      begin
        if fc.FieldByName('YN').AsString = '0' then
        begin
          upFC.Apply(ukDelete);
        end
        else
...
procedure TFlexCode.MenuItem1Click(Sender: TObject);
begin
  FlexCodeHistory:=TFlexCodeHistory.create(self);
  FlexCodeHistory.ShowModal();
end;
