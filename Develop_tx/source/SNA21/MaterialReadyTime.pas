...
uses
  ...
  Dialogs, DB, DBTables, GridsEh, DBGridEh, StdCtrls, ExtCtrls, ComCtrls, ehlibBDE, IniFiles, fununit;
...
type
  TMaterialReadyTime1 = class(TForm)
    ...
    qryc9get: TQuery;
    dsc9get: TDataSource;
    Query12CheckResult: TStringField;
    Query11CheckResult: TStringField;
...
procedure TMaterialReadyTime1.ReadIni();
var MyIni :Tinifile;
    AppDir:string;
begin
...
begin
  if PageControl1.ActivePageIndex = 0 then begin
    ...
    begin
      ...
      SQL.Add('update shoetest set Pending_P = ''1'',');
      SQL.Add('PreparationDate = getdate(),');
      SQL.Add('lamination_get = CASE');
      SQL.Add('  WHEN lamination_send IS NOT NULL THEN CONVERT(DATETIME, CONVERT(VARCHAR(10), getdate(), 120))');
      SQL.Add('  ELSE lamination_get');
      SQL.Add('END');
    ...
    begin
      Active:=false;
      SQL.Clear;
      //12.11.2025 bo cap nhat C9_get khi cap nhat insole_send
      SQL.Add('update shoetest set insole_send=getdate()  where Testno='''+edit9.text+'''');
      //01.11.2025 cap nhat bao bi (insole_send) chung voi C9_get khi C9_send da co du lieu
      {SQL.Add('update shoetest set');
      SQL.Add('insole_send = getdate(),');
      SQL.Add('C9_get = CASE');
      SQL.Add('  WHEN C9_send IS NOT NULL THEN CONVERT(DATETIME, CONVERT(VARCHAR(10), getdate(), 120))');
      SQL.Add('  ELSE C9_get');
      SQL.Add('END');
      SQL.Add('where Testno = ''' + edit9.Text + '''');  }
  ...
  //01.11.2025 them scan c9_send/c9_get
  else if PageControl1.ActivePageIndex = 18 then begin
  with qryc9send do
  begin
    Active:=false;
    SQL.Clear;
      //SQL.Add('update shoetest set C9_SEND=getdate()  where Testno='''+edt1.text+'''');
      //13.11.2025 update C9_GET = C9_SEND + 3 days, neu ngay chu nhat thi tang thanh thu hai
      SQL.Add('update shoetest');
      SQL.Add('set C9_SEND = getdate(),');
      SQL.Add('    C9_GET = case');
      SQL.Add('        when datepart(weekday, dateadd(day, 3, getdate())) = 1');
      SQL.Add('             then dateadd(day, 4, getdate()) ');
      SQL.Add('        else dateadd(day, 3, getdate())');
      SQL.Add('    end');
      SQL.Add('where Testno = ''' + edt1.Text + '''');
      SQL.Add('select testno,Article,TBReceive,Pending_P,C9_SEND from shoetest where Testno='''+edt1.text+'''');
    Active:=true;
  end;
  edt1.Clear;
  end
  else if PageControl1.ActivePageIndex = 19 then begin
  with qryc9get do
  begin
    Active:=false;
    SQL.Clear;
      SQL.Add('update shoetest set C9_GET=getdate()  where Testno='''+edt2.text+'''');
      SQL.Add('select testno,Article,TBReceive,Pending_P,C9_GET from shoetest where Testno='''+edt2.text+'''');
    Active:=true;
  end;
  edt2.Clear;
  end
...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('select Class,Season,Stage,SR,Testno,SO,Supplier,SupplierName, isnull(Status,'''') Status,ETA,Material,MaterialDES,Memo,ywsm,CheckResult from ( ');
    ...
    //===============
    SQL.Add('Temp_YPZL.CLBH Material,CLZL.YWPM MaterialDES,Temp_YPZL.Testno,CGZLSs1.Memo,ywsm,CGZLInvoiceS.CheckResult from (');
    ...
    //SQL.Add('	left join CGZLInvoiceS on CGZLInvoiceS.CLBH=CGZLSS.CLBH and CGZLInvoiceS.CGNO=CGZLSS.CGNO');      //2026.04.23
    //SQL.Add('	left join CGZLInvoice on CGZLInvoiceS.cino=CGZLInvoice.cino');                                    //2026.04.23
    ...
    SQL.Add('	Group by CGZLSS.ZLBH, CGZLSS.CLBH,CLZL.cltd,MaterialMOQ.Location,cgzl.cgdate,CGZLS.Memo,CGZLS.ETA,CGZLS.CFMDate  )CGZLSS1 on CGZLSS1.CLBH=Temp_YPZL.CLBH and CGZLSS1.YPDH =Temp_YPZL.YPDH and CGZLSS1.CGNO=CGZLSS.CGNO');
    SQL.Add('left join');
    SQL.Add('	(');
    ...
    SQL.Add(' )main   group by Class,Season,Stage,SR,Testno,SO,Supplier,ETA,SupplierName,isnull(Status,'''') ,Material,MaterialDES,Memo,PurReceiveDate,ywsm,CheckResult');
...
begin
  with Query12 do begin
    ...
    SQL.Add('case when  kcrkstemp.qty is not null or PurReceiveDate is not null then ''Arrive'' when (CGKCUSE.Qty is not null and CGKCUSE.Qty<>0) then ''UseStock'' end ''Status'',');
    //SQL.Add('case when  ((kcrkstemp.qty is not null) or  (PurReceiveDate is not null)  or (CGKCUSE.Qty is not null and CGKCUSE.Qty<>0)) then ''OK'' else  convert(varchar, CGZLSS1.ETA, 111) end ''ETA'',');
    //2026.02.06 Tinh ngay ETA giong voi Develop NB5
    SQL.Add('case when  ((kcrkstemp.qty is not null) or (PurReceiveDate is not null)) then ''OK''  when (CGKCUSE.Qty is not null and CGKCUSE.Qty<>0 and (CGZLSS1.Qty is null or CGZLSS1.Qty=0)) then ''UseStock'' ');
    ...
    SQL.Add('    (LEFT(Temp_YPZL.CLBH,3)  in (''O02'',''J05'',''J04'',''A48'',''G03'',''I01'',''J03''))  or');    //2025.09.20 xet them ma vat tu J03
    SQL.Add('    (LEFT(Temp_YPZL.CLBH,1) in (''P'',''G'')  and csbh in (''V097'',''V194'',''A104'',''A088'',''K110'',''V162'',''A159'',''A385''))) and');     //2025.08.25 them ma NCU A385
...
begin
  ...
  if query14.FieldByName('TBReceive').value='1' then
  begin
      if query14.FieldByName('kietphong_Get').value <> null then
      begin
...
procedure TMaterialReadyTime1.edt1KeyPress(Sender: TObject; var Key: Char);
begin
  if (Key = #13)  then
    begin
     with qryc9send do
    ...
    if qryc9send.FieldByName('TBReceive').value<>'1' then
    begin
       showmessage('TB khong co Receive');
       edt1.Clear;
       abort;
    ...
    if qryc9send.FieldByName('TBReceive').value='1' then
    begin
        if qryc9send.FieldByName('C9_Send').value <> null then
        begin
          showmessage('TB already C9_Send OK');
          edt1.Clear;
          abort;
        end;
        if qryc9send.FieldByName('C9_Send').value = null then
...
procedure TMaterialReadyTime1.edt2KeyPress(Sender: TObject; var Key: Char);
begin
  if (Key = #13)  then
    begin
     with qryc9get do
    ...
    if qryc9get.FieldByName('TBReceive').value<>'1' then
    begin
       showmessage('TB khong co Receive');
       edt2.Clear;
       abort;
    ...
    if qryc9get.FieldByName('TBReceive').value='1' then
    begin
        if qryc9get.FieldByName('C9_Get').value <> null then
        begin
          showmessage('TB already C9_Get OK');
          edt2.Clear;
          abort;
        end;
        if qryc9get.FieldByName('C9_Get').value = null then
