 ...
 //2017/03/08  取消 因一樣SR有可能存在兩種季節//JOHNY
  ...
  begin
    ...
    begin
        ...
        SQL.Add('   SELECT ZLBH,CLBH,GSBH,QTY,memo FROM dbo.CGKCUSE   ');
        SQL.Add('  WHERE  ');
        ...
        SQL.Add('    ');
        SQL.Add('  UNION ALL    ');
        SQL.Add('  SELECT ZLBH,CLBH,GSBH,QTY,memo FROM dbo.CGKCUSE_TMKT   ');
        SQL.Add('  WHERE  ');
        ...
        SQL.Add('  	CASE WHEN MAX(YPZLZL.INSDATE) < ''2026-03-01'' THEN ISNULL(CGKCUSE_TMKT.CLBH,CGKCUSE.CLBH) ELSE CGKCUSE_TMKT.CLBH END AS CLBH, CGKCUSE.GSBH,');
        ...
        SQL.Add('  GROUP BY ISNULL(CGKCUSE_TMKT.ZLBH,CGKCUSE.ZLBH),CGKCUSE_TMKT.ZLBH,ISNULL(CGKCUSE_TMKT.CLBH,CGKCUSE.CLBH),CGKCUSE_TMKT.CLBH,CGKCUSE.GSBH,');
        ...
        sql.Add(' left join clzl on clzl.cldh=ypzls.clbh');
        sql.Add(' left join #CGKCUSE_temp cgkcuse on cgkcuse.zlbh=ypzls.ypdh and cgkcuse.clbh=ypzls.clbh and cgkcuse.GSBH='''+GSBH+''' ');
...
//        sql.Add('       and clzl.clzmlb=''N''');		
        ...
        sql.Add(' left join clzl on clzl.cldh=CLZHZL.CLDH1');
        sql.Add(' left join #CGKCUSE_temp cgkcuse on cgkcuse.zlbh=ypzls.ypdh and cgkcuse.clbh=CLZHZL.CLDH1 and cgkcuse.GSBH='''+GSBH+''' ');
        //sql.Add(' left join MaterialMOQ on MaterialMOQ.clbh=CLZHZL.CLDH1 and MaterialMOQ.Season='''+SeasonEdit.Text+'''');
        sql.Add(' left join MaterialMOQ on MaterialMOQ.clbh=CLZHZL.CLDH1 ');
        sql.Add(' where ypzl.YPDH=:YPDH and kfjd=:Stage');		
        ...
        SQL.Add(' left join #CGKCUSE_temp cgkcuse on cgkcuse.zlbh=:YPDH  and cgkcuse.clbh=CLZHZL.CLDH1 and cgkcuse.GSBH='''+GSBH+''' ');
        SQL.Add(' left join zszl on zszl.zsdh=CLZHZL.ZSDH');		
        //SQL.Add(' left join MaterialMOQ on MaterialMOQ.clbh=CLZHZL.CLDH1 and MaterialMOQ.Season='''+SeasonEdit.Text+''' ');
        SQL.Add(' left join MaterialMOQ on MaterialMOQ.clbh=CLZHZL.CLDH1  ');
        //		
        ...
        sql.Add(')KCRKS on KCRKS.DOCNO=cs.cgno and KCRKS.clbh=cs.clbh ');
        //===========
        sql.Add('  left join (');
        ...
        sql.Add('  left join MaterialMOQ on MaterialMOQ.clbh=CG.clbh ');
...
begin
  ...
  begin
    ...
    FDStatus_ETD.edt_ZLBH.Text := FDStatus.qry1.fieldbyname('ypdh').AsString;
    FDStatus_ETD.button1click(nil);
    FDStatus_ETD.show;
  end
  else
