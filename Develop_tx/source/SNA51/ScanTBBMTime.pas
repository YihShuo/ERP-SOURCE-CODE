...
type
  TScanTBBMTime1 = class(TForm)
    ...
    Query3AssembiyDate_first: TStringField;
  ...
  private
     procedure SacnTBComplete(i:integer);
...
procedure TScanTBBMTime1.SacnTBComplete(i:integer);
begin
  if PageControl1.ActivePageIndex = 2 then begin
    ...
    begin
      ...
      if i = 0 then
        ...
        SQL.Add('update shoetest set Assemblecomplete=getdate(), AssemblyID = ''' + main.Edit1.Text + '''  where Testno='''+edit3.text+'''')
      else
        //SQL.Add('update shoetest set Assemblecomplete=getdate(),AssembiyDate_first = getdate() where Testno='''+edit3.text+'''');
        SQL.Add('update shoetest set Assemblecomplete=getdate(),AssembiyDate_first = getdate(), AssemblyID = ''' + main.Edit1.Text + '''  where Testno='''+edit3.text+'''');
      ExecSQL;   //2025.10.21
      SQL.Clear;
      SQL.Add('select testno,Article,TBReceive,Assemblecomplete,convert(varchar, AssembiyDate_first, 111) as AssembiyDate_first from shoetest where Testno='''+edit3.text+'''');
      Active:=true;
    end;
    Edit3.Clear;
  end
...
begin
  ...
  begin
      ...
      if query3.FieldByName('Assemblecomplete').value = null then
      begin
        if query3.FieldByName('AssembiyDate_first').Value <= formatdatetime('YYYY/MM/DD',now) then
          SacnTBComplete(0)
        else
          SacnTBComplete(1);
      end;
...
begin
  if PageControl1.ActivePageIndex = 0 then begin
    ...
    begin
      Active:=false;
      SQL.Clear;
      SQL.Add('select testno,Article,TBReceive,Assemblecomplete,convert(varchar, AssembiyDate_first, 111) as AssembiyDate_first from shoetest where CONVERT(varchar(100), Assemblecomplete, 112) = CONVERT(varchar(100), GETDATE(), 112)');
      Active:=true;
    end;
  end
