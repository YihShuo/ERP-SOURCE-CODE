...
type
  TFDExcelData = class(TForm)
    ...
    RE_CheckTRANSFER: TBooleanField;
    RE_CheckTransferIn: TBooleanField;
    QReportResult: TStringField;
    YPZLScldhflex: TStringField;
    YPZLSComment: TStringField;
    ColorDialog1: TColorDialog;
    btnColor: TButton;
    YPZLbgColor: TIntegerField;
    ...
    BitBtn10: TBitBtn;
    chk1: TCheckBox;
    dsCheck_KT_DS: TDataSource;
    qryCheck_KT: TQuery;
    UpCheck_KT: TUpdateSQL;
    strngfldYYPDH: TStringField;
    qryCheck_KTKT_Check: TBooleanField;
    dtmfldYKT_CheckDate: TDateTimeField;
    YPZLKT_Check: TBooleanField;
    dtmfldYPZLKT_CheckDate: TDateTimeField;
    BitBtn11: TBitBtn;
    QReportCustomerApprove: TBooleanField;
    strngfldQReportCustomer_USERID: TStringField;
    dtmfldQReportCustomer_UserDate: TDateTimeField;
    strngfldQReportZSYWJC: TStringField;
    ...
    procedure YPZLAfterClose(DataSet: TDataSet);
    ...
    procedure DBGridEh12Columns20UpdateData(Sender: TObject;
      var Text: String; var Value: Variant; var UseText, Handled: Boolean);
  private
      DelList:TStringlist;
      NDate:TDate;
...
procedure TFDExcelData.ReadIni();
var MyIni :Tinifile;
...
begin
  ...
  begin
    try
      ...
      UserIDList:=MyIni.ReadString('ERP','Develop_N741_CheckUserID','');
    finally
      MyIni.Free;
    end;
  end;
...
begin
  ...
  begin
    ...
    SQL.Add('        ,YPZL.BuildDate,YPZL.bgColor, YPZL.KT_Check, YPZL.KT_CheckDate');
  ...
  begin
      ...
      begin
        ...
        SQL.Add(',CLBH,BWLB,CSBH,loss,CLSL,CLDJ,Currency,Rate,Remark,BWMCNM,zwsm,ywsm,CLMC,dwbh,zsjc,clzmlb,zsywjc,ywpm ,SamplePurchaser,MatUser,UserID,UserDate,CQDH,cldhflex,Comment ');
        SQL.Add('from ( ');
        SQL.Add('select DENSE_RANK() OVER(ORDER BY YPZLS.XH) AS ROWID,BWBH  ');
        //SQL.Add(',YPDH,xh,xh1,CQDH,CLBH,BWLB,CSBH,loss,CLSL,CLDJ,Currency,Rate,Remark,BWMCNM,zwsm,ywsm,CLMC,dwbh,zsjc,clzmlb,zsywjc,ywpm ,SamplePurchaser,MatUser,UserID,UserDate,cldhflex,Comment ');
        SQL.Add(',YPDH,xh,xh1,CQDH,CLBH,BWLB,CSBH,loss,CLSL,CLDJ,Currency,Rate,Remark,BWMCNM,zwsm,ywsm,CLMC,dwbh,zsjc,clzmlb,zsywjc,ywpm ,SamplePurchaser,MatUser,YPZLS.UserID,YPZLS.UserDate,flex.cldhflex,MATERIAL.Comment  ');
        SQL.Add('from ( ');
        ...
        //SQL.Add(' ,clzl.ywpm ,zszl_dev.SamplePurchaser,CLZL.UserID as MatUser,ypzls.UserID,ypzls.UserDate,flex.cldhflex,MATERIAL.Comment ');
        ...
        // add colunm by web
        //SQL.Add('LEFT JOIN CLZL_FLEX flex ON flex.cldh = clzl.cldh   ');
        //SQL.Add('LEFT JOIN [WEB_23_35].[MATERIAL_LIBRARY].[dbo].[MATERIAL] MATERIAL ON flex.cldhflex = MATERIAL.Material_ID  ');
        //
        ...
        //SQL.Add(' ,clzl.ywpm , zszl_devV.SamplePurchaser,CLZL.UserID as MatUser,ypzls.UserID,ypzls.UserDate,flex.cldhflex,MATERIAL.Comment ');
       ...
       // SQL.Add('LEFT JOIN CLZL_FLEX flex ON flex.cldh = clzl.cldh   ');
        ...
        SQL.Add(' case when  ((clzhzl_dev.zsdh is not null) and (clzhzl_dev.zsdh<>'''')) then zszl_dev.zsywjc else  zszl.zsywjc  end as zsywjc   ');
        //SQL.Add(',clzhzl2.ywpm ,clzhzl2.SamplePurchaser,clzhzl2.MatUser,clzhzl2.UserID,clzhzl2.UserDate,flex.cldhflex,MATERIAL.Comment ');
        ...
        //
        SQL.Add(')  YPZLS');
        //  group by too slow, as check only [WEB_23_35].[MATERIAL_LIBRARY].[dbo].[MATERIAL] needs group by
        SQL.Add('LEFT JOIN CLZL_FLEX flex ON flex.cldh = YPZLS.CLBH     ');
        SQL.Add('LEFT JOIN (select Material_ID,Comment from [WEB_23_35].[MATERIAL_LIBRARY].[dbo].[MATERIAL] group by Material_ID,Comment) MATERIAL ON flex.cldhflex = MATERIAL.Material_ID   ');

        //SQL.Add('group by CQDH,XH,YPDH,xh1,BWBH,CLBH,BWLB,CSBH,loss,CLSL,CLDJ,Currency,Rate,Remark,BWMCNM,zwsm,ywsm,CLMC,dwbh,zsjc,clzmlb,zsywjc,ywpm ,SamplePurchaser,MatUser,UserID,UserDate,cldhflex,Comment ');
        SQL.Add(')  YPZLS');
        //funcobj.WriteErrorLog(sql.Text);
        //ShowMessage(SQL.Text);
        Active:=true;
  ...
  begin
      ...
      begin
        ...
        SQL.Add(',clzl.ywpm ,clzl.cqdh,zszl_dev.SamplePurchaser,CLZL.UserID as MatUser,ypzls.UserID,ypzls.UserDate,flex.cldhflex,MATERIAL.Comment  FROM ypzls   ');
        SQL.Add('  LEFT JOIN bwzl bwzl ON ypzls.BWBH = bwzl.bwdh   ');
        SQL.Add('  LEFT JOIN clzl clzl ON ypzls.CLBH = clzl.cldh   ');    
        // add colunm by web
        SQL.Add('LEFT JOIN CLZL_FLEX flex ON flex.cldh = clzl.cldh   ');
...
begin
  ...
  if not YPZL.FieldByName('bgColor').IsNull then
  begin
    YPZLGrid.canvas.Brush.Color:=YPZL.FieldByName('bgColor').AsInteger;
    YPZLGrid.DefaultDrawColumnCell(Rect,DataCol,Column,State);
  end
  else
  begin
...
begin
  ...
  begin
    ...
    SQL.Add('left join MaterialMOQ on MaterialMOQ.CLBH=YPZLS.CLBH');
    {SQL.Add('left join (');
...
begin
  ...
  if (uppercase(YPZL.fieldbyname('FD').asstring)=uppercase(engname)) or (main.edit1.text='55614') or (main.edit1.text='51265')  or (main.edit1.text='45077')
  ...
  try
      ...
      begin
          case RE_CHECK.updatestatus of
            usmodified:
            begin
                if (uppercase(YPZL.fieldbyname('FD').asstring)=uppercase(engname)) or (main.edit1.text='55614') or (main.edit1.text='45077')
...
begin
   ...
   if (UpperCase(main.Edit2.Text)='CDC') and (not YPZL.FieldByName('bgColor').IsNull) then
      btnClearColor.Visible:=True
   else
      btnClearColor.Visible:=False;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      DataSet := YPZLGrid.DataSource.DataSet;
      if not Assigned(DataSet) then
      begin
        ShowMessage('No data to export!');
        Exit;
      end;
      for j := 0 to YPZLGrid.Columns.Count - 1 do
        Sheet.Cells[1, j + 1] := YPZLGrid.Columns[j].Title.Caption;
      i := 2;
      ...
      try
        ...
        begin
          for j := 0 to YPZLGrid.Columns.Count - 1 do
            Sheet.Cells[i, j + 1] := DataSet.FieldByName(YPZLGrid.Columns[j].FieldName).AsString;
          Inc(i);
          DataSet.Next;
        end;
      finally
    ...
    begin
      ...
      try
        ...
        begin
          ...
          eclApp.Cells(RowPos,4):=ypzl.FieldByName('subtype').value+'/'+sticker2.FieldByName('jijie').value;
          //eclApp.Cells(RowPos+1,2):=sticker2.FieldByName('FlexCode').value+'/'+sticker2.FieldByName('clbh').value;
          eclApp.Cells(RowPos+1,2):=sticker2.FieldByName('FlexCode').AsString + '/' +sticker2.FieldByName('clbh').AsString;
          eclApp.Cells(RowPos+1,4):=sticker2.FieldByName('kfjd').value;
          //eclApp.Cells(RowPos+3,2):=FormatDatetime('YYYY/MM/DD',Date());
          eclApp.Cells(RowPos+4,2):=sticker2.FieldByName('article').value+'/'+sticker2.FieldByName('devcode').value+'/'+sticker2.FieldByName('xieming').value+'/'+sticker2.FieldByName('YSSM').value;
...
begin
  ...
  begin
    active:=false;
    sql.Clear;
    SQL.Add('select ypdh,RE_check,XXZLKF.TRANSFER,XXZLKF.TransferIn from YPzl left join XXZLKF on XXZLKF.XieXing=YPZL.XieXing and XXZLKF.SheHao=ypzl.SheHao');
    SQL.Add('where YPDH=:YPDH ');
    //memo2.text:=sql.text;
    active:=true;
    requestlive:=false;
...
begin
  ...
  begin
    Active := false;
    SQL.Clear;
    SQL.Add('SELECT CGZLInvoiceS.CINO, CGZLInvoiceS.ScanDate, CGZLInvoiceS.CGNO, CGZLInvoiceS.CLBH, CLZL.YWPM, CLZL.DWBH,CGZLInvoiceS.Qty, ');
    SQL.Add('Case when cgzl_color.Result is null then CGZL_color2.Result else cgzl_color.Result end as Result ,');
    SQL.Add('CAST(YPZL.DEVCODE AS VARCHAR(250)) AS DEVCODE, CGZL.PURPOSE, CAST(YPZL.FD AS VARCHAR(100)) AS FD,');
    SQL.Add('CGZLInvoiceS.Qty, cgzl_color.Result, CAST(YPZL.DEVCODE AS VARCHAR(250)) AS DEVCODE, CGZL.PURPOSE, CAST(YPZL.FD AS VARCHAR(100)) AS FD,');
    ...
    SQL.Add('CGZLInvoiceS.MEMO1 AS Noted, CGZLInvoiceS.CheckID, CGZLInvoiceS.CheckDate,');
    SQL.Add('Case when cgzl_color.Remark is null then CGZL_color2.Remark else cgzl_color.Remark end as Remark ,');
    SQL.Add('MaterialNG.Remark AS NG_Remark, ZSZL.ZSYWJC as ZSYWJC, CGZLInvoiceS.CustomerApprove as CustomerApprove, CGZLInvoiceS.Customer_USERID as Customer_USERID, CGZLInvoiceS.Customer_UserDate as Customer_UserDate FROM CGZLInvoiceS');
    ...
    SQL.Add('left join CLZL_LS on CLZL_LS.CLDH=CGZLInvoiceS.CLBH or CLZL_LS.JHDH=CGZLInvoiceS.CLBH');
    SQL.Add('LEFT JOIN CGZL_color CGZL_color2 on CGZL_color2.CGNO=CGZLInvoiceS.CGNO and (CGZL_color2.CLBH=CLZL_LS.CLDH or CGZL_color2.CLBH=CLZL_LS.JHDH )');
    ...
    //========2025.04.22
    SQL.Add('INNER JOIN CGZLInvoice ON CGZLInvoice.CINO = CGZLInvoiceS.CINO AND CGZLInvoice.GSBH = '''+main.Edit2.Text+''' ');
    //========
    SQL.Add('WHERE CONVERT(VARCHAR, CGZLInvoiceS.ScanDate, 111) BETWEEN ''' + FormatDateTime('yyyy/MM/dd', DTP5.Date) + ''' AND ''' + FormatDateTime('yyyy/MM/dd', DTP6.Date) + '''');
      ...
      case QReport.UpdateStatus of
        ...
        begin
          ...
          begin
            ...
            SQL.Add(', CustomerApprove = ' + BoolToStr(QReport.FieldByName('CustomerApprove').AsBoolean, False)+ ', Customer_USERID = ''' + QReport.FieldByName('Customer_USERID').AsString + ''', Customer_UserDate = ''' + FormatDateTime('yyyy/MM/dd hh:mm:ss', QReport.FieldByName('Customer_UserDate').AsDateTime)+ '''');
...
procedure TFDExcelData.btnColorClick(Sender: TObject);
begin
  if ColorDialog1.Execute then
  begin
    YPZL.DisableControls;
    YPZLS.DisableControls;
    try
      with TempQry do
      begin
        Active := False;
        sql.Clear;
        SQL.Add('update YPZL');
        SQL.Add('set bgColor=' + IntToStr(ColorDialog1.Color));
        SQL.Add('where YPDH=''' + YPZL.FieldByName('YPDH').AsString + '''');
        SQL.Add('and KFJD=''' + YPZL.FieldByName('KFJD').AsString + '''');
        ExecSQL;
        Active := False;
      end;
      YPZl.CachedUpdates := True;
      YPZL.Edit;
      YPZL.FieldByName('bgColor').Value := ColorDialog1.Color;
      YPZL.Post;
      btnClearColor.Visible := True;
    finally
      YPZL.EnableControls; 
      YPZLS.EnableControls;
    end;
  end;
end;

procedure TFDExcelData.YPZLAfterOpen(DataSet: TDataSet);
begin
  if (UpperCase(main.Edit2.Text)='CDC') and (YPZL.RecordCount > 0)  then
...
procedure TFDExcelData.YPZLAfterClose(DataSet: TDataSet);
begin
  btnColor.Visible:=False;
  btnClearColor.Visible:=False;
end;

procedure TFDExcelData.btnClearColorClick(Sender: TObject);
begin
  if not YPZL.FieldByName('bgColor').IsNull then
  begin
    try
      ...
      YPZl.CachedUpdates := True;
      YPZL.Edit;
      YPZL.FieldByName('bgColor').Value := null;
    //YPZL.Post;
      btnClearColor.Visible := False;
    finally
      YPZL.EnableControls;
...
procedure TFDExcelData.BitBtn7Click(Sender: TObject);
begin
  with qryCheck_KT do
  begin
    active:=false;
    sql.Clear;
    SQL.Add('select ypdh,KT_Check,KT_CheckDate from YPzl');
    SQL.Add('where YPDH=:YPDH ');
    //memo2.text:=sql.text;
    active:=true;
    requestlive:=false;
...
procedure TFDExcelData.BitBtn8Click(Sender: TObject);
begin
  if ts4.TabVisible and (UpperCase(Copy(YPZL.FieldByName('ypdh').AsString, 1, 4)) = 'LAIT') then
  begin
    with qryCheck_KT do
    begin
      RequestLive := True;
  ...
  begin
    ShowMessage('It is not yours and YPDH must like "LAIT", can not modify.');
  end;
end;

procedure TFDExcelData.BitBtn9Click(Sender: TObject);
var i:integer;
begin
  try
    qryCheck_KT.First;
    for i := 1 to qryCheck_KT.RecordCount do
    begin
      case qryCheck_KT.UpdateStatus of
        usModified:
        begin
          if (ts4.TabVisible) and
             (UpperCase(Copy(YPZL.FieldByName('ypdh').AsString,1,4)) = 'LAIT') then
          begin
            qryCheck_KT.Edit;
            UpCheck_KT.Apply(ukmodify);
          end
          else
          begin
            ShowMessage('It is not yours, can not modify.');
    ...
    qryCheck_KT.Active := false;
    qryCheck_KT.CachedUpdates := false;
    qryCheck_KT.RequestLive := false;
    qryCheck_KT.Active := true;
...
procedure TFDExcelData.BitBtn10Click(Sender: TObject);
begin
  with qryCheck_KT do
  begin
    active:=false;
...
procedure TFDExcelData.BitBtn11Click(Sender: TObject);
var
 eclApp,WorkBook:olevariant;
  i,j,k,l,stop:integer;
  PartALl:string;
...
begin
    ...
    try
      ...
      //
      ExportList.Sorted := True;
      ExportList.Duplicates := dupIgnore;

      WorkBook:=eclApp.workbooks.Add;
      eclApp.ActiveSheet.Cells.NumberFormatLocal:='@';
      eclApp.ActiveSheet.PageSetup.TopMargin := 1/0.035;
      eclApp.ActiveSheet.PageSetup.BottomMargin := 1/0.035;
      eclApp.ActiveSheet.PageSetup.LeftMargin := 0;
      eclApp.ActiveSheet.PageSetup.RightMargin := 0;
      eclApp.ActiveSheet.PageSetup.HeaderMargin := 0;
      ...
      eclApp.Cells(1,1):='Material List';
      eclApp.ActiveSheet.Rows[1].Font.Size:= 20;
      eclApp.ActiveSheet.Range['A1:D2'].Merge;
      eclApp.ActiveSheet.Range['A1','D2'].HorizontalAlignment := $FFFFEFF4;
      eclApp.Cells(3,1):='SR#:'+sticker.FieldByName('devcode').value+'('+sticker.FieldByName('article').value+')'+' - ArtName: ( '+ypzl.FieldByName('XieMing').value+') '+' - ColorName: ( '+ypzl.FieldByName('YSSM').value+')';
      eclApp.ActiveSheet.Range['A3:D3'].Merge;
      eclApp.Cells(4,1):='NO.';
      eclApp.Cells(4,2):='Material No';
      eclApp.Cells(4,3):='Material Name';
      eclApp.Cells(4,4):='Supplier';
      ypzls.First;

      while not ypzls.Eof do
      begin
        PartAll := ypzls.FieldByName('clbh').AsString;

        if (Copy(PartAll, 1, 3) <> 'A13') and
           (Copy(PartAll, 1, 3) <> 'A09') and
           ((PartAll = '') or not (PartAll[1] in ['L', 'H', 'W'])) then
        begin
          KeyValue :=
            ypzls.FieldByName('clbh').AsString + '|' +
            ypzls.FieldByName('ywpm').AsString + '|' +
            ypzls.FieldByName('zsywjc').AsString;

          if ExportList.IndexOf(KeyValue) = -1 then
          begin
            ExportList.Add(KeyValue);

            eclApp.Cells(4 + i, 1) := i;
            eclApp.Cells(4 + i, 2) := ypzls.FieldByName('clbh').AsString;
            eclApp.Cells(4 + i, 3) := ypzls.FieldByName('ywpm').AsString;
            eclApp.Cells(4 + i, 4) := ypzls.FieldByName('zsywjc').AsString;
      ...
      eclApp.Columns[2].Columns.Autofit;
      eclApp.Columns[3].Columns.Autofit;
      eclApp.Columns[4].Columns.Autofit;
        for   l:=1   to  i+3 do
        begin
          eclApp.Cells[l,1].Borders[7].LineStyle := 1;
          eclApp.Cells[l,1].Borders[8].LineStyle := 1;
          eclApp.Cells[l,1].Borders[9].LineStyle := 1;
          eclApp.Cells[l,1].Borders[10].LineStyle := 1;
          eclApp.Cells[l,1].Borders[11].LineStyle := 1;
          eclApp.Cells[l,1].Borders[12].LineStyle := 1;
          eclApp.Cells[l,2].Borders[7].LineStyle := 1;
          eclApp.Cells[l,2].Borders[8].LineStyle := 1;
          eclApp.Cells[l,2].Borders[9].LineStyle := 1;
          eclApp.Cells[l,2].Borders[10].LineStyle := 1;
          eclApp.Cells[l,2].Borders[11].LineStyle := 1;
          eclApp.Cells[l,2].Borders[12].LineStyle := 1;
          eclApp.Cells[l,3].Borders[7].LineStyle := 1;
          eclApp.Cells[l,3].Borders[8].LineStyle := 1;
          eclApp.Cells[l,3].Borders[9].LineStyle := 1;
          eclApp.Cells[l,3].Borders[10].LineStyle := 1;
          eclApp.Cells[l,3].Borders[11].LineStyle := 1;
          eclApp.Cells[l,3].Borders[12].LineStyle := 1;
          eclApp.Cells[l,4].Borders[7].LineStyle := 1;
          eclApp.Cells[l,4].Borders[8].LineStyle := 1;
          eclApp.Cells[l,4].Borders[9].LineStyle := 1;
          eclApp.Cells[l,4].Borders[10].LineStyle := 1;
          eclApp.Cells[l,4].Borders[11].LineStyle := 1;
          eclApp.Cells[l,4].Borders[12].LineStyle := 1;
        end;
        inc(l);
        showmessage('Succeed.');
        eclApp.ActiveSheet.PageSetup.Orientation := 1;
...
procedure TFDExcelData.DBGridEh12Columns20UpdateData(Sender: TObject;
  var Text: String; var Value: Variant; var UseText, Handled: Boolean);
begin
  QReport.FieldByName('Customer_USERID').Value := main.Edit1.Text;
  QReport.FieldByName('Customer_UserDate').Value := Now;
end;
