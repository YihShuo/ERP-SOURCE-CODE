...
begin
  ...
  begin
      ...
      if query1.FieldByName('Pending_C').value<>'1' then
      begin
        if query1.FieldByName('CutDate').Value = null then
        begin
          if query1.FieldByName('CutDate_First').Value <= formatdatetime('YYYY/MM/DD',now) then
          begin
            fill := 0;
            SacnTBCutting();
          end else
...
begin
    ...
    begin
      ...
      if fill = 0 then
        ...
        SQL.Add('UPDATE shoetest SET Pending_C = ''1'', CutDate = GETDATE(), CutID = ''' + main.Edit1.Text + ''' WHERE Testno = ''' + edit1.Text + '''')
      else
        //SQL.Add('update shoetest set Pending_C =''1'',CutDate=getdate(),CutDate_First = getdate()  where Testno='''+edit1.text+'''');
        SQL.Add('update shoetest set Pending_C =''1'',CutDate=getdate(),CutDate_First = getdate(), CutID = ''' + main.Edit1.Text + '''  where Testno='''+edit1.text+'''');
      ExecSQL;   //2025.10.21
      SQL.Clear;
      SQL.Add('select testno,Article,TBReceive,Pending_C,CutDate,convert(varchar, CutDate_First, 111) as CutDate_First from shoetest where Testno='''+edit1.text+'''');
      Active:=true;
