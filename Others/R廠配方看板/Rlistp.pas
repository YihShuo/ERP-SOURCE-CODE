...
procedure TRlists.FormCreate(Sender: TObject);
var s:string;
begin
...
procedure TRlists.cbx1Change(Sender: TObject);
begin
  ...
  begin
    active:=false;
    sql.Clear;
    sql.add('select pfbh2 from GSPF ');
    sql.add('where gsbh = ''' +  Edit2.Text + ''' and pfbh1 = ''' + cbx1.Text + '''');
    if mark = 0 then
      sql.add('and invisible = 0');
    //ShowMessage(sql.Text);
    //memo2.Text:=sql.Text;
    active:=true;
    if RecordCount <> 0 then
...
procedure TRlists.Timer1Timer(Sender: TObject);
var
...
procedure TRlists.Button2Click(Sender: TObject);
begin
...
procedure TRlists.Button1Click(Sender: TObject);
var okPrint:integer;
begin
  with Query1 do
  begin
...
// 刪除所有資料
procedure TRlists.Button3Click(Sender: TObject);
begin
...
procedure TRlists.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
...
procedure TRlists.FormClose(Sender: TObject; var Action: TCloseAction);
begin
action:=cafree;
end;

procedure TRlists.FormDestroy(Sender: TObject);
begin
Rlists:=nil;
end;

procedure TRlists.Label4Click(Sender: TObject);
begin
  if mark = 0 then
  begin
    mark:=1;
