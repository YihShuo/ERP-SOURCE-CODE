...
uses Messages, {$IFDEF LINUX} WinUtils, {$ENDIF} Windows, SysUtils, Classes,
  Variants, Graphics, Menus, Controls, Forms, StdCtrls, Mask, D7GesturesHeader;
...
type
  ...
  TTouchCustomGrid = class;

  TInplaceEdit = class(TCustomMaskEdit)
  private
    FGrid: TTouchCustomGrid;
    FClickTime: Longint;
    procedure InternalMove(const Loc: TRect; Redraw: Boolean);
    procedure SetGrid(Value: TTouchCustomGrid);
  ...
  protected
    ...
    property  Grid: TTouchCustomGrid read FGrid;
  public
  ...
  TTouchCustomGrid = class(TCustomControl)
  private
    FAnchor: TGridCoord;
  ...
  TTouchCustomDrawGrid = class(TTouchCustomGrid)
  private
  ...
  TTouchDrawGrid = class(TTouchCustomDrawGrid)
  private
  ...
  TTouchStringGrid = class;

  TStringGridStrings = class(TStrings)
  private
    FGrid: TTouchStringGrid;
    FIndex: Integer;
  ...
  public
    constructor Create(AGrid: TTouchStringGrid; AIndex: Longint);
  ...
  TTouchStringGrid = class(TTouchDrawGrid)
  private
    FData: Pointer;
    FRows: Pointer;
    FCols: Pointer;
...
procedure Register;
begin
  RegisterComponents('Samples', [TTouchStringGrid]);
end;
...
procedure TInplaceEdit.SetGrid(Value: TTouchCustomGrid);
begin
  FGrid := Value;
end;
...
constructor TTouchCustomGrid.Create(AOwner: TComponent);
const
...
destructor TTouchCustomGrid.Destroy;
begin
  FInplaceEdit.Free;
  inherited Destroy;
  FreeMem(FColWidths);
...
procedure TTouchCustomGrid.AdjustSize(Index, Amount: Longint; Rows: Boolean);
var
  NewCur: TGridCoord;
...
function TTouchCustomGrid.BoxRect(ALeft, ATop, ARight, ABottom: Longint): TRect;
var
  GridRect: TGridRect;
begin
  GridRect.Left := ALeft;
...
procedure TTouchCustomGrid.DoExit;
begin
  inherited DoExit;
  if not (goAlwaysShowEditor in Options) then HideEditor;
end;

function TTouchCustomGrid.CellRect(ACol, ARow: Longint): TRect;
begin
  Result := BoxRect(ACol, ARow, ACol, ARow);
end;

function TTouchCustomGrid.CanEditAcceptKey(Key: Char): Boolean;
begin
  Result := True;
end;

function TTouchCustomGrid.CanGridAcceptKey(Key: Word; Shift: TShiftState): Boolean;
begin
  Result := True;
end;

function TTouchCustomGrid.CanEditModify: Boolean;
begin
  Result := FCanEditModify;
end;

function TTouchCustomGrid.CanEditShow: Boolean;
begin
...
function TTouchCustomGrid.IsActiveControl: Boolean;
var
  H: Hwnd;
...
function TTouchCustomGrid.GetEditMask(ACol, ARow: Longint): string;
begin
  Result := '';
end;

function TTouchCustomGrid.GetEditText(ACol, ARow: Longint): string;
begin
  Result := '';
end;

procedure TTouchCustomGrid.SetEditText(ACol, ARow: Longint; const Value: string);
begin
end;

function TTouchCustomGrid.GetEditLimit: Integer;
begin
  Result := 0;
end;

function TTouchCustomGrid.GetEditStyle(ACol, ARow: Longint): TEditStyle;
begin
  Result := esSimple;
end;

procedure TTouchCustomGrid.HideEditor;
begin
  FEditorMode := False;
  HideEdit;
end;

procedure TTouchCustomGrid.ShowEditor;
begin
  FEditorMode := True;
  UpdateEdit;
end;

procedure TTouchCustomGrid.ShowEditorChar(Ch: Char);
begin
  ShowEditor;
...
procedure TTouchCustomGrid.InvalidateEditor;
begin
  FInplaceCol := -1;
  FInplaceRow := -1;
  UpdateEdit;
...
procedure TTouchCustomGrid.ReadColWidths(Reader: TReader);
var
  I: Integer;
begin
  with Reader do
...
procedure TTouchCustomGrid.ReadRowHeights(Reader: TReader);
var
  I: Integer;
begin
  with Reader do
...
procedure TTouchCustomGrid.WriteColWidths(Writer: TWriter);
var
  I: Integer;
begin
  with Writer do
...
procedure TTouchCustomGrid.WriteRowHeights(Writer: TWriter);
var
  I: Integer;
begin
  with Writer do
...
procedure TTouchCustomGrid.DefineProperties(Filer: TFiler);

  function DoColWidths: Boolean;
  begin
    if Filer.Ancestor <> nil then
      Result := not CompareExtents(TTouchCustomGrid(Filer.Ancestor).FColWidths, FColWidths)
    else
  ...
  begin
    if Filer.Ancestor <> nil then
      Result := not CompareExtents(TTouchCustomGrid(Filer.Ancestor).FRowHeights, FRowHeights)
    else
...
procedure TTouchCustomGrid.MoveColumn(FromIndex, ToIndex: Longint);
var
  Rect: TGridRect;
begin
...
procedure TTouchCustomGrid.ColumnMoved(FromIndex, ToIndex: Longint);
begin
end;

procedure TTouchCustomGrid.MoveRow(FromIndex, ToIndex: Longint);
begin
...
procedure TTouchCustomGrid.RowMoved(FromIndex, ToIndex: Longint);
begin
end;

function TTouchCustomGrid.MouseCoord(X, Y: Integer): TGridCoord;
var
...
procedure TTouchCustomGrid.MoveColRow(ACol, ARow: Longint; MoveAnchor,
  Show: Boolean);
begin
  MoveCurrent(ACol, ARow, MoveAnchor, Show);
end;

function TTouchCustomGrid.SelectCell(ACol, ARow: Longint): Boolean;
begin
  Result := True;
end;

procedure TTouchCustomGrid.SizeChanged(OldColCount, OldRowCount: Longint);
begin
end;

function TTouchCustomGrid.Sizing(X, Y: Integer): Boolean;
var
  DrawInfo: TGridDrawInfo;
  State: TGridState;
  Index: Longint;
...
procedure TTouchCustomGrid.TopLeftChanged;
begin
...
procedure TTouchCustomGrid.Paint;
var
  LineColor: TColor;
  DrawInfo: TGridDrawInfo;
  Sel: TGridRect;
...
function TTouchCustomGrid.CalcCoordFromPoint(X, Y: Integer;
  const DrawInfo: TGridDrawInfo): TGridCoord;
...
procedure TTouchCustomGrid.CalcDrawInfo(var DrawInfo: TGridDrawInfo);
begin
  CalcDrawInfoXY(DrawInfo, ClientWidth, ClientHeight);
end;

procedure TTouchCustomGrid.CalcDrawInfoXY(var DrawInfo: TGridDrawInfo;
  UseWidth, UseHeight: Integer);
...
procedure TTouchCustomGrid.CalcFixedInfo(var DrawInfo: TGridDrawInfo);
...
function TTouchCustomGrid.CalcMaxTopLeft(const Coord: TGridCoord;
  const DrawInfo: TGridDrawInfo): TGridCoord;
...
procedure TTouchCustomGrid.CalcSizingState(X, Y: Integer; var State: TGridState;
  var Index: Longint; var SizingPos, SizingOfs: Integer;
...
procedure TTouchCustomGrid.ChangeGridOrientation(RightToLeftOrientation: Boolean);
var
  Org: TPoint;
  Ext: TPoint;
begin
...
procedure TTouchCustomGrid.ChangeSize(NewColCount, NewRowCount: Longint);
var
...
procedure TTouchCustomGrid.ClampInView(const Coord: TGridCoord);
var
  DrawInfo: TGridDrawInfo;
...
procedure TTouchCustomGrid.DrawSizingLine(const DrawInfo: TGridDrawInfo);
var
  OldPen: TPen;
begin
  OldPen := TPen.Create;
...
procedure TTouchCustomGrid.DrawMove;
var
  OldPen: TPen;
  Pos: Integer;
  R: TRect;
...
procedure TTouchCustomGrid.FocusCell(ACol, ARow: Longint; MoveAnchor: Boolean);
begin
...
procedure TTouchCustomGrid.GridRectToScreenRect(GridRect: TGridRect;
  var ScreenRect: TRect; IncludeLine: Boolean);
...
procedure TTouchCustomGrid.Initialize;
begin
...
procedure TTouchCustomGrid.InvalidateCell(ACol, ARow: Longint);
var
  Rect: TGridRect;
begin
  Rect.Top := ARow;
...
procedure TTouchCustomGrid.InvalidateCol(ACol: Longint);
var
  Rect: TGridRect;
begin
  if not HandleAllocated then Exit;
...
procedure TTouchCustomGrid.InvalidateRow(ARow: Longint);
var
  Rect: TGridRect;
begin
  if not HandleAllocated then Exit;
...
procedure TTouchCustomGrid.InvalidateGrid;
begin
  Invalidate;
end;

procedure TTouchCustomGrid.InvalidateRect(ARect: TGridRect);
var
  InvalidRect: TRect;
begin
  if not HandleAllocated then Exit;
...
procedure TTouchCustomGrid.ModifyScrollBar(ScrollBar, ScrollCode, Pos: Cardinal;
  UseRightToLeft: Boolean);
var
...
procedure TTouchCustomGrid.MoveAdjust(var CellPos: Longint; FromIndex, ToIndex: Longint);
var
  Min, Max: Longint;
begin
...
procedure TTouchCustomGrid.MoveAnchor(const NewAnchor: TGridCoord);
var
  OldSel: TGridRect;
begin
...
procedure TTouchCustomGrid.MoveCurrent(ACol, ARow: Longint; MoveAnchor,
  Show: Boolean);
var
  OldSel: TGridRect;
...
procedure TTouchCustomGrid.MoveTopLeft(ALeft, ATop: Longint);
var
  OldTopLeft: TGridCoord;
begin
...
procedure TTouchCustomGrid.ResizeCol(Index: Longint; OldSize, NewSize: Integer);
begin
  InvalidateGrid;
end;

procedure TTouchCustomGrid.ResizeRow(Index: Longint; OldSize, NewSize: Integer);
begin
  InvalidateGrid;
end;

procedure TTouchCustomGrid.SelectionMoved(const OldSel: TGridRect);
var
...
procedure TTouchCustomGrid.ScrollDataInfo(DX, DY: Integer;
...
procedure TTouchCustomGrid.ScrollData(DX, DY: Integer);
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
procedure TTouchCustomGrid.TopLeftMoved(const OldTopLeft: TGridCoord);
...
procedure TTouchCustomGrid.UpdateScrollPos;
var
  DrawInfo: TGridDrawInfo;
  MaxTopLeft: TGridCoord;
  GridSpace, ColWidth: Integer;
...
procedure TTouchCustomGrid.UpdateScrollRange;
var
...
function TTouchCustomGrid.CreateEditor: TInplaceEdit;
begin
  Result := TInplaceEdit.Create(Self);
end;

procedure TTouchCustomGrid.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  with Params do
  begin
...
procedure TTouchCustomGrid.KeyDown(var Key: Word; Shift: TShiftState);
var
...
procedure TTouchCustomGrid.KeyPress(var Key: Char);
begin
...
procedure TTouchCustomGrid.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  CellHit: TGridCoord;
  DrawInfo: TGridDrawInfo;
...
procedure TTouchCustomGrid.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  DrawInfo: TGridDrawInfo;
  CellHit: TGridCoord;
begin
...
procedure TTouchCustomGrid.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  DrawInfo: TGridDrawInfo;
  NewSize: Integer;
...
procedure TTouchCustomGrid.MoveAndScroll(Mouse, CellHit: Integer;
...
function TTouchCustomGrid.GetColWidths(Index: Longint): Integer;
begin
...
function TTouchCustomGrid.GetRowHeights(Index: Longint): Integer;
begin
...
function TTouchCustomGrid.GetGridWidth: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
function TTouchCustomGrid.GetGridHeight: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
function TTouchCustomGrid.GetSelection: TGridRect;
begin
  Result := GridRect(FCurrent, FAnchor);
end;

function TTouchCustomGrid.GetTabStops(Index: Longint): Boolean;
begin
  if FTabStops = nil then Result := True
  else Result := Boolean(PIntArray(FTabStops)^[Index + 1]);
end;

function TTouchCustomGrid.GetVisibleColCount: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
function TTouchCustomGrid.GetVisibleRowCount: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
procedure TTouchCustomGrid.SetBorderStyle(Value: TBorderStyle);
begin
...
procedure TTouchCustomGrid.SetCol(Value: Longint);
begin
  if Col <> Value then FocusCell(Value, Row, True);
end;

procedure TTouchCustomGrid.SetColCount(Value: Longint);
begin
...
procedure TTouchCustomGrid.SetColWidths(Index: Longint; Value: Integer);
begin
  if FColWidths = nil then
    UpdateExtents(FColWidths, ColCount, DefaultColWidth);
...
procedure TTouchCustomGrid.SetDefaultColWidth(Value: Integer);
begin
...
procedure TTouchCustomGrid.SetDefaultRowHeight(Value: Integer);
begin
...
procedure TTouchCustomGrid.SetFixedColor(Value: TColor);
begin
...
procedure TTouchCustomGrid.SetFixedCols(Value: Integer);
begin
...
procedure TTouchCustomGrid.SetFixedRows(Value: Integer);
begin
...
procedure TTouchCustomGrid.SetEditorMode(Value: Boolean);
begin
  if not Value then
    HideEditor
  else
...
procedure TTouchCustomGrid.SetGridLineWidth(Value: Integer);
begin
...
procedure TTouchCustomGrid.SetLeftCol(Value: Longint);
begin
  if FTopLeft.X <> Value then MoveTopLeft(Value, TopRow);
end;

procedure TTouchCustomGrid.SetOptions(Value: TGridOptions);
begin
...
procedure TTouchCustomGrid.SetRow(Value: Longint);
begin
  if Row <> Value then FocusCell(Col, Value, True);
end;

procedure TTouchCustomGrid.SetRowCount(Value: Longint);
begin
...
procedure TTouchCustomGrid.SetRowHeights(Index: Longint; Value: Integer);
begin
  if FRowHeights = nil then
    UpdateExtents(FRowHeights, RowCount, DefaultRowHeight);
...
procedure TTouchCustomGrid.SetScrollBars(Value: TScrollStyle);
begin
...
procedure TTouchCustomGrid.SetSelection(Value: TGridRect);
var
  OldSel: TGridRect;
begin
  OldSel := Selection;
...
procedure TTouchCustomGrid.SetTabStops(Index: Longint; Value: Boolean);
begin
  if FTabStops = nil then
    UpdateExtents(FTabStops, ColCount, Integer(True));
  if Index >= ColCount then InvalidOp(SIndexOutOfRange);
...
procedure TTouchCustomGrid.SetTopRow(Value: Longint);
begin
  if FTopLeft.Y <> Value then MoveTopLeft(LeftCol, Value);
end;

procedure TTouchCustomGrid.HideEdit;
begin
  if FInplaceEdit <> nil then
    try
      UpdateText;
...
procedure TTouchCustomGrid.UpdateEdit;
...
procedure TTouchCustomGrid.WMCommand(var Message: TWMCommand);
begin
  with Message do
  begin
...
procedure TTouchCustomGrid.WMGetDlgCode(var Msg: TWMGetDlgCode);
begin
...
procedure TTouchCustomGrid.WMKillFocus(var Msg: TWMKillFocus);
begin
  inherited;
  InvalidateRect(Selection);
...
procedure TTouchCustomGrid.WMLButtonDown(var Message: TMessage);
begin
  inherited;
  if FInplaceEdit <> nil then FInplaceEdit.FClickTime := GetMessageTime;
end;

procedure TTouchCustomGrid.WMNCHitTest(var Msg: TWMNCHitTest);
begin
  DefaultHandler(Msg);
  FHitTest := ScreenToClient(SmallPointToPoint(Msg.Pos));
end;

procedure TTouchCustomGrid.WMSetCursor(var Msg: TWMSetCursor);
var
  DrawInfo: TGridDrawInfo;
  State: TGridState;
  Index: Longint;
...
procedure TTouchCustomGrid.WMSetFocus(var Msg: TWMSetFocus);
begin
  inherited;
...
procedure TTouchCustomGrid.WMSize(var Msg: TWMSize);
begin
  inherited;
  UpdateScrollRange;
...
procedure TTouchCustomGrid.WMVScroll(var Msg: TWMVScroll);
begin
  ModifyScrollBar(SB_VERT, Msg.ScrollCode, Msg.Pos, True);
end;

procedure TTouchCustomGrid.WMHScroll(var Msg: TWMHScroll);
begin
  ModifyScrollBar(SB_HORZ, Msg.ScrollCode, Msg.Pos, True);
end;

procedure TTouchCustomGrid.CancelMode;
var
  DrawInfo: TGridDrawInfo;
begin
  try
...
procedure TTouchCustomGrid.WMCancelMode(var Msg: TWMCancelMode);
begin
  inherited;
  CancelMode;
end;

procedure TTouchCustomGrid.CMCancelMode(var Msg: TMessage);
begin
...
procedure TTouchCustomGrid.CMFontChanged(var Message: TMessage);
begin
  if FInplaceEdit <> nil then FInplaceEdit.Font := Font;
  inherited;
end;

procedure TTouchCustomGrid.CMCtl3DChanged(var Message: TMessage);
begin
  inherited;
  RecreateWnd;
end;

procedure TTouchCustomGrid.CMDesignHitTest(var Msg: TCMDesignHitTest);
begin
  Msg.Result := Longint(BOOL(Sizing(Msg.Pos.X, Msg.Pos.Y)));
end;

procedure TTouchCustomGrid.CMWantSpecialKey(var Msg: TCMWantSpecialKey);
begin
  inherited;
  if (goEditing in Options) and (Char(Msg.CharCode) = #13) then Msg.Result := 1;
end;

procedure TTouchCustomGrid.TimedScroll(Direction: TGridScrollDirection);
var
...
procedure TTouchCustomGrid.WMTimer(var Msg: TWMTimer);
var
  Point: TPoint;
  DrawInfo: TGridDrawInfo;
...
procedure TTouchCustomGrid.ColWidthsChanged;
begin
  UpdateScrollRange;
  UpdateEdit;
end;

procedure TTouchCustomGrid.RowHeightsChanged;
begin
  UpdateScrollRange;
  UpdateEdit;
end;

procedure TTouchCustomGrid.DeleteColumn(ACol: Longint);
begin
  MoveColumn(ACol, ColCount-1);
  ColCount := ColCount - 1;
end;

procedure TTouchCustomGrid.DeleteRow(ARow: Longint);
begin
  MoveRow(ARow, RowCount - 1);
  RowCount := RowCount - 1;
end;

procedure TTouchCustomGrid.UpdateDesigner;
var
  ParentForm: TCustomForm;
begin
...
function TTouchCustomGrid.DoMouseWheelDown(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
...
function TTouchCustomGrid.DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
...
function TTouchCustomGrid.CheckColumnDrag(var Origin,
  Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TTouchCustomGrid.CheckRowDrag(var Origin,
  Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TTouchCustomGrid.BeginColumnDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TTouchCustomGrid.BeginRowDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TTouchCustomGrid.EndColumnDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TTouchCustomGrid.EndRowDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

procedure TTouchCustomGrid.CMShowingChanged(var Message: TMessage);
begin
  inherited;
...
function TTouchCustomDrawGrid.CellRect(ACol, ARow: Longint): TRect;
begin
  Result := inherited CellRect(ACol, ARow);
end;

procedure TTouchCustomDrawGrid.MouseToCell(X, Y: Integer; var ACol, ARow: Longint);
var
  Coord: TGridCoord;
begin
...
procedure TTouchCustomDrawGrid.ColumnMoved(FromIndex, ToIndex: Longint);
begin
  if Assigned(FOnColumnMoved) then FOnColumnMoved(Self, FromIndex, ToIndex);
end;

function TTouchCustomDrawGrid.GetEditMask(ACol, ARow: Longint): string;
begin
  Result := '';
  if Assigned(FOnGetEditMask) then FOnGetEditMask(Self, ACol, ARow, Result);
end;

function TTouchCustomDrawGrid.GetEditText(ACol, ARow: Longint): string;
begin
  Result := '';
  if Assigned(FOnGetEditText) then FOnGetEditText(Self, ACol, ARow, Result);
end;

procedure TTouchCustomDrawGrid.RowMoved(FromIndex, ToIndex: Longint);
begin
  if Assigned(FOnRowMoved) then FOnRowMoved(Self, FromIndex, ToIndex);
end;

function TTouchCustomDrawGrid.SelectCell(ACol, ARow: Longint): Boolean;
begin
  Result := True;
  if Assigned(FOnSelectCell) then FOnSelectCell(Self, ACol, ARow, Result);
end;

procedure TTouchCustomDrawGrid.SetEditText(ACol, ARow: Longint; const Value: string);
begin
  if Assigned(FOnSetEditText) then FOnSetEditText(Self, ACol, ARow, Value);
end;

procedure TTouchCustomDrawGrid.DrawCell(ACol, ARow: Longint; ARect: TRect;
...
procedure TTouchCustomDrawGrid.TopLeftChanged;
begin
...
constructor TStringGridStrings.Create(AGrid: TTouchStringGrid; AIndex: Longint);
begin
  inherited Create;
  FGrid := AGrid;
  FIndex := AIndex;
...
procedure TTouchDrawGrid.WMTouch(var Msg: TMessage);
...
procedure TTouchStringGrid.ColumnMoved(FromIndex, ToIndex: Longint);
...
procedure TTouchStringGrid.RowMoved(FromIndex, ToIndex: Longint);
begin
...
function TTouchStringGrid.GetEditText(ACol, ARow: Longint): string;
begin
  Result := Cells[ACol, ARow];
  if Assigned(FOnGetEditText) then FOnGetEditText(Self, ACol, ARow, Result);
end;

procedure TTouchStringGrid.SetEditText(ACol, ARow: Longint; const Value: string);
begin
  DisableEditUpdate;
  try
...
procedure TTouchStringGrid.DrawCell(ACol, ARow: Longint; ARect: TRect;
  AState: TGridDrawState);
begin
...
procedure TTouchStringGrid.DisableEditUpdate;
begin
  Inc(FEditUpdate);
end;

procedure TTouchStringGrid.EnableEditUpdate;
begin
  Dec(FEditUpdate);
end;

procedure TTouchStringGrid.Initialize;
var
  quantum: TSPAQuantum;
begin
  if FCols = nil then
...
procedure TTouchStringGrid.SetUpdateState(Updating: Boolean);
begin
  FUpdating := Updating; 
...
procedure TTouchStringGrid.Update(ACol, ARow: Integer);
begin
...
function  TTouchStringGrid.EnsureColRow(Index: Integer; IsCol: Boolean):
  TStringGridStrings;
var
  RCIndex: Integer;
  PList: ^TSparseList;
...
function  TTouchStringGrid.EnsureDataRow(ARow: Integer): Pointer;
var
  quantum: TSPAQuantum;
begin
...
function TTouchStringGrid.GetCols(Index: Integer): TStrings;
begin
  Result := EnsureColRow(Index, True);
end;

function TTouchStringGrid.GetObjects(ACol, ARow: Integer): TObject;
var
  ssl: TStringSparseList;
begin
  ssl := TStringSparseList(TSparseList(FData)[ARow]);
...
function TTouchStringGrid.GetRows(Index: Integer): TStrings;
begin
  Result := EnsureColRow(Index, False);
end;

procedure TTouchStringGrid.SetCells(ACol, ARow: Integer; const Value: string);
begin
...
procedure TTouchStringGrid.SetCols(Index: Integer; Value: TStrings);
begin
  EnsureColRow(Index, True).Assign(Value);
end;

procedure TTouchStringGrid.SetObjects(ACol, ARow: Integer; Value: TObject);
begin
...
procedure TTouchStringGrid.SetRows(Index: Integer; Value: TStrings);
begin
