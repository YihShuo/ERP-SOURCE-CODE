...
begin
  ...
  begin
    active:=false;
    sql.Clear;
    sql.add('select CQDH from CQZL order by CQDH');
    active:=true;
...
procedure TRWeightp.DBGrid3EditButtonClick(Sender: TObject);
begin
...
procedure TRWeightp.btModifyPFBHClick(Sender: TObject);
begin
  PFBH:=TPFBH.create(self);
  PFBH.show();
end;

procedure TRWeightp.BitBtn2Click(Sender: TObject);
var i,j:integer;
begin
 GSPF.First;
 j:=0;
 ...
 begin
    ...
    begin
      Active:=false;
      sql.Clear;
      Sql.Add('select  *  from GSPFS where GSPFS.PFBH2 = '''+GSPF.fieldByName('PFBH2').AsString+'''');
      sql.Add('and CLBH not in (select cldhz from clzlsl where CLDH='''+GSPF.fieldByName('PFBH2').asstring+''')');
      active:=true;
      ...
      begin
        ...
        begin
          active:=false;
          sql.clear;
          Sql.Add('delete  from GSPFS where GSPFS.PFBH2 = '''+GSPF.fieldByName('PFBH2').AsString+'''');
          sql.Add('and CLBH not in (select cldhz from clzlsl where CLDH='''+GSPF.fieldByName('PFBH2').asstring+''')');
          execsql;
          inc(j);
        end;
...
procedure TRWeightp.Database1Login(Database: TDatabase;
