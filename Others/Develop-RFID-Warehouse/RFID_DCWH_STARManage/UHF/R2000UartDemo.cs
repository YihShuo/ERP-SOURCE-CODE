...
namespace UHF
{
    public partial class R2000UartDemo : Form
    {
        ...
        private string str = "server=192.168.23.9;database=LIY_TYXUAN;uid=tyx;pwd=tyx";
            ...
            if (btRealTimeInventory.Text == "Start Read")
            {
                ...
                for (int i = 0; i <= lvRealList.Items.Count - 1; i++)
                {
                    ...
                    else
                    {
                        ...
                        if (r > 0)
                        {
                            sql = "Insert into LIY_ERP.dbo.BDelRec (TNO,CLBH,DELID,DELDATE,OldID,TableName) " +
                                  "Values('" + textBox1.Text + "', '" + lvRealList.Items[i].SubItems[2].Text + "', '" 
                                             + lvRealList.Items[i].SubItems[4].Text + "', '" 
                                             + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "', 'DCWH_STAR', 'RFID_TB')";
                            try
                            {
                                cmd = new SqlCommand(sql, conn);
                                cmd.ExecuteNonQuery();
                            ...
                            if (IO == "RECEIVE")
                            {
                                ...
                                sql = "Insert into RFID_TRACK (TESTNO,RFIDNO,USERID,USERDATE,IO,DEPARTMENT,MEMO,ARTICLE,WorkID,WHNO,SNO,PROGRAM_ID) " +
                                      "Values ('" + textBox1.Text + "', '" + lvRealList.Items[i].SubItems[2].Text + "', '" 
                                                  ...
                                                  + "', 'MOVE OUT', '" + comboBox1.Text + "', '" + memo + "', '" + label34.Text + "', '" 
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            if (btRealTimeInventory.Text == "Start Read")
            {
                ...
                for (int i = 0; i <= lvRealList.Items.Count - 1; i++)
                {
                    ...
                    if (DEPARTMENT == "" && IO == "" && comboBox1.Text != "")
                    {
                        lrtxtLog.AppendText(lvRealList.Items[i].SubItems[2].Text + "\n");
                    }
                    else if (IO == "MOVE OUT" | temp==1 | (DEPARTMENT == "" && IO == "" && comboBox1.Text == ""))
                    {
                        ...
                        else
                        {
                            ...
                            conn.Close();
                            conn.Open();
                            if (comboBox1.Text == "" | comboBox1.Text == "DC_REF")
                            {
                                sql = "Insert into RFID_TRACK (TESTNO,RFIDNO,USERID,USERDATE,IO,DEPARTMENT,ARTICLE,WHNO,SNO,PROGRAM_ID) " +
                                      "Values ('" + textBox1.Text + "', '" + lvRealList.Items[i].SubItems[2].Text + "', '" 
                                                + lvRealList.Items[i].SubItems[4].Text + "', '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") 
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            string sql = "select Article from LIY_ERP.dbo.shoetest where testno='" + textBox1.Text + "' ";
            SqlCommand cmd = new SqlCommand(sql, conn);
            SqlDataReader dataReader = cmd.ExecuteReader();
        ...
        private void menuItem22_Click(object sender, EventArgs e)
        {
            ...
            lrtxtLog.Clear();
            if (lvdel.SelectedItems[0].SubItems[9].Text.Equals("Not in warehouse"))
            {
            ...
            {
                int promptValue = ShowDialog(comboBox4,lvdel.SelectedItems[0].SubItems[9].Text, lvdel.SelectedItems[0].SubItems[10].Text, lvdel.SelectedItems[0].SubItems[1].Text, lvdel.SelectedItems[0].SubItems[2].Text, textBox8.Text, lvdel.SelectedItems[0].SubItems[4].Text);
                if (promptValue == 1)
                {
        ...
        public static int ShowDialog(ComboBox cbt, string wh, string s, string tesno, string rfid, string id, string sr)
        {
            ...
            btnsave.Click += (sender, e) => {
                SqlConnection conn;
                string str = "server=192.168.23.9;database= LIY_TYXUAN;uid=sa;pwd=jack";
                SqlCommand cmd;
                conn = new SqlConnection(str);
                conn.Open();
        ...
        private void menuItem11_Click(object sender, EventArgs e)
        {
            lvtempe.Items.Clear();
            int i = 1;
            string[] dates = lvdel.SelectedItems[0].SubItems[8].Text.Split(new char[] { '/' });
            ...
            SqlConnection conn = new SqlConnection(str);
            conn.Open();
            string sql = "select IO, DEPARTMENT, MEMO, RFID_TRACK.USERID, convert(varchar, RFID_TRACK.USERDATE, 103) as USERDATE," +
            ...
            while (dataReader.Read())
            {
                ...
                if (i == 1)
                {
                    ...
                    item.SubItems.Add("(Date Input) " + lvdel.SelectedItems[0].SubItems[8].Text);
                    item.SubItems.Add(dataReader["WHNO"].ToString());
                    item.SubItems.Add(dataReader["SNO"].ToString());
                ...
                else
                {
                    ...
                    item.SubItems.Add(lvdel.SelectedItems[0].SubItems[6].Text);
                    item.SubItems.Add(lvdel.SelectedItems[0].SubItems[7].Text);
                    if (dataReader["WorkID"].ToString().Equals(""))
                    {
            ...
            if (btRealTimeInventory2.Text == "Start Read")
            {
                if (lvdel.CheckedItems.Count > 0)
                {
                    ...
                    if (confirmation == DialogResult.Yes)
                    {
                        ...
                        {
                            sql = "select top 1 DEPARTMENT,IO from RFID_TRACK where RFIDNO='" + lv.SubItems[2].Text + "' order by USERDATE DESC";
                            cmd = new SqlCommand(sql, conn);
                            SqlDataReader dataReader = cmd.ExecuteReader();
                            string DEPARTMENT = "";
                            string IO = "";
                            ...
                            dataReader.Close();
                            //if ((IO == "RECEIVE" && DEPARTMENT == "DC QC") | (IO == "RECEIVE" && DEPARTMENT == "QC"))
                            //{
                            //    conn.Close();
                            //    MessageBox.Show("RFIDNO already in data QC receive !!!");
                            //    return;
                            //}
                            ListViewItem item = new ListViewItem(i.ToString());
                            ...
                            foreach (ListViewItem lv in lvdel.CheckedItems)
                            {
                                sql = "Insert into RFID_DEL (TESTNO,RFIDNO,DELUSER,DELDATE,PROGRAM_ID,MEMO) " +
                                      "Values ('" + lv.SubItems[1].Text + "', '" + lv.SubItems[2].Text + "', '" + textBox8.Text + "', '"
                                                  + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "', 'DCWH_STAR', 'Waste report')";
                                try
                                {
                                    cmd = new SqlCommand(sql, conn);
                                    cmd.ExecuteNonQuery();
                    ...
                    if (confirmation == DialogResult.Yes)
                    {
                        ...
                        if (lrtxtLog.Text == "")
                        {
                            foreach (ListViewItem lv in lvdel.CheckedItems)
                            {
                                sql = "Insert into RFID_DEL (TESTNO,RFIDNO,DELUSER,DELDATE,PROGRAM_ID,MEMO) " +
                                      "Values ('" + lv.SubItems[1].Text + "', '" + lv.SubItems[2].Text + "', '" + textBox8.Text + "', '"
                                                  + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "', 'DCWH_STAR', 'Ship for customers')";
                                try
                                {
                                    cmd = new SqlCommand(sql, conn);
                                    cmd.ExecuteNonQuery();
                ...
                if (lvdel.CheckedItems.Count > 0)
                {
                    ...
                    foreach (ListViewItem lv in lvdel.CheckedItems)
                    {
                        ...
                        else
                        {
                            ...
                            else if (userid[0].Equals("DC_REF"))
                            {
                    ...
                    if (confirmation == DialogResult.Yes)
                    {
                        ...
                        if (lrtxtLog.Text == "")
                        {
                            foreach (ListViewItem lv in lvdel.CheckedItems)
                            {
                                sql = "Insert into RFID_DEL (TESTNO,RFIDNO,DELUSER,DELDATE,PROGRAM_ID,MEMO) " +
                                      "Values ('" + lv.SubItems[1].Text + "', '" + lv.SubItems[2].Text + "', '" + textBox8.Text + "', '"
                                                  + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "', 'DCWH_STAR', '" + richTextBox1.Text + "')";
                                try
                                {
                                    cmd = new SqlCommand(sql, conn);
                                    cmd.ExecuteNonQuery();
