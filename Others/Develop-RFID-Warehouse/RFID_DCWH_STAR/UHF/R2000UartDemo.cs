...
namespace UHF
{
    public partial class R2000UartDemo : Form
    {
        ...
        private void FrmMain_Load(object sender, EventArgs e)
        {
            ...
            DP1.Value = DateTime.Today.AddDays(-30);
            ...
            //textBox4.Text = "41065";
            //label71.Text = "NGUYEN TRAM MINH QUAN";
        ...
        private void tabCtrMain_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (tabCtrMain.SelectedIndex == 4 && btnConnectRs232.Enabled == true)
            {
                tabCtrMain.SelectedIndex = 0;
                MessageBox.Show("Please Connect RFID Reader First !");
                return;
            }
            if (tabCtrMain.SelectedIndex == 4 && button2.Text == "Login")
            {
                tabCtrMain.SelectedIndex = 1;
                MessageBox.Show("Please Login First !");
                return;
            ...
            lvdel.Height = pagList.Height - (groupBox6.Height + 12);
            lvRealList.Height = pageEpcTest.Height - (lbRealTagCount.Top + 28);
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (btRealTimeInventory.Text == "Start Read")
            {
                ...
                for (int i = 0; i <= lvRealList.Items.Count - 1; i++)
                {
                    ...
                    sql = "select COUNT(*) from LIY_ERP.dbo.BDelRec " +
                    ...
                    if (r == 0 && y == 0 && t == 0)
                    {
                        ...
                        sql = "Insert into RFID_TB (TESTNO,RFIDNO,USERID,USERDATE,ARTICLE,PROGRAM_ID) " +
                              "Values('" + textBox1.Text + "', '" + lvRealList.Items[i].SubItems[2].Text + "', '"
                                         + lvRealList.Items[i].SubItems[4].Text + "', '"
                                         + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "', '" + label34.Text + "', 'DCWH_STAR')";
                        try
                        {
                            cmd = new SqlCommand(sql, conn);
                            cmd.ExecuteNonQuery();
                    ...
                    else
                    {
                        ...
                        if (y != 0 | t != 0)
                        {
                            MessageBox.Show("RFIDNO already in data delete !!!");
                        }
                    }
                }
                if (j > 0)
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (btRealTimeInventory.Text == "Start Read")
            {
                ...
                for (int i = 0; i <= lvRealList.Items.Count - 1; i++)
                {
                    sql = "select COUNT(*) from RFID_TRACK where RFIDNO='" + lvRealList.Items[i].SubItems[2].Text + "'";
                    cmd = new SqlCommand(sql, conn);
                    ...
                    {
                        sql = "select COUNT(*) from RFID_TB where RFIDNO='" + lvRealList.Items[i].SubItems[2].Text + "' and RFID_TB.PROGRAM_ID='DCWH_STAR'";
                        cmd = new SqlCommand(sql, conn);
                        int r = Convert.ToInt32(cmd.ExecuteScalar());
                        if (r > 0)
                        {
                            sql = "DELETE FROM RFID_TB Where RFIDNO = '" + lvRealList.Items[i].SubItems[2].Text + "'";
                            try
                            {
                                cmd = new SqlCommand(sql, conn);
                                cmd.ExecuteNonQuery();
                ...
                for (int i = 0; i <= lvRealList.Items.Count - 1; i++)
                {
                    ...
                    else if (IO == "RECEIVE" | IO == "")
                    {
                        ...
                        else
                        {
                            ...
                            string dep;
                            if (comboBox1.Text == "DC_REF_7D")
                            {
                                dep = "";
                            }
                            else if (comboBox1.Text == "DC_REF_30D")
                            {
                                dep = "DC_REF";
                            }
                            else { dep = comboBox1.Text; }
                            if (IO == "RECEIVE")
                            {
                                conn.Open();
                                sql = "Insert into RFID_TRACK (TESTNO,RFIDNO,USERID,USERDATE,IO,DEPARTMENT,MEMO,ARTICLE,WorkID,WHNO,SNO,PROGRAM_ID) " +
                                      "Values ('" + textBox1.Text + "', '" + lvRealList.Items[i].SubItems[2].Text + "', '"
                                                  + lvRealList.Items[i].SubItems[4].Text + "', '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")
                                                  + "', 'MOVE OUT', '" + dep + "', '" + memo + "', '" + label34.Text + "', '"
                                                  + textBox4.Text + "', '" + comboBox2.Text + "', '" + textBox6.Text + "', 'DCWH_STAR')";
                            ...
                            else
                            {
                                ...
                                try
                                {
                                    sql = "Insert into RFID_TRACK (TESTNO,RFIDNO,USERID,USERDATE,IO,DEPARTMENT,ARTICLE,WHNO,SNO,PROGRAM_ID) " +
                                          "Values ('" + textBox1.Text + "', '" + lvRealList.Items[i].SubItems[2].Text + "', '"
                                                      + lvRealList.Items[i].SubItems[4].Text + "', '"
                                                      + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "', 'RECEIVE', 'DC WH_STAR', '"
                                                      + label34.Text + "', 'ROOM TEMP', '1', 'DCWH_STAR')";
                                    cmd = new SqlCommand(sql, conn);
                                    cmd.ExecuteNonQuery();

                                    sql = "Insert into RFID_TRACK (TESTNO,RFIDNO,USERID,USERDATE,IO,DEPARTMENT,MEMO,ARTICLE,WorkID,WHNO,SNO,PROGRAM_ID) " +
                                          "Values ('" + textBox1.Text + "', '" + lvRealList.Items[i].SubItems[2].Text + "', '"
                                                      + lvRealList.Items[i].SubItems[4].Text + "', '" + DateTime.Now.AddSeconds(3).ToString("yyyy-MM-dd HH:mm:ss")
                                                      + "', 'MOVE OUT', '" + dep + "', '" + memo + "', '" + label34.Text
                                                      + "', '" + textBox4.Text + "', 'ROOM TEMP', '1', 'DCWH_STAR')";
                                    cmd = new SqlCommand(sql, conn);
                                    cmd.ExecuteNonQuery();
                                }
                                catch (Exception ex)
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            if (btRealTimeInventory.Text == "Start Read")
            {
                ...
                for (int i = 0; i <= lvRealList.Items.Count - 1; i++)
                {
                    ...
                    if (IO == "RECEIVE")
                    {
                        ...
                        if (c > 0)
                        {
                            sql = "select COUNT(*) from RFID_TB " +
                                  "where RFIDNO='" + lvRealList.Items[i].SubItems[2].Text + "' AND WHNO is not null AND SNO is not null";
                            cmd = new SqlCommand(sql, conn);
                            int t = Convert.ToInt32(cmd.ExecuteScalar());
                            if (t > 0)
                            {
                    ...
                    if (DEPARTMENT == "" && IO == "" && comboBox1.Text != "DC_REF_7D")
                    {
                        lrtxtLog.AppendText(lvRealList.Items[i].SubItems[2].Text + "\n");
                    }
                    else if (IO == "MOVE OUT" | temp == 1 | (DEPARTMENT == "" && IO == "" && comboBox1.Text == "DC_REF_7D"))
                    {
                        sql = "select TESTNO,ARTICLE from RFID_TB where RFIDNO='" + lvRealList.Items[i].SubItems[2].Text + "'";
                        cmd = new SqlCommand(sql, conn);
                        SqlDataReader dataReader1 = cmd.ExecuteReader();
                        ...
                        else
                        {
                            ...
                            conn.Close();
                            conn.Open();
                            if (comboBox1.Text == "DC_REF_7D" | comboBox1.Text == "DC_REF_30D")
                            {
                                sql = "Insert into RFID_TRACK (TESTNO,RFIDNO,USERID,USERDATE,IO,DEPARTMENT,ARTICLE,WHNO,SNO,PROGRAM_ID) " +
                                          "Values ('" + textBox1.Text + "', '" + lvRealList.Items[i].SubItems[2].Text + "', '"
                                                    + lvRealList.Items[i].SubItems[4].Text + "', '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")
                                                    + "', 'RECEIVE', 'DC WH_STAR', '" + label34.Text + "', '" + comboBox2.Text + "', '"
                                                    + textBox6.Text + "', 'DCWH_STAR')";
                                ...
                                //if (Int32.Parse(label29.Text.ToString()) < 15)
                                //{

                                //}
                                //else
                                //{
                                //    lrtxtLog.AppendText("Exceed the amount" + "\n");
                                //}
                            }
                            else
                            {
                                lrtxtLog.AppendText("Don't receive DC WH_QC" + "\n");
                            }
                    ...
                    case 0x8B:
                        {
                            ...
                            if (nEpcCount < nEpcLength)
                            {
                                ...
                                if (m_temp == 1)
                                {
                                    ...
                                    string sql1 = "select WHNO,SNO from RFID_TB t1 " +
                                ...
                                else if (m_temp == 2)
                                {
                                    ...
                                    cmd.Parameters.Add("@Stage", SqlDbType.VarChar).Value = "";
                                    ...
                                    while (dataReader.Read())
                                    {
                                        ...
                                        item.SubItems.Add(dataReader["SIZ"].ToString());
                                        ...
                                        if ((dataReader["DEPARTMENT"].ToString().Equals("DC_REF") && Int32.Parse(dataReader["day"].ToString()) == 30)
                                           || (dataReader["DEPARTMENT"].ToString().Equals("") && Int32.Parse(dataReader["day"].ToString()) == 7)
                                           || (Int32.Parse(dataReader["day"].ToString()) == -1))
                                        {
                                    ...
                                    lvdel.Items[nEpcCount].EnsureVisible();
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            string sql = "select Article, YPDH from LIY_ERP.dbo.shoetest where testno='" + textBox1.Text + "' ";
            SqlCommand cmd = new SqlCommand(sql, conn);
            SqlDataReader dataReader = cmd.ExecuteReader();
            ...
            int qty = 0;
            int mqty = 0;
            sql = "select purpose.YWSM + ' quantity: ' YWSM,CAST(ISNULL(shoetest2.Qty*2,0) as int) Qty, RFID_TB.QtyRe " +
                  "from (SELECT TESTNO,sum(QtyRe) QtyRe from " +
                  "(select RFID_TB.TestNo, isnull(count(RFID_TB.TestNo),0) QtyRe from RFID_TB " +
                  " WHERE PROGRAM_ID='DCWH_STAR' group by RFID_TB.TESTNO " +
                  " UNION all " +
                  " select RFID_DEL.TestNo, isnull(count(RFID_DEL.TestNo),0) QtyRe FROM RFID_DEL " +
                  " WHERE PROGRAM_ID='DCWH_STAR' AND RFID_DEL.MEMO IN ('Destroy','Ship for customers','Ship out for customer','Ship out for supervisor','Ship out for Mass pro','Ship out for Tech') " +
                  " group by RFID_DEL.TESTNO) RFID group by TESTNO) RFID_TB " +
                  "left join LIY_ERP.dbo.shoetest2 shoetest2 on shoetest2.TESTNO = RFID_TB.TestNo " +
                  "left join LIY_ERP.dbo.shoetest shoetest on shoetest.TESTNO = shoetest2.TestNo " +
                  "left join LIY_ERP.dbo.shoetestDetail purpose on purpose.YYBH=shoetest2.DepName " +
                  "where RFID_TB.testno='" + textBox1.Text + "' and DepName like 'MD%' AND parts IN ('BW01','BW10') " +
                  "      AND ((shoetest2.DepName NOT IN ('MD01','MD03','MD04','MD08','MD18') and shoetest.Complete>='2024/07/01') " +
                  "      or (shoetest2.DepName NOT IN ('MD01','MD03','MD04','MD07','MD08','MD18') and shoetest.Complete between '2024/01/01' and '2024/06/30')) " +
                  "group by shoetest2.Qty, purpose.YWSM, RFID_TB.QtyRe ";
            SqlCommand cmd1 = new SqlCommand(sql, conn);
            SqlDataReader dataReader1 = cmd1.ExecuteReader();
            lrtxtLog.AppendText(" \n");
            while ((dataReader1.Read()))
            {
                if (!dataReader1[0].ToString().Equals("Shipping quantity: "))
                {
                    lrtxtLog.AppendText(dataReader1[0].ToString() + dataReader1[1].ToString() + " \n");
                    qty += int.Parse(dataReader1[1].ToString());
                }
                qtyre = int.Parse(dataReader1[2].ToString());
            }
            mqty = qtyre - qty;
            lrtxtLog.AppendText("Reality scaning quantity : " + qtyre + " \n");
            if (mqty <= 0)
            {
                lrtxtLog.AppendText("Missing quantity : " + mqty + " \n");
            }
            else lrtxtLog.AppendText("Missing quantity : +" + mqty + " \n");
            dataReader1.Close();
            conn.Close();
        }
        ...
        private void textBox4_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (e.KeyChar == (char)Keys.Enter) //判斷是否輸入 enter
            {
                ...
                string sql = "select USERNAME from LIY_ERP.dbo.Busers where USERID='" + textBox4.Text + "'";
                SqlCommand cmd = new SqlCommand(sql, conn);
                SqlDataReader dataReader1 = cmd.ExecuteReader();
        ...
        private void lvdel_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            System.Windows.Forms.MenuItem mItem3 = new System.Windows.Forms.MenuItem();
            mItem3.Text = "Load not receive";
            mItem3.Click += new System.EventHandler(this.menuItem33_Click);
        ...
        private void menuItem11_Click(object sender, EventArgs e)
        {
            ...
            SqlConnection conn = new SqlConnection(str);
            conn.Open();
            string sql = "select IO, DEPARTMENT, RFID_TRACK.MEMO, RFID_TRACK.USERID, convert(varchar, RFID_TRACK.USERDATE, 103) as USERDATE," +
                         "       WorkID, case when WorkID is null then t2.username else t1.username end as USERNAME, WHNO, SNO " +
                         " from RFID_TRACK " +
                         " left join LIY_ERP.dbo.Busers t1 on t1.userid=RFID_TRACK.workID " +
                         " left join LIY_ERP.dbo.Busers t2 on t2.userid=RFID_TRACK.USERID " +
                         " where TESTNO = '" + lvdel.SelectedItems[0].SubItems[1].Text + "' AND RFIDNO = '" + lvdel.SelectedItems[0].SubItems[2].Text +
                         "'  and RFID_TRACK.PROGRAM_ID='DCWH_STAR' and RFID_TRACK.USERDATE BETWEEN '" + datestart + "' AND '" + dateend + "'";
            SqlCommand cmd = new SqlCommand(sql, conn);
            SqlDataReader dataReader = cmd.ExecuteReader();
            ...
            while (dataReader.Read())
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            item.SubItems.Add("Move out for " + dataReader["DEPARTMENT"].ToString() + "_" + dataReader["MEMO"].ToString()
                                              + ": " + dataReader["WorkID"].ToString() + ", " + dataReader["USERNAME"].ToString());
                        }
                        else if (dataReader["DEPARTMENT"].ToString().Equals("DC_REF"))
                        {
                            item.SubItems.Add("DC_REF_30D: " + dataReader["WorkID"].ToString() + ", " + dataReader["USERNAME"].ToString());
                        }
                        else
                        {
                            item.SubItems.Add("DC_REF_7D: " + dataReader["WorkID"].ToString() + ", " + dataReader["USERNAME"].ToString());
                        }
                        item.SubItems.Add("(Date Move out) " + dataReader["USERDATE"].ToString());
                        item.SubItems.Add(dataReader["WHNO"].ToString());
                        item.SubItems.Add(dataReader["SNO"].ToString() + " (Not in warehouse)");
                    }
                    item.SubItems.Add(textBox8.Text);
                    item.SubItems.Add(" " + DateTime.Now.ToString("dd/MM/yyyy"));
                }
        ...
        private void menuItem22_Click(object sender, EventArgs e)
        {
            ...
            {
                int promptValue = ShowDialog(comboBox4, lvdel.SelectedItems[0].SubItems[10].Text, lvdel.SelectedItems[0].SubItems[11].Text, lvdel.SelectedItems[0].SubItems[1].Text, lvdel.SelectedItems[0].SubItems[2].Text, textBox8.Text, lvdel.SelectedItems[0].SubItems[4].Text);
                if (promptValue == 1)
                {
        ...
        private void menuItem33_Click(object sender, EventArgs e)
        {
            ...
            if (btRealTimeInventory2.Text == "Start Read")
            {
                try
                {
                    ...
                    if (!textBox13.Text.Replace(" ", "").Equals(""))
                    {
                        cmd.Parameters.Add("@RFIDNO", SqlDbType.VarChar).Value = textBox13.Text;
                    }
                    else
                    {
                    ...
                    cmd.Parameters.Add("@WHNO", SqlDbType.VarChar).Value = "Not In Warehouse";
                    cmd.Parameters.Add("@SNO", SqlDbType.VarChar).Value = textBox9.Text;
                    cmd.Parameters.Add("@Stage", SqlDbType.VarChar).Value = textBox14.Text;
                    cmd.Parameters.Add("@id", SqlDbType.VarChar).Value = textBox15.Text;
        ...
        public static int ShowDialog(ComboBox cbt, string wh, string s, string tesno, string rfid, string id, string sr)
        {
            ...
            btnsave.SetBounds(110, 90, 70, 35);
            btnsave.Text = "Save";
            btnsave.Click += (sender, e) =>
            {
                SqlConnection conn;
                SqlCommand cmd;
                string str = "server=192.168.23.9;database=LIY_TYXUAN;uid=tyxuan;pwd=Itkhua@!admin";
                try
                {
                    string ip = "", name = "", user = "", pass = "";
                ...
                else
                {
                    ...
                    sql = "Insert into RFID_TRACK (TESTNO,RFIDNO,USERID,USERDATE,IO,DEPARTMENT,ARTICLE,WorkID,WHNO,SNO,PROGRAM_ID) " +
                          "Values ('" + tesno + "', '" + rfid + "', '" + id + "', '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")
                                      + "', 'MOVE OUT', '', '" + sr + "', '" + id + "', '" + wh + "', '" + s + "', 'DCWH_STAR')";
                    cmd = new SqlCommand(sql, conn);
                    cmd.ExecuteNonQuery();

                    sql = "Insert into RFID_TRACK (TESTNO,RFIDNO,USERID,USERDATE,IO,DEPARTMENT,ARTICLE,WHNO,SNO,PROGRAM_ID) " +
                          "Values ('" + tesno + "', '" + rfid + "', '" + id + "', '" + DateTime.Now.AddSeconds(1).ToString("yyyy-MM-dd HH:mm:ss")
                                      + "', 'RECEIVE', '" + DEPARTMENT + "', '" + sr + "', '" + cb.Text + "', '" + tb.Text + "', 'DCWH_STAR')";
                    cmd = new SqlCommand(sql, conn);
                    cmd.ExecuteNonQuery();

                    sql = "Update RFID_TB set WHNO = '" + cb.Text + "', SNO = '" + tb.Text +
            ...
            prompt.Controls.AddRange(new Control[] { label13, label1, cb, tb, btnsave });
            prompt.ShowDialog();
            return (int)temp;
        }
                    ...
                    if (confirmation == DialogResult.Yes)
                    {
                        ...
                        else
                        {
                            ...
                            if (lrtxtLog.Text == "")
                            {
                                foreach (ListViewItem lv in lvdel.CheckedItems)
                                {
                                    sql = "Insert into RFID_DEL (TESTNO,RFIDNO,DELUSER,DELDATE,PROGRAM_ID,MEMO,OUTDATE) " +
                                          "Values ('" + lv.SubItems[1].Text + "', '" + lv.SubItems[2].Text + "', '" + textBox8.Text + "', '"
                                                      + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "', 'DCWH_STAR', 'Destroy', '"+ DateOut.Value.ToString("yyyy-MM-dd") + "')";
                                    try
                                    {
                                        cmd = new SqlCommand(sql, conn);
                                        cmd.ExecuteNonQuery();
                        ...
                        else
                        {
                            ...
                            if (lrtxtLog.Text == "")
                            {
                                foreach (ListViewItem lv in lvdel.CheckedItems)
                                {
                                    sql = "Insert into RFID_DEL (TESTNO,RFIDNO,DELUSER,DELDATE,PROGRAM_ID,MEMO,OUTDATE) " +
                                          "Values ('" + lv.SubItems[1].Text + "', '" + lv.SubItems[2].Text + "', '" + textBox8.Text + "', '"
                                                      + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "', 'DCWH_STAR', 'Ship out for customer', '" + DateOut.Value.ToString("yyyy-MM-dd") + "')";
                                    try
                                    {
                                        cmd = new SqlCommand(sql, conn);
                                        cmd.ExecuteNonQuery();
                ...
                if (lvdel.CheckedItems.Count > 0)
                {
                    ...
                    foreach (ListViewItem lv in lvdel.CheckedItems)
                    {
                        ...
                        else
                        {
                            ...
                            conn = new SqlConnection(str);
                            conn.Open();
                            string a = lv.SubItems[12].Text;
                            a = a.Replace(" ", "");
                            string[] userid = a.Split(new char[] { ':' });
                            string sql;
                            sql = "select USERNAME from LIY_ERP.dbo.Busers Busers where USERID='" + userid[1] + "'";
                            SqlCommand cmd = new SqlCommand(sql, conn);
                            String r1 = Convert.ToString(cmd.ExecuteScalar());
                            if (userid[0].Equals("QC"))
                            {
                                item.SubItems.Add("Move out for QC" + "_" + userid[2] + ": " + userid[1] + ", " + r1);
                            }
                            else if (userid[0].Equals("DC_REF_30D"))
                            {
                                item.SubItems.Add("DC_REF_30D: " + userid[1] + ", " + r1);
                            }
                            else
                            {
                                item.SubItems.Add("DC_REF_7D: " + userid[1] + ", " + r1);
                            }
                            conn.Close();
                        }
                        item.SubItems.Add(" " + lv.SubItems[11].Text);
                        item.SubItems.Add(lv.SubItems[13].Text);
                        item.SubItems.Add(textBox8.Text);
                        item.SubItems.Add(" " + DateTime.Now.ToString("dd/MM/yyyy"));
                    ...
                    if (confirmation == DialogResult.Yes)
                    {
                        ...
                        foreach (ListViewItem lv in lvdel.CheckedItems)
                        {
                            ...
                            if (c > 0)
                            {
                                sql = "select COUNT(*) from RFID_TB " +
                                      "where RFIDNO='" + lv.SubItems[2].Text + "' AND WHNO is not null AND SNO is not null " +
                                      "  and RFID_TB.PROGRAM_ID='DCWH_STAR' " +
                                      "  AND EXISTS (select WHNO,SNO from RFID_WH where RFID_TB.WHNO=RFID_WH.WHNO and SNO='#####')";
                                cmd = new SqlCommand(sql, conn);
                                int r = Convert.ToInt32(cmd.ExecuteScalar());
                                if (r == 0)
                        ...
                        else
                        {
                            foreach (ListViewItem lv in lvdel.CheckedItems)
                            {
                                sql = "Insert into RFID_DEL (TESTNO,RFIDNO,DELUSER,DELDATE,PROGRAM_ID,MEMO) " +
                                      "Values ('" + lv.SubItems[1].Text + "', '" + lv.SubItems[2].Text + "', '" + textBox8.Text + "', '"
                                                  + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "', 'DCWH_STAR', 'Wrong')";
                                try
                                {
                                    cmd = new SqlCommand(sql, conn);
                                    cmd.ExecuteNonQuery();
        ...
        {
            if (comboBox1.SelectedItem.ToString().Equals("QC"))
            {
        ...
        private void textBox6_TextChanged(object sender, EventArgs e)
        {
            ...
            SqlConnection conn = new SqlConnection(str);
            conn.Open();
            string sql = "select COUNT(*) from RFID_TB where WHNO='" + comboBox2.Text + "' and SNO='" + textBox6.Text + "'";
            SqlCommand cmd = new SqlCommand(sql, conn);
            int r = Convert.ToInt32(cmd.ExecuteScalar());
            if (r >= 15) { label29.ForeColor = System.Drawing.Color.Red; }
            else label29.ForeColor = System.Drawing.Color.Green;
            label29.Text = r + "";
            conn.Close();
        }
        private void button10_Click(object sender, EventArgs e)
        {
                ...
                for (int i = 0; i <= lvRealList.Items.Count - 1; i++)
                {
                    ...
                    else if (IO == "MOVE OUT" | temp == 1)
                    {
                        ...
                        else
                        {
                            ...
                            if (comboBox1.Text == "DC_REF_7D" | comboBox1.Text == "DC_REF_30D")
                            {
                                sql = "select top 1 WHNO,SNO from RFID_TRACK " +
                                      "where RFIDNO='" + lvRealList.Items[i].SubItems[2].Text + "' and IO='MOVE OUT' " +
                                      "      and TESTNO='" + textBox1.Text + "' and PROGRAM_ID='DCWH_STAR' " +
                                      "order by USERDATE desc";
                                cmd = new SqlCommand(sql, conn);
                                SqlDataReader dataReader2 = cmd.ExecuteReader();
                                while ((dataReader2.Read()))
                                {
                                    comboBox2.Text = dataReader2[0].ToString();
                                    textBox6.Text = dataReader2[1].ToString();
                                }
                                dataReader2.Close();
                            }
                            else
            ...
            if (lrtxtLog.Text != "")
            {
                lrtxtLog.AppendText("Please Check the above RFIDNO ，cant find Old_WareHouse !");
            }
            btRealTimeInventory.Enabled = true;
            button2.Enabled = true;
        }
        ...
        private void button14_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand()
                {
                    CommandText = "LoadListRFIDOUT",
                    Connection = conn,
                    CommandType = CommandType.StoredProcedure
                };
                cmd.Parameters.Add("@TESTNO", SqlDbType.VarChar).Value = textBox19.Text;
                cmd.Parameters.Add("@ARTICLE", SqlDbType.VarChar).Value = textBox21.Text;
                cmd.Parameters.Add("@MEMO", SqlDbType.VarChar).Value = comboBox7.SelectedItem.ToString();
                cmd.Parameters.Add("@dates", SqlDbType.VarChar).Value = DP1.Value.ToString("yyyy/MM/dd");
                cmd.Parameters.Add("@datee", SqlDbType.VarChar).Value = DP2.Value.ToString("yyyy/MM/dd");
                SqlDataReader dataReader = cmd.ExecuteReader();
                while ((dataReader.Read()))
                {
                    ListViewItem item = new ListViewItem(dataReader["TESTNO"].ToString());
                    item.SubItems.Add(dataReader["RFIDNO"].ToString());
                    item.SubItems.Add(dataReader["YPDH"].ToString());
                    item.SubItems.Add(dataReader["Article"].ToString());
                    item.SubItems.Add(dataReader["Season"].ToString());
                    ...
                    item.SubItems.Add(dataReader["FD"].ToString());
                    item.SubItems.Add(dataReader["SIZ"].ToString());
                    item.SubItems.Add(dataReader["DELDATE"].ToString());
                ...
                ListViewItem[] arr = items.ToArray();
                lvOut.BeginUpdate();
                lvOut.Items.AddRange(arr);
                lvOut.EndUpdate();
                ledReal3.Text = lvOut.Items.Count.ToString();
            }
            catch (Exception ex)
            {
        ...
        private void button16_Click(object sender, EventArgs e)
        {
            ...
            if (btRealTimeInventory.Text == "Start Read")
            {
                ...
                if (confirmation == DialogResult.Yes)
                {
                    ...
                    for (int i = 0; i <= lvRealList.Items.Count - 1; i++)
                    {
                        ...
                        {
                            sql = "select Testno from RFID_TB where RFIDNO='" + lvRealList.Items[i].SubItems[2].Text + "'";
                            cmd = new SqlCommand(sql, conn);
                            string testno = Convert.ToString(cmd.ExecuteScalar());

                            sql = "Insert into RFID_DEL (TESTNO,RFIDNO,DELUSER,DELDATE,PROGRAM_ID,MEMO) " +
                                          "Values ('" + testno + "', '" + lvRealList.Items[i].SubItems[2].Text + "', '" + lvRealList.Items[i].SubItems[4].Text + "', '"
                                                      + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "', 'DCWH_STAR', '" + comboBox8.Text + "')";
                            try
                            {
                                cmd = new SqlCommand(sql, conn);
                                cmd.ExecuteNonQuery();
            ...
            if (lrtxtLog.Text != "")
            {
                lrtxtLog.AppendText("The above RFIDNO cannot be " + comboBox8.Text + " ，because there are have not in Warehouse !");
            }
            else
            {
                lrtxtLog.AppendText(comboBox8.Text + " Completed !");
            }
            btRealTimeInventory.Enabled = true;
            button2.Enabled = true;
        }
        ...
        {
            if (R2000UartDemo.ActiveForm != null)
            {
                lvdel.Width = R2000UartDemo.ActiveForm.Width - 30;
                ckClearOperationRec.Left = label6.Width + 2;
                ckClearOperationRec.Top = label6.Top - 2;
                tabCtrMain.Height = label6.Top - 4;
                lvdel.Height = pagList.Height - (groupBox6.Height + 12);
                lvRealList.Height = pageEpcTest.Height - (lbRealTagCount.Top + 28);
            }
        }
        ...
        private void textBox12_TextChanged(object sender, EventArgs e)
        {
            ...
            string sql = "select SUM(CAST(ISNULL(shoetest2.Qty*2,0) as int)) Qty from LIY_ERP.dbo.shoetest2 " +
                         " left join LIY_ERP.dbo.shoetest shoetest on shoetest.TestNo = shoetest2.TestNo " +
                         " where shoetest2.testno='" + textBox12.Text + "' " +
                         "       and DepName like 'MD%'  AND parts IN ('BW01','BW10') AND ((shoetest2.DepName NOT IN ('MD01','MD03','MD04','MD08','MD18') and shoetest.Complete>='2024/07/01') " +
                         "       or (shoetest2.DepName NOT IN ('MD01','MD03','MD04','MD07','MD08','MD18') and shoetest.Complete between '2024/01/01' and '2024/06/30')) ";
            SqlCommand cmd = new SqlCommand(sql, conn);
            SqlDataReader dataReader = cmd.ExecuteReader();
