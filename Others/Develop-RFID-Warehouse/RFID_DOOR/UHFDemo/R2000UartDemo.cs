...
namespace UHFDemo
{
    ...
    {
        [DllImport("EastRiver.dll", EntryPoint = "OpenClientSocket")]
        public static extern IntPtr OpenClientSocket(string RemoteAddr, int Port);

        [DllImport("EastRiver.dll", EntryPoint = "CallClock")]
        public static extern bool CallClock(IntPtr hPort, int clockId);

        [DllImport("EastRiver.dll", EntryPoint = "SetDoorAction")]
        public static extern int SetDoorAction(IntPtr hPort, int DoorNo, int Action);
        ...
        //盤存操作前，需要先設置工作天線，用於標識當前是否在執行盤存操作
        private bool m_bInventory = false;
        //標識是否統計命令執行時間，當前僅盤存命令需要統計時間
        ...
        private IntPtr Port;
        string Siren_IP;
        int Siren_Port, Siren_Time;
        System.Data.DataTable BlackList = new System.Data.DataTable();
        ...
        private void R2000UartDemo_Load(object sender, EventArgs e)
        {
            ...
            {
                WriteLog(lrtxtLog, "Error server!!!", 1);
                WriteLog(lrtxtLog, ex.Message, 1);
            }

            //讀取名稱 By Danny 2020/06/12
            try
            {
                using (StreamReader sReader = new StreamReader(@"Name.txt", Encoding.UTF8))
                {
                    Text = sReader.ReadLine(); //載入名稱
                }
            }
            catch (Exception ex)
            {
                WriteLog(lrtxtLog, "無法取得程式名稱。", 1);
                WriteLog(lrtxtLog, ex.Message, 1);
            }
            ...
            //初始化連接天線預設配置 By Danny 2020/06/12
            cmbComPort.SelectedIndex = 0;
            cmbBaudrate.SelectedIndex = 1;
            try
            {
                using (StreamReader sReader = new StreamReader(@"RFID_Controller_IP.txt", Encoding.UTF8))
                {
                    ipIpServer.IpAddressStr = sReader.ReadLine(); //預設192.168.0.178
                    txtTcpPort.Text = sReader.ReadLine(); //預設4001
                }
            }
            catch (Exception ex)
            {
                WriteLog(lrtxtLog, "無法取得RFID控制器IP設定。", 1);
                WriteLog(lrtxtLog, ex.Message, 1);
            }

            //取得天線偵測範圍設定值 By Danny 2020/06/12
            try
            {
                reader.GetOutputPower(m_curSetting.btReadId);
                using (StreamReader sReader = new StreamReader(@"RFID_Controller_Range.txt", Encoding.UTF8))
                { 
                    string Range = sReader.ReadLine();
                    if (Range != txtOutputPower.Text)
                    {
                        txtOutputPower.Text = Range;
                        btnSetOutputPower.PerformClick();
                    }
                }
            }
            catch (Exception ex)
            {
                WriteLog(lrtxtLog, "無法設定天線偵測範圍。", 1);
                WriteLog(lrtxtLog, ex.Message, 1);
            }

            //初始化連接警報器 By Danny 2020/06/12
            try
            {
                using (StreamReader sReader = new StreamReader(@"Siren_IP.txt", Encoding.UTF8))
                {
                    Siren_IP = sReader.ReadLine();
                    Siren_Port = int.Parse(sReader.ReadLine());
                    Port = OpenClientSocket(Siren_IP, Siren_Port);
                    if (CallClock(Port, 1) == false)
                    {
                        WriteLog(lrtxtLog, "無法連接警報器(IP: " + Siren_IP + ":" + Siren_Port.ToString() + ")。", 1);
                    }
                }
            }
            catch (Exception ex)
            ...
            //取得警報器作響時間設定值 By Danny 2020/06/12
            try
            {
                using (StreamReader sReader = new StreamReader(@"Siren_Time.txt", Encoding.UTF8))
                {
                    Siren_Time = int.Parse(sReader.ReadLine()) * 1000;
                }
            }
            catch (Exception ex)
            {
                Siren_Time = 20000;
                WriteLog(lrtxtLog, "無法設定警報器作響時間。", 1);
                WriteLog(lrtxtLog, ex.Message, 1);
            }

            //取得資料庫黑名單 By Danny 2020/06/12
            try
            {
                string SQL = "SELECT RFID FROM Data_RFIDCard";

                SqlConnection connection = new SqlConnection(str);
                SqlCommand command = new SqlCommand(SQL, connection);
                connection.Open();
                SqlDataReader DataReader = command.ExecuteReader();
                BlackList.Load(DataReader);
            }
            catch (Exception ex)
            {
                WriteLog(lrtxtLog, "無法取得黑名單資訊。", 1);
                WriteLog(lrtxtLog, ex.Message, 1);
            }
                    ...
                    case 0x91:
                        {
                            ...
                            labelBufferTagCount.Text = "標籤列表： " + m_curInventoryBuffer.nTagCount.ToString() + "個";
                    ...
                    case 0x8B:
                        {
                            ...
                            if (m_curInventoryBuffer.nReadRate == 0) //讀寫器沒有返回速度前軟件測速度
                            {
                                if (nTotalTime > 0)
                                {
                            ...
                            ledReal4.Text = nCommandDuation.ToString();  //實際的命令執行時間
                            ...
                            lbRealTagCount.Text = "標籤EPC號列表（不重複）： " + nTagCount.ToString() + "個";
                            ...
                            if (nEpcCount < nEpcLength)
                            {
                                ...
                                string sql1 = "select Testno from RFID_TB where RFIDNO='" + row[2].ToString() + "'";
                                SqlCommand cmd1 = new SqlCommand(sql1, conn);
                                r2 = Convert.ToString(cmd1.ExecuteScalar());
                                if (r2 == "")
                                {
                                    ListViewItem item = new ListViewItem();
                                    item.Text = (nEpcCount + 1).ToString();
                                    item.SubItems.Add("Thẻ trống");
                                    item.SubItems.Add(row[2].ToString());
                                    item.SubItems.Add(row[0].ToString());
                                    item.SubItems.Add(row[5].ToString());
                                    item.SubItems.Add((Convert.ToInt32(row[4]) - 129).ToString() + "dBm");
                                    item.SubItems.Add(row[6].ToString());
                                    if (row[2].ToString().Substring(0, 2).Equals(" E") && row[2].ToString().Length >= 36)
                                    {
                                ...
                                else
                                {
                                    ListViewItem item = new ListViewItem();
                                    item.Text = (nEpcCount + 1).ToString();
                                    item.SubItems.Add(r2);
                                    item.SubItems.Add(row[2].ToString());
                                    item.SubItems.Add(row[0].ToString());
                                    item.SubItems.Add(row[5].ToString());
                                    item.SubItems.Add((Convert.ToInt32(row[4]) - 129).ToString() + "dBm");
                                    item.SubItems.Add(row[6].ToString());
                                    string sql = "select top 1 IO from RFID_TRACK where RFIDNO='" + row[2].ToString() + "' order by UserDate Desc";
                    ...
                    case 0x00:
                        {
                            ...
                            ledFast1.Text = nTagCount.ToString(); //標籤總數
                            if (m_curInventoryBuffer.nCommandDuration > 0)
                            {
                                ledFast2.Text = (m_curInventoryBuffer.nDataCount * 1000 / m_curInventoryBuffer.nCommandDuration).ToString(); //讀標籤速度
                            }
                            else
                            {
                                ledFast2.Text = "";
                            ...
                            ledFast3.Text = m_curInventoryBuffer.nCommandDuration.ToString(); //命令執行時間

                            ledFast5.Text = nTotalTime.ToString(); //命令累計執行時間
                            ...
                            txtFastTagList.Text = "標籤EPC號列表（不重複）： " + nTagCount.ToString() + "個";
                ...
                if (m_curInventoryBuffer.nIndexAntenna < m_curInventoryBuffer.lAntenna.Count - 1 || m_curInventoryBuffer.nCommond == 0)
                {
                    if (m_curInventoryBuffer.nCommond == 0)
                    {
                        ...
                        if (m_curInventoryBuffer.bLoopInventoryReal)
                        {
                            ...
                            if (m_curInventoryBuffer.bLoopCustomizedSession)//自定義Session和Inventoried Flag 
                            {
                    ...
                    case 0xb3:
                        {
                            ...
                            //{
                            //    case 0x00:
                            //        MessageBox.Show("該字節成功鎖定");
                            //        break;
                            //    case 0xFE:
                            //        MessageBox.Show("該字節已是鎖定狀態");
                            //        break;
                            //    case 0xFF:
                            //        MessageBox.Show("該字節無法鎖定");
                            //        break;
                            //    default:
                            //        break;
                            //}
                    ...
                    case 0xb4:
                        {
                            ...
                            {
                                case 0x00:
                                    txtStatus.Text = "該字節未鎖定";
                                    break;
                                case 0xFE:
                                    txtStatus.Text = "該字節已處於鎖定狀態";
                                    break;
                                default:
                                    break;
                            }
        ...
        private void SetRadioButtonBold(System.Windows.Forms.CheckBox ckBold)
        {
            System.Drawing.Font oldFont = ckBold.Font;
            System.Drawing.Font newFont = new System.Drawing.Font(oldFont, oldFont.Style ^ FontStyle.Bold);
            ckBold.Font = newFont;
        }
        ...
        private void btnConnectRs232_Click(object sender, EventArgs e)
        {
            ...
            if (nRet != 0)
            {
                string strLog = "讀寫器連接失敗，失敗原因： " + strException;
            ...
            {
                string strLog = "連接讀寫器 " + strComPort + ":" + nBaudrate.ToString();
        ...
        private void btnConnectTcp_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string strLog = "連接讀寫器 " + ipIpServer.IpAddressStr + ":" + nPort.ToString();
                    PagISO18000.Parent = null;
                    PagTranDataLog.Parent = null;
                    pageBufferedMode.Parent = null;
                    pageFast8AntMode.Parent = null;
                    pageAcessTag.Parent = null;
                    tabCtrMain.SelectTab(1);
                    try
                    {
                        using (StreamReader sReader = new StreamReader(@"Antenna_Setting.txt", Encoding.UTF8))
                        {
                            for (int i = 1; i <= 8; i++)
                            {
                                string Data = sReader.ReadLine();
                                bool switcher = Data.Split(':')[1] == "1" ? true : false;
                                var CBox = (System.Windows.Forms.CheckBox)Controls.Find("cbRealWorkant" + i.ToString(), true)[0];
                                CBox.Checked = switcher;
                            }
                        }
                        Connection = true;
                    }
                    ...
                    {
                        WriteLog(lrtxtLog, "無法取得RFID天線設定。", 1);
                        WriteLog(lrtxtLog, ex.Message, 1);
                        return;
                    }
                    WriteLog(lrtxtLog, strLog, 0);
        ...
        private void ProcessSetReadAddress(Reader.MessageTran msgTran)
        {
            ...
            string strLog = strCmd + "失敗，失敗原因： " + strErrorCode;
            WriteLog(lrtxtLog, strLog, 1);
        }
        ...
        {
            string strCmd = "取得工作天線";
            string strErrorCode = string.Empty;

            if (msgTran.AryData.Length == 1)
            {
                if (msgTran.AryData[0] == 0x00 || msgTran.AryData[0] == 0x01 || msgTran.AryData[0] == 0x02 || msgTran.AryData[0] == 0x03 || msgTran.AryData[0] == 0x04 || msgTran.AryData[0] == 0x05 || msgTran.AryData[0] == 0x06 || msgTran.AryData[0] == 0x07)
                {
                    m_curSetting.btReadId = msgTran.ReadId;
        ...
        private void ProcessSetWorkAntenna(Reader.MessageTran msgTran)
        {
            ...
            string strCmd = "設置工作天線成功,當前工作天線: 天線" + intCurrentAnt.ToString();
        ...
        private void btnSetFrequencyRegion_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        MessageBox.Show("頻譜範圍不符合規範，請參考通訊協議");
                        return;
                    }
        ...
        {
            string strCmd = "讀取Impinj Monza快速讀TID功能";
            string strErrorCode = string.Empty;
        ...
        {
            string strCmd = "設置Impinj Monza快速讀TID功能";
            string strErrorCode = string.Empty;
        ...
        private void btnInventory_Click(object sender, EventArgs e)
        {
            ...
            {                
                ...
                if (m_curInventoryBuffer.bLoopInventory)
                {
                    ...
                    btnInventory.Text = "開始盤存";
                    return;
                }
                else
                {
                    //ISO 18000-6B盤存是否正在運行
                    if (m_bContinue)
                    {
                        if (MessageBox.Show("ISO 18000-6B標籤正在盤存，是否停止?", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Cancel)
                        {
                            return;
                        }
                        else
        ...
        private void ProcessFastSwitch(Reader.MessageTran msgTran)
        {
            ...
            else if (msgTran.AryData.Length == 2)
            {
                ...
                string strLog = strCmd + "失敗，失敗原因： " + strErrorCode + "--" + "天線" + (msgTran.AryData[0] + 1);
            ...
            else
            {
                ...
                byte btAntId = (byte)((btTemp & 0x07) + 1);
                ...
                else
                {
                    foreach (DataRow dr in drs)
                    {
                        ...
                        if (btAntId >= 1 && btAntId <= 8)
                        {
                            nTemp = Convert.ToInt32(dr[btAntId - 1 + 7]);
                            dr[btAntId - 1 + 7] = (nTemp + 1).ToString();
                        }
        ...
        private void ProcessInventoryReal(Reader.MessageTran msgTran)
        {
            ...
            if (msgTran.Cmd == 0x8B)
            {
                strCmd = "自定義Session和Inventoried Flag盤存";
            }
            string strErrorCode = string.Empty;
...
//                CheckBlackList(strEPC.Replace(" ", ""));   
                CheckBlackList(strEPC);
            }
        }

        //比對資料庫黑名單 By Danny 2020/06/12
        private void CheckBlackList(string RFID)
        {
            //大門管控
            SqlConnection conn;
            String r1 = "";
            try
            {
                conn = new SqlConnection(str);
                conn.Open();
                string sql = "select top 1 IO from RFID_TRACK where RFIDNO='" + RFID + "' order by UserDate Desc";
                SqlCommand cmd = new SqlCommand(sql, conn);
                r1 = Convert.ToString(cmd.ExecuteScalar());
                conn.Close();
            }
            ...
            if (RFID.Substring(0,2).Equals(" E") && RFID.Length >= 36)
            {
                if (r1 == "RECEIVE" || r1 == "")
                {
                    if (CallClock(Port, 1))
                    {
                        SetDoorAction(Port, 0, 1);
                        Thread.Sleep(Siren_Time);
                        SetDoorAction(Port, 0, 0);
                    }
                }
            }            
            //緊急逃生門
 /*           if (CallClock(Port, 1))
            ...
            /*            
                        DataRow BList = BlackList.Select("RFID = '" + RFID + "'")[0];
        ...
        private void ProcessReadTag(Reader.MessageTran msgTran)
        {
            ...
            else
            {
                ...
                byte byAntId = (byte)((byTemp & 0x07) + 1);
        ...
        private void btnWriteTag_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (reslut == null)
                {
                    MessageBox.Show("輸入字元無效");
                    return;
                }
                else if (reslut.GetLength(0) < 4)
                {
                    MessageBox.Show("至少輸入4個字節");
                    return;
                }
        ...
        private void btnInventoryISO18000_Click(object sender, EventArgs e)
        {
            if (m_bContinue)
            {
                ...
                btnInventoryISO18000.Text = "開始盤存";
            }
            else
            {
                //判斷EPC盤存是否正在進行
                if (m_curInventoryBuffer.bLoopInventory)
                {
                    if (MessageBox.Show("EPC C1G2標籤正在盤存，是否停止?", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Cancel)
                    {
                        return;
                    }
                    else
                ...
                btnInventoryISO18000.Text = "停止盤存";
        ...
        private void ProcessInventoryISO18000(Reader.MessageTran msgTran)
        {
            ...
            else if (msgTran.AryData.Length == 9)
            {
                ...
                //增加保存標籤列表，原未盤存則增加記錄，否則將標籤盤存數量加1
        ...
        private void btnReadTagISO18000_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("請輸入UID");
                return;
            }
            if (htxtReadStartAdd.Text.Length == 0)
            {
                MessageBox.Show("請輸入讀取起始地址");
                return;
            }
            if (txtReadLength.Text.Length == 0)
            {
                MessageBox.Show("請輸入讀取長度");
                return;
            }
        ...
        private void WriteTagISO18000()
        {
            ...
            {
                MessageBox.Show("請輸入寫入地址");
                return;
            }
            if (htxtWriteData18000.Text.Length == 0)
            {
                MessageBox.Show("請輸入寫入數據");
                return;
            }
        ...
        private void ProcessWriteTagISO18000(Reader.MessageTran msgTran)
        {
            ...
            else
            {
                ...
                string strLog = strCmd + ": " + "成功寫入" + strLength + "字節";
                WriteLog(lrtxtLog, strLog, 0);
        ...
        private void btnLockTagISO18000_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("請輸入寫保護地址");
                return;
            }

            //確認寫保護提示
            if (MessageBox.Show("是否確定對該地址永久寫保護?", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Cancel)
            {
                return;
            }
        ...
        private void btRealTimeInventory_Click(object sender, EventArgs e)
        {
            if (CallClock(Port, 1) == false)
            {
                WriteLog(lrtxtLog, "無法連接警報器(IP: " + Siren_IP + ":" + Siren_Port.ToString() + ")，無法開始進行監控。", 1);
                return;
            }
            ...
            try
            {
                ...
                if (cbRealSession.Checked == true)
                {
                    ...
                    {
                        MessageBox.Show("請輸入Session ID");
                        return;
                    }
                    if (cmbTarget.SelectedIndex == -1)
                    {
                        MessageBox.Show("請輸入Inventoried Flag");
                        return;
                    }
                ...
                else
                {
                    ...
                    if (m_bContinue)
                    {
                        if (MessageBox.Show("正在監控ISO 18000-6B標籤，是否停止?", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Cancel)
                        {
                            return;
                        }
                        else
        ...
        private void btBufferInventory_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cbBufferWorkant5.Checked)
                {
                    m_curInventoryBuffer.lAntenna.Add(0x04);
                }
                if (cbBufferWorkant6.Checked)
                {
                    m_curInventoryBuffer.lAntenna.Add(0x05);
                }
                if (cbBufferWorkant7.Checked)
                {
                    m_curInventoryBuffer.lAntenna.Add(0x06);
                }
                if (cbBufferWorkant8.Checked)
                {
                    m_curInventoryBuffer.lAntenna.Add(0x07);
                }
                if (m_curInventoryBuffer.lAntenna.Count == 0)
        ...
        private void btFastInventory_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("請至少選擇一個天線至少輪詢一次，重複次數至少一次。");
                    m_bInventory = false;
                    m_curInventoryBuffer.bLoopInventory = false;
                    btFastInventory.BackColor = Color.WhiteSmoke;
                    btFastInventory.ForeColor = Color.DarkBlue;
        ...
        {
            if ((m_curSetting.btRegion < 1) || (m_curSetting.btRegion > 4)) //如果是自定義的頻譜則需要先提取自定義頻率信息
            {
                reader.GetFrequencyRegion(m_curSetting.btReadId);
                Thread.Sleep(5);
        ...
        private void cbRealWorkant8_CheckedChanged(object sender, EventArgs e)
        {

        }

        private void cbRealWorkant7_CheckedChanged(object sender, EventArgs e)
        {

        }

        private void cbRealWorkant6_CheckedChanged(object sender, EventArgs e)
        {

        }

        private void cbRealWorkant5_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private void cbBufferWorkant1_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private void ckClearOperationRec_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private void lvRealList_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
        ...
        {
            if (btRealTimeInventory.Text == "停止監控")
            {
                MessageBox.Show("Please Stop Read First !");
                return;
            }
            if (btRealTimeInventory.Text == "開始監控")
            {
                ListView lvtempe = new ListView();
                for (int j = 0; j <= lvRealList.Columns.Count-1; j++)
                {
                    lvtempe.Columns.Add(lvRealList.Columns[j].Text);
                }
                lvtempe.Items.Clear();
                var items = new List<ListViewItem>();
                foreach (ListViewItem lv in lvRealList.Items)
                {
                    ListViewItem item = new ListViewItem(lv.SubItems[0].Text);
                    ...
                    if (sfd.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        foreach (ListViewItem item in lv.Items)
                        {
                            for (int k = 1; k <= item.SubItems.Count; k++)
                            {
                                ...
                                Range r = (Range)ws.Cells[i, k];
                                if (item.ForeColor == Color.Red)
                                  r.Font.Color = System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Red);
                                 //   ws.Cells[i, k].Font.Color = System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Red);
                                //else
                                //    ws.Cells.Font.Color = System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Black);
                            }
                            i++;
                        }
        ...
        private void pageEpcTest_Click(object sender, EventArgs e)
        {
