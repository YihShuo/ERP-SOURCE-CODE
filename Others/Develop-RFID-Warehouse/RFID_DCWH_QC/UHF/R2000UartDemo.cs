    ...
    public partial class R2000UartDemo : Form
    {
        ...
        private void btRealTimeInventory_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (btRealTimeInventory.Text == "Start Read" && textBox1.Text != "")
                {
                    ...
                    conn.Open();
                    string sql = "select COUNT(*) from LIY_ERP.dbo.shoetest where Testno='" + textBox1.Text + "'";
                    SqlCommand cmd = new SqlCommand(sql, conn);
                ...
                for (int i = 0; i <= lvRealList.Items.Count - 1; i++)
                {
                    ...
                    else
                    {
                        sql = "select COUNT(*) from RFID_TB " +
                              "where RFIDNO='" + lvRealList.Items[i].SubItems[2].Text + "' and RFID_TB.PROGRAM_ID='DCWH_QC'";
                        cmd = new SqlCommand(sql, conn);
                        int r = Convert.ToInt32(cmd.ExecuteScalar());
                        if (r > 0)
                        {
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            string sql = "select shoetest.Article,kfxxzl.ARTICLE as Material# " +
                         " from LIY_ERP.dbo.shoetest " +
                         " left join LIY_ERP.dbo.ypzl on ypzl.YPDH=ShoeTest.YPDH " +
                         " left join LIY_ERP.dbo.kfxxzl on ypzl.XieXing=kfxxzl.XieXing and ypzl.SheHao=kfxxzl.SheHao " +
            ...
            else
            {
                sql = "select top 1 kfxxzl.DEVCODE " +
                      "from LIY_ERP.dbo.kfxxzl " +
                      "left join LIY_ERP.dbo.xxzl on xxzl.XieXing=kfxxzl.XieXing and xxzl.SheHao=kfxxzl.SheHao " +
        ...
        public static int ShowDialog(ComboBox cbt, string wh, string s, string ca, string xh, string tesno, string rfid, string id, string sr)
        {
            ...
            {
                SqlConnection conn;
                string str = "server=192.168.23.9;database= LIY_ERP;uid=tyxuan;pwd=Itkhua@!admin";
                try
                {
                    string ip = "", name = "", user = "", pass = "";
                    ...
                    foreach (ListViewItem lv in lvdel.CheckedItems)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                string sql;
                                sql = "select USERNAME from LIY_ERP.dbo.busers where USERID='" + userid[1] + "'";
                                SqlCommand cmd = new SqlCommand(sql, conn);
