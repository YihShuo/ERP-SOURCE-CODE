...
namespace UHFDemo
{
    public partial class R2000UartDemo : Form
    {
        ...
        private void R2000UartDemo_Load(object sender, EventArgs e)
        {
            ...
            ipIpServer.IpAddressStr = "192.168.1.178";
            ...
            System.IO.StreamReader sr = new System.IO.StreamReader("data.txt", Encoding.Default);
            String line;
            while ((line = sr.ReadLine()) != null)
            {
                dt.Rows.Add(new object[]{ line});
            }
        }
        ...
        //[DllImport("EastRiver.dll", CharSet = CharSet.Ansi, EntryPoint = "SelectCommStyle")]
        //public static extern Boolean SelectCommStyle(int CommStyle);

        [DllImport("EastRiver.dll", CharSet = CharSet.Ansi, EntryPoint = "OpenClientSocket")]
        public static extern uint OpenClientSocket(string RemoteAddr, uint Port);

        [DllImport("EastRiver.dll", CharSet = CharSet.Ansi, EntryPoint = "CallClock")]
        public static extern Boolean CallClock(uint hPort, uint clock_id);

        [DllImport("EastRiver.dll", CharSet = CharSet.Ansi, EntryPoint = "UnCallClock")]
        public static extern Boolean UnCallClock(uint hPort);


        [DllImport("EastRiver.dll", CharSet = CharSet.Ansi, EntryPoint = "CloseClientSocket", CallingConvention = CallingConvention.StdCall)]
        public static extern Boolean CloseClientSocket(uint hSocket);

        //[DllImport("EastRiver.dll", CharSet = CharSet.Ansi, EntryPoint = "SetDoorAction")]
        //public static extern uint SetDoorAction(uint hPort, uint DoorNo, int Action);

        [DllImport("EastRiver.dll", CharSet = CharSet.Ansi, EntryPoint = "MDoorRemoteAction")]
        public static extern uint MDoorRemoteAction(uint hPort, byte Action, byte AID, int AParam);   
 

        //灯控制
        private void set_light(string ip, UInt32 port, UInt32 ma_no, string light_no)
        {
            uint i = OpenClientSocket(ip, port);
            Boolean c = CallClock(i, ma_no);
            if (c == false)
            {
                //write_err2(ip + "联机失败!");
                return;
            ...
            int st = 10;
            try { st = Convert.ToInt16(UHFDemo.Properties.Settings.Default.sleeptime); }
            catch { st = 10; }
            MDoorRemoteAction(i, 3, Convert.ToByte(light_no, 16), st);
            UnCallClock(i);
            Boolean b = CloseClientSocket(i);
        }
                    ...
                    case 0x8B:
                        {
                            ...
                            if (nEpcCount < nEpcLength)
                            {
                                ...
                                DataRow dr = dt.Rows.Find(new object[] { row[2].ToString().Trim() });
                                if (dr != null)
                                {
                                    //MessageBox.Show("卡" + row[2].ToString() + "是黑名单");
                                    set_light(UHFDemo.Properties.Settings.Default.lightip, 4001, 1, "0");
                                }
