...
uses SysUtils, Windows, Messages, Classes, Controls, Forms, Math, Dialogs,
    Graphics, Menus, StdCtrls, buttons,  extctrls; // ERCommon,

type
  TERProcMeth = procedure of object;
  TERBoolFunc = function: Boolean of object;

  TERVAlignment = (vaTop, vaVCenter, vaBottom);

  TERTextStyle = (fclsDefault, fclsLowered, fclsRaised,
    fclsOutline);

  TEROrientation = (fcTopLeft, fcTopRight, fcBottomLeft, fcBottomRight,
    fcTop, fcRight, fcLeft, fcBottom);

  TERTextOption = (toShowAccel, toShowEllipsis, toFullJustify);
  TERTextOptions = set of TERTextOption;

  TERGetWordOption = (fcgwSkipLeadingBlanks, fcgwQuotesAsWords, fcgwStripQuotes,
                      fcgwSpacesInWords);
  TERGetWordOptions = set of TERGetWordOption;
  fcstrCharSet = Set of char;

type
  {
  // Any component that uses TERText MUST implement this interface.
  ...
  // (i.e. "result := Enabled;").
  //
  // AdjustBounds will be called whenever a property of TERText is
  // manipulated such that the rect that the text uses changes.
  // TERCustomLabel uses this method in conjunction with the AutoSize
  ...
  }
{
  ...
  TERTextCallbacks = record
    Invalidate: TERProcMeth;
    AdjustBounds: TERProcMeth;
    GetTextEnabled: TERBoolFunc;
  end;

  TERText = class;

  {
  // Properties related to the Shadow effects of TERText.
  //
  // Properties:
  ...
  TERShadowEffects = class(TPersistent)
  private
    FText: TERText;
  ...
  public
    constructor Create(Text: TERText);
  ...
  TERExtrudeEffects = class(TPersistent)
  private
    FText: TERText;
    ...
    procedure SetOrientation(Value: TEROrientation);
  ...
  published
    ...
    property Orientation: TEROrientation read FOrientation write SetOrientation default fcBottomRight;
    property Striated: Boolean read FStriated write SetStriated default False;
  end;

  TERDisabledColors = class(TPersistent)
  private
    FText: TERText;
  ...
  private
    ...
    FDisabledColors: TERDisabledColors;
    FExtrudeEffects: TERExtrudeEffects;
    FHighlightColor: TColor;
    FFlags: UINT;
    FFont: TFont;
    FLineSpacing: Integer;
    FOptions: TERTextOptions;
    FOutlineColor: TColor;
    FRotation: Integer;
    FScaledFont: Boolean;
    FShadeColor: TColor;
    FShadow: TERShadowEffects;
    FStyle: TERTextStyle;
    FText: string;
    FCallbacks: TERTextCallbacks;
    FTextRect: TRect;
    FVAlignment: TERVAlignment;
    FWordWrap: Boolean;
  ...
  public
    ...
    constructor Create(ACallbacks: TERTextCallbacks; ACanvas: TCanvas; AFont: TFont);
    ...
    property Callbacks: TERTextCallbacks read FCallbacks write FCallbacks;
    property DisabledColors: TERDisabledColors read FDisabledColors write FDisabledColors;
    property ExtrudeEffects: TERExtrudeEffects read FExtrudeEffects write FExtrudeEffects;
    ...
    property Options: TERTextOptions read FOptions write SetOptions default [toShowAccel];
    ...
    property ShadeColor: TColor read FShadeColor write SetShadeColor default clBtnShadow;
    property Shadow: TERShadowEffects read FShadow write FShadow;
    property Style: TERTextStyle read FStyle write SetStyle default fclsDefault;
    property Text: string read FText write SetText;
    property TextRect: TRect read FTextRect write SetTextRect;
    property VAlignment: TERVAlignment read FVAlignment write SetVAlignment;
    property WordWrap: Boolean read FWordWrap write SetWordWrap default False;
    property DoubleBuffered: boolean read FDoubleBuffered write FDoubleBuffered default False;
  end;

  TERCaptionText = class(TERText)
  published
    property Alignment;
  ...
  function MakeCallbacks(InvalidateProc, AdjustBoundsProc: TERProcMeth;
    GetTextEnabledProc: TERBoolFunc): TERTextCallbacks;

implementation

const
  OFFSETCOORD: array[TEROrientation] of TPoint = (
...
Function fcGetWord(s: string; var APos: integer;
                   Options: TERGetWordOptions; DelimSet: fcstrCharSet): string;
var i: integer;
...
constructor TERDisabledColors.Create(Text: TERText);
begin
  inherited Create;
  FText := Text;
...
procedure TERDisabledColors.SetHighlightColor(Value: TColor);
begin
...
procedure TERDisabledColors.SetShadeColor(Value: TColor);
begin
...
procedure TERShadowEffects.AssignTo(Dest: TPersistent);
begin
  with Dest as TERShadowEffects do
  begin
    Color := self.Color;
...
constructor TERShadowEffects.Create(Text: TERText);
begin
  inherited Create;
...
function TERShadowEffects.EffectiveOffset: TPoint;
begin
  result := Point(0,0);
...
procedure TERShadowEffects.SetColor(Value:TColor);
begin
...
procedure TERShadowEffects.SetEnabled(Value: Boolean);
begin
...
procedure TERShadowEffects.SetXOffset(Value: Integer);
begin
...
procedure TERShadowEffects.SetYOffset(Value: Integer);
begin
...
constructor TERExtrudeEffects.Create(Text: TERText);
begin
  inherited Create;
  FText := Text;
...
procedure TERDisabledColors.AssignTo(Dest: TPersistent);
begin
  with Dest as TERDisabledColors do
  begin
...
procedure TERExtrudeEffects.AssignTo(Dest: TPersistent);
begin
  with Dest as TERExtrudeEffects do
  begin
    Depth := self.Depth;
    Enabled := self.Enabled;
...
function TERExtrudeEffects.EffectiveDepth(CheckOrient: Boolean): TSize;
begin
...
procedure TERExtrudeEffects.SetDepth(Value: Integer);
begin
...
procedure TERExtrudeEffects.SetEnabled(Value: Boolean);
begin
  if FEnabled <> Value then
  begin
    FEnabled := Value;
...
procedure TERExtrudeEffects.SetFarColor(Value: TColor);
begin
...
procedure TERExtrudeEffects.SetNearColor(Value: TColor);
begin
...
procedure TERExtrudeEffects.SetOrientation(Value: TEROrientation);
begin
...
procedure TERExtrudeEffects.SetStriated(Value: Boolean);
begin
...
constructor TERText.Create(ACallbacks: TERTextCallbacks; ACanvas: TCanvas; AFont: TFont);
begin
  inherited Create;
  ...
  FExtrudeEffects := TERExtrudeEffects.Create(self);
  FHighlightColor := clBtnHighlight;
  FLineSpacing := 5;
  FOptions := [toShowAccel];
  FShadeColor := clBtnShadow;
  FShadow := TERShadowEffects.Create(self);
  FDisabledColors := TERDisabledColors.Create(self);

  FFlags := DT_NOCLIP;
end;

destructor TERText.Destroy;
begin
...
procedure TERText.AssignTo(Dest: TPersistent);
begin
  with Dest as TERText do
  begin
...
function TERText.GetAngle: Extended;
begin
  result := DegToRad(Rotation);
end;

procedure TERText.SetAlignment(Value: TAlignment);
begin
...
procedure TERText.SetLineSpacing(Value: Integer);
begin
...
procedure TERText.SetHighlightColor(Value: TColor);
begin
  if FHighlightColor <> Value then
  begin
    FHighlightColor := Value;
...
procedure TERText.SetOptions(Value: TERTextOptions);
begin
...
procedure TERText.SetOutlineColor(Value: TColor);
begin
...
procedure TERText.SetRotation(Value: Integer);
begin
...
procedure TERText.SetScaledFont(Value: Boolean);
begin
...
procedure TERText.SetShadeColor(Value: TColor);
begin
  if FShadeColor <> Value then
  begin
    FShadeColor := Value;
...
procedure TERText.SetStyle(Value: TERTextStyle);
begin
...
procedure TERText.SetText(Value: string);
begin
  if Value <> FText then
  begin
    FText := Value;
...
procedure TERText.SetTextRect(Value: TRect);
begin
  FTextRect := Value;
end;

procedure TERText.SetVAlignment(Value: TERVAlignment);
begin
...
procedure TERText.SetWordWrap(Value: Boolean);
begin
...
function TERText.GetLogFont: TLogFont;
...
function TERText.CalcTextSize(IgnoreRect: Boolean): TSize;
var Angle: Extended;
    TextSize: TSize;
begin
  Angle := self.Angle;
...
function TERText.CalcRect(IgnoreRect: Boolean): TRect;
var Angle: Extended;
    TextSize: TSize;
begin
  Angle := Self.Angle;
...
function TERText.GetTextSize: TSize;
var s: string;
  r:TRect;
  sz:TSize;
  xoffset:integer;
...
procedure TERText.DrawHighlight;
var r: TRect;
begin
  r := FRect;
...
procedure TERText.DrawShadow(r: TRect);
begin
...
procedure TERText.DrawOutline;
var i: TEROrientation;
    r: TRect;
begin
...
procedure TERText.DrawEmbossed(Raised: Boolean);
var r: TRect;
...
procedure TERText.DrawText(r: TRect);
var i: Integer;
    s: string;
    Angle: Extended;
...
procedure TERText.PrepareCanvas;
begin
  // Must Free This!
...
function TERText.CalcDrawRect(IgnoreRect: Boolean): TRect;
begin
  Canvas.Font.Handle := CreateFontIndirect(GetLogFont);
  try
...
procedure TERText.CallInvalidate;
begin
  if Assigned(Callbacks.Invalidate) then Callbacks.Invalidate;
end;

procedure TERText.UpdateFont(Value:TFont);
begin
...
procedure TERText.DrawStandardText;
begin
...
procedure TERText.DrawOutlineText;
begin
  PrepareCanvas;
  try
    DrawExtrusion;
...
procedure TERText.DrawEmbossedText(Raised: Boolean);
begin
  PrepareCanvas;
  try
    Canvas.Lock;
...
procedure TERText.DrawExtrusion;
...
function TERText.GetCanvas: TCanvas;
begin
  if InDraw then
     result:= FPaintCanvas
  else
