  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ShowMessage(IntToStr(hPort));
        if not CheckHandle(hPort) then
        begin
          hPort := 0;
          btnConnect.Caption := '聯機';
          stat1.Panels[0].Text := '聯機失敗！';
          Windows.Beep(2500, 1000);
          Exit;
        end;
        if CallClock(hPort, clockId) then
        begin
          btnConnect.Caption := '脫機';
          stat1.Panels[0].Text := '聯機成功！';
          Windows.Beep(2500, 100);
          iclockId := clockId;
          ShowMessage(IntToStr(clockId));
        end
        else
        begin
          hPort := 0;
...
begin
  ...
  begin
    ...
    begin
      ...
      self.stat1.Panels[0].Text := '獲取設備門信息成功！';
    end else
    begin
      self.stat1.Panels[0].Text := '獲取設備門信息失敗！';
    end;
  end else
  begin
    self.stat1.Panels[0].Text := '請先聯機！';
  end;
end;
...
begin
  if hPort <> 0 then
  begin
    if Application.MessageBox(PChar('確定要初始化選中的設備嗎？'), PChar('詢問'), MB_YESNO + MB_ICONQUESTION) = mrYes then
    begin
      bflag := False;
      ...
      if bflag then
        ...
        self.stat1.Panels[0].Text := '初始化失敗！';
    end;
  end else
  begin
    self.stat1.Panels[0].Text := '請先聯機！';
...
begin
  ...
  begin
    ...
    begin
      self.stat1.Panels[0].Text := '讀取時間失敗！';
      Windows.Beep(2500, 1000);
    end;
  end else
  begin
...
begin
  ...
  begin
    ...
    begin
      self.stat1.Panels[0].Text := '設置時間成功！';
      Windows.Beep(2500, 100);
    end else
    begin
      self.stat1.Panels[0].Text := '設置時間失敗！';
      Windows.Beep(2500, 1000);
    end;
  end else
  begin
...
begin
  if hPort <> 0 then
  begin
    if InputQuery('更改機號', '請輸入新的機號', sclockid) then
    begin
      if sclockid <> '' then
      begin
        ...
        begin
          ...
          begin
            ...
            if QuickSetDeviceIP(SourceIP, SourcePort, Serial, DevId, IPAddr, Maskaddr, Gateway2, IPPort, DHCP) = 0 then
              self.stat1.Panels[0].Text := '設置通訊參數成功！'
            else
              self.stat1.Panels[0].Text := '設置通訊參數失敗！';
            Break;
          end
        end;
      end;
...
begin
  ...
  begin
    ...
    if SetDoorAction(hPort, cbb2.ItemIndex, 1) = 0 then
      self.stat1.Panels[0].Text := Format('開門%d成功！', [cbb2.ItemIndex])
    else
      self.stat1.Panels[0].Text := Format('開門%d失敗！', [cbb2.ItemIndex]);
  end else
  begin
    self.stat1.Panels[0].Text := '請先聯機！';
  end;
...
begin
  ...
  begin
    ...
    if SetDoorAction(hPort, cbb2.ItemIndex, 0) = 0 then
      self.stat1.Panels[0].Text := Format('關門%d成功！', [cbb2.ItemIndex])
    else
      self.stat1.Panels[0].Text := Format('關門%d失敗！', [cbb2.ItemIndex]);
  end else
  begin
    self.stat1.Panels[0].Text := '請先聯機！';
  end;
...
begin
  ...
  begin
    ...
    if SetTimeGroupEx(hPort, ID, BeginDate, EndDate, WeekCtrl, pt1, pt2, pt3, et1, et2, et3) = 0 then
      self.stat1.Panels[0].Text := '設置時間組成功！'
    else
      self.stat1.Panels[0].Text := '設置時間組失敗！';
  end else
  begin
    self.stat1.Panels[0].Text := '請先聯機！';
  end;
...
begin
  ...
  begin
    ...
    if SetHolidayGroupEx(hPort, ReaderCtrl, ID, BeginDate, EndDate, WeekCtrl, pt1, pt2, pt3, et1, et2, et3) = 0 then
      self.stat1.Panels[0].Text := '設置節假日成功！'
    else
      self.stat1.Panels[0].Text := '設置節假日失敗！';
  end else
  begin
    self.stat1.Panels[0].Text := '請先聯機！';
  end;
...
begin
  ...
  begin
    ...
    if AddCardHolder(hPort, CardNo, Group, Password, expirationDate, HolidayPassed, ReaderCtrl, TimeGroupCtrl) = 0 then
      self.stat1.Panels[0].Text := '下載權限成功！'
    else
      self.stat1.Panels[0].Text := '下載權限失敗！';
  end else
  begin
    self.stat1.Panels[0].Text := '請先聯機！';
  end;
...
begin
  ...
  begin
    ...
    if DeleteCardHolder(hPort, CardNo) = 0 then
      self.stat1.Panels[0].Text := '刪除權限成功！'
    else
      self.stat1.Panels[0].Text := '刪除權限失敗！';
  end else
  begin
    self.stat1.Panels[0].Text := '請先聯機！';
  end;
...
begin
  ...
  begin
    ...
    self.stat1.Panels[0].Text := '設置設備門信息成功！';
...
begin
  ...
  begin
    ...
    begin
      ...
      self.stat1.Panels[0].Text := '獲取節假日成功！'
    end else
    begin
      self.stat1.Panels[0].Text := '獲取節假日失敗！';
    end;
  end else
  begin
    self.stat1.Panels[0].Text := '請先聯機！';
...
begin
  ...
  begin
    ...
    begin
      ...
      self.stat1.Panels[0].Text := '獲取時間組成功！'
    end else
    begin
      self.stat1.Panels[0].Text := '獲取時間組失敗！';
    end;
  end else
  begin
    self.stat1.Panels[0].Text := '請先聯機！';
...
begin
  ...
  try
    if (Pos('-', DateTimeStr) > 0) then //有年月日時分秒
    begin
    ...
    else if (Pos('-', DateTimeStr) = 0) and (Pos(':', DateTimeStr) > 0) then //沒有年月日，只有時分秒
    begin
      Year := 1899;
      Month := 12;
      Day := 30;
...
begin
  ...
  if lpReadData^.Verify_Error then  //如果校驗錯，保存原始文本
  begin
  ...
  try
    //Flag 0正常卡 14黑名單卡 11非法卡
    //卡號全0表示開門關門等事件，只有有實際卡號時才表示刷卡開門
    ...
    MealRecord.PwdKind:= lpReadData^.dev_ver;    //密碼類型
...
begin
  ...
  begin
    if ReadClockRecordTotal(hPort,Count) then
      Mmo1.Lines.Add('當前共存在' + IntToStr(Count) + '條記錄！')
    else
      Exit;
    pck.hPort := hPort;
    ...
    if Count>=0 then
      Mmo1.Lines.Add('採集成功，共採集數據' + IntToStr(Count) + '條！')
    else
      Mmo1.Lines.Add('採集失敗！');
  end else
  begin
...
begin
  ...
  until count <= 0;
  Mmo1.Lines.Add('共採集數據' + IntToStr(total) + '條！');
end;
...
begin
  ...
  while 1=1 do
  begin
    Count:=BatchReadRecordAsString(hPort,buf);                     //這個每次只能取16條數據。
    if Count>0 then
...
begin
  ...
  begin
    ...
    if MDoorRemoteAction(hPort, 3, cbb2.ItemIndex, 3) = 0 then
      self.stat1.Panels[0].Text := Format('遠程開門%d成功！', [cbb2.ItemIndex])
    else
      self.stat1.Panels[0].Text := Format('遠程開門%d失敗！', [cbb2.ItemIndex]);
  end else
  begin
    self.stat1.Panels[0].Text := '請先聯機！';
  end;
...
begin
  ...
  begin
    ...
    if MDoorRemoteAction(hPort, 4, cbb2.ItemIndex, 3) = 0 then
      self.stat1.Panels[0].Text := Format('遠程關門%d成功！', [cbb2.ItemIndex])
    else
      self.stat1.Panels[0].Text := Format('遠程關門%d失敗！', [cbb2.ItemIndex]);
  end else
  begin
    self.stat1.Panels[0].Text := '請先聯機！';
  end;
...
begin
  ...
  begin
     ...
     stat1.Panels[0].text:='讀取成功';
  end
  else
     stat1.Panels[0].text:='讀取失敗';
end;
...
begin
  ...
  if SetRemoteIP(hPort,PChar(RemoteIP),RemotePort,3,'',0)=0  then      //第四個參數 傳3 是啟用主動上傳，傳0是關閉主動上傳
     stat1.Panels[0].text:='設置成功'
  else
     stat1.Panels[0].text:='設置失敗';
end;
...
begin
  ...
  begin
     ...
     begin
        ...
        if State=0 then
        begin
          it.SubItems.Strings[3]:='脫機';
        end
        else
        begin
          it.SubItems.Strings[3]:='聯機';
        end;
        break;
     end;
  end;
...
begin
  ...
  begin
    ...
    strtmp:=Format('機號：%d;收到記錄,卡號:%s,序列號:%s,時間:%s,門號:%d,門磁狀態:%d,門狀態:%d,讀頭:%d,事件:%d',
