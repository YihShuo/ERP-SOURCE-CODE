...
begin
  ...
  begin
    ...
    SQL.Add('  SELECT ''A廠區'' AS Factory, ''開發-鞋廠-幕僚'' AS Department, MAX(Total_TW) + MAX(Total_CN) + MAX(Total_PH) + MAX(Total_Others) AS Total,');
    ...
    SQL.Add('    WHERE  IsNull(Certificate.Resigned,0)=0 and Directory.FactoryS IN (''009'',''General'', ''003'',''004'',''002'',''DC'',''006'') and Directory.FID = ''01'' and Certificate.NID<>''VN'' ');
    SQL.Add('    GROUP BY Certificate.NID');
    SQL.Add('  ) AS Nationality');
    SQL.Add('  LEFT JOIN (');
    SQL.Add('    SELECT ''A廠區'' AS Factory, ''開發-鞋廠-幕僚'' AS Department, SUM(Absence) AS Absence, SUM(BusinessTrip) AS BusinessTrip FROM (');
    SQL.Add('      SELECT');
    ...
    SQL.Add('      and Directory.FactoryS IN (''009'',''General'', ''003'',''004'',''002'',''DC'',''006'') and Directory.FID = ''01''');
