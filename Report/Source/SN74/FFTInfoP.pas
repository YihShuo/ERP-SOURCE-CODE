...
implementation
  uses  main1, FFT_Dep, FunUnit;
{$R *.dfm}

procedure TFFTInfo.FormClose(Sender: TObject; var Action: TCloseAction);
begin
   action:=cafree;
end;

procedure TFFTInfo.FormDestroy(Sender: TObject);
begin
  self:=nil;
end;

procedure TFFTInfo.Button1Click(Sender: TObject);
begin
  FFTDep:=TFFTDep.Create(self);
  FFTDep.Parent:=self;
  FFTDep.ShowModal;
  FFTDep.Free;
...
//顯示報表
procedure TFFTInfo.ShowSumlineFFTReport();
begin
   //顯示產線FFT
   with FFTQry do
   begin
...
//
procedure TFFTInfo.ReLoadTimerTimer(Sender: TObject);
begin
...
procedure TFFTInfo.FormCreate(Sender: TObject);
begin
  TDate.Date:=Date();//今天日期
end;

procedure TFFTInfo.GroupBox1DblClick(Sender: TObject);
begin
  If TDate.Visible=false then
    TDate.Visible:=true
  else
    TDate.Visible:=false;
end;
