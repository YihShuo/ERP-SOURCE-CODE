    ...
    begin
      ...
      begin
        ...
        begin
            ...
            workid := FieldByName('workid').Value;
            SIZE := FieldByName('SIZE').Value;
            WorkScanQry.Edit;
            WorkScanQry.FieldByName('Qty').Value:=Qty;
            WorkScanQry.Post;
        end;
      ...
      TmpQuery.SQL.Clear;
      TmpQuery.SQL.Add('select workplansss.*,workplanss.ShipDate,(Quantity - InQty) lastqty from '+main.LIY_DD+'.dbo.workplansss workplansss');
      TmpQuery.SQL.Add('left join '+main.LIY_DD+'.dbo.workplanss workplanss on workplanss.WorkID = workplansss.WorkID and workplanss.DDBH = workplansss.DDBH');
      TmpQuery.SQL.Add('where workplansss.WorkID = '''+workplansQry.fieldbyname('WorkID').Value+''' and size = '''+workplansQry.fieldbyname('Size').Value+'''');
      TmpQuery.SQL.Add('and Quantity - InQty > 0');
      TmpQuery.SQL.Add('order by ShipDate');
      TmpQuery.Active := true;
      TmpQuery.First;
      if TmpQuery.RecordCount > 0 then begin
        if TmpQuery.FieldByName('lastqty').Value > Qty then begin
          TmpQuery2.Active := false;
          TmpQuery2.SQL.Clear;
          TmpQuery2.SQL.Add('Update '+main.LIY_DD+'.dbo.Workplansss set inqty = '+inttostr(Qty)+'');
          TmpQuery2.SQL.Add('where WorkID = '''+workplansQry.fieldbyname('WorkID').Value+''' and size = '''+workplansQry.fieldbyname('Size').Value+'''');
          TmpQuery2.SQL.Add('and DDBH = '''+workplansQry.fieldbyname('DDBH').Value+'''');
          TmpQuery2.ExecSQL;
        end
        else begin
          TmpQuery2.Active := false;
          TmpQuery2.SQL.Clear;
          TmpQuery2.SQL.Add('Update '+main.LIY_DD+'.dbo.Workplansss set inqty = '+TmpQuery.fieldbyname('Quantity').AsString+'');
          TmpQuery2.SQL.Add('where WorkID = '''+workplansQry.fieldbyname('WorkID').Value+''' and size = '''+workplansQry.fieldbyname('Size').Value+'''');
          TmpQuery2.SQL.Add('and DDBH = '''+workplansQry.fieldbyname('DDBH').Value+'''');
          TmpQuery2.ExecSQL;
          TmpQuery.Next;
...
begin
  if SecondsBetween(WorkDTP.Date,Date())>(2*24*60*60) then
  begin
    WorkDTP.Date:=now();
  end;
  RefreshWorkDate();
