...
procedure  TDataInput.InitUpdateSQL();
begin
   UpdateSQL1.InsertSQL.Clear;
   UpdateSQL1.InsertSQL.Add('insert into '+main.LIY_DD+'.dbo.workoverreport(workno, inputdate, xiexing, MAXcount, THIScount, sqty, rqty, qty, cost)');
   UpdateSQL1.InsertSQL.Add('values (:workno, :inputdate, :xiexing, :MAXcount, :THIScount, :sqty, :rqty, :qty, :cost)');

   UpdateSQL1.ModifySQL.Clear;
   UpdateSQL1.ModifySQL.Add('update '+main.LIY_DD+'.dbo.workoverreport');
   UpdateSQL1.ModifySQL.Add('set workno = :workno,inputdate = :inputdate,xiexing = :xiexing,MAXcount = :MAXcount,THIScount = :THIScount,');
   UpdateSQL1.ModifySQL.Add('sqty = :sqty,rqty = :rqty,qty = :qty,cost = :cost');
   UpdateSQL1.ModifySQL.Add('where workno = :OLD_workno and inputdate = :OLD_inputdate and xiexing = :OLD_xiexing');

   UpdateSQL1.DeleteSQL.Clear;
   UpdateSQL1.DeleteSQL.Add('delete from '+main.LIY_DD+'.dbo.workoverreport');
   UpdateSQL1.DeleteSQL.Add('where workno = :OLD_workno and inputdate = :OLD_inputdate and xiexing = :OLD_xiexing');
end;
...
procedure TDataInput.FormDestroy(Sender: TObject);
begin
  DataInput:=nil;
end;
...
procedure TDataInput.selebClick(Sender: TObject);
var monthstr :string;
begin
  with showdata do
  begin
...
procedure TDataInput.BB2Click(Sender: TObject);
begin
  with showdata do
  begin
    requestlive:=true;
...
procedure TDataInput.BB3Click(Sender: TObject);
begin
  with showdata do
  begin
    requestlive:=true;
...
procedure TDataInput.BB4Click(Sender: TObject);
begin
  with showdata do
  begin
    requestlive:=true;
...
procedure TDataInput.BB6Click(Sender: TObject);
begin
 with showdata do
 begin
    active:=false;
...
procedure TDataInput.BB5Click(Sender: TObject);
var i:integer;
begin
  try
    showdata.First;
...
procedure TDataInput.dbg1DrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumnEh; State: TGridDrawState);
begin
  if showdata.FieldByName('YN').value='0' then
  begin
...
procedure TDataInput.BB7Click(Sender: TObject);
begin
  with showdata do
  begin
    requestlive:=true;
