...
begin
  ...
  if length(Day1)=1 then
    Day1:='0'+Day1;
  QRLabel36.Caption:='Ngay日 '+Day1+' thang月 '+Month1+' nam年 '+Year1;
end;
...
begin
  ...
  begin
    ...
    SQL.Clear;
     SQL.Add('SELECT');
     SQL.Add('  ROW_NUMBER() OVER (ORDER BY YSBH) AS STT,');
     ...
     SQL.Add('GROUP BY YSBH, Article');
     SQL.Add('HAVING SUM(Win) <> 0');
    Open;
  end;
end;
