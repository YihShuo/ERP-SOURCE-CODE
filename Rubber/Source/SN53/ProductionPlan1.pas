...
type
  TProductionPlan = class(TForm)
    ...
    workplanSPLB: TStringField;
    Label8: TLabel;
    SPLBEdit: TEdit;
    workplanOKQty: TIntegerField;
    WorkPlanSSARTICLE: TStringField;
    WorkPlanSSShipDate: TDateTimeField;
    Splitter1: TSplitter;
    DBGridEh4: TDBGridEh;
    WorkPlanSSS: TQuery;
    WPSSS: TDataSource;
    WorkPlanSSSWorkID: TStringField;
    WorkPlanSSSDDBH: TStringField;
    WorkPlanSSSSIZE: TStringField;
    WorkPlanSSSQuantity: TIntegerField;
    WorkPlanSSSOutQTY: TIntegerField;
    WorkPlanSSSUserID: TStringField;
    WorkPlanSSSCreatDate: TDateTimeField;
    WorkPlanSSSInQTY: TIntegerField;
...
begin
  ...
  upwp.InsertSQL.Add('insert into '+main.LIY_DD+'.dbo.workplan(WorkID, CQDH, SPLB, DDLB, MJBH, ColorNo, Memo, WorkQty, WorkStartDate, WorkEndDate, UserID,CreatDate, UpdateDate, LastUser, YN)');
  upwp.InsertSQL.Add('values(:WorkID, :CQDH, :SPLB, :DDLB, :MJBH, :ColorNo, :Memo, :WorkQty, :WorkStartDate, :WorkEndDate, :UserID, :CreatDate, :UpdateDate, :LastUser, :YN)');

  upwp.ModifySQL.Clear;
  upwp.ModifySQL.Add('update '+main.LIY_DD+'.dbo.workplan');
  upwp.ModifySQL.Add('set  SPLB=:SPLB, DDLB=:DDLB, MJBH = :MJBH,ColorNo = :ColorNo, Memo =:Memo, WorkQty = :WorkQty,WorkStartDate = :WorkStartDate,');
  ...
  upwps.ModifySQL.Add('set WorkID = :WorkID,DDBH = :DDBH,Package =:Package,SNO = :SNO, Size = :Size,Qty = :Qty,OKQTY = :OKQTY, OutQTY=:OutQTY ,NBarCode = :NBarCode,');
  ...
  upwpss.InsertSQL.Add('insert into '+main.LIY_DD+'.dbo.workplanss(WorkID, DDBH, UserID, CreatDate, UpdateDate, LastUser, YN, ShipDate)');
  upwpss.InsertSQL.Add('values (:WorkID, :DDBH, :UserID, :CreatDate, :UpdateDate, :LastUser, :YN, :ShipDate)');
  ...
  upwpss.ModifySQL.Add('CreatDate = :CreatDate,UpdateDate = :UpdateDate,LastUser = :LastUser,ShipDate = :ShipDate');
...
begin
  ...
  begin
    active:=false;
    sql.Clear;
    sql.add('select *,(select SUM(OKQty) as OKQty from '+main.LIY_DD+'.dbo.WorkPlanS workplans where workplans.WorkID=workplan.WorkID) as OKQty ');
    sql.add('from '+main.LIY_DD+'.dbo.workplan workplan ');
    ...
    if SPLBEdit.Text<>'' then
      sql.add(' and workplan.SPLB like ''%'+SPLBEdit.Text+'%'' ');
    sql.add('and workplan.CQDH='''+main.Edit2.Text+''' ');
  ...
  begin
    active:=false;
    sql.Clear;
    sql.Add('select workplanss.*,DDZL.Article ');
    sql.Add('from '+main.LIY_DD+'.dbo.workplanss workplanss');
    sql.Add('left join '+main.LIY_DD+'.dbo.DDZL DDZL on DDZL.DDBH=workplanss.DDBH and DDZL.CQDH='''+main.Edit2.text+''' ');
    sql.Add('where workid=:workid');
    active:=true;
  end;
  ...
  begin
    active:=false;
    sql.Clear;
    sql.Add('select workplansss.* ');
    sql.Add('from '+main.LIY_DD+'.dbo.workplansss workplansss');
    sql.Add('where workid=:workid and ddbh =:ddbh');
    active:=true;
  end;
  bb2.enabled:=true;
  bb3.enabled:=true;
      ...
      case workplan.updatestatus of
        ...
        begin
          ...
          begin
            ...
            if workplan.fieldbyname('SPLB').AsString='' then
              workplan.fieldbyname('SPLB').Value:='DE';
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if workplan.fieldbyname('SPLB').AsString='' then
                   workplan.fieldbyname('SPLB').Value:='DE';
                if workplan.fieldbyname('SPLB').Value<>workplan.fieldbyname('SPLB').OldValue then
                begin
                  with TmpQuery do
                  begin
                    Active:=false;
                    SQL.Clear;
                    SQL.Add('Select * from (');
                    SQL.Add('Select workplan.WorkID  from '+main.LIY_DD+'.dbo.workplanss workplanss,'+main.LIY_DD+'.dbo.workplan workplan ');
                    SQL.Add('where workplan.workID=workplanss.WorkID and workplan.SPLB='''+workplan.FieldByName('SPLB').AsString+''' ');
                    SQL.Add(') workPlan where WorkID is not null');
                    Active:=true;
                    if RecordCount>0 then
                    begin
                      workplan.fieldbyname('SPLB').Value:=workplan.fieldbyname('SPLB').OldValue;
                      Showmessage('SPLB can''t change. Order have exists!'+TmpQuery.FieldByName('WorkID').AsString);
                    end;
                    Active:=false;
                  end;
                end;
    ...
    try
      ...
      begin
        ...
        begin
          Active:=false;
          sql.Clear;
          if ((workplan.FieldByName('SPLB').AsString='DE') or (workplan.FieldByName('SPLB').AsString='DE GC')) then //大底 根據LIY_DD.DDZLS已經有進退號
          begin
          ...
          begin
            SQL.Add('  select DDBH,CC,Qty,(Qty % '+inttostr(Package)+') as Remain,Round((Qty/'+inttostr(Package)+'),0,1) as CTS from(  ');
            SQL.Add('  select DDBH,CC,cast(sum(Quantity) as int) as Qty from '+main.LIY_ERP+'.dbo.ddzls');
            SQL.Add('	 where DDBH in 	(select ddbh from '+main.LIY_DD+'.dbo.workplanss  ');
            SQL.Add('                  where workid='''+workplan.FieldByName('workid').Value+''') ');
            SQL.Add('  group by DDBH,CC  ) as OrderList	    ');
            SQL.Add('  order by CC,DDBH   ');
          end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            SQL.Add('  order by CC,DDBH   ');
          end else  //邊條或前包頭 Foxing TA LONG/TOE CAP  Bao mui
          begin
            SQL.Add('  select ''ZZZZZZZZZZ'' as DDBH,CC,Qty,(Qty % '+inttostr(Package)+') as Remain,Round((Qty/'+inttostr(Package)+'),0,1) as CTS from(  ');
            SQL.Add('  select CC,cast(sum(Quantity) as int) as Qty from '+main.LIY_ERP+'.dbo.ddzls');
            SQL.Add('	 where DDBH in 	(select ddbh from '+main.LIY_DD+'.dbo.workplanss  ');
            SQL.Add('                  where workid='''+workplan.FieldByName('workid').Value+''') ');
            SQL.Add('  group by CC  ) as OrderList	    ');
            SQL.Add('  order by CC,DDBH   ');
      ...
      begin
        case workplans.updatestatus of
          ...
          begin
              ...
              if (workplans.FieldByName('Qty').value mod Package)=0 then
              begin
                workplans.FieldByName('SNO').value:=1;
                workplans.FieldByName('Package').value:=Package;
                workplans.FieldByName('NBarCode').value:=workplans.FieldByName('Qty').value div Package;
                workplans.FieldByName('YN').Value:='1';
              end else
              begin
                workplans.FieldByName('Qty').value:=(workplans.FieldByName('Qty').value mod Package);
                workplans.FieldByName('SNO').value:=2;
                workplans.FieldByName('NBarCode').value:=1;
                workplans.FieldByName('Package').value:=(workplans.FieldByName('Qty').value mod Package);
...
begin
  try
      ...
      case workplanss.updatestatus of
        ...
        begin
          ...
          begin
            ...
            TmpQuery.SQL.Clear;
            TmpQuery.SQL.Add('Select ShipDate from '+main.LIY_ERP+'.dbo.DDZL where DDBH = '''+workplanss.fieldbyname('ddbh').Value+'''');
            if main.LIY_ERP = 'TB_ERP' then begin
              TmpQuery.SQL.Add('union all');
              TmpQuery.SQL.Add('Select ShipDate from Tythac.LYS_ERP.dbo.DDZL where DDBH = '''+workplanss.fieldbyname('ddbh').Value+'''');
            end;
            ...
            workplanss.FieldByName('ShipDate').Value:=TmpQuery.FieldByName('ShipDate').Value;
            workplanss.FieldByName('YN').Value:='1';
            upwpss.apply(ukinsert);
            TmpQuery.Active := false;
            TmpQuery.SQL.Clear;
            TmpQuery.SQL.Add('INSERT INTO '+main.LIY_DD+'.dbo.workplansss (WorkID, DDBH, SIZE, Quantity, OutQTY, UserID, CreatDate, InQty)');
            TmpQuery.SQL.Add('SELECT '''+workplan.FieldByName('workid').Value+''', '''+workplanss.fieldbyname('ddbh').Value+''', CC, Quantity, 0 , '''+main.edit1.text+''', getdate(), 0');
            TmpQuery.SQL.Add('FROM '+main.LIY_ERP+'.dbo.DDZLS');
            TmpQuery.SQL.Add('WHERE ddbh = '''+workplanss.fieldbyname('ddbh').Value+'''');
            if main.LIY_ERP = 'TB_ERP' then begin
              TmpQuery.SQL.Add('union');
              TmpQuery.SQL.Add('SELECT '''+workplan.FieldByName('workid').Value+''', '''+workplanss.fieldbyname('ddbh').Value+''', CC, Quantity, 0 , '''+main.edit1.text+''', getdate(), 0');
              TmpQuery.SQL.Add('FROM Tythac.LYS_ERP.dbo.DDZLS');
              TmpQuery.SQL.Add('WHERE ddbh = '''+workplanss.fieldbyname('ddbh').Value+'''');
            end;
            TmpQuery.ExecSQL;
          end;
        ...
        begin
          ...
          begin
            ...
            begin
              TmpQuery.Active := false;
              TmpQuery.SQL.Clear;
              TmpQuery.SQL.Add('delete from '+main.LIY_DD+'.dbo.workplansss ');
              TmpQuery.SQL.Add('where DDBH = '''+workplanss.fieldbyname('ddbh').Value+''' ');
              TmpQuery.SQL.Add('and workid = '''+workplanss.fieldbyname('workid').Value+''' ');
              TmpQuery.ExecSQL;
    ...
    workplansss.requestlive:=false;
    workplansss.active:=true;
    BE5.enabled:=false;
    BE6.Enabled:=false;
  except
    Messagedlg('Have wrong, can not save data!',mtwarning,[mbyes],0);
