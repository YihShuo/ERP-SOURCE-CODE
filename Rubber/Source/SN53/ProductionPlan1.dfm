object ProductionPlan: TProductionPlan
  Left = 336
  Top = 126
  Width = 1077
  Height = 607
  Caption = 'Production Plan'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1061
    Height = 65
    Align = alTop
    TabOrder = 0
    object Label18: TLabel
      Left = 704
      Top = 32
      Width = 74
      Height = 24
      Caption = 'Master'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -19
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
    object BB1: TBitBtn
      Left = 8
      Top = 8
      Width = 49
      Height = 49
      Caption = 'Query'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      OnClick = BB1Click
      Glyph.Data = {
        
        3000333333333377377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB2: TBitBtn
      Left = 56
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Insert one New Record'
      Caption = 'Insert'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      OnClick = BB2Click
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB3: TBitBtn
      Left = 104
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Delete one Record'
      Caption = 'Delete'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      OnClick = BB3Click
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB4: TBitBtn
      Left = 152
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Modify'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 3
      OnClick = BB4Click
      Glyph.Data = {
        
        50BB555555555555575F555555555555550B5555555555555575}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB5: TBitBtn
      Left = 200
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Save Current'
      Caption = 'Save'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 4
      OnClick = BB5Click
      Glyph.Data = {
        
        5990555555555555577755555555555555555555555555555555}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB6: TBitBtn
      Left = 248
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Cancel'
      Caption = 'Cancel'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 5
      OnClick = BB6Click
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB7: TBitBtn
      Left = 296
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Exit Current Form'
      Caption = 'Exit'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 6
      OnClick = BB7Click
      Glyph.Data = {
        
        EE33337FFFFFFFFFFF7F33EEEEEEEEEEEE333377777777777773}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BBT1: TBitBtn
      Left = 368
      Top = 8
      Width = 49
      Height = 49
      Caption = 'First'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 7
      OnClick = BBT1Click
      Glyph.Data = {
        
        CCC333333333333777FF33333333333333CC3333333333333773}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BBT2: TBitBtn
      Left = 416
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Insert one New Record'
      Caption = 'Prior'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 8
      OnClick = BBT2Click
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BBT3: TBitBtn
      Left = 464
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Delete one Record'
      Caption = 'Next'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 9
      OnClick = BBT3Click
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BBT4: TBitBtn
      Left = 512
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Last'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 10
      OnClick = BBT4Click
      Glyph.Data = {
        
        33333777333333333333CC333333333333337733333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object bbt5: TBitBtn
      Left = 584
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Print'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 11
      Glyph.Data = {
        
        33333337FFFF7733333333300000033333333337777773333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object bbt6: TBitBtn
      Left = 632
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Excel'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 12
      OnClick = bbt6Click
      Glyph.Data = {
        
        3BB33773333773333773B333333B3333333B7333333733333337}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
  end
  object pc1: TPageControl
    Left = 0
    Top = 195
    Width = 1061
    Height = 373
    ActivePage = TabSheet1
    Align = alClient
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    OnChange = pc1Change
    object TabSheet1: TTabSheet
      Caption = 'WorkPlan'
      object DBGridEh1: TDBGridEh
        Left = 0
        Top = 80
        Width = 1053
        Height = 262
        Align = alClient
        DataSource = WP
        Flat = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -11
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        FooterRowCount = 1
        ParentFont = False
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -13
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        UseMultiTitle = True
        OnDblClick = DBGridEh1DblClick
        OnGetCellParams = DBGridEh1GetCellParams
        Columns = <
          item
            EditButtons = <>
            FieldName = 'WorkID'
            Footer.ValueType = fvtCount
            Footers = <>
            ReadOnly = True
            Width = 82
          end
          item
            EditButtons = <>
            FieldName = 'CQDH'
            Footers = <>
            ReadOnly = True
            Width = 47
          end
          item
            Color = clYellow
            EditButtons = <>
            FieldName = 'SPLB'
            Footers = <>
            PickList.Strings = (
              'DE'
              'BAO MUI'
              'TA LONG'
              'DE GC')
          end
          item
            Color = clYellow
            EditButtons = <>
            FieldName = 'DDLB'
            Footers = <>
            PickList.Strings = (
              'N'
              'B')
            Title.Caption = 'LB'
            Width = 27
          end
          item
            Color = clYellow
            EditButtons = <>
            FieldName = 'mjname'
            Footers = <>
            Title.Caption = 'Model Name'
            Width = 128
          end
          item
            Color = clYellow
            EditButtons = <>
            FieldName = 'colorname'
            Footers = <>
            Title.Caption = 'Color Name'
            Width = 219
          end
          item
            EditButtons = <>
            FieldName = 'WorkQty'
            Footers = <>
            ReadOnly = True
          end
          item
            EditButtons = <>
            FieldName = 'OKQty'
            Footers = <>
            ReadOnly = True
            Width = 58
          end
          item
            Color = clYellow
            EditButtons = <>
            FieldName = 'WorkStartDate'
            Footers = <>
            Width = 95
          end
          item
            Color = clYellow
            EditButtons = <>
            FieldName = 'WorkEndDate'
            Footers = <>
            Width = 84
          end
          item
            EditButtons = <>
            FieldName = 'Memo'
            Footers = <>
            Width = 145
          end
          item
            EditButtons = <>
            FieldName = 'UserID'
            Footers = <>
            Width = 73
          end>
      end
      object Panel2: TPanel
        Left = 0
        Top = 0
        Width = 1053
        Height = 80
        Align = alTop
        TabOrder = 1
        Visible = False
        object Label1: TLabel
          Left = 23
          Top = 19
          Width = 88
          Height = 16
          Caption = 'WorkStartDate'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label2: TLabel
          Left = 209
          Top = 19
          Width = 19
          Height = 16
          Caption = 'TO'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label3: TLabel
          Left = 24
          Top = 52
          Width = 58
          Height = 16
          Caption = 'Mode NO'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label4: TLabel
          Left = 215
          Top = 53
          Width = 32
          Height = 16
          Caption = 'Color'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label6: TLabel
          Left = 466
          Top = 52
          Width = 60
          Height = 16
          Caption = 'Order NO:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label7: TLabel
          Left = 330
          Top = 20
          Width = 48
          Height = 16
          Caption = 'WorkID:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label8: TLabel
          Left = 490
          Top = 20
          Width = 37
          Height = 16
          Caption = 'SPLB:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object DTP1: TDateTimePicker
          Left = 114
          Top = 16
          Width = 89
          Height = 24
          Date = 40904.646302638890000000
          Time = 40904.646302638890000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
        end
        object DTP2: TDateTimePicker
          Left = 234
          Top = 16
          Width = 89
          Height = 24
          Date = 40904.646444421300000000
          Time = 40904.646444421300000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
        end
        object MJCB: TComboBox
          Left = 85
          Top = 48
          Width = 121
          Height = 24
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ItemHeight = 16
          ItemIndex = 0
          ParentFont = False
          TabOrder = 2
          Text = 'ALL'
          Items.Strings = (
            'ALL')
        end
        object CLCB: TComboBox
          Left = 250
          Top = 48
          Width = 193
          Height = 24
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ItemHeight = 16
          ParentFont = False
          TabOrder = 3
          Text = 'ALL'
          OnChange = CLCBChange
          Items.Strings = (
            'ALL')
        end
        object Button1: TButton
          Left = 655
          Top = 43
          Width = 75
          Height = 31
          Caption = 'QUERY'
          TabOrder = 4
          OnClick = Button1Click
        end
        object DDBHEdit: TEdit
          Left = 531
          Top = 48
          Width = 121
          Height = 24
          TabOrder = 5
        end
        object WorkIDEdit: TEdit
          Left = 379
          Top = 16
          Width = 86
          Height = 24
          TabOrder = 6
        end
        object CheckBox: TCheckBox
          Left = 739
          Top = 53
          Width = 57
          Height = 17
          Caption = 'Mine'
          Checked = True
          State = cbChecked
          TabOrder = 7
        end
        object SPLBEdit: TEdit
          Left = 531
          Top = 16
          Width = 118
          Height = 24
          TabOrder = 8
        end
      end
    end
    object TabSheet2: TTabSheet
      Caption = 'SIZE List'
      ImageIndex = 1
      object DBGridEh2: TDBGridEh
        Left = 0
        Top = 0
        Width = 1053
        Height = 342
        Align = alClient
        DataSource = WPS
        Flat = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -11
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        FooterRowCount = 1
        ParentFont = False
        SumList.Active = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -16
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        UseMultiTitle = True
        OnGetCellParams = DBGridEh2GetCellParams
        Columns = <
          item
            EditButtons = <>
            FieldName = 'Sno'
            Footer.ValueType = fvtCount
            Footers = <>
            ReadOnly = True
            Title.Caption = 'SNO'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -13
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = []
            Width = 43
          end
          item
            EditButtons = <>
            FieldName = 'Size'
            Footers = <>
            Title.Caption = 'SIZE'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -13
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = []
            Width = 50
          end
          item
            Color = clYellow
            EditButtons = <>
            FieldName = 'Qty'
            Footer.FieldName = 'Qty'
            Footer.ValueType = fvtSum
            Footers = <>
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -13
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = []
            Width = 88
          end
          item
            EditButtons = <>
            FieldName = 'OKQTY'
            Footers = <>
            ReadOnly = True
            Title.Caption = 'OKQty'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -13
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = []
            Width = 85
          end
          item
            EditButtons = <>
            FieldName = 'NBarCode'
            Footers = <>
            ReadOnly = True
            Title.Caption = 'CTS'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -13
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = []
            Width = 82
          end
          item
            EditButtons = <>
            FieldName = 'Pbarcode'
            Footers = <>
            ReadOnly = True
            Title.Caption = 'Print'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -13
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = []
            Width = 76
          end
          item
            EditButtons = <>
            FieldName = 'DDBH'
            Footers = <>
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -13
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = []
            Width = 97
          end>
      end
    end
    object TabSheet3: TTabSheet
      Caption = 'Order List'
      ImageIndex = 2
      object Splitter1: TSplitter
        Left = 377
        Top = 0
        Height = 342
      end
      object DBGridEh3: TDBGridEh
        Left = 0
        Top = 0
        Width = 377
        Height = 342
        Align = alLeft
        DataSource = WPSS
        Flat = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -11
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        FooterRowCount = 1
        ParentFont = False
        SumList.Active = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -16
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        UseMultiTitle = True
        OnGetCellParams = DBGridEh3GetCellParams
        Columns = <
          item
            EditButtons = <>
            FieldName = 'DDBH'
            Footers = <>
            ReadOnly = True
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -13
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = []
            Width = 104
          end
          item
            EditButtons = <>
            FieldName = 'ARTICLE'
            Footers = <>
            ReadOnly = True
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -13
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = []
            Width = 105
          end
          item
            EditButtons = <>
            FieldName = 'ShipDate'
            Footers = <>
            ReadOnly = True
          end>
      end
      object DBGridEh4: TDBGridEh
        Left = 380
        Top = 0
        Width = 377
        Height = 342
        Align = alLeft
        DataSource = WPSSS
        Flat = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -11
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        FooterRowCount = 1
        ParentFont = False
        SumList.Active = True
        TabOrder = 1
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -16
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        UseMultiTitle = True
        OnGetCellParams = DBGridEh3GetCellParams
        Columns = <
          item
            EditButtons = <>
            FieldName = 'SIZE'
            Footers = <>
          end
          item
            EditButtons = <>
            FieldName = 'Quantity'
            Footers = <>
          end>
      end
    end
  end
  object Panel6: TPanel
    Left = 0
    Top = 65
    Width = 1061
    Height = 65
    Align = alTop
    TabOrder = 2
    Visible = False
    object Label17: TLabel
      Left = 646
      Top = 32
      Width = 64
      Height = 24
      Caption = 'Detail'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clLime
      Font.Height = -19
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
    object BD2: TBitBtn
      Left = 56
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Insert one New Record'
      Caption = 'Insert'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      OnClick = BD2Click
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BD3: TBitBtn
      Left = 104
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Delete one Record'
      Caption = 'Delete'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      OnClick = BD3Click
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BD4: TBitBtn
      Left = 152
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Modify'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      OnClick = BD4Click
      Glyph.Data = {
        
        50BB555555555555575F555555555555550B5555555555555575}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BD5: TBitBtn
      Left = 200
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Save Current'
      Caption = 'Save'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 3
      OnClick = BD5Click
      Glyph.Data = {
        
        5990555555555555577755555555555555555555555555555555}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BD6: TBitBtn
      Left = 248
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Cancel'
      Caption = 'Cancel'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 4
      OnClick = BD6Click
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BDT1: TBitBtn
      Left = 368
      Top = 8
      Width = 49
      Height = 49
      Caption = 'First'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 5
      Glyph.Data = {
        
        CCC333333333333777FF33333333333333CC3333333333333773}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BDT2: TBitBtn
      Left = 416
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Insert one New Record'
      Caption = 'Prior'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 6
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BDT3: TBitBtn
      Left = 464
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Delete one Record'
      Caption = 'Next'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 7
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BDT4: TBitBtn
      Left = 512
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Last'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 8
      Glyph.Data = {
        
        33333777333333333333CC333333333333337733333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BDT5: TBitBtn
      Left = 584
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Print'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 9
      OnClick = BDT5Click
      Glyph.Data = {
        
        33333337FFFF7733333333300000033333333337777773333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
  end
  object Panel4: TPanel
    Left = 0
    Top = 130
    Width = 1061
    Height = 65
    Align = alTop
    TabOrder = 3
    Visible = False
    object Label5: TLabel
      Left = 646
      Top = 32
      Width = 103
      Height = 24
      Caption = 'Order List'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clLime
      Font.Height = -19
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
    object BE2: TBitBtn
      Left = 56
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Insert one New Record'
      Caption = 'Insert'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      OnClick = BE2Click
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BE3: TBitBtn
      Left = 104
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Delete one Record'
      Caption = 'Delete'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      OnClick = BE3Click
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BE4: TBitBtn
      Left = 152
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Modify'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      OnClick = BE4Click
      Glyph.Data = {
        
        50BB555555555555575F555555555555550B5555555555555575}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BE5: TBitBtn
      Left = 200
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Save Current'
      Caption = 'Save'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 3
      OnClick = BE5Click
      Glyph.Data = {
        
        5990555555555555577755555555555555555555555555555555}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BE6: TBitBtn
      Left = 248
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Cancel'
      Caption = 'Cancel'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 4
      OnClick = BE6Click
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object ComputeBtn: TButton
      Left = 368
      Top = 8
      Width = 57
      Height = 49
      Caption = 'Comput'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 5
      OnClick = ComputeBtnClick
    end
  end
  object workplan: TQuery
    AfterOpen = workplanAfterOpen
    AfterScroll = workplanAfterScroll
    DatabaseName = 'DB'
    SQL.Strings = (
      'select *'
      
        '      ,(select SUM(OKQty) as OKQty from NHU_DD.dbo.WorkPlanS wor' +
        'kplans where workplans.WorkID=workplan.WorkID) as OKQty '
      'from NHU_DD.dbo.workplan workplan'
      
        'where convert(smalldatetime,convert(varchar,workStartDate,111)) ' +
        'between '
      '      '#39'2020/10/07'#39' and '#39'2020/10/14'#39
      ' and workplan.UserID='#39'21191'#39' '
      'order by workid desc')
    UpdateObject = upwp
    Left = 72
    Top = 360
    object workplanWorkID: TStringField
      FieldName = 'WorkID'
      Origin = 'DB.workplan.WorkID'
      FixedChar = True
      Size = 12
    end
    object workplanCQDH: TStringField
      FieldName = 'CQDH'
      Origin = 'DB.workplan.CQDH'
      FixedChar = True
      Size = 3
    end
    object workplanSPLB: TStringField
      FieldName = 'SPLB'
      FixedChar = True
      Size = 10
    end
    object workplanDDLB: TStringField
      FieldName = 'DDLB'
      FixedChar = True
      Size = 1
    end
    object workplanMJBH: TStringField
      FieldName = 'MJBH'
      FixedChar = True
    end
    object workplanmjname: TStringField
      FieldKind = fkLookup
      FieldName = 'mjname'
      LookupDataSet = MJZLQry
      LookupKeyFields = 'MJBH'
      LookupResultField = 'MJBH'
      KeyFields = 'MJBH'
      Size = 30
      Lookup = True
    end
    object workplanColorNo: TStringField
      FieldName = 'ColorNo'
      Origin = 'DB.workplan.ColorNo'
      FixedChar = True
      Size = 4
    end
    object workplancolorname: TStringField
      DisplayWidth = 100
      FieldKind = fkLookup
      FieldName = 'colorname'
      LookupDataSet = colorclassQry
      LookupKeyFields = 'ColorNo'
      LookupResultField = 'EnglishName'
      KeyFields = 'ColorNo'
      Size = 100
      Lookup = True
    end
    object workplanMemo: TStringField
      FieldName = 'Memo'
      FixedChar = True
      Size = 100
    end
    object workplanWorkQty: TIntegerField
      FieldName = 'WorkQty'
      Origin = 'DB.workplan.WorkQty'
    end
    object workplanWorkStartDate: TDateTimeField
      FieldName = 'WorkStartDate'
      Origin = 'DB.workplan.WorkStartDate'
    end
    object workplanWorkEndDate: TDateTimeField
      FieldName = 'WorkEndDate'
      Origin = 'DB.workplan.WorkEndDate'
    end
    object workplanUserID: TStringField
      FieldName = 'UserID'
      Origin = 'DB.workplan.UserID'
      FixedChar = True
    end
    object workplanCreatDate: TDateTimeField
      FieldName = 'CreatDate'
      Origin = 'DB.workplan.CreatDate'
    end
    object workplanUpdateDate: TDateTimeField
      FieldName = 'UpdateDate'
      Origin = 'DB.workplan.UpdateDate'
    end
    object workplanLastUser: TStringField
      FieldName = 'LastUser'
      Origin = 'DB.workplan.LastUser'
      FixedChar = True
    end
    object workplanYN: TStringField
      FieldName = 'YN'
      FixedChar = True
      Size = 1
    end
    object workplanOKQty: TIntegerField
      FieldName = 'OKQty'
    end
  end
  object upwp: TUpdateSQL
    ModifySQL.Strings = (
      'update LIY_DD.dbo.workplan'
      'set'
      '  WorkID = :WorkID,'
      '  CQDH = :CQDH,'
      '  DDLB = :DDLB,'
      '  MJBH = :MJBH,'
      '  ColorNo = :ColorNo,'
      '  Memo =:Memo,'
      '  WorkQty = :WorkQty,'
      '  WorkStartDate = :WorkStartDate,'
      '  WorkEndDate = :WorkEndDate,'
      '  UpdateDate = :UpdateDate,'
      '  LastUser = :LastUser'
      'where'
      '  WorkID = :OLD_WorkID')
    InsertSQL.Strings = (
      'insert into LIY_DD.dbo.workplan'
      
        '  (WorkID, CQDH, DDLB, MJBH, ColorNo, Memo, WorkQty, WorkStartDa' +
        'te, WorkEndDate, UserID,'
      '   CreatDate, UpdateDate, LastUser, YN)'
      'values'
      
        '  (:WorkID, :CQDH, :DDLB, :MJBH, :ColorNo, :Memo, :WorkQty, :Wor' +
        'kStartDate, :WorkEndDate,'
      '   :UserID, :CreatDate, :UpdateDate, :LastUser, :YN)')
    DeleteSQL.Strings = (
      'delete from LIY_DD.dbo.workplan'
      'where'
      '  WorkID = :OLD_WorkID ')
    Left = 72
    Top = 391
  end
  object WP: TDataSource
    DataSet = workplan
    Left = 105
    Top = 360
  end
  object WorkPlanS: TQuery
    AfterOpen = WorkPlanSAfterOpen
    DatabaseName = 'DB'
    DataSource = WP
    SQL.Strings = (
      'select * from LIY_DD.dbo.workplans'
      'where workID=:workid'
      'order by size,sno')
    UpdateObject = upwps
    Left = 192
    Top = 361
    ParamData = <
      item
        DataType = ftFixedChar
        Name = 'WorkID'
        ParamType = ptUnknown
        Size = 13
      end>
    object WorkPlanSWorkID: TStringField
      FieldName = 'WorkID'
      Origin = 'DB.workplans.WorkID'
      FixedChar = True
      Size = 12
    end
    object WorkPlanSDDBH: TStringField
      FieldName = 'DDBH'
      FixedChar = True
      Size = 18
    end
    object WorkPlanSSno: TIntegerField
      FieldName = 'Sno'
      Origin = 'DB.workplans.Sno'
    end
    object WorkPlanSSize: TStringField
      FieldName = 'Size'
      Origin = 'DB.workplans.Size'
      FixedChar = True
      Size = 6
    end
    object WorkPlanSPackage: TIntegerField
      FieldName = 'Package'
    end
    object WorkPlanSQty: TIntegerField
      FieldName = 'Qty'
      Origin = 'DB.workplans.Qty'
    end
    object WorkPlanSOKQTY: TIntegerField
      FieldName = 'OKQTY'
      Origin = 'DB.workplans.OKQTY'
    end
    object WorkPlanSOutQTY: TIntegerField
      FieldName = 'OutQTY'
    end
    object WorkPlanSNBarCode: TIntegerField
      FieldName = 'NBarCode'
      Origin = 'DB.workplans.NBarCode'
    end
    object WorkPlanSPbarcode: TIntegerField
      FieldName = 'Pbarcode'
      Origin = 'DB.workplans.Pbarcode'
    end
    object WorkPlanSUserID: TStringField
      FieldName = 'UserID'
      Origin = 'DB.workplans.UserID'
      FixedChar = True
    end
    object WorkPlanSCreatDate: TDateTimeField
      FieldName = 'CreatDate'
      Origin = 'DB.workplans.CreatDate'
    end
    object WorkPlanSUpdateDate: TDateTimeField
      FieldName = 'UpdateDate'
      Origin = 'DB.workplans.UpdateDate'
    end
    object WorkPlanSLastUser: TStringField
      FieldName = 'LastUser'
      Origin = 'DB.workplans.LastUser'
      FixedChar = True
    end
    object WorkPlanScodebar: TStringField
      FieldName = 'codebar'
      FixedChar = True
      Size = 22
    end
    object WorkPlanSYN: TStringField
      FieldName = 'YN'
      FixedChar = True
      Size = 1
    end
  end
  object upwps: TUpdateSQL
    ModifySQL.Strings = (
      'update LIY_DD.dbo.workplans'
      'set'
      '  WorkID = :WorkID,'
      '  SNO = :SNO,'
      '  Size = :Size,'
      '  DDBH = :DDBH,'
      '  Package =:Package,'
      '  Qty = :Qty,'
      '  OKQTY = :OKQTY,'
      '  NBarCode = :NBarCode,'
      '  Pbarcode = :Pbarcode,'
      '  UserID = :UserID,'
      '  CreatDate = :CreatDate,'
      '  UpdateDate = :UpdateDate,'
      '  LastUser = :LastUser,'
      '  codebar = :codebar'
      'where'
      '  WorkID = :OLD_WorkID and'
      '  DDBH =:OLD_DDBH and'
      '  Sno = :OLD_Sno and'
      '  Size = :OLD_Size')
    InsertSQL.Strings = (
      'insert into LIY_DD.dbo.workplans'
      
        '  (WorkID, DDBH, SNO, Size, Package, Qty, OKQTY, NBarCode, Pbarc' +
        'ode, UserID,'
      '   CreatDate, UpdateDate, LastUser, codebar, YN)'
      'values'
      
        '  (:WorkID, :DDBH, :SNO, :Size, :Package, :Qty, :OKQTY, :NBarCod' +
        'e, :Pbarcode,'
      '   :UserID, :CreatDate, :UpdateDate, :LastUser, :codebar, :YN)')
    DeleteSQL.Strings = (
      'delete from LIY_DD.dbo.workplans'
      'where'
      '  WorkID = :OLD_WorkID and'
      '  DDBH =:OLD_DDBH and'
      '  Sno = :OLD_Sno and'
      '  Size = :OLD_Size')
    Left = 192
    Top = 391
  end
  object WPS: TDataSource
    DataSet = WorkPlanS
    Left = 223
    Top = 361
  end
  object WorkPlanSS: TQuery
    AfterOpen = WorkPlanSSAfterOpen
    DatabaseName = 'DB'
    DataSource = WP
    SQL.Strings = (
      'select workplanss.*,DDZL.ARTICLE '
      'from LIY_DD.dbo.workplanss workplanss'
      'left join LIY_DD.dbo.DDZL DDZL on DDZL.DDBH=workplanss.DDBH'
      'where workid='#39'202010005'#39)
    UpdateObject = upwpss
    Left = 338
    Top = 361
    object WorkPlanSSWorkID: TStringField
      FieldName = 'WorkID'
      Origin = 'DB.workplanss.WorkID'
      FixedChar = True
      Size = 12
    end
    object WorkPlanSSDDBH: TStringField
      FieldName = 'DDBH'
      FixedChar = True
      Size = 18
    end
    object WorkPlanSSUserID: TStringField
      FieldName = 'UserID'
      Origin = 'DB.workplanss.UserID'
      FixedChar = True
    end
    object WorkPlanSSCreatDate: TDateTimeField
      FieldName = 'CreatDate'
      Origin = 'DB.workplanss.CreatDate'
    end
    object WorkPlanSSUpdateDate: TDateTimeField
      FieldName = 'UpdateDate'
      Origin = 'DB.workplanss.UpdateDate'
    end
    object WorkPlanSSLastUser: TStringField
      FieldName = 'LastUser'
      Origin = 'DB.workplanss.LastUser'
      FixedChar = True
    end
    object WorkPlanSSYN: TStringField
      FieldName = 'YN'
      FixedChar = True
      Size = 1
    end
    object WorkPlanSSARTICLE: TStringField
      FieldName = 'ARTICLE'
      FixedChar = True
    end
    object WorkPlanSSShipDate: TDateTimeField
      FieldName = 'ShipDate'
    end
  end
  object upwpss: TUpdateSQL
    ModifySQL.Strings = (
      'update LIY_DD.dbo.workplanss'
      'set'
      '  WorkID = :WorkID,'
      '  DDBh = :DDBh,'
      '  UserID = :UserID,'
      '  CreatDate = :CreatDate,'
      '  UpdateDate = :UpdateDate,'
      '  LastUser = :LastUser'
      'where'
      '  WorkID = :OLD_WorkID and'
      '  DDBh = :OLD_DDBh')
    InsertSQL.Strings = (
      'insert into LIY_DD.dbo.workplanss'
      '  (WorkID, DDBH, UserID, CreatDate, UpdateDate, LastUser, YN)'
      'values'
      
        '  (:WorkID, :DDBH, :UserID, :CreatDate, :UpdateDate, :LastUser, ' +
        ':YN)')
    DeleteSQL.Strings = (
      'delete from LIY_DD.dbo.workplanss'
      'where'
      '  WorkID = :OLD_WorkID and'
      '  DDBh = :OLD_DDBh')
    Left = 338
    Top = 391
  end
  object WPSS: TDataSource
    DataSet = WorkPlanSS
    Left = 367
    Top = 361
  end
  object TmpQuery: TQuery
    DatabaseName = 'DB'
    SQL.Strings = (
      'select bz2 as mjdh from LIY_DD.dbo.mjzl group by bz2')
    Left = 36
    Top = 361
  end
  object colorclassQry: TQuery
    DatabaseName = 'DB'
    SQL.Strings = (
      'select * from LIY_DD.dbo.colorclass')
    Left = 452
    Top = 361
    object colorclassQryColorNo: TStringField
      FieldName = 'ColorNo'
      Origin = 'DB.colorclass.ColorNo'
      FixedChar = True
      Size = 4
    end
    object colorclassQryEnglishName: TStringField
      FieldName = 'EnglishName'
      Origin = 'DB.colorclass.EnglishName'
      FixedChar = True
      Size = 50
    end
  end
  object MJZLQry: TQuery
    DatabaseName = 'DB'
    SQL.Strings = (
      'select bz2 as MJBH from LIY_DD.dbo.mjzl group by bz2')
    Left = 452
    Top = 393
  end
  object WorkPlanSSS: TQuery
    AfterOpen = WorkPlanSSAfterOpen
    DatabaseName = 'DB'
    DataSource = WPSS
    SQL.Strings = (
      'select workplansss.*'
      'from LIY_DD.dbo.workplansss workplansss'
      'where workid=:workid and DDBH = :DDBH'
      '')
    Left = 546
    Top = 361
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'workid'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DDBH'
        ParamType = ptUnknown
      end>
    object WorkPlanSSSWorkID: TStringField
      FieldName = 'WorkID'
      FixedChar = True
      Size = 12
    end
    object WorkPlanSSSDDBH: TStringField
      FieldName = 'DDBH'
      FixedChar = True
      Size = 18
    end
    object WorkPlanSSSSIZE: TStringField
      FieldName = 'SIZE'
      FixedChar = True
      Size = 6
    end
    object WorkPlanSSSQuantity: TIntegerField
      FieldName = 'Quantity'
    end
    object WorkPlanSSSOutQTY: TIntegerField
      FieldName = 'OutQTY'
    end
    object WorkPlanSSSUserID: TStringField
      FieldName = 'UserID'
      FixedChar = True
    end
    object WorkPlanSSSCreatDate: TDateTimeField
      FieldName = 'CreatDate'
    end
    object WorkPlanSSSInQTY: TIntegerField
      FieldName = 'InQTY'
    end
  end
  object WPSSS: TDataSource
    DataSet = WorkPlanSSS
    Left = 591
    Top = 361
  end
end
