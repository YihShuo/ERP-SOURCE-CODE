...
type
  TDeliver_EquipmentB = class(TForm)
    Panel1: TPanel;
    Label18: TLabel;
    BB2: TBitBtn;
    BB3: TBitBtn;
  ...
  private
    LB,AssetsN213_CheckPrint,AppDir:String;
    DepID_CD:String;
    DepName_CD:String;
...
var
  Deliver_EquipmentB: TDeliver_EquipmentB;
  NDate:TDate;

implementation

uses main1, EquipmentB_Dep1, EquipmentB1, Deliver_EquipmentB_print1,
     Deliver_EquipmentB_printTW1, Deliver_EquipmentB_printTT1;

{$R *.dfm}

procedure TDeliver_EquipmentB.ReadIni();
var MyIni :Tinifile;
...
begin
  ...
  begin
    try
      ...
      AssetsN213_CheckPrint:=MyIni.ReadString('ERP','AssetsN213_CheckPrint','TX');
...
//
procedure TDeliver_EquipmentB.Init_LB();
begin
  LB:='BBBBBBBBBB';
end;
//異動單 Column Show or Not
procedure TDeliver_EquipmentB.DelDetAfterOpen_ColumnShow();
...
begin
  ...
  DBGrid2.Columns[7].Title.Caption:='報廢|Bao Phe';
  DBGrid2.Columns[10].Visible:=true;
  DBGrid2.Columns[10].Title.Caption:='報廢類別|Loai Bao Phe';
  DBGrid2.Columns[13].Visible:=true;
  DBGrid2.Columns[13].Title.Caption:='報廢原因|Nguyen nhan';
  BB_GBox.Visible:=true;
  BB_GBox.Left:=GroupBox_Left;
  //
end;
//
procedure TDeliver_EquipmentB.DelDet_LB_Edit();
begin
  with DelDet do
  begin
    Edit;
    ...
    FieldByName('PARAMENT1').Value:='Status';
    FieldByName('Value1').Value:='B';
    FieldByName('COLUMN1').Value:='Bao Phe';
    Post;
  end;
end;
procedure TDeliver_EquipmentB.FormCreate(Sender: TObject);
var i:integer;
begin
  with Qtemp do
  begin
...
procedure TDeliver_EquipmentB.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if BB5.enabled  then
  begin
...
procedure TDeliver_EquipmentB.FormDestroy(Sender: TObject);
begin
  Deliver_EquipmentB:=nil;
end;

procedure TDeliver_EquipmentB.Button1Click(Sender: TObject);
begin
  Init_LB();
  with DelMas do
  begin
    active:=false;
    sql.Clear;
    sql.add('select TSCD_KCBB.*,BDepartment.DepName  ');
    sql.add('from TSCD_KCBB');
    sql.add('left join TSCD_BDepartment BDepartment on TSCD_KCBB.DepID=BDepartment.ID');
    sql.add('where ISNULL(flowflag,'''')<>''X'' and TSCD_KCBB.LLNO like ');
    sql.add(''''+edit1.Text+'%'+'''');
    ...
    sql.add('and convert(smalldatetime,convert(varchar,TSCD_KCBB.USERDATE,111)) between ');
    sql.add(''''+formatdatetime('yyyy/MM/dd',DTP1.Date) +'''');
    sql.add(' and  ');
    sql.add(''''+formatdatetime('yyyy/MM/dd',DTP2.Date) +'''');
    sql.add('and TSCD_KCBB.LB='''+LB+''' ');
    if Check.Checked then
    begin
      sql.Add('and TSCD_KCBB.USERID='+''''+main.Edit1.Text+'''');
    end;
    sql.add('and TSCD_KCBB.LB='''+LB+''' ');
    SQL.Add('and TSCD_KCBB.GSBH='''+main.Edit2.Text+''' ');
...
procedure TDeliver_EquipmentB.BBTT1Click(Sender: TObject);
begin
  EquipmentB_Dep:=TEquipmentB_Dep.Create(self);
  EquipmentB_Dep.Show;
end;


procedure TDeliver_EquipmentB.BB1Click(Sender: TObject);
begin
   panel2.Visible:=true;
end;

procedure TDeliver_EquipmentB.BB2Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive:=true;
...
procedure TDeliver_EquipmentB.BB4Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive:=true;
...
procedure TDeliver_EquipmentB.BB5Click(Sender: TObject);
var y,m,a:string;
    i:integer;
begin
  with Qtemp do
                ...
                DelMas.delete;
                 ...
                 begin
                     ...
                     begin
                       active:=false;
                       sql.Clear;
                       sql.Add('select LLNO from TSCD_KCBB where LLNO like '+''''+y+m+'%'+'''');
                   ...
                   if DelMas.fieldbyname('YN').value='0'then
                     begin
                         ...
                         begin
                           ...
                           sql.add('values ('+''''+'TSCD_KCBB'+''''+','+''''+DelMas.fieldbyname('LLNO').Value+'''');
                           sql.add(','+''''+' '+''''+','+''''+DelMas.fieldbyname('USERID').Value+''''+',');
                           sql.add(''''+main.Edit1.Text+''''+',getdate())'); }
                           SQL.Add('Update TSCD_KCBB Set flowflag=''X'' where LLNO='''+DelMas.FieldByName('LLNO').AsString+''' ');
                           execsql;
                           active:=false;
                         end;
...
procedure TDeliver_EquipmentB.BD2Click(Sender: TObject);
begin
...
procedure TDeliver_EquipmentB.DBGrid2EditButtonClick(Sender: TObject);
var Style: String;
    i:integer;
begin
...
procedure TDeliver_EquipmentB.BD3Click(Sender: TObject);
begin
 if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TDeliver_EquipmentB.BD4Click(Sender: TObject);
begin
 if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TDeliver_EquipmentB.BB3Click(Sender: TObject);
begin
...
procedure TDeliver_EquipmentB.BB6Click(Sender: TObject);
begin
  with DelMas do
  begin
    active:=false;
...
procedure TDeliver_EquipmentB.bbt6Click(Sender: TObject);
...
procedure TDeliver_EquipmentB.BD6Click(Sender: TObject);
begin
  with DelDet do
  begin
    active:=false;
...
procedure TDeliver_EquipmentB.PC1Change(Sender: TObject);
var i:integer;
begin
...
procedure TDeliver_EquipmentB.DelDetAfterOpen(DataSet: TDataSet);
var i:integer;
begin
  ...
  //BDT5.Enabled:=true;
  BDT6.Enabled:=true;
  if AssetsN213_CheckPrint='TX' then
  begin
     BDT7.Enabled:=true;
     BDT8.Enabled:=false;
  end else
...
//
Function TDeliver_EquipmentB.DelDet_LB_Check():boolean;
var iRes:boolean;
begin
  iRes:=true;
  ...
  with DelDet do
  begin
    if DelMas.fieldbyname('LB').AsString ='BBBBBBBBBB' then  //報廢 Bao Phe
    begin
      //
    end;
  end;
...
procedure TDeliver_EquipmentB.BD5Click(Sender: TObject);
var i:integer;
begin
                 ...
                 begin
                    ...
                    begin
                       ...
                       begin
                         ...
                         sql.add('insert into BDelRec ');
                         sql.add('(TableName ,TNO,CLBH,OldID,DelID,DelDate)');
                         sql.add('values ('+''''+'TSCD_KCBBS'+''''+','+''''+DelMas.fieldbyname('LLNO').Value+'''');
                         sql.add(','+''''+' '+''''+','+''''+DelMas.fieldbyname('USERID').Value+''''+',');
                         sql.add(''''+main.Edit1.Text+''''+',getdate())');
                         execsql;
                         active:=false;
...
procedure TDeliver_EquipmentB.BBTT2Click(Sender: TObject);
begin
  EquipmentB:=TEquipmentB.Create(self);
  EquipmentB.Show;
end;

procedure TDeliver_EquipmentB.DBGrid1GetCellParams(Sender: TObject;
...
procedure TDeliver_EquipmentB.DBGrid2GetCellParams(Sender: TObject;
  Column: TColumnEh; AFont: TFont; var Background: TColor;
  State: TGridDrawState);
begin
  if DelDet.FieldByName('YN').value='0' then
...
procedure TDeliver_EquipmentB.DBGrid1DblClick(Sender: TObject);
begin
  if  DelMas.Active then
  begin
...
procedure TDeliver_EquipmentB.DBGrid1EditButtonClick(Sender: TObject);
begin
...
procedure TDeliver_EquipmentB.BDT5Click(Sender: TObject);
begin
  ...
  if DelDet.RecordCount = 0  then
  begin
    showmessage('Pls insert the Discard Equipment first');
  end else
  begin
    Deliver_EquipmentB_print:=TDeliver_EquipmentB_print.Create(self);
    Deliver_EquipmentB_print.quickrep1.prepare;
    Deliver_EquipmentB_print.quickrep1.preview;
    Deliver_EquipmentB_print.free;
  end;
end;

procedure TDeliver_EquipmentB.BDT6Click(Sender: TObject);
var eclApp,WorkBook:olevariant;
    i,j:integer;
begin
    ...
    try
        ...
        while   not   DelDet.Eof   do
          begin
            for   i:=0   to  DBGrid2.Columns.Count-1  do
            begin
              eclApp.Cells(j,i+1):=DBGrid2.Fields[i].AsString;
            end;
          DelDet.Next;
          inc(j);
          end;
...
procedure TDeliver_EquipmentB.BDT7Click(Sender: TObject);
begin
  if DelDet.RequestLive then
  begin
    showmessage('Pls save Master data first.');
  ...
  begin
    Deliver_EquipmentB_printTW:=TDeliver_EquipmentB_printTW.Create(self);
    with Deliver_EquipmentB.DelDet do
    begin
      First;
      while Not Eof do
      begin
        if Deliver_EquipmentB.DelDet.FieldByName('COLUMN6').Value='1' then
          Deliver_EquipmentB_printTW.LB3.Caption:='V';
        if Deliver_EquipmentB.DelDet.FieldByName('COLUMN6').Value='2' then
          Deliver_EquipmentB_printTW.LB5.Caption:='V';
        Next;
      end;
    end;
    Deliver_EquipmentB_printTW.quickrep1.prepare;
    Deliver_EquipmentB_printTW.quickrep1.preview;
    Deliver_EquipmentB_printTW.free;
  end;

end;

procedure TDeliver_EquipmentB.BDT8Click(Sender: TObject);
begin
  if DelDet.RequestLive then
  begin
    showmessage('Pls save Master data first.');
  ...
  begin
    Deliver_EquipmentB_printTT:=TDeliver_EquipmentB_printTT.Create(self);
    with Deliver_EquipmentB.DelDet do
    begin
      First;
      while Not Eof do
      begin
        if Deliver_EquipmentB.DelDet.FieldByName('COLUMN6').Value='1' then
          Deliver_EquipmentB_printTT.LB3.Caption:='V';
        if Deliver_EquipmentB.DelDet.FieldByName('COLUMN6').Value='2' then
          Deliver_EquipmentB_printTT.LB5.Caption:='V';
        Next;
      end;
    end;
    Deliver_EquipmentB_printTT.quickrep1.prepare;
    Deliver_EquipmentB_printTT.quickrep1.preview;
    Deliver_EquipmentB_printTT.free;
  end;
end;
