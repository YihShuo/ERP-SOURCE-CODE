...
type
  TEquipmentConfirm_FF = class(TForm)
    Panel1: TPanel;
    Label1: TLabel;
    Label2: TLabel;
    Button1: TButton;
...
var
  EquipmentConfirm_FF: TEquipmentConfirm_FF;
  NDate:TDate;
...
procedure TEquipmentConfirm_FF.DBGrid2ColumnShow();
var i:integer;
begin
  //
  for i:=5 to 22 do
  ...
  //
  with EquipmentConfirm_FF.DelMas do
  begin
...
procedure TEquipmentConfirm_FF.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TEquipmentConfirm_FF.FormDestroy(Sender: TObject);
begin
  EquipmentConfirm_FF:=nil;
end;

procedure TEquipmentConfirm_FF.FormCreate(Sender: TObject);
var i: integer;
begin
  with Qtemp do
  begin
...
procedure TEquipmentConfirm_FF.Button1Click(Sender: TObject);
begin
  with DelMas do
  begin
    DelDet.Active:=false;
    ...
    Active:=false;
    sql.Clear;
    sql.add('select TSCD_KCFF.*,BDepartment.DepName,CAST(dbo.fSignsToNoSigns(TSCD_LB.YWSM) AS VARCHAR(200)) as YWSM,TSCD_LB.ZWSM  ');
    sql.add('from TSCD_KCFF ');
    sql.add('left join TSCD_BDepartment BDepartment on TSCD_KCFF.DepID=BDepartment.ID  ');
    sql.Add('left join TSCD_LB on TSCD_LB.LB=TSCD_KCFF.LB ');
    sql.add('where 1=1 and ISNULL(flowflag,'''')<>''X'' ');
    if CheckBox1.Checked=false then
    SQL.Add(' and TSCD_KCFF.CFMID=''NO'' ');
    sql.add('and convert(smalldatetime,convert(varchar,TSCD_KCFF.USERDATE,111)) between'+''''+formatdatetime('yyyy/MM/dd',DTP1.date )+'''');
    sql.add('and '''+formatdatetime('yyyy/MM/dd',DTP2.date )+'''');
    sql.add('and TSCD_KCFF.LLNO like '+''''+edit1.Text+'%'+'''');
    //sql.add('and BDepartment.GSBH='+''''+main.edit2.text+'''');
    sql.add('order by TSCD_KCFF.LLNO  ');
...
procedure TEquipmentConfirm_FF.Button2Click(Sender: TObject);
var i:integer;
begin
...
procedure TEquipmentConfirm_FF.DelDetAfterOpen(DataSet: TDataSet);
begin
  DBGrid2ColumnShow();
end;
