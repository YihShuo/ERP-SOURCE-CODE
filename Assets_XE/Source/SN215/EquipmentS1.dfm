object EquipmentS: TEquipmentS
  ...
  object DBGridEh1: TDBGridEh
    ...
    Columns = <
      ...
      item
        ...
        FieldName = 'LABEL_Tag'
        Footers = <>
        Title.Caption = #32232#34399'|LABEL_Tag'
      end>
  ...
  object Query1: TQuery
    ...
    SQL.Strings = (
        ...
        '       TSCD_SB.LSBH,TSCD_SB.QUCBH,TSCD_SB.AuthCE,IsNull(TSCD_SB.' +
        'YWPM,CLZL.YWPM) as YWPM,IsNull(TSCD_SB.ZWPM,CLZL.ZWPM) as ZWPM,B' +
        'dep_appy.DepName as applyDepname,TSCD.LABEL_Tag '
      'from TSCD '
      'left join TSCD_SB on TSCD_SB.SBBH=TSCD.SBBH '
      'left join CLZL on CLZL.cldh=TSCD.SBBH'
      
        'left join TSCD_BDepartment Bdep_loca on Bdep_loca.ID=TSCD.DepID ' +
        ' '
      'left join TSCD_ZSZL on TSCD_ZSZL.ZSDH=TSCD_SB.ZSDH '
      'left join TSCD_KCRK on TSCD_KCRK.RKNO=TSCD.RKNO '
      
        'left join TSCD_BDepartment Bdep_appy on Bdep_appy.ID=TSCD_KCRK.D' +
        'epID '
      'left join TSCD_BDepartment JDBDep on JDBDep.ID=TSCD.JDTS '
      'where TSCD.SBBH like '#39'%'#39
      'and TSCD.TSBH like '#39'21191%'#39
      ' and TSCD.Status in ('#39'Y'#39') '
      'order by TSID')
    Left = 232
    Top = 192
