...
type
  TDeliver_EquipmentS = class(TForm)
    Panel1: TPanel;
    Label18: TLabel;
    BB2: TBitBtn;
    BB3: TBitBtn;
...
var
  Deliver_EquipmentS: TDeliver_EquipmentS;
  NDate:TDate;

implementation

uses main1, EquipmentS_Dep1, EquipmentS_DepName1, EquipmentS1, Deliver_EquipmentS_print1,
  Deliver_EquipmentS_PrintTW1;

{$R *.dfm}
//
procedure TDeliver_EquipmentS.Init_LB();
begin
  LB:='SSSSSSSSSS';
end;
//異動單 Column Show or Not
procedure TDeliver_EquipmentS.DelDetAfterOpen_ColumnShow();
...
begin
  ...
  DBGrid2.Columns[10].Title.Caption:='異動原因|Nguyen do';
  SS_GBox.Visible:=true;
  SS_GBox.Left:=GroupBox_Left;
  //
end;
//
procedure TDeliver_EquipmentS.DelDet_LB_Edit();
begin
  with DelDet do
  begin
    Edit;
    ...
    //FieldByName('Value1').Value:=DelMas.fieldbyname('DepID').AsString;
    //FieldByName('COLUMN1').Value:=DelMas.fieldbyname('DepName').AsString;
    Post;
  end;
end;
procedure TDeliver_EquipmentS.FormCreate(Sender: TObject);
var i:integer;
begin
  with Qtemp do
  begin
...
procedure TDeliver_EquipmentS.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if BB5.enabled  then
  begin
...
procedure TDeliver_EquipmentS.FormDestroy(Sender: TObject);
begin
  Deliver_EquipmentS:=nil;
end;

procedure TDeliver_EquipmentS.Button1Click(Sender: TObject);
begin
  Init_LB();
  with DelMas do
  begin
    ...
    sql.add('and convert(smalldatetime,convert(varchar,TSCD_KCSS.USERDATE,111)) between ');
    sql.add(''''+formatdatetime('yyyy/MM/dd',DTP1.Date) +'''');
    sql.add(' and  ');
    sql.add(''''+formatdatetime('yyyy/MM/dd',DTP2.Date) +'''');
    sql.add('and TSCD_KCSS.LB='''+LB+''' ');
    if Check.Checked then
    begin
      sql.Add('and TSCD_KCSS.USERID='+''''+main.Edit1.Text+'''');
    end;
    sql.add('and TSCD_KCSS.LB='''+LB+''' ');
    SQL.Add('and TSCD_KCSS.GSBH='''+main.Edit2.Text+''' ');
...
procedure TDeliver_EquipmentS.BBTT1Click(Sender: TObject);
begin
  EquipmentS_Dep:=TEquipmentS_Dep.Create(self);
  EquipmentS_Dep.Show;
end;


procedure TDeliver_EquipmentS.BB1Click(Sender: TObject);
begin
   panel2.Visible:=true;
end;

procedure TDeliver_EquipmentS.BB2Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive:=true;
...
procedure TDeliver_EquipmentS.BB4Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive:=true;
...
procedure TDeliver_EquipmentS.BB5Click(Sender: TObject);
var y,m,a:string;
    i:integer;
begin
  with Qtemp do
                ...
                DelMas.delete;
                 ...
                 begin
                     ...
                     begin
                       active:=false;
                       sql.Clear;
                       sql.Add('select LLNO from TSCD_KCSS where LLNO like '+''''+y+m+'%'+'''');
                   ...
                   if DelMas.fieldbyname('YN').value='0'then
                     begin
                         ...
                         begin
                           ...
                           sql.add('values ('+''''+'TSCD_KCSS'+''''+','+''''+DelMas.fieldbyname('LLNO').Value+'''');
                           sql.add(','+''''+' '+''''+','+''''+DelMas.fieldbyname('USERID').Value+''''+',');
                           sql.add(''''+main.Edit1.Text+''''+',getdate())'); }
                           SQL.Add('Update TSCD_KCSS Set flowflag=''X'' where LLNO='''+DelMas.FieldByName('LLNO').AsString+''' ');
                           execsql;
                           active:=false;
                         end;
...
procedure TDeliver_EquipmentS.BD2Click(Sender: TObject);
begin
...
procedure TDeliver_EquipmentS.DBGrid2EditButtonClick(Sender: TObject);
var Style: String;
    i:integer;
begin
  ...
  begin
    EquipmentS_DepName:=TEquipmentS_DepName.Create(self);
    EquipmentS_DepName.COLUMN_Index:=1;
    EquipmentS_DepName.Show;
  end;
end;

procedure TDeliver_EquipmentS.BD3Click(Sender: TObject);
begin
 if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TDeliver_EquipmentS.BD4Click(Sender: TObject);
begin
 if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TDeliver_EquipmentS.BB3Click(Sender: TObject);
begin
...
procedure TDeliver_EquipmentS.BB6Click(Sender: TObject);
begin
  with DelMas do
  begin
    active:=false;
...
procedure TDeliver_EquipmentS.BD6Click(Sender: TObject);
begin
  with DelDet do
  begin
    active:=false;
...
procedure TDeliver_EquipmentS.PC1Change(Sender: TObject);
var i:integer;
begin
...
procedure TDeliver_EquipmentS.DelDetAfterOpen(DataSet: TDataSet);
var i:integer;
begin
...
//
Function TDeliver_EquipmentS.DelDet_LB_Check():boolean;
var iRes:boolean;
begin
  iRes:=true;
  with DelDet do
  begin
    if DelMas.fieldbyname('LB').AsString ='SSSSSSSSS' then  //換位置 chuyen vi tri
    begin
      if ((FieldByName('VALUE1').Isnull) or (FieldByName('VALUE2').Isnull) ) then
      begin
        iRes:=false;
      end;
    end;
...
procedure TDeliver_EquipmentS.BD5Click(Sender: TObject);
var i:integer;
    depid, dep, memo, reason: string;
begin
    ...
    try
    DelDet.first;
    depid:=DelDet.fieldbyname('Value1').AsString;
    dep:=DelDet.fieldbyname('COLUMN1').AsString;
    memo:=DelDet.fieldbyname('VALUE2').AsString;
    reason:=DelDet.fieldbyname('COLUMN2').AsString;
              ...
              begin
                ...
                begin
                  ...
                  begin
                      DelDet_LB_Edit();
                      DelDet.edit;
                      if DelDet.fieldbyname('Value1').AsString='' then
                      begin
                        DelDet.FieldByName('Value1').Value:=depid;
                        DelDet.FieldByName('COLUMN1').Value:=dep;
                        if depid <> 'VP01' then DelDet.FieldByName('VALUE2').Value:=memo;
                        DelDet.FieldByName('COLUMN2').Value:=reason;
                      end;
                 ...
                 begin
                    ...
                    begin
                       ...
                       begin
                         ...
                         sql.add('insert into BDelRec ');
                         sql.add('(TableName ,TNO,CLBH,OldID,DelID,DelDate)');
                         sql.add('values ('+''''+'TSCD_KCSSS'+''''+','+''''+DelMas.fieldbyname('LLNO').Value+'''');
                         sql.add(','+''''+' '+''''+','+''''+DelMas.fieldbyname('USERID').Value+''''+',');
                         sql.add(''''+main.Edit1.Text+''''+',getdate())');
                         execsql;
                         active:=false;
...
procedure TDeliver_EquipmentS.BBTT2Click(Sender: TObject);
begin
  EquipmentS:=TEquipmentS.Create(self);
  EquipmentS.Show;
end;

procedure TDeliver_EquipmentS.DBGrid1GetCellParams(Sender: TObject;
...
procedure TDeliver_EquipmentS.DBGrid2GetCellParams(Sender: TObject;
  Column: TColumnEh; AFont: TFont; var Background: TColor;
  State: TGridDrawState);
begin
  if DelDet.FieldByName('YN').value='0' then
...
procedure TDeliver_EquipmentS.DBGrid1DblClick(Sender: TObject);
begin
...
procedure TDeliver_EquipmentS.DBGrid1EditButtonClick(Sender: TObject);
begin
...
procedure TDeliver_EquipmentS.BDT5Click(Sender: TObject);
begin
  ...
  Deliver_EquipmentS_print:=TDeliver_EquipmentS_print.Create(self);
  Deliver_EquipmentS_print.quickrep1.prepare;
  Deliver_EquipmentS_print.quickrep1.preview;
  Deliver_EquipmentS_print.free;
end;

procedure TDeliver_EquipmentS.BitBtn1Click(Sender: TObject);
begin
  if DelDet.RequestLive then
  begin
    showmessage('Pls save Master data first.');
  ...
  Deliver_EquipmentS_PrintTW:=TDeliver_EquipmentS_PrintTW.Create(self);
  Deliver_EquipmentS_PrintTW.quickrep1.prepare;
  Deliver_EquipmentS_PrintTW.quickrep1.preview;
  Deliver_EquipmentS_PrintTW.free;
end;
