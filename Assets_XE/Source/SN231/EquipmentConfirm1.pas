...
var
  EquipmentConfirm: TEquipmentConfirm;
  NDate:TDate;
...
procedure TEquipmentConfirm.DBGrid2ColumnShow();
var i:integer;
begin
  //
  for i:=5 to 22 do
  ...
  //
  with EquipmentConfirm.DelMas do
  begin
...
procedure TEquipmentConfirm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TEquipmentConfirm.FormDestroy(Sender: TObject);
begin
  EquipmentConfirm:=nil;
end;

procedure TEquipmentConfirm.FormCreate(Sender: TObject);
var i: integer;
begin
  with Qtemp do
  begin
...
procedure TEquipmentConfirm.Button1Click(Sender: TObject);
begin
  with DelMas do
  begin
    DelDet.Active:=false;
    ...
    Active:=false;
    sql.Clear;
    sql.add('select TSCD_KCLL.*,BDepartment.DepName,CAST(dbo.fSignsToNoSigns(TSCD_LB.YWSM) AS VARCHAR(200)) as YWSM,TSCD_LB.ZWSM  ');
    sql.add('from TSCD_KCLL ');
    sql.add('left join TSCD_BDepartment BDepartment on TSCD_KCLL.DepID=BDepartment.ID  ');
    sql.Add('left join TSCD_LB on TSCD_LB.LB=TSCD_KCLL.LB ');
    sql.add('where 1=1 and ISNULL(flowflag,'''')<>''X'' and TSCD_KCLL.LB=''LLLLLLLLLL'' ');
    if CheckBox1.Checked=false then
    SQL.Add(' and TSCD_KCLL.CFMID=''NO'' ');
...
procedure TEquipmentConfirm.Button2Click(Sender: TObject);
var i:integer;
begin
...
procedure TEquipmentConfirm.DelDetAfterOpen(DataSet: TDataSet);
begin
  DBGrid2ColumnShow();
end;
