...
type
  TEquipmentConfirm_CC = class(TForm)
    Panel1: TPanel;
    Label1: TLabel;
    Label2: TLabel;
    Button1: TButton;
...
var
  EquipmentConfirm_CC: TEquipmentConfirm_CC;
  NDate:TDate;
...
procedure TEquipmentConfirm_CC.DBGrid2ColumnShow();
var i:integer;
begin
  //
  for i:=5 to 22 do
  ...
  //
  with EquipmentConfirm_CC.DelMas do
  begin
...
procedure TEquipmentConfirm_CC.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TEquipmentConfirm_CC.FormDestroy(Sender: TObject);
begin
  EquipmentConfirm_CC:=nil;
end;

procedure TEquipmentConfirm_CC.FormCreate(Sender: TObject);
var i: integer;
begin
  with Qtemp do
  begin
...
procedure TEquipmentConfirm_CC.Button1Click(Sender: TObject);
begin
  with DelMas do
  begin
    DelDet.Active:=false;
    ...
    Active:=false;
    sql.Clear;
    sql.add('select TSCD_KCCC.*,BDepartment.DepName,CAST(dbo.fSignsToNoSigns(TSCD_LB.YWSM) AS VARCHAR(200)) as YWSM,TSCD_LB.ZWSM  ');
    sql.add('from TSCD_KCCC ');
    sql.add('left join TSCD_BDepartment BDepartment on TSCD_KCCC.DepID=BDepartment.ID  ');
    sql.Add('left join TSCD_LB on TSCD_LB.LB=TSCD_KCCC.LB ');
    sql.add('where 1=1 and ISNULL(flowflag,'''')<>''X'' ');
    if CheckBox1.Checked=false then
    SQL.Add(' and TSCD_KCCC.CFMID=''NO'' ');
    sql.add('and convert(smalldatetime,convert(varchar,TSCD_KCCC.USERDATE,111)) between'+''''+formatdatetime('yyyy/MM/dd',DTP1.date )+'''');
    sql.add('and '''+formatdatetime('yyyy/MM/dd',DTP2.date )+'''');
    sql.add('and TSCD_KCCC.LLNO like '+''''+edit1.Text+'%'+'''');
    //sql.add('and BDepartment.GSBH='+''''+main.edit2.text+'''');
    sql.add('order by TSCD_KCCC.LLNO  ');
...
procedure TEquipmentConfirm_CC.Button2Click(Sender: TObject);
var i:integer;
begin
...
procedure TEquipmentConfirm_CC.DelDetAfterOpen(DataSet: TDataSet);
begin
  DBGrid2ColumnShow();
end;
