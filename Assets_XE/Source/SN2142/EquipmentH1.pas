...
var
  EquipmentH: TEquipmentH;

implementation

uses Deliver_EquipmentH1, main1, FunUnit;

{$R *.dfm}


procedure TEquipmentH.DBGridEh1DblClick(Sender: TObject);
...
begin
  ...
  begin
    ...
    begin
      ...
      //20230217   修改帶入借出組別單位
      if Deliver_EquipmentH.DelMas.FieldByName('Memo').AsString<>'' then
      begin
        with Qtemp do
        begin
          Active:=false;
          ...
          SQL.Add('from TSCD_KCMM TSCD_KCLL');
          SQL.Add('Left join TSCD_BDepartment on TSCD_BDepartment.ID=TSCD_KCLL.DepID ');
          SQL.Add('where TSCD_KCLL.LLNO='''+Deliver_EquipmentH.DelMas.FieldByName('Memo').AsString+''' --and TSCD_KCLl.LB=''MMMMMMMMMM'' ');
      ...
      begin
        FieldByName('Value1').Value:=Deliver_EquipmentH.DepID_CD;
        FieldByName('COLUMN1').Value:=Deliver_EquipmentH.DepName_CD;
        FieldByName('Value2').Value:=Deliver_EquipmentH.DepName_CD;
        FieldByName('COLUMN2').Value:=Deliver_EquipmentH.DepName_CD;
      end;
...
procedure TEquipmentH.Button1Click(Sender: TObject);
var SubSQL:String;
begin
  //
  with query1 do
  begin
    ...
    if LLNOEdit.Text<>'' then
      sql.Add('and TSCD.TSID in (select TSID from TSCD_KCMMS where LLNO='''+LLNOEdit.Text+''') ');
...
procedure TEquipmentH.FormDestroy(Sender: TObject);
begin
  EquipmentH:=nil;
end;

procedure TEquipmentH.FormClose(Sender: TObject;
...
procedure TEquipmentH.Button2Click(Sender: TObject);
var i,j:integer;
    bm:Tbookmark;
