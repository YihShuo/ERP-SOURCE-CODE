...
type
  TDeliver_EquipmentH = class(TForm)
    Panel1: TPanel;
    Label18: TLabel;
    BB2: TBitBtn;
    BB3: TBitBtn;
...
var
  Deliver_EquipmentH: TDeliver_EquipmentH;
  NDate:TDate;

implementation

uses main1, EquipmentH_Dep1, EquipmentH1, EquipmentH_DepName1, Deliver_EquipmentH_print1, Deliver_EquipmentH_printTW1;

{$R *.dfm}
//
procedure TDeliver_EquipmentH.Init_LB();
begin
  LB:='HHHHHHHHHH';
end;
//異動單 Column Show or Not
procedure TDeliver_EquipmentH.DelDetAfterOpen_ColumnShow();
...
//
procedure TDeliver_EquipmentH.DelDet_LB_Edit();
begin
  with DelDet do
  begin
    Edit;
    ...
    FieldByName('Value3').Value:='H';
    Post;
  end;
end;
procedure TDeliver_EquipmentH.FormCreate(Sender: TObject);
var i:integer;
begin
  with Qtemp do
  begin
...
procedure TDeliver_EquipmentH.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if BB5.enabled  then
  begin
...
procedure TDeliver_EquipmentH.FormDestroy(Sender: TObject);
begin
  Deliver_EquipmentH:=nil;
end;

procedure TDeliver_EquipmentH.Button1Click(Sender: TObject);
begin
  Init_LB();
  with DelMas do
  begin
    active:=false;
    sql.Clear;
    sql.add('select TSCD_KCHH.*,BDepartment.DepName  ');
    sql.add('from TSCD_KCHH');
    sql.add('left join TSCD_BDepartment BDepartment on TSCD_KCHH.DepID=BDepartment.ID');
    sql.add('where ISNULL(flowflag,'''')<>''X'' and TSCD_KCHH.LLNO like ');
    sql.add(''''+edit1.Text+'%'+'''');
    ...
    sql.add('and convert(smalldatetime,convert(varchar,TSCD_KCHH.USERDATE,111)) between ');
    sql.add(''''+formatdatetime('yyyy/MM/dd',DTP1.Date) +'''');
    sql.add(' and  ');
    sql.add(''''+formatdatetime('yyyy/MM/dd',DTP2.Date) +'''');
    sql.add('and TSCD_KCHH.LB='''+LB+''' ');
    if Check.Checked then
    begin
      sql.Add('and TSCD_KCHH.USERID='+''''+main.Edit1.Text+'''');
    end;
    sql.add('and TSCD_KCHH.LB='''+LB+''' ');
    SQL.Add('and TSCD_KCHH.GSBH='''+main.Edit2.Text+''' ');
...
procedure TDeliver_EquipmentH.BBTT1Click(Sender: TObject);
begin
  EquipmentH_Dep:=TEquipmentH_Dep.Create(self);
  EquipmentH_Dep.Show;
end;

procedure TDeliver_EquipmentH.BB1Click(Sender: TObject);
begin
   panel2.Visible:=true;
end;

procedure TDeliver_EquipmentH.BB2Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive:=true;
...
procedure TDeliver_EquipmentH.BB4Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive:=true;
...
procedure TDeliver_EquipmentH.BB5Click(Sender: TObject);
var y,m,a:string;
    i:integer;
begin
  with Qtemp do
                ...
                DelMas.delete;
                 ...
                 begin
                     ...
                     begin
                       active:=false;
                       sql.Clear;
                       sql.Add('select LLNO from TSCD_KCHH where LLNO like '+''''+y+m+'%'+'''');
                   ...
                   if DelMas.fieldbyname('YN').value='0'then
                     begin
                         ...
                         begin
                           ...
                           sql.add('values ('+''''+'TSCD_KCHH'+''''+','+''''+DelMas.fieldbyname('LLNO').Value+'''');
                           sql.add(','+''''+' '+''''+','+''''+DelMas.fieldbyname('USERID').Value+''''+',');
                           sql.add(''''+main.Edit1.Text+''''+',getdate())'); }
                           SQL.Add('Update TSCD_KCHH Set flowflag=''X'' where LLNO='''+DelMas.FieldByName('LLNO').AsString+''' ');
                           execsql;
                           active:=false;
                         end;
...
procedure TDeliver_EquipmentH.BD2Click(Sender: TObject);
begin
...
procedure TDeliver_EquipmentH.DBGrid2EditButtonClick(Sender: TObject);
var Style: String;
    i:integer;
begin
  ...
  begin
    EquipmentH_DepName:=TEquipmentH_DepName.Create(self);
    EquipmentH_DepName.COLUMN_Index:=1;
    EquipmentH_DepName.Show;
  end;

end;

procedure TDeliver_EquipmentH.BD3Click(Sender: TObject);
begin
  if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TDeliver_EquipmentH.BD4Click(Sender: TObject);
begin
  if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TDeliver_EquipmentH.BB3Click(Sender: TObject);
begin
...
procedure TDeliver_EquipmentH.BB6Click(Sender: TObject);
begin
  with DelMas do
  begin
    active:=false;
...
procedure TDeliver_EquipmentH.BD6Click(Sender: TObject);
begin
  with DelDet do
  begin
    active:=false;
...
procedure TDeliver_EquipmentH.PC1Change(Sender: TObject);
var i:integer;
begin
...
procedure TDeliver_EquipmentH.DelDetAfterOpen(DataSet: TDataSet);
var i:integer;
begin
...
//
Function TDeliver_EquipmentH.DelDet_LB_Check():boolean;
var iRes:boolean;
begin
  iRes:=true;
  with DelDet do
...
procedure TDeliver_EquipmentH.BD5Click(Sender: TObject);
var i:integer;
begin
                 ...
                 begin
                    ...
                    begin
                       ...
                       begin
                         ...
                         sql.add('insert into BDelRec ');
                         sql.add('(TableName ,TNO,CLBH,OldID,DelID,DelDate)');
                         sql.add('values ('+''''+'TSCD_KCHHS'+''''+','+''''+DelMas.fieldbyname('LLNO').Value+'''');
                         sql.add(','+''''+' '+''''+','+''''+DelMas.fieldbyname('USERID').Value+''''+',');
                         sql.add(''''+main.Edit1.Text+''''+',getdate())');
                         execsql;
                         active:=false;
...
procedure TDeliver_EquipmentH.BBTT2Click(Sender: TObject);
begin
  EquipmentH:=TEquipmentH.Create(self);
  EquipmentH.LLNOEdit.Text:=DelMas.FieldByName('Memo').AsString;
  EquipmentH.Show;
end;

procedure TDeliver_EquipmentH.DBGrid1GetCellParams(Sender: TObject;
...
procedure TDeliver_EquipmentH.DBGrid2GetCellParams(Sender: TObject;
  Column: TColumnEh; AFont: TFont; var Background: TColor;
  State: TGridDrawState);
begin
  if DelDet.FieldByName('YN').value='0' then
...
procedure TDeliver_EquipmentH.DBGrid1DblClick(Sender: TObject);
begin
...
procedure TDeliver_EquipmentH.DBGrid1EditButtonClick(Sender: TObject);
begin
...
procedure TDeliver_EquipmentH.BDT5Click(Sender: TObject);
begin
  ...
  if DelDet.RecordCount = 0  then
  begin
    showmessage('Pls insert the Lend Equipment(借出設備歸還) first ');
  end else
  begin
    Deliver_EquipmentH_print:=TDeliver_EquipmentH_print.Create(self);
    Deliver_EquipmentH_print.quickrep1.prepare;
    Deliver_EquipmentH_print.quickrep1.preview;
    Deliver_EquipmentH_print.free;
  end;
end;

procedure TDeliver_EquipmentH.BDT7Click(Sender: TObject);
begin
  if DelDet.RequestLive then
  begin
    showmessage('Pls save Master data first.');
  ...
  begin
    Deliver_EquipmentH_printTW:=TDeliver_EquipmentH_printTW.Create(self);
    Deliver_EquipmentH_printTW.quickrep1.prepare;
    Deliver_EquipmentH_printTW.quickrep1.preview;
    Deliver_EquipmentH_printTW.free;
  end;
end;
