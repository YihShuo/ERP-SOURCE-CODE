unit Deliver_EquipmentH_print1;
...
type
  TDeliver_EquipmentH_print = class(TForm)
    QuickRep1: TQuickRep;
...
var
  Deliver_EquipmentH_print: TDeliver_EquipmentH_print;

implementation
  uses Deliver_EquipmentH1, main1;
{$R *.dfm}

procedure TDeliver_EquipmentH_print.Readini();
var MyIni :Tinifile;
...
procedure TDeliver_EquipmentH_print.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=CaFree;
end;

procedure TDeliver_EquipmentH_print.FormCreate(Sender: TObject);
begin
  ReadIni();
  //簽核系統審核  20221215
  ...
  begin
    ...
    SQL.Add('      WHERE FORM_TABLE = ''TSCD_KCHH'' AND STATUS IN (''N'', ''Z'') AND FORM_PRESENTATION LIKE ''%'+Deliver_EquipmentH.DelMas.FieldByName('LLNO').AsString+'%''  ');
...
procedure TDeliver_EquipmentH_print.FormDestroy(Sender: TObject);
begin
  Deliver_EquipmentH_print:=nil;
end;
