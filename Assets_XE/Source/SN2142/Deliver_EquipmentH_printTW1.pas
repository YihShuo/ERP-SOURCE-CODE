unit Deliver_EquipmentH_printTW1;
...
uses
  ...
  Data.DB, Bde.DBTables, IniFiles, qrpctrls, Data.Win.ADODB;

type
  TDeliver_EquipmentH_printTW = class(TForm)
    QuickRep1: TQuickRep;
...
var
  Deliver_EquipmentH_printTW: TDeliver_EquipmentH_printTW;

implementation
  uses Deliver_EquipmentH1, main1;
{$R *.dfm}

procedure TDeliver_EquipmentH_printTW.ReadIni();
var MyIni :Tinifile;
    AppDir,ComName,AssetsN2142_ISO:string;
        // trinh ky
    sline_PW,tempstr_PW:string;
    ad_PW,i:integer;
begin
  ComName:='CONG TY TNHH TY XUAN';
  AssetsN2142_ISO:='T-DCC-091A';
  //
  ...
  begin
    try
      ...
      AssetsN2142_ISO:=MyIni.ReadString('ERP','AssetsN2142_ISO','T-DCC-091A');
       //
  ...
  L1.Caption:=ComName;
  QRISO.Caption:=AssetsN2142_ISO;
end;

procedure TDeliver_EquipmentH_printTW.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=CaFree;
end;

procedure TDeliver_EquipmentH_printTW.FormCreate(Sender: TObject);
begin
  DelMas.Active:=true;
  DelDet.Active:=true;
  ReadIni();
  ...
  begin
    ...
    SQL.Add('      WHERE FORM_TABLE = ''TSCD_KCLL'' AND STATUS IN (''N'', ''Z'') AND FORM_PRESENTATION LIKE ''%'+DelMas.FieldByName('LLNO').AsString+'%''  ');
...
procedure TDeliver_EquipmentH_printTW.FormDestroy(Sender: TObject);
begin
  Deliver_EquipmentH_printTW:=nil;
end;
