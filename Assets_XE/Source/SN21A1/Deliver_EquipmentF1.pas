...
type
  TDeliver_EquipmentF = class(TForm)
    Panel1: TPanel;
    Label18: TLabel;
    BB2: TBitBtn;
    BB3: TBitBtn;
  ...
  private
    ...
    Function InsertTSCD(Qty:double):String;
  public
    { Public declarations }
  end;

var
  Deliver_EquipmentF: TDeliver_EquipmentF;
  NDate:TDate;

implementation

uses main1, EquipmentF_Dep1, EquipmentF_DepName1, EquipmentF1, Deliver_EquipmentF_print1
     ,Deliver_EquipmentF_printTW1, FunUnit;

{$R *.dfm}
//
procedure TDeliver_EquipmentF.Init_LB();
begin
  LB:='FFFFFFFFFF';
end;
//異動單 Column Show or Not
procedure TDeliver_EquipmentF.DelDetAfterOpen_ColumnShow();
...
begin
  ...
  MM_GBox.Visible:=true;
  MM_GBox.Left:=GroupBox_Left;
  //
end;
//
procedure TDeliver_EquipmentF.DelDet_LB_Edit();
begin
  with DelDet do
  begin
    Edit;
    ...
    FieldByName('PARAMENT1').Value:='JRTS';
    FieldByName('PARAMENT2').Value:='JRBDATE';
    FieldByName('PARAMENT3').Value:='JREDATE';
    FieldByName('PARAMENT4').Value:='DepID_Memo';
    FieldByName('PARAMENT5').Value:='Status';
    FieldByName('Value5').Value:='F';
    Post;
  end;
end;
procedure TDeliver_EquipmentF.FormCreate(Sender: TObject);
var i:integer;
begin
  with Qtemp do
  begin
...
procedure TDeliver_EquipmentF.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if BB5.enabled  then
  begin
...
procedure TDeliver_EquipmentF.FormDestroy(Sender: TObject);
begin
  Deliver_EquipmentF:=nil;
end;

procedure TDeliver_EquipmentF.Button1Click(Sender: TObject);
begin
  Init_LB();
  with DelMas do
  begin
    active:=false;
    sql.Clear;
    sql.add('select TSCD_KCFF.*,BDepartment.DepName  ');
    sql.add('from TSCD_KCFF');
    sql.add('left join TSCD_BDepartment BDepartment on TSCD_KCFF.DepID=BDepartment.ID');
    sql.add('where ISNULL(flowflag,'''')<>''X'' and TSCD_KCFF.LLNO like ');
    sql.add(''''+edit1.Text+'%'+'''');
    ...
    sql.add('and convert(smalldatetime,convert(varchar,TSCD_KCFF.USERDATE,111)) between ');
    sql.add(''''+formatdatetime('yyyy/MM/dd',DTP1.Date) +'''');
    sql.add(' and  ');
    sql.add(''''+formatdatetime('yyyy/MM/dd',DTP2.Date) +'''');
    sql.add('and TSCD_KCFF.LB='''+LB+''' ');
    if Check.Checked then
    begin
      sql.Add('and TSCD_KCFF.USERID='+''''+main.Edit1.Text+'''');
    end;
    sql.add('and TSCD_KCFF.LB='''+LB+''' ');
    SQL.Add('and TSCD_KCFF.GSBH='''+main.Edit2.Text+''' ');
...
procedure TDeliver_EquipmentF.BBTT1Click(Sender: TObject);
begin
  EquipmentF_Dep:=TEquipmentF_Dep.Create(self);
  EquipmentF_Dep.Show;
end;


procedure TDeliver_EquipmentF.BB1Click(Sender: TObject);
begin
   panel2.Visible:=true;
end;

procedure TDeliver_EquipmentF.BB2Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive:=true;
...
procedure TDeliver_EquipmentF.BB4Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive:=true;
...
procedure TDeliver_EquipmentF.BB5Click(Sender: TObject);
var y,m,a:string;
    i:integer;
begin
  with Qtemp do
            ...
            if DelMas.fieldbyname('DepID').isnull then
              ...
              begin
                 ...
                 begin
                   active:=false;
                   sql.Clear;
                   sql.Add('select LLNO from TSCD_KCFF where LLNO like '+''''+y+m+'%'+'''');
                   ...
                   if DelMas.fieldbyname('YN').value='0'then
                     begin
                       ...
                       begin
                         ...
                         sql.add('values ('+''''+'TSCD_KCFF'+''''+','+''''+DelMas.fieldbyname('LLNO').Value+'''');
                         sql.add(','+''''+' '+''''+','+''''+DelMas.fieldbyname('USERID').Value+''''+',');
                         sql.add(''''+main.Edit1.Text+''''+',getdate())'); }
                         SQL.Add('Update TSCD_KCFF Set flowflag=''X'' where LLNO='''+DelMas.FieldByName('LLNO').AsString+''' ');
                         execsql;
                         active:=false;
                       end;
...
procedure TDeliver_EquipmentF.BD2Click(Sender: TObject);
begin
...
procedure TDeliver_EquipmentF.DBGrid2EditButtonClick(Sender: TObject);
var Style: String;
    i:integer;
begin
  ...
  begin
    EquipmentF_DepName:=TEquipmentF_DepName.Create(self);
    EquipmentF_DepName.COLUMN_Index:=1;
    EquipmentF_DepName.Show;
  end;
end;

procedure TDeliver_EquipmentF.BD3Click(Sender: TObject);
begin
 if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TDeliver_EquipmentF.BD4Click(Sender: TObject);
begin
 if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TDeliver_EquipmentF.BB3Click(Sender: TObject);
begin
...
procedure TDeliver_EquipmentF.BB6Click(Sender: TObject);
begin
  with DelMas do
  begin
    active:=false;
...
procedure TDeliver_EquipmentF.BD6Click(Sender: TObject);
begin
  with DelDet do
  begin
    active:=false;
...
procedure TDeliver_EquipmentF.PC1Change(Sender: TObject);
var i:integer;
begin
...
procedure TDeliver_EquipmentF.DelDetAfterOpen(DataSet: TDataSet);
var i:integer;
begin
...
//
Function TDeliver_EquipmentF.DelDet_LB_Check():boolean;
var iRes:boolean;
begin
  iRes:=true;
  ...
  with DelDet do
  begin
    if DelMas.fieldbyname('LB').AsString ='FFFFFFFFFF' then  //借出 Cho Muon
    begin
      if ((FieldByName('VALUE1').Isnull) or (FieldByName('VALUE2').Isnull) or (FieldByName('VALUE3').Isnull) or (FieldByName('VALUE4').Isnull) ) then
      begin
        iRes:=false;
      end;
    end;
...
//新增設備
Function TDeliver_EquipmentF.InsertTSCD(Qty:double):String;
begin
    //
    with Qtemp do
    begin
    ...
    TSCDQry.fieldbyname('SBBH').Value:=DelDet.fieldbyname('SBBH').Value;
    TSCDQry.fieldbyname('TSBH').Value:=DelDet.fieldbyname('TSBH').Value;
    TSCDQry.fieldbyname('Qty').Value:=Qty;
    TSCDQry.FieldByName('Status').AsString:='I';
    TSCDQry.FieldByName('DepID').AsString:=DelMas.FieldByName('DepID').AsString;//
    TSCDQry.FieldByName('DepName').AsString:=DelMas.FieldByName('DepName').AsString;//
    TSCDQry.FieldByName('DepID_Memo').AsString:='';
    ...
    TSCDQry.fieldbyname('RKNO').Value:=DelMas.fieldbyname('LLNO').Value;
...
procedure TDeliver_EquipmentF.BD5Click(Sender: TObject);
var i:integer;
    TSID:String;
begin
            ...
            begin
              ...
              begin
                  ...
                  begin
                    ...
                    DelDet.fieldbyname('TSID').Value:=TSID;
                 ...
                 begin
                    ...
                    begin
                       ...
                       begin
                         ...
                         sql.add('insert into BDelRec ');
                         sql.add('(TableName ,TNO,CLBH,OldID,DelID,DelDate)');
                         sql.add('values ('+''''+'TSCD_KCFFS'+''''+','+''''+DelMas.fieldbyname('LLNO').Value+'''');
                         sql.add(','+''''+' '+''''+','+''''+DelMas.fieldbyname('USERID').Value+''''+',');
                         sql.add(''''+main.Edit1.Text+''''+',getdate())');
                         execsql;
                         active:=false;
                       ...
                       begin
                        active:=false;
                        sql.Clear;
                        sql.Add('Delete from TSCD where RKNO='''+DelDet.FieldByName('LLNO').AsString+''' and SBBH='''+DelDet.FieldByName('SBBH').AsString+''' and CFMID_RK=''NO''  and LB=''1'' ');
                        funcObj.WriteErrorLog(sql.Text);
                        ExecSQL();
                       end;
...
procedure TDeliver_EquipmentF.BBTT2Click(Sender: TObject);
begin
  EquipmentF:=TEquipmentF.Create(self);
  EquipmentF.Show;
end;

procedure TDeliver_EquipmentF.DBGrid1GetCellParams(Sender: TObject;
...
procedure TDeliver_EquipmentF.DBGrid2GetCellParams(Sender: TObject;
  Column: TColumnEh; AFont: TFont; var Background: TColor;
  State: TGridDrawState);
begin
  if DelDet.FieldByName('YN').value='0' then
...
procedure TDeliver_EquipmentF.DBGrid1DblClick(Sender: TObject);
begin
...
procedure TDeliver_EquipmentF.DBGrid1EditButtonClick(Sender: TObject);
begin
...
procedure TDeliver_EquipmentF.BDT5Click(Sender: TObject);
begin
  ...
  Deliver_EquipmentF_print:=TDeliver_EquipmentF_print.Create(self);
  Deliver_EquipmentF_print.quickrep1.prepare;
  Deliver_EquipmentF_print.quickrep1.preview;
  Deliver_EquipmentF_print.free;
end;

procedure TDeliver_EquipmentF.BDT7Click(Sender: TObject);
begin
  if DelDet.RequestLive then
  begin
    showmessage('Pls save Master data first.');
  ...
  Deliver_EquipmentF_printTW:=TDeliver_EquipmentF_printTW.Create(self);
  Deliver_EquipmentF_printTW.quickrep1.prepare;
  Deliver_EquipmentF_printTW.quickrep1.preview;
  Deliver_EquipmentF_printTW.free;
end;
