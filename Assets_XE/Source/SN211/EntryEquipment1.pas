...
begin
    ...
    TSCDQry.FieldByName('CFMID_LL').Value:='NO';
     if RB3.Checked  then  TSCDQry.FieldByName('LB').Value:='3' else
     if RB2.Checked  then  TSCDQry.FieldByName('LB').Value:='2' else
     if RB1.Checked  then  TSCDQry.FieldByName('LB').Value:='1';
   ...
   begin
     ...
     begin
       ...
       begin
         Active:=false;
         SQL.Clear;
         SQL.Add('Delete from TSCD where RKNO='''+EnDet.FieldByName('RKNO').Value+''' and SBBH='''+EnDet.FieldByName('SBBH').Value+'''');
         if RB3.Checked  then  SQL.Add(' and LB=3 and CFMID=''NO'' ')else
         if RB2.Checked  then  SQL.Add(' and LB=2 and CFMID=''NO'' ') else
         if RB1.Checked  then  SQL.Add(' and LB=1 and CFMID=''NO'' ');
         ExecSQL();
       end;
       with TSCDQry do
       begin
...
begin
   ...
   begin
     ...
     begin
      active:=false;
      sql.Clear;
      sql.Add('Delete from TSCD where RKNO='''+EnDet.FieldByName('RKNO').AsString+''' and SBBH='''+EnDet.FieldByName('SBBH').AsString+''' and CFMID_RK=''NO''');
         if RB3.Checked  then  SQL.Add(' and LB=3 and CFMID=''NO'' ')else
         if RB2.Checked  then  SQL.Add(' and LB=2 and CFMID=''NO'' ') else
         if RB1.Checked  then  SQL.Add(' and LB=1 and CFMID=''NO'' ');
      ExecSQL();
