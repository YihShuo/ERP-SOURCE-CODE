object EntryEquipment: TEntryEquipment
  Left = 315
  Top = 235
  Caption = 'Entry Equipment'
  ClientHeight = 749
  ClientWidth = 1370
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1370
    Height = 65
    Align = alTop
    TabOrder = 0
    object Label18: TLabel
      Left = 704
      Top = 32
      Width = 74
      Height = 24
      Caption = 'Master'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -19
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
    object bbInsert: TBitBtn
      Left = 56
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Insert one New Record'
      Caption = 'Insert'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 1
      OnClick = bbInsertClick
    end
    object bbDelete: TBitBtn
      Left = 104
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Delete one Record'
      Caption = 'Delete'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 2
      OnClick = bbDeleteClick
    end
    object bbModify: TBitBtn
      Left = 152
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Modify'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        50BB555555555555575F555555555555550B5555555555555575}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 3
      OnClick = bbModifyClick
    end
    object bbSave: TBitBtn
      Left = 200
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Save Current'
      Caption = 'Save'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        5990555555555555577755555555555555555555555555555555}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 4
      OnClick = bbSaveClick
    end
    object bbCancel: TBitBtn
      Left = 248
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Cancel'
      Caption = 'Cancel'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 5
      OnClick = bbCancelClick
    end
    object bbExit: TBitBtn
      Left = 296
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Exit Current Form'
      Caption = 'Exit'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        EE33337FFFFFFFFFFF7F33EEEEEEEEEEEE333377777777777773}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 6
    end
    object bbQuery: TBitBtn
      Left = 8
      Top = 8
      Width = 49
      Height = 49
      Caption = 'Query'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        3000333333333377377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 0
      OnClick = bbQueryClick
    end
    object bbFirst: TBitBtn
      Left = 368
      Top = 8
      Width = 49
      Height = 49
      Caption = 'First'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        CCC333333333333777FF33333333333333CC3333333333333773}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 7
    end
    object bbPrior: TBitBtn
      Left = 416
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Insert one New Record'
      Caption = 'Prior'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 8
    end
    object bbNext: TBitBtn
      Left = 464
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Delete one Record'
      Caption = 'Next'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 9
    end
    object bbLast: TBitBtn
      Left = 512
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Last'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        33333777333333333333CC333333333333337733333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 10
    end
    object bbExcel: TBitBtn
      Left = 608
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Excel'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        3BB33773333773333773B333333B3333333B7333333733333337}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 11
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 65
    Width = 1370
    Height = 65
    Align = alTop
    TabOrder = 1
    Visible = False
    object bdInsert: TBitBtn
      Left = 56
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Insert one New Record'
      Caption = 'Insert'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 0
      OnClick = bdInsertClick
    end
    object bdDelete: TBitBtn
      Left = 104
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Delete one Record'
      Caption = 'Delete'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 1
      OnClick = bdDeleteClick
    end
    object bdModify: TBitBtn
      Left = 152
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Modify'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        50BB555555555555575F555555555555550B5555555555555575}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 2
      OnClick = bdModifyClick
    end
    object bdSave: TBitBtn
      Left = 200
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Save Current'
      Caption = 'Save'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        5990555555555555577755555555555555555555555555555555}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 3
      OnClick = bdSaveClick
    end
    object bdCancel: TBitBtn
      Left = 248
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Cancel'
      Caption = 'Cancel'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 4
      OnClick = bdCancelClick
    end
    object bdExit: TBitBtn
      Left = 296
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Exit Current Form'
      Caption = 'Exit'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        EE33337FFFFFFFFFFF7F33EEEEEEEEEEEE333377777777777773}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 5
    end
    object bdFirst: TBitBtn
      Left = 368
      Top = 8
      Width = 49
      Height = 49
      Caption = 'First'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        CCC333333333333777FF33333333333333CC3333333333333773}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 6
    end
    object bdPrior: TBitBtn
      Left = 416
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Insert one New Record'
      Caption = 'Prior'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 7
    end
    object bdNext: TBitBtn
      Left = 464
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Delete one Record'
      Caption = 'Next'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 8
    end
    object bdLast: TBitBtn
      Left = 512
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Last'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        33333777333333333333CC333333333333337733333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 9
    end
    object bdPrint: TBitBtn
      Left = 576
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'PrintTW'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        33333337FFFF7733333333300000033333333337777773333333}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 10
      OnClick = bdPrintClick
    end
    object bdExcel: TBitBtn
      Left = 632
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Excel'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      Glyph.Data = {
        
        3BB33773333773333773B333333B3333333B7333333733333337}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 11
    end
  end
  object PC1: TPageControl
    Left = 0
    Top = 130
    Width = 1370
    Height = 619
    ActivePage = TS1
    Align = alClient
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 2
    OnChange = PC1Change
    object TS1: TTabSheet
      Caption = 'EnMas'
      ImageIndex = 2
      object Splitter2: TSplitter
        Left = 1126
        Top = 113
        Width = 2
        Height = 475
        Align = alRight
        ExplicitLeft = 1123
        ExplicitHeight = 317
      end
      object Panel3: TPanel
        Left = 0
        Top = 0
        Width = 1362
        Height = 57
        Align = alTop
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        Visible = False
        object Label1: TLabel
          Left = 16
          Top = 19
          Width = 40
          Height = 17
          Caption = 'RKNO:'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'VNI-Times'
          Font.Style = []
          ParentFont = False
        end
        object Label3: TLabel
          Left = 191
          Top = 19
          Width = 32
          Height = 17
          Caption = 'From:'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'VNI-Times'
          Font.Style = []
          ParentFont = False
        end
        object Label4: TLabel
          Left = 349
          Top = 19
          Width = 19
          Height = 17
          Caption = 'To:'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'VNI-Times'
          Font.Style = []
          ParentFont = False
        end
        object edtRKNO: TEdit
          Left = 59
          Top = 16
          Width = 120
          Height = 25
          AutoSelect = False
          CharCase = ecUpperCase
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'VNI-Times'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
        end
        object btnQuery: TButton
          Left = 569
          Top = 12
          Width = 81
          Height = 33
          Caption = 'QUERY'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'VNI-Times'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          OnClick = btnQueryClick
        end
        object CheckMine: TCheckBox
          Left = 499
          Top = 20
          Width = 57
          Height = 17
          Caption = 'Mine'
          Checked = True
          State = cbChecked
          TabOrder = 2
        end
        object DTP1: TDateTimePicker
          Left = 229
          Top = 16
          Width = 113
          Height = 25
          Date = 39255.000000000000000000
          Format = 'yyyy/MM/dd'
          Time = 39255.000000000000000000
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'VNI-Times'
          Font.Style = []
          ParentFont = False
          TabOrder = 3
        end
        object DTP2: TDateTimePicker
          Left = 373
          Top = 16
          Width = 113
          Height = 25
          Date = 39255.000000000000000000
          Format = 'yyyy/MM/dd'
          Time = 39255.000000000000000000
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'VNI-Times'
          Font.Style = []
          ParentFont = False
          TabOrder = 4
        end
      end
      object Panel4: TPanel
        Left = 0
        Top = 57
        Width = 1362
        Height = 56
        Align = alTop
        TabOrder = 1
        object Label9: TLabel
          Left = 592
          Top = 13
          Width = 52
          Height = 16
          Caption = 'DOCNO:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label11: TLabel
          Left = 4
          Top = 13
          Width = 41
          Height = 16
          Caption = 'RKNO:'
        end
        object Label2: TLabel
          Left = 371
          Top = 13
          Width = 66
          Height = 16
          Caption = 'DepName:'
        end
        object Label5: TLabel
          Left = 179
          Top = 13
          Width = 53
          Height = 16
          Caption = 'Supplier:'
        end
        object dbe_DOCNO: TDBEdit
          Left = 647
          Top = 10
          Width = 121
          Height = 24
          CharCase = ecUpperCase
          Color = 15987699
          DataField = 'DOCNO'
          DataSource = DS1
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
        end
        object dbe_RKNO: TDBEdit
          Left = 47
          Top = 10
          Width = 120
          Height = 24
          CharCase = ecUpperCase
          Color = 15987699
          DataField = 'RKNO'
          DataSource = DS1
          ReadOnly = True
          TabOrder = 1
        end
        object dbe_USERDATE: TDBEdit
          Left = 438
          Top = 10
          Width = 115
          Height = 24
          CharCase = ecUpperCase
          Color = 15987699
          DataField = 'DepName'
          DataSource = DS1
          ReadOnly = True
          TabOrder = 2
        end
        object DepIDBtn: TBitBtn
          Left = 556
          Top = 9
          Width = 19
          Height = 23
          Caption = '...'
          TabOrder = 3
          Visible = False
          OnClick = DepIDBtnClick
        end
        object DBEdit1: TDBEdit
          Left = 238
          Top = 10
          Width = 107
          Height = 24
          CharCase = ecUpperCase
          Color = 15987699
          DataField = 'ZSJC_YW'
          DataSource = DS1
          ReadOnly = True
          TabOrder = 4
        end
        object ZSBHBtn: TBitBtn
          Left = 345
          Top = 9
          Width = 19
          Height = 23
          Caption = '...'
          TabOrder = 5
          Visible = False
          OnClick = ZSBHBtnClick
        end
        object Panel8: TPanel
          Left = 1128
          Top = 1
          Width = 233
          Height = 54
          Align = alRight
          TabOrder = 6
          object btInsert: TBitBtn
            Left = 4
            Top = 5
            Width = 48
            Height = 49
            Hint = 'Insert one New Record'
            Caption = 'Insert'
            Enabled = False
            Font.Charset = ANSI_CHARSET
            Font.Color = clWindowText
            Font.Height = -15
            Font.Name = 'Arial'
            Font.Style = []
            Glyph.Data = {
              
              3000333333333333377733333333333333333333333333333333}
            Layout = blGlyphTop
            NumGlyphs = 2
            ParentFont = False
            TabOrder = 0
            OnClick = btInsertClick
          end
          object btDelete: TBitBtn
            Left = 49
            Top = 5
            Width = 47
            Height = 49
            Hint = 'Delete one Record'
            Caption = 'Delete'
            Enabled = False
            Font.Charset = ANSI_CHARSET
            Font.Color = clWindowText
            Font.Height = -15
            Font.Name = 'Arial'
            Font.Style = []
            Glyph.Data = {
              
              3000333333333333377733333333333333333333333333333333}
            Layout = blGlyphTop
            NumGlyphs = 2
            ParentFont = False
            TabOrder = 1
            OnClick = btDeleteClick
          end
          object btModify: TBitBtn
            Left = 94
            Top = 5
            Width = 48
            Height = 49
            Hint = 'Modify Current'
            Caption = 'Modify'
            Enabled = False
            Font.Charset = ANSI_CHARSET
            Font.Color = clWindowText
            Font.Height = -15
            Font.Name = 'Arial'
            Font.Style = []
            Glyph.Data = {
              
              50BB555555555555575F555555555555550B5555555555555575}
            Layout = blGlyphTop
            NumGlyphs = 2
            ParentFont = False
            TabOrder = 2
            OnClick = btModifyClick
          end
          object btSave: TBitBtn
            Left = 140
            Top = 5
            Width = 44
            Height = 49
            Hint = 'Save Current'
            Caption = 'Save'
            Enabled = False
            Font.Charset = ANSI_CHARSET
            Font.Color = clWindowText
            Font.Height = -15
            Font.Name = 'Arial'
            Font.Style = []
            Glyph.Data = {
              
              5990555555555555577755555555555555555555555555555555}
            Layout = blGlyphTop
            NumGlyphs = 2
            ParentFont = False
            TabOrder = 3
            OnClick = btSaveClick
          end
          object btCancel: TBitBtn
            Left = 182
            Top = 5
            Width = 50
            Height = 49
            Hint = 'Cancel'
            Caption = 'Cancel'
            Enabled = False
            Font.Charset = ANSI_CHARSET
            Font.Color = clWindowText
            Font.Height = -15
            Font.Name = 'Arial'
            Font.Style = []
            Glyph.Data = {
              
              3333333333333333333333333333333333333333333333333333}
            Layout = blGlyphTop
            NumGlyphs = 2
            ParentFont = False
            TabOrder = 4
            OnClick = btCancelClick
          end
        end
      end
      object DBGrid1: TDBGridEh
        Left = 0
        Top = 113
        Width = 1126
        Height = 475
        Align = alClient
        DataSource = DS1
        DynProps = <>
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        FooterParams.Color = clWindow
        ParentFont = False
        TabOrder = 2
        TitleParams.MultiTitle = True
        OnDblClick = DBGrid1DblClick
        OnEditButtonClick = DBGrid1EditButtonClick
        OnGetCellParams = DBGrid1GetCellParams
        Columns = <
          item
            CellButtons = <>
            Color = clBtnFace
            DynProps = <>
            EditButtons = <>
            FieldName = 'RKNO'
            Footers = <>
            ReadOnly = True
            Title.Caption = #20837#24235#32232#34399'|RKNO'
          end
          item
            CellButtons = <>
            Color = clBtnFace
            DynProps = <>
            EditButtons = <>
            FieldName = 'LB'
            Footers = <>
            ReadOnly = True
            Title.Caption = #39006#21029'|LB'
            Width = 38
          end
          item
            CellButtons = <>
            DynProps = <>
            EditButtons = <>
            FieldName = 'ZSBH'
            Footers = <>
            ReadOnly = True
            Title.Caption = #24288#21830'|ID'
          end
          item
            CellButtons = <>
            DynProps = <>
            EditButtons = <>
            FieldName = 'ZSJC_YW'
            Footers = <>
            ReadOnly = True
            Title.Caption = #24288#21830'|Supplier'
            Width = 80
          end
          item
            CellButtons = <>
            DynProps = <>
            EditButtons = <>
            FieldName = 'ZSNO'
            Footers = <>
            Title.Caption = #25505#36092#21934'|CGNO'
            Width = 82
          end
          item
            CellButtons = <>
            DynProps = <>
            EditButtons = <>
            FieldName = 'SOHIEU'
            Footers = <>
            Title.Caption = #32232#34399'|SOHIEU'
            Width = 70
          end
          item
            CellButtons = <>
            DynProps = <>
            EditButtons = <>
            FieldName = 'DOCNO'
            Footers = <>
            Title.Caption = #30332#31080#32232#34399'|DOCNO'
            Width = 130
          end
          item
            CellButtons = <>
            DynProps = <>
            EditButtons = <>
            FieldName = 'DOCDate'
            Footers = <>
            Title.Caption = #30332#31080#26085#26399'|DOCDate'
            Width = 75
          end
          item
            CellButtons = <>
            DynProps = <>
            EditButtons = <>
            FieldName = 'HDNO'
            Footers = <>
            Title.Caption = #21512#21516#32232#34399'|HDNO'
            Width = 130
          end
          item
            CellButtons = <>
            DynProps = <>
            EditButtons = <>
            FieldName = 'HDDate'
            Footers = <>
            Title.Caption = #21512#21516#26085#26399'|HDDate'
            Width = 75
          end
          item
            CellButtons = <>
            DynProps = <>
            EditButtons = <>
            FieldName = 'DepName'
            Footers = <>
            ReadOnly = True
            Title.Caption = #21934#20301'|DepName'
            Width = 115
          end
          item
            CellButtons = <>
            Color = clBtnFace
            DynProps = <>
            EditButtons = <>
            FieldName = 'flowflag'
            Footers = <>
            Title.Caption = #31805#26680'|flowflag'
            Width = 38
          end
          item
            CellButtons = <>
            Color = clBtnFace
            DynProps = <>
            EditButtons = <>
            FieldName = 'USERID'
            Footers = <>
            ReadOnly = True
            Title.Caption = #20351#29992#32773'|USERID'
          end
          item
            CellButtons = <>
            Color = clBtnFace
            DynProps = <>
            EditButtons = <>
            FieldName = 'USERDATE'
            Footers = <>
            Title.Caption = #26085#26399'|USERDATE'
            Width = 100
          end
          item
            CellButtons = <>
            Color = clBtnFace
            DynProps = <>
            EditButtons = <>
            FieldName = 'CFMID'
            Footers = <>
            ReadOnly = True
            Title.Caption = #30906#35469'ID|CFMID'
          end
          item
            CellButtons = <>
            Color = clBtnFace
            DynProps = <>
            EditButtons = <>
            FieldName = 'CFMDATE'
            Footers = <>
            ReadOnly = True
            Title.Caption = #30906#35469#26085#26399'|CFMDATE'
            Width = 97
          end
          item
            CellButtons = <>
            Color = clBtnFace
            DynProps = <>
            EditButtons = <>
            FieldName = 'YN'
            Footers = <>
            ReadOnly = True
            Width = 23
          end>
        object RowDetailData: TRowDetailPanelControlEh
        end
      end
      object DBGridEh2: TDBGridEh
        Left = 1128
        Top = 113
        Width = 234
        Height = 475
        Align = alRight
        DataSource = DSKCPK
        DynProps = <>
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        FooterParams.Color = clWindow
        ParentFont = False
        TabOrder = 3
        TitleParams.MultiTitle = True
        Columns = <
          item
            CellButtons = <>
            DynProps = <>
            EditButtons = <>
            FieldName = 'Declaretion'
            Footers = <>
            Title.Caption = #22577#38364#21934'|TOKHAI'
            Width = 91
          end
          item
            CellButtons = <>
            DynProps = <>
            EditButtons = <>
            FieldName = 'Export'
            Footers = <>
            Title.Caption = #39006#21029'|Export'
            Width = 77
          end
          item
            CellButtons = <>
            DynProps = <>
            EditButtons = <>
            FieldName = 'TKDate'
            Footers = <>
            Title.Caption = #22577#38364#26085#26399'|TKDate'
            Width = 75
          end>
        object RowDetailData: TRowDetailPanelControlEh
        end
      end
    end
    object TS2: TTabSheet
      Caption = 'EnDet'
      ImageIndex = 3
      object TS_DelDet: TPanel
        Left = 0
        Top = 0
        Width = 1362
        Height = 588
        Align = alClient
        Caption = 'DelDet'
        TabOrder = 0
        object Panel5: TPanel
          Left = 1
          Top = 1
          Width = 1360
          Height = 80
          Align = alTop
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
          object Label7: TLabel
            Left = 8
            Top = 19
            Width = 41
            Height = 16
            Caption = 'RKNO:'
          end
          object Label6: TLabel
            Left = 9
            Top = 53
            Width = 40
            Height = 16
            Caption = 'SBBH:'
          end
          object Label12: TLabel
            Left = 257
            Top = 20
            Width = 45
            Height = 16
            Caption = 'YWPM:'
          end
          object Label14: TLabel
            Left = 258
            Top = 52
            Width = 44
            Height = 16
            Caption = 'ZWPM:'
          end
          object Label8: TLabel
            Left = 655
            Top = 58
            Width = 55
            Height = 16
            Alignment = taRightJustify
            AutoSize = False
            Caption = 'LB:'
          end
          object dbeRKNO: TDBEdit
            Left = 53
            Top = 16
            Width = 121
            Height = 24
            CharCase = ecUpperCase
            Color = 15987699
            DataField = 'RKNO'
            DataSource = DS1
            TabOrder = 0
          end
          object dbeSBBH: TDBEdit
            Left = 53
            Top = 49
            Width = 121
            Height = 24
            CharCase = ecUpperCase
            DataField = 'SBBH'
            DataSource = DS2
            TabOrder = 1
          end
          object dbeZWPM: TDBEdit
            Left = 306
            Top = 48
            Width = 335
            Height = 24
            CharCase = ecUpperCase
            DataField = 'ZWPM'
            DataSource = DS2
            TabOrder = 2
          end
          object dbeYWPM: TDBEdit
            Left = 305
            Top = 17
            Width = 336
            Height = 24
            CharCase = ecUpperCase
            DataField = 'YWPM'
            DataSource = DS2
            TabOrder = 3
          end
          object bbtSBBH: TBitBtn
            Left = 155
            Top = 50
            Width = 19
            Height = 24
            Caption = '...'
            TabOrder = 4
            Visible = False
            OnClick = bbtSBBHClick
          end
          object GroupBox2: TGroupBox
            Left = 716
            Top = 52
            Width = 357
            Height = 22
            TabOrder = 5
            object RB2: TRadioButton
              Left = 168
              Top = 3
              Width = 81
              Height = 17
              Caption = 'Thiet bi IT'
              TabOrder = 0
            end
            object RB1: TRadioButton
              Left = 10
              Top = 3
              Width = 136
              Height = 17
              Caption = 'Thiet bi  may moc'
              Checked = True
              TabOrder = 1
              TabStop = True
            end
            object RB3: TRadioButton
              Left = 288
              Top = 3
              Width = 57
              Height = 17
              Caption = 'IT DC'
              TabOrder = 2
            end
          end
        end
        object DBGrid2: TDBGridEh
          Left = 1
          Top = 81
          Width = 584
          Height = 506
          Align = alLeft
          DataSource = DS2
          DynProps = <>
          FooterParams.Color = clWindow
          TabOrder = 1
          TitleParams.MultiTitle = True
          OnGetCellParams = DBGrid2GetCellParams
          Columns = <
            item
              CellButtons = <>
              DynProps = <>
              EditButtons = <>
              FieldName = 'RKNO'
              Footers = <>
              ReadOnly = True
              Title.Caption = #20837#24235#21934#34399'|RKNO'
              Visible = False
              Width = 87
            end
            item
              CellButtons = <>
              Color = clYellow
              DynProps = <>
              EditButton.OnClick = DBGrid2Columns2EditButtonClick
              EditButtons = <>
              FieldName = 'SBBH'
              Footers = <>
              ReadOnly = True
              Title.Caption = #30446#37636#32232#34399'|SBBH'
              Width = 82
              OnEditButtonClick = DBGrid2Columns2EditButtonClick
            end
            item
              CellButtons = <>
              Color = clBtnFace
              DynProps = <>
              EditButtons = <>
              FieldName = 'VWPM'
              Footers = <>
              ReadOnly = True
              Title.Caption = #36234#25991#21517#31281'|VWPM'
              Width = 170
            end
            item
              CellButtons = <>
              DynProps = <>
              EditButtons = <>
              FieldName = 'Qty'
              Footers = <>
              Title.Caption = #25976#37327'|Qty'
              Width = 37
            end
            item
              CellButtons = <>
              Color = clBtnFace
              DynProps = <>
              EditButtons = <>
              FieldName = 'QUCBH'
              Footers = <>
              ReadOnly = True
              Title.Caption = #35215#26684'|QUCBH'
              Width = 165
            end
            item
              CellButtons = <>
              Color = clBtnFace
              DynProps = <>
              EditButtons = <>
              FieldName = 'LSBH'
              Footers = <>
              ReadOnly = True
              Title.Caption = #22411#34399'|LSBH'
              Width = 162
            end
            item
              CellButtons = <>
              DropDownSpecRow.Value = 0.000000000000000000
              DynProps = <>
              EditButtons = <>
              FieldName = 'VNPrice'
              Footers = <>
              Title.Caption = #21934#20729'|VND'
              Visible = False
              Width = 55
            end
            item
              CellButtons = <>
              DropDownSpecRow.Value = 0.000000000000000000
              DynProps = <>
              EditButtons = <>
              FieldName = 'USPrice'
              Footers = <>
              Title.Caption = #21934#20729'|USD'
              Visible = False
              Width = 57
            end
            item
              CellButtons = <>
              Color = clBtnFace
              DynProps = <>
              EditButtons = <>
              FieldName = 'USERID'
              Footers = <>
              ReadOnly = True
              Title.Caption = #20351#29992#32773'|USERID'
              Width = 60
            end
            item
              CellButtons = <>
              Color = clBtnFace
              DisplayFormat = 'YYYY/MM/DD'
              DynProps = <>
              EditButtons = <>
              FieldName = 'USERDATE'
              Footers = <>
              ReadOnly = True
              Title.Caption = #20351#29992#32773#26085#26399'|USERDATE'
              Width = 81
            end
            item
              CellButtons = <>
              Color = clBtnFace
              DynProps = <>
              EditButtons = <>
              FieldName = 'YN'
              Footers = <>
              ReadOnly = True
              Width = 25
            end>
          object RowDetailData: TRowDetailPanelControlEh
          end
        end
        object Panel6: TPanel
          Left = 585
          Top = 81
          Width = 776
          Height = 506
          Align = alClient
          TabOrder = 2
          object Panel7: TPanel
            Left = 1
            Top = 1
            Width = 774
            Height = 52
            Align = alTop
            TabOrder = 0
            object BO1: TBitBtn
              Left = 0
              Top = -1
              Width = 49
              Height = 49
              Hint = 'Insert one New Record'
              Caption = 'Insert'
              Enabled = False
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = []
              Glyph.Data = {
                
                3000333333333333377733333333333333333333333333333333}
              Layout = blGlyphTop
              NumGlyphs = 2
              ParentFont = False
              TabOrder = 0
              OnClick = BO1Click
            end
            object BO2: TBitBtn
              Left = 49
              Top = -1
              Width = 49
              Height = 49
              Hint = 'Delete one Record'
              Caption = 'Delete'
              Enabled = False
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = []
              Glyph.Data = {
                
                3000333333333333377733333333333333333333333333333333}
              Layout = blGlyphTop
              NumGlyphs = 2
              ParentFont = False
              TabOrder = 1
              OnClick = BO2Click
            end
            object BO5: TBitBtn
              Left = 98
              Top = -1
              Width = 46
              Height = 49
              Hint = 'Modify Current'
              Caption = 'Modify'
              Enabled = False
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = []
              Glyph.Data = {
                
                50BB555555555555575F555555555555550B5555555555555575}
              Layout = blGlyphTop
              NumGlyphs = 2
              ParentFont = False
              TabOrder = 2
              OnClick = BO5Click
            end
            object BO3: TBitBtn
              Left = 144
              Top = -1
              Width = 49
              Height = 49
              Hint = 'Save Current'
              Caption = 'Save'
              Enabled = False
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = []
              Glyph.Data = {
                
                5990555555555555577755555555555555555555555555555555}
              Layout = blGlyphTop
              NumGlyphs = 2
              ParentFont = False
              TabOrder = 3
              OnClick = BO3Click
            end
            object BO4: TBitBtn
              Left = 193
              Top = -1
              Width = 48
              Height = 49
              Hint = 'Cancel'
              Caption = 'Cancel'
              Enabled = False
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = []
              Glyph.Data = {
                
                3333333333333333333333333333333333333333333333333333}
              Layout = blGlyphTop
              NumGlyphs = 2
              ParentFont = False
              TabOrder = 4
              OnClick = BO4Click
            end
          end
          object DBGrid3: TDBGridEh
            Left = 1
            Top = 53
            Width = 774
            Height = 228
            Align = alTop
            DataSource = DS3
            DynProps = <>
            FooterParams.Color = clWindow
            TabOrder = 1
            TitleParams.MultiTitle = True
            OnEditButtonClick = DBGrid3EditButtonClick
            Columns = <
              item
                CellButtons = <>
                Color = clBtnFace
                DynProps = <>
                EditButtons = <>
                FieldName = 'TSID'
                Footers = <>
                ReadOnly = True
                Title.Caption = #36001#29986'ID|TSID'
              end
              item
                CellButtons = <>
                Color = clBtnFace
                DynProps = <>
                EditButtons = <>
                FieldName = 'Qty'
                Footers = <>
                ReadOnly = True
                Title.Caption = #25976#37327'|Qty'
                Width = 69
              end
              item
                CellButtons = <>
                Color = clBtnFace
                DynProps = <>
                EditButtons = <>
                FieldName = 'Unit_Name'
                Footers = <>
                Title.Caption = #22577#38364#21934#20301'|UnitName'
              end
              item
                CellButtons = <>
                DynProps = <>
                EditButtons = <>
                FieldName = 'TSBH'
                Footers = <>
                Title.Caption = #36001#29986#32232#34399'|TSBH'
                Width = 65
              end
              item
                CellButtons = <>
                DynProps = <>
                EditButtons = <>
                FieldName = 'XSBH'
                Footers = <>
                Title.Caption = #20986#24288#34399#30908'|XSBH'
                Width = 84
              end
              item
                CellButtons = <>
                DynProps = <>
                EditButtons = <>
                FieldName = 'NSX'
                Footers = <>
                Title.Caption = #35069#36896#24180#26376'|NSX'
              end
              item
                CellButtons = <>
                DynProps = <>
                EditButtons = <>
                FieldName = 'InDATE'
                Footers = <>
                Title.Caption = #20837#24288#26085'|InDATE'
                Width = 65
              end
              item
                CellButtons = <>
                DynProps = <>
                EditButtons = <>
                FieldName = 'DepID'
                Footers = <>
                Title.Caption = #23384#25918#20301#32622'|DepID'
              end
              item
                CellButtons = <>
                Color = clBtnFace
                DynProps = <>
                EditButtons = <>
                FieldName = 'DepName'
                Footers = <>
                ReadOnly = True
                Title.Caption = #23384#25918#20301#32622'|DepName'
                Width = 80
              end
              item
                CellButtons = <>
                DynProps = <>
                EditButtons = <>
                FieldName = 'DepID_Memo'
                Footers = <>
                Title.Caption = #23384#25918#20301#32622'|Memo'
                Width = 177
              end>
            object RowDetailData: TRowDetailPanelControlEh
            end
          end
          object Panel9: TPanel
            Left = 1
            Top = 281
            Width = 774
            Height = 52
            Align = alTop
            TabOrder = 2
            object BE1: TBitBtn
              Left = 49
              Top = -1
              Width = 49
              Height = 49
              Hint = 'Insert one New Record'
              Caption = 'Insert'
              Enabled = False
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = []
              Glyph.Data = {
                
                3000333333333333377733333333333333333333333333333333}
              Layout = blGlyphTop
              NumGlyphs = 2
              ParentFont = False
              TabOrder = 0
              OnClick = BE1Click
            end
            object BE2: TBitBtn
              Left = 98
              Top = -1
              Width = 49
              Height = 49
              Hint = 'Delete one Record'
              Caption = 'Delete'
              Enabled = False
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = []
              Glyph.Data = {
                
                3000333333333333377733333333333333333333333333333333}
              Layout = blGlyphTop
              NumGlyphs = 2
              ParentFont = False
              TabOrder = 1
              OnClick = BE2Click
            end
            object BE3: TBitBtn
              Left = 147
              Top = -1
              Width = 46
              Height = 49
              Hint = 'Modify Current'
              Caption = 'Modify'
              Enabled = False
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = []
              Glyph.Data = {
                
                50BB555555555555575F555555555555550B5555555555555575}
              Layout = blGlyphTop
              NumGlyphs = 2
              ParentFont = False
              TabOrder = 2
              OnClick = BE3Click
            end
            object BE4: TBitBtn
              Left = 193
              Top = -1
              Width = 49
              Height = 49
              Hint = 'Save Current'
              Caption = 'Save'
              Enabled = False
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = []
              Glyph.Data = {
                
                5990555555555555577755555555555555555555555555555555}
              Layout = blGlyphTop
              NumGlyphs = 2
              ParentFont = False
              TabOrder = 3
              OnClick = BE4Click
            end
            object BE5: TBitBtn
              Left = 242
              Top = -1
              Width = 48
              Height = 49
              Hint = 'Cancel'
              Caption = 'Cancel'
              Enabled = False
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = []
              Glyph.Data = {
                
                3333333333333333333333333333333333333333333333333333}
              Layout = blGlyphTop
              NumGlyphs = 2
              ParentFont = False
              TabOrder = 4
              OnClick = BE5Click
            end
            object btnImport: TBitBtn
              Left = 0
              Top = -1
              Width = 49
              Height = 49
              Hint = 'Insert one New Record'
              Caption = 'Import'
              Enabled = False
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = []
              Glyph.Data = {
                
                3000333333333333377733333333333333333333333333333333}
              Layout = blGlyphTop
              NumGlyphs = 2
              ParentFont = False
              TabOrder = 5
              OnClick = btnImportClick
            end
          end
          object DBGrid4: TDBGridEh
            Left = 1
            Top = 333
            Width = 774
            Height = 172
            Align = alClient
            DataSource = DS4
            DynProps = <>
            FooterParams.Color = clWindow
            TabOrder = 3
            TitleParams.MultiTitle = True
            OnEditButtonClick = DBGrid4EditButtonClick
            OnGetCellParams = DBGrid4GetCellParams
            Columns = <
              item
                CellButtons = <>
                DynProps = <>
                EditButtons = <>
                FieldName = 'SBBH1'
                Footers = <>
                Title.Caption = #30446#37636#32232#34399'1|SBBH1'
              end
              item
                CellButtons = <>
                DynProps = <>
                EditButtons = <>
                FieldName = 'VWPM'
                Footers = <>
                Title.Caption = #36234#25991#21517#31281'|VWPM'
                Width = 100
              end
              item
                CellButtons = <>
                DynProps = <>
                EditButtons = <>
                FieldName = 'Qty'
                Footers = <>
                Title.Caption = #25976#37327'|Qty'
              end
              item
                CellButtons = <>
                DynProps = <>
                EditButtons = <>
                FieldName = 'USERID'
                Footers = <>
                Title.Caption = #20351#29992#32773'|USERID'
              end
              item
                CellButtons = <>
                DisplayFormat = 'YYYY/MM/DD'
                DynProps = <>
                EditButtons = <>
                FieldName = 'USERDATE'
                Footers = <>
                Title.Caption = #20351#29992#32773#26085#26399'|USERDATE'
              end
              item
                CellButtons = <>
                DynProps = <>
                EditButtons = <>
                FieldName = 'YN'
                Footers = <>
                Width = 27
              end>
            object RowDetailData: TRowDetailPanelControlEh
            end
          end
        end
      end
    end
  end
  object EnMas: TQuery
    AfterOpen = EnMasAfterOpen
    AfterScroll = EnMasAfterScroll
    DatabaseName = 'DB'
    SQL.Strings = (
      
        'select TSCD_KCRK.*,BDepartment.DepName,TSCD_ZSZL.ZSJC_YW,TSCD_ZS' +
        'ZL.ZSJC_ZW'
      'from TSCD_KCRK'
      'left join  BDepartment on BDepartment.ID=TSCD_KCRK.DepID '
      'left join TSCD_ZSZL on TSCD_ZSZL.zsdh=TSCD_KCRK.ZSBH '
      'where TSCD_KCRK.RKNO like '#39'%'#39' '
      
        'and convert(smalldatetime,convert(varchar,TSCD_KCRK.USERDATE,111' +
        ')) between '
      #39'2020/03/24'#39
      ' and '
      #39'2020/03/27'#39
      'and TSCD_KCRK.USERID='#39'21191'#39
      'and TSCD_KCRK.LB='#39'NNNNNNNNNN'#39' '
      'order by RKNO DESC')
    UpdateObject = UpMas
    Left = 204
    Top = 315
    object EnMasRKNO: TStringField
      FieldName = 'RKNO'
      FixedChar = True
      Size = 11
    end
    object EnMasGSBH: TStringField
      FieldName = 'GSBH'
      FixedChar = True
      Size = 4
    end
    object EnMasLB: TStringField
      FieldName = 'LB'
      FixedChar = True
    end
    object EnMasZSNO: TStringField
      FieldName = 'ZSNO'
      OnSetText = EnMasZSNOSetText
      FixedChar = True
      Size = 11
    end
    object EnMasSOHIEU: TStringField
      FieldName = 'SOHIEU'
      FixedChar = True
    end
    object EnMasDOCNO: TStringField
      DisplayWidth = 50
      FieldName = 'DOCNO'
      FixedChar = True
      Size = 50
    end
    object EnMasDOCDate: TDateTimeField
      FieldName = 'DOCDate'
      DisplayFormat = 'YYYY/MM/DD'
    end
    object EnMasHDNO: TStringField
      DisplayWidth = 50
      FieldName = 'HDNO'
      FixedChar = True
      Size = 50
    end
    object EnMasHDDate: TDateTimeField
      FieldName = 'HDDate'
      DisplayFormat = 'YYYY/MM/DD'
    end
    object EnMasZSBH: TStringField
      FieldName = 'ZSBH'
      FixedChar = True
      Size = 6
    end
    object EnMasZSJC_YW: TStringField
      FieldName = 'ZSJC_YW'
      FixedChar = True
    end
    object EnMasZSJC_ZW: TStringField
      FieldName = 'ZSJC_ZW'
      FixedChar = True
      Size = 50
    end
    object EnMasDepID: TStringField
      FieldName = 'DepID'
      FixedChar = True
      Size = 10
    end
    object EnMasDepName: TStringField
      FieldName = 'DepName'
      FixedChar = True
      Size = 50
    end
    object EnMasflowflag: TStringField
      FieldName = 'flowflag'
      FixedChar = True
      Size = 1
    end
    object EnMasUSERDATE: TDateTimeField
      FieldName = 'USERDATE'
      DisplayFormat = 'YYYY/MM/DD'
    end
    object EnMasUSERID: TStringField
      FieldName = 'USERID'
      FixedChar = True
      Size = 10
    end
    object EnMasCFMID: TStringField
      FieldName = 'CFMID'
      FixedChar = True
      Size = 10
    end
    object EnMasCFMDATE: TDateTimeField
      FieldName = 'CFMDATE'
      DisplayFormat = 'YYYY/MM/DD'
    end
    object EnMasYN: TStringField
      FieldName = 'YN'
      FixedChar = True
      Size = 1
    end
  end
  object UpMas: TUpdateSQL
    ModifySQL.Strings = (
      'update TSCD_KCRK'
      'set'
      '  LB =:LB,'
      '  ZSBH=:ZSBH,'
      '  DepID =:DepID,'
      '  SOHIEU =:SOHIEU,'
      '  DOCNO =:DOCNO,'
      '  HDNO =:HDNO,'
      '  DOCDate =:DOCDate,'
      '  HDDate =:HDDate,'
      '  ZSNO =:ZSNO,'
      '  USERID =:USERID,'
      '  USERDATE =:USERDATE,'
      '  CFMID =:CFMID,'
      '  CFMDate =:CFMDate,'
      '  YN =:YN'
      'where'
      '  RKNO =:OLD_RKNO')
    InsertSQL.Strings = (
      'insert into TSCD_KCRK'
      
        '  (RKNO, GSBH, LB, DepID, ZSBH, ZSNO, SOHIEU,DOCNO, HDNO,DOCDate' +
        ', HDDate, USERID, USERDATE, CFMID, CFMDate, YN)'
      'values'
      
        '  (:RKNO, :GSBH, :LB, :DepID, :ZSBH, :ZSNO,:SOHIEU, :DOCNO, :HDN' +
        'O, :DOCDate, :HDDate, :USERID, :USERDATE, :CFMID, :CFMDate, :YN)')
    DeleteSQL.Strings = (
      'delete from TSCD_KCRK'
      'where'
      '  RKNO = :OLD_RKNO')
    Left = 204
    Top = 381
  end
  object DS1: TDataSource
    DataSet = EnMas
    Left = 239
    Top = 316
  end
  object UPDet: TUpdateSQL
    ModifySQL.Strings = (
      'update TSCD_KCRKS'
      'set'
      '  SBBH =:SBBH,'
      '  Qty =:Qty,'
      '  USPrice =:USPrice,'
      '  VNPrice =:VNPrice,'
      '  USERID =:USERID,'
      '  USERDATE =:USERDATE,'
      '  YN =:YN'
      'where'
      '  RKNO =:OLD_RKNO and'
      '  SBBH =:OLD_SBBH'
      '')
    InsertSQL.Strings = (
      'insert into TSCD_KCRKS'
      '  (RKNO,  Qty, SBBH, VNPrice, USPrice , USERDATE, USERID, YN)'
      'values'
      
        '  (:RKNO,  :Qty, :SBBH ,:VNPrice, :USPrice , :USERDATE, :USERID,' +
        ' :YN)'
      ''
      '')
    DeleteSQL.Strings = (
      'delete from TSCD_KCRKS'
      'where'
      '  RKNO = :OLD_RKNO and '
      '  SBBH=:OLD_SBBH'
      '')
    Left = 286
    Top = 382
  end
  object EnDet: TQuery
    AfterOpen = EnDetAfterOpen
    DatabaseName = 'DB'
    DataSource = DS1
    SQL.Strings = (
      
        'select TSCD_KCRKS.RKNO,TSCD_SB.SBBH,TSCD_KCRKS.USERID,TSCD_KCRKS' +
        '.USERDATE,TSCD_KCRKS.YN'
      
        '       ,TSCD_SB.YWPM,TSCD_SB.ZWPM,TSCD_SB.VWPM,TSCD_SB.LSBH,TSCD' +
        '_SB.QUCBH,TSCD_KCRKS.Qty'
      '       ,TSCD_KCRKS.VNPrice,TSCD_KCRKS.USPrice'
      'from TSCD_KCRKS'
      'left join TSCD_SB on TSCD_SB.SBBH = TSCD_KCRKS.SBBH'
      'where TSCD_KCRKS.RKNO=:RKNO'
      'order by TSCD_KCRKS.RKNO')
    UpdateObject = UPDet
    Left = 294
    Top = 318
    ParamData = <
      item
        DataType = ftFixedChar
        Name = 'RKNO'
        ParamType = ptUnknown
        Size = 12
      end>
    object EnDetRKNO: TStringField
      FieldName = 'RKNO'
      Origin = 'DB.TSCD_KCRKS.RKNO'
      FixedChar = True
      Size = 11
    end
    object EnDetSBBH: TStringField
      FieldName = 'SBBH'
      Origin = 'DB.TSCD_SB.SBBH'
      FixedChar = True
      Size = 15
    end
    object EnDetUSERID: TStringField
      FieldName = 'USERID'
      Origin = 'DB.TSCD_KCRKS.USERID'
      FixedChar = True
      Size = 10
    end
    object EnDetUSERDATE: TDateTimeField
      FieldName = 'USERDATE'
      Origin = 'DB.TSCD_KCRKS.USERDATE'
    end
    object EnDetYN: TStringField
      FieldName = 'YN'
      Origin = 'DB.TSCD_KCRKS.YN'
      FixedChar = True
      Size = 1
    end
    object EnDetYWPM: TStringField
      FieldName = 'YWPM'
      Origin = 'DB.TSCD_SB.YWPM'
      FixedChar = True
      Size = 200
    end
    object EnDetZWPM: TStringField
      FieldName = 'ZWPM'
      Origin = 'DB.TSCD_SB.ZWPM'
      FixedChar = True
      Size = 200
    end
    object EnDetLSBH: TStringField
      FieldName = 'LSBH'
      Origin = 'DB.TSCD_SB.LSBH'
      FixedChar = True
      Size = 30
    end
    object EnDetQUCBH: TStringField
      FieldName = 'QUCBH'
      Origin = 'DB.TSCD_SB.QUCBH'
      FixedChar = True
      Size = 30
    end
    object EnDetQty: TFloatField
      FieldName = 'Qty'
      Origin = 'DB.TSCD_KCRKS.Qty'
    end
    object EnDetVNPrice: TCurrencyField
      FieldName = 'VNPrice'
      Origin = 'DB.TSCD_KCRKS.VNPrice'
    end
    object EnDetUSPrice: TFloatField
      FieldName = 'USPrice'
      Origin = 'DB.TSCD_KCRKS.USPrice'
    end
    object EnDetVWPM: TStringField
      FieldName = 'VWPM'
      FixedChar = True
      Size = 250
    end
  end
  object DS2: TDataSource
    DataSet = EnDet
    Left = 326
    Top = 317
  end
  object Qtemp: TQuery
    DatabaseName = 'DB'
    Left = 528
    Top = 316
  end
  object BDelRec: TQuery
    DatabaseName = 'DB'
    Left = 492
    Top = 316
  end
  object TSCDQry: TQuery
    AfterOpen = TSCDQryAfterOpen
    DatabaseName = 'DB'
    DataSource = DS2
    SQL.Strings = (
      
        'select TSCD.RKNO,TSCD.TSID,TSCD.TSBH,TSCD_SB.SBBH,TSCD.XSBH,TSCD' +
        '.NSX,TSCD.InDATE,TSCD.Status,TSCD.DepID,BDepartment.DepName,TSCD' +
        '.DepID_Memo,TSCD.USERID,TSCD.USERDATE,TSCD.YN'
      
        '       ,TSCD_SB.YWPM,TSCD_SB.ZWPM,TSCD_SB.ZSDH,TSCD_ZSZL.ZSJC_YW' +
        ',TSCD_SB.LSBH,TSCD_SB.QUCBH,TSCD.Qty,TSCD.CFMID,TSCD.CFMID_LL, T' +
        'SCD.CFMID_RK,TSCD.LB,TSCD.Unit_Name'
      'from TSCD'
      'left join TSCD_SB on TSCD_SB.SBBH = TSCD.SBBH'
      'left join TSCD_ZSZL on TSCD_SB.ZSDH = TSCD_ZSZL.zsdh'
      
        'left join TSCD_BDepartment BDepartment on BDepartment.ID = TSCD.' +
        'DepID'
      'where TSCD.RKNO=:RKNO and TSCD.SBBH=:SBBH and TSCD.Status<>'#39'I'#39)
    UpdateObject = UpdTSCD
    Left = 382
    Top = 318
    ParamData = <
      item
        DataType = ftFixedChar
        Name = 'RKNO'
        ParamType = ptUnknown
        Size = 12
      end
      item
        DataType = ftFixedChar
        Name = 'SBBH'
        ParamType = ptUnknown
        Size = 16
      end>
    object TSCDQryTSID: TStringField
      FieldName = 'TSID'
      FixedChar = True
      Size = 11
    end
    object TSCDQryTSBH: TStringField
      FieldName = 'TSBH'
      FixedChar = True
      Size = 15
    end
    object TSCDQrySBBH: TStringField
      FieldName = 'SBBH'
      FixedChar = True
      Size = 15
    end
    object TSCDQryXSBH: TStringField
      FieldName = 'XSBH'
      FixedChar = True
      Size = 30
    end
    object TSCDQryNSX: TStringField
      FieldName = 'NSX'
      FixedChar = True
      Size = 10
    end
    object TSCDQryInDATE: TDateTimeField
      FieldName = 'InDATE'
    end
    object TSCDQryDepID: TStringField
      FieldName = 'DepID'
      FixedChar = True
      Size = 10
    end
    object TSCDQryDepName: TStringField
      FieldName = 'DepName'
      FixedChar = True
      Size = 50
    end
    object TSCDQryDepID_Memo: TStringField
      FieldName = 'DepID_Memo'
      FixedChar = True
    end
    object TSCDQryYWPM: TStringField
      FieldName = 'YWPM'
      FixedChar = True
      Size = 200
    end
    object TSCDQryZWPM: TStringField
      FieldName = 'ZWPM'
      FixedChar = True
      Size = 200
    end
    object TSCDQryZSDH: TStringField
      FieldName = 'ZSDH'
      FixedChar = True
      Size = 6
    end
    object TSCDQryZSJC_YW: TStringField
      FieldName = 'ZSJC_YW'
      FixedChar = True
      Size = 50
    end
    object TSCDQryLSBH: TStringField
      FieldName = 'LSBH'
      FixedChar = True
      Size = 30
    end
    object TSCDQryQUCBH: TStringField
      FieldName = 'QUCBH'
      FixedChar = True
      Size = 30
    end
    object TSCDQryStatus: TStringField
      FieldName = 'Status'
      FixedChar = True
      Size = 2
    end
    object TSCDQryQty: TFloatField
      FieldName = 'Qty'
    end
    object TSCDQryRKNO: TStringField
      FieldName = 'RKNO'
      FixedChar = True
      Size = 11
    end
    object TSCDQryCFMID: TStringField
      FieldName = 'CFMID'
      FixedChar = True
      Size = 10
    end
    object TSCDQryCFMID_LL: TStringField
      FieldName = 'CFMID_LL'
      FixedChar = True
      Size = 10
    end
    object TSCDQryCFMID_RK: TStringField
      FieldName = 'CFMID_RK'
      FixedChar = True
      Size = 10
    end
    object TSCDQryLB: TStringField
      FieldName = 'LB'
      FixedChar = True
      Size = 1
    end
    object TSCDQryUSERDATE: TDateTimeField
      FieldName = 'USERDATE'
    end
    object TSCDQryUSERID: TStringField
      FieldName = 'USERID'
      FixedChar = True
      Size = 10
    end
    object TSCDQryYN: TStringField
      FieldName = 'YN'
      FixedChar = True
      Size = 1
    end
    object TSCDQryUnit_Name: TStringField
      FieldName = 'Unit_Name'
    end
  end
  object DS3: TDataSource
    DataSet = TSCDQry
    Left = 422
    Top = 317
  end
  object UpdTSCD: TUpdateSQL
    ModifySQL.Strings = (
      'Update TSCD'
      'set'
      '  TSBH =:TSBH,'
      '  SBBH =:SBBH,'
      '  Qty =:Qty,'
      '  XSBH =:XSBH,'
      '  NSX =:NSX,'
      '  InDate =:InDate,'
      '  DepID =:DepID,'
      '  DepID_Memo =:DepID_Memo,'
      '  USERID =:USERID,'
      '  USERDATE =:USERDATE,'
      '  CFMID =:CFMID,'
      '  CFMID_LL =:CFMID_LL,'
      '  CFMID_RK =:CFMID_RK,'
      '  YN =:YN,'
      '  RKNO=:RKNO,'
      ' Unit_Name=:Unit_Name'
      'where'
      '  TSID =:OLD_TSID')
    InsertSQL.Strings = (
      'insert into TSCD'
      
        '  (TSID , TSBH  , SBBH, Qty, XSBH, NSX, InDate, DepID, DepID_Mem' +
        'o,'
      
        '   USERID ,USERDATE ,YN , CFMID, CFMID_RK, CFMID_LL, RKNO, LB, S' +
        'tatus,Unit_Name)'
      'values'
      
        '  (:TSID , :TSBH  , :SBBH, :Qty, :XSBH, :NSX, :InDate, :DepID, :' +
        'DepID_Memo,'
      
        '    :USERID , :USERDATE , :YN , :CFMID, :CFMID_RK, :CFMID_LL, :R' +
        'KNO, :LB, :Status,:Unit_Name)')
    DeleteSQL.Strings = (
      'delete TSCD'
      'where'
      '  TSID =:OLD_TSID')
    Left = 382
    Top = 390
  end
  object KCPK: TQuery
    DatabaseName = 'DB'
    DataSource = DS1
    SQL.Strings = (
      'select *'
      'from TSCD_KCPK'
      'where PKNO=:RKNO')
    UpdateObject = UpKCPK
    Left = 912
    Top = 397
    ParamData = <
      item
        DataType = ftFixedChar
        Name = 'RKNO'
        ParamType = ptUnknown
        Size = 12
      end>
    object KCPKPKNO: TStringField
      FieldName = 'PKNO'
      Origin = 'DB.KCPK.PKNO'
      FixedChar = True
      Size = 11
    end
    object KCPKDeclaretion: TStringField
      FieldName = 'Declaretion'
      Origin = 'DB.KCPK.Declaretion'
      FixedChar = True
    end
    object KCPKExport: TStringField
      FieldName = 'Export'
      Origin = 'DB.KCPK.Export'
      FixedChar = True
      Size = 5
    end
    object KCPKTKDate: TDateTimeField
      FieldName = 'TKDate'
      Origin = 'DB.TSCD_KCPK.TKDate'
      DisplayFormat = 'YYYY/MM/DD'
    end
    object KCPKUserID: TStringField
      FieldName = 'UserID'
      Origin = 'DB.KCPK.UserID'
      FixedChar = True
      Size = 15
    end
    object KCPKUserDate: TDateTimeField
      FieldName = 'UserDate'
      Origin = 'DB.KCPK.UserDate'
    end
    object KCPKYN: TStringField
      FieldName = 'YN'
      Origin = 'DB.KCPK.YN'
      FixedChar = True
      Size = 1
    end
  end
  object UpKCPK: TUpdateSQL
    ModifySQL.Strings = (
      'update TSCD_KCPK'
      'set '
      ' Declaretion=:Declaretion,'
      ' Export=:Export,'
      ' TKDate=:TKDate,'
      ' UserID=:UserID,'
      ' UserDate=:UserDate,'
      ' YN=:YN'
      'where PKNO=:OLD_PKNO'
      'and Declaretion=:OLD_Declaretion')
    InsertSQL.Strings = (
      'Insert into TSCD_KCPK'
      '(PKNO, Declaretion, Export,TKDate, UserID, UserDate, YN)'
      'values'
      '(:PKNO, :Declaretion, :Export,:TKDate, :UserID, :UserDate, :YN)')
    DeleteSQL.Strings = (
      'Delete TSCD_KCPK'
      'where PKNO=:OLD_PKNO'
      'and Declaretion=:OLD_Declaretion')
    Left = 913
    Top = 429
  end
  object DSKCPK: TDataSource
    DataSet = KCPK
    Left = 950
    Top = 397
  end
  object EnDetS: TQuery
    AfterOpen = EnDetSAfterOpen
    DatabaseName = 'DB'
    DataSource = DS2
    SQL.Strings = (
      'select TSCD_KCRKSS.*,TSCD_SB.VWPM'
      'from TSCD_KCRKSS'
      'left join TSCD_SB on TSCD_SB.SBBH = TSCD_KCRKSS.SBBH1'
      'where TSCD_KCRKSS.RKNO=:RKNO'
      'and TSCD_KCRKSS.SBBH=:SBBH'
      'order by TSCD_KCRKSS.SBBH1')
    UpdateObject = UpDetS
    Left = 914
    Top = 618
    ParamData = <
      item
        DataType = ftFixedChar
        Name = 'RKNO'
        ParamType = ptUnknown
        Size = 12
      end
      item
        DataType = ftFixedChar
        Name = 'SBBH'
        ParamType = ptUnknown
        Size = 16
      end>
    object EnDetSRKNO: TStringField
      FieldName = 'RKNO'
      Origin = 'DB.TSCD_KCRKSS.RKNO'
      FixedChar = True
      Size = 11
    end
    object EnDetSSBBH: TStringField
      FieldName = 'SBBH'
      Origin = 'DB.TSCD_KCRKSS.SBBH'
      FixedChar = True
      Size = 15
    end
    object EnDetSSBBH1: TStringField
      FieldName = 'SBBH1'
      Origin = 'DB.TSCD_KCRKSS.SBBH1'
      FixedChar = True
      Size = 15
    end
    object EnDetSQty: TFloatField
      FieldName = 'Qty'
      Origin = 'DB.TSCD_KCRKSS.Qty'
    end
    object EnDetSVNPrice: TCurrencyField
      FieldName = 'VNPrice'
      Origin = 'DB.TSCD_KCRKSS.VNPrice'
      DisplayFormat = '#,##0'
    end
    object EnDetSUSPrice: TFloatField
      FieldName = 'USPrice'
      Origin = 'DB.TSCD_KCRKSS.USPrice'
      DisplayFormat = '#,##0.00'
    end
    object EnDetSUSERID: TStringField
      FieldName = 'USERID'
      Origin = 'DB.TSCD_KCRKSS.USERID'
      FixedChar = True
      Size = 10
    end
    object EnDetSUSERDATE: TDateTimeField
      FieldName = 'USERDATE'
      Origin = 'DB.TSCD_KCRKSS.USERDATE'
    end
    object EnDetSYN: TStringField
      FieldName = 'YN'
      Origin = 'DB.TSCD_KCRKSS.YN'
      FixedChar = True
      Size = 1
    end
    object EnDetSVWPM: TStringField
      FieldName = 'VWPM'
      FixedChar = True
      Size = 255
    end
  end
  object DS4: TDataSource
    DataSet = EnDetS
    Left = 950
    Top = 619
  end
  object UpDetS: TUpdateSQL
    ModifySQL.Strings = (
      'update TSCD_KCRKSS'
      'set'
      '  SBBH =:SBBH,'
      '  SBBH1 =:SBBH1,'
      '  Qty =:Qty,'
      '  USPrice =:USPrice,'
      '  VNPrice =:VNPrice,'
      '  USERID =:USERID,'
      '  USERDATE =:USERDATE,'
      '  YN =:YN'
      'where'
      '  RKNO =:OLD_RKNO and'
      '  SBBH =:OLD_SBBH and'
      '  SBBH1 =:OLD_SBBH1'
      ''
      '')
    InsertSQL.Strings = (
      'insert into TSCD_KCRKSS'
      
        '  (RKNO,  SBBH, SBBH1,  Qty, VNPrice, USPrice , USERDATE, USERID' +
        ', YN)'
      'values'
      
        '  (:RKNO,  :SBBH , :SBBH1, :Qty, :VNPrice, :USPrice , :USERDATE,' +
        ' :USERID, :YN)')
    DeleteSQL.Strings = (
      'delete from TSCD_KCRKSS'
      'where'
      '  RKNO = :OLD_RKNO and '
      '  SBBH=:OLD_SBBH and'
      '  SBBH1=:OLD_SBBH1'
      '')
    Left = 913
    Top = 662
  end
end
