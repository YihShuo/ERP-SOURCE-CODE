...
type
  TDeliver_EquipmentR = class(TForm)
    Panel1: TPanel;
    Label18: TLabel;
    BB2: TBitBtn;
    BB3: TBitBtn;
...
var
  Deliver_EquipmentR: TDeliver_EquipmentR;
  NDate:TDate;

implementation

uses main1, EquipmentR_Dep1, EquipmentR1, EquipmentR_DepName1, Deliver_EquipmentR_print1;

{$R *.dfm}
//
procedure TDeliver_EquipmentR.Init_LB();
begin
  LB:='RRRRRRRRRR';
end;
//異動單 Column Show or Not
procedure TDeliver_EquipmentR.DelDetAfterOpen_ColumnShow();
...
//
procedure TDeliver_EquipmentR.DelDet_LB_Edit();
begin
  with DelDet do
  begin
    Edit;
...
procedure TDeliver_EquipmentR.FormCreate(Sender: TObject);
var i:integer;
begin
  with Qtemp do
  begin
...
procedure TDeliver_EquipmentR.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if BB5.enabled  then
  begin
...
procedure TDeliver_EquipmentR.FormDestroy(Sender: TObject);
begin
  Deliver_EquipmentR:=nil;
end;

procedure TDeliver_EquipmentR.Button1Click(Sender: TObject);
begin
  Init_LB();
  with DelMas do
  begin
    active:=false;
    sql.Clear;
    sql.add('select TSCD_KCRR.*,BDepartment.DepName  ');
    sql.add('from TSCD_KCRR');
    sql.add('left join TSCD_BDepartment BDepartment on TSCD_KCRR.DepID=BDepartment.ID');
    sql.add('where ISNULL(flowflag,'''')<>''X'' and TSCD_KCRR.LLNO like ');
    sql.add(''''+edit1.Text+'%'+'''');
    ...
    sql.add('and convert(smalldatetime,convert(varchar,TSCD_KCRR.USERDATE,111)) between ');
    sql.add(''''+formatdatetime('yyyy/MM/dd',DTP1.Date) +'''');
    sql.add(' and  ');
    sql.add(''''+formatdatetime('yyyy/MM/dd',DTP2.Date) +'''');
    sql.add('and TSCD_KCRR.LB='''+LB+''' ');
    if Check.Checked then
    begin
      sql.Add('and TSCD_KCRR.USERID='+''''+main.Edit1.Text+'''');
    end;
    sql.add('and TSCD_KCRR.LB='''+LB+''' ');
    SQL.Add('and TSCD_KCRR.GSBH='''+main.Edit2.Text+''' ');
...
procedure TDeliver_EquipmentR.BBTT1Click(Sender: TObject);
begin
  EquipmentR_Dep:=TEquipmentR_Dep.Create(self);
  EquipmentR_Dep.Show;
end;

procedure TDeliver_EquipmentR.BB1Click(Sender: TObject);
begin
   panel2.Visible:=true;
end;

procedure TDeliver_EquipmentR.BB2Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive:=true;
...
procedure TDeliver_EquipmentR.BB4Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive:=true;
...
procedure TDeliver_EquipmentR.BB5Click(Sender: TObject);
var y,m,a:string;
    i:integer;
begin
  with Qtemp do
                ...
                DelMas.delete;
                 ...
                 begin
                     ...
                     begin
                       active:=false;
                       sql.Clear;
                       sql.Add('select LLNO from TSCD_KCRR where LLNO like '+''''+y+m+'%'+'''');
                   ...
                   if DelMas.fieldbyname('YN').value='0'then
                     begin
                         ...
                         begin
                           ...
                           sql.add('values ('+''''+'TSCD_KCRR'+''''+','+''''+DelMas.fieldbyname('LLNO').Value+'''');
                           sql.add(','+''''+' '+''''+','+''''+DelMas.fieldbyname('USERID').Value+''''+',');
                           sql.add(''''+main.Edit1.Text+''''+',getdate())');  }
                           SQL.Add('Update TSCD_KCRR Set flowflag=''X'' where LLNO='''+DelMas.FieldByName('LLNO').AsString+''' ');
                           execsql;
                           active:=false;
                         end;
...
procedure TDeliver_EquipmentR.BD2Click(Sender: TObject);
begin
...
procedure TDeliver_EquipmentR.DBGrid2EditButtonClick(Sender: TObject);
var Style: String;
    i:integer;
begin
  ...
  begin
    EquipmentR_DepName:=TEquipmentR_DepName.Create(self);
    EquipmentR_DepName.COLUMN_Index:=1;
    EquipmentR_DepName.Show;
  end;
end;

procedure TDeliver_EquipmentR.BD3Click(Sender: TObject);
begin
  if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TDeliver_EquipmentR.BD4Click(Sender: TObject);
begin
  if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TDeliver_EquipmentR.BB3Click(Sender: TObject);
begin
...
procedure TDeliver_EquipmentR.BB6Click(Sender: TObject);
begin
  with DelMas do
  begin
    active:=false;
...
procedure TDeliver_EquipmentR.bbt6Click(Sender: TObject);
var   a:string;
...
begin
  ...
  begin
    active:=false;
    sql.Clear;
    sql.Add('select a.LLNO,tscd.TSBH, TSCD_SB.ZWPM,TSCD_SB.VWPM, b.COLUMN3, a.USERDATE, TSCD_BDepartment.DepName  from TSCD_KCRR A');
    sql.Add('left join TSCD_KCRRS B on a.LLNO=b.LLNO');
    sql.Add('left join tscd on b.TSID= tscd.TSID');
    sql.Add('left join TSCD_SB on tscd.SBBH=TSCD_SB.SBBH');
    sql.Add('left join TSCD_BDepartment on TSCD_BDepartment.ID=a.DepID');
    sql.Add('where');
    sql.add('CONVERT(smalldatetime, A.USERDATE, 111) BETWEEN ');
    sql.add(''''+formatdatetime('yyyy/MM/dd',DTP1.Date) +'''');
    sql.add(' and  ');
    sql.add(''''+formatdatetime('yyyy/MM/dd',DTP2.Date) +'''');
...
procedure TDeliver_EquipmentR.BD6Click(Sender: TObject);
begin
  with DelDet do
  begin
    active:=false;
...
procedure TDeliver_EquipmentR.PC1Change(Sender: TObject);
var i:integer;
begin
...
procedure TDeliver_EquipmentR.DelDetAfterOpen(DataSet: TDataSet);
var i:integer;
begin
...
//
Function TDeliver_EquipmentR.DelDet_LB_Check():boolean;
var iRes:boolean;
begin
  iRes:=true;
  with DelDet do
...
procedure TDeliver_EquipmentR.BD5Click(Sender: TObject);
var i:integer;
begin
                 ...
                 begin
                    ...
                    begin
                       ...
                       begin
                         ...
                         sql.add('insert into BDelRec ');
                         sql.add('(TableName ,TNO,CLBH,OldID,DelID,DelDate)');
                         sql.add('values ('+''''+'TSCD_KCRRS'+''''+','+''''+DelMas.fieldbyname('LLNO').Value+'''');
                         sql.add(','+''''+' '+''''+','+''''+DelMas.fieldbyname('USERID').Value+''''+',');
                         sql.add(''''+main.Edit1.Text+''''+',getdate())');
                         execsql;
                         active:=false;
...
procedure TDeliver_EquipmentR.BBTT2Click(Sender: TObject);
begin
  EquipmentR:=TEquipmentR.Create(self);
  EquipmentR.LLNOEdit.Text:=DelMas.FieldByName('Memo').AsString;
  EquipmentR.Show;
end;

procedure TDeliver_EquipmentR.DBGrid1GetCellParams(Sender: TObject;
...
procedure TDeliver_EquipmentR.DBGrid2GetCellParams(Sender: TObject;
  Column: TColumnEh; AFont: TFont; var Background: TColor;
  State: TGridDrawState);
begin
  if DelDet.FieldByName('YN').value='0' then
...
procedure TDeliver_EquipmentR.DBGrid1DblClick(Sender: TObject);
begin
...
procedure TDeliver_EquipmentR.DBGrid1EditButtonClick(Sender: TObject);
begin
...
procedure TDeliver_EquipmentR.BDT5Click(Sender: TObject);
begin
  ...
  if DelDet.RecordCount = 0  then
  begin
    showmessage('Pls insert the  Back/Return Equipment first');
  end else
  begin
    Deliver_EquipmentR_print:=TDeliver_EquipmentR_print.Create(self);
    Deliver_EquipmentR_print.quickrep1.prepare;
    Deliver_EquipmentR_print.quickrep1.preview;
    Deliver_EquipmentR_print.free;
  end;
end;
