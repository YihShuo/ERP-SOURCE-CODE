unit Deliver_EquipmentR_print1;
...
type
  TDeliver_EquipmentR_print = class(TForm)
    QuickRep1: TQuickRep;
...
var
  Deliver_EquipmentR_print: TDeliver_EquipmentR_print;

implementation
  uses Deliver_EquipmentR1, main1;
{$R *.dfm}

procedure TDeliver_EquipmentR_print.Readini();
var MyIni :Tinifile;
    AppDir,ComName,ComNameCH,AssetsN216_ISO:string;
    // trinh ky
...
begin
  ...
  begin
    try
      ...
      AssetsN216_ISO:=MyIni.ReadString('ERP','AssetsN216_ISO','T-DCC-103A(ERP)');
      //
...
procedure TDeliver_EquipmentR_print.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=CaFree;
end;

procedure TDeliver_EquipmentR_print.FormCreate(Sender: TObject);
begin
  ReadIni();
  //簽核系統審核  20221215
  ...
  begin
    ...
    SQL.Add('      WHERE FORM_TABLE = ''TSCD_KCLL'' AND STATUS IN (''N'', ''Z'') AND FORM_PRESENTATION LIKE ''%'+Deliver_EquipmentR.DelMas.FieldByName('LLNO').AsString+'%''  ');
...
procedure TDeliver_EquipmentR_print.FormDestroy(Sender: TObject);
begin
  Deliver_EquipmentR_print:=nil;
end;
