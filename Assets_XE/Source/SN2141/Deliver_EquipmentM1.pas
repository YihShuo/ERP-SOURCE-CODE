...
type
  TDeliver_EquipmentM = class(TForm)
    Panel1: TPanel;
    Label18: TLabel;
    BB2: TBitBtn;
    BB3: TBitBtn;
...
var
  Deliver_EquipmentM: TDeliver_EquipmentM;
  NDate:TDate;

implementation

uses main1, EquipmentM_Dep1, EquipmentM_DepName1, EquipmentM1, Deliver_EquipmentM_print1
     ,Deliver_EquipmentM_printTW1;

{$R *.dfm}
//
procedure TDeliver_EquipmentM.Init_LB();
begin
  LB:='MMMMMMMMMM';
end;
//異動單 Column Show or Not
procedure TDeliver_EquipmentM.DelDetAfterOpen_ColumnShow();
...
//
procedure TDeliver_EquipmentM.DelDet_LB_Edit();
begin
  with DelDet do
  begin
    Edit;
    ...
    FieldByName('PARAMENT1').Value:='JDTS';
    FieldByName('PARAMENT2').Value:='JDBDATE';
    FieldByName('PARAMENT3').Value:='JDEDATE';
    FieldByName('PARAMENT4').Value:='DepID_Memo';
    FieldByName('PARAMENT5').Value:='Status';
    FieldByName('Value5').Value:='M';
    Post;
  end;
end;
procedure TDeliver_EquipmentM.FormCreate(Sender: TObject);
var i:integer;
begin
  with Qtemp do
  begin
...
procedure TDeliver_EquipmentM.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if BB5.enabled  then
  begin
...
procedure TDeliver_EquipmentM.FormDestroy(Sender: TObject);
begin
  Deliver_EquipmentM:=nil;
end;

procedure TDeliver_EquipmentM.Button1Click(Sender: TObject);
begin
  Init_LB();
  with DelMas do
  begin
    active:=false;
    sql.Clear;
    sql.add('select TSCD_KCMM.*,BDepartment.DepName  ');
    sql.add('from TSCD_KCMM');
    sql.add('left join TSCD_BDepartment BDepartment on TSCD_KCMM.DepID=BDepartment.ID');
    sql.add('where ISNULL(flowflag,'''')<>''X'' and TSCD_KCMM.LLNO like ');
    sql.add(''''+edit1.Text+'%'+'''');
    ...
    sql.add('and convert(smalldatetime,convert(varchar,TSCD_KCMM.USERDATE,111)) between ');
    sql.add(''''+formatdatetime('yyyy/MM/dd',DTP1.Date) +'''');
    sql.add(' and  ');
    sql.add(''''+formatdatetime('yyyy/MM/dd',DTP2.Date) +'''');
    sql.add('and TSCD_KCMM.LB='''+LB+''' ');
    if Check.Checked then
    begin
      sql.Add('and TSCD_KCMM.USERID='+''''+main.Edit1.Text+'''');
    end;
    sql.add('and TSCD_KCMM.LB='''+LB+''' ');
    SQL.Add('and TSCD_KCMM.GSBH='''+main.Edit2.Text+''' ');
...
procedure TDeliver_EquipmentM.BBTT1Click(Sender: TObject);
begin
  EquipmentM_Dep:=TEquipmentM_Dep.Create(self);
  EquipmentM_Dep.Show;
end;


procedure TDeliver_EquipmentM.BB1Click(Sender: TObject);
begin
   panel2.Visible:=true;
end;

procedure TDeliver_EquipmentM.BB2Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive:=true;
...
procedure TDeliver_EquipmentM.BB4Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive:=true;
...
procedure TDeliver_EquipmentM.BB5Click(Sender: TObject);
var y,m,a:string;
    i:integer;
begin
  with Qtemp do
                ...
                DelMas.delete;
                 ...
                 begin
                     ...
                     begin
                       active:=false;
                       sql.Clear;
                       sql.Add('select LLNO from TSCD_KCMM where LLNO like '+''''+y+m+'%'+'''');
                   ...
                   if DelMas.fieldbyname('YN').value='0'then
                     begin
                         ...
                         begin
                           ...
                           sql.add('values ('+''''+'TSCD_KCMM'+''''+','+''''+DelMas.fieldbyname('LLNO').Value+'''');
                           sql.add(','+''''+' '+''''+','+''''+DelMas.fieldbyname('USERID').Value+''''+',');
                           sql.add(''''+main.Edit1.Text+''''+',getdate())'); }
                           SQL.Add('Update TSCD_KCMM Set flowflag=''X'' where LLNO='''+DelMas.FieldByName('LLNO').AsString+''' ');
                           execsql;
                           active:=false;
                         end;
...
procedure TDeliver_EquipmentM.BD2Click(Sender: TObject);
begin
...
procedure TDeliver_EquipmentM.DBGrid2EditButtonClick(Sender: TObject);
var Style: String;
    i:integer;
begin
  ...
  begin
    EquipmentM_DepName:=TEquipmentM_DepName.Create(self);
    EquipmentM_DepName.COLUMN_Index:=1;
    EquipmentM_DepName.Show;
  end;
end;

procedure TDeliver_EquipmentM.BD3Click(Sender: TObject);
begin
 if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TDeliver_EquipmentM.BD4Click(Sender: TObject);
begin
 if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TDeliver_EquipmentM.BB3Click(Sender: TObject);
begin
...
procedure TDeliver_EquipmentM.BB6Click(Sender: TObject);
begin
  with DelMas do
  begin
    active:=false;
...
procedure TDeliver_EquipmentM.BD6Click(Sender: TObject);
begin
  with DelDet do
  begin
    active:=false;
...
procedure TDeliver_EquipmentM.PC1Change(Sender: TObject);
var i:integer;
begin
...
procedure TDeliver_EquipmentM.DelDetAfterOpen(DataSet: TDataSet);
var i:integer;
begin
...
//
Function TDeliver_EquipmentM.DelDet_LB_Check():boolean;
var iRes:boolean;
begin
  iRes:=true;
  ...
  with DelDet do
  begin
    if DelMas.fieldbyname('LB').AsString ='MMMMMMMMMM' then  //借出 Cho Muon
    begin
...
procedure TDeliver_EquipmentM.BD5Click(Sender: TObject);
var i:integer;
begin
                 ...
                 begin
                    ...
                    begin
                       ...
                       begin
                         ...
                         sql.add('insert into BDelRec ');
                         sql.add('(TableName ,TNO,CLBH,OldID,DelID,DelDate)');
                         sql.add('values ('+''''+'TSCD_KCMMS'+''''+','+''''+DelMas.fieldbyname('LLNO').Value+'''');
                         sql.add(','+''''+' '+''''+','+''''+DelMas.fieldbyname('USERID').Value+''''+',');
                         sql.add(''''+main.Edit1.Text+''''+',getdate())');
                         execsql;
                         active:=false;
...
procedure TDeliver_EquipmentM.BBTT2Click(Sender: TObject);
begin
  EquipmentM:=TEquipmentM.Create(self);
  EquipmentM.Show;
end;

procedure TDeliver_EquipmentM.DBGrid1GetCellParams(Sender: TObject;
...
procedure TDeliver_EquipmentM.DBGrid2GetCellParams(Sender: TObject;
  Column: TColumnEh; AFont: TFont; var Background: TColor;
  State: TGridDrawState);
begin
  if DelDet.FieldByName('YN').value='0' then
...
procedure TDeliver_EquipmentM.DBGrid1DblClick(Sender: TObject);
begin
...
procedure TDeliver_EquipmentM.DBGrid1EditButtonClick(Sender: TObject);
begin
...
procedure TDeliver_EquipmentM.BDT5Click(Sender: TObject);
begin
  ...
  if DelDet.RecordCount = 0  then
  begin
    showmessage('Pls insert the Lend Equipment(借出設備) first ');
  end else
  begin
    Deliver_EquipmentM_print:=TDeliver_EquipmentM_print.Create(self);
    Deliver_EquipmentM_print.quickrep1.prepare;
    Deliver_EquipmentM_print.quickrep1.preview;
    Deliver_EquipmentM_print.free;
  end;
end;

procedure TDeliver_EquipmentM.BDT7Click(Sender: TObject);
begin
  if DelDet.RequestLive then
  begin
    showmessage('Pls save Master data first.');
  ...
  begin
    Deliver_EquipmentM_printTW:=TDeliver_EquipmentM_printTW.Create(self);
    Deliver_EquipmentM_printTW.quickrep1.prepare;
    Deliver_EquipmentM_printTW.quickrep1.preview;
    Deliver_EquipmentM_printTW.free;
  end;
