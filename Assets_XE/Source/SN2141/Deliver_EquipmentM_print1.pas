unit Deliver_EquipmentM_print1;
...
type
  TDeliver_EquipmentM_print = class(TForm)
    QuickRep1: TQuickRep;
...
var
  Deliver_EquipmentM_print: TDeliver_EquipmentM_print;

implementation
  uses Deliver_EquipmentM1, main1;
{$R *.dfm}

procedure TDeliver_EquipmentM_print.Readini();
var MyIni :Tinifile;
...
procedure TDeliver_EquipmentM_print.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=CaFree;
end;

procedure TDeliver_EquipmentM_print.FormCreate(Sender: TObject);
begin
  ReadIni();
  //簽核系統審核  20221215
  ...
  begin
    ...
    SQL.Add('      WHERE FORM_TABLE = ''TSCD_KCMM'' AND STATUS IN (''N'', ''Z'') AND FORM_PRESENTATION LIKE ''%'+Deliver_EquipmentM.DelMas.FieldByName('LLNO').AsString+'%''  ');
...
procedure TDeliver_EquipmentM_print.FormDestroy(Sender: TObject);
begin
  Deliver_EquipmentM_print:=nil;
end;
