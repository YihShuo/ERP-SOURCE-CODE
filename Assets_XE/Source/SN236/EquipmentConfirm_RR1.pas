...
type
  TEquipmentConfirm_RR = class(TForm)
    Panel1: TPanel;
    Label1: TLabel;
    Label2: TLabel;
    Button1: TButton;
...
var
  EquipmentConfirm_RR: TEquipmentConfirm_RR;
  NDate:TDate;
...
procedure TEquipmentConfirm_RR.DBGrid2ColumnShow();
var i:integer;
begin
  //
  for i:=5 to 22 do
  ...
  //
  with EquipmentConfirm_RR.DelMas do
  begin
...
procedure TEquipmentConfirm_RR.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TEquipmentConfirm_RR.FormDestroy(Sender: TObject);
begin
  EquipmentConfirm_RR:=nil;
end;

procedure TEquipmentConfirm_RR.FormCreate(Sender: TObject);
var i: integer;
begin
  with Qtemp do
  begin
...
procedure TEquipmentConfirm_RR.Button1Click(Sender: TObject);
begin
  with DelMas do
  begin
    DelDet.Active:=false;
    ...
    Active:=false;
    sql.Clear;
    sql.add('select TSCD_KCRR.*,BDepartment.DepName,CAST(dbo.fSignsToNoSigns(TSCD_LB.YWSM) AS VARCHAR(200)) as YWSM,TSCD_LB.ZWSM  ');
    sql.add('from TSCD_KCRR ');
    sql.add('left join TSCD_BDepartment BDepartment on TSCD_KCRR.DepID=BDepartment.ID  ');
    sql.Add('left join TSCD_LB on TSCD_LB.LB=TSCD_KCRR.LB ');
    sql.add('where 1=1 and ISNULL(flowflag,'''')<>''X'' ');
    if CheckBox1.Checked=false then
    SQL.Add(' and TSCD_KCRR.CFMID=''NO'' ');
    sql.add('and convert(smalldatetime,convert(varchar,TSCD_KCRR.USERDATE,111)) between'+''''+formatdatetime('yyyy/MM/dd',DTP1.date )+'''');
    sql.add('and '''+formatdatetime('yyyy/MM/dd',DTP2.date )+'''');
    sql.add('and TSCD_KCRR.LLNO like '+''''+edit1.Text+'%'+'''');
    //sql.add('and BDepartment.GSBH='+''''+main.edit2.text+'''');
    sql.add('order by TSCD_KCRR.LLNO  ');
...
procedure TEquipmentConfirm_RR.Button2Click(Sender: TObject);
var i:integer;
begin
...
procedure TEquipmentConfirm_RR.DelDetAfterOpen(DataSet: TDataSet);
begin
  DBGrid2ColumnShow();
end;
