...
type
  TApplyEquipment_printTW = class(TForm)
    QuickRep1: TQuickRep;
...
var
  ApplyEquipment_printTW: TApplyEquipment_printTW;

implementation
  uses main1, ApplyEquipment1, FunUnit;
{$R *.dfm}

procedure TApplyEquipment_printTW.Readini();
var MyIni :Tinifile;
    ComName,ComNameCH,AssetsN218_ISO:string;
    sline_PW,tempstr_PW:string;
    ad_PW,i:integer;
begin
  ComName:='CONG TY TNHH TY XUAN';
  ComNameCH:='億春責任有限公司';
  AssetsN218_ISO:='T-DCC-089B(ERP)';
  //
  ...
  begin
    try
      ...
      AssetsN218_ISO:=MyIni.ReadString('ERP','AssetsN218_ISO','T-DCC-089B(ERP)');
      //
  ...
  QRLabel7.Caption:=AssetsN218_ISO;
end;

procedure TApplyEquipment_printTW.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=CaFree;
end;

procedure TApplyEquipment_printTW.FormCreate(Sender: TObject);
begin
  Readini();
  //簽核系統審核  20221215
  ...
  begin
    ...
    SQL.Add('  MAX(CASE WHEN ((S_STEP_DESC = ''廠最高主管'' ) or (S_STEP_DESC = ''廠主管'' and LEVEL_NO>30) or (S_STEP_DESC = ''單位主管'' and LEVEL_NO>30) or (S_STEP_DESC = ''部門主管'' and LEVEL_NO>30) or (S_STEP_DESC = ''申請人'' and LEVEL_NO>30) )  THEN S_USER_ID END) AS TopManagerID,    ');
    SQL.Add('  MAX(CASE WHEN ((S_STEP_DESC = ''廠最高主管'' ) or (S_STEP_DESC = ''廠主管'' and LEVEL_NO>30) or (S_STEP_DESC = ''單位主管'' and LEVEL_NO>30) or (S_STEP_DESC = ''部門主管'' and LEVEL_NO>30) or (S_STEP_DESC = ''申請人'' and LEVEL_NO>30) ) THEN USERNAME END) AS TopManager,    ');
    SQL.Add('  MAX(CASE WHEN ((S_STEP_DESC = ''廠最高主管'' ) or (S_STEP_DESC = ''廠主管'' and LEVEL_NO>30) or (S_STEP_DESC = ''單位主管'' and LEVEL_NO>30) or (S_STEP_DESC = ''部門主管'' and LEVEL_NO>30) or (S_STEP_DESC = ''申請人'' and LEVEL_NO>30) ) THEN ApproveDate END) AS TopManagerDate,');
    SQL.Add('  MAX(CASE WHEN S_STEP_DESC = ''廠區最高主管''   THEN S_USER_ID END) AS TopDirectorID,    ');
    SQL.Add('  MAX(CASE WHEN S_STEP_DESC = ''廠區最高主管''   THEN USERNAME END) AS TopDirector,    ');
    SQL.Add('  MAX(CASE WHEN S_STEP_DESC = ''廠區最高主管''   THEN ApproveDate END) AS TopDirectorDate  ');
    SQL.Add('       FROM (  ');
    SQL.Add('      ');
    ...
    SQL.Add('      WHERE FORM_TABLE = ''TSCD_SGDH'' AND STATUS IN (''N'', ''Z'') AND FORM_PRESENTATION LIKE ''%'+ApplyEquipment.SGMas.FieldByName('SGNO').AsString+'%''  ');
   ...
   { if FieldByName('ApplicantID').AsString='' then
    begin
      ...
      SQL.Add('  MAX(CASE WHEN SITE_CODE = ''GD'' THEN ACCOUNT END) AS TopDirectorID,  ');
      SQL.Add('  MAX(CASE WHEN SITE_CODE = ''GD'' THEN NAME END) AS TopDirector,  ');
      SQL.Add('  MAX(CASE WHEN SITE_CODE = ''GD'' THEN FINISH_TIME END) AS TopDirectorDate  ');
      ...
      SQL.Add('             ROW_NUMBER() OVER (PARTITION BY FINISH_TIME ORDER BY FINISH_TIME DESC) AS RowID, TB_WKF_TASK_TRIGGER_RECORD.SITE_CODE, TB_EB_GROUP.LEV, APPGROUP.LEV APPGROUP_LEV, APPGROUP.GROUP_NAME APPGROUP ');
      SQL.Add('      FROM [UOF].[dbo].[TB_WKF_TASK] TB_WKF_TASK  ');
      SQL.Add('      LEFT JOIN [UOF].[dbo].[LYN_FormSigner] LYN_FormSigner on LYN_FormSigner.TableName=''''TSCD_SGDH'''' and LYN_FormSigner.Type=''''TSCD_SG'''' and LYN_FormSigner.FNO=TB_WKF_TASK.DOC_NBR ');
      ...
      SQL.Add('      WHERE ACTUAL_SIGNER IS NOT NULL AND LYN_FormSigner.TNO=''''' + ApplyEquipment.SGMas.FieldByName('SGNO').AsString + ''''''') AS OP  ');
    ...
    begin
      ...
      //
      CopyFile(Pchar('\\'+main.ServerIP+'\liy_erp\Images\'+FieldByName('TopDirectorID').AsString+'.bmp'),Pchar(AppDir+'Images\'+FieldByName('TopDirectorID').AsString+'.bmp'),false);
      if FileExists(AppDir+'Images\'+FieldByName('TopDirectorID').AsString+'.bmp') then
      begin
        TopDirector.Picture.LoadFromFile(AppDir+'Images\'+FieldByName('TopDirectorID').AsString+'.bmp');
      end;
    end;
    Active:=false;
  end;
...
procedure TApplyEquipment_printTW.FormDestroy(Sender: TObject);
begin
  ApplyEquipment_printTW:=nil;
end;
