unit Deliver_EquipmentD_print1;
...
type
  TDeliver_EquipmentD_print = class(TForm)
    QuickRep1: TQuickRep;
...
var
  Deliver_EquipmentD_print: TDeliver_EquipmentD_print;

implementation
  uses Deliver_EquipmentD1, main1;
{$R *.dfm}

procedure TDeliver_EquipmentD_print.Readini();
var MyIni :Tinifile;
...
procedure TDeliver_EquipmentD_print.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=CaFree;
end;

procedure TDeliver_EquipmentD_print.FormCreate(Sender: TObject);
begin
  ReadIni();
  //簽核系統審核  20221215
  ...
  begin
    ...
    begin
      ...
      CopyFile(Pchar('\\'+main.ServerIP+'\lys_erp\Images\'+FieldByName('ApplicantID').AsString+'.bmp'),Pchar(AppDir+'Images\'+FieldByName('ApplicantID').AsString+'.bmp'),false);
      ...
      //
      CopyFile(Pchar('\\'+main.ServerIP+'\lys_erp\Images\'+FieldByName('SupvervisorID').AsString+'.bmp'),Pchar(AppDir+'Images\'+FieldByName('SupvervisorID').AsString+'.bmp'),false);
      ...
      //
      CopyFile(Pchar('\\'+main.ServerIP+'\lys_erp\Images\'+FieldByName('LeaderID').AsString+'.bmp'),Pchar(AppDir+'Images\'+FieldByName('LeaderID').AsString+'.bmp'),false);
...
procedure TDeliver_EquipmentD_print.FormDestroy(Sender: TObject);
begin
  Deliver_EquipmentD_print:=nil;
end;
