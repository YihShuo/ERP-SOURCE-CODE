unit Deliver_EquipmentD_printTW1;
...
type
  TDeliver_EquipmentD_printTW = class(TForm)
    QuickRep1: TQuickRep;
    ...
    DelDetApplyDate: TStringField;
    DelDetCKRepair: TStringField;
    DelDetRepairResult: TStringField;
    DelDetUSERNAME: TStringField;
    DelDetCFMUserName: TStringField;
...
var
  Deliver_EquipmentD_printTW: TDeliver_EquipmentD_printTW;

implementation
  uses main1, Deliver_EquipmentD1;
{$R *.dfm}

procedure TDeliver_EquipmentD_printTW.ReadIni();
var MyIni :Tinifile;
...
begin
  ...
  begin
    try
      ...
      AssetsN219_ISO:=MyIni.ReadString('ERP','AssetsN219_ISO','T-DCC-082C');
      //
...
procedure TDeliver_EquipmentD_printTW.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=CaFree;
end;

procedure TDeliver_EquipmentD_printTW.FormCreate(Sender: TObject);
...
begin
  ...
  ApplyDateLabel.Caption:=DelDet.FieldByName('ApplyDate').AsString;
  ...
  begin
    All_AssetBH:=All_AssetBH+DelDet.FieldByName('TSBH').AsString+',';
    All_AssetName:=All_AssetName+DelDet.FieldByName('VWPM').AsString+DelDet.FieldByName('ZWPM').AsString+',';

    //26.12.2024 chi lay khi co du lieu Reason
    if not DelDet.FieldByName('Reason').IsNull and (Trim(DelDet.FieldByName('Reason').AsString) <> '') then
  ...
  //26.12.2024 Set co chu Ma so Tai San khi nhieu hon 10 ma thi giam size
  if (All_AssetBH <> '') and (Pos(',', All_AssetBH) > 0) then
  begin
    if Length(All_AssetBH) - Length(StringReplace(All_AssetBH, ',', '', [rfReplaceAll])) + 1 > 10 then
      AssetBHLabel.Font.Size := 7
    else
      AssetBHLabel.Font.Size := 9;
    end
    else
  AssetBHLabel.Font.Size := 9;
  //26.12.2024
...
procedure TDeliver_EquipmentD_printTW.FormDestroy(Sender: TObject);
begin
  Deliver_EquipmentD_printTW:=nil;
end;
