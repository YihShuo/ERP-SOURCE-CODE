...
uses Deliver_Equipment1, main1, FunUnit;

{$R *.dfm}

procedure TEquipment.Init_LB(LB:String);
begin
  if LB='LLLLLLLLLL' then
  begin
    CKRK.Checked:=true;
    CKLL.Checked:=false;
    CKBB.Checked:=false;
  ...
  if ((LB='SSSSSSSSSS') or (LB='RRRRRRRRRR')) then
  begin
    CKRK.Checked:=false;
    CKLL.Checked:=true;
    CKBB.Checked:=false;
...
procedure TEquipment.DBGridEh1DblClick(Sender: TObject);
begin
...
procedure TEquipment.Button1Click(Sender: TObject);
var SubSQL:String;
begin
  //
  with query1 do
  begin
    ...
    sql.Add('       TSCD_SB.LSBH,TSCD_SB.QUCBH,TSCD_SB.AuthCE,TSCD_SB.YWPM,TSCD_SB.ZWPM,Bdep_appy.DepName as applyDepname,TSCD_SB.VWPM ');
    sql.add('from TSCD ');
...
procedure TEquipment.FormDestroy(Sender: TObject);
begin
  Equipment:=nil;
end;

procedure TEquipment.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=Cafree;
end;

procedure TEquipment.Button2Click(Sender: TObject);
var i,j:integer;
    bm:Tbookmark;
