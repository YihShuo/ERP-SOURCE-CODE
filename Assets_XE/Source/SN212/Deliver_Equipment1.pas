...
type
  TDeliver_Equipment = class(TForm)
    Panel1: TPanel;
    Label18: TLabel;
    BB2: TBitBtn;
    BB3: TBitBtn;
...
var
  Deliver_Equipment: TDeliver_Equipment;
  NDate:TDate;

implementation

uses main1, Equipment_Dep1, Equipment_DepName1, Equipment1, Deliver_Equipment_print1;

{$R *.dfm}
//
procedure TDeliver_Equipment.Init_LB();
begin
  LB:='LLLLLLLLLL';
end;
//異動單 Column Show or Not
procedure TDeliver_Equipment.DelDetAfterOpen_ColumnShow();
...
begin
  ...
  for i:=5 to 16 do
    ...
    LL_GBox.Visible:=true;
    LL_GBox.Left:=GroupBox_Left;
  //
end;
//
procedure TDeliver_Equipment.DelDet_LB_Edit();
begin
  with DelDet do
  begin
    Edit;
    ...
    FieldByName('Value3').Value:='Y';
    Post;
  end;
end;
procedure TDeliver_Equipment.FormCreate(Sender: TObject);
var i:integer;
begin
...
procedure TDeliver_Equipment.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if BB5.enabled  then
  begin
...
procedure TDeliver_Equipment.FormDestroy(Sender: TObject);
begin
  Deliver_Equipment:=nil;
end;

procedure TDeliver_Equipment.Button1Click(Sender: TObject);
begin
  Init_LB();
  with DelMas do
  begin
    ...
    sql.add('where ISNULL(flowflag,'''')<>''X'' and TSCD_KCLL.LLNO like ');
    sql.add(''''+edit1.Text+'%'+'''');
    ...
    sql.add('and TSCD_KCLL.LB='''+LB+''' ');
    if Check.Checked then
    begin
      sql.Add('and TSCD_KCLL.USERID='+''''+main.Edit1.Text+'''');
    end;
    sql.add('and TSCD_KCLL.LB='''+LB+''' ');
    SQL.Add('and TSCD_KCLL.GSBH='''+main.Edit2.Text+''' ');
...
procedure TDeliver_Equipment.BBTT1Click(Sender: TObject);
begin
  Equipment_Dep:=TEquipment_Dep.Create(self);
  Equipment_Dep.Show;
end;


procedure TDeliver_Equipment.BB1Click(Sender: TObject);
begin
   panel2.Visible:=true;
end;

procedure TDeliver_Equipment.BB2Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive:=true;
...
procedure TDeliver_Equipment.BB4Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive:=true;
...
procedure TDeliver_Equipment.BB5Click(Sender: TObject);
var y,m,a:string;
    i:integer;
begin
  with Qtemp do
                ...
                DelMas.delete;
                 ...
                 begin
                     ...
                     begin
                       active:=false;
                       sql.Clear;
                       sql.Add('select LLNO from TSCD_KCLL where LLNO like '+''''+y+m+'%'+'''');
                   ...
                   if DelMas.fieldbyname('YN').value='0'then
                     begin
                         ...
                         begin
                           ...
                           sql.add('values ('+''''+'TSCD_KCLL'+''''+','+''''+DelMas.fieldbyname('LLNO').Value+'''');
                           sql.add(','+''''+' '+''''+','+''''+DelMas.fieldbyname('USERID').Value+''''+',');
                           sql.add(''''+main.Edit1.Text+''''+',getdate())');  }
                           SQL.Add('Update TSCD_KCLL Set flowflag=''X'' where LLNO='''+DelMas.FieldByName('LLNO').AsString+''' ');
                           execsql;
                           active:=false;
                         end;
...
procedure TDeliver_Equipment.BD2Click(Sender: TObject);
begin
...
procedure TDeliver_Equipment.DBGrid2EditButtonClick(Sender: TObject);
var Style: String;
    i:integer;
begin
  ...
  begin
    Equipment_DepName:=TEquipment_DepName.Create(self);
    Equipment_DepName.COLUMN_Index:=1;
    Equipment_DepName.Show;
  end;

end;

procedure TDeliver_Equipment.BD3Click(Sender: TObject);
begin
  if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TDeliver_Equipment.BD4Click(Sender: TObject);
begin
  if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TDeliver_Equipment.BB3Click(Sender: TObject);
begin
...
procedure TDeliver_Equipment.BB6Click(Sender: TObject);
begin
  with DelMas do
  begin
    active:=false;
...
procedure TDeliver_Equipment.bbt6Click(Sender: TObject);
var   a:string;
...
begin
  ...
  begin
    active:=false;
    sql.Clear;
    sql.Add('select a.LLNO,tscd.TSBH, TSCD_SB.ZWPM,TSCD_SB.VWPM, a.USERDATE, TSCD_BDepartment.DepName  from TSCD_KCLL A');
    sql.Add('left join TSCD_KCLLS B on a.LLNO=b.LLNO');
...
procedure TDeliver_Equipment.BD6Click(Sender: TObject);
begin
  with DelDet do
  begin
    active:=false;
...
procedure TDeliver_Equipment.PC1Change(Sender: TObject);
var i:integer;
begin
...
procedure TDeliver_Equipment.DelDetAfterOpen(DataSet: TDataSet);
var i:integer;
begin
...
//
Function TDeliver_Equipment.DelDet_LB_Check():boolean;
var iRes:boolean;
begin
  iRes:=true;
  with DelDet do
  begin
    if DelMas.fieldbyname('LB').AsString ='LLLLLLLLLL' then  //領設備 Lanh thiet bi
    begin
...
procedure TDeliver_Equipment.BD5Click(Sender: TObject);
var i:integer;
begin
                 ...
                 begin
                    ...
                    begin
                       ...
                       begin
                         ...
                         sql.add('insert into BDelRec ');
                         sql.add('(TableName ,TNO,CLBH,OldID,DelID,DelDate)');
                         sql.add('values ('+''''+'TSCD_KCLLS'+''''+','+''''+DelMas.fieldbyname('LLNO').Value+'''');
                         sql.add(','+''''+' '+''''+','+''''+DelMas.fieldbyname('USERID').Value+''''+',');
                         sql.add(''''+main.Edit1.Text+''''+',getdate())');
                         execsql;
                         active:=false;
...
procedure TDeliver_Equipment.BBTT2Click(Sender: TObject);
begin
  Equipment:=TEquipment.Create(self);
  Equipment.Init_LB(LB);
  Equipment.Show;
end;

procedure TDeliver_Equipment.DBGrid1GetCellParams(Sender: TObject;
...
procedure TDeliver_Equipment.DBGrid2GetCellParams(Sender: TObject;
  Column: TColumnEh; AFont: TFont; var Background: TColor;
  State: TGridDrawState);
begin
  if DelDet.FieldByName('YN').value='0' then
...
procedure TDeliver_Equipment.DBGrid1DblClick(Sender: TObject);
begin
...
procedure TDeliver_Equipment.DBGrid1EditButtonClick(Sender: TObject);
begin
...
procedure TDeliver_Equipment.BDT5Click(Sender: TObject);
begin
  ...
  if DelDet.RecordCount = 0  then
  begin
    showmessage('Pls insert the Delivery Equipment first');
  end else
  begin
    Deliver_Equipment_print:=TDeliver_Equipment_print.Create(self);
    Deliver_Equipment_print.quickrep1.prepare;
    ...
    Deliver_Equipment_print.quickrep1.preview;
    Deliver_Equipment_print.free;
  end;
end;
