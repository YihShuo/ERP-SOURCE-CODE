...
begin
  ...
  qryCheck.SQL.Add('    ISNULL(S.CTS, 0) AS CTS');
  qryCheck.SQL.Add('FROM');
  qryCheck.SQL.Add('(');
  qryCheck.SQL.Add('    SELECT');
  qryCheck.SQL.Add('        Size,');
  qryCheck.SQL.Add('        SUM(Pairs) AS Pairs');
  ...
  qryCheck.SQL.Add('        SUM(CTS * SMDDSS.Qty) AS CTS');
  ...
  qryCheck.SQL.Add('        AND SMDD.GXLB = ''A''');
  qryCheck.SQL.Add('    WHERE YSBH = :DDBH');
  qryCheck.SQL.Add('      AND SMDDSS.GXLB = ''A''');
  ...
  begin
    ...
    MaxCTS := qryCheck.FieldByName('CTS').AsInteger;
    TotalNewQty := 0;
    ...
    begin
      ...
      if (DispatchPairs + TotalNewQty) > MaxCTS then
      begin
        Application.MessageBox(
          PChar(Format('Loi: Size %s vuot qua gioi han!' + #13#10 +
                       ...
                       '- Toi da cho phep: %d',
                       [SizeName, DispatchPairs, TotalNewQty, MaxCTS])),
