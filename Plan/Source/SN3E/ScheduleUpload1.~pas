...
type
  ...
  private
    ...
    procedure ValidateAndSaveDispatch(AqryMaster, AqryDetail: TQuery);
  public
    { Public declarations }
    Version: string;
...
// Ham kiem tra va luu du lieu CachedUpdates (Ho tro MSSQL 2008 R2)
// Tham so:
// - AqryMaster: Dataset chua thong tin don hang (Master)
// - AqryDetail: Dataset chua luoi nhap lieu Size can luu (Detail)
procedure TScheduleUpload.ValidateAndSaveDispatch(AqryMaster, AqryDetail: TQuery);
var
  TotalNewQty: Integer;
  SizeName: string;
  DispatchPairs, MatchingPairs: Integer;
  bm: TBookmark;
  FieldSize: TField;
  OldQty: Integer;
begin
  // 1. Ép lu?i ghi nh?n d? li?u dang nh?p d? vào b? d?m (Bu?c ph?i có d? UpdatesPending nh?n di?n)
  if AqryDetail.State in [dsEdit, dsInsert] then
    AqryDetail.Post;

  // Bo qua neu khong co du lieu nao can luu
  if not AqryDetail.UpdatesPending then Exit;

  // 2. K? th?a chung du?ng truy?n Database v?i Query Detail d? Profiler b?t du?c
  //qryCheck.DatabaseName := AqryDetail.DatabaseName;
  qryCheck.Active := False;
  qryCheck.SQL.Clear;

  // Cau lenh SQL chay tren he quan tri MSSQL 2008 R2
  qryCheck.SQL.Clear;
  ...
  qryCheck.SQL.Add('(');
  qryCheck.SQL.Add('    SELECT');
  qryCheck.SQL.Add('        Size,');
  qryCheck.SQL.Add('        ISNULL(SUM(Pairs), 0) AS Pairs');
  qryCheck.SQL.Add('    FROM SMDDS_Dispatch');
  qryCheck.SQL.Add('    WHERE ZLBH = :DDBH');
  qryCheck.SQL.Add('    GROUP BY Size');
  qryCheck.SQL.Add(') D');
  qryCheck.SQL.Add('FULL JOIN');
  qryCheck.SQL.Add('(');
  qryCheck.SQL.Add('    SELECT');
  ...
  qryCheck.SQL.Add('            ON SMDD.DDBH = SMDDSS.DDBH');
  qryCheck.SQL.Add('            AND SMDD.GXLB = SMDDSS.GXLB');
  qryCheck.SQL.Add('            AND SMDD.GXLB IN (''O'',''S'')');
  qryCheck.SQL.Add('        WHERE YSBH = :DDBH');
  qryCheck.SQL.Add('          AND SMDDSS.GXLB IN (''O'',''S'')');
  qryCheck.SQL.Add('        GROUP BY XXCC, SMDDSS.GXLB');
  qryCheck.SQL.Add('    ) S1');
  qryCheck.SQL.Add('    GROUP BY XXCC');
  qryCheck.SQL.Add(') S');
  qryCheck.SQL.Add('    ON D.Size = S.XXCC');
  qryCheck.SQL.Add('ORDER BY Size;');

  // Lay ma don hang tu Dataset Master (Cot 'ZLBH')
  qryCheck.ParamByName('DDBH').AsString := AqryMaster.FieldByName('ZLBH').AsString;
  qryCheck.Open;

  // Duyet tung size tu database de doi chieu voi du lieu tren luoi
  qryCheck.First;
  while not qryCheck.Eof do
  begin
    SizeName := qryCheck.FieldByName('Size').AsString;
    DispatchPairs := qryCheck.FieldByName('DispatchPairs').AsInteger;
    MatchingPairs := qryCheck.FieldByName('MatchingPairs').AsInteger;
    TotalNewQty := 0;

    // Tim cot Size tuong ung tren luoi Detail
    FieldSize := AqryDetail.FindField(SizeName);
    
    if Assigned(FieldSize) then
    begin
      // Quet toan bo so luong nhap tren luoi (dang luu tam trong CachedUpdates)
      AqryDetail.DisableControls; 
      bm := AqryDetail.GetBookmark;
      try
        AqryDetail.First;
        while not AqryDetail.Eof do
        begin
          if AqryDetail.UpdateStatus = usInserted then
          begin
            TotalNewQty := TotalNewQty + FieldSize.AsInteger;
          end
          else if AqryDetail.UpdateStatus = usModified then
          begin
            if VarIsNull(FieldSize.OldValue) or VarIsEmpty(FieldSize.OldValue) then
              OldQty := 0
            else
              OldQty := FieldSize.OldValue;
              
            TotalNewQty := TotalNewQty + (FieldSize.AsInteger - OldQty);
          end;
          
          AqryDetail.Next;
        end;
      finally
        if AqryDetail.BookmarkValid(bm) then
        begin
          AqryDetail.GotoBookmark(bm);
          AqryDetail.FreeBookmark(bm);
        end;
        AqryDetail.EnableControls;
      end;

      // Thong bao loi khong dau neu vuot qua so luong CTS
      if (DispatchPairs + TotalNewQty) > MatchingPairs then
      begin
        Application.MessageBox(
          PChar(Format('Loi: Size %s vuot qua gioi han!' + #13#10 +
                       '- Da phat truoc do: %d' + #13#10 +
                       '- Dang nhap them: %d' + #13#10 +
                       '- Toi da cho phep: %d',
                       [SizeName, DispatchPairs, TotalNewQty, MatchingPairs])),
          'Warning', MB_OK + MB_ICONWARNING);
          
        qryCheck.Close; // Dong ket noi cua qryCheck truoc khi thoat
        Abort;          // Ngat toan bo cac lenh luu thuc thi phia sau
      end;
    end;
    qryCheck.Next;
  end;
        ...
        begin
          ...
          {try
            try
              ...
              ShowMessage('¤W¶Ç¦¨¥\¡A±Æµ{ªí¤j¬ù¤T¤ÀÄÁ«á§Y¥i¬d¸ß¡C' + #13#10 + Response);
            except
              ShowMessage('¨t²Î¥¿¦b¦P¨B±Æµ{¡A½Ðµy«á¦A¤W¶Ç¡C');
            end;
          finally
            // do something
          end;}
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  else if (Sheet.Cells[Row, 1].Text = '§Ç¸¹') then
                  begin
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      if (Sheet.Cells[i, 1].Text = '½u§O') then
                      begin
                        Row_Lean := i;
                        Break;
                      end;
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    SQL.Add('MIN(CASE WHEN SMDD.DDBH = SMDD.YSBH THEN 1 ELSE CAST(REVERSE(SUBSTRING(REVERSE(SMDD.DDBH), 1, CHARINDEX(''-'', REVERSE(SMDD.DDBH)) - 1)) AS INT) END) AS MinCycle,');
                    SQL.Add('MAX(CASE WHEN SMDD.DDBH = SMDD.YSBH THEN 1 ELSE CAST(REVERSE(SUBSTRING(REVERSE(SMDD.DDBH), 1, CHARINDEX(''-'', REVERSE(SMDD.DDBH)) - 1)) AS INT) END) AS MaxCycle  FROM DDZL');
                    SQL.Add('LEFT JOIN XXZL ON XXZL.XieXing = DDZL.XieXing AND XXZL.SheHao = DDZL.SheHao');
                    SQL.Add('LEFT JOIN SMDD ON SMDD.YSBH = DDZL.DDBH');
                    SQL.Add('WHERE SMDD.GXLB = ''A'' AND DDZL.DDBH = CASE WHEN LEN(@RY) - LEN(REPLACE(@RY, ''-'', '''')) < 2 THEN @RY ELSE SUBSTRING(@RY, 1, LEN(@RY) - CHARINDEX(''-'', REVERSE(@RY))) END');
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                PlanBlock[0][6] := Col
              else if (Pos('¦^Âà¼Æ', Sheet.Cells[TRow, Col]) > 0) then
                PlanBlock[0][7] := Col;
            end;
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    if (PlanBlock[i][2] > 0) then
                      FieldByName('NT').Value := StringReplace(StringReplace(StringReplace(Sheet.Cells[CRow, PlanBlock[i][2]].Text, ' ', '', [rfReplaceAll]), '(', ' (', [rfReplaceAll]), '«G', '', [rfReplaceAll]);
                    if (PlanBlock[i][3] > 0) then
                    ...
                    if (QTemp.FieldByName('RYTYPE').AsString = 'SLT') then
                      FieldByName('Remark').Value := 'DON HANG XUAT NHANH (§Ö³t­q³æ)';
                    if (PlanBlock[i][7] > 0) then
                    begin
                      FieldByName('MinCycle').Value := QTemp.FieldByName('MinCycle').AsInteger;
                      FieldByName('MaxCycle').Value := QTemp.FieldByName('MaxCycle').AsInteger;
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    if (Pos('¦L¨ê', RowTitle) = 1) or (Pos('¾c­±', RowTitle) = 1) then
                    ...
                    if (RowTitle = 'NL') then   //¦¨«¬¤W½u¤é
                      begin
                      Row_Date := i;
                      end
                    else if (RowTitle = 'SE') then     //¨C¤é¶¶§Ç
                      begin
                      Row_Seq := i;
                      end
                    else if (RowTitle = 'AR') then     //¾c«¬
                      ScheduleBlock[0][0] := i
                    else if (RowTitle = 'RY') then     //RY
                      ScheduleBlock[0][1] := i
                    else if (RowTitle = 'VT') then    //¤£­«­n
                      begin
                      ScheduleBlock[0][2] := i;
                      //ShowMessage('Gia tri VT: ' + IntToStr(i));
                      end
                    else if (RowTitle = 'LA') then     //ÝÉÀY
                      ScheduleBlock[0][4] := i
                    else if (RowTitle = 'BU') then     //BUY§O
                      ScheduleBlock[0][5] := i
//                    else if (RowTitle = 'BT') then     //¤£­«­n
//                      ScheduleBlock[0][6] := i
//                    else if (RowTitle = '(C)') then    //µôÂ_¤W½u¤é
//                      ScheduleBlock[0][7] := i
//                    else if (RowTitle = 'SK') then     //SKU
//                      ScheduleBlock[0][8] := i
//                    else if (RowTitle = '(S)') then    //°w¨®¤W½u¤é
//                      ScheduleBlock[0][9] := i
                    else if (RowTitle = '(S)') then    //¼Æ¶q
                    begin
                    ScheduleBlock[0][10] := i;
                    //ShowMessage('Gia tri index: ' + IntToStr(i));
                    end
//                    else if (RowTitle = '(A)') then    //¤â¤u¦¨«¬³Æµù
//                      ScheduleBlock[0][11] := i
                    else if (RowTitle = 'CS') then     //¥X³f¤é
                      ScheduleBlock[0][12] := i
                    else if (RowTitle = '(N)') then    //°ê®a
                      ScheduleBlock[0][13] := i
                    else if (RowTitle = 'NE') then     //§÷®ÆºØÃþ
                      ScheduleBlock[0][14] := i
                    else if (RowTitle = '(D)') then    //¤â¤u¦¨«¬³Æµù
                    begin
                      ScheduleBlock[0][15] := i;
                    end
                    else if (RowTitle = '(H)') then     //¥X³f¤é
                    begin
                      ScheduleBlock[0][16] := i;
                    end
                    else if (RowTitle = '(1)') then    //°ê®a
                    begin
                      ScheduleBlock[0][17] := i;
                    end
                    else if (RowTitle = '(2)') then     //§÷®ÆºØÃþ
                    begin
                      ScheduleBlock[0][18] := i;
                    end
                    else if (RowTitle = '(A)') then     //§÷®ÆºØÃþ
                    begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        NewColumn.Title.Caption := '¤Ø½X|' + QTemp.FieldByName('Size').AsString + '#';
...
begin
  ...
  ValidateAndSaveDispatch(Query_AP, Query_APS);
