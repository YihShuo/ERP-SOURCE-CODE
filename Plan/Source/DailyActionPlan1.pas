...
var
  DailyActionPlan: TDailyActionPlan;
  NDate:TDate;
...
procedure TDailyActionPlan.FormDestroy(Sender: TObject);
begin
DailyActionPlan:=nil;
end;

procedure TDailyActionPlan.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
action:=cafree;
end;

procedure TDailyActionPlan.Button1Click(Sender: TObject);
var e:string;
 i:integer;
begin
 {with QTemp do
  ...
  begin
    ...
    if not Check.Checked then
      begin //將基准產能加上
        ...
        sql.add('       isnull(sum(SCBBS.Qty),0) as Qty,isnull(SCBZCL.Qty,0) as bzQty,SCRL.SCGS,RSCQ.SDGS ');
        sql.add('from SCBB ');
        ...
        sql.add('Group by convert(varchar,SCBB.SCDate,111) ,SCBB.DepNo,BDepartment.DepName ,SCBZCL.Qty,RSCQ.SDGS,SCRL.SCGS');

        sql.add('union all ');
      end;

    sql.add('select '+''''+formatdatetime('yyyy/MM/dd',DTP2.Date)+''''+' as SCDate ,SCBB.DepNO,'+''''+'Total'+''''+' as DepName,BDepartment.DepName as DepName2,');
    sql.add('        isnull(sum(SCBBS.Qty),0) as Qty,isnull(SCBZCLA.Qty,0) as bzQty,SCRL.SCGS,RSCQ.SDGS');
    sql.add('from SCBB ');
    sql.add('left join SCBBS on  SCBB.ProNo=SCBBS.ProNo');
    sql.add('left join BDepartment on BDepartment.ID=SCBB.DepNo  ');
    ...
    sql.add('Group by SCBB.DepNo,BDepartment.DepName,SCBZCLA.Qty,SCRL.SCGS,RSCQ.SDGS');
...
procedure TDailyActionPlan.FormCreate(Sender: TObject);
var i:integer;
begin
with Qtemp do
  begin
...
procedure TDailyActionPlan.DBGrid1DrawColumnCell(Sender: TObject;
...
procedure TDailyActionPlan.Query1AfterOpen(DataSet: TDataSet);
begin
with DBGrid1 do
  begin
...
procedure TDailyActionPlan.Query1CalcFields(DataSet: TDataSet);
begin
...
procedure TDailyActionPlan.NN3Click(Sender: TObject);
begin
PrintDBGrideh1.Preview;
end;

procedure TDailyActionPlan.NN2Click(Sender: TObject);
var
...
procedure TDailyActionPlan.DBGrid1GetCellParams(Sender: TObject;
  Column: TColumnEh; AFont: TFont; var Background: TColor;
  State: TGridDrawState);
begin
if  query1.FieldByName('Person').value<95 then
  begin
...
procedure TDailyActionPlan.PrintDBGridEh1BeforePrint(Sender: TObject);
begin
