...
begin
  ...
  begin
    ...
    begin
      ...
      // 4. LOGIC CHAN VUOT MA VACH (Chi tin In khong tru OUT)
      SQL.Add('SELECT @TotalHienTai = ISNULL(SUM(Qty), 0) FROM smzl_make WITH (NOLOCK) ');
      SQL.Add('WHERE codebar = ' + QuotedStr(Edit3.Text) + ' AND Status = ''IN''; ');
      SQL.Add('IF (@TotalHienTai + @DonViQty) > @MaxLimitCodebar ');
      SQL.Add('BEGIN ');
      SQL.Add('    SET @MaxStr = CAST(@MaxLimitCodebar AS VARCHAR(20)); ');
      SQL.Add('    RAISERROR(''Ma vach nay da nhap du han muc (%s). Khong the nhap them!'', 16, 1, @MaxStr); ');
      SQL.Add('    RETURN; ');
      SQL.Add('END; ');
