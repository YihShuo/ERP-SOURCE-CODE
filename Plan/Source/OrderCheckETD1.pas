...
var
  OrderCheckETD: TOrderCheckETD;

implementation

uses main1, OrderCheckETD_Det1;

{$R *.dfm}

procedure TOrderCheckETD.FormDestroy(Sender: TObject);
begin
OrderCheckETD:=nil;
end;

procedure TOrderCheckETD.FormClose(Sender: TObject;
...
with QTemp do
  begin
    active:=false;
    sql.Clear;
    sql.add('  if object_id('+''''+'tempdb..#SCBBSETD'+''''+') is not null  ');
    sql.add('begin   drop table #SCBBSETD end   ');
    execsql;
  end;
action:=cafree;
end;

procedure TOrderCheckETD.FormCreate(Sender: TObject);
var i:integer;
NDate:TDate;
begin
with Qtemp do
...
procedure TOrderCheckETD.Panel3DblClick(Sender: TObject);
begin
...
procedure TOrderCheckETD.Panel2DblClick(Sender: TObject);
begin
...
procedure TOrderCheckETD.N2Click(Sender: TObject);
begin
...
procedure TOrderCheckETD.N1Click(Sender: TObject);
begin
...
procedure TOrderCheckETD.Print1Click(Sender: TObject);
begin

PrintDBGrideh1.Preview;
end;

procedure TOrderCheckETD.NN2Click(Sender: TObject);
...
procedure TOrderCheckETD.Button1Click(Sender: TObject);
var i:integer;
begin
with QTemp do
  begin
    ...
    sql.add('into #SCBBSETD from SCBBSS ');
...
with query1 do
  begin
    active:=false;
    sql.clear;
    sql.add('select KFJC,ETD,sum(Pairs) as Pairs,sum(C_Qty) as C_Qty,sum(Lack_C) as Lack_C');
    ...
    sql.add('       ,SCBBF.FinDate as F_Date ,YWDD.ETD ');
    sql.add('from DDZL ');
    sql.add('left join XXZL on XXZL.XieXing=DDZL.XieXing and DDZl.SheHao=XXZL.SheHao ');
    sql.add('left join KFZL on KFZl.KFDH=DDZL.KHBH ');
    sql.add('left join SCZLDate on SCZLDate.ZLBH=DDZL.ZLBH ');
    sql.add('left join (select #SCBBSETD.DDBH,sum(#SCBBSETD.Qty) as C_Qty,max(#SCBBSETD.FinDate) as FinDate ');
    sql.add('           from #SCBBSETD ');
    sql.add('           where #SCBBSETD.GX='+''''+'C'+'''');
    sql.add('           group by #SCBBSETD.DDBH) SCBBC on SCBBC.DDBH=DDZL.ZLBH ');
    sql.add('left join (select #SCBBSETD.DDBH,sum(#SCBBSETD.Qty) as S_Qty,max(#SCBBSETD.FinDate) as FinDate ');
    sql.add('           from #SCBBSETD ');
    sql.add('           where #SCBBSETD.GX='+''''+'S'+'''');
    sql.add('           group by #SCBBSETD.DDBH) SCBBS on SCBBS.DDBH=DDZL.ZLBH ');
    sql.add('left join (select #SCBBSETD.DDBH,sum(#SCBBSETD.Qty) as W_Qty ,max(#SCBBSETD.FinDate) as FinDate');
    sql.add('           from #SCBBSETD ');
    sql.add('           where #SCBBSETD.GX='+''''+'W'+'''');
    sql.add('           group by #SCBBSETD.DDBH) SCBBW on SCBBW.DDBH=DDZL.ZLBH ');
    sql.add('left join (select #SCBBSETD.DDBH,sum(#SCBBSETD.Qty) as O_Qty ,max(#SCBBSETD.FinDate) as FinDate');
    sql.add('           from #SCBBSETD ');
    sql.add('           where #SCBBSETD.GX='+''''+'O'+'''');
    sql.add('           group by #SCBBSETD.DDBH) SCBBO on SCBBO.DDBH=DDZL.ZLBH ');
    sql.add('left join (select #SCBBSETD.DDBH,sum(#SCBBSETD.Qty) as Z_Qty ,max(#SCBBSETD.FinDate) as FinDate');
    sql.add('           from #SCBBSETD ');
    sql.add('           where #SCBBSETD.GX='+''''+'Z'+'''');
    sql.add('           group by #SCBBSETD.DDBH) SCBBZ on SCBBZ.DDBH=DDZL.ZLBH ');
    sql.add('left join (select #SCBBSETD.DDBH,sum(#SCBBSETD.Qty) as A_Qty ,max(#SCBBSETD.FinDate) as FinDate');
    sql.add('           from #SCBBSETD ');
    sql.add('           where #SCBBSETD.GX='+''''+'A'+'''');
    sql.add('           group by #SCBBSETD.DDBH) SCBBA on SCBBA.DDBH=DDZL.ZLBH ');
    sql.add('left join (select #SCBBSETD.DDBH,sum(#SCBBSETD.Qty) as F_Qty ,max(#SCBBSETD.FinDate) as FinDate');
    sql.add('           from #SCBBSETD ');
    sql.add('           where #SCBBSETD.GX='+''''+'F'+'''');    
    sql.add('           group by #SCBBSETD.DDBH) SCBBF on SCBBF.DDBH=DDZL.ZLBH ');
    ...
    sql.add(') DDZL ');
    sql.add('group by KFJC,ETD ');
    sql.add('order by KFJC,ETD');
    active:=true;
  end;

end;

procedure TOrderCheckETD.DBGrid1GetCellParams(Sender: TObject;
...
procedure TOrderCheckETD.NN1Click(Sender: TObject);
begin 
OrderCheckETD_Det:=TOrderCheckETD_Det.create(self);
OrderCheckETD_Det.show;
end;

procedure TOrderCheckETD.PrintDBGridEh1BeforePrint(Sender: TObject);
begin
