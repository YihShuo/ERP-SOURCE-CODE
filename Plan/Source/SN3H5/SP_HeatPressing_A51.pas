...
type
  TSP_HeatPressing_A5 = class(TForm)
    QTemp: TQuery;
    DS_PSI: TDataSource;
    Q_PSI: TQuery;
    U_PSI: TUpdateSQL;
...
var
  SP_HeatPressing_A5: TSP_HeatPressing_A5;

const
  ProcessKey: array[0..1] of string = (
    'ALL',
    'HEAT PRESSING'
  );
  ...
  ProcessEN: array[0..0] of string = (
    'HEAT PRESSING'
  );

  ProcessVN: array[0..0] of string = (
    'EP'
  );

  ProcessCH: array[0..0] of string = (
    '高週波'
  );
...
procedure TSP_HeatPressing_A5.ReloadBuilding(ComboBox: TComboBox; DTP_Start, DTP_End: TDateTimePicker; Mode: String);
var
  Building: string;
  i: integer;
begin
...
procedure TSP_HeatPressing_A5.ReloadLean(ComboBox, SourceBox: TComboBox; DTP_Start, DTP_End: TDateTimePicker; Mode: String);
var
  Lean: string;
  i: integer;
begin
...
procedure TSP_HeatPressing_A5.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TSP_HeatPressing_A5.FormDestroy(Sender: TObject);
begin
  SP_HeatPressing_A5 := Nil;
end;

procedure TSP_HeatPressing_A5.FormCreate(Sender: TObject);
begin
  DTP_PS.Date := Date;
...
procedure TSP_HeatPressing_A5.FormShow(Sender: TObject);
begin
  Panel1.SetFocus;
end;

procedure TSP_HeatPressing_A5.Button6Click(Sender: TObject);
var
  i, Index: integer;
begin
...
procedure TSP_HeatPressing_A5.Button5Click(Sender: TObject);
var
  RY: string;
  RYQty: integer;
begin
  ...
  begin
    ...
    SQL.Add('WHERE SPI.Date = ''' + FormatDateTime('yyyy/MM/dd', DTP_PS.Date) + ''' AND SPI.Process = ''' + ProcessKey[CB_Process_PS.ItemIndex + 1] + ''' AND MSP.Supplier = ''' + SupplierKey[CB_Supplier_PS.ItemIndex + 1] + '''');
  ...
  begin
    ...
    SQL.Add('WHERE SPO.Date = ''' + FormatDateTime('yyyy/MM/dd', DTP_PS.Date) + ''' AND SPO.Process = ''' + ProcessKey[CB_Process_PS.ItemIndex + 1] + ''' AND MSP.Supplier = ''' + SupplierKey[CB_Supplier_PS.ItemIndex + 1] + '''');
    if (CB_Building_PS.Text <> 'ALL') then
...
procedure TSP_HeatPressing_A5.BB1_PSIClick(Sender: TObject);
var
...
procedure TSP_HeatPressing_A5.Q_PSIAfterOpen(DataSet: TDataSet);
begin
  BB1_PSI.Enabled := true;
end;

procedure TSP_HeatPressing_A5.Q_PSOAfterOpen(DataSet: TDataSet);
begin
  BB1_PSO.Enabled := true;
end;
       
procedure TSP_HeatPressing_A5.BB2_PSIClick(Sender: TObject);
var
  i: integer;
begin
  try
    ...
    begin
      ...
      SQL.Add('  LEFT JOIN DDZL ON DDZL.DDBH = SPI.RY');
      SQL.Add('  LEFT JOIN ModelSecondProcess AS MSP ON MSP.XieXing = DDZL.XieXing AND MSP.SheHao = DDZL.SheHao AND MSP.Part = SPI.Part AND MSP.Process = SPI.Process');
      SQL.Add('  WHERE SPI.Date = ''' + FormatDateTime('yyyy/MM/dd', psDate) + ''' AND SPI.Process = ''' + psProcess + ''' AND MSP.Supplier = ''' + psSupplier + '''');
      SQL.Add(')');
      ExecSQL;
    end;
...
procedure TSP_HeatPressing_A5.BB3_PSIClick(Sender: TObject);
begin
  with Q_PSI do
  begin
    Active := false;
...
procedure TSP_HeatPressing_A5.BB1_PSOClick(Sender: TObject);
var
  eclApp, Sheet: OleVariant;
  SelectedFileName, Part: string;
...
procedure TSP_HeatPressing_A5.BB2_PSOClick(Sender: TObject);
var
  i: integer;
begin
  try
    ...
    begin
      ...
      SQL.Add('  LEFT JOIN DDZL ON DDZL.DDBH = SPO.RY');
      SQL.Add('  LEFT JOIN ModelSecondProcess AS MSP ON MSP.XieXing = DDZL.XieXing AND MSP.SheHao = DDZL.SheHao AND MSP.Part = SPO.Part AND MSP.Process = SPO.Process');
      SQL.Add('  WHERE SPO.Date = ''' + FormatDateTime('yyyy/MM/dd', psDate) + ''' AND SPO.Process = ''' + psProcess + ''' AND MSP.Supplier = ''' + psSupplier + '''');
      SQL.Add(')');
      ExecSQL;
    end;
...
procedure TSP_HeatPressing_A5.BB3_PSOClick(Sender: TObject);
begin
  with Q_PSO do
  begin
    Active := false;
...
procedure TSP_HeatPressing_A5.DBGridEh4DrawColumnCell(
...
procedure TSP_HeatPressing_A5.DBGridEh5DrawColumnCell(
  Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumnEh;
  State: TGridDrawState);
begin
...
procedure TSP_HeatPressing_A5.DTP_PSChange(Sender: TObject);
begin
  ReloadBuilding(CB_Building_PS, DTP_PS, DTP_PS, 'ALL');
  ReloadLean(CB_Lean_PS, CB_Building_PS, DTP_PS, DTP_PS, 'ALL');
end;

procedure TSP_HeatPressing_A5.CB_Building_PSChange(Sender: TObject);
begin
  ReloadLean(CB_Lean_PS, CB_Building_PS, DTP_PS, DTP_PS, 'ALL');
end;
