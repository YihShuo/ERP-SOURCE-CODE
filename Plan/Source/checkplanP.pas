...
begin
  ...
  begin
    ...
    SQL.Add('as  rn from RTSSmdd,DDZL where DDZL.DDBH=RTSSmdd.DDBH and DDZL.shipdate-180>GetDate() and IsNull(DDZL.RYTYPE,'''')<>''STL'' and RTSSmdd.YN=''N'' ');
    ...
    SQL.Add('as  rn from RTSSmdd,DDZL where DDZL.DDBH=RTSSmdd.DDBH and DDZL.shipdate-180>GetDate() and IsNull(DDZL.RYTYPE,'''')<>''STL'' and RTSSmdd.YN=''Y'' ');
...
begin
  ...
  begin
    ...
    sql.Add('left join');
    sql.Add('YWCP on ywcp.ddbh=DDZL.ddbh ');
    sql.Add('left join ');
    sql.Add('    (select smdd.ysbh,min(smdd.plandate) as strt,max(smdd.planedate) as endd, ');
    sql.Add('			min(smdd.PRdate) as strtR,max(smdd.OKdate) as enddR ');
    sql.Add('      from smdd ');
    ...
    sql.add('where (case when IsNull(RYType,'''')<>''STL'' then ');
...
begin
  ...
  begin
    ...
    sql.Add('			from ywcp');
    sql.Add('			group by ddbh) as ywcp on ywcp.ddbh=ddzl.ddbh ');
