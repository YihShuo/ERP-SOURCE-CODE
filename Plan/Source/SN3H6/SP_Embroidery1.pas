...
procedure TSP_Embroidery.ReloadBuilding(ComboBox: TComboBox; DTP_Start, DTP_End: TDateTimePicker; Mode: String);
var
  Building: string;
  i: integer;
begin
...
procedure TSP_Embroidery.ReloadLean(ComboBox, SourceBox: TComboBox; DTP_Start, DTP_End: TDateTimePicker; Mode: String);
var
  Lean: string;
  i: integer;
begin
...
procedure TSP_Embroidery.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TSP_Embroidery.FormDestroy(Sender: TObject);
begin
  SP_Embroidery := Nil;
end;

procedure TSP_Embroidery.FormCreate(Sender: TObject);
begin
  DTP_PS.Date := Date;
...
procedure TSP_Embroidery.FormShow(Sender: TObject);
begin
  Panel1.SetFocus;
end;

procedure TSP_Embroidery.Button6Click(Sender: TObject);
var
  i, Index: integer;
begin
...
procedure TSP_Embroidery.Button5Click(Sender: TObject);
var
  RY: string;
  RYQty: integer;
begin
...
procedure TSP_Embroidery.BB1_PSIClick(Sender: TObject);
var
...
procedure TSP_Embroidery.Q_PSIAfterOpen(DataSet: TDataSet);
begin
  BB1_PSI.Enabled := (DTP_PS.Date = main.Today);
end;

procedure TSP_Embroidery.Q_PSOAfterOpen(DataSet: TDataSet);
begin
  BB1_PSO.Enabled := (DTP_PS.Date = main.Today);
end;
       
procedure TSP_Embroidery.BB2_PSIClick(Sender: TObject);
var
  i: integer;
begin
  try
...
procedure TSP_Embroidery.BB3_PSIClick(Sender: TObject);
begin
  with Q_PSI do
  begin
    Active := false;
...
procedure TSP_Embroidery.BB1_PSOClick(Sender: TObject);
var
  eclApp, Sheet: OleVariant;
  SelectedFileName, Part: string;
...
procedure TSP_Embroidery.BB2_PSOClick(Sender: TObject);
var
  i: integer;
begin
  try
...
procedure TSP_Embroidery.BB3_PSOClick(Sender: TObject);
begin
  with Q_PSO do
  begin
    Active := false;
...
procedure TSP_Embroidery.DBGridEh4DrawColumnCell(
...
procedure TSP_Embroidery.DBGridEh5DrawColumnCell(
  Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumnEh;
  State: TGridDrawState);
begin
...
procedure TSP_Embroidery.DTP_PSChange(Sender: TObject);
begin
  ReloadBuilding(CB_Building_PS, DTP_PS, DTP_PS, 'ALL');
  ReloadLean(CB_Lean_PS, CB_Building_PS, DTP_PS, DTP_PS, 'ALL');
end;

procedure TSP_Embroidery.CB_Building_PSChange(Sender: TObject);
begin
  ReloadLean(CB_Lean_PS, CB_Building_PS, DTP_PS, DTP_PS, 'ALL');
end;
