...
uses
  ...
  ComCtrls, DBCtrls, Mask, GridsEh, DBGridEh,ehlibBDE, Menus;
...
uses main1, OnTime_Dep1, DM3, OnTime_sczl1, OnTime_XH1, Ontime_Check1, FunUnit;
...
begin
  ...
  begin
    active:=false;
    sql.clear;
    sql.add('select SCBB.*,SCSJ.FTime+'+''''+'-'+''''+'+SCSJ.TTime as SCTIME,SCSJ.YXGS,BDepartment.DepName,(Select Sum(Qty) from SCBBS where SCBBS.ProNo=SCBB.ProNO  ');
    sql.add('and SCBBS.GXLB='''+GXLBCombo.Text+''' ');
    if SCBHEdit.Text<>'' then
      sql.add('and SCBBS.SCBH like '''+SCBHEdit.Text+'%'' ');
    sql.add(') as Qty ');
    sql.add('from SCBB');
    sql.add('left join SCSJ on SCBB.SJXH=SCSJ.XH');
    ...
    sql.add('and exists  (select ProNo from SCBBS where SCBBS.ProNo=SCBB.ProNo and SCBH like '''+SCBHEdit.Text+'%'' and SCBBS.GXLB='''+GXLBCombo.Text+'''  Group by ProNo) ');
    if CB1.Checked then
    begin
      sql.add('and SCBB.USERID='+''''+main.Edit1.Text+'''');
    end;
    sql.add('order by SCBB.ProNO Desc');
    active:=true;
  end;
  with QRepDet do
  begin
    active:=false;
    sql.Clear;
    sql.add('select * from scbbs ');
    sql.add('where SCBBS.ProNo=:ProNo ');
    sql.add('  and SCBBS.GXLB='''+GXLBCombo.Text+''' ');
    if SCBHEdit.Text<>'' then
      sql.add('and SCBBS.SCBH like '''+SCBHEdit.Text+'%'' ');
    active:=true;
              ...
              if QRepMas.fieldbyname('SCDate').isnull then
                begin
                  QRepMas.delete;
                   ...
                   begin
                     if QRepMas.FieldByName('SCDate').value<NDate-5 then
                       begin
                         showmessage('You can not insert the production report 5 days ago.');
                         //abort;
                       end
                       else
                         begin
                 ...
                 begin
                   if QRepMas.fieldbyname('YN').value='0'then
                     begin
                       ...
                       begin
                         if DTP1.date<NDate-5 then
                           begin
                             showmessage('You can not modify the production report 5 days ago.');
                             //abort;
                           end;
                         QRepMas.edit;
...
begin
  ...
  if messagedlg('Do you really want to delete this record?',mtconfirmation,[mbYes,Mbno],0)=mrYes then
  begin
    if messagedlg('There size detail will be delete too! Really?',mtconfirmation,[mbYes,Mbno],0)=mrYes then
      begin
      try
        if (Qscbbss.RecordCount>0) then
          begin
            Qtemp.Active:=false;
            Qtemp.SQL.Clear;
            Qtemp.sql.add('delete SCBBSS ');
            Qtemp.SQL.add('where ProNo='+''''+QRepDet.fieldbyname('ProNo').Value+'''') ;
            Qtemp.SQL.add('      and SCBH='+''''+QRepDet.fieldbyname('SCBH').Value+'''');
            Qtemp.SQL.add('      and GXLB='+''''+ QRepDet.fieldbyname('GXLB').Value+'''');
            QTemp.ExecSQL;
            QTemp.Active:=false;
          end;
        with QRepDet do
                  ...
                  begin
                    QRepdet.delete;
                      ...
                      begin
                        ...
                        if QRepDet.FieldByName('Qty').isnull then
                          begin
...
procedure TOntime.BT1Click(Sender: TObject);
begin
...
procedure TOntime.BT3Click(Sender: TObject);
begin 
  OnTime_Dep:=TOnTime_Dep.create(self);
  OnTime_Dep.show;
end;
...
procedure TOntime.DBGrid2GetCellParams(Sender: TObject; Column: TColumnEh;
  AFont: TFont; var Background: TColor; State: TGridDrawState);
begin
...
procedure TOntime.DBGrid1GetCellParams(Sender: TObject; Column: TColumnEh;
  AFont: TFont; var Background: TColor; State: TGridDrawState);
begin
...
procedure TOntime.DBGrid3GetCellParams(Sender: TObject; Column: TColumnEh;
  AFont: TFont; var Background: TColor; State: TGridDrawState);
begin
...
procedure TOntime.BT2Click(Sender: TObject);
begin
  OnTime_XH:=TOnTime_XH.create(self);
  OnTime_XH.show;
end;
...
procedure TOntime.QscbbssNewRecord(DataSet: TDataSet);
begin
  abort;
end;

procedure TOntime.QscbbssAfterInsert(DataSet: TDataSet);
begin
abort;
end;

procedure TOntime.N1Click(Sender: TObject);
begin
...
procedure TOntime.Check1Click(Sender: TObject);
begin
  Ontime_Check:=TOntime_Check.create(self);
  Ontime_Check.show;
end;
