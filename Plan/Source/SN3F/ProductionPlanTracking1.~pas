  ...
  begin
    ...
    try
      ...
      begin
        ...
        eclApp.Cells[Row + 5, 1] := '¼Æ¶q/SL';
        eclApp.Cells[Row + 6, 1] := 'CSD';
        eclApp.Cells[Row + 7, 1] := '°ê®a/N';
        Inc(Row, 8);
      end;
...
begin
  ...
  begin
    ...
    SQL.Add('IF OBJECT_ID(''tempdb..#SMDD'') IS NOT NULL');
    SQL.Add('BEGIN DROP TABLE #SMDD END;');
    SQL.Add('IF OBJECT_ID(''tempdb..#Dispatch'') IS NOT NULL');
    SQL.Add('BEGIN DROP TABLE #Dispatch END;');
    ExecSQL;
    ...
    // --- 3. T?O B?NG T?M #SMDD (Ðã fix l?i nhân b?n s? lu?ng b?ng DISTINCT DDBH) ---
    SQL.Add('SELECT SC.Building, SC.Lean, SC.schedule_date, SC.ry_index, SC.DDBH, SC.Article, SC.XieMing, SC.Pairs, SC.ShipDate, SMDD.GXLB');
    QTemp.First;
    while not QTemp.Eof do
    begin
    ...
    SQL.Add('INTO #SMDD FROM #SC AS SC');
    SQL.Add('LEFT JOIN (');
    SQL.Add(' SELECT SC.DDBH, ''Total'' AS GXLB, DDZLS.CC AS Size, DDZLS.Quantity AS Pairs FROM (SELECT DISTINCT DDBH FROM #SC) AS SC');
    SQL.Add('  LEFT JOIN DDZLS ON DDZLS.DDBH = SC.DDBH');
    SQL.Add('  UNION ALL');
    SQL.Add('  SELECT SC.DDBH, Section.Name AS GXLB, SMDDSS.XXCC AS Size, SUM(SMDDSS.Qty * SMDDSS.okCTS) AS Finished FROM (SELECT DISTINCT DDBH FROM #SC) AS SC');
    SQL.Add('  LEFT JOIN (');
    SQL.Add('    SELECT ''S'' AS GXLB, ''Upper'' AS Name UNION ALL SELECT ''O'' AS GXLB, ''Bottom'' AS Name');
    SQL.Add('  ) AS Section ON 1 = 1');
    SQL.Add('  LEFT JOIN SMDD ON SMDD.YSBH = SC.DDBH AND SMDD.GXLB = Section.GXLB');
    ...
    // --- 4. T?O B?NG T?M #Dispatch (Ðã fix l?i liên k?t khóa ZLBH) ---
    SQL.Add('SELECT ZLBH AS DDBH'); 
    QTemp.First;
    while not QTemp.Eof do
    begin
    ...
    SQL.Add('WHERE ZLBH IN (SELECT DISTINCT DDBH FROM #SC)'); 
    SQL.Add('GROUP BY ZLBH'); 

    // --- 5. T?NG H?P (UNION ALL) ---
    SQL.Add('SELECT *, ROW_NUMBER() OVER(ORDER BY Building, Lean, schedule_date, ry_index, CASE SMDD.GXLB ');
    sql.add(' WHEN ''Upper'' THEN 1 WHEN ''Bottom'' THEN 2 WHEN ''Matching'' THEN 3 WHEN ''Dispatch'' THEN 4 WHEN ''Remain'' THEN 5 WHEN ''Total'' THEN 6 WHEN ''Shortage'' THEN 7 END) AS Seq FROM (');

    // Kh?i 1: Upper, Bottom, Total
    SQL.Add('  SELECT *,');
    QTemp.First;
    while not QTemp.Eof do
    begin
    ...
    // Kh?i 3: Dispatch 
    SQL.Add('  SELECT SC.Building, SC.Lean, SC.schedule_date, SC.ry_index, SC.DDBH, SC.Article, SC.XieMing, SC.Pairs, SC.ShipDate, ''Dispatch'' AS GXLB');
    QTemp.First;
    while not QTemp.Eof do
    begin
    ...
    SQL.Add('  0 AS Total FROM (SELECT DISTINCT Building, Lean, schedule_date, ry_index, DDBH, Article, XieMing, Pairs, ShipDate FROM #SC) AS SC');
    SQL.Add('  LEFT JOIN #Dispatch D ON SC.DDBH = D.DDBH');
    SQL.Add('  UNION ALL');

    // Kh?i 4: Remain (Matching - Dispatch)
    SQL.Add('  SELECT M.Building, M.Lean, M.schedule_date, M.ry_index, M.DDBH, M.Article, M.XieMing, M.Pairs, M.ShipDate, ''Remain'' AS GXLB');
    QTemp.First;
    while not QTemp.Eof do
    begin
      SQL.Add('  , M.[' + QTemp.FieldByName('Size').AsString + '] - ISNULL(D.[' + QTemp.FieldByName('Size').AsString + '], 0) AS [' + QTemp.FieldByName('Size').AsString + ']');
      QTemp.Next;
    end;
    if (QTemp.RecordCount > 0) then
      SQL.Add('  ,');
    ...
    while not QTemp.Eof do
    begin
      SQL.Add('  (ISNULL(M.[' + QTemp.FieldByName('Size').AsString + '], 0) - ISNULL(D.[' + QTemp.FieldByName('Size').AsString + '], 0)) +');
      QTemp.Next;
    end;
    SQL.Add('  0 AS Total FROM (');
    SQL.Add('    SELECT Building, Lean, schedule_date, ry_index, DDBH, Article, XieMing, Pairs, ShipDate');
    QTemp.First;
    while not QTemp.Eof do
    begin
      SQL.Add('    , MIN([' + QTemp.FieldByName('Size').AsString + ']) AS [' + QTemp.FieldByName('Size').AsString + ']');
    ...
    SQL.Add('    GROUP BY Building, Lean, schedule_date, ry_index, DDBH, Article, XieMing, Pairs, ShipDate');
    SQL.Add('  ) AS M');
    SQL.Add('  LEFT JOIN #Dispatch AS D ON M.DDBH = D.DDBH');
    SQL.Add('  UNION ALL');
    ...
    // Ð?nh nghia th? t? s?p x?p (ORDER BY)
    SQL.Add('ORDER BY Building, Lean, schedule_date, ry_index, CASE SMDD.GXLB WHEN ''Upper'' THEN 1 WHEN ''Bottom'' THEN 2 WHEN ''Matching'' THEN 3 WHEN ''Dispatch'' THEN 4 WHEN ''Remain'' THEN 5 WHEN ''Total'' THEN 6 WHEN ''Shortage'' THEN 7 END');
...
var
  ...
  IsHiddenRow: Boolean;
begin
  // L?y giá tr? c?a c?t Section (GXLB) d? làm m?c x? lý
  GXLB_Val := Q_UBMatching.FieldByName('GXLB').AsString;
  
  // ?n d? li?u l?p: Ch? hi?n th? text ? c?m thông tin chung n?u là dòng 'Upper' (dòng 1)
  IsHiddenRow := (GXLB_Val <> 'Upper');

  // --- BU?C 1: XÁC Ð?NH MÀU N?N ---
  if not (gdSelected in State) then
  begin
    if (GXLB_Val = 'Matching') or (GXLB_Val = 'Total') or (GXLB_Val = 'Shortage') then
      DBGridEh7.Canvas.Brush.Color := RGB(204, 255, 255) // Xanh nh?t cho dòng 3 (Matching)
    else if (GXLB_Val = 'Remain') then
      DBGridEh7.Canvas.Brush.Color := RGB(255, 235, 204) // Cam/Vàng nh?t cho dòng 5 (Remain)
    else
      DBGridEh7.Canvas.Brush.Color := clWindow; // Tr?ng m?c d?nh cho dòng 1, 2, 4
  end
  else begin
    // Màu ch? tr?ng khi dòng dang du?c click ch?n
    DBGridEh7.Canvas.Font.Color := clWindow;
  end;

  // --- BU?C 2: V? D? LI?U CÁC C?T BÊN TRÁI (Thông tin chung) ---
  if (DataCol <= 7) then
  begin
    if IsHiddenRow then
    begin
      // Ch? tô màu n?n d? che di d? li?u l?p, không v? Text
      DBGridEh7.Canvas.FillRect(Rect);
    end
    else begin
      DBGridEh7.DefaultDrawColumnCell(Rect, DataCol, Column, State);
  ...
  // --- BU?C 3: V? D? LI?U CÁC C?T BÊN PH?I (Section & Size) ---
  else begin
    // Ð?m b?o ch? màu den trên các dòng du?c tô màu n?n (n?u không b? bôi den/ch?n)
    if not (gdSelected in State) and 
       ((GXLB_Val = 'Matching') or (GXLB_Val = 'Remain') or 
        (GXLB_Val = 'Total') or (GXLB_Val = 'Shortage')) then
    begin
      DBGridEh7.Canvas.Font.Color := clWindowText;
    end;
  ...
  // --- BU?C 4: V? ÐU?NG K? PHÂN CÁCH CÁC L?NH S?N XU?T ---
  // V? m?t v?ch ngang ? du?i cùng c?a ô thu?c dòng 'Remain'
  if (GXLB_Val = 'Remain') then
  begin
    DBGridEh7.Canvas.Pen.Color := clSilver; // Màu xám vi?n ngan cách
    DBGridEh7.Canvas.Pen.Style := psSolid;
    DBGridEh7.Canvas.MoveTo(Rect.Left, Rect.Bottom - 1);
    DBGridEh7.Canvas.LineTo(Rect.Right, Rect.Bottom - 1);
  end;
end;
...
begin
  ...
  begin
    try
      eclApp := CreateOleObject('Excel.Application');
      // T?t hi?n th? Excel trong lúc v? d? tang t?c d? xu?t d? li?u
    ...
    try
      ...
      begin
        ...
        // X? lý b? l?c (S?a l?i treo vòng l?p c?a code cu)
        if (CB_SUM.Text = 'Matching Pairs') AND ((GXLB_Val = 'Total') OR (GXLB_Val = 'Shortage')) then
          SkipRow := True
        else if (CB_SUM.Text = 'Total Pairs') AND ((GXLB_Val = 'Matching') OR (GXLB_Val = 'Shortage') OR (GXLB_Val = 'Dispatch') OR (GXLB_Val = 'Remain')) then
          SkipRow := True
        else if (CB_SUM.Text = 'Shortage') AND ((GXLB_Val = 'Matching') OR (GXLB_Val = 'Total') OR (GXLB_Val = 'Dispatch') OR (GXLB_Val = 'Remain')) then
          SkipRow := True;

        if SkipRow then
        begin
          Q_UBMatching.Next;
          Continue; // B? qua và di t?i dòng ti?p theo
        end;

        // Ði?n d? li?u
        for Col := 0 to DBGridEh7.Columns.Count - 1 do
        begin
          // N?u không ph?i là dòng 'Upper', ?n các c?t d? li?u thông tin chung (c?t 0 d?n 7)
          if (Col <= 7) and (GXLB_Val <> 'Upper') then
            eclApp.Cells[Row, Col+1] := ''
          else
            eclApp.Cells[Row, Col+1] := Q_UBMatching.FieldByName(DBGridEh7.Columns[Col].FieldName).Value;
        end;
        ...
        // Tô màu n?n cho Excel (Kh?p v?i UI c?a DBGrid)
        if (GXLB_Val = 'Total') OR (GXLB_Val = 'Matching') OR (GXLB_Val = 'Shortage') then
          eclApp.Range[eclApp.Cells[Row, 1], eclApp.Cells[Row, DBGridEh7.Columns.Count]].Interior.Color := RGB(204, 255, 255) // Xanh l?t
        else if (GXLB_Val = 'Remain') then
          eclApp.Range[eclApp.Cells[Row, 1], eclApp.Cells[Row, DBGridEh7.Columns.Count]].Interior.Color := RGB(255, 235, 204); // Cam vàng l?t
      ...
      begin
        eclApp.Visible := True; // Ð?m b?o hi?n lên d? user có th? t?t n?u l?i gi?a ch?ng
        ShowMessage(F.Message);
      end;
    end;
  end;
