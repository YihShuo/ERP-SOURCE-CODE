...
var
  OrderCheckCut: TOrderCheckCut;

implementation

uses main1, OrderCheckCut_Det1;

{$R *.dfm}

procedure TOrderCheckCut.FormDestroy(Sender: TObject);
begin
OrderCheckCut:=nil;
end;

procedure TOrderCheckCut.FormClose(Sender: TObject;
...
with QTemp do
  begin
    active:=false;
    sql.Clear;
    sql.add('  if object_id('+''''+'tempdb..#SCBBSCut'+''''+') is not null  ');
    sql.add('begin   drop table #SCBBSCut end   ');
    execsql;
  end;
action:=cafree;
end;

procedure TOrderCheckCut.FormCreate(Sender: TObject);
var i:integer;
NDate:TDate;
begin
with Qtemp do
...
procedure TOrderCheckCut.Panel3DblClick(Sender: TObject);
begin
...
procedure TOrderCheckCut.Panel2DblClick(Sender: TObject);
begin
...
procedure TOrderCheckCut.N2Click(Sender: TObject);
begin
...
procedure TOrderCheckCut.N1Click(Sender: TObject);
begin
...
procedure TOrderCheckCut.Print1Click(Sender: TObject);
begin

PrintDBGrideh1.Preview;
end;

procedure TOrderCheckCut.NN2Click(Sender: TObject);
...
procedure TOrderCheckCut.Button1Click(Sender: TObject);
var i:integer;
begin
with QTemp do
  begin
    ...
    sql.add('into #SCBBSCut from SCBBSS ');
...
with query1 do
  begin
    active:=false;
    sql.clear;
    sql.add('select max(XieXing) as XieXing,max(XieMing) as XieMing,DaoMH ,KFJC,sum(Pairs) as Pairs,sum(C_Qty) as C_Qty,sum(Lack_C) as Lack_C');
    ...
    sql.add('left join (select #SCBBSCut.DDBH,sum(#SCBBSCut.Qty) as C_Qty,max(#SCBBSCut.FinDate) as FinDate ');
    sql.add('           from #SCBBSCut ');
    sql.add('           where #SCBBSCut.GX='+''''+'C'+'''');
    sql.add('           group by #SCBBSCut.DDBH) SCBBC on SCBBC.DDBH=DDZL.ZLBH ');
    sql.add('left join (select #SCBBSCut.DDBH,sum(#SCBBSCut.Qty) as S_Qty,max(#SCBBSCut.FinDate) as FinDate ');
    sql.add('           from #SCBBSCut ');
    sql.add('           where #SCBBSCut.GX='+''''+'S'+'''');
    sql.add('           group by #SCBBSCut.DDBH) SCBBS on SCBBS.DDBH=DDZL.ZLBH ');
    sql.add('left join (select #SCBBSCut.DDBH,sum(#SCBBSCut.Qty) as W_Qty ,max(#SCBBSCut.FinDate) as FinDate');
    sql.add('           from #SCBBSCut ');
    sql.add('           where #SCBBSCut.GX='+''''+'W'+'''');
    sql.add('           group by #SCBBSCut.DDBH) SCBBW on SCBBW.DDBH=DDZL.ZLBH ');
    sql.add('left join (select #SCBBSCut.DDBH,sum(#SCBBSCut.Qty) as O_Qty ,max(#SCBBSCut.FinDate) as FinDate');
    sql.add('           from #SCBBSCut ');
    sql.add('           where #SCBBSCut.GX='+''''+'O'+'''');
    sql.add('           group by #SCBBSCut.DDBH) SCBBO on SCBBO.DDBH=DDZL.ZLBH ');
    sql.add('left join (select #SCBBSCut.DDBH,sum(#SCBBSCut.Qty) as Z_Qty ,max(#SCBBSCut.FinDate) as FinDate');
    sql.add('           from #SCBBSCut ');
    sql.add('           where #SCBBSCut.GX='+''''+'Z'+'''');
    sql.add('           group by #SCBBSCut.DDBH) SCBBZ on SCBBZ.DDBH=DDZL.ZLBH ');
    sql.add('left join (select #SCBBSCut.DDBH,sum(#SCBBSCut.Qty) as A_Qty ,max(#SCBBSCut.FinDate) as FinDate');
    sql.add('           from #SCBBSCut ');
    sql.add('           where #SCBBSCut.GX='+''''+'A'+'''');
    sql.add('           group by #SCBBSCut.DDBH) SCBBA on SCBBA.DDBH=DDZL.ZLBH ');
    sql.add('left join (select #SCBBSCut.DDBH,sum(#SCBBSCut.Qty) as F_Qty ,max(#SCBBSCut.FinDate) as FinDate');
    sql.add('           from #SCBBSCut ');
    sql.add('           where #SCBBSCut.GX='+''''+'F'+'''');   
    sql.add('           group by #SCBBSCut.DDBH) SCBBF on SCBBF.DDBH=DDZL.ZLBH ');
...
procedure TOrderCheckCut.DBGrid1GetCellParams(Sender: TObject;
...
procedure TOrderCheckCut.NN1Click(Sender: TObject);
begin

OrderCheckCut_Det:=TOrderCheckCut_Det.create(self);
OrderCheckCut_Det.show;
end;

procedure TOrderCheckCut.PrintDBGridEh1BeforePrint(Sender: TObject);
begin
if panel2.Visible then
