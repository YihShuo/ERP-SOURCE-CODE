...
begin
  ...
  begin
    ...
    if Length(BuyNoEdit.Text)>=4 then
    begin
      sql.add('select DDZL.XieXing,DDZL.SheHao,DDZL.GXLB,DDZL.BZLB,SCXXCL.XXGS,SCXXCL.BZRS,SCXXCL.BZJS,SCXXCL.BZCL,SCXXCL.USERDATE,SCXXCL.USERID,SCXXCL.YN,XXZL.XieMing,XXZL.DaoMH,xxzl.article,(BZRS+BZJS) as RSJS   ');
      sql.add('from (');
      ...
      sql.add('Union all ');
      sql.add('select XieXing,SheHao,''AR'' as GXLB,''3'' as BZLB from DDZL where DDZL.GSBH='''+main.Edit2.Text+''' and DDZL.BUYNO like '''+BuyNoEdit.Text+'%'' Group by XieXing,SheHao  ');
      sql.add(' ) DDZL ');
    ...
    begin
      sql.add('select SCXXCL.*,XXZL.XieMing,XXZL.DaoMH,xxzl.article,(BZRS+BZJS) as RSJS  ');
      sql.add('from SCXXCL  ');
    ...
    if Length(BuyNoEdit.Text)>=4 then
       sql.add('order by XXZL.Article,DDZL.GXLB')
    else
       sql.add('order by XXZL.Article,SCXXCL.GXLB');
    active:=true;
  end;
...
begin
 ...
 while not query1.Eof do
 begin
   if query1.fieldbyname('BZRS').isnull then
     begin
       showmessage('BZRS can not be empty.');
       abort;
     end;
   if query1.fieldbyname('BZCL').isnull then
     begin
       showmessage('BZCL can not be empty.');
       abort;
     end;
   query1.next;
 end;
...
{procedure TModelCapacity.DBGrid1DblClick(Sender: TObject);  //新增此項 20160826
 var GXLB:array[0..2] of string;
     k:integer;
begin
