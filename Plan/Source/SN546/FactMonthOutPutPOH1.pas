...
type
  TFactMonthOutPutPOH = class(TForm)
    Qtemp: TQuery;
...
var
  FactMonthOutPutPOH: TFactMonthOutPutPOH;

implementation
  uses main1,FunUnit, ExcelSheetDlg1;
{$R *.dfm}
procedure TFactMonthOutPutPOH.ReadIni();
var MyIni :Tinifile;
    AppDir:string;
    DiskNM:string;
begin
...
procedure TFactMonthOutPutPOH.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=CaFree;
end;

procedure TFactMonthOutPutPOH.FormDestroy(Sender: TObject);
begin
  FactMonthOutPutPOH:=nil;
  CreateHRM_DisConnect();
end;

procedure TFactMonthOutPutPOH.CreateHRM_Connect();
begin
  //
  with Qtemp do
  begin
...
//關閉HRM連線
procedure TFactMonthOutPutPOH.CreateHRM_DisConnect();
begin
  with Qtemp do
  begin
    Active:=false;
...
procedure TFactMonthOutPutPOH.Button1Click(Sender: TObject);
...
begin
  ...
  begin
    ...
    SQL.Add('       --工時                  ');
    SQL.Add('       union all');
    SQL.Add('       select DFL,GXLB,''5.Hours'' as SB,Avg(SCRL.SCGS) as Qty,datepart(day, SCBB.SCDate)  as NDay   from (');
    SQL.Add('       select BDepartment.DFL ,BDepartment.ID as DepNo,BDepartment.GXLB');
    SQL.Add('               ,SCBB.SCDate');
    SQL.Add('       from SCBB');
    SQL.Add('       left join BDepartment on BDepartment.ID=SCBB.DepNO ');
    SQL.Add('       where  convert(smalldatetime,convert(varchar,SCBB.SCDate,111)) between ');
    ...
    SQL.Add('              and BDepartment.GSBH='''+main.Edit2.Text+'''  and BDepartment.YN=1 ');
    SQL.Add('              and BDepartment.sumline<>'''+editBB.Text+''' ');
    SQL.Add('       Group by  BDepartment.ID,BDepartment.DFL,BDepartment.GXLB,SCBB.SCDate  ) SCBB');
    ...
    SQL.Add('       Group by DFL ,GXLB,datepart(day, SCBB.SCDate)');
    SQL.Add('       union all');
    SQL.Add('       select DFL ,GXLB,''5.Hours'' as SB,Avg(Qty) as Qty,32 as NDay  from ( ');
    SQL.Add('       select DFL,GXLB,''5.Hours'' as SB,Sum(SCRL.SCGS) as Qty,32 as NDay   from (');
    SQL.Add('       select BDepartment.DFL,BDepartment.ID as DepNo,BDepartment.GXLB');
    SQL.Add('               ,SCBB.SCDate');
    SQL.Add('       from SCBB');
    SQL.Add('       left join BDepartment on BDepartment.ID=SCBB.DepNO ');
    ...
    SQL.Add('       Group by  BDepartment.ID,BDepartment.DFL,BDepartment.GXLB,SCBB.SCDate  ) SCBB');
    SQL.Add('       left join SCRL on SCRL.SCYEAR=year(SCBB.SCDate) and SCRL.SCMONTH=month(SCBB.SCDate)and SCRL.SCDAY=day(SCBB.SCDate)and SCRL.DepNo=SCBB.Depno');
    SQL.Add('       Group by DFL,GXLB  ) SCBB Group by DFL,GXLB       ');
    SQL.Add('       --刷卡人數');
    SQL.Add('       Union all');
    SQL.Add('       select  BDepartment.DFL ,BDepartment.GXLB,''6.Workers'' as SB,Sum(#HumDataPOH.dv_dilam) as Qty,datepart(day, SCBB.SCDate) as NDay');
    SQL.Add('       from SCBB');
    SQL.Add('       left join BDepartment on BDepartment.ID=SCBB.DepNO');
    ...
    SQL.Add('       group by BDepartment.DFL,BDepartment.GXLB,datepart(day, SCBB.SCDate)');
    SQL.Add('       union all');
    SQL.Add('       select DFL,GXLB,''6.Workers'' as SB,Sum(Qty) as Qty,32 as NDay  from ( ');
    SQL.Add('       select  BDepartment.DFL,BDepartment.GXLB,''6.Workers'' as SB,Avg(#HumDataPOH.dv_dilam) as Qty,32 as NDay');
    SQL.Add('       from SCBB');
    SQL.Add('       left join BDepartment on BDepartment.ID=SCBB.DepNO');
    SQL.Add('       left join #HumDataPOH on #HumDataPOH.dv_ma COLLATE DATABASE_DEFAULT =BDepartment.ID_HRM COLLATE DATABASE_DEFAULT  and #HumDataPOH.qt_ngay=SCBB.SCDate');
    SQL.Add('       where  convert(smalldatetime,convert(varchar,SCBB.SCDate,111)) between');
    ...
    SQL.Add('             and BDepartment.GSBH='''+main.Edit2.Text+'''  and BDepartment.YN=1 ');
    SQL.Add('              and BDepartment.sumline<>'''+editBB.Text+''' ');
    SQL.Add('       group by BDepartment.DFL,BDepartment.GXLB ) SCBB Group by DFL  ,GXLB   ');
    SQL.Add('       --                 ');
    SQL.Add('       union all');
    SQL.Add('       ');
    SQL.Add('       select DFL ,GXLB,''1.Sdr'' as SB,Sum(Qty) as Qty, NDay  from ( ');
    SQL.Add('       select T1.DFL,T1.GXLB,T1.SB,sum(T1.Qty*T1.Hours)as Qty,T1.NDay');
    SQL.Add('       from(select  BDepartment.DFL,BDepartment.GXLB,''1.Sdr'' as SB ');
    ...
    SQL.Add('              and BDepartment.GSBH='''+main.Edit2.Text+'''  and BDepartment.YN=1 ');
    SQL.Add('              and BDepartment.sumline<>'''+editBB.Text+''' ');
    SQL.Add('       group by BDepartment.DFL ,BDepartment.GXLB,datepart(day, SCBB.SCDate),SCRL.SCGS )T1 ');
    SQL.Add('       group by  T1.DFL,T1.GXLB,T1.SB,T1.NDay ) T1 Group by DFL,GXLB,T1.SB,T1.NDay    ');
    SQL.Add('       ');
    SQL.Add('       union all');
    SQL.Add('       select DFL ,GXLB,''1.Sdr'' as SB,Sum(Qty) as Qty,32 as NDay  from ( ');
    SQL.Add('       select T1.DFL,T1.GXLB,T1.SB,sum(T1.Qty*T1.Hours)as Qty,32 as NDay');
    SQL.Add('       from(select  BDepartment.DFL,BDepartment.GXLB,''1.Sdr'' as SB ');
    SQL.Add('               ,isnull(avg(SCXXCL.BZCL),0) as Qty,datepart(day, SCBB.SCDate) as NDay, SCRL.SCGS as Hours  ');
    SQL.Add('       from SCBBS ');
    SQL.Add('       left join SCBB on SCBB.ProNo=SCBBS.ProNO ');
    ...
    SQL.Add('              and BDepartment.sumline<>'''+editBB.Text+''' ');
    SQL.Add('       group by BDepartment.DFL ,BDepartment.GXLB,datepart(day, SCBB.SCDate),SCRL.SCGS )T1 ');
    SQL.Add('       group by  T1.DFL,T1.GXLB,T1.SB,T1.NDay ) T1 Group by DFL,GXLB,T1.SB ');
    //-- BB ---------------------------------------------------------
    SQL.Add('       Union all ');
    SQL.Add('       select BDepartment.DFL,BDepartment.GXLB,''3.Actual'' as SB,sum(SCBBS.Qty) as Qty,datepart(day, SCBB.SCDate) as NDay ');
    SQL.Add('                 ');
    SQL.Add('       from SCBBS ');
    SQL.Add('       left join SCBB on SCBB.ProNo=SCBBS.ProNO ');
    SQL.Add('       left join BDepartment on BDepartment.ID=SCBB.DepNO ');
    ...
    SQL.Add('              '''+Formatdatetime('yyyy/MM/dd',sDate)+'''and '''+Formatdatetime('yyyy/MM/dd',eDate)+'''');
    SQL.Add('              and SCBBS.GXLB='''+GXLB+''' ');
    SQL.Add('              and BDepartment.GSBH='''+main.Edit2.Text+''' and BDepartment.YN=1 and BDepartment.sumline='''+editBB.Text+''' ');
    SQL.Add('       group by BDepartment.DFL ,BDepartment.GXLB,datepart(day, SCBB.SCDate)  ');
    SQL.Add('      union  all ');
    SQL.Add('      select BDepartment.DFL,BDepartment.GXLB,''3.Actual'' as SB,isnull(sum(SCBBS.Qty),0) as Qty,32 as NDay  ');
    SQL.Add('      from SCBBS ');
    SQL.Add('      left join SCBB on SCBB.ProNo=SCBBS.ProNO ');
    SQL.Add('      left join BDepartment on BDepartment.ID=SCBB.DepNO ');
    SQL.Add('      where  convert(smalldatetime,convert(varchar,SCBB.SCDate,111)) between ');
    ...
    SQL.Add('             and SCBBS.GXLB='''+GXLB+''' ');
    SQL.Add('             and BDepartment.GSBH='''+main.Edit2.Text+'''  and BDepartment.YN=1 and BDepartment.sumline='''+editBB.Text+'''  ');
    SQL.Add('      group by DFL  ,BDepartment.GXLB');
    SQL.Add('      union  all           ');
    SQL.Add('      select BDepartment.DFL,BDepartment.GXLB,''2.Plan'' as SB,sum(isnull(SCBZCL.Qty,0)) as Qty ,datepart(day, SCBZCL.BZDate) as NDay ');
    SQL.Add('      from SCBZCL ');
    SQL.Add('      left join BDepartment on BDepartment.ID=SCBZCL.DepNO ');
    SQL.Add('      where  convert(smalldatetime,convert(varchar,SCBZCL.BZDate,111)) between ');
    SQL.Add('              '''+Formatdatetime('yyyy/MM/dd',sDate)+'''and '''+Formatdatetime('yyyy/MM/dd',eDate)+'''');
    SQL.Add('              and BDepartment.GXLB='''+DepGXLB+''' ');
    SQL.Add('              and BDepartment.GSBH='''+main.Edit2.Text+'''  and BDepartment.YN=1 and SCBZCL.Qty>0 and BDepartment.sumline='''+editBB.Text+''' ');
    SQL.Add('      group by  BDepartment.DFL,BDepartment.GXLB,datepart(day, SCBZCL.BZDate) ');
    SQL.Add('      union all ');
    SQL.Add('      select BDepartment.DFL,BDepartment.GXLB,''2.Plan'' as SB,isnull(sum(SCBZCL.Qty),0) as Qty ,32 as NDay ');
    SQL.Add('       from SCBZCL ');
    SQL.Add('       left join BDepartment on BDepartment.ID=SCBZCL.DepNO ');
    SQL.Add('       where  convert(smalldatetime,convert(varchar,SCBZCL.BZDate,111)) between ');
    SQL.Add('              '''+Formatdatetime('yyyy/MM/dd',sDate)+'''and '''+Formatdatetime('yyyy/MM/dd',eDate)+'''');
    ...
    SQL.Add('left join (select DepName,GXLB,Qty,NDay from #MonSCBBSPOH  where SB=''6.Workers'' and Qty>0 ) A2 on A1.DepName=A2.Depname and A1.GXLB=A2.GXLB and A1.NDay=A2.NDay');
    SQL.Add('');
    //日平均
    SQL.Add('insert into #MonSCBBSPOH ');
    ...
    SQL.Add('from #MonSCBBSPOH where (SB=''5.Hours'' or SB=''8.POH'' )');
    SQL.Add('group by #MonSCBBSPOH.GXLB,#MonSCBBSPOH.SB,#MonSCBBSPOH.NDay');
    SQL.Add('');
...
procedure TFactMonthOutPutPOH.FormCreate(Sender: TObject);
var i:integer;
...
procedure TFactMonthOutPutPOH.Query1AfterOpen(DataSet: TDataSet);
var i:integer;
begin
   with DBGrideh1 do
   begin
...
procedure TFactMonthOutPutPOH.DBGridEh1DrawColumnCell(Sender: TObject;
...
//外部檔案
procedure TFactMonthOutPutPOH.ExportExternalExcel();
  //
...
//
//預設Excel
procedure TFactMonthOutPutPOH.ExportFormatExcel();
  //
  function GetExcelColumnID(i:integer):string;
  var str:string;
  begin
...
//
procedure TFactMonthOutPutPOH.ExportDefaultExcel();
...
procedure TFactMonthOutPutPOH.Button2Click(Sender: TObject);
begin
