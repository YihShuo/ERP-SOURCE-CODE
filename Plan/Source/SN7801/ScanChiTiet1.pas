...
type
  ...
  private
    ...
    function KiemTraKhungGioTuDB(DataScanTime: TDateTime): Boolean;
  public
    GXLB :String;
    DepName :String;
    GSBH :String;
...
var
  CurrentHM: string;
begin
CurrentHM := FormatDateTime('HH:mm', Now);
  with SMZL do
  begin
    active := false;
    sql.Clear;
    ...
    // --- L?C THEO KHUNG GI? C?A TABLE C?U HÌNH ---
    SQL.Add(' and EXISTS ( ');
    SQL.Add('    select 1 from smzl_Config_EditTimeRange ');
    SQL.Add('    where YN = ''Y'' ');
    SQL.Add('    and ''' + CurrentHM + ''' BETWEEN StartTime AND EndTime '); // Bây gi? ph?i trong khung
    SQL.Add('    and CONVERT(VARCHAR(5), SMZL.ScanDate, 108) BETWEEN StartTime AND EndTime '); // D? li?u cung ph?i trong khung
    SQL.Add(' ) ');
    // ---------------------------------------------
...
begin
    ShowDetail();
      ...
      if (Main.Edit1.Text = SMZL.FieldByName('USERID').AsString) then
...
function TScanChiTiet.KiemTraKhungGioTuDB(DataScanTime: TDateTime): Boolean;
var
  CurrentHM, DataHM: string;
begin
  Result := False;
  
  // L?y gi? hi?n t?i và gi? quét d? li?u d?nh d?ng HH:mm
  CurrentHM := FormatDateTime('HH:mm', Now);
  DataHM    := FormatDateTime('HH:mm', DataScanTime);

  with Qtemp do
  begin
    Active := False;
    SQL.Clear;
    SQL.Add('SELECT TOP 1 StartTime FROM smzl_Config_EditTimeRange');
    SQL.Add('WHERE YN = ''Y''');
    // Ði?u ki?n 1: Th?i gian hi?n t?i (Now) ph?i n?m trong khung cho phép
    SQL.Add('AND ''' + CurrentHM + ''' BETWEEN StartTime AND EndTime');
    // Ði?u ki?n 2: Th?i gian quét (ScanDate) ph?i n?m trong cùng khung dó
    SQL.Add('AND ''' + DataHM + ''' BETWEEN StartTime AND EndTime');
