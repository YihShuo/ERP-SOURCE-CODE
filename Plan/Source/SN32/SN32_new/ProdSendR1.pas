...
begin
  ...
  if (PairsSpinEdit.Value>1) and (XXCCQry.RecordCount > 0) then
  begin
    with Query1 do
    begin
      active:=false;
      ...
      sql.add(' select ');
      XXCCQry.first;
      for i := 0 to XXCCQry.RecordCount -1 do
      begin
        if i < XXCCQry.RecordCount -1 then
          sql.add(' case when sum(['+''+XXCCQry.fieldbyname('GJCC').value+''+'])>0 then round((cast(sum(['+''+XXCCQry.fieldbyname('GJCC').value+''+']) as float)/' +inttostr(PairsSpinEdit.Value) + ') -(sum(['+''+XXCCQry.fieldbyname('GJCC').value+''+'])/' +inttostr(PairsSpinEdit.Value)+'),1)  else null end as '+' '''+XXCCQry.fieldbyname('GJCC').value+''' ,')
        else
          sql.add(' case when sum(['+''+XXCCQry.fieldbyname('GJCC').value+''+'])>0 then round((cast(sum(['+''+XXCCQry.fieldbyname('GJCC').value+''+']) as float)/' +inttostr(PairsSpinEdit.Value) + ') -(sum(['+''+XXCCQry.fieldbyname('GJCC').value+''+'])/' +inttostr(PairsSpinEdit.Value)+'),1)  else null end as '+' '''+XXCCQry.fieldbyname('GJCC').value+''' ');
        XXCCQry.Next;
      end;
...
begin
  ...
  begin
    Active:=false;
    sql.Clear;
    sql.Add('select 0 as YN,CLLBZL.ywsm as Memo,lastsizeR.CLBH,ywpm ');
    XXCCQry.first;
    while not XXCCQry.Eof do
    begin
    ...
    sql.Add('left join '+main.LIY_DD+'.dbo.CLLBZL CLLBZL on CLLBZL.cllb=lastsizefox.lbdh ');
    sql.Add('where 1=1 ');
    sql.Add('and CLZL.YWPM ='''+SMDDS.FieldByName('XTMH').AsString+''' ');
    sql.Add('and CLLBZL.YWSM ='+''''+SizeFoxTypeEdit.Text+'''');
    sql.Add('group by CLLBZL.ywsm,lastsizeR.CLBH,ywpm  ');
    active:=true;
    with DBGridEh2 do
    begin
