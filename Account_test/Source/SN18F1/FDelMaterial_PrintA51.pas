...
type
  TFDelMaterial_PrintA5 = class(TForm)
    Quickrep1: TQuickRep;
    Qry_DelDet: TQuery;
...
var
  FDelMaterial_PrintA5: TFDelMaterial_PrintA5;
  sPaperSize: string;
  sHeight: Real;

implementation

uses DeliverList1, main1, Deliver_Account_OLD1;
{$R *.dfm}

procedure TFDelMaterial_PrintA5.FormDestroy(Sender: TObject);
begin
  FDelMaterial_PrintA5 := nil;
end;

procedure TFDelMaterial_PrintA5.Formclose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFDelMaterial_PrintA5.ReadIni();
var MyIni :Tinifile;
...
begin
  ...
  if (Deliver_Account_OLD.DelMas.FieldByName('CKBH').AsString='VTXW') then
  begin
    NPVT:='NP_HC';
  end;
  if ((Deliver_Account_OLD.DelMas.FieldByName('CKBH').AsString='VA12')) then
  begin
    NPVT:='00186';
  end;
  if ((Deliver_Account_OLD.DelMas.FieldByName('CKBH').AsString='VA3')) then
  begin
    NPVT:='NP_A3';
  end;
  if ((Deliver_Account_OLD.DelMas.FieldByName('CKBH').AsString='VB2')) then
  begin
    NPVT:='NP_B2';
  end;
  if ((Deliver_Account_OLD.DelMas.FieldByName('CKBH').AsString='CDC')) then
  begin
    NPVT:='NP_CDC';
  end;
  if ((Deliver_Account_OLD.DelMas.FieldByName('CKBH').AsString='KDC')) then
  begin
    NPVT:='NP_KDC';
  end;
  if ((Deliver_Account_OLD.DelMas.FieldByName('CKBH').AsString='VB1') or (Deliver_Account_OLD.DelMas.FieldByName('CKBH').AsString='VB7'))  then
  begin
    NPVT:='01460';
  end;
  ...
  begin
    ...
    SQL.Add('where KCLL.LLNO='''+Deliver_Account_OLD.DelMas.fieldbyname('LLNO').AsString+''' ');
    Active:=true;
...
procedure TFDelMaterial_PrintA5.FormCreate(Sender: TObject);
var i,j:integer;
    ADate, BDate : TDateTime;
begin
  ReadIni();
  ADate := Deliver_Account_OLD.DelMas.fieldbyname('USERDATE').Value;
  BDate := EncodeDate(2023, 6, 30);
  if (CompareDate(ADate,BDate)>0) and (check='TYXUAN') then
  begin
    QRCodeBitmap:=TBitmap.Create;
    UpLoadQRImg(Deliver_Account_OLD.DelMas.fieldbyname('LLNO').AsString);
...
procedure TFDelMaterial_PrintA5.UpLoadQRImg(QRdata:string);
var
...
procedure TFDelMaterial_PrintA5.Qry_DelDetCalcFields(DataSet: TDataSet);
begin
...
procedure TFDelMaterial_PrintA5.DetailBand1AfterPrint(Sender: TQRCustomBand; BandPrinted: Boolean);
begin
  sHeight := sHeight + DetailBand1.size.Height + DetailBand1.Expanded;
end;

procedure TFDelMaterial_PrintA5.Quickrep1EndPage(Sender: TCustomQuickRep);
begin
