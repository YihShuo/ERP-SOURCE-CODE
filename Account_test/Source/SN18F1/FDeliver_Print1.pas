...
var
  FDeliver_Print: TFDeliver_Print;

implementation
   uses DeliverList1, main1, Deliver_Account_OLD1;
{$R *.dfm}

procedure TFDeliver_Print.FormDestroy(Sender: TObject);
begin
  FDeliver_Print:=nil;
end;

procedure TFDeliver_Print.ReadIni();
var MyIni :Tinifile;
...
procedure TFDeliver_Print.FormCreate(Sender: TObject);
var i,j:integer;
...
begin
  ...
  GSBHLabel.Caption:=Deliver_Account_OLD.DelMas.fieldByName('GSBH').AsString;
end;

procedure TFDeliver_Print.UpLoadQRImg(QRdata:string);
var
...
procedure TFDeliver_Print.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:=Cafree;
end;
