...
begin
  ...
  begin
    active := false;
    sql.Clear;
    if main.Edit2.Text = 'TBA' then begin
      SQL.Add('select invoiced.ExFty_Date,invoiced.INV_NO, invoiced.INV_DATE,invoiced.RISK,invoiced.Company_Name,invoiced.RYNO,invoiced.ARTICLE,invoiced.PAIRS, invoiced.UNIT_PRICE,');
      SQL.Add('invoiced.AMOUNT,invoiced.Customs_No, invoiced.Sailing_Date,invoiced.Bill_FCR_No,invoiced.Kind_Loading,');
      SQL.Add('invoiced.Forwarder,invoiced.CUSTID,invoiced.COUNTRYEN,invoiced.inv_Type,');
      SQL.Add('invoiced.Shipmode,invoiced.SPECID,invoiced.REF,invoiced.TC_PL_Date,invoiced.Booking_No,');
      SQL.Add('invoiced.IODate,');
      SQL.Add('case when con = 1 then invoiced.Qty else invoiced.PAIRS end as Qty,');
      SQL.Add('invoiced.CWHL,invoiced.AcVNACC,invoiced.YSBH,invoiced.VAT_NO,invoiced.VAT_Date from(');
      SQL.Add('select *,(select count(*) from RY_VNPRICE where RYNO = invoice.RYNO) as con from(');
    end;
    ...
    Sql.Add('LEFT JOIN ( SELECT YWCP.DDBH,CONVERT(VARCHAR(7),ISNULL(YWCP_OUT.ExeDate,YWCP.ExeDate),111) IODate, SUM(ISNULL(YWCP_OUT.Qty_Out,YWCP.Qty)) QTY ,INV_NO');
    ...
    sql.Add('       		 WHERE YWCP.SB=''3'' GROUP BY YWCP.DDBH, CONVERT(VARCHAR(7),ISNULL(YWCP_OUT.ExeDate,YWCP.ExeDate),111),INV_NO) DDZLWHEX ');
    sql.Add('         ON DDZLWHEX.DDBH=inv.RYNO AND DDZLWHEX.IODate=CONVERT(VARCHAR(7),sb.ExFty_Date,111) and  DDZLWHEX.INV_NO=sb.INV_NO');
