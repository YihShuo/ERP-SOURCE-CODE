unit SemiMaterialMonth_Sample1;
...
type
  TSemiMaterialMonth_Sample = class(TForm)
    PC1: TPageControl;
    TS1: TTabSheet;
    Panel2: TPanel;
    Label4: TLabel;
    ...
    procedure cbCKBHChange(Sender: TObject);
    procedure cbCKBH1Change(Sender: TObject);
    procedure PC1Change(Sender: TObject);
    procedure Adjust_NVL_DKClick(Sender: TObject);
  private
    AppDir:string;
...
var
  SemiMaterialMonth_Sample: TSemiMaterialMonth_Sample;

implementation

uses fununit, main1, Adjust_CPNVL_PB_Sample1,
  SemiMaterialMonth_Sample_Detail1, SemiMaterialMonth_Sample_Print1,
  SemiMaterialMonth_Sample_ZZ1;

{$R *.dfm}
procedure TSemiMaterialMonth_Sample.Readini();
var MyIni:TiniFile;
begin
  ...
  begin
    try
      ...
      CKBHID:=MyIni.ReadString('Account_N179222_Sample','CKBHID','CDC');
      CKBH_SQL:=MyIni.ReadString('Account_N179222_Sample','CKBH_SQL',' in (''CDC'',''CDT'',''BYC'',''BYT'',''SRC'',''SRT'',''SDC'',''SDT'')');
    finally
      MyIni.Free;
    end;
  end;
...
procedure TSemiMaterialMonth_Sample.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TSemiMaterialMonth_Sample.FormDestroy(Sender: TObject);
begin
  SemiMaterialMonth_Sample:=nil;
end;

procedure TSemiMaterialMonth_Sample.CW_MatCostMonth();
begin
...
procedure TSemiMaterialMonth_Sample.CW_MatCostMonth_Temp();
var
...
begin
  ...
  //
  //計算攤提金額
  if Formatdatetime('yyyy/MM/dd',endofthemonth(sDate)) < '2023/10/01' then
  begin
    VNACC := '0.0';
  end else
  begin
    ...
    begin
      ...
      //2025.05.07
      SQL.Add('             or KCLLS.SCBH like ''CUIT%'' or KCLLS.SCBH like ''CUIK%'' or KCLLS.SCBH like ''UQIT%'' or KCLLS.SCBH like ''UQIK%'' ');
      //2025.08.04
      SQL.Add('             or KCLLS.SCBH like ''VSIT%'' or KCLLS.SCBH like ''VSIK%'' ');
      //2025.12.03
      SQL.Add('             or KCLLS.SCBH like ''VJIT%'' or KCLLS.SCBH like ''VJIK%'') ');
      //==========
      ...
      //2025.05.07
      SQL.Add('             or KCLLS_CB.SCBH_Cost like ''CUIT%'' or KCLLS_CB.SCBH_Cost like ''CUIK%'' or KCLLS_CB.SCBH_Cost like ''UQIT%'' or KCLLS_CB.SCBH_Cost like ''UQIK%'' ');
      //2025.08.04
      SQL.Add('             or KCLLS_CB.SCBH_Cost like ''VSIT%'' or KCLLS_CB.SCBH_Cost like ''VSIK%'' ');
      //2025.12.03
      SQL.Add('             or KCLLS_CB.SCBH_Cost like ''VJIT%'' or KCLLS_CB.SCBH_Cost like ''VJIK%'') ');
      //==========
      SQL.Add('     			) KCLL');
      SQL.Add('   	 ) KCLL');
      ...
      SQL.Add('             From ( ');
      SQL.Add('       			Select shoetest.YPDH,sum(shoetest2.qty) as InQty');
      ...
      //2025.05.07
      SQL.Add('             AND YPZL.GSDH='''+CKBHID+''' ');
      //==========
      SQL.Add('             Group by shoetest.YPDH )YPZL');
      SQL.Add('       		) B on 1=1');
  ...
  begin
    ...
    SQL.Add('  Left join KCCK on KCCK.CKBH=KCLL.CKBH ');
    SQL.Add('  WHERE KCLL.CFMID <> ''NO'' AND CONVERT(VARCHAR, KCLL.CFMDate, 111) BETWEEN '''+Formatdatetime('yyyy/MM/dd',startofthemonth(sDate))+''' and '''+Formatdatetime('yyyy/MM/dd',endofthemonth(sDate))+''' ');
    SQL.Add('  AND SUBSTRING(KCLLS.SCBH,4,1)=''V'' AND KCLLS.SCBH NOT LIKE ''CBYV%''');
    SQL.Add('  and KCCK.CKBH'+CKBH_SQL);   //20230623
    if (CBX_HGLB2.ItemIndex > 0) AND (CBX_HGLB2.ItemIndex <> 5) then
    SQL.Add('  AND KCLLS.HGLB = ''' + CBX_HGLB2.Text + '''');
    if (CKBHID='VC1') then
    SQL.Add('  AND KCLLS.HGLB<>''GC'' ');
    ...
    SQL.Add('                  	 ,IsNull(YWCPKC.KCQty,0) as KCQty, YWCP_Mon.InQty,Isnull(CWKCLLMonth.VNACC,0)+isnull(CWKCLLMonth_Adjust_DK.ExchACC,0) as VNACCMon,IsNull(KCLLS.VNACC,0)- Isnull(RK_THUHOI.VNACC,0)+isnull(CWKCLLMonth_Adjust_TT.ExchACC,0)  as VNACC');
    ...
    //開發樣品訂單分批數量出貨
    SQL.Add('Left join (');
    SQL.Add(' SELECT YPZL.YPDH AS YSBH, SUM(shoetestshipping.qty) AS Pairs ');
    SQL.Add(' FROM YPZL');
    ...
    SQL.Add('left join ( select DDBH,isnull(ExchACC,0) as ExchACC');
    SQL.Add('            from CWKCLLMonth_Adjust_PB');
    SQL.Add('      			where KCYEAR='''+ssYear+''' and KCMONTH='''+ssMonth+'''  and LB=''DK''');
    SQL.Add('             AND SUBSTRING(CWKCLLMonth_Adjust_PB.DDBH,4,1)=''V'' ');
    SQL.Add('			        and CKBH='''+CKBHID+''' ');
    SQL.Add('          ) CWKCLLMonth_Adjust_DK on CWKCLLMonth_Adjust_DK.DDBH = DDZL.DDBH');
    //當期領料
    ...
    begin
      SQL.Add('Union All');
      SQL.Add('select ''DICH VU KHAI THAC'' as DDBH,1 as Pairs, 0 as FKCQty, 1 as NoFKCQty,1 as InQty,0 as VNACCMon,KCLL_DICHVUKHAITHAC.VNACC,round('+VNACC+',0) as ZVNACC,0 as NotInQty,0 as SemiVNACC,round(KCLL_DICHVUKHAITHAC.VNACC+'+VNACC+',0) as FShoeVNACC,round(KCLL_DICHVUKHAITHAC.VNACC+'+VNACC+',0)  as FShoe1Pair_VNACC');
      SQL.Add('from (	select sum(VNACC) as VNACC ');
      SQL.Add('   		  from(	Select Sum(VNACC_HG) as VNACC ');
      SQL.Add('              From KCLL');
      SQL.Add('     				  Left join KCLLS on KCLL.LLNO=KCLLS.LLNO ');
      ...
      SQL.Add('               and (KCLLS.SCBH like ''BYIT%'' or KCLLS.SCBH like ''BYIK%'' or KCLLS.SCBH like ''CBYK%'' or  KCLLS.SCBH like ''LAIK%'' or KCLLS.SCBH like ''LAIT%'' or KCLLS.SCBH like ''ZZZZ%'')');  //BYIK,BYIT,CBYK,LAIK,LAIT,ZZZZZ
      SQL.Add('     				  and not exists(select * from KCLLS_CB where LLNO=KCLLS.LLNO and SCBH=KCLLS.SCBH and CLBH=KCLLS.CLBH) ');
      SQL.Add('     				  UNION ALL');
      SQL.Add('     				  SELECT Sum(VNACC) as VNACC ');
      SQL.Add('     				  FROM KCLLS_CB ');
      ...
      if CBX_HGLB2.ItemIndex=5 then
      SQL.Add('     				  and left(KCLLS_CB.HGLB,2) in (''NK'',''KD'',''HD'',''TC'',''NQ'') ');
      SQL.Add('      			  and KCLLS_CB.CostID like ''621%'' ');
      SQL.Add('             and (KCLLS_CB.SCBH_Cost like ''BYIT%'' or KCLLS_CB.SCBH_Cost like ''BYIK%'' or KCLLS_CB.SCBH_Cost like ''CBYK%'' or KCLLS_CB.SCBH_Cost like ''LAIK%'' or KCLLS_CB.SCBH_Cost like ''LAIT%'' or KCLLS_CB.SCBH_Cost like ''ZZZZ%'')');      //BYIK,BYIT,CBYK,LAIK,LAIT,ZZZZZ
...
procedure TSemiMaterialMonth_Sample.Button1Click(Sender: TObject);
var sDate:TDate;
begin
  sDate := EncodeDate(StrToInt(CBX3.Text), StrToInt(CBX4.Text), 1);
...
procedure TSemiMaterialMonth_Sample.FormCreate(Sender: TObject);
var
...
procedure TSemiMaterialMonth_Sample.Button5Click(Sender: TObject);
var sDate:TDate;
begin
  sDate := EncodeDate(StrToInt(CBX3.Text), StrToInt(CBX4.Text), 1);
  if Formatdatetime('yyyy/MM/dd',endofthemonth(sDate)) < '2023/05/01' then
...
procedure TSemiMaterialMonth_Sample.Adjust_NVL_PBClick(Sender: TObject);
begin
  Adjust_CPNVL_PB_Sample:=TAdjust_CPNVL_PB_Sample.create(self);
  Adjust_CPNVL_PB_Sample.Lbl_LB.Caption:='PB';
  Adjust_CPNVL_PB_Sample.Button1.Click;
  Adjust_CPNVL_PB_Sample.show;
end;

procedure TSemiMaterialMonth_Sample.Adjust_NVL_TTClick(Sender: TObject);
begin
  Adjust_CPNVL_PB_Sample:=TAdjust_CPNVL_PB_Sample.create(self);
  Adjust_CPNVL_PB_Sample.Lbl_LB.Caption:='TT';
  Adjust_CPNVL_PB_Sample.Button1.Click;
  Adjust_CPNVL_PB_Sample.show;
end;

procedure TSemiMaterialMonth_Sample.Button4Click(Sender: TObject);
begin
...
procedure TSemiMaterialMonth_Sample.Modify1Click(Sender: TObject);
begin
  with Query1 do
  begin
    requestlive:=true;
...
procedure TSemiMaterialMonth_Sample.Save1Click(Sender: TObject);
var i:integer;
begin
  Query1.First;
...
procedure TSemiMaterialMonth_Sample.Cancel1Click(Sender: TObject);
begin
  with Query1 do
  begin
    active:=false;
...
procedure TSemiMaterialMonth_Sample.Button3Click(Sender: TObject);
begin
  ExportDefaultExcel(Query1);
end;

procedure TSemiMaterialMonth_Sample.Button2Click(Sender: TObject);
begin
...
procedure TSemiMaterialMonth_Sample.ExportDefaultExcel(Query:TQuery);
var
...
procedure TSemiMaterialMonth_Sample.ExportFormatExcel(Query: TQuery);
var
  eclApp,WorkBook: OleVariant;
  i, j, k, l: integer;
  ...
  sYear, sMonth: string;
begin
  sYear := SemiMaterialMonth_Sample.CBX3.Text;
  sMonth := SemiMaterialMonth_Sample.CBX4.Text;
  if (Length(sMonth) = 1) then
    sMonth := '0' + sMonth;
  ...
  begin
    ...
    try
      ...
      begin
        eclApp.Cells[j, 1] := SemiMaterialMonth_Sample.CKBHID;
...
procedure TSemiMaterialMonth_Sample.mnu1Click(Sender: TObject);
begin
  SemiMaterialMonth_Sample_Detail:=TSemiMaterialMonth_Sample_Detail.Create(self);
  SemiMaterialMonth_Sample_Detail.Show;
end;

procedure TSemiMaterialMonth_Sample.Button8Click(Sender: TObject);
begin
  SemiMaterialMonth_Sample_Print := TSemiMaterialMonth_Sample_Print.Create(self);
  SemiMaterialMonth_Sample_Print.Quickrep1.Prepare;
  SemiMaterialMonth_Sample_Print.PageN.Caption := IntToStr(SemiMaterialMonth_Sample_Print.Quickrep1.QRPrinter.PageCount);
  SemiMaterialMonth_Sample_Print.Quickrep1.Preview;
  SemiMaterialMonth_Sample_Print.Free;
end;

procedure TSemiMaterialMonth_Sample.DBGridEh2DrawColumnCell(
...
procedure TSemiMaterialMonth_Sample.Save_GTClick(Sender: TObject);
begin
  if Query2.Active=false then
  begin
    Showmessage('Please Query first');
...
procedure TSemiMaterialMonth_Sample.Confirm_GTClick(Sender: TObject);
begin
  with TempQry do
  begin
    Active := false;
...
procedure TSemiMaterialMonth_Sample.Query2AfterOpen(DataSet: TDataSet);
begin
  with TempQry do
  begin
    active:=false;
...
procedure TSemiMaterialMonth_Sample.Modify_SLClick(Sender: TObject);
begin
  with Query2 do
  begin
    requestlive:=true;
...
procedure TSemiMaterialMonth_Sample.Save_SLClick(Sender: TObject);
var i:integer;
begin
  Query2.First;
...
procedure TSemiMaterialMonth_Sample.Cancel_SLClick(Sender: TObject);
begin
  with Query2 do
  begin
    active:=false;
...
procedure TSemiMaterialMonth_Sample.Button6Click(Sender: TObject);
var sDate:TDate;
begin
  sDate := EncodeDate(StrToInt(CBX3.Text), StrToInt(CBX4.Text), 1);
  if Formatdatetime('yyyy/MM/dd',endofthemonth(sDate)) >='2023/10/01' then
  begin
    SemiMaterialMonth_Sample_ZZ:=TSemiMaterialMonth_Sample_ZZ.Create(Self);
    SemiMaterialMonth_Sample_ZZ.Show;
  end else
  begin
    showmessage(' Truoc 2023/10 Khong Co CPNVL Phan Bo ');
    abort;
  end;
end;

procedure TSemiMaterialMonth_Sample.cbCKBHChange(Sender: TObject);
begin
  CKBHID:=cbCKBH.Text;
  CKBH_SQL:=' in ('''+cbCKBH.Text+''')';
end;

procedure TSemiMaterialMonth_Sample.cbCKBH1Change(Sender: TObject);
begin
  CKBHID:=cbCKBH1.Text;
  CKBH_SQL:=' in ('''+cbCKBH1.Text+''')';
end;

procedure TSemiMaterialMonth_Sample.PC1Change(Sender: TObject);
begin
...
procedure TSemiMaterialMonth_Sample.Adjust_NVL_DKClick(Sender: TObject);
begin
  Adjust_CPNVL_PB_Sample:=TAdjust_CPNVL_PB_Sample.create(self);
  Adjust_CPNVL_PB_Sample.Lbl_LB.Caption:='DK';
  Adjust_CPNVL_PB_Sample.Button1.Click;
  Adjust_CPNVL_PB_Sample.show;
end;
