...
type
  ...
  public
    ...
    CFMID,CFMID1,CFMID2,NVK,NVTM,Change:string;
  end;
...
begin
  ...
  begin
    try
      ...
      Change:=MyIni.ReadString('ERP','Account_N18D_Change_'+main.Edit2.Text,'N');
    finally
      MyIni.Free;
    end;
  end;
  ...
  begin
    ...
    ePD.ReadOnly:=False;
    eCQK.ReadOnly:=False;
    eCQBP.ReadOnly:=False;
    eNVK.ReadOnly:=False;  
    eNVTM.ReadOnly:=False;
  end;
...
begin
  if (Change = 'Y') and ((ePD.Text='') or (eCQBP.Text='') or (eCQK.Text='') or (eNVK.Text='') or (eNVTM.Text='')) then
  begin
    showmessage('Pls set signed.');
    abort;
  end;
  with BDelRec do
  begin
    ...
    begin
      ...
      SQL.Add('  set CFMID='''+eCQK.Text+''', CFMDATE=getdate()  ');
      SQL.Add('      ,CFMID1='''+eCQBP.Text+''', CFMDATE1=getdate()  ');
      SQL.Add('  	   ,CFMID2='''+ePD.Text+''', CFMDATE2=getdate()  ');
      SQL.Add('  where RKNO='''+EnMas.FieldByName('RKNO').AsString+'''  ');
      ExecSQL;
    end else
    begin
      ...
      SQL.Add('      ,'''+eCQK.Text+''', getdate()  ');
      SQL.Add('      ,'''+eCQBP.Text+''', getdate()  ');
      SQL.Add('  	   ,'''+ePD.Text+''', getdate() )');
      ExecSQL;
    end;
  end;
