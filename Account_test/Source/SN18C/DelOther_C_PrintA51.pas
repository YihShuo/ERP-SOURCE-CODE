...
type
  TDelother_C_PrintA5 = class(TForm)
    Quickrep1: TQuickRep;
    Qry_DelDet: TQuery;
...
var  Delother_C_PrintA5: TDelother_C_PrintA5;
  sPaperSize: string;
  sHeight: Real;
...
procedure TDelother_C_PrintA5.FormDestroy(Sender: TObject);
begin
  Delother_C_PrintA5 := nil;
end;

procedure TDelother_C_PrintA5.Formclose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TDelother_C_PrintA5.ReadIni();
var MyIni :Tinifile;
...
begin
  ...
  if Deliver_Account.eNPVT.Text <> '' then NPVT_Img:= Deliver_Account.eNPVT.Text+'.bmp';
  with TmpQuery do
  begin
    Active:=false;
    SQL.Clear;
...
procedure TDelother_C_PrintA5.FormCreate(Sender: TObject);
var i,j:integer;
...
procedure TDelother_C_PrintA5.UpLoadQRImg(QRdata:string);
var
...
procedure TDelother_C_PrintA5.Qry_DelDetCalcFields(DataSet: TDataSet);
begin
  Qry_DelDetS.Close;
  Qry_DelDetS.Open;
  Qry_DelDetS.First;
...
procedure TDelother_C_PrintA5.DetailBand1AfterPrint(Sender: TQRCustomBand; BandPrinted: Boolean);
begin
  sHeight := sHeight + DetailBand1.size.Height + DetailBand1.Expanded;
end;

procedure TDelother_C_PrintA5.Quickrep1EndPage(Sender: TCustomQuickRep);
begin
