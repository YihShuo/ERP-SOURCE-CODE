...
begin
  ...
  begin
    Active:=false;
    sql.Clear;
    sql.add('select KCRK.*,ZSZL.ZSYWJC,KCPK.Declaretion  ');
    sql.add('from KCRK');
  ...
  begin
    ...
    ExecSQL();
    sql.Clear;
    sql.Add('select KCRKS.*,CLZL.YWPM,CLZL.DWBH,CGZL_DeclarationS.Y_DonGia as DONGIA  ');
    sql.Add('from KCRKS ');
...
var IsUpdate,IsUpdate_USACC:boolean;
begin
  //檢查
  with EnMas do
  begin
  ...
  try  //更新真正庫存
    ...
    begin
        ...
        begin
            ...
            begin
              IsUpdate:=true;
              if (EnDet.fieldbyname('USACC').OldValue <> EnDet.fieldbyname('USACC').Value) then
              begin
                 IsUpdate_USACC:=true;
                 with  BDelRec do
                 begin
                    ...
                    sql.add(','''+EnDet.fieldbyname('CLBH').Value+'***USACC:'+FloatToStr(EnDet.fieldbyname('USACC').OldValue)+'***'+FloatToStr(EnDet.fieldbyname('USACC').Value)+''','+''''+EnMas.fieldbyname('USERID').Value+''''+',');
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if ((IsUpdate_USACC=true) and (EnMas.FieldByName('CFMID').AsString<>'NO')) then
          begin
