...
type
  TSemiMaterialMonth = class(TForm)
    ...
    procedure Cancel_GTClick(Sender: TObject);
  private
    AppDir:string;
...
begin
  ...
  begin
    ...
    SQL.Add('     				and IsNull(YWCP.SB,'''')<>'''' and DDZL.GSBH='''+CKBHID+'''  ');
    ...
    if cbCKBH.Text='VA12' then
      SQL.Add('       		where DDBH like ''Y%'' ');
    if cbCKBH.Text='VC2' then
      SQL.Add('       		where DDBH like ''SJ%'' ');
    ...
    SQL.Add('             and DDBH in (select KCLLS.SCBH from KCLLS left join KCLL on KCLLS.LLNO=KCLL.LLNO Left join KCCK on KCCK.CKBH=KCLL.CKBH where KCLL.CFMID<>''NO'' and Convert(varchar,KCLL.CFMDate,111) between ''' + FormatDatetime('yyyy/MM/dd', StartOfTheMonth(sDate)) + ''' AND ''' + FormatDatetime('yyyy/MM/dd', EndOfTheMonth(sDate))+''' ');
    SQL.Add('             and KCCK.CKBH'+SemiMaterialMonth.CKBH_SQL);
    SQL.Add('             )');
  ...
  begin
    ...
    SQL.Add('     and KCCK.CKBH'+SemiMaterialMonth.CKBH_SQL);
     ...
     SQL.Add('  	   from ( select DDZL.DDBH,DDZL.Pairs,Case when ((IsNull(CWKCLLMonth.VNACC,0)=0) and (IsNull(YWCPKC.KCQty,0)=0)) then null else  DDZL.Pairs- IsNull(YWCPKC.KCQty,0)- IsNull(YWCP_Shipping.ShipQty,0)  end as FKCQy');
     SQL.Add('                  	 ,IsNull(DDZL.Pairs- IsNull(YWCPKC.KCQty,0),DDZL.Pairs)- IsNull(YWCP_Shipping.ShipQty,0) as NoFKCQty1,DDZL.Pairs-IsNull(YWCPKC.KCQty,0)-IsNull(YWCP_Mon.InQty,0)- IsNull(YWCP_Shipping.ShipQty,0) as NotInQty');
    ...
    sql.add('           left join YWCP_OUT on YWCP_OUT.CARTONBAR=YWCP.CARTONBAR');
     ...
     //
     SQL.Add('          ) YWCP_Shipping on YWCP_Shipping.YSBH=DDZL.DDBH ');
     //調整
    ...
    //20230325全部原料品倉
    SQL.Add('             and KCCK.CKBH'+SemiMaterialMonth.CKBH_SQL);
     SQL.Add('      			AND KCRKS.CostID like ''6211%'''); //THU HOI
     if (CBX_HGLB2.ItemIndex>0) and (CBX_HGLB2.ItemIndex<>5) then
     SQL.Add('            AND KCRK.HGLB ='''+CBX_HGLB2.Text+'''');
     if CBX_HGLB2.ItemIndex=5 then
     SQL.Add('            AND left(KCRK.HGLB,2) in (''NK'',''KD'',''HD'',''TC'',''NQ'')');
...
procedure TSemiMaterialMonth.cbCKBHChange(Sender: TObject);
begin
  CKBHID:=cbCKBH.Text;
  CKBH_SQL:=' in ('''+cbCKBH.Text+''')';  
end;

procedure TSemiMaterialMonth.cbCKBH1Change(Sender: TObject);
begin
  CKBHID:=cbCKBH1.Text;
  CKBH_SQL:=' in ('''+cbCKBH1.Text+''')';
end;

procedure TSemiMaterialMonth.PC1Change(Sender: TObject);
begin
  if PC1.ActivePageIndex = 1 then
  begin
    CKBHID:=cbCKBH.Text;
...
procedure TSemiMaterialMonth.Cancel_GTClick(Sender: TObject);
begin
  if Query2.Active=false then
  begin
    Showmessage('Please Query first');
  ...
  begin
    ...
    begin
      Active := false;
      SQL.Clear;
      SQL.Add('INSERT INTO CW_MatCostMonth_Temp(KCYEAR, KCMONTH, CKBH, DDBH, DELID,DELDATE,Pairs,FKCQy,NoFKCQty1,InQty,VNACCMon,VNACC,ZVNACC,NotInQty,SemiVNACC,FShoeVNACC,FShoe1Pair_VNACC,USERID,USERDATE,YN)');
      SQL.Add('SELECT ''' + CBX3.Text + ''' AS KCYEAR, ''' + CBX4.Text + ''' AS KCMONTH, ''' + CKBHID + ''' AS CKBH,DDBH, ''' + main.Edit1.Text + ''' AS DELID, GetDate() AS DELDATE,Pairs,FKCQy,NoFKCQty1,InQty,VNACCMon,VNACC,ZVNACC,NotInQty,SemiVNACC,FShoeVNACC,FShoe1Pair_VNACC,USERID,USERDATE, ''2'' AS YN FROM (');
      SQL.Add('Select * from CW_MatCostMonth WHERE KCYEAR = ''' + CBX3.Text + ''' AND KCMONTH = ''' + CBX4.Text + ''' AND CKBH = ''' + CKBHID + ''' and YN=''2'' ');
      SQL.Add(') SemiMat_Mon');
      SQL.Add('DELETE FROM CW_MatCostMonth WHERE KCYEAR = ''' + CBX3.Text + ''' AND KCMONTH = ''' + CBX4.Text + ''' AND CKBH = ''' + CKBHID + ''' and YN=''2'' ');
      ExecSQL;
    end;
