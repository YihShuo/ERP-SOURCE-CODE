...
type
  TDeliverCheck = class(TForm)
    ...
    chk_GC: TCheckBox;
    Query3DOCDATE: TDateTimeField;
    chkTK: TCheckBox;
    Query2TypeCost: TStringField;
...
begin
    ...
    try
      ...
      begin
        if (DBGrid.Columns[i].Title.Caption = 'DepID') or (DBGrid.Columns[i].Visible) then
...
begin
  ...
  begin
    ...
    begin
      ...
      sql.Add('       ,isnull(KCLLS.HGLB,''ZZZZ'')as HGLB,CLZL.clzmlb');
  ...
  begin
    ...
    begin
      ...
      begin
        SQL.Add('       ,'''' as Declaretion ');
      end;
      SQL.Add('       ,clzmlb,TypeCost ');
      SQL.add('from ( ');
      SQL.Add('select KCLL.CKBH,KCLLS.LLNO,KCLL.DepID,BDepartment.DepName,KCLL.CFMDATE,KCLLS.CLBH,CLZL.YWPM,CLZL.DWBH,KCZLS.CWBH,KCLLS.DFL');
      ...
      sql.Add('       ,isnull(KCLLS.HGLB,''ZZZZ'') as HGLB,KCLLS.LLMemo,KCLLS.CNO1,ZLZLS2.CLSL,TCLYL,CLZL.clzmlb,Bdepartment_Cost.TypeCost');
      SQL.add('from (select * from KCLLS union all select * from KCLLS_2014) KCLLS ');
      SQL.add('left join (select * from KCLL union all select * from KCLL_2014) KCLL on KCLL.LLNO=KCLLS.LLNO');
      SQL.add('left join CLZL on CLZL.CLDH=KCLLS.CLBH');
      SQL.add('left join BDepartment on BDepartment.ID=KCLL.DepID');
      SQL.add('left join Bdepartment_Cost on Bdepartment_Cost.ID=BDepartment.ID');
      SQL.add('left join SCBLYY on SCBLYY.YYBH=KCLLS.YYBH ');
      SQL.add('left join KCZLS on KCZLS.CLBH=KCLLS.CLBH and KCZLS.CKBH=KCLL.CKBH');
      SQL.add('left join KCCK on KCCK.CKBH=KCLL.CKBH ');
      sql.Add('left join CLHG on CLHG.CLBH=KCLLS.CLBH and KCLLS.HGLB =''NK'' and Year(KCLLS.USERDATE)>=2017');
      ...
      SQL.add(') KCLLS ');
      SQL.add('group by CKBH,LLNO,DepID,DepName,CFMDATE,CLBH,YWPM,DWBH,CWBH,DFL,Qty,VNPrice,VNACC,SCBH,CostID,YWSM,MEMO,BLSB,HGBH,UnitC,RateC,HGLB,LLMemo,CNO1,TCLYL,CLSL,clzmlb,TypeCost');
  ...
  begin
    ...
    begin
      ...
      SQL.Add('       ,IsNull(KCLLS.CNO,'''') as HGBH,KCRK.HGLB,KCRK.DOCNO,KCPK.Declaretion,KCPKLL.PKNO,KCRK.UserDate as InWH_Date,KCRK.CFMDate as F_Date,KCLL.UserDate,KCRK.DOCDATE ');
      SQL.Add('	  from KCPKLL');
      ...
      SQL.Add('	  left join (Select * From KCRK union all Select * From KCRK_2014) KCRK on KCRK.RKNO=KCPKLL.PKNO');
...
procedure TDeliverCheck.Query2AfterOpen(DataSet: TDataSet);
var
  VTX_CKBH: TStringList;
  VC2_CKBH: TStringList;
  VLX_CKBH: TStringList;
  i: Integer;
  TypeCost_Col: TColumnEh;
  DepID_Col: TColumnEh;
begin
  // Check user drag Typecost columns
  if DBGridEh2.Columns[27].Title.Caption = 'Dep ID' then
    DepID_Col := DBGridEh2.Columns[27];
  if DBGridEh2.Columns[28].FieldName = 'TypeCost' then
    TypeCost_Col := DBGridEh2.Columns[28];

  if (DepID_Col = nil) or (TypeCost_Col = nil) then
  begin
    for i := 0 to DBGridEh2.Columns.Count - 1 do
    begin
      if (DBGridEh2.Columns[i].Title.Caption = 'Dep ID') and (DepID_Col = nil) then
        DepID_Col := DBGridEh2.Columns[i]
      else if (DBGridEh2.Columns[i].FieldName = 'TypeCost') and (TypeCost_Col = nil) then
        TypeCost_Col := DBGridEh2.Columns[i];

      if (DepID_Col <> nil) and (TypeCost_Col <> nil) then
        Break;
    end;
  end;
  ...
  begin
    VTX_CKBH := TStringList.Create;
    VC2_CKBH := TStringList.Create;
    VLX_CKBH := TStringList.Create;
    VTX_CKBH.CommaText := 'VTXX, VTXY, VTXZ';
    VC2_CKBH.CommaText := 'VC2X, VC2Y, VC2Z';
    VLX_CKBH.CommaText := 'VLXX, VLXY, VLXZ';

    with Query2 do
    begin
      if(Pos('153', FieldByName('CWBH').AsString) = 1) and (
        (VTX_CKBH.IndexOf(FieldByName('CKBH').AsString) <> -1) or
        (VC2_CKBH.IndexOf(FieldByName('CKBH').AsString) <> -1) or
        (VLX_CKBH.IndexOf(FieldByName('CKBH').AsString) <> -1)) then
      begin
        DepID_Col.Visible := True;
        TypeCost_Col.Visible := True;
      end
      else
        begin
          DepID_Col.Visible := False;
          TypeCost_Col.Visible := False;
        end;
    end;
    VTX_CKBH.Free;
    VC2_CKBH.Free;
