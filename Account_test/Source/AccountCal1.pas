...
begin
  ...
  begin
    try
      ...
      //
      ToKhaiBtn_CKBH:=MyIni.ReadString('ERP','Account_N16_ToKhai_CKBH','''CDC'',''CDT'',''BYC'',''BYT'',''CBY'',''SDC'',''SDT'',''SRC'',''VA12'',''VTXW'',''VC1'',''VC2''');
    finally
      MyIni.Free;
    end;
  end;
...
begin
    ...
    if (iYear<=2024) then
    begin
      showmessage('Do not allow calculate 2024');
      abort;
    end;
    if (iYear>2999) or (iYear<1900) then
    begin
    ...
    begin
      ...
      SQL.Add(' and convert(smalldatetime,convert(varchar,KCLL.CFMDate,111)) between  ');
      SQL.Add('  '''+formatdatetime('yyyy/MM/dd',sDate)+''' and '''+formatdatetime('yyyy/MM/dd',eDate)+''' ');
      SQL.Add(' and  KCPKLL.YN=''1'' ');     //20240521 don't get KCLLS.LLNO choose declaretion by hand
      //funcObj.WriteErrorLog(sql.Text);
      ExecSQL();
    end;
    //TC倉入庫剩餘數量
    ...
    begin
      ...
      SQL.Add('	Select KCPKRK.RKNO,KCRK.CKBH,KCRK.HGLB,KCPKRK.CLBH,KCPKRK.Qty as RKQty,KCRK.UserDate as RKDate ');
      SQL.Add('	from KCPKRK');
      SQL.Add('	Left join (Select * From KCRK union all Select * From KCRK_2014) KCRK on KCRK.RKNO=KCPKRK.RKNO');
      SQL.Add('	where left(KCRK.HGLB,2)= '''+HGLB+''' and KCPKRK.KCYEAR='''+ayear+''' and KCPKRK.KCMONTH='''+amonth+'''');
      SQL.Add('       and KCRK.CKBH in ('+ToKhaiBtn_CKBH+') ');
      SQL.Add('	union all');
      SQL.Add('	Select KCRK.RKNO,KCRK.CKBH,KCRK.HGLB,KCRKS.CLBH,Sum(KCRKS.Qty) as RKQty,Max(KCRK.UserDate) as RKDate ');
      SQL.Add(' from (Select * From KCRK union all Select * From KCRK_2014) KCRK');
      SQL.Add('	Left join (Select * From KCRKS union all Select * From KCRKS_2014) KCRKS on KCRK.RKNO=KCRKS.RKNO');
      ...
      SQL.Add('where RemQty>0');
      //SQL.Add('Select *,(select SUM(RKQty) from #tmpInvoiceRK B where B.RKRN<=A.RKRN and B.CLBH=A.CLBH and B.CKBH=A.CKBH and B.HGLB=A.HGLB ) as RKSumQty   into #tmpInvoiceRKAll ');
      SQL.Add('Select *,(select SUM(RemQty) from #tmpInvoiceRK B where B.RKRN<=A.RKRN and B.CLBH=A.CLBH and B.CKBH=A.CKBH and B.HGLB=A.HGLB ) as RKSumQty   into #tmpInvoiceRKAll ');
      SQL.Add('from #tmpInvoiceRK A');
      //funcObj.WriteErrorLog(sql.Text);
      ExecSQL();
    end;
    ...
    begin
      ...
      SQL.Add('	     and convert(smalldatetime,convert(varchar,KCLL.CFMDate,111)) between ');
      SQL.Add('  '''+formatdatetime('yyyy/MM/dd',sDate)+''' and '''+formatdatetime('yyyy/MM/dd',eDate)+''' ');
      SQL.Add('  and KCLLS.PKNO is NULL '); //20240521 don't get KCLLS.LLNO choose declaretion by hand
      SQL.Add('Group by KCLL.LLNO,KCLLS.SCBH,KCLLS.DFL,KCLLS.CLBH,KCLL.CKBH,KCLLS.HGLB');
      SQL.Add(') KCLL');
      SQL.Add('Order by CKBH,CLBH,LLDate');
      //funcObj.WriteErrorLog(sql.Text);
    ...
    begin
      ...
      SQL.Add('Left join (Select * From KCRK union all Select * From KCRK_2014) KCRK on KCPKRK.RKNO=kcrk.RKNO ');
      SQL.Add('where KCPKRK.KCYEAR='''+CBX1.Text+''' and KCPKRK.KCMONTH='''+CBX2.Text+'''  and left(KCRK.HGLB,2)= '''+HGLB+''' and KCRK.CKBH in ('+ToKhaiBtn_CKBH+') ');
      ...
      SQL.Add('	from KCPKRK');
      SQL.Add('	Left join (Select * From KCRK union all Select * From KCRK_2014) KCRK on KCRK.RKNO=KCPKRK.RKNO');
      SQL.Add('	where left(KCRK.HGLB,2)= '''+HGLB+'''  and KCPKRK.KCYEAR='''+ayear+''' and KCPKRK.KCMONTH='''+amonth+''' and KCRK.CKBH in ('+ToKhaiBtn_CKBH+')');
      SQL.Add('	union all');
      SQL.Add('	Select KCRK.RKNO,KCRK.CKBH,KCRK.HGLB,KCRKS.CLBH,Sum(KCRKS.Qty) as RKQty,Max(KCRK.UserDate) as RKDate  ');
      SQL.Add(' from (Select * From KCRK union all Select * From KCRK_2014) KCRK');
      SQL.Add('	Left join (Select * From KCRKS union all Select * From KCRKS_2014) KCRKS on KCRK.RKNO=KCRKS.RKNO');
