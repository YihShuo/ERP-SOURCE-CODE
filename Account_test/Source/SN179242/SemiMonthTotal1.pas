...
implementation
uses Fununit, main1, Adjust_CP1, Extra_CP1;
...
begin
  ...
  begin
    ...
    SQL.Add('  	   ,(Total.M_Rate_TP / 100) AS M_Rate_TP,Total.N_SP_TuongDuong,Total.O_VNACC_Mas,Total.P_ZVNACC_Mas,Total.Q_VNACC_HC,Total.R_ZVNACC_HC ');
    SQL.Add('  	   ,ISNULL(Total.S_CPPB,0)  + Isnull(Adjust.S_CPPB,0) + Isnull(Adjust_PB.S_CPPB,0) as S_CPPB,ISNULL(Total.T_TCPPS,0) + Isnull(Adjust.S_CPPB,0) + Isnull(Adjust_PB.S_CPPB,0) as T_TCPPS  ');
    SQL.Add('  	   ,Total.U_CP622 + Isnull(Adjust.U_CP622,0) as U_CP622,Total.V_CP627 + Isnull(Adjust.V_CP627,0) as V_CP627  ');
    SQL.Add('  	   ,ISNULL(Total.W_TOTAL,0) + Isnull(Adjust.S_CPPB,0) + Isnull(Adjust_PB.S_CPPB,0) + Isnull(Adjust.U_CP622,0) + Isnull(Adjust.V_CP627,0) as W_TOTAL,X_NotInQty,Y_SemiVNACC_Mas,Z_SemiVNACCC_HC,AA_SemiVNACC  ');
    SQL.Add('  	   ,AB_FShoeVNACC_Mas,AC_FShoeVNACC_HC,ISNULL(AD_CPPB,0) + Isnull(Adjust.S_CPPB,0) + Isnull(Adjust_PB.S_CPPB,0) as AD_CPPB, AE_TCPNVL + Isnull(Adjust.S_CPPB,0) + Isnull(Adjust_PB.S_CPPB,0) as AE_TCPNVL ');
    SQL.Add('  	   ,ISNULL(AF_CPNCTT,0) + Isnull(Adjust.U_CP622,0) as AF_CPNCTT,ISNULL(AG_CPSXC,0) + Isnull(Adjust.V_CP627,0) as AG_CPSXC  ');
    SQL.Add('  	   ,AH_TGTSP + Isnull(Adjust.S_CPPB,0) + Isnull(Adjust_PB.S_CPPB,0) + Isnull(Adjust.U_CP622,0) + Isnull(Adjust.V_CP627,0) as AH_TGTSP  ');
    SQL.Add('  	   ,CASE WHEN ISNULL(I_InQty,0)>0 THEN (ISNULL(AH_TGTSP,0) + Isnull(Adjust.S_CPPB,0) + Isnull(Adjust_PB.S_CPPB,0) + Isnull(Adjust.U_CP622,0) + Isnull(Adjust.V_CP627,0))/I_InQty ELSE 0 END as AI_GIATHANH_1DOI    ');
    SQL.Add('  	   ,CASE WHEN ISNULL(I_InQty,0)>0 THEN (ISNULL(AH_TGTSP,0) + Isnull(Adjust.S_CPPB,0) + Isnull(Adjust_PB.S_CPPB,0) + Isnull(Adjust.U_CP622,0) + Isnull(Adjust.V_CP627,0))/I_InQty/CWHL ELSE 0 END as AJ_GT1DOI_USD   ');
    SQL.Add('  	   ,AK_GIABAN,AL_DOANHTHU as AL_DOANHTHU ');
    SQL.Add('      ,CASE WHEN ISNULL(I_InQty,0)>0 THEN ((ISNULL(AH_TGTSP,0) + Isnull(Adjust.S_CPPB,0) + Isnull(Adjust_PB.S_CPPB,0) + Isnull(Adjust.U_CP622,0) + Isnull(Adjust.V_CP627,0))/I_InQty/CWHL/AK_GIABAN) ELSE 0 END as AM_GVGB ');
    SQL.Add('	     ,AN_TLPB ');
    ...
    if cb1.Text='A' then
    begin
       SQL.Add(' and Total.Factory like ''A%''');
    end;
    if cb1.Text='C' then
    begin
       SQL.Add(' and Total.Factory like ''C%''');
    end;
...
begin
  ...
  begin
    active:=false;
    sql.Clear;
    sql.Add('SELECT top 1 YN FROM CW_MatCostMonth_Total');
    sql.Add('WHERE KCYEAR='''+CBX3.Text+''' AND KCMONTH='''+CBX4.Text+''' ');
    if cb1.Text='A' then
    begin
      sql.Add(' AND Factory like ''A%'' ');
    end;
    if cb1.Text='C' then
    begin
      sql.Add(' AND Factory like ''C%'' ');
    end;
    active:=true;
    if RecordCount>0 then
    begin
      if CheckQry.FieldByName('YN').AsString='5' then
      begin
  ...
  begin
    ...
    SQL.Add('  SELECT YEARS,MONTHS,TK621,TK622,TK627,PB6212,CKTM into #FactoryCost    ');
    SQL.Add('    FROM(    ');
    SQL.Add('         SELECT YEARS,MONTHS      ');
    SQL.Add('      	        ,SUM(CASE WHEN (TK_Ketoan like ''6212%'') THEN Sotien_ben_co_Noi_te ');
    SQL.Add('      	        WHEN (TK_Ketoan = ''62111'' AND Maso_TK_doi_ung = ''33332'') THEN Sotien_ben_no_Noi_te ');
    SQL.Add('      	        WHEN (TK_Ketoan = ''62113'' AND Maso_TK_doi_ung = ''33111'') THEN Sotien_ben_no_Noi_te-Sotien_ben_co_Noi_te END) ''TK621'' ');
    SQL.Add('      	        ,SUM(CASE WHEN (TK_Ketoan like ''6212%'') OR (TK_Ketoan = ''62111'' AND Maso_TK_doi_ung = ''33332'') THEN Sotien_ben_co_Noi_te END) ''PB6212'' ');
    //SQL.Add('      	        ,SUM(CASE WHEN ((TK_Ketoan = ''62113'' OR TK_Ketoan = ''62111'') AND Maso_TK_doi_ung = ''33111'') THEN Sotien_ben_no_Noi_te END) ''CKTM''');
    SQL.Add('               ,SUM(CASE WHEN ((TK_Ketoan = ''62113'' OR TK_Ketoan = ''62111'') AND Maso_TK_doi_ung = ''33111'') THEN Sotien_ben_no_Noi_te END)');
    SQL.Add('               - SUM(CASE WHEN ((TK_Ketoan = ''62113'' OR TK_Ketoan = ''62111'') AND Maso_TK_doi_ung = ''33111'') THEN Sotien_ben_co_Noi_te END) ''CKTM'' ');
    ...
    SQL.Add('         FROM(      ');
    SQL.Add('              SELECT YEAR(Ngay_CT) YEARS,MONTH(Ngay_CT) MONTHS,LEFT(TK_Ketoan,5) TK_Ketoan, Maso_TK_doi_ung      ');
    ...
    begin
      ...
      if (CBX3.Text = '2025') and (CBX4.Text = '01') then
        SQL.Add('   AND Maso_TK_doi_ung IN (''1541'',''1543'') ')
      else
        SQL.Add('   AND Maso_TK_doi_ung IN (''1541'',''33332'',''33111'') ');
    end;
    if cb1.Text='C' then
    begin
      SQL.Add('               AND LEFT(Ma_bo_phan,1) =''C''    ');
      SQL.Add('      	        AND Maso_TK_doi_ung=''1543''      ');
    end;
    SQL.Add('              GROUP BY YEAR(Ngay_CT),MONTH(Ngay_CT),LEFT(TK_Ketoan,5),Maso_TK_doi_ung      ');
    SQL.Add('              UNION ALL      ');
    SQL.Add('              SELECT YEAR(Ngay_CT) YEARS,MONTH(Ngay_CT) MONTHS,LEFT(TK_Ketoan,4) TK_Ketoan,Maso_TK_doi_ung      ');
    ...
    SQL.Add('              GROUP BY YEAR(Ngay_CT),MONTH(Ngay_CT),LEFT(TK_Ketoan,4),Maso_TK_doi_ung      ');
    SQL.Add('      	    )CP      ');
    ...
    SQL.Add('  		    ,case when max(InQty)>0 then 100 else 0 end M_Rate_TP --M ty le thuc hien TP  ');
    SQL.Add('  		    ,max(InQty) as N_SP_TuongDuong --N SP hoan thanh tuong duong  ');
    ...
    SQL.Add('  		    ,case when max(InQty)>0  then ISNULL(CW_MatCostMonth_Price.USPrice,RY_VNPRICE.VNPRICE) else null end as AK_GIABAN -- AK Gia ban 1 doi  ');
    SQL.Add('  		    ,case when max(InQty)>0 then ISNULL(CW_MatCostMonth_Price.USPrice,RY_VNPRICE.VNPRICE) *max(InQty) else null end as AL_DOANHTHU --AL Doanh thu du kien  ');
    SQL.Add('  into #MonthlyMatCost      ');
    SQL.Add('  From (  ');
    SQL.Add('   select * from CW_MatCostMonth');
    SQL.Add('   union all select KCYEAR,KCMONTH,GSBH CKBH, ISNULL(MAX(NameExtra),'''') AS DDBH, SUM(Qty) Pairs, SUM(Qty) FKCQy, NULL NoFKCQty1,');
    SQL.Add('   SUM(Qty) InQty, NULL VNACCMon, NULL VNACC, NULL ZVNACC, NULL NotInQty, NULL SemiVNACC, NULL FShoeVNACC, NULL FShoe1Pair_VNACC,');
    SQL.Add('   UserID, UserDate, YN from CW_MatCostMonth_GC GROUP BY KCYEAR,KCMONTH,GSBH,Status,UserID,UserDate,YN');
    SQL.Add('   ) AS CW_MatCostMonth');
    SQL.Add('  Left join (	Select CW_MatCostMonth.DDBH,RY_VNPRICE.VNPRICE  ');
    SQL.Add('  			        From CW_MatCostMonth left join RY_VNPRICE   ');
    SQL.Add('  			        on CW_MatCostMonth.DDBH=RY_VNPRICE.RYNO  ');
    SQL.Add('               Left join (select DDBH, GSBH from DDZL union all select DDBH, GSBH from DDZL_OEM) DDZL on CW_MatCostMonth.DDBH=DDZL.DDBH ');
    SQL.Add('  			        Where KCYEAR='''+CBX3.Text+'''  and KCMONTH='''+CBX4.Text+'''   ');
    if cb1.Text='A' then
    begin
       SQL.Add('  			      and DDZL.GSBH=''VA12'' and InQty>0  ');
    end;
    if cb1.Text='C' then
    begin
       SQL.Add('  			      and DDZL.GSBH=''VC2'' and InQty>0  ');
    end;
    SQL.Add('               Group by  CW_MatCostMonth.DDBH,RY_VNPRICE.VNPRICE ');
    if cb1.Text='A' then
    begin
      SQL.Add('  			        Union all  ');
      SQL.Add('  			        Select CW_MatCostMonth.DDBH,max(RY_VNPRICE.VNPRICE) as VNPRICE  ');
      ...
      SQL.Add('               Group by  CW_MatCostMonth.DDBH ');
    end;
    SQL.Add('  			        Union all  ');
    ...
    SQL.Add('               Left join YPZL on CW_MatCostMonth.DDBH=YPZL.YPDH ');
    ...
    if cb1.Text='A' then
    begin
      SQL.Add('  			        and YPZL.GSDH=''CDC''  and SUBSTRING(DDBH,4,1)=''V''  and InQty>0   ');
    end;
    if cb1.Text='C' then
    begin
      SQL.Add('  			        and YPZL.GSDH=''VC1''  and SUBSTRING(DDBH,4,1)=''V''  and InQty>0   ');
    end;
    ...
    if cb1.Text='A' then
    begin
       SQL.Add('  and  CW_MatCostMonth.CKBH in (''VA12'',''CDC'',''CBY'',''VTXW'') and left(CW_MatCostMonth.DDBH,2) not in (''CU'',''UQ'',''SJ'',''SV'',''SY'',''DH'') and left(CW_MatCostMonth.DDBH,3) not in (''VSV'',''VSI'') ');
    end;
    if cb1.Text='C' then
    begin
       SQL.Add('  and  CW_MatCostMonth.CKBH in (''VC2'',''VC1'',''VTXW'') and (left(CW_MatCostMonth.DDBH,2) in (''CU'',''UQ'',''SJ'',''SV'',''GC'',''SY'',''DH'') OR left(CW_MatCostMonth.DDBH,3) in (''VSV'',''VSI''))');
    end;
    SQL.Add('  Group by CW_MatCostMonth.KCyear,CW_MatCostMonth.KCmonth,CW_MatCostMonth.DDBH,RY_VNPRICE.VNPRICE,CW_MatCostMonth_Price.USPrice ');
    ...
    SQL.Add('  			         , Case when Isnull(Cost.I_InQty,0) > 0 AND Cost.G_DDBH not in (select distinct NameExtra from CW_MatCostMonth_GC UNION ALL select distinct DDBH from DDZL_OEM) ');
    if cb1.Text='A' then
    begin
      SQL.Add('  			         THEN ROUND(convert(float,FCost.PB6212)*ISNULL(Cost.AL_DOANHTHU,0)/MC.DOANHTHU_ALL,0) + ROUND(convert(float,FCost.CKTM)*ISNULL(Cost.AL_DOANHTHU,0)/MC.DOANHTHU_ALL,0) ELSE 0 END S_CPPB');
    end;
    if cb1.Text='C' then
    begin
      SQL.Add('  			         THEN ROUND(convert(float,FCost.TK621)*ISNULL(Cost.AL_DOANHTHU,0)/MC.DOANHTHU_ALL,0) ELSE 0 END S_CPPB');
    end;
    ...
    begin
      SQL.Add('               ,CASE ');
      SQL.Add('                   WHEN (LEFT(Cost.G_DDBH,2) IN (''SY'')) OR (LEFT(Cost.G_DDBH,3) = ''VSI'') THEN ''VESSI'' ');
      SQL.Add('                   WHEN LEFT(Cost.G_DDBH,2) = ''GC'' THEN '''' ');     
      SQL.Add('                   WHEN LEFT(Cost.G_DDBH,2) = ''DH'' THEN ''HOKA'' ');
      SQL.Add('                   WHEN (LEFT(Cost.G_DDBH,2) IN (''CU'',''SJ'',''SV'')) ');
      SQL.Add('                        OR (LEFT(Cost.G_DDBH,3) = ''VSV'') THEN ''CARIUMA'' ');
      SQL.Add('                   ELSE ''UNIQLO'' ');
      SQL.Add('               END AS AP_KHACHHANG ');
    end;
     ...
     begin
        ...
        try
          ...
          begin
            ...
            eclApp.Cells(j,8):=Query2.FieldByName('M_Rate_TP').Value;
            ...
            eclApp.Cells(j,34):=Query2.FieldByName('AM_GVGB').Value;
            eclApp.Cells(j,35):=Query2.FieldByName('AN_TLPB').Value;
...
begin
  ...
  begin
    Active:=false;
    SQL.Clear;
    SQL.Add(' Select top 1 YN From CW_MatCostMonth_Total Where KCYEAR='''+CBX3.Text+''' and KCMONTH='''+CBX4.Text+''' ');
    if cb1.Text='A' then
    begin
      SQL.Add(' and Factory like ''A%'' ');
    end;
...
procedure TSemiMonthTotal.Extra1Click(Sender: TObject);
begin       
  with CheckQry do
  begin
    Active:=false;
  ...
  Extra_CP:=TExtra_CP.create(self);
  Extra_CP.Button1.Click;
  Extra_CP.show;
end;

procedure TSemiMonthTotal.Button3Click(Sender: TObject);
begin
  with TempQry do
  begin
    Active := false;
    SQL.Clear;
    SQL.Add('UPDATE CW_MatCostMonth_Total ');
    SQL.Add('SET YN=''5'' ');
    SQL.Add('WHERE KCYEAR = ''' + CBX3.Text + ''' AND KCMONTH = ''' + CBX4.Text + ''' ');
    if cb1.Text='A' then
    begin
...
procedure TSemiMonthTotal.Button4Click(Sender: TObject);
begin
  with TempQry do
  begin
    Active := false;
