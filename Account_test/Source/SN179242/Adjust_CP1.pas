...
uses  main1,fununit, SemiMonthTotal1, Math;
...
begin
  ...
  begin
    ...
    fieldbyname('Factory').Value:=SemiMonthTotal.Query2.fieldbyname('Factory').Value;
    if (SemiMonthTotal.Query2.fieldbyname('AP_KHACHHANG').Value<>'') then
      fieldbyname('DDBH').Value:=SemiMonthTotal.Query2.fieldbyname('G_DDBH').Value
    else fieldbyname('DDBH').Value:='';
    FieldByName('S_CPPB').Value := RoundTo(
    (SemiMonthTotal.Query2.FieldByName('AN_TLPB').AsFloat *
     SemiMonthTotal.Query2.FieldByName('CP621_GP').AsCurrency) -
     SemiMonthTotal.Query2.FieldByName('S_CPPB').AsCurrency
    ,0);
  end;
  bb4.enabled:=true;
  bb5.enabled:=true;
...
begin
  ...
  begin
    ...
    if SemiMonthTotal.cb1.Text='A' then
    begin
      sql.Add('AND Factory like ''A%'' ');
    end;
    if SemiMonthTotal.cb1.Text='C' then
    begin
