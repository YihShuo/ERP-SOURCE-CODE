...
begin
  ...
  try
    ...
    begin
      ...
      while not eof do
      begin
        //20240905 xoa bang KCPKLL khi ke toan reset
        ...
        begin
          ...
          begin
            ...
            sql.Add('Where LLNO='''+DelDet.fieldbyname('LLNO').AsString+''' and YN=2');
            sql.Add('Update KCLLS Set PKNO=NULL Where LLNO='''+DelDet.fieldbyname('LLNO').AsString+''' ');
...
begin
  ...
  begin
    ...
    if DBGrid2.Columns[15].FieldName = 'TypeCost' then
      TypeCost_Col := DBGrid2.Columns[15]
    else
    begin
      for i := 0 to DBGrid2.Columns.Count - 1 do
      begin
        if DBGrid2.Columns[i].FieldName = 'TypeCost' then
        begin
          TypeCost_Col := DBGrid2.Columns[i];
          Break;
        end;
      end;
    end;
...
procedure TDeliverCon.Button7Click(Sender: TObject);
begin
  DelDet.Active:=false;
  DelMas.Active:=false;
  DelMas.Active:=true;
...
procedure TDeliverCon.Button6Click(Sender: TObject);
begin
    with DelDet do
    begin
      first;
...
procedure TDeliverCon.Button5Click(Sender: TObject);
begin
  with DelMas do
  begin
    Active:=false;
    ...
    sql.add('left join BDepartment on KCLL.DepID=BDepartment.ID  ');
    sql.add('left join KCCK on KCCK.CKBH=KCLL.CKBH ');
    sql.add('where ((KCLL.SCBH=''XXXXXXXXXX'') ');
    sql.add('or (KCLL.SCBH<>''XXXXXXXXXX'' AND ISNULL(KCLL.flowflag,'''')<>''X'')) and KCLL.CFMID<>'+''''+'NO'+''' ');
    sql.add('and KCLL.YN<>'+''''+'5'+'''');
    sql.add('and convert(smalldatetime,convert(varchar,KCLL.CFMDATE,111)) between'+''''+formatdatetime('yyyy/MM/dd',DTP1.date )+'''');
    sql.add('and '+''''+formatdatetime('yyyy/MM/dd',DTP2.date )+'''');
    sql.add('and KCLL.LLNO like '+''''+edit1.Text+'%'+'''');
