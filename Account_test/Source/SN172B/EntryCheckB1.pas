...
procedure TEntryCheckB.readini();
var MyIni :Tinifile;
    AppDir:string;
begin
  //
...
//
procedure TEntryCheckB.Button1Click(Sender: TObject);
...
begin
  ...
  begin
    ...
    sql.Add('       KCPK.Declaretion,KCPK.Export,KCRKS.RKMemo,KCRK.ZSBH,CWFK.FKBH,KCRK.DOCDATE,KCRKS.CNO1,ZSZL.tybh');
    sql.add('from KCRKS_1722 KCRKS');
    sql.add('left join KCRK_1722 KCRK on KCRK.RKNO=KCRKS.RKNO');
    ...
    sql.add('left join KCPK_1722 KCPK on KCPK.PKNO=KCRK.RKNO ');
  ...
  begin
    active:=false;
    sql.Clear;
    sql.add('select distinct DOCNO from KCRK_1722 KCRK ');
...
procedure TEntryCheckB.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TEntryCheckB.ExportDefaultExcel();
var
...
procedure TEntryCheckB.ExportFormatExcel();
...
procedure TEntryCheckB.Button2Click(Sender: TObject);
begin
...
procedure TEntryCheckB.FormDestroy(Sender: TObject);
begin
  EntryCheckB:=nil;
end;

procedure TEntryCheckB.FormCreate(Sender: TObject);
var
...
procedure TEntryCheckB.CBX1Change(Sender: TObject);
begin
  CBX4.Text:=CBX1.Text;
end;

procedure TEntryCheckB.CBX2Change(Sender: TObject);
begin
  CBX5.Text:=CBX2.Text;
end;


procedure TEntryCheckB.GSBHCBXChange(Sender: TObject);
begin
  if GSBHCBX.Text<>'ALL' then
  begin
    with MatSite do
