...
with ZLList do
  begin
    ...
    SQL.Add('left join (select YWDD.YSBH as DDBH,sum(ISNULL(YWCP_OUT.Qty_Out,YWCP.Qty)) as CHQty,max(ISNULL(YWCP_OUT.ExeDate,YWCP.ExeDate)) as ETD from YWCP');
    SQL.Add('           left join YWCP_OUT on YWCP_OUT.CARTONBAR=YWCP.CARTONBAR');
    SQL.Add('           left join YWDD on YWDD.DDBH=YWCP.DDBH');
    SQL.Add('           where ISNULL(YWCP_OUT.ExeDate,YWCP.ExeDate) is not null');
