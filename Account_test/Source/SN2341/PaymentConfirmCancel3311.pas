...
type
  TPaymentConfirmCancel331 = class(TForm)
    Panel1: TPanel;
    Label1: TLabel;
    Label2: TLabel;
    Button1: TButton;
    ...
    procedure qry2AfterScroll(DataSet: TDataSet);
  private
...
var
  PaymentConfirmCancel331: TPaymentConfirmCancel331;
...
{$R *.dfm}
procedure TPaymentConfirmCancel331.ReadIni();
var MyIni :Tinifile;
...
begin
  ...
  begin
    try
      ...
      UserIDList:=MyIni.ReadString('ERP','Account_N2341_CheckUserID','');
    finally
      MyIni.Free;
    end;
  end;
...
procedure TPaymentConfirmCancel331.InitTitleClick(IsOpen:boolean);
var i:integer;
begin
  if IsOpen=false then
  begin
    for i:=0 to DBGridEh8.Columns.Count-1 do
      DBGridEh8.Columns[i].Title.TitleButton:=false;
  end else
  begin
    if ((qry_CGPayment.Active=true) and (DBGridEh8.Columns[0].Title.TitleButton=false))then
      for i:=0 to DBGridEh8.Columns.Count-3 do
        DBGridEh8.Columns[i].Title.TitleButton:=true;
  end;
end;


procedure TPaymentConfirmCancel331.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TPaymentConfirmCancel331.FormDestroy(Sender: TObject);
begin
  PaymentConfirmCancel331:=nil;
  InitTitleClick(true);
end;

procedure TPaymentConfirmCancel331.FormCreate(Sender: TObject);
begin
  DTP1.Date:=date-3;
  DTP2.Date:=date;
  with qryTemp do
  ...
  DBGridEh8.Columns[0].Title.TitleButton := True;
  ReadIni();
  panel2.Visible:=true;
...
procedure TPaymentConfirmCancel331.EnMasAfterOpen(DataSet: TDataSet);
begin
...
procedure TPaymentConfirmCancel331.Button2Click(Sender: TObject);
begin      
  if qry_CGPayment.FieldByName('CFMID').AsString<>'NO' then
  begin
    showmessage('Already Confirm 331, can''t reset');
    abort;
  end;
  if ((qry_CGPayment.FieldByName('HD_LOAI').AsString='3') or (qry_CGPayment.FieldByName('HD_LOAI').AsString='4') or (qry_CGPayment.FieldByName('HD_LOAI').AsString='6')) then
  begin
    if messagedlg('CAN THANH TOAN LAI HAY KHONG? ',mtinformation,[mbYes,mbNo],0)=mrYes then
    begin
      With qryTemp do
      begin
        Close;
        SQL.Clear;
        SQL.Add('  update CGZL_Payment SET Is_Notice = ''1'' WHERE FKBH='''+qry_CGPayment.FieldByName('FKBH').Value+'''  ');
        ExecSQL;
      end;
    end else
    begin
      ...
      begin
        Close;
        SQL.Clear;
        SQL.Add('  update CGZL_Payment SET Is_Notice = ''0'' WHERE FKBH='''+qry_CGPayment.FieldByName('FKBH').Value+'''  ');
        ExecSQL;
      end;
    end;
  end;
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('  update CGZL_Payment SET CFMID = '''+main.Edit1.Text+''' , CFMDATE=GETDATE(), YN=''5'' WHERE FKBH='''+qry_CGPayment.FieldByName('FKBH').Value+'''  ');
    ExecSQL;
  end;
  showmessage('Success.');
  Button1Click(nil);
...
procedure TPaymentConfirmCancel331.Button4Click(Sender: TObject);
begin
  if qry_CGPayment.FieldByName('CFMID1').AsString<>'NO' then
  begin
    showmessage('Already Confirm 112, can''t reset');
    abort;
  end;
  if qry_CGPayment.FieldByName('YN').AsString='1' then
  begin
  ...
  begin 
    Close; 
    SQL.Clear;
    SQL.Add('  update CGZL_Payment SET CFMID = ''NO'' , YN=''1'', Is_Notice=''0'' WHERE YN=''5'' and FKBH='''+qry_CGPayment.FieldByName('FKBH').Value+'''  ');
    ExecSQL;
  end;
  showmessage('Success.');
  Button1Click(nil);
...
procedure TPaymentConfirmCancel331.CallQuery();
begin
  ...
  begin
    ...
    begin
      ...
      SQL.Add('           ,case when (ISNULL(DF1.VATFile_SV, '''') <> '''') OR (ISNULL(DF2.VATFile_SV, '''') <> '''') then 1 else 0 end as PLX  ');
      ...
      SQL.Add('   LEFT JOIN [EEP].[LingYi].[dbo].[DOCNO_File_SV] DF1 ON ISNULL(DOCNO_File.VATFile, '''') = DF1.VATFile_SV  ');
      SQL.Add('   LEFT JOIN [EEP].[LingYi].[dbo].[DOCNO_File_SV] DF2 ON ISNULL(DOCNO_File.VATFile1, '''') = DF2.VATFile_SV ');
  ...
  begin
    ...
    begin
      ...
      SQL.Add('         ,CASE WHEN isnull(TRU_CHIETKHAU,0)=0 THEN round(b.TONGTIEN_CHUATHUE,2) + isnull(round(b.TONGTIEN_PHI,2),0) ELSE ');
      SQL.Add('         (round(b.TONGTIEN_CHUATHUE,2) -isnull(round(b.TONGTIEN_CHIETKHAU,2),0)) + isnull(round(b.TONGTIEN_PHI,2),0) end TONGTIEN_CHUATHUE  ');
      SQL.Add('  		    ,round(b.TONGTIEN_THUE,2) as TONGTIEN_THUE, b.TONGTIEN_THANHTOAN ,a.USERID, a.USERDATE, a.YN   ');
      SQL.Add('         ,case when (ISNULL(DF1.VATFile_SV, '''') <> '''') OR (ISNULL(DF2.VATFile_SV, '''') <> '''') then 1 else 0 end as PLX, b.DONVI_TIENTE  ');
      ...
      SQL.Add('  		    ,-1*a.TONGTIEN_THUE, -1*a.TONGTIEN_THANHTOAN ,a.USERID, a.USERDATE, a.YN, 0 as PLX, ''VND'' DONVI_TIENTE   ');
      SQL.Add('  FROM CGZL_PaymentSS a  ');
      SQL.Add('  WHERE FKBH=:FKBH  	and a.ZSBH_MST=''TRUTIEN''  ');
      SQL.Add('  UNION ALL ');
      SQL.Add('  SELECT  a.FKBH,c.ZSBH_MST +'' ( ''+ a.FKBH_TTT +'' )'' ,c.VATNO,c.KYHIEU_HOADON,c.VATDATE,-1*isnull(d.TONGTIEN_CHUATHUE,c.TONGTIEN_CHUATHUE), ');
      SQL.Add('          -1*isnull(d.TONGTIEN_THUE,c.TONGTIEN_THUE),-1*isnull(d.TONGTIEN_THANHTOAN,c.TONGTIEN_THANHTOAN),a.USERID,a.USERDATE,a.YN, 0 as PLX, ''VND'' DONVI_TIENTE ');
      SQL.Add('  FROM  CGZL_Payment_Map a ');
      SQL.Add('  LEFT JOIN CGZL_Payment b on a.FKBH_TTT=b.FKBH ');
      SQL.Add('  LEFT JOIN CGZL_PaymentSS c on c.FKBH=b.FKBH  ');
      SQL.Add('  LEFT JOIN CGZL_PaymentSS_Advance d on d.FKBH_S=a.FKBH  ');
  ...
  begin
    ...
    begin
      ...
      SQL.Add('  		        ,a.USERID, a.USERDATE, a.YN, 0 as PLX, ''VND'' DONVI_TIENTE   ');
      SQL.Add('    FROM CGZL_PaymentSS a  ');
      SQL.Add('    WHERE FKBH=:FKBH  ');
      SQL.Add('    ');
      Open;
  ...
  begin
    ...
    begin 
      ...
      SQL.Add('  		  a.USERID, null USERDATE, a.YN, null FKBH_TTT, null ZSBH_MST, null as TIENTE ');
      SQL.Add('FROM cgzl_payments_advance a ');
      SQL.Add('LEFT JOIN cgzl_payment_map b ON b.FKBH = a.FKBH ');
      SQL.Add('LEFT JOIN (SELECT cgzl_advances.FKBH, cgzl_payment.NGAY_TAMUNG, Sum(cgzl_advances.SOTIEN_TT) THANHTOAN ');
      SQL.Add('           FROM cgzl_advances LEFT JOIN cgzl_payment ON cgzl_payment.FKBH = cgzl_advances.FKBH ');
...
procedure TPaymentConfirmCancel331.Button1Click(Sender: TObject);
begin
    ...
    begin
      ...
      SQL.Add('       HD_NOIDUNG, HD_NOIDUNG_TW, HD_LOAI, CGZL_Payment.USERID, CGZL_Payment.USERDATE, CGZL_Payment.YN , isnull(CGZL_Payment.CFMID,''NO'') CFMID,CGZL_Payment.CFMDate,isnull(CGZL_Payment.CFMID1,''NO'') CFMID1');
      SQL.Add('       ,CGZL_Payment.CFMDate1,isnull(Is_Notice,''0'') Is_notice,DATEDIFF(DAY,convert(varchar,CGZL_Payment.USERDATE,111),convert(varchar,GETDATE(),111)) ManyDay, isnull(FKBH_S,''N'') FKBH_S  ');
      ...
      if rd1.Checked then
        SQL.Add('and CGZL_Payment.YN=''1'' ');
      if rd2.Checked then
        SQL.Add('and CGZL_Payment.YN=''5'' ');
...
procedure TPaymentConfirmCancel331.qry_CGPaymentAfterOpen(DataSet: TDataSet);
begin
  CallQuery();
end;

procedure TPaymentConfirmCancel331.qry_CGPaymentBeforeClose(
  DataSet: TDataSet);
begin
  qry1.Active:=false;
  qry2.Active:=false;
...
procedure TPaymentConfirmCancel331.DBGridEh8DrawColumnCell(Sender: TObject;
...
procedure TPaymentConfirmCancel331.Button3Click(Sender: TObject);
var
...
procedure TPaymentConfirmCancel331.qry_CGPaymentAfterScroll(
  DataSet: TDataSet);
begin
  CallQuery();
end;

procedure TPaymentConfirmCancel331.cbx1Change(Sender: TObject);
begin
...
procedure TPaymentConfirmCancel331.Button5Click(Sender: TObject);
var
  eclApp,WorkBook:olevariant;
  i,j:integer;
begin
  ...
  begin
    ...
    sql.Add('       			 UNION ALL ');
    sql.Add('        		   SELECT  a.FKBH,c.ZSBH_MST +'' ( ''+ a.FKBH_TTT +'' )'' as ZSBH_MST ,c.VATNO,c.KYHIEU_HOADON,c.VATDATE,-1*isnull(d.TONGTIEN_CHUATHUE,c.TONGTIEN_CHUATHUE), ');
    sql.Add('        		           -1*isnull(d.TONGTIEN_THUE,c.TONGTIEN_THUE),-1*isnull(d.TONGTIEN_THANHTOAN,c.TONGTIEN_THANHTOAN),a.USERID,a.USERDATE,a.YN  ');
    sql.Add('      			   FROM  CGZL_Payment_Map a  ');
    sql.Add('     			   LEFT JOIN CGZL_Payment b on a.FKBH_TTT=b.FKBH ');
    sql.Add('       			 LEFT JOIN CGZL_PaymentSS c on c.FKBH=b.FKBH  ');      
    sql.Add('       			 LEFT JOIN CGZL_PaymentSS_Advance d on d.FKBH_S=a.FKBH    ');
    ...
    sql.add('          and '+''''+formatdatetime('yyyy/MM/dd',DTP2.Date)+''' ');
    if rd1.Checked then
        SQL.Add('and Main.YN=''1'' ');
    if rd2.Checked then
        SQL.Add('and Main.YN=''5'' ');
    if cbx1.Text <>'' then
...
procedure TPaymentConfirmCancel331.DBGridEh3DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumnEh;
  State: TGridDrawState);
begin
...
procedure TPaymentConfirmCancel331.qry4AfterOpen(DataSet: TDataSet);
begin
...
procedure TPaymentConfirmCancel331.qry4AfterScroll(DataSet: TDataSet);
begin
  if qry4.FieldByName('TIENTE').AsString='USD' then
  begin
    DBGridEh3.Columns[7].DisplayFormat:='##,#0.00';
...
procedure TPaymentConfirmCancel331.qry2AfterOpen(DataSet: TDataSet);
begin
  if qry2.FieldByName('DONVI_TIENTE').AsString='USD' then
  begin
    DBGridEh1.Columns[5].DisplayFormat:='##,#0.00';
    DBGridEh1.Columns[6].DisplayFormat:='##,#0.00';
    DBGridEh1.Columns[7].DisplayFormat:='##,#0.00';
    DBGridEh1.Columns[5].Footer.DisplayFormat:='##,#0.00';
    DBGridEh1.Columns[6].Footer.DisplayFormat:='##,#0.00';
    DBGridEh1.Columns[7].Footer.DisplayFormat:='##,#0.00';
  end else
  begin
    DBGridEh1.Columns[5].DisplayFormat:='#,##0';
    DBGridEh1.Columns[6].DisplayFormat:='#,##0';
    DBGridEh1.Columns[7].DisplayFormat:='#,##0';
    DBGridEh1.Columns[5].Footer.DisplayFormat:='#,##0';
    DBGridEh1.Columns[6].Footer.DisplayFormat:='#,##0';
    DBGridEh1.Columns[7].Footer.DisplayFormat:='#,##0';
  end;
end;

procedure TPaymentConfirmCancel331.qry2AfterScroll(DataSet: TDataSet);
begin
  if qry2.FieldByName('DONVI_TIENTE').AsString='USD' then
  begin
    DBGridEh1.Columns[5].DisplayFormat:='##,#0.00';
