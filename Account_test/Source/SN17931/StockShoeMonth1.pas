...
type
  TStockShoeMonth = class(TForm)
    ...
    queryGC: TQuery;
    DBGridEh5: TDBGridEh;
    queryGCDDBH: TStringField;
    queryGCCKBH: TStringField;
    queryGCPairs: TIntegerField;
    queryGCRKQty: TIntegerField;
    queryGCXHQty: TIntegerField;
    queryGCQty: TIntegerField;
    queryGCShipDate: TDateTimeField;
    queryGCArticle: TStringField;
    queryGCINDATE: TDateTimeField;
    queryGCEXEDATE: TDateTimeField;
    queryGCLastRem: TIntegerField;
    queryGCMemo: TStringField;
    btnCFM_GC: TButton;
    Button28: TButton;
    btnRS_GC: TButton;
    Label34: TLabel;
    ...
    procedure btnQueryGCClick(Sender: TObject);
    procedure btnExcel_GCClick(Sender: TObject);
    procedure queryGCAfterOpen(DataSet: TDataSet);
    procedure btnCFM_GCClick(Sender: TObject);
    procedure btnRS_GCClick(Sender: TObject);
...
uses main1,StockShoeMonth_Print1,SampleShoeStock_Detail1,CBYShoeStock_Detail1,SampleFinishShoeStock_Print1
    ,CBYFinishShoeStock_Print1, OutsoleStockMonth_Print1, StockShoeMonthGC_Print1, Math;
...
begin
  ...
  DTP4.Date:=Date();
  PC1.ActivePage:=TS1;
  //cbFactory.Items := ComboBox1.Items;
  cbFactory.ItemIndex:=0;
  DTP_form_GC.Date := Date();
end;
...
//20240306 億柏月結缺少DH2310-114B這張訂單數量，如果將拆單改為拆單後的訂單編號，月結數量就會正確，可繼續觀察
  ...
  begin
    ...
    SQL.Add('	Select Max(YWCP.KCBH) as CKBH,case when ddzltr.DDBH1 is not null then ddzltr.DDBH else '+YWCP_DDBH+' end DDBH,Sum(YWCP.Qty) as Qty,''2'' as SB');
    SQL.Add('	from '+YWCP+'');
    SQL.Add(' left join DDZL on YWCP.DDBH=DDZL.DDBH ');
    SQL.Add('	where convert(smalldatetime,convert(varchar,YWCP.INDATE,111)) between '''+Formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+Formatdatetime('yyyy/MM/dd',DTP1.date)+''' and IsNull(SB,'''')<>'''' ');
    SQL.Add(' and ywcp.DDBH not in (select DDBH from DDZLTR) ');
    SQL.Add('	Group by '+YWCP_DDBH+',ddzltr.DDBH1,ddzltr.DDBH) YWCPIn Group by CKBH,DDBH,SB');
    SQL.Add('	union all');
    SQL.Add(' Select CKBH,DDBH,Sum(Qty) as Qty,SB From (');
    SQL.Add('	Select Max(YWCP.KCBH) as CKBH,case when ddzltr.DDBH1 is not null then ddzltr.DDBH else '+YWCP_DDBH+' end DDBH,-1*Sum( Case when YWCP.SB=''3'' then ISNULL(YWCP_OUT.Qty_Out,YWCP.Qty) else 0 end) as Qty,''3'' as SB');
    SQL.Add('	from '+YWCP+'');
    sql.add(' left join YWCP_OUT on YWCP_OUT.CARTONBAR=YWCP.CARTONBAR');
    SQL.Add(' left join DDZL on YWCP.DDBH=DDZL.DDBH ');
    SQL.Add('	where convert(smalldatetime,convert(varchar,ISNULL(YWCP_OUT.ExeDate,YWCP.ExeDate),111)) between '''+Formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+Formatdatetime('yyyy/MM/dd',DTP1.date)+''' and IsNull(YWCP.SB,'''')<>''''');
    SQL.Add(' and ywcp.DDBH not in (select DDBH from DDZLTR) ');
    SQL.Add('	Group by '+YWCP_DDBH+',ddzltr.DDBH1,ddzltr.DDBH) YWCPOut Group by CKBH,DDBH,SB ');
    SQL.Add('	) YWCPMONTH');
    ...
    SQL.Add('	where convert(smalldatetime,convert(varchar,YWCP.INDATE,111)) between '''+Formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+Formatdatetime('yyyy/MM/dd',DTP1.date)+''' and IsNull(SB,'''')<>'''' ');
    SQL.Add(' and ywcp.DDBH not in (select DDBH from DDZLTR) ');
    SQL.Add('	Group by '+YWCP_DDBH+',ddzltr.DDBH1,ddzltr.DDBH ) YWCPIn Group by CKBH,DDBH,SB ) YWCPRK on YWCP.DDBH=YWCPRK.DDBH');
    SQL.Add('  Left join  (');
    SQL.Add(' Select CKBH,DDBH,Sum(Qty) as Qty,SB From (' );
    SQL.Add('	Select Max(YWCP.KCBH) as CKBH,case when ddzltr.DDBH1 is not null then ddzltr.DDBH else '+YWCP_DDBH+' end DDBH,Sum( Case when YWCP.SB=''3'' then ISNULL(YWCP_OUT.Qty_Out,YWCP.Qty) else 0 end) as Qty,''5'' as SB');
    SQL.Add('	from '+YWCP+' ');                                                                                                                                                      
    sql.add(' left join YWCP_OUT on YWCP_OUT.CARTONBAR=YWCP.CARTONBAR');
    SQL.Add(' left join DDZL on YWCP.DDBH=DDZL.DDBH ');
    SQL.Add('	where convert(smalldatetime,convert(varchar,ISNULL(YWCP_OUT.ExeDate,YWCP.ExeDate),111)) between '''+Formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+Formatdatetime('yyyy/MM/dd',DTP1.date)+''' and IsNull(YWCP.SB,'''')<>''''');
    SQL.Add(' and ywcp.DDBH not in (select DDBH from DDZLTR) ');
    SQL.Add('	Group by '+YWCP_DDBH+',ddzltr.DDBH1,ddzltr.DDBH )YWCPOut Group by CKBH,DDBH,SB ) YWCPXH on YWCP.DDBH=YWCPXH.DDBH');
    SQL.Add('   Left join (');
    //============
...
procedure DBGridToExcel(Query: TQuery; DBGridEh :TDBGridEh);
var
  ExcelApp: OleVariant;
  i, j, titleRowCount, dataStartRow: Integer;
...
begin
  ...
  //Count title line
    ...
    begin
      titleList := funcObj.SplitString(DBGridEh.Columns[i].title.Caption, '|');
      titleRowCount := Max(titleList.Count, titleRowCount);
    end;
  //Draw title
    for i := 0 to DBGridEh.Columns.Count - 1 do
    begin
      ...
      begin
        titleList := funcObj.SplitString(DBGridEh.Columns[i].title.Caption, '|');
        if titleList.Count < titleRowCount then
        begin
          ExcelApp.Range[ExcelApp.Cells[1, i + 1], ExcelApp.Cells[1 + titleRowCount - titleList.Count, i + 1]].Merge;
          ExcelApp.Range[ExcelApp.Cells[1, i + 1], ExcelApp.Cells[1 + titleRowCount - titleList.Count, i + 1]].VerticalAlignment := -4108;
          ExcelApp.Range[ExcelApp.Cells[1, i + 1], ExcelApp.Cells[1 + titleRowCount - titleList.Count, i + 1]].HorizontalAlignment := -4108;
          ExcelApp.Cells(1, i + 1) := titleList[0];

          for j := 1 to titleList.Count - 1 do
          begin
            ExcelApp.Cells(titleRowCount - titleList.Count + j + 1, i + 1) := titleList[j];
          end;
        end
        else
          for j := 0 to titleList.Count - 1 do
          begin
            ExcelApp.Cells(j + 1, i + 1) := titleList[j];
          end;

        Field := Query.FieldByName(DBGridEh.Columns[i].FieldName);
        if Field is TNumericField then
          ExcelApp.Columns[i+1].NumberFormat :=  TNumericField(Field).DisplayFormat;
      end;
    end;

    ExcelApp.Range[ExcelApp.Cells[1,1], ExcelApp.Cells[titleRowCount , DBGridEh.Columns.Count]].Interior.Color := RGB(79, 224, 185);
    ExcelApp.Range[ExcelApp.Cells[1,1], ExcelApp.Cells[titleRowCount , DBGridEh.Columns.Count]].Font.Bold := True;
    ExcelApp.Range[ExcelApp.Cells[1,1], ExcelApp.Cells[titleRowCount , DBGridEh.Columns.Count]].HorizontalAlignment :=  -4108;

    //Data
    dataStartRow := titleRowCount+1;
    with Query do
    begin
      First;
      while not Eof do
      begin
        ...
        begin
          if DBGridEh.Columns[i].Visible then
            ExcelApp.Cells(dataStartRow, i + 1) := FieldByName(DBGridEh.Columns[i].FieldName).Value;
        end;
        Inc(dataStartRow);
        Next;
      end;
    end;
    ExcelApp.Range[ExcelApp.Cells[1, 1], ExcelApp.Cells[Query.RecordCount+titleRowCount, DBGridEh.Columns.Count]].Borders.LineStyle := 1;
    ...
    on E: Exception do
    begin
      MessageDlg('Have wrong: ' + E.Message, mtError, [mbOK], 0);
...
begin
  ...
  if PC1.ActivePageIndex=8 then
  begin
    DTP:=DTP_form_GC.Date;
    GSDH:='VC2';
  end;
  //
...
procedure TStockShoeMonth.btnQueryGCClick(Sender: TObject);
var iYear,iMonth,iDay:word;
    sYear,sMonth:string;
begin
  decodedate(DTP_form_GC.Date,iYear,iMonth,iDay);
  //incAMonth(iYear,iMonth,iDay,-1);
  sYear:=floattostr(iYear);
  sMonth:=floattostr(iMonth);
  if length(sMonth)=1 then
  ...
  begin
    ...
    SQL.Clear;
    SQL.Add('  SELECT   ');
    SQL.Add('   YWCP_OEM.DDBH, YWCP_OEM.CKBH, dbo.DDZL_OEM.Qty AS Pairs, YWCP_OEM.Qty AS RKQty, YWCP_OEM.Qty AS XHQty    ');
    SQL.Add('   , Qty=0, LastRem=0, '''' as MEMO, DDZL_OEM.ShipDate, DDZL_OEM.Article, YWCP_OEM.INDATE, YWCP_OEM.EXEDATE  ');
    SQL.Add('  FROM dbo.YWCP_OEM  ');
    SQL.Add('  LEFT JOIN dbo.DDZL_OEM ON DDZL_OEM.DDBH = YWCP_OEM.DDBH  ');
    SQL.Add('  WHERE ISNULL(SB,'''') <> ''''  ');
    SQL.Add('   and YEAR(YWCP_OEM.EXEDATE) ='''+sYear+''' and MONTH(YWCP_OEM.EXEDATE) = '''+sMonth+''' ');
    if cbFactory.Text <> 'ALL' then
      SQL.Add('and DDZL_OEM.GSBH=''' + cbFactory.Text + ''' ');
    if editDDBH_GC.Text <> '' then
      SQL.Add('   and YWCP_OEM.DDBH like ''' + editDDBH_GC.Text + '%'' ');
    Open;
  end;
end;

procedure TStockShoeMonth.btnExcel_GCClick(Sender: TObject);
begin
  if  Messagedlg(Pchar('Format Excel?'),mtInformation,[mbNo,mbYes],0)=mrYes then
  begin
    ExportFormatExcel(queryGC);
  end else
  begin
    ExportDefaultExcel(queryGC);
  end;
end;

procedure TStockShoeMonth.queryGCAfterOpen(DataSet: TDataSet);
var iYear,iMonth,iDay:word;
    sYear,sMonth:string;
begin
  decodedate(DTP_form_GC.Date,iYear,iMonth,iDay);
  ...
  begin
    active:=false;
    sql.Clear;
    sql.Add(' Select top 1.* From YWCP_OEM ');
    SQL.Add('  WHERE ISNULL(SB,'''') <> '''' AND YN=5 ');
    SQL.Add('   and YEAR(YWCP_OEM.EXEDATE) ='''+sYear+''' and MONTH(YWCP_OEM.EXEDATE) = '''+sMonth+''' ');
    if editDDBH_GC.Text <> '' then
      SQL.Add('   and YWCP_OEM.DDBH like ''' + editDDBH_GC.Text + '%'' ');
    active:=true;
  ...
  if TempQry.RecordCount >0 then
  begin
     btnCFM_GC.Enabled:=false;
     btnRS_GC.Enabled:=true;
  end else
  begin
    btnCFM_GC.Enabled:=true;
    btnRS_GC.Enabled:=false;
  end;
end;

procedure TStockShoeMonth.btnCFM_GCClick(Sender: TObject);
var iYear,iMonth,iDay:word;
    sYear,sMonth:string;
begin
  decodedate(DTP_form_GC.Date,iYear,iMonth,iDay);
  ...
  try
    queryGC.First;
    while not queryGC.Eof do
    begin
      with TempQry do
      begin
        active:=false;
        sql.Clear;
        sql.Add('update YWCP_OEM set YN=5');
        sql.Add('where YEAR(YWCP_OEM.EXEDATE) ='''+sYear+''' and MONTH(YWCP_OEM.EXEDATE) ='''+sMonth+''' and DDBH ='''+queryGC.fieldbyname('DDBH').AsString+''' ');
        execsql;
      end;
      queryGC.Next;
    end;
    ...
    begin
      ...
      sql.add('insert into BDelRec ');
      sql.add('(TableName ,TNO,CLBH,OldID,DelID,DelDate)');
      sql.add('values (''YWCP_OEM'',''Confirm'','''+sYear+''' +''/''+ '''+sMonth+''' ');
      sql.add(','''+main.Edit1.Text+''' ,'''+main.Edit1.Text+''' ,getdate())');
      //funcobj.WriteErrorLog(sql.Text);
      execsql;
    end;
    showmessage('Already lock '+FormatDateTime('YYYY/MM',DTP_form_GC.Date)+' ');
  except
    on E:Exception do
    showmessage('Have wrong:'+E.Message);
  end;
  queryGC.Active:=false;
  queryGC.Active:=true;
end;

procedure TStockShoeMonth.btnRS_GCClick(Sender: TObject);
var iYear,iMonth,iDay:word;
    sYear,sMonth:string;
begin
  decodedate(DTP_form_GC.Date,iYear,iMonth,iDay);
  ...
  try
    ...
    begin
      ...
      begin
        active:=false;
        sql.Clear;
        sql.Add('update YWCP_OEM set YN=1');
        sql.Add('where YEAR(YWCP_OEM.EXEDATE) ='''+sYear+''' and MONTH(YWCP_OEM.EXEDATE) ='''+sMonth+''' and DDBH ='''+queryGC.fieldbyname('DDBH').AsString+''' ');
        execsql;
      end;
      queryGC.Next;
    ...
    begin
      ...
      sql.add('insert into BDelRec ');
      sql.add('(TableName ,TNO,CLBH,OldID,DelID,DelDate)');
      sql.add('values (''YWCP_OEM'',''Reset'','''+sYear+''' +''/''+ '''+sMonth+''' ');
      sql.add(','''+main.Edit1.Text+''' ,'''+main.Edit1.Text+''' ,getdate())');
      //funcobj.WriteErrorLog(sql.Text);
      execsql;
    end;
    showmessage('Already unlock '+FormatDateTime('YYYY/MM',DTP_form_GC.Date)+' ');
  except
    on E:Exception do
    showmessage('Have wrong:'+E.Message);
  end;
...
procedure TStockShoeMonth.Button28Click(Sender: TObject);
begin
  StockShoeMonthGC_Print:=TStockShoeMonthGC_Print.Create(self);
  StockShoeMonthGC_Print.Quickrep1.prepare;
  StockShoeMonthGC_Print.PageN.caption:=inttostr(StockShoeMonthGC_Print.quickrep1.QRPrinter.pagecount);
  StockShoeMonthGC_Print.Quickrep1.preview;
  StockShoeMonthGC_Print.Free;
end;
