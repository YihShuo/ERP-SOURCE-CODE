...
begin
  ...
  begin
    active:=false;
    SQL.Clear;
    SQL.Add('  SELECT zsdh, zsqm, zsywjc,tybh ');
    SQL.Add('  FROM (SELECT zsdh, zsqm, zsywjc,YN,tybh,tyjh FROM zszl UNION ALL SELECT zsdh, CAST(dbo.fSignsToNoSigns(zsqm) AS VARCHAR(200)) AS zsqm, CAST(dbo.fSignsToNoSigns(zsywjc) AS VARCHAR(200)) AS zsywjc,YN,tybh,tyjh FROM zszl_pay )ZSZL  ');
    SQL.Add('  WHERE 1=1    ');
