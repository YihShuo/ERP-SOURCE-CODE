...
type
  TPaymentVATNO_PrintHD = class(TForm)
    ...
    qry_TTTVNACC_TTTU: TCurrencyField;
    qry_TTTFKBH_TTTU: TMemoField;
    LTTVN5: TQRDBText;
  ...
  public      
    ...
    function RMBFloatToChineseUSD(ARMBCash: Real): string;
    function RMBFloatToVietnamUSD(inputNumber: Real): string;
  end;

var
  PaymentVATNO_PrintHD: TPaymentVATNO_PrintHD;
...
function TPaymentVATNO_PrintHD.RMBFloatToChinese(ARMBCash: Real): string;
const
...
function TPaymentVATNO_PrintHD.RMBFloatToVietnam(inputNumber: Real): string;
const
...
function TPaymentVATNO_PrintHD.RMBFloatToChineseUSD(ARMBCash: Real): string;
const
  cNum: WideString = '箂滁禠把竩ヮ嘲琺╤-窾ㄕ珺货ㄕ珺窾ㄕ珺じàだ';
  cCha: array [0 .. 1, 0 .. 11] of string =
    (('箂', '箂ㄕ', '箂珺', '箂箂箂', '箂箂', '箂货', '箂窾', '箂じ', '货窾', '箂à', '箂だ', '箂俱'),
...
function TPaymentVATNO_PrintHD.RMBFloatToVietnamUSD(inputNumber: Real): string;
const
  unitNumbers:array [0..9] of String =('khong','mot','hai','ba','bon','nam','sau','bay','tam','chin');
  placeValues:array [0..3] of String =('','nghin','trieu','ty');
var
...
procedure TPaymentVATNO_PrintHD.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TPaymentVATNO_PrintHD.FormDestroy(Sender: TObject);
begin
  PaymentVATNO_PrintHD:=nil;
end;

procedure TPaymentVATNO_PrintHD.VN_WORDPrint(sender: TObject;
  var Value: String);
  var acc, str:string;
begin
  ...
  begin
    ...
    begin
      ...
      if (fieldbyname('b').Value=0) then
        str:=RMBFloatToVietnamUSD(StrToFloat(fieldbyname('a').AsString)) +' do'
      else if (fieldbyname('b').Value>0) and (fieldbyname('b').Value<10) then
        str:=RMBFloatToVietnamUSD(StrToFloat(fieldbyname('a').AsString))+' do va khong '+RMBFloatToVietnamUSD(StrToFloat(fieldbyname('b').AsString))+' cents'
      else str:=RMBFloatToVietnamUSD(StrToFloat(fieldbyname('a').AsString))+' do va '+RMBFloatToVietnamUSD(StrToFloat(fieldbyname('b').AsString))+' cents';
      VN_WORD.Caption:=Uppercase(Copy(str,1,1))+Lowercase(Copy(str,2,Length(str)));
    end;
  end else begin
...
procedure TPaymentVATNO_PrintHD.TW_WORDPrint(sender: TObject;
  var Value: String);
  var acc:string;
begin
  acc:=StringReplace(QRExpr4.Value.strResult,',','',[rfReplaceAll, rfIgnoreCase]);     
  ...
  begin
    LTTVN3.Caption:='USD';
    QRLabel96.Caption:='USD';
    TW_WORD.Caption:=RMBFloatToChineseUSD(StrToFloat(acc));
  end else begin
    LTTVN3.Caption:='VND';
    QRLabel96.Caption:='VND';
...
procedure TPaymentVATNO_PrintHD.DatePrint(sender: TObject;
  var Value: String);
begin
  date.Caption:=PaymentVATNO.GetChineseDate(PaymentVATNO.qry_CGPayment.FieldByName('USERDATE').value);
end;

procedure TPaymentVATNO_PrintHD.FormCreate(Sender: TObject);
begin
  ...
  begin
    active:=false;
    sql.Clear;
    sql.Add('  SELECT SUM(TONGTIEN_CHUATHUE) as TONGTIEN_CHUATHUE,SUM(TONGTIEN_THUE) as TONGTIEN_THUE ');
    sql.Add('  FROM (  SELECT CASE WHEN isnull(TRU_CHIETKHAU,0)=0 then (round(b.TONGTIEN_CHUATHUE,2) +isnull(round(b.TONGTIEN_PHI,2),0))');
    sql.Add('                 else (round(b.TONGTIEN_CHUATHUE,2) -isnull(round(b.TONGTIEN_CHIETKHAU,2),0)+isnull(round(b.TONGTIEN_PHI,2),0)) end as TONGTIEN_CHUATHUE,round(b.TONGTIEN_THUE,2) as TONGTIEN_THUE ');
    sql.Add('  FROM CGZL_PaymentSS a  ');
    sql.Add('  LEFT JOIN '+PaymentVATNO.SQL_LinkServer+'.CGZL_VATNO b ON a.VATNO=b.VATNO  AND a.ZSBH_MST=b.ZSBH_MST  and a.KYHIEU_HOADON=b.KYHIEU_HOADON ');
    sql.Add('  WHERE FKBH=:FKBH and a.ZSBH_MST<>''TRUTIEN'' ');
    sql.Add('  UNION ALL ');
    sql.Add('  SELECT -1*isnull(a.TONGTIEN_CHUATHUE,0) as TONGTIEN_CHUATHUE,-1*isnull(a.TONGTIEN_THUE,0) as TONGTIEN_THUE ');
    sql.Add('  FROM CGZL_PaymentSS a ');
    sql.Add('  WHERE FKBH=:FKBH and a.ZSBH_MST=''TRUTIEN'' ) CGZL_PAYMENTSS ');
    active:=true;
  end;
  if qry_TTT.FieldByName('FKBH_TTT').Value='()' then
  begin
  ...
  if qry_TTT.FieldByName('FKBH_TTTU').Value='()' then
  begin
    LTTVN5.Enabled:=false;
  end;
end;
