object PaymentVATNO_PrintHD: TPaymentVATNO_PrintHD
  Left = 497
  Top = 257
  Width = 845
  Height = 788
  Caption = 'PaymentVATNO_PrintHD'
  Color = clBtnFace
  ...
  object QuickRep2: TQuickRep
    ...
    object QRBand9: TQRBand
      ...
      object QRExpr4: TQRExpr
        ...
        Expression = 
          'FORMATNUMERIC('#39'#,##0'#39',SUM(TONGTIEN_THANHTOAN)-qry_CGPaymentSTT2.' +
          'TONGTIEN_THANHTOAN1-qry_TTT.VNACC_TTTU)'
        FontSize = 10
      end
  ...
  object qry_TTT: TQuery
    ...
    SQL.Strings = (
      ...
      'WHERE  FKBH=:FKBH'
        ...
        'KBH_TTT,'
      ' '#39'('#39' +(CAST((SELECT FKBH_TTTU+'#39'; '#39
      
        'FROM( SELECT '#39'L'#39'+STT+'#39' TT:'#39'+ISNULL(REPLACE(CONVERT(varchar, TONG' +
        'TIEN_THANHTOAN, 1), '#39'.00'#39', '#39#39'), '#39'0'#39')+'#39' VND, C:'#39'+ISNULL(REPLACE(C' +
        'ONVERT(varchar, TONGTIEN_THANHTOAN_CONLAI, 1), '#39'.00'#39', '#39#39'), '#39'0'#39')+' +
        #39' VND'#39' FKBH_TTTU FROM CGZL_PaymentSS_Advance  '
      
        'LEFT JOIN (SELECT FKBH_TTT, FKBH FROM CGZL_Payment_Map WHERE CGZ' +
        'L_Payment_Map.FKBH=:FKBH) CGZL_Payment_Map ON 1=1'
      'WHERE CGZL_PaymentSS_Advance.FKBH=CGZL_Payment_Map.FKBH_TTT'
      
        'and CAST(CGZL_PaymentSS_Advance.STT AS INT) <= ( SELECT MAX(CAST' +
        '(STT AS INT)) FROM CGZL_PaymentSS_Advance  WHERE FKBH_S = :FKBH ' +
        ')'
      
        'GROUP BY STT,TONGTIEN_THANHTOAN,TONGTIEN_THANHTOAN_CONLAI)a FOR ' +
        'XML PATH('#39#39'))AS VARCHAR(MAX))) +'#39')'#39' AS FKBH_TTTU,'
      ...
      ' FROM CGZL_Paymentss '
        ...
        ' RIGHT JOIN CGZL_Payment_Map ON CGZL_Payment_Map.FKBH_TTT = CGZL' +
        '_Paymentss.FKBH '
      ' WHERE  CGZL_Payment_Map.FKBH =:FKBH)  AS VNACC_TTT,'
      
        ' (SELECT isnull(SUM(TONGTIEN_THANHTOAN_UNG),isnull(SUM(TONGTIEN_' +
        'THANHTOAN),0)) VNACC'
      ' FROM CGZL_Paymentss '
        ...
        ' LEFT JOIN (select isnull(TONGTIEN_THANHTOAN,0) TONGTIEN_THANHTO' +
        'AN_UNG, FKBH,VATNO,ZSBH_MST,KYHIEU_HOADON from CGZL_PaymentSS_Ad' +
        'vance WHERE FKBH_S =:FKBH) a '
      
        ' on a.FKBH=CGZL_Paymentss.FKBH and a.KYHIEU_HOADON=CGZL_Payments' +
        's.KYHIEU_HOADON and a.VATNO=CGZL_Paymentss.VATNO and a.ZSBH_MST=' +
        'CGZL_Paymentss.ZSBH_MST'
      ' WHERE  CGZL_Payment_Map.FKBH =:FKBH)  AS VNACC_TTTU')
    Left = 692
    Top = 107
    ParamData = <
      item
    ...
    object qry_TTTVNACC_TTTU: TCurrencyField
      FieldName = 'VNACC_TTTU'
    end
    object qry_TTTFKBH_TTTU: TMemoField
      FieldName = 'FKBH_TTTU'
      BlobType = ftMemo
    end
  end
  ...
  object Query1: TQuery
    ...
    SQL.Strings = (
      ...
      'FROM ('
        ...
        'N_CHIETKHAU,0),0)+isnull(round(b.TONGTIEN_PHI,0),0))TONGTIEN_CHU' +
        'ATHUE,round(b.TONGTIEN_THUE,0) as TONGTIEN_THUE'
      '    FROM CGZL_PaymentSS a'
