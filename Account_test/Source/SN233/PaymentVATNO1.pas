...
type
  TPaymentVATNO = class(TForm)
    ...
    BtnTAM_P_TB: TBitBtn;
    qry_CGPaymenttybh: TStringField;
    BtnS_P_VN: TBitBtn;
    BtnSS_P_US: TBitBtn;
    ...
    procedure BtnS_P_VNClick(Sender: TObject);
    procedure BtnSS_P_USClick(Sender: TObject);
...
begin
  ...
  begin
    ...
    begin
      active:=false;
      SQL.Clear;
      SQL.Add('SELECT FKBH,HDNO, ZSBH, ZSZL.zsqm, ZSZL.zsywjc,ZSZL.tybh, CGZL_Payment.GSBH, CGZL_Payment.DEPID, BDepartment.DepName, BDepartment.DepMemo, ');
      ...
      SQL.Add('LEFT JOIN (SELECT zsdh, zsqm, zsywjc,yn,tybh FROM zszl UNION ALL SELECT zsdh, CAST(dbo.fSignsToNoSigns(zsqm) AS VARCHAR(200)) AS zsqm, CAST(dbo.fSignsToNoSigns(zsywjc) AS VARCHAR(200)) AS zsywjc,yn,tybh FROM zszl_pay )ZSZL ');
      SQL.Add('       ON CGZL_Payment.ZSBH=ZSDH and ZSZL.yn=''2'' ');
    ...
    begin
      ...
      begin
        BtnS_P.Enabled := true;   
        BtnS_P_VN.Enabled := true;
      end else
      begin
        BtnS_P.Enabled := false;   
        BtnS_P_VN.Enabled := false;
      end;
  ...
  begin
    ...
    begin
      ...
      SQL.Add('FROM CGZL_Payment   ');
      SQL.Add('LEFT JOIN (SELECT zsdh, zsqm, zsywjc,yn,tybh FROM zszl UNION ALL SELECT zsdh, CAST(dbo.fSignsToNoSigns(zsqm) AS VARCHAR(200)) AS zsqm, CAST(dbo.fSignsToNoSigns(zsywjc) AS VARCHAR(200)) AS zsywjc,yn,tybh FROM zszl_pay )ZSZL ');
      SQL.Add('      ON CGZL_Payment.ZSBH=ZSDH ');
    ...
    begin
      ...
      begin
        ...
        BtnSS_P_US.Enabled := true;
      end else
      begin
        BtnSS_P.Enabled := false;
        BtnSS_P_US.Enabled := false;
      end;
      if AccountN233_Check_Print_TB='Y' then
      begin
...
begin
  ...
  PaymentVATNO_PrintHD.TIENTE:='VND';
      ...
      case qry_TTHD.updatestatus of
          ...
          begin
            ...
            begin
              ...
              SQL.Add('  update CGZL_PaymentSS_Advance SET FKBH_S = '''+qry_TTHD.FieldByName('FKBH').asstring+'''      ');
              SQL.Add('  WHERE isnull(FKBH_S,'''')='''' AND FKBH='''+qry_TTHD.FieldByName('FKBH_TTT').asstring+'''   ');
              ExecSQL;
            end;
          end;
          usmodified:
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  SQL.Add('  update CGZL_PaymentSS_Advance SET FKBH_S = NULL      ');
                  SQL.Add('  WHERE FKBH_S='''+qry_TTHD.FieldByName('FKBH').asstring+'''   ');
                  execsql;
                  active:=false;
                end;
...
procedure TPaymentVATNO.BtnS_P_VNClick(Sender: TObject);
  var pt_nt, pt_tt: string;
begin
  with qry_id do
  begin
  ...
  PaymentVATNO_Print.LTT_US.Caption:= pt_tt+'USD';
  PaymentVATNO_Print.LTT_VN.Caption:= pt_tt+'VND';   
  PaymentVATNO_Print.TIENTE:='VND';
  PaymentVATNO_Print.QRExpr11.Expression:='FORMATNUMERIC(''#,##0'',SUM(VNACC_THANHTOAN)-qry_CGPaymentSTT.VNACC_THANHTOAN1-qry_TTT.VNACC_TTT)';
  qry_CGPaymentSTT.active := false;
  qry_CGPaymentSTT.cachedupdates := false;
  qry_CGPaymentSTT.requestlive := false;
  qry_CGPaymentSTT.active := true;
...
procedure TPaymentVATNO.BtnSS_P_USClick(Sender: TObject);
begin
  with qry_id do
  begin
    active:=false;
  ...
  if qry_CGPaymentSTT2.RecordCount>0 then
    ...
    PaymentVATNO_PrintHD.QRSubDetail1.PrintIfEmpty:=false;
  PaymentVATNO_PrintHD.TIENTE:='USD';
  PaymentVATNO_PrintHD.QRDBText48.Mask:='##,#0.00';
  PaymentVATNO_PrintHD.QRDBText47.Mask:='##,#0.00';
  PaymentVATNO_PrintHD.QRDBText30.Mask:='##,#0.00';   
  PaymentVATNO_PrintHD.QRDBText1.Mask:='##,#0.00';
  PaymentVATNO_PrintHD.QRDBText3.Mask:='##,#0.00';
  PaymentVATNO_PrintHD.QRDBText4.Mask:='##,#0.00';
  PaymentVATNO_PrintHD.QRDBText9.Mask:='##,#0.00';
  PaymentVATNO_PrintHD.QRDBText17.Mask:='##,#0.00';
  PaymentVATNO_PrintHD.LTTVN2.Mask:='##,#0.00';
  PaymentVATNO_PrintHD.QRLabel87.Caption:='THUE(USD)';
  PaymentVATNO_PrintHD.QRLabel81.Caption:='(USD)';
  PaymentVATNO_PrintHD.QRLabel101.Caption:='THANH TIEN (USD)';
  PaymentVATNO_PrintHD.QRLabel103.Caption:='金額(美元)';    
  PaymentVATNO_PrintHD.QRExpr3.Expression:='FORMATNUMERIC(''##,#0.00'',SUM(TONGTIEN_THANHTOAN)-qry_CGPaymentSTT2.TONGTIEN_THANHTOAN1)';
  PaymentVATNO_PrintHD.QRExpr4.Expression:='FORMATNUMERIC(''##,#0.00'',SUM(TONGTIEN_THANHTOAN)-qry_CGPaymentSTT2.TONGTIEN_THANHTOAN1-qry_TTT.VNACC_TTTU)';
  PaymentVATNO_PrintHD.quickrep2.prepare;
  PaymentVATNO_PrintHD.page2.caption:=inttostr(PaymentVATNO_PrintHD.quickrep2.qrprinter.pagecount);
  PaymentVATNO_PrintHD.quickrep2.preview;
  PaymentVATNO_PrintHD.free;
...
procedure TPaymentVATNO.BitBtn1Click(Sender: TObject);
var i,j,k:integer;
    eclApp,WorkBook:olevariant;
begin
  with Query1 do
  begin
      ...
      sql.Add(' LEFT JOIN [EEP].[LingYi].[dbo].[DOCNO_File_SV] DF1 ON ISNULL(DOCNO_File.VATFile, '''') = DF1.VATFile_SV  ');
      sql.Add(' LEFT JOIN [EEP].[LingYi].[dbo].[DOCNO_File_SV] DF2 ON ISNULL(DOCNO_File.VATFile1, '''') = DF2.VATFile_SV  ');
      sql.Add(' WHERE 1=1 and a.ZSBH_MST<>''TRUTIEN'' and HD_LOAI=''2'' and CGZL_Payment.GSBH='''+main.Edit2.Text+''' ');
      sql.add('      and convert(varchar,CGZL_Payment.USERDATE,111) between ');
      sql.add('          '''+formatdatetime('yyyy/MM/dd',DTP7.Date)+''' ');
      sql.add('          and '+''''+formatdatetime('yyyy/MM/dd',DTP8.Date)+''' ');
      active:=true;
