...
var
  CostFeeDep_Det: TCostFeeDep_Det;

implementation

uses CostFee1, CostFee_Det1, CostFeeDep1;

{$R *.dfm}

procedure TCostFeeDep_Det.FormCreate(Sender: TObject);
begin
with query1 do
  begin
    active:=false;
    ...
    sql.add(''''+formatdatetime('yyyy/MM/dd',CostFeeDep.DTP1.Date)+'''');
    sql.add(' and '+''''+ formatdatetime('yyyy/MM/dd',CostFeeDep.DTP2.Date)+'''');
    sql.add('and KCLLS.Qty<>0');  
    sql.add('and not exists(select ZLZL.ZLBH from ZLZL where ZLZL.ZLBH=KCLLS.SCBH )');
    sql.add('and SCBLYY.YWSM like '+''''+'%'+CostFeeDep.edit1.Text+'%'+'''');
    sql.add('order by KCLLS.CLBH,KCLLS.LLNO ');
    active:=true;
  end;
end;

procedure TCostFeeDep_Det.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
action:=cafree;
end;

procedure TCostFeeDep_Det.Query1CalcFields(DataSet: TDataSet);
begin
query1.FieldByName('VNACC').Value:=query1.FieldByName('VNPrice').Value*query1.FieldByName('Qty').Value ;
end;

procedure TCostFeeDep_Det.Excel1Click(Sender: TObject);
var
...
procedure TCostFeeDep_Det.FormDestroy(Sender: TObject);
begin
costfeedep_det:=nil;
end;
