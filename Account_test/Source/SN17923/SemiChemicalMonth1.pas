...
var iYear,iMonth,iDay,iiYear,iiMonth,iiDay:word;
    sYear,sMonth,VNACC,ssYear,ssMonth,VNACC_KHUA,VNACC_KHUC:string;
    sDate:TDate;
begin
  ...
  begin
    ...
    SQL.Add('             From ( Select case when DDZLTR.DDBH is not null then DDZLTR.DDBH else  YWDD.YSBH end as DDBH,Sum(YWCP.Qty) as InQty');
    ...
    SQL.Add('       			where 1=1 --(DDBH like ''AUR%'' or DDBH like ''Y%'') ');
  ...
  //===2025.05.12 Tach chi phi NVL theo khu (KHU A va KHU C) ap dung tinh tu thang 4/2025
  //Chi phi nguyen vat lieu phan bo ( gia tri xuat 6211 SCBH like 'ZZZZ%' - gia tri nhap 6211 CGBH like 'ZZZZ%' )/ so luong nhap kho thanh pham trong thang
  VNACC_KHUA:='0.0';
  VNACC_KHUC:='0.0';
  with  TempQry do
  begin
    ...
    SQL.Clear;
    // 20221112 Chi phi nguyen vat lieu phan bo ( gia tri xuat 6211 SCBH like 'ZZZZ%' - gia tri nhap 6211 CGBH like 'ZZZZ%' )/ so luong nhap kho thanh pham trong thang
    SQL.Add('Select sum(isnull(KCLL.VNACC_KHUA,0)) - sum(isnull(KCRK.VNACC,0)) as VNACC_KHUA,sum(InQty_KHUA) as InQty_KHUA,(sum(isnull(convert(float,KCLL.VNACC_KHUA),0)) - sum(isnull(convert(float,KCRK.VNACC),0)))/sum(InQty_KHUA) as AVGZVNACC_KHUA');
    SQL.Add('      ,sum(isnull(KCLL.VNACC_KHUC,0)) - sum(isnull(KCRK.VNACC,0)) as VNACC_KHUC,sum(InQty_KHUC) as InQty_KHUC,(sum(isnull(convert(float,KCLL.VNACC_KHUC),0)) - sum(isnull(convert(float,KCRK.VNACC),0)))/sum(InQty_KHUC) as AVGZVNACC_KHUC');
    SQL.Add('from (	select sum(VNACC_KHUA) as VNACC_KHUA, sum(VNACC_KHUC) as VNACC_KHUC  ');
    SQL.Add('   		from(                      ');
    SQL.Add('             Select Sum(VNACC_HG) as VNACC_KHUA, 0 as VNACC_KHUC ');
    SQL.Add('             From KCLL');
    SQL.Add('     				Left join KCLLS on KCLL.LLNO=KCLLS.LLNO ');
    SQL.Add('             Left join KCCK on KCCK.CKBH=KCLL.CKBH ');
    SQL.Add('             left join KCZLS on KCZLS.CLBH=KCLLS.CLBH and KCZLS.CKBH=KCLL.CKBH  ');
    ...
    SQL.Add('     				and not exists(select* from KCLLS_CB where LLNO=KCLLS.LLNO and SCBH=KCLLS.SCBH and CLBH=KCLLS.CLBH) ');
    SQL.Add('     				UNION ALL');
    SQL.Add('     				SELECT Sum(VNACC) as VNACC_KHUA, 0 as VNACC_KHUC ');
    SQL.Add('     				FROM KCLLS_CB ');
    SQL.Add('     				Left join KCLL on KCLL.LLNO=KCLLS_CB.LLNO');
    SQL.Add('             Left join KCCK on KCCK.CKBH=KCLL.CKBH ');
    SQL.Add('     				where KCLL.CFMID<>''NO'' and Convert(varchar,KCLL.CFMDate,111) between '''+Formatdatetime('yyyy/MM/dd',startofthemonth(sDate))+''' and '''+Formatdatetime('yyyy/MM/dd',endofthemonth(sDate))+''' ');
    ...
    SQL.Add('     				UNION ALL');
    SQL.Add('   		      Select  0 as VNACC_KHUA,Sum(VNACC_HG) as VNACC_KHUC ');
    SQL.Add('             From KCLL');
    SQL.Add('     				Left join KCLLS on KCLL.LLNO=KCLLS.LLNO ');
    SQL.Add('             Left join KCCK on KCCK.CKBH=KCLL.CKBH ');
    SQL.Add('             left join KCZLS on KCZLS.CLBH=KCLLS.CLBH and KCZLS.CKBH=KCLL.CKBH  ');
    ...
    if CBX_HGLB2.ItemIndex=5 then
    SQL.Add('     				and left(KCLLS.HGLB,2) in (''NK'',''KD'',''HD'',''TC'',''NQ'') ');
    SQL.Add('      			  and KCLLS.CostID like ''621%'' and (KCLLS.SCBH like ''CUIT%'' or KCLLS.SCBH like ''CUIK%'' or KCLLS.SCBH like ''UQIT%'' or KCLLS.SCBH like ''UQIK%'' or KCLLS.SCBH like ''VSIT%'' or KCLLS.SCBH like ''VSIK%'' ');
    SQL.Add('             or KCLLS.SCBH like ''VJIT%'' or KCLLS.SCBH like ''VJIK%'') ');  //2025.12.03 xet them lenh nhan hang VEJA
    //end;
    SQL.Add('     				and not exists(select* from KCLLS_CB where LLNO=KCLLS.LLNO and SCBH=KCLLS.SCBH and CLBH=KCLLS.CLBH) ');
    SQL.Add('     				UNION ALL');
    SQL.Add('     				SELECT 0 as VNACC_KHUA,Sum(VNACC) as VNACC_KHUC ');
    SQL.Add('     				FROM KCLLS_CB ');
    SQL.Add('     				Left join KCLL on KCLL.LLNO=KCLLS_CB.LLNO');
    SQL.Add('             Left join KCCK on KCCK.CKBH=KCLL.CKBH ');
    SQL.Add('     				where KCLL.CFMID<>''NO'' and Convert(varchar,KCLL.CFMDate,111) between '''+Formatdatetime('yyyy/MM/dd',startofthemonth(sDate))+''' and '''+Formatdatetime('yyyy/MM/dd',endofthemonth(sDate))+''' ');
    ...
    if CBX_HGLB2.ItemIndex=5 then
    SQL.Add('     				and left(KCLLS_CB.HGLB,2) in (''NK'',''KD'',''HD'',''TC'',''NQ'') ');
    SQL.Add('      		    and KCLLS_CB.CostID like ''621%'' and (KCLLS_CB.SCBH_Cost like ''CUIT%'' or KCLLS_CB.SCBH_Cost like ''CUIK%'' or KCLLS_CB.SCBH_Cost like ''UQIT%'' or KCLLS_CB.SCBH_Cost like ''UQIK%'' ');
    //=====2025.08.04
    SQL.Add('                  or KCLLS_CB.SCBH_Cost like ''VSIT%'' or KCLLS_CB.SCBH_Cost like ''VSIK%'' or KCLLS_CB.SCBH_Cost like ''VJIT%'' or KCLLS_CB.SCBH_Cost like ''VJIK%'') '); //2025.12.03
    //end;
    SQL.Add('     			) KCLL');
    SQL.Add('   	 ) KCLL');
    SQL.Add('left join (  SELECT SUM(KCRKS.VNACC+ISNULL(EXCHACC,0)) VNACC');
    ...
    SQL.Add('             AND KCRKS.CGBH LIKE ''ZZZZ%''');
    SQL.Add('          )KCRK ON 1=1');
    //=======TONG SO LUONG NHAP KHO THANH PHAM KHU A
    SQL.Add('left join (  select Sum(InQty) as InQty_KHUA ');
    SQL.Add('             From ( Select case WHEN YWCP.MEMO = ''OEM'' THEN YWCP.DDBH when DDZLTR.DDBH is not null then DDZLTR.DDBH else  YWDD.YSBH end as DDBH,Sum(YWCP.Qty) as InQty');
    SQL.Add('       		  from (select * from YWCP union all select * from YWCPOld union all select * from YWCP_OEM) YWCP ');
    SQL.Add('             left join YWDD  with (nolock) on YWDD.DDBH=YWCP.DDBH');
    SQL.Add('             left join DDZLTR on YWCP.DDBH=DDZLTR.DDBH1 ');
    SQL.Add('       			where convert(varchar,YWCP.Indate,111) between '''+Formatdatetime('yyyy/MM/dd',startofthemonth(sDate))+''' and '''+Formatdatetime('yyyy/MM/dd',endofthemonth(sDate))+''' ');
    SQL.Add('     				and IsNull(YWCP.SB,'''')<>'''' ');
    SQL.Add('             and YWCP.DDBH not in (select DDBH from DDZLTR) ');
    SQL.Add('             Group by case WHEN YWCP.MEMO = ''OEM'' THEN YWCP.DDBH when DDZLTR.DDBH is not null then DDZLTR.DDBH else  YWDD.YSBH end ');
    SQL.Add('             Union all ');
    SQL.Add('       			SELECT MasterOrder,SUM(total) AS InQty');
    SQL.Add('       			FROM CBY_Orders');
    SQL.Add('       			WHERE CONVERT(VARCHAR, StockDate, 111) BETWEEN '''+Formatdatetime('yyyy/MM/dd',startofthemonth(sDate))+''' and '''+Formatdatetime('yyyy/MM/dd',endofthemonth(sDate))+''' ');
    ...
    SQL.Add('     	                            and left(KCLLS.HGLB,2) in (''NK'',''KD'',''HD'',''TC'',''NQ'') ');
    SQL.Add('                                   and KCLLS.Qty<>0 and SUBSTRING(KCLLS.SCBH,4,1) not in(''T'',''K'')');
    SQL.Add('                                   and (KCLLS.SCBH like ''Y%'' or KCLLS.SCBH like ''LAIV%'' or KCLLS.SCBH like ''CBYV%'' or KCLLS.SCBH like ''CC%'')');   //2025.09.12 THEM LENH DE CC
    SQL.Add('                   								Group by KCLLS.SCBH ');
    SQL.Add('                   								union all ');
    SQL.Add('                   								Select DDBH ');
    SQL.Add('              								      from CWKCLLMonth ');
    SQL.Add('	                                  where KCYEAR='''+sYear+''' and KCMONTH='''+sMonth+''' ');
    SQL.Add('		                                and CWKCLLMonth.CKBH='''+CKBHID+''' ');
    SQL.Add('                                   and (DDBH like ''Y%'' or DDBH like ''LAIV%'' or DDBH like ''CBYV%'' or DDBH like ''CC%'')');   //2025.09.12 THEM LENH DE CC
    SQL.Add('                                   and CWKCLLMonth.VNACC>0)');
    SQL.Add('       		) B on 1=1');
    //=======TONG SO LUONG NHAP KHO THANH PHAM KHU C
    SQL.Add('left join (  select Sum(InQty) as InQty_KHUC ');
    SQL.Add('             From ( Select case WHEN YWCP.MEMO = ''OEM'' THEN YWCP.DDBH when DDZLTR.DDBH is not null then DDZLTR.DDBH else  YWDD.YSBH end as DDBH,Sum(YWCP.Qty) as InQty');
    SQL.Add('       		  from (select * from YWCP union all select * from YWCPOld union all select * from YWCP_OEM) YWCP ');
    SQL.Add('             left join YWDD  with (nolock) on YWDD.DDBH=YWCP.DDBH');
    SQL.Add('             left join DDZLTR on YWCP.DDBH=DDZLTR.DDBH1 ');
    ...
    SQL.Add('     	                            and left(KCLLS.HGLB,2) in (''NK'',''KD'',''HD'',''TC'',''NQ'') ');
    SQL.Add('                                   and KCLLS.Qty<>0 and SUBSTRING(KCLLS.SCBH,4,1) not in(''T'',''K'')');
    SQL.Add('                                   and (KCLLS.SCBH like ''S%'' or KCLLS.SCBH like ''CUIV%'' or KCLLS.SCBH like ''UQIV%'' or KCLLS.SCBH like ''VS%'' or KCLLS.SCBH like ''VJIV%'')');
    SQL.Add('                   								Group by KCLLS.SCBH ');
    SQL.Add('                   								union all ');
    SQL.Add('                   								Select DDBH ');
    SQL.Add('              								      from CWKCLLMonth ');
    SQL.Add('	                                  where KCYEAR='''+sYear+''' and KCMONTH='''+sMonth+''' ');
    SQL.Add('		                                and CWKCLLMonth.CKBH='''+CKBHID+''' ');
    SQL.Add('                                   and (DDBH like ''S%'' or DDBH like ''CUIV%'' or DDBH like ''UQIV%'' or DDBH like ''VS%'' or DDBH like ''VJIV%'')');
    SQL.Add('                                   and CWKCLLMonth.VNACC>0)');
    SQL.Add('       		) C on 1=1');
    //funcObj.WriteErrorLog(sql.Text);
    Active:=true;
    if RecordCount>0 then
    begin
      if FieldByName('AVGZVNACC_KHUA').AsString<>'' then
      VNACC_KHUA:=FieldByName('AVGZVNACC_KHUA').AsString;
      if FieldByName('AVGZVNACC_KHUC').AsString<>'' then
      VNACC_KHUC:=FieldByName('AVGZVNACC_KHUC').AsString;
    end;
    Active:=false;
  end;
  //==================================================
  ...
  begin
     ...
     SQL.Add('                  	 ,YWCP_Mon.InQty,Isnull(CWKCLLMonth.VNACC,0)+isnull(CWKCLLMonth_Adjust_DK.ExchACC,0) as VNACCMon,IsNull(KCLLS.VNACC,0)- Isnull(RK_THUHOI.VNACC,0)+isnull(CWKCLLMonth_Adjust_TT.ExchACC,0)  as VNACC');
     //SQL.Add('                  	 ,case when DDZL.DDBH like ''SJ%'' then 0 else Round(convert(float,YWCP_Mon.InQty*'+VNACC+'),0)+isnull(CWKCLLMonth_Adjust_PB.ExchACC,0) end as ZVNACC ');   //20250306 khong tinh chi phi phan bo khu C
     //========2025.05.07 Tinh chi phi NVL phan bo khu C
     //SQL.Add('                  	 ,Round(convert(float,YWCP_Mon.InQty*'+VNACC+'),0)+isnull(CWKCLLMonth_Adjust_PB.ExchACC,0) as ZVNACC ');
     //========2025.05.12 Tinh chi phi NVL theo khu
     SQL.Add('          ,case when (DDZL.DDBH like ''Y%'' or DDZL.DDBH like ''LAIV%'' or DDZL.DDBH like ''CBYV%'' or DDZL.DDBH like ''CC%'') then Round(convert(float,YWCP_Mon.InQty*'+VNACC_KHUA+'),0)+isnull(CWKCLLMonth_Adjust_PB.ExchACC,0)');  //2025.09.12 THEM LENH DE CC
     SQL.Add('                when (DDZL.DDBH like ''S%'' or DDZL.DDBH like ''CUIV%'' or DDZL.DDBH like ''UQIV%'' or DDZL.DDBH like ''VS%'' or DDZL.DDBH like ''VJIV%'') then Round(convert(float,YWCP_Mon.InQty*'+VNACC_KHUC+'),0)+isnull(CWKCLLMonth_Adjust_PB.ExchACC,0) end as ZVNACC');
     //========
     //訂單列表
     SQL.Add('from ( Select *');
     ...
     SQL.Add('		           group by MasterOrder ');   
     SQL.Add('               Union all');
     SQL.Add('		           select DDZL_OEM.DDBH,DDZL_OEM.Qty as Pairs');
     SQL.Add('		           from DDZL_OEM');
     SQL.Add('               Union all');
     SQL.Add('               select DDBH,Pairs ');
     SQL.Add('               from '+main.LIY_DD+'.dbo.DDZL ');
     SQL.Add('    				   where  (DDBH like ''AUR%'' or DDBH like ''SCI%'' or DDBH like ''IMG%'' or DDBH like ''GS%'' or DDBH like ''CC%'')  ');  //2025.09.12 THEM LENH DE CC
     SQL.Add('             ) DDZL');
     ...
     SQL.Add('            Select case WHEN YWCP.MEMO = ''OEM'' THEN YWCP.DDBH when DDZLTR.DDBH is not null then DDZLTR.DDBH else  YWDD.YSBH end as YSBH,Sum(YWCP.Qty) as InQty  ');
     SQL.Add('	          from (select * from YWCP union all select * from YWCPOld union all select * from YWCP_OEM) YWCP');
     SQL.Add('	          left join YWDD  with (nolock) on YWDD.DDBH=YWCP.DDBH ');
     ...
     //20241204
     SQL.Add('          UNION ALL ');
     SQL.Add('          Select YWCP.DDBH as YSBH,sum(ISNULL(YWCP_OUT.Qty_Out,YWCP.Qty)) as ShipQty  ');
     SQL.Add('          From (Select * from YWCP union all select * from YWCPold union all select * from YWCP_OEM) YWCP  ');                                                                                                               
    sql.add('           left join YWCP_OUT on YWCP_OUT.CARTONBAR=YWCP.CARTONBAR');
     SQL.Add('          Where convert(varchar,ISNULL(YWCP_OUT.ExeDate,YWCP.ExeDate),111)< '''+Formatdatetime('yyyy/MM/dd',startofthemonth(sDate))+''' ');
     SQL.Add('          and YWCP.DDBH in (Select DDBH From #DDZL_Mon)  ');
     ...
     //===2025.08.06 Them dieu chinh TONG CHI PHI NVL DAU KY
     SQL.Add('left join ( select DDBH,isnull(ExchACC,0) as ExchACC');
     SQL.Add('            from CWKCLLMonth_Adjust_PB');
     ...
     SQL.Add('                  FROM KCLLS_CB');
     SQL.Add('          	      Left join KCLL on KCLLS_CB.LLNO=KCLL.LLNO AND KCLLS_CB.CKBH=KCLL.CKBH');
     SQL.Add('                  Left join KCCK on KCCK.CKBH=KCLL.CKBH ');
     SQL.Add('	                where KCLL.CFMID<>''NO'' and Convert(varchar,KCLL.CFMDate,111) between '''+Formatdatetime('yyyy/MM/dd',startofthemonth(sDate))+''' and '''+Formatdatetime('yyyy/MM/dd',endofthemonth(sDate))+'''  ');
     //20230325 全部化學品倉
     SQL.Add('                      and KCCK.CKBH'+CKBH_SQL);
     ...
     begin
       ...
       SQL.Add('     				  SELECT Sum(VNACC) as VNACC ');
       SQL.Add('     				  FROM KCLLS_CB ');
       SQL.Add('     				  Left join KCLL on KCLL.LLNO=KCLLS_CB.LLNO AND KCLLS_CB.CKBH=KCLL.CKBH');
       SQL.Add('              Left join KCCK on KCCK.CKBH=KCLL.CKBH ');
       SQL.Add('     				  where KCLL.CFMID<>''NO'' and Convert(varchar,KCLL.CFMDate,111) between '''+Formatdatetime('yyyy/MM/dd',startofthemonth(sDate))+''' and '''+Formatdatetime('yyyy/MM/dd',endofthemonth(sDate))+'''  ');
       //20230325全部化學品倉
       SQL.Add('                  and KCCK.CKBH'+CKBH_SQL);
...
procedure TSemiChemicalMonth.Adjust_NVL_DKClick(Sender: TObject);
begin
  Adjust_CPNVL_PB_Chemical:=TAdjust_CPNVL_PB_Chemical.create(self);
  Adjust_CPNVL_PB_Chemical.Lbl_LB.Caption:='DK';
  Adjust_CPNVL_PB_Chemical.Button1.Click;
  Adjust_CPNVL_PB_Chemical.show;
end;
