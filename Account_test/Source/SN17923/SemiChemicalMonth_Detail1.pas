...
begin
  ...
  begin
    ...
    SQL.Add('Left join (');
    SQL.Add('Select DDZL.DDBH,Sum(ISNULL(YWCP_OUT.Qty_Out,YWCP.Qty)) as Qty,Min(ISNULL(YWCP_OUT.ExeDate,YWCP.ExeDate)) as EXEDATE');
    SQL.Add('from YWCP ');                                                                                                         
    sql.add('left join YWCP_OUT on YWCP_OUT.CARTONBAR=YWCP.CARTONBAR ');
    SQL.Add('left join YWDD on YWDD.DDBH=YWCP.DDBH');
    SQL.Add('left join DDZL on YWDD.YSBH=DDZL.DDBH');
    SQL.Add('where DDZL.DDBH=:DDBH and ISNULL(YWCP_OUT.ExeDate,YWCP.ExeDate) is not null');
    SQL.Add('Group by DDZL.DDBH');
