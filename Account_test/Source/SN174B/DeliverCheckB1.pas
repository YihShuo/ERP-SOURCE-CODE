...
procedure TDeliverCheckB.readini();
var MyIni :Tinifile;
    AppDir:string;
begin
  //
...
procedure TDeliverCheckB.ExportExcel(Query:TQuery;DBGrid:TDBGridEh);
var
...
procedure TDeliverCheckB.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TDeliverCheckB.Button1Click(Sender: TObject);
...
begin
  ...
  begin
    ...
    begin
      ...
      SQL.add('from KCLLS_1722 KCLLS ');
      SQL.add('left join KCLL_1722 KCLL on KCLL.LLNO=KCLLS.LLNO');
  ...
  begin
    ...
    begin
      ...
      SQL.Add('       ,case when KCLLS.VNPrice_HG  is null then KCLLS.VNPrice else KCLLS.VNPrice_HG end as VNPrice,case when KCLLS.VNACC_HG is null then KCLLS.VNACC else KCLLS.VNACC_HG end  as VNACC');
      SQL.Add('       ,KCLLS.SCBH,KCLLS.CostID,IsNull(SCBLYY.YWSM,'''') as YWSM,IsNull(KCLLS.MEMO,'''') as MEMO,IsNull(KCLLS.BLSB,'''') as BLSB');
      SQL.Add('       ,IsNull(KCLLS.CNO,'''') as HGBH');
      ...
      sql.Add('       ,isnull(KCLLS.HGLB,''ZZZZ'') as HGLB,KCLLS.LLMemo,KCLLS.CNO1,ZLZLS2.CLSL,TCLYL,CLZL.clzmlb');
      SQL.add('from KCLLS_1722 KCLLS ');
      SQL.add('left join KCLL_1722 KCLL on KCLL.LLNO=KCLLS.LLNO');
      SQL.add('left join CLZL on CLZL.CLDH=KCLLS.CLBH');
      SQL.add('left join BDepartment on BDepartment.ID=KCLL.DepID');
      ...
      SQL.add(') KCLLS ');
      SQL.add('group by CKBH,LLNO,DepID,DepName,CFMDATE,CLBH,YWPM,DWBH,CWBH,DFL,Qty,VNPrice,VNACC,SCBH,CostID,YWSM,MEMO,BLSB,HGBH,UnitC,RateC,HGLB,LLMemo,CNO1,TCLYL,CLSL,clzmlb');
  ...
  begin
    ...
    begin
      ...
      SQL.Add('	  from KCPKLL');
      SQL.Add('	  left join KCLL_1722 KCLL on KCPKLL.LLNO=KCLL.LLNO');
      SQL.Add('	  left join KCLLS_1722 KCLLS on KCLLS.LLNO=KCPKLL.LLNO and KCLLS.CLBH=KCPKLL.CLBH and KCLLS.DFL=KCPKLL.DFL and KCLLS.SCBH=KCPKLL.SCBH');
      SQL.Add('	  left join KCRK_1722 KCRK on KCRK.RKNO=KCPKLL.PKNO');
      SQL.Add('	  Left join KCPK_1722 KCPK on KCPK.PKNO=KCPKLL.PKNO');
...
procedure TDeliverCheckB.CheckPrice1Click(Sender: TObject);
begin
...
procedure TDeliverCheckB.FormDestroy(Sender: TObject);
begin
  DeliverCheckB:=nil
end;

procedure TDeliverCheckB.SonMatCode1Click(Sender: TObject);
begin 
  if query1.recordcount>0 then
   begin
...
procedure TDeliverCheckB.FormCreate(Sender: TObject);
var
...
procedure TDeliverCheckB.Button2Click(Sender: TObject);
var
...
procedure TDeliverCheckB.CBX1Change(Sender: TObject);
begin
  CBX4.Text:=CBX1.Text;
end;

procedure TDeliverCheckB.CBX2Change(Sender: TObject);
begin
  CBX5.Text:=CBX2.Text;
end;

procedure TDeliverCheckB.GSBHCBXChange(Sender: TObject);
begin
  if GSBHCBX.Text<>'ALL' then
  begin
    with MatSite do
