...
type
  TMonthSemiFinishProduct = class(TForm)
    ...
    Qry_MonthStockRemCost08T: TFloatField;
    Label2: TLabel;
    Button3: TButton;
    Button4: TButton;
    TempQry: TQuery;
...
begin
  ...
  begin
    active:=false;
    sql.Clear;
    sql.Add('SELECT top 1 YN FROM CW_MastCostMonth_SemiFG');
  ...
  begin 
    ...
    SQL.Add('            ,case when DATEDIFF(MONTH,KC.KCYEAR+''-''+KC.KCMONTH+''-01'','''+STD+''' ) >12 then 12 else DATEDIFF(MONTH,KC.KCYEAR+''-''+KC.KCMONTH+''-01'','''+STD+''' ) end as [Month],KC.End_UnFQty ');
    SQL.Add('  	into #EntryStockRY_Semi    ');
    SQL.Add('   from (		SELECT Total.KCYEAR, Total.KCMONTH, G_DDBH RY,case when G_DDBH like ''Y%'' then ''A'' else ''A-KT'' end Depot, sum(AA_SemiVNACC) as RemCost,');
    SQL.Add('                    sum(Isnull(T_TCPPS,0) + Isnull(Adjust.S_CPPB,0) + Isnull(Adjust_PB.S_CPPB,0)) as T_Data , max(X_NotInQty) End_UnFQty ');
    ...
    SQL.Add('       left join (');
    SQL.Add('         SELECT AdjustPB.KCYEAR, AdjustPB.KCMONTH, AdjustPB.Factory, TotalM.DDBH, ROUND(ISNULL(AdjustPB.S_CPPB,0) / TotalSum.TotalInQty , 0) * ISNULL(TotalM.InQty,0) AS S_CPPB');
    ...
    SQL.Add('  			WHERE G_DDBH IN(SELECT DISTINCT G_DDBH FROM CW_MatCostMonth_Total WHERE KCYEAR='''+cbx_Year.Text+''' AND KCMONTH='''+cbx_Month.Text+''')   ');

    SQL.Add('  			GROUP BY  Total.KCYEAR, Total.KCMONTH, G_DDBH) KC         ');
    SQL.Add('    where DATEDIFF(MONTH,KC.KCYEAR+''-''+KC.KCMONTH+''-01'','''+STD+'''   )>-1      ');
    ...
    SQL.Add('    --==================================================    ');
    SQL.Add('  if object_id(''tempdb..#EntryStockRY_Semi5'') is not null');
    SQL.Add('      begin drop table #EntryStockRY_Semi5 end');
    SQL.Add('  select Semi4.KCYear,Semi4.KCMonth,Semi4.DDBH,Semi4.Depot,max(Semi4.RemQty) as RemQty');
    SQL.Add('     , max(Semi4.RemCost) as RemCost -- cot D gia tri ton');
    SQL.Add('     , max(Semi4.RemCost01T) as RemCost01T -- cot E ton 30 ngay');
    SQL.Add('     , max(Semi4.RemCost02T) as RemCost02T -- cot G ton 60 ngay');
    SQL.Add('     , max(Semi4.RemCost03T) as RemCost03T -- cot I ton 90 ngay');
    SQL.Add('     , max(Semi4.RemCost04T) as RemCost04T -- cot I ton 90 ngay');
    SQL.Add('     , max(Semi4.RemCost05T) as RemCost05T -- cot M ton 150 ngay');
    SQL.Add('     , max(Semi4.RemCost06T) as RemCost06T -- cot O ton 180 ngay');
    SQL.Add('     , max(Semi4.RemCost07T) as RemCost07T -- cot Q ton > 180 ngay');
    SQL.Add('     , case when sum(isnull(Semi1.T_c12,0)) > max(Semi4.RemCost) - max(Semi4.RemCost01T) - max(Semi4.RemCost02T) - max(Semi4.RemCost03T) - max(Semi4.RemCost04T) - max(Semi4.RemCost05T) - max(Semi4.RemCost06T)');
    SQL.Add('       - case when max(isnull(Semi1.T_c6,0)) > max(Semi4.RemCost) - max(Semi4.RemCost01T) - max(Semi4.RemCost02T) - max(Semi4.RemCost03T) - max(Semi4.RemCost04T) - max(Semi4.RemCost05T) - max(Semi4.RemCost06T) then');
    SQL.Add('         max(Semi4.RemCost) - max(Semi4.RemCost01T) - max(Semi4.RemCost02T) - max(Semi4.RemCost03T) - max(Semi4.RemCost04T) - max(Semi4.RemCost05T) - max(Semi4.RemCost06T) else max(isnull(Semi1.T_c6,0)) end  then');
    SQL.Add('         max(Semi4.RemCost) - max(Semi4.RemCost01T) - max(Semi4.RemCost02T) - max(Semi4.RemCost03T) - max(Semi4.RemCost04T) - max(Semi4.RemCost05T) - max(Semi4.RemCost06T)');
    SQL.Add('       - case when max(isnull(Semi1.T_c6,0)) > max(Semi4.RemCost) - max(Semi4.RemCost01T) - max(Semi4.RemCost02T) - max(Semi4.RemCost03T) - max(Semi4.RemCost04T) - max(Semi4.RemCost05T) - max(Semi4.RemCost06T) then');
    SQL.Add('         max(Semi4.RemCost) - max(Semi4.RemCost01T) - max(Semi4.RemCost02T) - max(Semi4.RemCost03T) - max(Semi4.RemCost04T) - max(Semi4.RemCost05T) - max(Semi4.RemCost06T) else max(isnull(Semi1.T_c6,0)) end');
    SQL.Add('       else sum(isnull(Semi1.T_c12,0)) end as RemCost08T -- cot P ton > 360 ngay');
    SQL.Add('  into #EntryStockRY_Semi5');
    SQL.Add('  from #EntryStockRY_Semi4 Semi4 left  join #EntryStockRY_Semi1 Semi1');
    SQL.Add('  on Semi4.KCYEAR = Semi1.KCYEAR and Semi4.KCMONTH = Semi1.KCMONTH and Semi4.DDBH = Semi1.RY');
    SQL.Add('  group by Semi4.KCYEAR, Semi4.KCMONTH, Semi4.DDBH, Semi4.Depot;');

    SQL.Add('    --==================================================    ');
    SQL.Add('  Insert into CW_MastCostMonth_SemiFG (KCYEAR, KCMONTH, DDBH, Depot,RemQty, RemCost, RemCost01T, RemCost02T, RemCost03T, RemCost04T, RemCost05T, RemCost06T, RemCost07T, RemCost08T, USERID, USERDATE, YN)       ');
    SQL.Add('  Select KCYEAR,KCMONTH,DDBH,Depot,RemQty,RemCost,RemCost01T, RemCost02T, RemCost03T, RemCost04T, RemCost05T, RemCost06T, RemCost07T, RemCost08T,'''+main.Edit1.Text+''' as USERID, GETDATE() as UserDate,''1'' as YN  ');
    SQL.Add('  from #EntryStockRY_Semi5  ');
...
begin
  ...
  begin 
    active:=false;
    SQL.Clear; 
    SQL.Add('SELECT KCYEAR,KCMONTH,DDBH,Depot,RemQty,RemCost,RemCost01T,RemCost02T,RemCost03T,RemCost04T,RemCost05T,RemCost06T,RemCost07T,RemCost08T   ');
...
begin
  ...
  begin
    active:=false;
    sql.Clear;
    sql.Add(' Select top 1 YN From CW_MastCostMonth_SemiFG Where KCYEAR='''+cbx_year.Text+''' and KCMONTH='''+cbx_month.Text+''' ');
    active:=true;
  end;
...
procedure TMonthSemiFinishProduct.Button3Click(Sender: TObject);
begin
  with TempQry do
  begin
    Active := false;
    SQL.Clear;
    SQL.Add('UPDATE CW_MastCostMonth_SemiFG ');
    SQL.Add('SET YN=''5'' ');
    SQL.Add('WHERE KCYEAR = ''' + cbx_Year.Text + ''' AND KCMONTH = ''' + cbx_Month.Text + ''' ');
    ExecSQL;
  end;
...
procedure TMonthSemiFinishProduct.Button4Click(Sender: TObject);
begin
  with TempQry do
  begin
    Active := false;
