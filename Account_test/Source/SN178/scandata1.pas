...
begin
  ...
  begin
    active:=false;
    sql.Clear;
    sql.add('SELECT YWCP.DDBH,DDZL.GSBH,CONVERT(char(10), ISNULL(YWCP_OUT.ExeDate,YWCP.ExeDate),111) as exedate,CONVERT(char(10), ywcp.indate,111) as indate,');
    sql.add('xxzl.XieMing as ''Shoe Name'',DDZL.KHPO as ''P.O.#'',ddzl.article as ''Stock NO'',xxzl.YSSM as ''Color'',xxzl.LOGO as ''Size'', ');
    if combobox1.ItemIndex=1 then
    begin
      sql.add('sum(ISNULL(YWCP_OUT.Qty_Out,YWCP.Qty)) as ''Qty(Pairs)'' ');
    end;
    if combobox1.ItemIndex=0  then
    begin
      sql.add('sum(YWCP.Qty) as ''Qty(Pairs)'' ');
    end;
    sql.add('FROM YWCP');
    ...
    if combobox1.ItemIndex=1 then
    begin
      sql.add('and CONVERT(char(10), ISNULL(YWCP_OUT.ExeDate,YWCP.ExeDate),111) Between ');
    ...
    sql.add('group by YWCP.DDBH,DDZL.GSBH,CONVERT(char(10), ISNULL(YWCP_OUT.ExeDate,YWCP.ExeDate),111),CONVERT(char(10), ywcp.indate,111),xxzl.XieMing,DDZL.KHPO,ddzl.article,xxzl.YSSM,xxzl.LOGO');
  ...
  begin
    ...
    begin
      active:=false;
      sql.Clear;
      sql.add('SELECT YWCP.DDBH,DDZL.GSBH,CONVERT(char(10), ISNULL(YWCP_OUT.ExeDate,YWCP.ExeDate),111) as exedate,');
      sql.add('xxzl.XieMing as ''Shoe Name'',DDZL.KHPO as ''P.O.#'',ddzl.article as ''Stock NO'',xxzl.YSSM as ''Color'',xxzl.LOGO as ''Size'', sum(ISNULL(YWCP_OUT.Qty_Out,YWCP.Qty)) as ''Qty(Pairs)'' FROM YWCP');
      sql.add('left join YWCP_OUT on YWCP_OUT.CARTONBAR=YWCP.CARTONBAR');
      sql.add('left join YWDD on YWDD.DDBH=YWCP.DDBH');
      sql.add('left join DDZL on DDZL.DDBH=YWDD.YSBH');
      sql.add('left join xxzl on xxzl.SheHao=DDZL.SheHao and xxzl.XieXing=DDZL.XieXing');
      ...
      if combobox2.text<>'' then
        sql.add('and DDZL.GSBH='''+combobox2.text+''' ');
      sql.add('group by YWCP.DDBH,DDZL.GSBH,CONVERT(char(10), ISNULL(YWCP_OUT.ExeDate,YWCP.ExeDate),111),xxzl.XieMing,DDZL.KHPO,ddzl.article,xxzl.YSSM,xxzl.LOGO');
      sql.add('order by ywcp.DDBH');
      active:=true;
    end;
