...
type
  TEntryCheckPay = class(TForm)
    ...
    Query2USDTotal: TCurrencyField;
    Query2VNDTotal: TCurrencyField;
  ...
  begin
        with query1 do
         begin
            active:=false;
            sql.Clear;
            sql.Add('select cgzl.cgno,CWFK.GSBH, CWYF.*,zszl.zsywjc,CGZL.ZSBH,zszl2.zsywjc as CGVendor,clzl.ywpm,clzl.zwpm,clzl.dwbh');
            ...
            if edt1.Text<>'' then
              sql.add(' and ZSZL2.ZSYWJC like '+''''+'%'+edt1.text+'%'+'''');
  ...
  begin     
        with query2 do
         begin
            active:=false;
            sql.Clear;
            sql.Add('select zszl2.zsywjc,sum(CWYF.USACC)as USDTotal');
            sql.Add('   ,sum(case when USprice is null then CWYF.VNACC end)as VNDTotal,sum(CWYF.VNACC)as VNACC');
            sql.Add('from CWYF left join zszl on CWYF.ZSBH=zszl.zsdh');
            sql.Add('left join CWFK on CWFK.FKBH=CWYF.FKBH');
            sql.Add('left join clzl on clzl.cldh=CWYF.CLBH');
            sql.Add('left join KCRK on KCRK.rkno=CWYF.djno');
            ...
            if edit8.Text<>'' then
              sql.Add(' and cgzl.cgno like '+''''+edit8.text+'%'+'''');
            sql.Add('group by zszl2.zsywjc  order by sum(CWYF.VNACC) desc');
            //ShowMessage(SQL.text);
            active:=true;
         end;
  end;
    ...
    begin
      ...
      try
          ...
          for   i:=0   to   Q.fieldcount-1   do
            begin
                eclApp.Cells(1,i+1):=Q.fields[i].FieldName;
            end;
          ...
          while   not   Q.Eof   do
            begin
              for   i:=0   to  Q.fieldcount-1  do
              begin
                eclApp.Cells(j,i+1):=Q.Fields[i].Value;
              end;
            Q.Next;
            inc(j);
            end;
