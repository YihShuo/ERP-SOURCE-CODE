...
type
  TMaterialMonthA = class(TForm)
    ...
    procedure Modfy1Click(Sender: TObject);
...
var
  MaterialMonthA: TMaterialMonthA;

implementation

uses MaterialMonth_Print1, main1,FunUnit, MaterialMonth_PrintA1;

{$R *.dfm}
procedure TMaterialMonthA.readini();
var MyIni :Tinifile;
    AppDir:string;
begin
...
//
procedure TMaterialMonthA.ExportExcel();
...
procedure TMaterialMonthA.Button2Click(Sender: TObject);
var
...
procedure TMaterialMonthA.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TMaterialMonthA.Button3Click(Sender: TObject);
var i:integer;
begin
  MaterialMonth_PrintA:=TMaterialMonth_PrintA.create(self);
  MaterialMonth_PrintA.quickrep1.prepare;
  i:=MaterialMonth_PrintA.quickrep1.qrprinter.pagecount;
  MaterialMonth_PrintA.page1.caption:=inttostr(i);
  MaterialMonth_PrintA.quickrep1.preview;
  MaterialMonth_PrintA.free;
end;

procedure TMaterialMonthA.Button1Click(Sender: TObject);
...
begin
  ...
  begin
    ...
    begin
      ...
      sql.add('from  CWCLZL_HG_1416 CWCLZL');
      sql.add('left join CWCLCOUNT_HG_1416 CWCLCOUNT on CWCLCOUNT.KCYEAR=CWCLZL.KCYEAR and CWCLCOUNT.KCMONTH=CWCLZL.KCMONTH ');
      ...
      //20170627 add hai quan
      sql.Add('left join CLHG_1416 CLHG on CWCLZL.CLBH=CLHG.CLBH --and CWCLZL.KCYear>=''2017''');
  ...
  begin
    ...
    begin
      ...
      sql.Add('       ,CWCLZL.CNO as HGBH,CLHG.HGPM,CLHG.UnitC,CLHG.RateC');
      sql.add('from  CWCLZL_HG_1416 CWCLZL');
      sql.add('left join CWCLCOUNT_HG_1416 CWCLCOUNT on CWCLCOUNT.KCYEAR=CWCLZL.KCYEAR and CWCLCOUNT.KCMONTH=CWCLZL.KCMONTH ');
      ...
      sql.Add('left join CLHG_1416 CLHG on CWCLZL.CLBH=CLHG.CLBH and CLHG.HGBH=CWCLZL.CNO  ');
      sql.add('where CWCLZL.KCYEAR='''+CBX2.Text+'''');
      sql.add('      and CWCLZL.KCMONTH='''+CBX3.Text+'''');
      sql.Add('      and CWCLZL.CLBH like '''+edit1.Text+'%''');
      sql.add('      and CLZL.YWPM like '''+'%'+edit2.Text+'%''');
...
procedure TMaterialMonthA.FormCreate(Sender: TObject);
var
...
procedure TMaterialMonthA.Query1CalcFields(DataSet: TDataSet);
begin
    with query1 do
    begin
...
procedure TMaterialMonthA.FormDestroy(Sender: TObject);
begin
  MaterialMonthA:=nil;
end;

procedure TMaterialMonthA.CBX4Change(Sender: TObject);
begin
...
procedure TMaterialMonthA.Query2CalcFields(DataSet: TDataSet);
begin
    with query2 do
    begin
      if fieldbyname('RemQty').value<>0 then
...
procedure TMaterialMonthA.Modfy1Click(Sender: TObject);
begin
  with query2 do
  begin
    RequestLive:=true;
...
procedure TMaterialMonthA.Cancel1Click(Sender: TObject);
begin
  with query2 do
  begin
     active:=false;
...
procedure TMaterialMonthA.Save1Click(Sender: TObject);
var i:integer;
begin
  try
    query2.First;
    ...
    begin
      if ((query2.FieldByName('HGLB').AsString<>'' ) and (query2.FieldByName('HGBH').AsString<>'' )) then
      begin
        with Matsite do
        begin
          active:=false;
          sql.Clear;
          sql.Add('Update CWCLZL_HG_1416 Set HGLB='''+query2.fieldbyname('HGLB').AsString+''',CNO='''+query2.fieldbyname('HGBH').AsString+''' ');
          sql.Add('Where KCYEAR='''+query2.fieldbyname('KCYEAR').AsString+''' and KCMONTH='''+query2.fieldbyname('KCMONTH').AsString+''' and CKBH='''+query2.fieldbyname('CKBH').AsString+''' and CLBH='''+query2.fieldbyname('CLBH').AsString+''' and HGLB='''+query2.fieldbyname('HGLB').OldValue+''' and CNO='''+query2.fieldbyname('HGBH').OldValue+''' ');
          //funcobj.WriteErrorLog(sql.Text);
          execsql;
        end;
      end;
