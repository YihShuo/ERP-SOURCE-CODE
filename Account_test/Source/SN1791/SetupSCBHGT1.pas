...
type
  TSetupSCBHGT = class(TForm)
    ...
    BDAll: TBitBtn;
    PopDeliver: TPopupMenu;
    Modifi1: TMenuItem;
    Save1: TMenuItem;
    Cancel1: TMenuItem;
    UpDeliver: TUpdateSQL;
    Qry_DeliverDFL: TStringField;
    ...
    procedure Modifi1Click(Sender: TObject);
...
begin
  ...
  begin
    ...
    SQL.Add('  	    ,KCLLS.Qty,isnull(KCLLS.VNPrice_HG,0)VNPRICE,KCLLS.VNACC_HG VNACC,KCLLS_CB.LLNO LLCB,KCLLS.DFL  ');
    SQL.Add('from KCLLS   ');
    ...
    SQL.Add('         ,KCLL.LLNO,KCLL.CFMDate,KCLLS.SCBH,KCZLS.CWBH,KCLLS.CostID ,KCLLS.Qty,isnull(KCLLS.VNPrice_HG,0),KCLLS.VNACC_HG ,KCLLS_CB.LLNO,KCLLS.DFL');
...
procedure TSetupSCBHGT.Modifi1Click(Sender: TObject);
begin
  with Qry_Deliver do
  begin
    RequestLive := True;
  ...
  Modifi1.Enabled:=False;
  Cancel1.Enabled:=True;
  Save1.Enabled:=True;
end;

procedure TSetupSCBHGT.Save1Click(Sender: TObject);
begin
  try
    with Qry_Deliver do
    begin
          ...
          usModified:
            begin
              ...
              begin
                ...
                SQL.Add('SET CostID=' + Qry_Deliver.FieldByName('CostID').AsString + ' ');
                SQL.Add('WHERE LLNO=''' + Qry_Deliver.FieldByName('LLNO').AsString + ''' AND CLBH=''' + Qry_Deliver.FieldByName('CLBH').AsString + '''');
                SQL.Add('and SCBH=''' + Qry_Deliver.FieldByName('SCBH').AsString + ''' AND DFL=''' + Qry_Deliver.FieldByName('DFL').AsString + '''');
                //funcobj.WriteErrorLog(sql.Text);
                ExecSQL;
              end;
            end;
  ...
  Modifi1.Enabled:=True;
  Cancel1.Enabled:=False;
  Save1.Enabled:=False;
end;

procedure TSetupSCBHGT.Cancel1Click(Sender: TObject);
begin
  with Qry_Deliver do
  begin
    Close;
