...
uses
  ...
  GridsEh, DBGridEh, EhlibBDE, dateutils, iniFiles, Menus;

type
  TEntryCheckA = class(TForm)
    Panel1: TPanel;
    Label1: TLabel;
    Label3: TLabel;
    Label5: TLabel;
    ...
    procedure ImportToKhai1Click(Sender: TObject);
...
var
  EntryCheckA: TEntryCheckA;

implementation

uses EntryCheck_Print1, main1,fununit, EntryCheck_PrintA1,
  ImportHaiQuanRK1, ImportToKhai1;
...
procedure TEntryCheckA.readini();
var MyIni :Tinifile;
    AppDir:string;
begin
  //
...
//
procedure TEntryCheckA.Button1Click(Sender: TObject);
...
begin
  ...
  begin
    ...
    sql.Add('       KCRKS.UnitC as UnitC, KCRKS.RateC as RateC ,  KCRKS.HGPM as HQName,');
    sql.Add('       KCRKS.Declaretion,KCPK.Export,KCRKS.RKMemo,KCRK.ZSBH,CWFK.FKBH,KCRK.DOCDATE,KCRKS.CNO1,ZSZL.tybh,KCRKS.PaymentNo,KCRKS.Quantity');
    sql.add('from KCRKS_1416 KCRKS');
    sql.add('left join KCRK_1416 KCRK on KCRK.RKNO=KCRKS.RKNO');
    sql.add('left join CLZL on CLZL.CLDH=KCRKS.CLBH');
    //sql.Add('left join CLHG on CLHG.CLBH=KCRKS.CLBH and CLHG.HGBH=KCRKS.CNO ');
    //sql.Add('left join CLHG_1416 CLHG on CLHG.CLBH=KCRKS.CLBH and CLHG.HGBH=KCRKS.CNO');
    ...
    sql.add('left join KCPK_1416 KCPK on KCPK.PKNO=KCRK.RKNO ');
...
procedure TEntryCheckA.Button3Click(Sender: TObject);
var i:integer;
begin
  EntryCheck_PrintA:=TEntryCheck_PrintA.create(self);
  EntryCheck_PrintA.quickrep1.prepare;
  i:=EntryCheck_PrintA.quickrep1.qrprinter.pagecount;
  EntryCheck_PrintA.page1.caption:=inttostr(i);
  EntryCheck_PrintA.quickrep1.preview;
  EntryCheck_PrintA.free;
end;

procedure TEntryCheckA.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TEntryCheckA.ExportDefaultExcel();
var
...
procedure TEntryCheckA.ExportFormatExcel();
...
procedure TEntryCheckA.Button2Click(Sender: TObject);
begin
...
procedure TEntryCheckA.FormDestroy(Sender: TObject);
begin
  EntryCheckA:=nil;
end;

procedure TEntryCheckA.FormCreate(Sender: TObject);
var
...
procedure TEntryCheckA.CBX1Change(Sender: TObject);
begin
  CBX4.Text:=CBX1.Text;
end;

procedure TEntryCheckA.CBX2Change(Sender: TObject);
begin
  CBX5.Text:=CBX2.Text;
end;

procedure TEntryCheckA.Button4Click(Sender: TObject);
var
  eclApp,WorkBook:olevariant;
  i,j:integer;
  sDate,eDate:Tdate;
...
procedure TEntryCheckA.Import1Click(Sender: TObject);
begin
  ImportHaiQuanRK:=TImportHaiQuanRK.create(self);
  ImportHaiQuanRK.show;
end;

procedure TEntryCheckA.Modify1Click(Sender: TObject);
begin
  with query1 do
  begin
    RequestLive:=true;
...
procedure TEntryCheckA.Cancel1Click(Sender: TObject);
begin
  with query1 do
  begin
     active:=false;
...
procedure TEntryCheckA.Save1Click(Sender: TObject);
var i:integer;
begin
  try
    query1.First;
    for i:=1 to query1.RecordCount do
    begin
      if (query1.FieldByName('CNO').AsString<>'' ) then
      begin
        with Matsite do
        begin
          active:=false;
          sql.Clear;
          //sql.Add('Update KCRK_1416 Set HGLB='''+Query1.fieldbyname('HGLB').AsString+''' Where RKNO='''+Query1.fieldbyname('RKNO').AsString+''' ');
          sql.Add('Update KCRKS_1416 Set CNO='''+Query1.fieldbyname('CNO').AsString+''' ');
          sql.Add('Where RKNO='''+Query1.fieldbyname('RKNO').AsString+'''  and CLBH='''+Query1.fieldbyname('CLBH').AsString+''' and CGBH='''+Query1.fieldbyname('CGBH').AsString+''' and RKSB='''+Query1.fieldbyname('RKSB').AsString+''' ');
          execsql;
        end;
      end;
      query1.Next;
...
procedure TEntryCheckA.ImportToKhai1Click(Sender: TObject);
begin
  ImportToKhai:=TImportToKhai.create(self);
  ImportToKhai.show;
end;

procedure TEntryCheckA.GSBHCBXChange(Sender: TObject);
begin
  if GSBHCBX.Text<>'ALL' then
  begin
    with MatSite do
