...
type
  TFCDelother_C_PrintA5 = class(TForm)
    Quickrep1: TQuickRep;
    Qry_DelDet: TQuery;
    ...
    QRLabel21: TQRLabel;
    QRLabel22: TQRLabel;
    Qry_DelDetYWPM_CLHQ: TStringField;
    QRDBText6: TQRDBText;
...
var  FCDelother_C_PrintA5: TFCDelother_C_PrintA5;
  sPaperSize: string;
  sHeight: Real;

implementation

uses DeliverList1, main1, Deliver_Account_OLD_Child1;
{$R *.dfm}

procedure TFCDelother_C_PrintA5.FormDestroy(Sender: TObject);
begin
  FCDelother_C_PrintA5 := nil;
end;

procedure TFCDelother_C_PrintA5.Formclose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFCDelother_C_PrintA5.ReadIni();
var MyIni :Tinifile;
...
begin
  ...
  if (Deliver_Account_OLD_Child.DelMas.FieldByName('CKBH').AsString='VTXW') then
  begin
    NPVT:='NP_HC';
  end;
  if ((Deliver_Account_OLD_Child.DelMas.FieldByName('CKBH').AsString='VA12')) then
  begin
    NPVT:='00186';
  end;
  if ((Deliver_Account_OLD_Child.DelMas.FieldByName('CKBH').AsString='VA3')) then
  begin
    NPVT:='NP_A3';
  end;
  if ((Deliver_Account_OLD_Child.DelMas.FieldByName('CKBH').AsString='VB2')) then
  begin
    NPVT:='NP_B2';
  end;
  if ((Deliver_Account_OLD_Child.DelMas.FieldByName('CKBH').AsString='CDC')) then
  begin
    NPVT:='NP_CDC';
  end;
  if ((Deliver_Account_OLD_Child.DelMas.FieldByName('CKBH').AsString='KDC')) then
  begin
    NPVT:='NP_KDC';
  end;
  if ((Deliver_Account_OLD_Child.DelMas.FieldByName('CKBH').AsString='VB1') or (Deliver_Account_OLD_Child.DelMas.FieldByName('CKBH').AsString='VB7'))  then
  begin
    NPVT:='01460';
  end;
  ...
  begin
    ...
    SQL.Add('where KCLL.LLNO='''+Deliver_Account_OLD_Child.DelMas.fieldbyname('LLNO').AsString+''' ');
    Active:=true;
...
procedure TFCDelother_C_PrintA5.FormCreate(Sender: TObject);
var i,j:integer;
    ADate, BDate : TDateTime;
begin
  ReadIni();
  ADate := Deliver_Account_OLD_Child.DelMas.fieldbyname('USERDATE').Value;
  BDate := EncodeDate(2023, 6, 30);
  if (CompareDate(ADate,BDate)>0) and (check='TYXUAN') then
  begin
    QRCodeBitmap:=TBitmap.Create;
    UpLoadQRImg(Deliver_Account_OLD_Child.DelMas.fieldbyname('LLNO').AsString);
...
procedure TFCDelother_C_PrintA5.UpLoadQRImg(QRdata:string);
var
...
procedure TFCDelother_C_PrintA5.Qry_DelDetCalcFields(DataSet: TDataSet);
begin
  Qry_DelDetS.Close;
  Qry_DelDetS.Open;
  Qry_DelDetS.First;
...
procedure TFCDelother_C_PrintA5.DetailBand1AfterPrint(Sender: TQRCustomBand; BandPrinted: Boolean);
begin
  sHeight := sHeight + DetailBand1.size.Height + DetailBand1.Expanded;
end;

procedure TFCDelother_C_PrintA5.Quickrep1EndPage(Sender: TCustomQuickRep);
begin
