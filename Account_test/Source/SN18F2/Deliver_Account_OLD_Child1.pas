unit Deliver_Account_OLD_Child1;
...
type
  TDeliver_Account_OLD_Child = class(TForm)
    Panel1: TPanel;
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
...
var
  Deliver_Account_OLD_Child: TDeliver_Account_OLD_Child;

implementation

uses main1, FCDeliver_Print1, FCDelOther_PrintA51, FCDelMaterial_PrintA51,
     FCDelother_C_PrintA51, FCDelOther_M_PrintA51;

{$R *.dfm}
procedure TDeliver_Account_OLD_Child.ReadIni();
var MyIni :Tinifile;
    AppDir:string;
begin
  //
...
procedure TDeliver_Account_OLD_Child.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TDeliver_Account_OLD_Child.FormDestroy(Sender: TObject);
begin
   Deliver_Account_OLD_Child:=nil;
end;

procedure TDeliver_Account_OLD_Child.FormCreate(Sender: TObject);
begin
  DTP1.Date:=date-3;
  DTP2.Date:=date;
  ReadIni();
...
procedure TDeliver_Account_OLD_Child.Button1Click(Sender: TObject);
begin
  with DelMas do
  begin
    Active:=false;
    ...
    sql.add('from KCLL_1416_Child KCLL ');
    ...
    if Edit2.Text<>'' then
    sql.add('and exists (select LLNO from KCLLS_1416_Child KCLLS where KCLLS.LLNO=KCLL.LLNO and  KCLLS.HGLB like '''+Edit2.Text+'%'' Group by LLNO ) ');
    if Edit3.Text<>'' then
    sql.add('and exists (select LLNO from KCLLS_1416_Child KCLLS where KCLLS.LLNO=KCLL.LLNO and  KCLLS.CNO like '''+Edit3.Text+'%'' Group by LLNO ) ');
...
procedure TDeliver_Account_OLD_Child.Button2Click(Sender: TObject);
var
...
procedure TDeliver_Account_OLD_Child.Excel1Click(Sender: TObject);
var
      eclApp,WorkBook:olevariant;
 //     xlsFileName:string;
      i,j:integer;
...
procedure TDeliver_Account_OLD_Child.Button3Click(Sender: TObject);
begin
  ...
  begin
    with DelDet do
    begin
      FCDeliver_Print:=TFCDeliver_Print.Create(nil);
      FCDeliver_Print.quickrep1.prepare;
      FCDeliver_Print.Qpage1.caption:=inttostr(FCDeliver_Print.quickrep1.QRPrinter.pagecount);
      FCDeliver_Print.quickrep1.preview;
      FCDeliver_Print.Free;
      Active:=false;
      cachedupdates:=false;
      requestlive:=false;
      Active:=true;
...
procedure TDeliver_Account_OLD_Child.Button4Click(Sender: TObject);
begin
  chk_A5.Checked:=True;
...
procedure TDeliver_Account_OLD_Child.Btn_Print2Click(Sender: TObject);
  var SCBH: string;
begin
  SCBH:='';
  ...
  begin
    ...
    begin
      if chk_A5.Checked then
        FCDelOther_C_PrintA51.sPaperSize := 'A5'
      else if chk_A4A5.Checked then
        FCDelOther_C_PrintA51.sPaperSize := 'A4_A5'
      else if chk_A4.Checked then
        FCDelOther_C_PrintA51.sPaperSize := 'A4';
      FCDelOther_C_PrintA5 := TFCDelOther_C_PrintA5.create(nil);
      FCDelOther_C_PrintA5.QRLabel6.Caption:=ComName;
      FCDelOther_C_PrintA5.qrlbl13.Caption:=ComNameCH;
      FCDelOther_C_PrintA5.qrlbl1.Caption:='T-KH-QP010-05AERP';
      FCDelOther_C_PrintA5.Quickrep1.Prepare;
      FCDelOther_C_PrintA5.Qpage1.caption:=inttostr(FCDelOther_C_PrintA5.quickrep1.QRPrinter.pagecount);
      FCDelOther_C_PrintA5.quickrep1.preview;
      FCDelOther_C_PrintA5.Free;
    end else
    if DelMas.FieldByName('SCBH').AsString=SCBH then
    begin
      if chk_A5.Checked then
        FCDelMaterial_PrintA51.sPaperSize := 'A5'
      else if chk_A4A5.Checked then
        FCDelMaterial_PrintA51.sPaperSize := 'A4_A5'
      else if chk_A4.Checked then
        FCDelMaterial_PrintA51.sPaperSize := 'A4';
      FCDelMaterial_PrintA5:=TFCDelMaterial_PrintA5.Create(nil);
      FCDelMaterial_PrintA5.QRLabel6.Caption:=ComName;
      FCDelMaterial_PrintA5.qrlbl13.Caption:=ComNameCH;
      FCDelMaterial_PrintA5.qrlbl1.Caption:='T-KH-QP010-05AERP';
      FCDelMaterial_PrintA5.quickrep1.prepare;
      FCDelMaterial_PrintA5.Qpage1.caption:=inttostr(FCDelMaterial_PrintA5.quickrep1.QRPrinter.pagecount);
      FCDelMaterial_PrintA5.quickrep1.preview;
      FCDelMaterial_PrintA5.Free;
    end else
    begin
      with DelDet do
      begin
        if chk_A5.Checked then
          FCDelOther_PrintA51.sPaperSize := 'A5'
        else if chk_A4A5.Checked then
          FCDelOther_PrintA51.sPaperSize := 'A4_A5'
        else if chk_A4.Checked then
          FCDelOther_PrintA51.sPaperSize := 'A4';
        FCDelOther_PrintA5:=TFCDelOther_PrintA5.Create(nil);
        FCDelOther_PrintA5.QRLabel6.Caption:=ComName;
        FCDelOther_PrintA5.qrlbl13.Caption:=ComNameCH;
        FCDelOther_PrintA5.qrlbl1.Caption:='T-KH-QP010-05AERP';
        FCDelOther_PrintA5.quickrep1.prepare;
        FCDelOther_PrintA5.Qpage1.caption:=inttostr(FCDelOther_PrintA5.quickrep1.QRPrinter.pagecount);
        FCDelOther_PrintA5.quickrep1.preview;
        FCDelOther_PrintA5.Free;
        Active:=false;
        cachedupdates:=false;
        requestlive:=false;
        Active:=true;
