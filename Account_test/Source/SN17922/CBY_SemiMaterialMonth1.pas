...
begin
  ...
  begin
    try
      ...
      CKBHID:=MyIni.ReadString('Account_N1792_Develop','CKBHID','CDC');
      CKBH_SQL:=MyIni.ReadString('Account_N1792_Develop','CKBH_SQL',' in (''CDC'',''CDT'',''BYC'',''BYT'',''SRC'',''SRT'',''SDC'',''SDT'',''CBY'')');
    finally
      MyIni.Free;
    end;
  end;  
...
begin
  ...
  begin
    Active := false;
    SQL.Clear;
    SQL.Add('DELETE FROM CWKCLLMonth WHERE KCYEAR = ''' + CBX3.Text + ''' AND KCMONTH = ''' + CBX4.Text + ''' AND CKBH = ''' + CKBHID + '''  ');
    SQL.Add('INSERT INTO CWKCLLMonth (KCYEAR, KCMONTH, CKBH, DDBH, VNACC, UserID, UserDate, YN)');
    SQL.Add('SELECT ''' + CBX3.Text + ''' AS KCYEAR, ''' + CBX4.Text + ''' AS KCMONTH, ''' + CKBHID + ''' AS CKBH, DDBH, SemiVNACC AS VNACC, ''' + main.Edit1.Text + ''' AS USERID, GetDate() AS UserDate, ''1'' AS YN FROM (');
