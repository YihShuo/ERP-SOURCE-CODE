...
var
   ...
   ayear,amonth:string;
begin
  decodedate(DTP1.Date-1,Year,Month,Day);   //т畐闽计沮
  ...
  begin
    ...
    //畐计秖
    sql.add('left join (select JGZLS.ZMLB,convert(money,sum(round(convert(float,JGZLS.Qty)*convert(float,JGZLM.Qty),2))) as FJGCK from JGZLS with (nolock) ');
   ...
   // sql.add('                      and JGZL.CFMID1 <>'+''''+'NO'+'''');
    ...
    sql.add('        ) JGCK on JGCK.ZMLB=#CLZLKC2.CLDH ');
    //ゴ虫ボ计秖
