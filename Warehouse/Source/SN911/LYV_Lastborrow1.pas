...
type
  TLYV_Lastborrow = class(TForm)
    ...
    procedure DBGridh3Columns15EditButtonClick(Sender: TObject;
      var Handled: Boolean);
    procedure DBGridh3CellClick(Column: TColumnEh);
    procedure DBGridh3DrawColumnCell(Sender: TObject; const Rect: TRect;
      DataCol: Integer; Column: TColumnEh; State: TGridDrawState);
    procedure DBGridh3Columns4EditButtonClick(Sender: TObject;
      var Handled: Boolean);
  private
    DCTFilePath: string;
    ZLBH_Value: string;
...
{$R *.dfm}
procedure TLYV_Lastborrow.ReadIni();
var MyIni :Tinifile;
    AppDir:string;
begin
  DCTFilePath := '\\192.168.123.113\PhomChuyenHT\';
  AppDir:=ExtractFilePath(Application.ExeName);
  if FileExists(AppDir+'\ComName.ini')=true then
  begin
    try
      MyIni := Tinifile.Create(AppDir+'\ComName.ini');
      DCTFilePath:=MyIni.ReadString('DCT','Warehouse_N911_DCTFilePath','');
    finally
      MyIni.Free;
    end;
  end;
...
begin
  ...
  begin
    ...
    sql.add('where LTLL.SB=''L'' and isnull(LTLL.flowflag,'''') <>''X'' ');
    if Edt1.TEXT <> '' then
...
begin
  ...
  //
  query1.Close;
  query1.SQL.Text := 'Select top 1 KCYEAR,KCMonth from Lastmonth where KCYEAR = ''' + ayear + ''' AND KCMONTH=''' + amonth +
  ...
  begin
    active := false;
    sql.Clear;
    sql.add('select CKBH from LastKCCK WHERE GSBH=''' + main.Edit2.text + ''' and LB=''01'' order by CKBH');
    active := true;
  ...
  begin
    ...
    SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO  and LTRK.CKBH in (Select CKBH from LastKCCK where LB=''01'' and GSBH=''' +
    ...
    SQL.Add('left join ( ');
    SQL.Add('   select CKBH,CLBH,SIZ,QTY as LastRem from Lastmonth where CKBH in (Select CKBH from LastKCCK where LB=''01'' and GSBH=''' + main.Edit2.Text + ''' )  and KCYEAR=''' + ayear + ''' and KCMonth=''' + amonth +
    ...
    SQL.Add('   where LTLLSS.LLNO=LTLL.LLNO  and LTLL.CFMID<>''NO'' and LTLL.CKBH in (Select CKBH from LastKCCK where LB=''01'' and GSBH=''' + main.Edit2.Text + ''' )  and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between ''' +
      formatdatetime('yyyy/MM/dd', startofthemonth(Date)) + ''' and ''' + formatdatetime('yyyy/MM/dd', Date) + ''' ');
...
begin
  ...
  DBGridh3.columns[15].ButtonStyle:=cbsEllipsis;
end;
...
begin
  ...
  try
    ...
    DBGridh3.columns[15].ButtonStyle:=cbsNone;
  except
...
procedure TLYV_Lastborrow.LTLLSS_ZLBH_Value();
begin
  if DelMas.FieldByName('SFL').AsString = '1' then
    ZLBH_Value := 'LAST';
  if DelMas.FieldByName('SFL').AsString = '2' then
    ZLBH_Value := 'DELL';
  if DelMas.FieldByName('SFL').AsString = '4' then
  ZLBH_Value := 'Transfer';
end;
...
procedure TLYV_Lastborrow.btnBE6Click(Sender: TObject);
begin
  if (DelMas.FieldByName('CFMID').value <> 'NO') then
  begin
    ShowMessage('Already Comfirm.');
...
procedure TLYV_Lastborrow.DBGridh3Columns15EditButtonClick(Sender: TObject;
  var Handled: Boolean);
...
begin
  ...
  begin
    ...
    begin
      DelMas.FieldByName('file_Image').Value := ExtractFileName(OpenDialog1.FileName);
...
procedure TLYV_Lastborrow.DBGridh3CellClick(Column: TColumnEh);
begin
  if (DBGridh3.SelectedField.FieldName = 'file_Image') and (DelMas.FieldByName('file_Image').AsString <> '') and (BB5.Enabled = false) then
    ShellExecute(0, 'open', PChar(DCTFilePath + DelMas.FieldByName('file_Image').AsString), nil, nil, SW_Normal);   
end;

procedure TLYV_Lastborrow.DBGridh3DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumnEh;
  State: TGridDrawState);
begin
  DBGridh3.DefaultDrawColumnCell(Rect, DataCol, Column, State);

  if Column.FieldName = 'file_Image' then
  begin
    DBGridh3.Canvas.Font.Color := clBlue;
    DBGridh3.Canvas.Font.Style := [fsUnderline];
    DBGridh3.Canvas.FillRect(Rect);
    DBGridh3.Canvas.TextRect(Rect, Rect.Left + 2, Rect.Top + 2, Column.Field.AsString);
  end;
end;

procedure TLYV_Lastborrow.DBGridh3Columns4EditButtonClick(Sender: TObject;
  var Handled: Boolean);
begin
  LastBorrow_Dep := TLastBorrow_Dep.Create(self);
  LastBorrow_Dep.Show;
