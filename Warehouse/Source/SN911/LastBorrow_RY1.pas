...
var
  LastBorrow_RY: TLastBorrow_RY;

implementation
  uses LYV_Lastborrow1,FunUnit, main1;
{$R *.dfm}

procedure TLastBorrow_RY.FormCreate(Sender: TObject);
begin
  RYQuery.Active:=true;
end;

procedure TLastBorrow_RY.RYQueryAfterPost(DataSet: TDataSet);
begin
...
procedure TLastBorrow_RY.DBGridEh1KeyPress(Sender: TObject; var Key: Char);
begin
    if ((key=#46) and (RYQuery.RecordCount>1)) then
    RYQuery.Delete;
end;

procedure TLastBorrow_RY.DBGridEh1GetCellParams(Sender: TObject;
...
procedure TLastBorrow_RY.BB4Click(Sender: TObject);
var i,count:integer;
...
procedure TLastBorrow_RY.BB7Click(Sender: TObject);
begin
  Close;
end;

procedure TLastBorrow_RY.BB5Click(Sender: TObject);
var i,j:integer;
begin
  //
...
procedure TLastBorrow_RY.LastQueryAfterOpen(DataSet: TDataSet);
begin
...
procedure TLastBorrow_RY.RYQueryAfterScroll(DataSet: TDataSet);
var i:integer;
...
procedure TLastBorrow_RY.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
   Action:=caFree;
end;

procedure TLastBorrow_RY.FormDestroy(Sender: TObject);
begin
  LastBorrow_RY:=nil;
end;

procedure TLastBorrow_RY.Excel1Click(Sender: TObject);
var
