...
var
  Outsolereturn_CL: TOutsolereturn_CL;

implementation

uses  Outsolereturn1,FunUnit;

{$R *.dfm}

procedure TOutsolereturn_CL.Edit4KeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
    button1Click(nil);
end;

procedure TOutsolereturn_CL.Button1Click(Sender: TObject);
begin
  ...
  begin
    active:=false;
    sql.Clear;
    sql.add('select OSLLS.LLNO');
    sql.add('       ,OSLLS.CLBH');
    sql.add('       ,OSLLS.LLQTY');
    sql.add('       ,OSRKSS.QTY as RKQTY');
    sql.add('       ,CLZL.YWPM');
    sql.add('       ,CLZL.DWBH');
    Sql.add('       ,OSLL.PlanDate');
    Sql.Add('       ,OSLL.DepID');
    sql.Add('       ,OSLL.MEMO');
    sql.add('       ,BDepartment.DepName');
    sql.add('from OSLLS ');
    sql.add('left join OSLL on OSLL.LLNO=OSLLS.LLNO ');
    sql.add('left join BDepartment on BDepartment.ID=OSLL.DepID ');
    sql.add('left join CLZL on OSLLS.CLBH=CLZL.CLDH ');
    sql.add('left join (select ZLBH,CLBH,SUM(QTY) AS qty from OSRKS group by ZLBH,CLBH');
    sql.add('           ) OSRKSS on OSLLS.LLNO=OSRKSS.ZLBH and OSLLS.CLBH=OSRKSS.CLBH');
    sql.add('where OSLLS.CLBH=CLZL.CLDH');
    sql.add('      and  OSLLS.LLNO like '''+LLEdit.Text+'%'' ');
    sql.add('      and CLZL.YWPM like '+''''+'%'+OutsoleEdit.Text+'%'+'''');
    sql.add('      and OSLL.YN='+''''+'5'+''''+'');
    if DepIDEdit.Text<>'' then
    sql.add(' and  OSLL.DepID like '''+DepIDEdit.Text+'%'' ');
    if DepNMEdit.Text<>'' then
    sql.add(' and  OSLL.MEMO like '''+DepNMEdit.Text+'%'' ');
    if MemoEdit.Text<>'' then
    sql.add(' and  OSLL.SCBH like ''%'+MemoEdit.Text+'%'' ');
    if CK1.Checked=false then
    sql.add(' and OSLLS.LLQTY>IsNull(OSRKSS.QTY,0) ');
...
procedure TOutsolereturn_CL.FormDestroy(Sender: TObject);
begin
  Outsolereturn_CL:=nil;
end;

procedure TOutsolereturn_CL.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TOutsolereturn_CL.dbgrdh1GetCellParams(Sender: TObject;
...
procedure TOutsolereturn_CL.dbgrdh1DblClick(Sender: TObject);
begin
...
procedure TOutsolereturn_CL.Button2Click(Sender: TObject);
begin
  with Query2 do
  begin
    active:=false;
    ...
    sql.add(''''+OutsoleNoEdit.Text+'%'+'''');
    sql.add('and YWPM like ');
    sql.add(''''+'%'+OutsoleNMEdit.Text+'%'+'''');
...
procedure TOutsolereturn_CL.dbgrdh2DblClick(Sender: TObject);
begin
...
procedure TOutsolereturn_CL.FormCreate(Sender: TObject);
begin
  DTP1.Date:=now-1;
  DTP2.Date:=now;
end;
