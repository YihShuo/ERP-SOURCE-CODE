...
var
  KUNBorrow_RY: TKUNBorrow_RY;

implementation
  uses LYV_Lastborrow1,FunUnit, main1;
{$R *.dfm}

procedure TKUNBorrow_RY.FormCreate(Sender: TObject);
begin
  RYQuery.Active:=true;
end;

procedure TKUNBorrow_RY.RYQueryAfterPost(DataSet: TDataSet);
begin
...
procedure TKUNBorrow_RY.DBGridEh1KeyPress(Sender: TObject; var Key: Char);
begin
    if ((key=#46) and (RYQuery.RecordCount>1)) then
    RYQuery.Delete;
end;

procedure TKUNBorrow_RY.DBGridEh1GetCellParams(Sender: TObject;
...
procedure TKUNBorrow_RY.BB4Click(Sender: TObject);
var i,count:integer;
...
procedure TKUNBorrow_RY.BB7Click(Sender: TObject);
begin
  Close;
end;

procedure TKUNBorrow_RY.BB5Click(Sender: TObject);
var i,j:integer;
begin
  //
...
procedure TKUNBorrow_RY.LastQueryAfterOpen(DataSet: TDataSet);
begin
...
procedure TKUNBorrow_RY.RYQueryAfterScroll(DataSet: TDataSet);
var i:integer;
...
procedure TKUNBorrow_RY.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
   Action:=caFree;
end;

procedure TKUNBorrow_RY.FormDestroy(Sender: TObject);
begin
  KUNBorrow_RY:=nil;
end;

procedure TKUNBorrow_RY.Excel1Click(Sender: TObject);
var
