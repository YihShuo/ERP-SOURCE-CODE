...
begin
  ...
  begin
    ...
    sql.add('left join KCCK on KCCK.CKBH=LTLL.CKBH ');
    sql.add('where LTLL.SB=''L'' ');
    if CB1.Checked=false then
    sql.add('and LTLL.YN=''1''  ');
  ...
  begin
    ...
    sql.Add('left join #CLZLKC_Last on #CLZLKC_Last.CLBH=LTLLSS.CLBH and  #CLZLKC_Last.SIZ=LTLLSS.SIZ ');
    if DepIDList.Count>0 then
    begin
    sql.add('left join (');
...
begin
  ...
  begin
    ...
    sql.add('Union All ');
    sql.add('select DepID,CLBH,SIZ,QTY as LastRem from Lastmonth_Out where CKBH='''+main.Edit2.Text+''' and  KCYEAR='''+ayear+''' and KCMonth='''+amonth+''') as tmpRKTLFL ');
    ...
    sql.add('where LTRKSS.RKNO=LTRK.RKNO  and LTRK.CKBH='''+main.Edit2.Text+''' and LTRKSS.ZLBH=''LAST'' and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+''' ');
