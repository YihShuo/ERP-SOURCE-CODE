...
var
  RAPreturn_CL: TRAPreturn_CL;

implementation

uses  LYV_RAPreturn1,FunUnit;

{$R *.dfm}

procedure TRAPreturn_CL.Edit4KeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
    button1Click(nil);
end;

procedure TRAPreturn_CL.Button1Click(Sender: TObject);
begin
  ...
  begin
    ...
    sql.add('      and LTLL.YN=''5'' and LTLL.SB=''R'' ');
    sql.Add('      and LTLL.CKBH='''+LYV_RAPreturn.DelMas.fieldbyname('CKBH').AsString+''' ');
...
procedure TRAPreturn_CL.FormDestroy(Sender: TObject);
begin
  RAPreturn_CL:=nil;
end;

procedure TRAPreturn_CL.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TRAPreturn_CL.dbgrdh1GetCellParams(Sender: TObject;
...
procedure TRAPreturn_CL.dbgrdh1DblClick(Sender: TObject);
begin
...
procedure TRAPreturn_CL.Button2Click(Sender: TObject);
begin
  with Query2 do
  begin
    active:=false;
...
procedure TRAPreturn_CL.DBGrdh2DblClick(Sender: TObject);
begin
...
procedure TRAPreturn_CL.FormCreate(Sender: TObject);
begin
  DTP1.Date:=now-1;
  DTP2.Date:=now;
end;
