...
var
  LYV_RAPreturn: TLYV_RAPreturn;
  sdate, edate, NDate: Tdate;

implementation

uses main1, LYV_RAPreturn_S1,
  RAPreturn_Dep1, RAPreturn_CL1, LYV_RAPreturn_PrintA41, RAPPurchase_CL1, FunUnit;
{$R *.dfm}

procedure TLYV_RAPreturn.FormClose(Sender: TObject;
...
procedure TLYV_RAPreturn.btn2Click(Sender: TObject);
var
  y, m: word;
begin
  ...
  begin
    ...
    sql.add('where LTRK.SB=''R'' and isnull(LTRK.flowflag,'''')<>''X'' ');
...
procedure TLYV_RAPreturn.BB7Click(Sender: TObject);
begin
  close;
end;

procedure TLYV_RAPreturn.FormCreate(Sender: TObject);
var
  i: integer;
begin
...
procedure TLYV_RAPreturn.DelMasAfterOpen(DataSet: TDataSet);
begin
  BB2.Enabled := true;
  BB3.Enabled := true;
  BB4.Enabled := true;
...
procedure TLYV_RAPreturn.BB2Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive := true;
...
procedure TLYV_RAPreturn.BB3Click(Sender: TObject);
begin
...
procedure TLYV_RAPreturn.BB4Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive := true;
...
procedure TLYV_RAPreturn.BB5Click(Sender: TObject);
var
  y, m, a: string;
  i: integer;
begin
...
procedure TLYV_RAPreturn.btnBBTT1Click(Sender: TObject);
begin
  RAPreturn_Dep := TRAPreturn_Dep.Create(self);
  RAPreturn_Dep.Show;
end;

procedure TLYV_RAPreturn.BB6Click(Sender: TObject);
begin
  with DelMas do
  begin
    active := false;
...
procedure TLYV_RAPreturn.DBGridh3EditButtonClick(Sender: TObject);
begin
  RAPreturn_Dep := TRAPreturn_Dep.Create(self);
  RAPreturn_Dep.Show;
end;

procedure TLYV_RAPreturn.DelDetAfterOpen(DataSet: TDataSet);
begin
  BD2.Enabled := true;
  BD3.Enabled := true;
  BD4.Enabled := true;
...
procedure TLYV_RAPreturn.BD2Click(Sender: TObject);
begin
  if (NDate - DelMas.FieldByName('USERDATE').value) > 2 then
  begin
    ShowMessage('Date>2, can not modify.');
...
procedure TLYV_RAPreturn.BD3Click(Sender: TObject);
begin
  if (NDate - DelMas.FieldByName('USERDATE').value) > 2 then
  begin
    ShowMessage('Date>2, can not modify.');
...
function TLYV_RAPreturn.CheckIfLLNO(LLNO: string): boolean;
begin
  with Query1 do
  begin
    Active := false;
...
procedure TLYV_RAPreturn.LTRKSS_ZLBH_Value();
begin
...
procedure TLYV_RAPreturn.BD5Click(Sender: TObject);
var
  i: integer;
begin
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    sql.add(' where LastSizeR.CLBH=' + '''' + DelDet.fieldbyname('CLBH').AsString + '''  and LB=''02'' ');
                    execsql;
                    active := false;
                    close;
                  end;
...
procedure TLYV_RAPreturn.BD4Click(Sender: TObject);
begin
  if (NDate - DelMas.FieldByName('USERDATE').value) > 2 then
  begin
    ShowMessage('Date>2, can not modify.');
...
procedure TLYV_RAPreturn.BD6Click(Sender: TObject);
begin
  with DelDet do
  begin
    active := false;
...
procedure TLYV_RAPreturn.btnBBTT2Click(Sender: TObject);
begin
  //if messagedlg('Phom giay tra ve hay hoac muc hang nhap kho ?',mtinformation,[mbYes,mbNo],0)=mrYes then
  if DelMas.FieldByName('SFL').AsString = '1' then
  begin
    RAPreturn_CL := TRAPreturn_CL.create(self);
    RAPreturn_CL.DepIDEdit.Text := DelMas.FieldByName('DepID').AsString;
    RAPreturn_CL.show;
  end
  else
  begin
    RAPPurchase_CL := TRAPPurchase_CL.create(self);
    RAPPurchase_CL.show;
  end;

end;

procedure TLYV_RAPreturn.dbgrdh4EditButtonClick(Sender: TObject);
begin
  btnBBTT2Click(nil);
end;

procedure TLYV_RAPreturn.BDT5Click(Sender: TObject);
begin
  ...
  begin
    ...
    LYV_RAPreturn_PrintA4 := TLYV_RAPreturn_PrintA4.create(nil);
  LYV_RAPreturn_PrintA4.quickrep1.prepare;
  LYV_RAPreturn_PrintA4.Qpage1.caption := inttostr(LYV_RAPreturn_PrintA4.quickrep1.QRPrinter.pagecount);
  LYV_RAPreturn_PrintA4.quickrep1.preview;
  LYV_RAPreturn_PrintA4.free;
...
procedure TLYV_RAPreturn.FormDestroy(Sender: TObject);
begin
  LYV_RAPreturn := nil;
end;

procedure TLYV_RAPreturn.btnBE1Click(Sender: TObject);
begin
  if (NDate - DelMas.FieldByName('USERDATE').value) > 2 then
  begin
    ShowMessage('Date>2, can not modify.');
...
procedure TLYV_RAPreturn.btnBE5Click(Sender: TObject);
begin
...
procedure TLYV_RAPreturn.btnBE2Click(Sender: TObject);
begin
  if (NDate - DelMas.FieldByName('USERDATE').value) > 2 then
  begin
    ShowMessage('Date>2, can not modify.');
...
procedure TLYV_RAPreturn.btnBE3Click(Sender: TObject);
begin
  if (NDate - DelMas.FieldByName('USERDATE').value) > 2 then
  begin
    ShowMessage('Date>2, can not modify.');
...
procedure TLYV_RAPreturn.btnBE4Click(Sender: TObject);
var
  i: integer;
  Qty: real;
begin
...
procedure TLYV_RAPreturn.pgcPC1Changing(Sender: TObject;
...
procedure TLYV_RAPreturn.DBGridh3GetCellParams(Sender: TObject;
...
procedure TLYV_RAPreturn.dbgrdh4GetCellParams(Sender: TObject;
  Column: TColumnEh; AFont: TFont; var Background: TColor;
  State: TGridDrawState);
begin
  if DelDet.FieldByName('YN').value = '0' then
...
procedure TLYV_RAPreturn.dbgrdh6GetCellParams(Sender: TObject;
  Column: TColumnEh; AFont: TFont; var Background: TColor;
  State: TGridDrawState);
begin
...
procedure TLYV_RAPreturn.DBGrid6Columns2NotInList(Sender: TObject;
...
procedure TLYV_RAPreturn.DBGrid6Columns3NotInList(Sender: TObject;
  NewText: string; var RecheckInList: Boolean);
begin
...
procedure TLYV_RAPreturn.DBGrid6Columns4NotInList(Sender: TObject;
  NewText: string; var RecheckInList: Boolean);
begin
  showmessage('Pls select Size from list ,can not alter by manually.');
  abort;
...
procedure TLYV_RAPreturn.KCLLSSCalcFields(DataSet: TDataSet);
begin
  KCLLSS.Active := false;
  KCLLSS.Active := true;
...
procedure TLYV_RAPreturn.dbgrdh6Columns1NotInList(Sender: TObject;
  NewText: string; var RecheckInList: Boolean);
begin
  showmessage('Pls select Size from list ,can not alter by manually.');
  abort;
...
procedure TLYV_RAPreturn.BDT7Click(Sender: TObject);
begin
  if DelDet.RequestLive then
  begin
    showmessage('Pls save data first.');
  ...
  begin
    LYV_RAPreturn_S := TLYV_RAPreturn_S.create(nil);
    LYV_RAPreturn_S.quickrep1.prepare;
    LYV_RAPreturn_S.Qpage1.caption := inttostr(LYV_RAPreturn_S.quickrep1.QRPrinter.pagecount);
    LYV_RAPreturn_S.quickrep1.preview;
    LYV_RAPreturn_S.free;
  end;
  with DelMas do
  begin
    requestlive := true;
...
procedure TLYV_RAPreturn.PGCPC1Change(Sender: TObject);
begin
...
procedure TLYV_RAPreturn.BB1Click(Sender: TObject);
begin
  MasToolPanel.Visible := true;
end;

procedure TLYV_RAPreturn.DBGridh3DblClick(Sender: TObject);
begin
...
procedure TLYV_RAPreturn.btnBE6Click(Sender: TObject);
begin
  if (NDate - DelMas.FieldByName('USERDATE').value) > 2 then
  begin
    ShowMessage('Date>2, can not modify.');
