...
uses  main1,FunUnit, TLstock_Det1, TLstock_Print1, Lstock_Print_Tong1;
...
//取得時間參數
procedure TTLstock.GetDateParam();
...
begin
    ...
    begin
      Active:=false;
      SQL.Clear;
      SQL.Add('Select Siz from LastSize where LB=''05'' ');
...
procedure TTLstock.btn1Click(Sender: TObject);
...
begin
 ...
 begin
   ...
   begin
      ...
      SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO and isnull(LTRK.flowflag,'''') <>''X'' and LTRK.SFL in (''1'',''2'') and LTRK.CKBH in (Select CKBH from LastKCCK where LB=''05'' and GSBH='''+main.Edit2.Text+''') and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+formatdatetime('yyyy/MM/dd',DTP1.date)+''' ');
      if CBX1.Text <> '' then
        SQL.Add('  and LTRK.CKBH='''+CBX1.Text+''' ');
      SQL.Add('  Group by LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ ');
      SQL.Add('  Union All ');
      SQL.Add('  select CKBH,CLBH,SIZ,QTY as LastRem from Lastmonth where KCYEAR='''+ayear+''' and KCMonth='''+amonth+'''  and CKBH in (Select CKBH from LastKCCK where LB=''05'' and GSBH='''+main.Edit2.Text+''')  ');
      if CBX1.Text <> '' then
 ...
 begin
   ...
   begin
      ...
      SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO and isnull(LTRK.flowflag,'''') <>''X'' and LTRK.SFL=''2''  and LTRK.CKBH in (Select CKBH from LastKCCK where LB=''05'' and GSBH='''+main.Edit2.Text+''') and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+formatdatetime('yyyy/MM/dd',DTP1.date)+''' ');
      if CBX1.Text <> '' then
        SQL.Add(' and LTRK.CKBH='''+CBX1.Text+''' ');
      SQL.Add('  Group by LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ ');
      SQL.Add('  Union All ');
      SQL.Add('  select CKBH,CLBH,SIZ,QTY as LastRem from Lastmonth where KCYEAR='''+ayear+''' and KCMonth='''+amonth+'''  and CKBH in (Select CKBH+''#L'' from LastKCCK where LB=''05'' and GSBH='''+main.Edit2.Text+''')   ');
      if CBX1.Text <> '' then
 ...
 begin
   ...
   begin
      ...
      sql.add('Union All ');
      sql.add('select DepID,CLBH,SIZ,QTY as LastRem from Lastmonth_Out where KCYEAR='''+ayear+''' and KCMonth='''+amonth+''' and CKBH in (Select CKBH from LastKCCK where LB=''05'' and GSBH='''+main.Edit2.Text+''')  ');
      if CBX1.Text <> '' then
        SQL.Add(' and CKBH='''+CBX1.Text+''' ');
      sql.add(') as tmpRKTLFL ');
      ...
      sql.add('where LTRKSS.RKNO=LTRK.RKNO and isnull(LTRK.flowflag,'''') <>''X'' and LTRK.SFL=''1'' and LTRK.CKBH in (Select CKBH from LastKCCK where LB=''05'' and GSBH='''+main.Edit2.Text+''')   and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+formatdatetime('yyyy/MM/dd',DTP1.date)+''' ');
      if CBX1.Text <> '' then
        SQL.Add(' and LTRK.CKBH='''+CBX1.Text+''' ');
 ...
 begin
   ...
   begin
      ...
      SQL.Add('   where LTLLSS.LLNO=LTLL.LLNO and isnull(LTLL.flowflag,'''') <>''X'' and LTLL.CFMID<>''NO'' and LTLL.SFL in (''2'') and LTLL.CKBH in (Select CKBH from LastKCCK where LB=''05'' and GSBH='''+main.Edit2.Text+''') ');
...
procedure TTLstock.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TTLstock.FormDestroy(Sender: TObject);
begin
 TLSTOCK:=nil;
end;

procedure TTLstock.S1Click(Sender: TObject);
begin
  TLSTOCK_Det:=TTLSTOCK_Det.create(self);
  TLSTOCK_Det.show;
end;
//Last Size Range
procedure TTLstock.Last_Size_Range_Init();
...
begin
  ...
  Qtemp1.Active:=false;
  Qtemp1.SQL.Clear;
  Qtemp1.SQL.Add('Select CLBH from LastSizeR where LB=''05'' Group by CLBH Order by CLBH ');
  Qtemp1.Active:=true;
  //
  ...
  begin
    ...
    begin
      Qtemp1.Active:=false;
      Qtemp1.SQL.Clear;
      Qtemp1.SQL.Add('Select SIZ from LastSizeR where CLBH='''+LastSizeR[i].CLBH+''' and LB=''05'' Order by SIZ ');
      Qtemp1.Active:=true;
...
//
procedure TTLstock.FormCreate(Sender: TObject);
var NDate:TDate;
begin
  with Qtemp1 do
  begin
    ...
    sql.add('where GSBH='''+main.edit2.text+''' and LB=''05'' ');
...
procedure TTLstock.OutputExcel(Query:TQuery);
...
procedure TTLstock.bbt6Click(Sender: TObject);
begin
  if PageControl1.ActivePage=TabSheet1 then
    OutputExcel(Qry1);
...
procedure TTLstock.BB1Click(Sender: TObject);
begin
  panel2.Visible:=true;
end;

//檢查上個月是否結算資料
function TTLstock.CheckLastMonthEnding():boolean;
begin
...
procedure TTLstock.BB2Click(Sender: TObject);
var IsCal:boolean;
begin
  ...
  begin
      ...
      begin
        ...
        SQL.Add('  select CKBH,CLBH,SIZ,Sum(RKQty) as NowRKQty from (');
        SQL.Add('  select LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ,SUM(LTRKSS.Qty) as RKQty from LTRKSS,LTRK ');
        SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO and isnull(LTRK.flowflag,'''') <>''X'' and LTRK.CKBH='''+CBX1.Text+''' and LTRK.SFL in (''1'',''2'') and LTRK.CKBH in (Select CKBH from LastKCCK where LB=''05'' and GSBH='''+main.Edit2.Text+''')  and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
        SQL.Add('  Group by LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ ');
        SQL.Add('  Union All ');
        SQL.Add('  select CKBH,CLBH,SIZ,QTY as LastRem from Lastmonth where CKBH='''+CBX1.Text+''' and KCYEAR='''+LastYear2+''' and KCMonth='''+LastMon2+'''  and CKBH in (Select CKBH from LastKCCK where LB=''05'' and GSBH='''+main.Edit2.Text+''') ) as tmpRKTLFL ');
        SQL.Add('  group by CKBH,CLBH,SIZ ) as NowKC');
        SQL.Add('left join ( ');
        SQL.Add('   select CKBH,CLBH,SIZ,QTY as LastRem from Lastmonth where CKBH='''+CBX1.Text+''' and KCYEAR='''+LastYear2+''' and KCMonth='''+LastMon2+''' and CKBH in (Select CKBH from LastKCCK where LB=''05'' and GSBH='''+main.Edit2.Text+''') ) as LastKC on NowKC.CKBH=LastKC.CKBH and NowKC.CLBH=LastKC.CLBH and NowKC.SIZ=LastKC.SIZ');
        SQL.Add('left join ( ');
        SQL.Add('  select LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ,SUM(LTRKSS.Qty) as RKQty from LTRKSS,LTRK ');
        SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO and isnull(LTRK.flowflag,'''') <>''X'' and LTRK.CKBH='''+CBX1.Text+''' and LTRK.SFL in (''1'',''2'') and LTRK.CKBH in (Select CKBH from LastKCCK where LB=''05'' and GSBH='''+main.Edit2.Text+''') and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
        SQL.Add('  Group by LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ  ) RK on NowKC.CKBH=RK.CKBH and NowKC.CLBH=RK.CLBH and NowKC.SIZ=RK.SIZ');
        ...
        SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO and isnull(LTRK.flowflag,'''') <>''X'' and LTRK.CKBH='''+CBX1.Text+''' and LTRK.SFL=''2'' and LTRK.CKBH in (Select CKBH from LastKCCK where LB=''05'' and GSBH='''+main.Edit2.Text+''') and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
        SQL.Add('  Group by LTRKSS.CLBH,LTRKSS.SIZ ');
        SQL.Add('  Union All ');
        SQL.Add('  select CLBH,SIZ,QTY as LastRem from Lastmonth where CKBH='''+CBX1.Text+'#L'' and KCYEAR='''+LastYear2+''' and KCMonth='''+LastMon2+''' and CKBH in (Select CKBH+''#L'' from LastKCCK where LB=''05'' and GSBH='''+main.Edit2.Text+''')  ) as tmpRKTLFL ');
        SQL.Add('  group by CLBH,SIZ) as NowKC');
        SQL.Add('left join ( ');
        SQL.Add('   select LTLLSS.CLBH,LTLLSS.SIZ,SUM(LTLLSS.Qty) as LLQty from LTLLSS,LTLL ');
        SQL.Add('   where LTLLSS.LLNO=LTLL.LLNO and isnull(LTLL.flowflag,'''') <>''X'' and LTLL.CFMID<>''NO'' and LTLL.CKBH='''+CBX1.Text+''' and LTLL.SFL=''2'' and LTLL.CKBH in (Select CKBH from LastKCCK where LB=''05'' and GSBH='''+main.Edit2.Text+''') and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
       ...
       // sql.Add('  group by NowKC.CLBH,NowKC.SIZ');
        ...
        sql.add('select DepID,CLBH,SIZ,Sum(LLQty) as NowLLQty from (');
        sql.add('select LTLL.DepID,LTLLSS.CLBH,LTLLSS.SIZ,SUM(LTLLSS.Qty) as LLQty from LTLLSS,LTLL ');
        sql.add('where LTLLSS.LLNO=LTLL.LLNO and isnull(LTLL.flowflag,'''') <>''X'' and LTLL.CFMID<>''NO'' and LTLL.CKBH='''+CBX1.Text+''' and LTLL.SFL=''1'' and LTLL.CKBH in (Select CKBH from LastKCCK where LB=''05'' and GSBH='''+main.Edit2.Text+''')  and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+'''  ');
        sql.add('Group by LTLL.DepID,LTLLSS.CLBH,LTLLSS.SIZ ');
        sql.add('Union All ');
        sql.add('select DepID,CLBH,SIZ,QTY as LastRem from Lastmonth_Out where  KCYEAR='''+LastYear2+''' and KCMonth='''+LastMon2+''' and CKBH='''+CBX1.Text+''' and CKBH in (Select CKBH from LastKCCK where LB=''05'' and GSBH='''+main.Edit2.Text+''')  ) as tmpRKTLFL ');
        sql.add('group by DepID,CLBH,SIZ ) as NowKC ');
        sql.add('left join ( select LTRK.DepID,LTRKSS.CLBH,LTRKSS.SIZ,SUM(LTRKSS.Qty) as RKQty from LTRKSS,LTRK  ');
        sql.add('where LTRKSS.RKNO=LTRK.RKNO and isnull(LTRK.flowflag,'''') <>''X'' and LTRK.CKBH='''+CBX1.Text+''' and LTRK.SFL=''1''  and LTRK.CKBH in (Select CKBH from LastKCCK where LB=''05'' and GSBH='''+main.Edit2.Text+''')  and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
        sql.add('Group by LTRK.DepID,LTRKSS.CLBH,LTRKSS.SIZ )  RK on NowKC.DepID=RK.DepID and NowKC.CLBH=RK.CLBH and NowKC.SIZ=RK.SIZ ');
        sql.add('where  IsNull(NowKC.NowLLQty,0)-IsNull(RK.RKQty,0) >0 ');
        ExecSQL();
      end;
...
procedure TTLstock.DTP1Change(Sender: TObject);
begin
  //取得時間參數
  GetDateParam();
end;
//
procedure TTLstock.dbgrdh1DrawColumnCell(Sender: TObject;
...
procedure TTLstock.dbgrdh2DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumnEh;
  State: TGridDrawState);
var i:integer;
begin
...
procedure TTLstock.dbgrdh4DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumnEh;
  State: TGridDrawState);
var i:integer;
begin
...
procedure TTLstock.RB1Click(Sender: TObject);
begin
  DBGridInit();
end;

procedure TTLstock.RB2Click(Sender: TObject);
begin
  DBGridInit();
end;

procedure TTLstock.RB3Click(Sender: TObject);
begin
  DBGridInit();
end;

procedure TTLstock.bbt7Click(Sender: TObject);
begin
  with Qtemp1 do
  begin
    active:=false;
    sql.Clear;
    sql.add('select CKBH,Memo from LastKCCK WHERE GSBH='''+main.Edit2.text+''' and LB =''05'' and CKBH ='''+CBX1.Text+''' order by CKBH');
    active:=true;
  end;
  TLstock_Print:=TTLstock_Print.Create(self);
  TLstock_Print.quickrep1.prepare;
  TLstock_Print.QRLabel39.Caption:=Qtemp1.FieldByName('CKBH').AsString+'-'+Qtemp1.FieldByName('Memo').AsString;
  TLstock_Print.QRLabel35.Caption:=main.Edit2.text;
  TLstock_Print.PageN.caption:=inttostr(TLstock_Print.quickrep1.QRPrinter.pagecount);
  TLstock_Print.quickrep1.preview;
  TLstock_Print.free;
end;



procedure TTLstock.BitBtn1Click(Sender: TObject);
begin
  with Qtemp1 do
  begin
    active:=false;
  ...
  Lstock_Print_Tong:=TLstock_Print_Tong.Create(self);
  Lstock_Print_Tong.quickrep1.prepare;
  Lstock_Print_Tong.QRLabel39.Caption:=Qtemp1.FieldByName('CKBH').AsString+'-'+Qtemp1.FieldByName('Memo').AsString;
  Lstock_Print_Tong.QRLabel35.Caption:=main.Edit2.text;
  Lstock_Print_Tong.PageN.caption:=inttostr(Lstock_Print_Tong.quickrep1.QRPrinter.pagecount);
  Lstock_Print_Tong.quickrep1.preview;
  Lstock_Print_Tong.free;
end;
