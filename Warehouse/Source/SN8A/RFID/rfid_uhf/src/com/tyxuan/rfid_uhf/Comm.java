...
import com.uhf.uhf.UHF5helper.InventoryBuffer;
...
import static com.tyxuan.rfid_uhf.Comm.DeviceType.supion_M_3;
import static com.tyxuan.rfid_uhf.Comm.DeviceType.supion_M_4;
import static com.tyxuan.rfid_uhf.Comm.DeviceType.supion_S_3;
import static com.tyxuan.rfid_uhf.Comm.DeviceType.supion_S_4;
import static com.tyxuan.rfid_uhf.Comm.Module.UHF005;
import static com.tyxuan.rfid_uhf.Comm.operateType.lockOpe;
import static com.tyxuan.rfid_uhf.Comm.operateType.readOpe;
import static com.tyxuan.rfid_uhf.Comm.operateType.setPower;
import static com.tyxuan.rfid_uhf.Comm.operateType.writeOpe;
import static com.uhf.uhf.UHF1.UHF001.Devaddrs;
import static com.uhf.uhf.UHF1.UHF001.er;
import static com.uhf.uhf.UHF1.UHF001.getAntPowerConf;
import static com.uhf.uhf.UHF1.UHF001.getGen2Session;
import static com.uhf.uhf.UHF1.UHF001.getRegion_Conf;
import static com.uhf.uhf.UHF1.UHF001.getgen2q;
import static com.uhf.uhf.UHF1.UHF001.setAntCheck;
import static com.uhf.uhf.UHF1.UHF001.setAntPowerConf;
import static com.uhf.uhf.UHF1.UHF001.setReg;
import static com.uhf.uhf.UHF1.UHF001.setRegion_Conf;
import static com.uhf.uhf.UHF1.UHF001.setUHF1Parameters;
import static com.uhf.uhf.UHF1.UHF001.uhf1lockOp;
import static com.uhf.uhf.UHF1.UHF001.uhf1readOp;
import static com.uhf.uhf.UHF5.RFIDOperate.mReader;
import static com.uhf.uhf.UHF5.RFIDOperate.m_curInventoryBuffer;
import static com.uhf.uhf.UHF5.RFIDOperate.m_curReaderSetting;
import static com.uhf.uhf.UHF5.RFIDOperate.setUHF5Parameters;
import static com.uhf.uhf.UHF5.RFIDOperate.uhf5lockOp;
import static com.uhf.uhf.UHF5.RFIDOperate.uhf5readOp;
import static com.uhf.uhf.UHF5.RFIDOperate.uhf5writeOp;
...
public class Comm {
    ...
    public static SPconfig spConfig;//璅∪��蔭
    public static Comm.Module moduleType = null;//璅∪�掩���
    public static AndroidWakeLock Awl;//AndroidWakeLock
    public static SoundPool soundPool; //SoundPool
    public static String COM = "/dev/ttyHSL0";//銝脣�
    public static int Baudrate = 115200;//瘜Ｙ���
    ...
    public static int btTarget = 0;//摮�芋撘� 0:A,1:B
    public static com.uhf.uhf.UHF5.RFIDOperate rfidOperate;//UHF005 rfidOperate
    ...
    public static int tagListSize = 0;//������倌��葵�
    public static int ReadCnt = 0;//瘥活霂餃��葵�
    ...
    // writeToCtrlFile("0"); //���MCU嚗SAM�
    // writeToCtrlFile("1"); //���MCU��葡�
    // writeToCtrlFile("2"); //���擃�FID��葡�
    // writeToCtrlFile("3"); //���擃�FID��葡�
    // writeToCtrlFile("4"); //蝵桅�LM300_EN
    // writeToCtrlFile("5"); //蝵桐�LM300_EN
    // writeToCtrlFile("6"); //蝵桅�LM300_BUZ
    // writeToCtrlFile("7"); //蝵桐�LM300_BUZ
    public static final String CTRL_FILE = "/sys/devices/platform/psam/psam_state";
    private static void writeToScannerCtrlFile(String data) {
        try {
    ...
    private static void switchSerialPin(String whichPin) {
    ...
    public static boolean powerUp() {
        try {
            if (dt==DeviceType.supion_S_3||dt==DeviceType.supion_M_3||dt==DeviceType.supion_M_4)
                ...
                switchSerialPin(new String("uart3"));//銝����
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
    ...
    public static boolean powerDown() {
        try {
            if (dt==DeviceType.supion_S_3||dt==DeviceType.supion_M_3||dt==DeviceType.supion_M_4)
                writeToScannerCtrlFile("3");
            else if(dt==DeviceType.supion_S_4) {
                switchSerialPin(new String("disable"));//銝����
                switchSerialPin(new String("uart2"));//��憭���
            }
            return true;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static void Connect(){
        ...
        if (UHF001.connect()) {
            ...
            if (moduleType != null) {
                b1.putString("Msg",moduleType.toString() + "模塊打開成功");
            ...
            UHF005connect(new Comm.SerialPortCallback() {
                ...
                public void onReceive(String strVersion, int i) {
                    ...
                    if (!strVersion.equals("0")) {
                        moduleType = Comm.Module.UHF005;
                        b2.putString("Msg", moduleType.toString() + "模塊打開成功");
                        m2.setData(b2);
    ...
    public static void readTag(int ant, int tagBank, String opCount, String startAdd, int datatype) {
        ...
        try {
            if (Comm.moduleType == Module.UHF001) {
                ...
                //weston test
                //Log.e("RFID", "2@read Tag val:"+val);
                if (val != "") {
                	//Log.e("RFID", "3@read Tag val:"+val);
                    b.putString("Err", val);
                }
            }
        } catch (Exception e) {
    ...
    public static void writeTag(int ant, int tagBank, String opCount, String startAdd, int datatype, String strWriteData) {
        ...
        try {
            if (moduleType == Module.UHF001) {
                Boolean writeSuc = UHF001.uhf1writeOp(datatype, ant, tagBank, opCount, startAdd, strWriteData);
                if (writeSuc)
    ...
    public static boolean UHF005connect(Comm.SerialPortCallback callback) {
        try {
            try {
                ...
                rfidOperate.onCreate(context, "30");//30���蕭嚙踝蕭
    ...
    //buf:str  len:str嚙踝��嚙篁exbuf��奕嚙踝蕭豯佗蕭嚙賢���
