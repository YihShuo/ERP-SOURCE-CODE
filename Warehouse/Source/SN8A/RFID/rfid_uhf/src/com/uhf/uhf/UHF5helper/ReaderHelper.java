...
import com.uhf.uhf.UHF5helper.ISO180006BOperateTagBuffer.ISO180006BOperateTagMap;
import com.uhf.uhf.UHF5helper.InventoryBuffer.InventoryTagMap;
import com.uhf.uhf.UHF5helper.OperateTagBuffer.OperateTagMap;
...
import static com.uhf.uhf.UHF5Base.HEAD.HEAD;
...
public class ReaderHelper {
	...
	/**
	 * ���Context嚙踝蕭
	 * @param context		霈曄蔭Context
	 * @throws Exception	敶ontext銝箇征�隡���秤
	 */
	...
	/**
	 * 餈�elper銝剖撅���粉��撣桀蝐颯��
	 * @return				餈�elper銝剖撅���粉��撣桀蝐�
	 * @throws Exception	敶elper銝剖撅���粉��撣桀蝐颱蛹蝛箸隡���秤
	 */
	 ...
	 * 嚙賣�log嚙踝蕭
	 * @param strLog	log靽⊥蹓�
	 * @param type		log蝑漣(0x10:甇�蝖�, 0x11:��秤)
	 */
	 ...
	 * ISO18000-6B ��倌����
	 * @param btCmd							�隞斤掩���(�鈭��掩����)
	 * @param curOperateTagISO18000Buffer	敶��倌�����
	 */
	...
	/**
	 * 霈曄蔭撟嗉��elper銝剖撅���粉���蝐鳴蕭
	 * @param in			颲瘚�
	 * @param out			颲瘚�
	 * @return				helper銝剖撅���粉���蝐�
	 * @throws Exception	敶n��ut銝箇征�隡���秤
	 */
	 ...
	 * 餈�elper銝剖撅���粉���蝐颯��
	 * @return				helper銝剖撅���粉���蝐�
	 * @throws Exception	敶elper銝剖撅���粉���蝐颱蛹蝛箸隡���秤
	 */
	...
	private void processSet(MessageTran msgTran) {
		...
		String strLog = strCmd + "����蕭��������� " + strErrorCode;
	...
	private void processSetWorkAntenna(MessageTran msgTran) {
    	...
    	byte []btAryData = msgTran.getAryData();
		String strErrorCode = "";
        String strCmd = "霈曄蔭撌乩�予蝥踵���,敶�極雿予蝥�: 憭拍瑪" + (m_curReaderSetting.btWorkAntenna + 1);
        ...
        String strLog = strCmd + "憭梯揖嚗仃韐亙���� " + strErrorCode;
        writeLog(strLog, ERROR.FAIL);
	...
	private void processReadTag(MessageTran msgTran) {
		...
		if (btAryData.length == 0x01) {
			...
			//weston test
			String allStr=StringTool.byteArrayToString(btAryData, 1, nLen);
			//Log.e("RFID", "nDataLen:"+Integer.toString(nDataLen)+" nEpcLen:"+Integer.toString(nEpcLen)+" allStr:"+allStr);
			//Log.e("RFID", "nLen:"+Integer.toString(nLen)+" nEpcLen:"+Integer.toString(nEpcLen)+" btAryData[2]:"+btAryData[2]);
			//Log.e("RFID", "strEPC:"+strEPC+" strPC:"+strPC+" strData:"+strData);
	...
	/**
	 * processWriteTag ��蕭processLockTag 擗�蕭��蕭�嚙踝蕭
	 * @param msgTran	����蕭嚙�
	 */
	...
	/**
	 * processKillTag ��蕭processLockTag 擗�蕭��蕭�嚙踝蕭
	 * @param msgTran	����蕭嚙�
	 */
	...
	private void processGetAccessEpcMatch(MessageTran msgTran) {
		...
		if (btAryData.length == 0x01) {
			...
			if (btAryData[0] == 0x00) {
				...
				strErrorCode = "嚙質貔蕭謕坐";
			}
		}

		String strLog = strCmd + "�璇舀��隞��蕭嚙踐漕嚙� " + strErrorCode;
	...
	private void processRealTimeInventory(MessageTran msgTran) {
		...
		if (btAryData.length == 0x01) {		////�甈∟���蛹��秤蝏��
			strErrorCode = ERROR.format(btAryData[0]);
			String strLog = strCmd + "憭梯揖嚗仃韐亙���� " + strErrorCode;
			...
			runLoopInventroy();
		} else if (btAryData.length == 0x07) {	//�甈∟��迤撣貊���
			...
			//weston test
			//Log.e("RFID", "Inventory nLength:"+Integer.toString(nLength)+" strEPC:"+strEPC);
	...
	private void processFastSwitchInventory(MessageTran msgTran) {
		...
		if (btAryData.length == 0x01) {		//���嚙踐�嚙踐�坐��嚙�
			strErrorCode = ERROR.format(btAryData[0]);
			String strLog = strCmd + "�璇舀��隞��蕭嚙踐漕嚙� " + strErrorCode;
			...
			runLoopFastSwitch();
		} else if (btAryData.length == 0x02) {		//���擗嚙賡��嚙賢�蕭���蕭���秤��嚙�
			strErrorCode = ERROR.format(btAryData[1]);
			String strLog = strCmd + "�璇舀��隞��蕭嚙踐漕嚙� " + strErrorCode + "--" + "���" + ((btAryData[0] & 0xFF) + 1);

			writeLog(strLog, ERROR.FAIL);
		} else if (btAryData.length == 0x07) {		//�����蕭���嚙�
	...
	/**
	 * processCustomizedSessionTargetInventory ��蕭processRealTimeInventory 擗�蕭��蕭�嚙踝蕭
	 * @param msgTran	����蕭�嚙賣嚙�
	 */
	...
	private void processISO180006BInventory(MessageTran msgTran) {
		...
		if (btAryData.length == 0x01) {
			...
			//��竣嚙賡豲蕭嚙踝�蕭謅��嚙踝蕭�嚙賣謢塚蕭��竣嚙賡�嚙賢���蕭謕蕭嚙踝�蕭謢塚蕭嚙賢�蕭嚙踝蕭
	...
	private void processISO180006BWriteTag(MessageTran msgTran) {
		...
		if (btAryData.length == 0x01) {
			...
			refreshISO180006B(btCmd, m_curOperateTagISO18000Buffer);
			String strLog = strCmd + ": " + "嚙踐�蕭嚙踐�" + (btAryData[1] & 0xFF) + "�謅蕭";
	...
	private void processISO180006BLockTag(MessageTran msgTran) {
		...
		if (btAryData.length == 0x01) {
			...
			switch (btAryData[1] & 0xFF) {
				case 0x00:
					strLog = strCmd + ": " + "嚙踐�蕭嚙踐撒嚙�";
					break;
				case 0xFE:
					strLog = strCmd + ": " + "��謢�嚙踐撒嚙踝蕭��蕭";
					break;
				case 0xFF:
					strLog = strCmd + ": " + "嚙踐�蕭嚙踐撒嚙�";
					break;
	...
	/**
	 * processGetAndResetInventoryBuffer 嚙踝蕭processGetInventoryBuffer 擗�蕭��蕭�嚙踝蕭
	 * @param msgTran	����蕭�嚙賣嚙�
	 */
	...
	private void processGetInventoryBufferTagCount(MessageTran msgTran) {
		...
		if (btAryData.length == 0x02) {
			...
			refreshInventory(btCmd, m_curInventoryBuffer);
			String strLog = strCmd + "��蕭 "+ String.valueOf(m_curInventoryBuffer.nTagCount);
			writeLog(strLog, ERROR.FAIL);
			return;
		} else if (btAryData.length == 0x01) {
			strErrorCode = ERROR.format(btAryData[0]);
	...
	private void runLoopInventroy() {
		...
		if ( m_curInventoryBuffer.nIndexAntenna < m_curInventoryBuffer.lAntenna.size() - 1 || m_curInventoryBuffer.nCommond == 0) {
			if (m_curInventoryBuffer.nCommond == 0) {
				...
				if (m_curInventoryBuffer.bLoopInventoryReal) {
					...
					if (m_curInventoryBuffer.bLoopCustomizedSession) {		//嚙質嚙賡�ession嚙踝�nventoried Flag
		...
		} else if (m_curInventoryBuffer.bLoopInventory || m_curInventoryBuffer.bLoopInventoryReal) {		//嚙賤嚙踝蕭������蕭謢塚蕭
