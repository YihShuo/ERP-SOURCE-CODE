...
begin
  //
  if ((INVEdit.Text='') and (MemoEdit.Text='')) then
  begin
    Showmessage('Xin nhap INVoice hoac Memo ');
    Exit;
  end;
  //
  with query1 do
...
procedure TEntryMatRFIDCode_CT.FormCreate(Sender: TObject);
begin
  INVEdit.Text:=EntryMatRFIDCode.KCRKRF.fieldByName('DOCNO').AsString;
  MemoEdit.Text:=EntryMatRFIDCode.KCRKRF.fieldByName('MEMO').AsString;
end;
