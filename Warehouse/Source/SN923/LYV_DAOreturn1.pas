...
var
  LYV_DAOreturn: TLYV_DAOreturn;
  sdate, edate, NDate: Tdate;

implementation

uses main1, LYV_DAOreturn_S1,
  DAOreturn_Dep1, DAOreturn_CL1, LYV_DAOreturn_PrintA41, DAOPurchase_CL1, FunUnit;
{$R *.dfm}

procedure TLYV_DAOreturn.FormClose(Sender: TObject;
...
procedure TLYV_DAOreturn.btn2Click(Sender: TObject);
var
  y, m: word;
begin
  ...
  begin
    ...
    sql.add('where LTRK.SB=''D'' and LTRK.GSBH=''' + main.edit2.text + ''' and isnull(LTRK.flowflag,'''')<>''X'' ');
    sql.Add('   AND LTRK.GSBH='''+main.Edit2.Text+''' ');
    if Edt1.TEXT <> '' then
...
procedure TLYV_DAOreturn.BB7Click(Sender: TObject);
begin
  close;
end;

procedure TLYV_DAOreturn.FormCreate(Sender: TObject);
var
  i: integer;
begin
...
procedure TLYV_DAOreturn.DelMasAfterOpen(DataSet: TDataSet);
begin
  BB2.Enabled := true;
  BB3.Enabled := true;
  BB4.Enabled := true;
...
procedure TLYV_DAOreturn.BB2Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive := true;
...
procedure TLYV_DAOreturn.BB3Click(Sender: TObject);
begin
...
procedure TLYV_DAOreturn.BB4Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive := true;
...
procedure TLYV_DAOreturn.BB5Click(Sender: TObject);
var
  y, m, a: string;
  i: integer;
begin
...
procedure TLYV_DAOreturn.btnBBTT1Click(Sender: TObject);
begin
  DAOreturn_Dep := TDAOreturn_Dep.Create(self);
  DAOreturn_Dep.Show;
end;

procedure TLYV_DAOreturn.BB6Click(Sender: TObject);
begin
  with DelMas do
  begin
    active := false;
...
procedure TLYV_DAOreturn.DBGridh3EditButtonClick(Sender: TObject);
begin
  DAOreturn_Dep := TDAOreturn_Dep.Create(self);
  DAOreturn_Dep.Show;
end;

procedure TLYV_DAOreturn.DelDetAfterOpen(DataSet: TDataSet);
begin
  BD2.Enabled := true;
  BD3.Enabled := true;
  BD4.Enabled := true;
...
procedure TLYV_DAOreturn.BD2Click(Sender: TObject);
begin
...
procedure TLYV_DAOreturn.BD3Click(Sender: TObject);
begin
  if (NDate - DelMas.FieldByName('USERDATE').value) > 2 then
  begin
    ShowMessage('Date>2, can not modify.');
...
function TLYV_DAOreturn.CheckIfLLNO(LLNO: string): boolean;
begin
  with Query1 do
  begin
    Active := false;
...
procedure TLYV_DAOreturn.LTRKSS_ZLBH_Value();
begin
...
procedure TLYV_DAOreturn.BD5Click(Sender: TObject);
var
  i: integer;
begin
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    sql.add(' where LastSizeR.CLBH=' + '''' + DelDet.fieldbyname('CLBH').AsString + '''  and LB=''03'' ');
                    execsql;
                    active := false;
                    close;
                  end;
...
procedure TLYV_DAOreturn.BD4Click(Sender: TObject);
begin
  if (NDate - DelMas.FieldByName('USERDATE').value) > 2 then
  begin
    ShowMessage('Date>2, can not modify.');
...
procedure TLYV_DAOreturn.BD6Click(Sender: TObject);
begin
  with DelDet do
  begin
    active := false;
...
procedure TLYV_DAOreturn.btnBBTT2Click(Sender: TObject);
begin
  //if messagedlg('Phom giay tra ve hay hoac muc hang nhap kho ?',mtinformation,[mbYes,mbNo],0)=mrYes then
  if DelMas.FieldByName('SFL').AsString = '1' then
  begin
    DAOreturn_CL := TDAOreturn_CL.create(self);
    DAOreturn_CL.DepIDEdit.Text := DelMas.FieldByName('DepID').AsString;
    DAOreturn_CL.show;
  end
  else
  begin
    DAOPurchase_CL := TDAOPurchase_CL.create(self);
    DAOPurchase_CL.show;
  end;

end;

procedure TLYV_DAOreturn.dbgrdh4EditButtonClick(Sender: TObject);
begin
  btnBBTT2Click(nil);
end;

procedure TLYV_DAOreturn.BDT5Click(Sender: TObject);
begin
  ...
  begin
    ...
    LYV_DAOreturn_PrintA4 := TLYV_DAOreturn_PrintA4.create(nil);
  LYV_DAOreturn_PrintA4.quickrep1.prepare;
  LYV_DAOreturn_PrintA4.Qpage1.caption := inttostr(LYV_DAOreturn_PrintA4.quickrep1.QRPrinter.pagecount);
  LYV_DAOreturn_PrintA4.quickrep1.preview;
  LYV_DAOreturn_PrintA4.free;
...
procedure TLYV_DAOreturn.FormDestroy(Sender: TObject);
begin
  LYV_DAOreturn := nil;
end;

procedure TLYV_DAOreturn.btnBE1Click(Sender: TObject);
begin
  if (NDate - DelMas.FieldByName('USERDATE').value) > 2 then
  begin
    ShowMessage('Date>2, can not modify.');
...
procedure TLYV_DAOreturn.btnBE5Click(Sender: TObject);
begin
...
procedure TLYV_DAOreturn.btnBE2Click(Sender: TObject);
begin
  if (NDate - DelMas.FieldByName('USERDATE').value) > 2 then
  begin
    ShowMessage('Date>2, can not modify.');
...
procedure TLYV_DAOreturn.btnBE3Click(Sender: TObject);
begin
  if (NDate - DelMas.FieldByName('USERDATE').value) > 2 then
  begin
    ShowMessage('Date>2, can not modify.');
...
procedure TLYV_DAOreturn.btnBE4Click(Sender: TObject);
var
  i: integer;
  Qty: real;
begin
...
procedure TLYV_DAOreturn.pgcPC1Changing(Sender: TObject;
...
procedure TLYV_DAOreturn.DBGridh3GetCellParams(Sender: TObject;
...
procedure TLYV_DAOreturn.dbgrdh4GetCellParams(Sender: TObject;
  Column: TColumnEh; AFont: TFont; var Background: TColor;
  State: TGridDrawState);
begin
  if DelDet.FieldByName('YN').value = '0' then
...
procedure TLYV_DAOreturn.dbgrdh6GetCellParams(Sender: TObject;
  Column: TColumnEh; AFont: TFont; var Background: TColor;
  State: TGridDrawState);
begin
...
procedure TLYV_DAOreturn.DBGrid6Columns2NotInList(Sender: TObject;
...
procedure TLYV_DAOreturn.DBGrid6Columns3NotInList(Sender: TObject;
  NewText: string; var RecheckInList: Boolean);
begin
...
procedure TLYV_DAOreturn.DBGrid6Columns4NotInList(Sender: TObject;
  NewText: string; var RecheckInList: Boolean);
begin
  showmessage('Pls select Size from list ,can not alter by manually.');
  abort;
...
procedure TLYV_DAOreturn.KCLLSSCalcFields(DataSet: TDataSet);
begin
  KCLLSS.Active := false;
  KCLLSS.Active := true;
...
procedure TLYV_DAOreturn.dbgrdh6Columns1NotInList(Sender: TObject;
  NewText: string; var RecheckInList: Boolean);
begin
  showmessage('Pls select Size from list ,can not alter by manually.');
  abort;
...
procedure TLYV_DAOreturn.BDT7Click(Sender: TObject);
begin
  if DelDet.RequestLive then
  begin
    showmessage('Pls save data first.');
  ...
  begin
    LYV_DAOreturn_S := TLYV_DAOreturn_S.create(nil);
    LYV_DAOreturn_S.quickrep1.prepare;
    LYV_DAOreturn_S.Qpage1.caption := inttostr(LYV_DAOreturn_S.quickrep1.QRPrinter.pagecount);
    LYV_DAOreturn_S.quickrep1.preview;
    LYV_DAOreturn_S.free;
  end;
  with DelMas do
  begin
    requestlive := true;
...
procedure TLYV_DAOreturn.PGCPC1Change(Sender: TObject);
begin
...
procedure TLYV_DAOreturn.BB1Click(Sender: TObject);
begin
  MasToolPanel.Visible := true;
end;

procedure TLYV_DAOreturn.DBGridh3DblClick(Sender: TObject);
begin
...
procedure TLYV_DAOreturn.btnBE6Click(Sender: TObject);
begin
  if (NDate - DelMas.FieldByName('USERDATE').value) > 2 then
  begin
    ShowMessage('Date>2, can not modify.');
...
procedure TLYV_DAOreturn.bbt6Click(Sender: TObject);
var
...
procedure TLYV_DAOreturn.BDT6Click(Sender: TObject);
var
  eclApp, WorkBook: olevariant;
  i, j: integer;
begin
