...
var
  DAOreturn_CL: TDAOreturn_CL;

implementation

uses  LYV_DAOreturn1,FunUnit;

{$R *.dfm}

procedure TDAOreturn_CL.Edit4KeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
    button1Click(nil);
end;

procedure TDAOreturn_CL.Button1Click(Sender: TObject);
begin
  ...
  begin
    ...
    sql.add('      and LTLL.YN=''5'' and LTLL.SB=''D'' ');
    sql.Add('      and LTLL.CKBH='''+LYV_DAOreturn.DelMas.fieldbyname('CKBH').AsString+''' ');
...
procedure TDAOreturn_CL.FormDestroy(Sender: TObject);
begin
  DAOreturn_CL:=nil;
end;

procedure TDAOreturn_CL.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TDAOreturn_CL.dbgrdh1GetCellParams(Sender: TObject;
...
procedure TDAOreturn_CL.dbgrdh1DblClick(Sender: TObject);
begin
...
procedure TDAOreturn_CL.Button2Click(Sender: TObject);
begin
  with Query2 do
  begin
    active:=false;
...
procedure TDAOreturn_CL.DBGrdh2DblClick(Sender: TObject);
begin
...
procedure TDAOreturn_CL.FormCreate(Sender: TObject);
begin
  DTP1.Date:=now-1;
  DTP2.Date:=now;
end;
