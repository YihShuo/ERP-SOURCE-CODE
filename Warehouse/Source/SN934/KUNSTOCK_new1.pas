...
type
  TKUNSTOCK = class(TForm)
    ...
    qry1BDEDesigner105060: TFloatField;
    ...
    qry1BDEDesigner025030: TFloatField;
    ...
    qry1BDEDesigner030055: TFloatField;
    qry1BDEDesigner030060: TFloatField;
    qry1BDEDesigner030065: TFloatField;
    qry1BDEDesigner030070: TFloatField;
    qry1BDEDesigner030075: TFloatField;
    qry1BDEDesigner030085: TFloatField;
    qry1BDEDesigner030095: TFloatField;
    qry1BDEDesigner030130: TFloatField;
    qry1BDEDesigner030135: TFloatField;
    qry1BDEDesigner030150: TFloatField;
    ...
    qry1BDEDesigner045050: TFloatField;
    ...
    qry1BDEDesigner050085: TFloatField;
    qry1BDEDesigner050090: TFloatField;
    qry1BDEDesigner055060: TFloatField;
    qry1BDEDesigner055065: TFloatField;
    qry1BDEDesigner055070: TFloatField;
    qry1BDEDesigner055075: TFloatField;
    qry1BDEDesigner0500802: TFloatField;
    qry1BDEDesigner060065: TFloatField;
    qry1BDEDesigner060070: TFloatField;
    qry1BDEDesigner060080: TFloatField;
    ...
    qry1BDEDesigner075120: TFloatField;
    ...
    qry1BDEDesigner090120: TFloatField;
    ...
    qry1BDEDesigner100160: TFloatField;
    qry1BDEDesigner100180: TFloatField;
    qry1BDEDesigner1050302: TFloatField;
    ...
    qry1BDEDesigner130160: TFloatField;
...
uses  main1, KUNSTOCK_Det1, FunUnit, KUNSTOCK_New_Print1,
  KUNSTOCK_New_Print_Tong1;
...
//取得時間參數
procedure TKUNSTOCK.GetDateParam();
...
procedure TKUNSTOCK.DBGridInit();
var i:integer;
    objColumn:TColumnEh;
begin
    DS1.DataSet:=nil;
    ...
    begin
      Active:=false;
      SQL.Clear;
      SQL.Add('Select Siz from LastSize where LB=''04'' ');
...
procedure TKUNSTOCK.btn1Click(Sender: TObject);
...
begin
 ...
 begin
   ...
   begin
      ...
      SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO and isnull(LTRK.flowflag,'''') <>''X'' and LTRK.SFL in (''1'',''2'') and LTRK.CKBH in (Select CKBH from LastKCCK where LB=''04'' and GSBH='''+main.Edit2.Text+''') and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+formatdatetime('yyyy/MM/dd',DTP1.date)+''' ');
      if CBX1.Text <> '' then
        SQL.Add('  and LTRK.CKBH='''+CBX1.Text+''' ');
      SQL.Add('  Group by LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ ');
      SQL.Add('  Union All ');
      SQL.Add('  select CKBH,CLBH,SIZ,QTY as LastRem from Lastmonth where KCYEAR='''+ayear+''' and KCMonth='''+amonth+'''  and CKBH in (Select CKBH from LastKCCK where LB=''04'' and GSBH='''+main.Edit2.Text+''')  ');
      if CBX1.Text <> '' then
      ...
      //
      SQL.Add('select #CLZLKC_Last.CLBH,CLZL.YWPM, SUM(Qty) as Total,CLZL.DWBH,');
 ...
 begin
   ...
   begin
      ...
      SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO and isnull(LTRK.flowflag,'''') <>''X'' and LTRK.SFL=''2''  and LTRK.CKBH in (Select CKBH from LastKCCK where LB=''04'' and GSBH='''+main.Edit2.Text+''') and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+formatdatetime('yyyy/MM/dd',DTP1.date)+''' ');
      if CBX1.Text <> '' then
        SQL.Add(' and LTRK.CKBH='''+CBX1.Text+''' ');
      SQL.Add('  Group by LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ ');
      SQL.Add('  Union All ');
      SQL.Add('  select CKBH,CLBH,SIZ,QTY as LastRem from Lastmonth where KCYEAR='''+ayear+''' and KCMonth='''+amonth+'''  and CKBH in (Select CKBH+''#L'' from LastKCCK where LB=''04'' and GSBH='''+main.Edit2.Text+''')   ');
      if CBX1.Text <> '' then
      ...
      SQL.Add('   Group by LTLL.CKBH,LTLLSS.CLBH,LTLLSS.SIZ ) LL on NowKC.CKBH=LL.CKBH  and NowKC.CLBH=LL.CLBH and NowKC.SIZ=LL.SIZ');
      //
      SQL.Add('select #CLZLKC_LastK.CLBH,CLZL.YWPM, SUM(Qty) as Total,CLZL.DWBH,');
      for i:=0 to High(XXCC) do
      begin
        SQL.Add('   Max(case when SIZ='''+XXCC[i]+''' then Qty end) as ''['+XXCC[i]+']'', ');
      end;
 ...
 begin
   ...
   begin
      ...
      sql.add('Union All ');
      sql.add('select DepID,CLBH,SIZ,QTY as LastRem from Lastmonth_Out where KCYEAR='''+ayear+''' and KCMonth='''+amonth+''' and CKBH in (Select CKBH from LastKCCK where LB=''04'' and GSBH='''+main.Edit2.Text+''')  ');
      if CBX1.Text <> '' then
        SQL.Add(' and CKBH='''+CBX1.Text+''' ');
      sql.add(') as tmpRKTLFL ');
      ...
      sql.add('where LTRKSS.RKNO=LTRK.RKNO and isnull(LTRK.flowflag,'''') <>''X'' and LTRK.SFL=''1'' and LTRK.CKBH in (Select CKBH from LastKCCK where LB=''04'' and GSBH='''+main.Edit2.Text+''')   and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+formatdatetime('yyyy/MM/dd',DTP1.date)+''' ');
      if CBX1.Text <> '' then
        SQL.Add(' and LTRK.CKBH='''+CBX1.Text+''' ');
 ...
 begin
   ...
   begin
      ...
      SQL.Add('   where LTLLSS.LLNO=LTLL.LLNO and isnull(LTLL.flowflag,'''') <>''X'' and LTLL.CFMID<>''NO'' and LTLL.SFL in (''2'') and LTLL.CKBH in (Select CKBH from LastKCCK where LB=''04'' and GSBH='''+main.Edit2.Text+''') ');
...
procedure TKUNSTOCK.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TKUNSTOCK.FormDestroy(Sender: TObject);
begin
  KUNSTOCK:=nil;
end;

procedure TKUNSTOCK.S1Click(Sender: TObject);
begin
  KUNSTOCK_Det:=TKUNSTOCK_Det.create(self);
  KUNSTOCK_Det.show;
end;
//Last Size Range
procedure TKUNSTOCK.Last_Size_Range_Init();
...
begin
  ...
  Qtemp1.Active:=false;
  Qtemp1.SQL.Clear;
  Qtemp1.SQL.Add('Select CLBH from LastSizeR where LB=''04'' Group by CLBH Order by CLBH ');
  Qtemp1.Active:=true;
  //
  ...
  begin
    ...
    begin
      Qtemp1.Active:=false;
      Qtemp1.SQL.Clear;
      Qtemp1.SQL.Add('Select SIZ from LastSizeR where CLBH='''+LastSizeR[i].CLBH+''' and LB=''04'' Order by SIZ ');
      Qtemp1.Active:=true;
...
//
procedure TKUNSTOCK.FormCreate(Sender: TObject);
var NDate:TDate;
begin
  with Qtemp1 do
  begin
    ...
    sql.add('where GSBH='''+main.edit2.text+''' and LB=''04'' ');
...
procedure TKUNSTOCK.OutputExcel(Query:TQuery);
...
procedure TKUNSTOCK.bbt6Click(Sender: TObject);
begin
  if PageControl1.ActivePage=TabSheet1 then
    OutputExcel(Qry1);
...
procedure TKUNSTOCK.BB1Click(Sender: TObject);
begin
  panel2.Visible:=true;
end;

//檢查上個月是否結算資料
function TKUNSTOCK.CheckLastMonthEnding():boolean;
begin
...
procedure TKUNSTOCK.BB2Click(Sender: TObject);
var IsCal:boolean;
begin
  ...
  begin
      ...
      begin
        ...
        SQL.Add('  select CKBH,CLBH,SIZ,Sum(RKQty) as NowRKQty from (');
        SQL.Add('  select LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ,SUM(LTRKSS.Qty) as RKQty from LTRKSS,LTRK ');
        SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO and isnull(LTRK.flowflag,'''') <>''X'' and LTRK.CKBH='''+CBX1.Text+''' and LTRK.SFL in (''1'',''2'') and LTRK.CKBH in (Select CKBH from LastKCCK where LB=''04'' and GSBH='''+main.Edit2.Text+''')  and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
        SQL.Add('  Group by LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ ');
        SQL.Add('  Union All ');
        SQL.Add('  select CKBH,CLBH,SIZ,QTY as LastRem from Lastmonth where CKBH='''+CBX1.Text+''' and KCYEAR='''+LastYear2+''' and KCMonth='''+LastMon2+'''  and CKBH in (Select CKBH from LastKCCK where LB=''04'' and GSBH='''+main.Edit2.Text+''') ) as tmpRKTLFL ');
        SQL.Add('  group by CKBH,CLBH,SIZ ) as NowKC');
        SQL.Add('left join ( ');
        SQL.Add('   select CKBH,CLBH,SIZ,QTY as LastRem from Lastmonth where CKBH='''+CBX1.Text+''' and KCYEAR='''+LastYear2+''' and KCMonth='''+LastMon2+''' and CKBH in (Select CKBH from LastKCCK where LB=''04'' and GSBH='''+main.Edit2.Text+''') ) as LastKC on NowKC.CKBH=LastKC.CKBH and NowKC.CLBH=LastKC.CLBH and NowKC.SIZ=LastKC.SIZ');
        SQL.Add('left join ( ');
        SQL.Add('  select LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ,SUM(LTRKSS.Qty) as RKQty from LTRKSS,LTRK ');
        SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO and isnull(LTRK.flowflag,'''') <>''X'' and LTRK.CKBH='''+CBX1.Text+''' and LTRK.SFL in (''1'',''2'') and LTRK.CKBH in (Select CKBH from LastKCCK where LB=''04'' and GSBH='''+main.Edit2.Text+''') and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
        SQL.Add('  Group by LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ  ) RK on NowKC.CKBH=RK.CKBH and NowKC.CLBH=RK.CLBH and NowKC.SIZ=RK.SIZ');
        ...
        SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO and isnull(LTRK.flowflag,'''') <>''X'' and LTRK.CKBH='''+CBX1.Text+''' and LTRK.SFL=''2'' and LTRK.CKBH in (Select CKBH from LastKCCK where LB=''04'' and GSBH='''+main.Edit2.Text+''') and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
        SQL.Add('  Group by LTRKSS.CLBH,LTRKSS.SIZ ');
        SQL.Add('  Union All ');
        SQL.Add('  select CLBH,SIZ,QTY as LastRem from Lastmonth where CKBH='''+CBX1.Text+'#L'' and KCYEAR='''+LastYear2+''' and KCMonth='''+LastMon2+''' and CKBH in (Select CKBH+''#L'' from LastKCCK where LB=''04'' and GSBH='''+main.Edit2.Text+''')  ) as tmpRKTLFL ');
        SQL.Add('  group by CLBH,SIZ) as NowKC');
        SQL.Add('left join ( ');
        SQL.Add('   select LTLLSS.CLBH,LTLLSS.SIZ,SUM(LTLLSS.Qty) as LLQty from LTLLSS,LTLL ');
        SQL.Add('   where LTLLSS.LLNO=LTLL.LLNO and isnull(LTLL.flowflag,'''') <>''X'' and LTLL.CFMID<>''NO'' and LTLL.CKBH='''+CBX1.Text+''' and LTLL.SFL=''2'' and LTLL.CKBH in (Select CKBH from LastKCCK where LB=''04'' and GSBH='''+main.Edit2.Text+''') and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
       ...
       // sql.Add('  group by NowKC.CLBH,NowKC.SIZ');
        ...
        sql.add('select DepID,CLBH,SIZ,Sum(LLQty) as NowLLQty from (');
        sql.add('select LTLL.DepID,LTLLSS.CLBH,LTLLSS.SIZ,SUM(LTLLSS.Qty) as LLQty from LTLLSS,LTLL ');
        sql.add('where LTLLSS.LLNO=LTLL.LLNO and isnull(LTLL.flowflag,'''') <>''X'' and LTLL.CFMID<>''NO'' and LTLL.CKBH='''+CBX1.Text+''' and LTLL.SFL=''1'' and LTLL.CKBH in (Select CKBH from LastKCCK where LB=''04'' and GSBH='''+main.Edit2.Text+''')  and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+'''  ');
        sql.add('Group by LTLL.DepID,LTLLSS.CLBH,LTLLSS.SIZ ');
        sql.add('Union All ');
        sql.add('select DepID,CLBH,SIZ,QTY as LastRem from Lastmonth_Out where  KCYEAR='''+LastYear2+''' and KCMonth='''+LastMon2+''' and CKBH='''+CBX1.Text+''' and CKBH in (Select CKBH from LastKCCK where LB=''04'' and GSBH='''+main.Edit2.Text+''')  ) as tmpRKTLFL ');
        sql.add('group by DepID,CLBH,SIZ ) as NowKC ');
        sql.add('left join ( select LTRK.DepID,LTRKSS.CLBH,LTRKSS.SIZ,SUM(LTRKSS.Qty) as RKQty from LTRKSS,LTRK  ');
        sql.add('where LTRKSS.RKNO=LTRK.RKNO and isnull(LTRK.flowflag,'''') <>''X'' and LTRK.CKBH='''+CBX1.Text+''' and LTRK.SFL=''1''  and LTRK.CKBH in (Select CKBH from LastKCCK where LB=''04'' and GSBH='''+main.Edit2.Text+''')  and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
        sql.add('Group by LTRK.DepID,LTRKSS.CLBH,LTRKSS.SIZ )  RK on NowKC.DepID=RK.DepID and NowKC.CLBH=RK.CLBH and NowKC.SIZ=RK.SIZ ');
        sql.add('where  IsNull(NowKC.NowLLQty,0)-IsNull(RK.RKQty,0) >0 ');
        ExecSQL();
      end;
...
procedure TKUNSTOCK.DTP1Change(Sender: TObject);
begin
  //取得時間參數
  GetDateParam();
end;
//
procedure TKUNSTOCK.dbgrdh1DrawColumnCell(Sender: TObject;
...
procedure TKUNSTOCK.dbgrdh2DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumnEh;
  State: TGridDrawState);
var i:integer;
begin
...
procedure TKUNSTOCK.dbgrdh4DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumnEh;
  State: TGridDrawState);
var i:integer;
begin
...
procedure TKUNSTOCK.RB1Click(Sender: TObject);
begin
  DBGridInit();
end;

procedure TKUNSTOCK.RB2Click(Sender: TObject);
begin
  DBGridInit();
end;

procedure TKUNSTOCK.RB3Click(Sender: TObject);
begin
  DBGridInit();
end;

procedure TKUNSTOCK.BDT5Click(Sender: TObject);
begin
  with Qtemp1 do
  begin
    active:=false;
    sql.Clear;
    sql.add('select CKBH,Memo from LastKCCK WHERE GSBH='''+main.Edit2.text+''' and LB =''04'' and CKBH ='''+CBX1.Text+''' order by CKBH');
    active:=true;
  end;
  KUNSTOCK_New_Print:=TKUNSTOCK_New_Print.Create(self);
  KUNSTOCK_New_Print.quickrep1.prepare;
  KUNSTOCK_New_Print.QRLabel39.Caption:=Qtemp1.FieldByName('CKBH').AsString+'-'+Qtemp1.FieldByName('Memo').AsString;
  KUNSTOCK_New_Print.QRLabel35.Caption:=main.Edit2.text;
  KUNSTOCK_New_Print.PageN.caption:=inttostr(KUNSTOCK_New_Print.quickrep1.QRPrinter.pagecount);
  KUNSTOCK_New_Print.quickrep1.preview;
  KUNSTOCK_New_Print.free;
end;

procedure TKUNSTOCK.BitBtn1Click(Sender: TObject);
begin
  with Qtemp1 do
  begin
    active:=false;
  ...
  KUNSTOCK_New_Print_Tong:=TKUNSTOCK_New_Print_Tong.Create(self);
  KUNSTOCK_New_Print_Tong.quickrep1.prepare;
  KUNSTOCK_New_Print_Tong.QRLabel39.Caption:=Qtemp1.FieldByName('CKBH').AsString+'-'+Qtemp1.FieldByName('Memo').AsString;
  KUNSTOCK_New_Print_Tong.QRLabel35.Caption:=main.Edit2.text;
  KUNSTOCK_New_Print_Tong.PageN.caption:=inttostr(KUNSTOCK_New_Print_Tong.quickrep1.QRPrinter.pagecount);
  KUNSTOCK_New_Print_Tong.quickrep1.preview;
  KUNSTOCK_New_Print_Tong.free;
