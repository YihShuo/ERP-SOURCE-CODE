object EntryMatBarcode: TEntryMatBarcode
  Left = 111
  Top = 90
  Width = 1181
  Height = 548
  Caption = 'EntryMatBarcode'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  PixelsPerInch = 96
  TextHeight = 13
  object Splitter1: TSplitter
    Left = 849
    Top = 138
    Width = 5
    Height = 372
    Color = clMedGray
    ParentColor = False
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1165
    Height = 65
    Align = alTop
    TabOrder = 0
    object BB2: TBitBtn
      Left = 56
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Insert one New Record'
      Caption = 'Insert'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB3: TBitBtn
      Left = 104
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Delete one Record'
      Caption = 'Delete'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB4: TBitBtn
      Left = 152
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Modify'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 3
      Glyph.Data = {
        
        50BB555555555555575F555555555555550B5555555555555575}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB5: TBitBtn
      Left = 200
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Save Current'
      Caption = 'Save'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 4
      Glyph.Data = {
        
        5990555555555555577755555555555555555555555555555555}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB6: TBitBtn
      Left = 248
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Cancel'
      Caption = 'Cancel'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 5
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB7: TBitBtn
      Left = 296
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Exit Current Form'
      Caption = 'Exit'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 6
      Glyph.Data = {
        
        EE33337FFFFFFFFFFF7F33EEEEEEEEEEEE333377777777777773}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BB1: TBitBtn
      Left = 8
      Top = 8
      Width = 49
      Height = 49
      Caption = 'Query'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      Glyph.Data = {
        
        3000333333333377377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BBT1: TBitBtn
      Left = 368
      Top = 8
      Width = 49
      Height = 49
      Caption = 'First'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 7
      Glyph.Data = {
        
        CCC333333333333777FF33333333333333CC3333333333333773}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BBT2: TBitBtn
      Left = 416
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Insert one New Record'
      Caption = 'Prior'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 8
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BBT3: TBitBtn
      Left = 464
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Delete one Record'
      Caption = 'Next'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 9
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BBT4: TBitBtn
      Left = 512
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Last'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 10
      Glyph.Data = {
        
        33333777333333333333CC333333333333337733333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object bbt6: TBitBtn
      Left = 584
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Excel'
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 11
      Glyph.Data = {
        
        3BB33773333773333773B333333B3333333B7333333733333337}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object BitBtn1: TBitBtn
      Left = 655
      Top = 7
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'PO'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 12
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        E6E67373735D5D5D6060605C5C5C5D5D5D606060636363DBDBDB}
      Layout = blGlyphTop
    end
    object BitBtn3: TBitBtn
      Left = 720
      Top = 8
      Width = 49
      Height = 49
      Hint = 'Modify Current'
      Caption = 'Print'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 13
      OnClick = BitBtn3Click
      Glyph.Data = {
        
        33333337FFFF7733333333300000033333333337777773333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 65
    Width = 1165
    Height = 73
    Align = alTop
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    object Label2: TLabel
      Left = 200
      Top = 13
      Width = 42
      Height = 16
      Caption = 'CGNO:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label3: TLabel
      Left = 384
      Top = 41
      Width = 34
      Height = 16
      Caption = 'From:'
    end
    object Label4: TLabel
      Left = 519
      Top = 41
      Width = 20
      Height = 16
      Caption = 'To:'
    end
    object Label5: TLabel
      Left = 208
      Top = 44
      Width = 35
      Height = 16
      Caption = 'Supp:'
    end
    object Label6: TLabel
      Left = 10
      Top = 39
      Width = 43
      Height = 16
      Caption = 'MatNo:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label1: TLabel
      Left = 11
      Top = 15
      Width = 41
      Height = 16
      Caption = 'RKNO:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object CGNOEdit: TEdit
      Left = 248
      Top = 11
      Width = 113
      Height = 24
      AutoSelect = False
      CharCase = ecUpperCase
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
    object DTP1: TDateTimePicker
      Left = 422
      Top = 38
      Width = 97
      Height = 24
      Date = 42177.000000000000000000
      Format = 'yyyy/MM/dd'
      Time = 42177.000000000000000000
      TabOrder = 2
    end
    object DTP2: TDateTimePicker
      Left = 542
      Top = 38
      Width = 97
      Height = 24
      Date = 42543.000000000000000000
      Format = 'yyyy/MM/dd'
      Time = 42543.000000000000000000
      TabOrder = 3
    end
    object SuppEdit: TEdit
      Left = 248
      Top = 40
      Width = 113
      Height = 24
      CharCase = ecUpperCase
      TabOrder = 1
    end
    object Check: TCheckBox
      Left = 646
      Top = 41
      Width = 51
      Height = 17
      Caption = 'Mine'
      TabOrder = 4
    end
    object MatNoEdit: TEdit
      Left = 56
      Top = 39
      Width = 113
      Height = 24
      CharCase = ecUpperCase
      TabOrder = 5
      Text = 'C'
    end
    object BitBtn2: TBitBtn
      Left = 714
      Top = 11
      Width = 49
      Height = 49
      Caption = 'Query'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 6
      OnClick = BitBtn2Click
      Glyph.Data = {
        
        3000333333333377377733333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object RKEdit: TEdit
      Left = 56
      Top = 11
      Width = 113
      Height = 24
      CharCase = ecUpperCase
      TabOrder = 7
    end
  end
  object DBGridEh1: TDBGridEh
    Left = 0
    Top = 138
    Width = 849
    Height = 372
    Align = alLeft
    DataSource = DS1
    Flat = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    FooterColor = clWindow
    FooterFont.Charset = DEFAULT_CHARSET
    FooterFont.Color = clWindowText
    FooterFont.Height = -13
    FooterFont.Name = 'MS Sans Serif'
    FooterFont.Style = []
    FooterRowCount = 1
    ParentFont = False
    SumList.Active = True
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -13
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    UseMultiTitle = True
    OnGetCellParams = DBGridEh1GetCellParams
    Columns = <
      item
        EditButtons = <>
        FieldName = 'RKNO'
        Footer.FieldName = 'CGNO'
        Footer.ValueType = fvtCount
        Footers = <>
        ReadOnly = True
        Title.Caption = #25505#36092#21934#34399'|PO'
        Width = 85
      end
      item
        EditButtons = <>
        FieldName = 'ZSBH'
        Footers = <>
        ReadOnly = True
        Title.Caption = #24288#21830'ID|SupID'
        Width = 48
      end
      item
        EditButtons = <>
        FieldName = 'ZSJC'
        Footers = <>
        ReadOnly = True
        Title.Caption = #20379#27171#21830'|Supplier'
        Width = 86
      end
      item
        EditButtons = <>
        FieldName = 'CLBH'
        Footers = <>
        ReadOnly = True
        Title.Caption = #26448#26009#32232#34399'|MatNo'
        Width = 78
      end
      item
        EditButtons = <>
        FieldName = 'YWPM'
        Footers = <>
        ReadOnly = True
        Title.Caption = #26448#26009#21517#31281'|MatNM'
        Width = 328
      end
      item
        EditButtons = <>
        FieldName = 'Qty'
        Footers = <>
        ReadOnly = True
        Title.Caption = #20837#24235#25976#37327'|Qty'
        Width = 55
      end
      item
        Color = clYellow
        EditButtons = <>
        FieldName = 'PackQty'
        Footers = <>
        Title.Caption = #21253#35037#25976#37327'|PackQty'
        Width = 60
      end
      item
        EditButtons = <>
        FieldName = 'Article'
        Footers = <>
        Width = 105
      end
      item
        EditButtons = <>
        FieldName = 'BuyNo'
        Footers = <>
        Width = 72
      end>
  end
  object Panel3: TPanel
    Left = 854
    Top = 138
    Width = 311
    Height = 372
    Align = alClient
    Caption = 'Panel3'
    TabOrder = 3
    object DBGridEh2: TDBGridEh
      Left = 1
      Top = 49
      Width = 309
      Height = 322
      Align = alClient
      DataSource = DS2
      Flat = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      FooterColor = clWindow
      FooterFont.Charset = DEFAULT_CHARSET
      FooterFont.Color = clWindowText
      FooterFont.Height = -13
      FooterFont.Name = 'MS Sans Serif'
      FooterFont.Style = []
      FooterRowCount = 1
      ParentFont = False
      SumList.Active = True
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -13
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      UseMultiTitle = True
      OnGetCellParams = DBGridEh2GetCellParams
      Columns = <
        item
          Color = clBtnFace
          EditButtons = <>
          FieldName = 'XH'
          Footers = <>
          ReadOnly = True
          Title.Caption = #21253#35037'|Pack'
          Width = 52
        end
        item
          EditButtons = <>
          FieldName = 'Qty'
          Footer.FieldName = 'Qty'
          Footer.ValueType = fvtSum
          Footers = <>
          Title.Caption = #25976#37327'|Qty'
          Width = 63
        end
        item
          Color = clBtnFace
          EditButtons = <>
          FieldName = 'LLQty'
          Footer.FieldName = 'LLQty'
          Footer.ValueType = fvtSum
          Footers = <>
          ReadOnly = True
          Title.Caption = #38936#26009#25976#37327'|LLQty'
          Width = 60
        end>
    end
    object Panel4: TPanel
      Left = 1
      Top = 1
      Width = 309
      Height = 48
      Align = alTop
      TabOrder = 1
      object BO1: TBitBtn
        Left = 0
        Top = -1
        Width = 41
        Height = 49
        Hint = 'Insert one New Record'
        Caption = 'Insert'
        Enabled = False
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        OnClick = BO1Click
        Glyph.Data = {
          
          3000333333333333377733333333333333333333333333333333}
        Layout = blGlyphTop
        NumGlyphs = 2
      end
      object BO2: TBitBtn
        Left = 40
        Top = -1
        Width = 41
        Height = 49
        Hint = 'Delete one Record'
        Caption = 'Delete'
        Enabled = False
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        OnClick = BO2Click
        Glyph.Data = {
          
          3000333333333333377733333333333333333333333333333333}
        Layout = blGlyphTop
        NumGlyphs = 2
      end
      object BO3: TBitBtn
        Left = 80
        Top = -1
        Width = 41
        Height = 49
        Hint = 'Modify Current'
        Caption = 'Modify'
        Enabled = False
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        OnClick = BO3Click
        Glyph.Data = {
          
          50BB555555555555575F555555555555550B5555555555555575}
        Layout = blGlyphTop
        NumGlyphs = 2
      end
      object BO4: TBitBtn
        Left = 120
        Top = -1
        Width = 41
        Height = 49
        Hint = 'Save Current'
        Caption = 'Save'
        Enabled = False
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        OnClick = BO4Click
        Glyph.Data = {
          
          5990555555555555577755555555555555555555555555555555}
        Layout = blGlyphTop
        NumGlyphs = 2
      end
      object BO5: TBitBtn
        Left = 160
        Top = -1
        Width = 41
        Height = 49
        Hint = 'Cancel'
        Caption = 'Cancel'
        Enabled = False
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 4
        OnClick = BO5Click
        Glyph.Data = {
          
          3333333333333333333333333333333333333333333333333333}
        Layout = blGlyphTop
        NumGlyphs = 2
      end
      object BO6: TBitBtn
        Left = 200
        Top = -1
        Width = 49
        Height = 49
        Hint = 'Modify Current'
        Caption = 'PO'
        Enabled = False
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 5
        OnClick = BO6Click
        Glyph.Data = {
          
          E6E67373735D5D5D6060605C5C5C5D5D5D606060636363DBDBDB}
        Layout = blGlyphTop
      end
      object BO7: TBitBtn
        Left = 248
        Top = -1
        Width = 49
        Height = 49
        Hint = 'Modify Current'
        Caption = 'Print'
        Enabled = False
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 6
        OnClick = BO7Click
        Glyph.Data = {
          
          33333337FFFF7733333333300000033333333337777773333333}
        Layout = blGlyphTop
        NumGlyphs = 2
      end
    end
  end
  object KCRKS: TQuery
    CachedUpdates = True
    DatabaseName = 'DB'
    RequestLive = True
    SQL.Strings = (
      
        'select KCRKS.RKNO,KCRK.ZSBH,ZSZL.ZSJC,KCRKS.CLBH,CLZL.YWPM,Sum(K' +
        'CRKS.Qty) as Qty,100 as PackQty,Max(KCRK_IM.BQty) as BQty ,'
      
        '    cast((select B.Article + '#39','#39'from KCRKS A ,DDZL B  where B.DD' +
        'BH=A.CGBH and A.RKNO=KCRKS.RKNO group by Article for XML Path ('#39 +
        #39')) as varchar(500)) as Article,'
      
        '    cast((select B.BuyNo + '#39','#39'from KCRKS A ,DDZL B  where B.DDBH' +
        '=A.CGBH and A.RKNO=KCRKS.RKNO group by BuyNo for XML Path ('#39#39')) ' +
        'as varchar(500)) as BuyNo  '
      'from KCRKS'
      'left join KCRK on KCRK.RKNO=KCRKS.RKNO'
      'left join CLZL on CLZL.CLDH=KCRKS.CLBH'
      'left join ZSZL on KCRK.ZSBH=ZSZL.ZSDH'
      'left join ('
      '  select RKNO,CLBH,Sum(Qty) as BQty from KCRKScan'
      '  where  YN<>5 and SB=1 '
      '  Group by RKNO,CLBH'
      
        '  ) KCRK_IM on KCRK_IM.RKNO=KCRK.RKNO and  KCRK_IM.CLBH=KCRKS.CL' +
        'BH'
      'where KCRKS.RKNO like '#39'20171000002%'#39
      
        '    and convert(smalldatetime,convert(varchar,KCRK.USERDATE,111)' +
        ') between '
      '          '#39'2017/01/23'#39' and '#39'2017/10/25'#39
      'Group by KCRKS.RKNO,KCRK.ZSBH,ZSZL.ZSJC,KCRKS.CLBH,CLZL.YWPM')
    UpdateObject = UpdKCKRS
    Left = 412
    Top = 236
    object KCRKSRKNO: TStringField
      FieldName = 'RKNO'
      FixedChar = True
      Size = 11
    end
    object KCRKSZSBH: TStringField
      FieldName = 'ZSBH'
      FixedChar = True
      Size = 6
    end
    object KCRKSZSJC: TStringField
      FieldName = 'ZSJC'
      FixedChar = True
    end
    object KCRKSCLBH: TStringField
      FieldName = 'CLBH'
      FixedChar = True
      Size = 15
    end
    object KCRKSYWPM: TStringField
      FieldName = 'YWPM'
      FixedChar = True
      Size = 200
    end
    object KCRKSQty: TCurrencyField
      FieldName = 'Qty'
      DisplayFormat = '#0.00'
    end
    object KCRKSPackQty: TIntegerField
      FieldName = 'PackQty'
    end
    object KCRKSBQty: TFloatField
      FieldName = 'BQty'
    end
    object KCRKSArticle: TStringField
      FieldName = 'Article'
      FixedChar = True
      Size = 255
    end
    object KCRKSBuyNo: TStringField
      FieldName = 'BuyNo'
      FixedChar = True
      Size = 255
    end
  end
  object DS1: TDataSource
    DataSet = KCRKS
    Left = 412
    Top = 204
  end
  object UpdKCKRS: TUpdateSQL
    ModifySQL.Strings = (
      'Update'
      '   KCRKScan'
      'Set'
      '   USERID =:USERID,'
      '   USERDATE =:USERDATE,'
      '   YN =:YN'
      'where'
      '   RKNO = :OLD_RKNO  and'
      '   CLBH = :OLD_CLBH  and'
      '   XH =:OLD_XH and'
      '   SB = :OLD_SB')
    InsertSQL.Strings = (
      'Insert into KCRKScan'
      '    (RKNO , CLBH, XH, SB, Qty, USERID, USERDATE, YN)'
      'Values'
      '    (:RKNO ,:CLBH , :XH, :SB , :Qty,  :USERID, :USERDATE, :YN)')
    DeleteSQL.Strings = (
      'Delete'
      'From'
      '   KCRKScan'
      'where'
      '   RKNO = :OLD_RKNO  and'
      '   CLBH = :OLD_CLBH  and'
      '   XH =:OLD_XH and'
      '   SB = :OLD_SB')
    Left = 411
    Top = 268
  end
  object DS2: TDataSource
    DataSet = KCRKSScan
    Left = 900
    Top = 284
  end
  object KCRKSScan: TQuery
    AfterOpen = KCRKSScanAfterOpen
    DatabaseName = 'DB'
    DataSource = DS1
    SQL.Strings = (
      'select KCRKScan.*,KCRK_OM.LLQty from KCRKScan'
      'left join ('
      '  select RKNO,CLBH,XH,Sum(IsNull(Qty,0)) as LLQty from KCRKScan'
      '  where  YN<>5 and SB=2 '
      '  Group by RKNO,CLBH,XH'
      
        '  ) KCRK_OM on KCRK_OM.RKNO=KCRKScan.RKNO and  KCRK_OM.CLBH=KCRK' +
        'Scan.CLBH and KCRK_OM.XH=KCRKScan.XH'
      'where KCRKScan.RKNO =:RKNO'
      '      and KCRKScan.CLBH=:CLBH'
      '      and KCRKScan.SB=1')
    UpdateObject = UpdateKCKRScan
    Left = 900
    Top = 316
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'RKNO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'CLBH'
        ParamType = ptUnknown
      end>
    object KCRKSScanRKNO: TStringField
      FieldName = 'RKNO'
      FixedChar = True
      Size = 11
    end
    object KCRKSScanCLBH: TStringField
      FieldName = 'CLBH'
      FixedChar = True
      Size = 15
    end
    object KCRKSScanXH: TStringField
      FieldName = 'XH'
      FixedChar = True
      Size = 3
    end
    object KCRKSScanSB: TStringField
      FieldName = 'SB'
      FixedChar = True
      Size = 1
    end
    object KCRKSScanQty: TFloatField
      FieldName = 'Qty'
    end
    object KCRKSScanUSERDATE: TDateTimeField
      FieldName = 'USERDATE'
    end
    object KCRKSScanUSERID: TStringField
      FieldName = 'USERID'
      FixedChar = True
      Size = 10
    end
    object KCRKSScanYN: TStringField
      FieldName = 'YN'
      FixedChar = True
      Size = 1
    end
    object KCRKSScanLLQty: TFloatField
      FieldName = 'LLQty'
    end
  end
  object UpdateKCKRScan: TUpdateSQL
    ModifySQL.Strings = (
      'Update'
      '   KCRKScan'
      'Set'
      '   Qty=:Qty,'
      '   USERID =:USERID,'
      '   USERDATE =:USERDATE,'
      '   YN =:YN'
      'where'
      '   RKNO = :OLD_RKNO  and'
      '   CLBH = :OLD_CLBH  and'
      '   XH =:OLD_XH and'
      '   SB = :OLD_SB')
    InsertSQL.Strings = (
      'Insert into KCRKScan'
      '    (RKNO , CLBH, XH, SB, Qty, USERID, USERDATE, YN)'
      'Values'
      '    (:RKNO ,:CLBH , :XH, :SB , :Qty,  :USERID, :USERDATE, :YN)')
    DeleteSQL.Strings = (
      'Delete'
      'From'
      '   KCRKScan'
      'where'
      '   RKNO = :OLD_RKNO  and'
      '   CLBH = :OLD_CLBH  and'
      '   XH =:OLD_XH and'
      '   SB = :OLD_SB')
    Left = 899
    Top = 348
  end
  object tmpQry: TQuery
    DatabaseName = 'DB'
    Left = 380
    Top = 236
  end
end
