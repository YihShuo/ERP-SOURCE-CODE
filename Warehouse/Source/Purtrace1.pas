...
type
  TPurtrace = class(TForm)
    ...
    Query1cgcsbh: TStringField;
    Label8: TLabel;
    SuppEdit: TEdit;
    Query1cg_zsjc: TStringField;
    Query1zs_zsywjc: TStringField;
...
var wherestrt,wherestrv : string;
begin
case cb1.ItemIndex of //物料採購情況
  ...
  1:   //未採或未驗
  begin
    wherestrt:=' and CGKCUSE.Qty is null and CGTW.RKQty is null';
    wherestrv:=' and CGKCUSE.Qty is null and RKZL.RKQty is null';
  end;
  2:  //採購+存貨不足用量
  begin
    wherestrt:=' and (CGTW.RKQty > 0  or cgkcuse.qty>0) and isnull(su.sqty,0) > (isnull(CGKCUSE.Qty,0)+isnull(CGTW.RKQty,0))';
    wherestrv:=' and (RKZL.RKQty > 0  or cgkcuse.qty>0) and isnull(su.sqty,0) > (isnull(CGKCUSE.Qty,0)+isnull(RKZL.RKQty,0))';
  end;
  3:  //採購+存貨大於用量
  begin
    wherestrt:=' and isnull(su.sqty,0) <  (isnull(CGKCUSE.Qty,0)+isnull(CGTW.RKQty,0))';
    wherestrv:=' and isnull(su.sqty,0) <  (isnull(CGKCUSE.Qty,0)+isnull(RKZL.RKQty,0))';
  end;
end;  
  ...
  begin
    ...
    if  checkbox1.Checked then
      begin
        ...
        //2011/4/15 billweng edit 加入實際採購廠商 <<zlzls2.csbh,cgzl.zsbh>>
        sql.add('        ,zlzls2.csbh,'''' as cgcsbh,'''' as cg_zsjc,zlzls2_zszl.zsywjc as zs_zsywjc  ');
        sql.add('from ZLZLS2 with (nolock) ');
        sql.add('inner join DDZl  with (nolock) on DDZl.DDBH=ZLZLS2.ZLBH and DDZL.DDBH like '+''''+edit5.Text+'%'+''' ');
        if edit1.Text<>'' then
           sql.add('inner join CLZL  with (nolock) on CLZL.CLDH=ZLZLs2.CLBH and CLZL.CLDH like '+''''+edit1.Text+'%'+'''')
        else
     ...
     //                                CGHZZLS.ZLBH,CGHZZLS.CLBH,CONZL.Ex_Qty,CONZL.CON_Date,CONZL.CON_NO
        ...
        sql.add('left join zszl as zlzls2_zszl on zlzls2_zszl.zsdh=zlzls2.csbh  ');
     ...
     //edit by BillWeng 2010/3/17 算出需求總合
        sql.add('join (select zlbh,clbh,sum(clsl) as Sqty from zlzls2 ');
        sql.Add(' where  ZLZLS2.ZLBH like '+''''+edit5.Text+'%'+'''');
        sql.add(' group by zlbh,clbh) su on su.zlbh=zlzls2.zlbh and su.clbh=zlzls2.clbh');
     //edit end
        ...
        sql.add('      and CLZL.CQDH='+''''+'TW'+'''');
        //台灣材料採購情況  edit by BillWeng 2010/3/16
        sql.Add(wherestrt);
        ...
        sql.add('        ,zlzls2.csbh,zlzls2_zszl.zsywjc');
    ...
    if  checkbox2.Checked then
      begin
        ...
        //2011/4/15 billweng edit 加入實際採購廠商 <<zlzls2.csbh,cgzl.zsbh>>
        sql.add('        ,zlzls2.csbh,cgzl.zsbh as cgcsbh,cgzl_zszl.zsjc as cg_zsjc,zlzls2_zszl.zsywjc as zs_zsywjc ');
        sql.add('from ZLZLS2  with (nolock) ');
        sql.add('inner join DDZl  with (nolock) on DDZl.DDBH=ZLZLS2.ZLBH and DDZL.DDBH like '+''''+edit5.Text+'%'+''' ');
        if edit1.Text<>'' then
           sql.add('left join CLZL  with (nolock) on CLZL.CLDH=ZLZLs2.CLBH and CLZL.CLDH like '+''''+edit1.Text+'%'+'''')
        else
           sql.add('left join CLZL  with (nolock) on CLZL.CLDH=ZLZLs2.CLBH ');
        sql.add('left join KFZL  with (nolock) on KFZl.KFDH=DDZl.KHBH ');
        sql.add('left join ZLZL  with (nolock) on ZLZL.ZLBH=ZLZLS2.ZLBH');
        ...
        //2011/4/15 billweng edit 加入實際採購廠商 <<cgzl.zsbh>>
        sql.add('                  ,cgzl.zsbh ');
        sql.add('           from CGZLSS with (nolock) ');
        sql.Add('           inner join DDZL on DDZL.DDBH=CGZLSS.ZLBH and DDZL.GSBH='''+main.edit2.Text+''' and DDZL.DDBH like '+''''+edit5.Text+'%'+'''') ;
        sql.add('           left join CGZL  with (nolock) on CGZl.CGNO=CGZLSS.CGNO');
        {
        sql.add('           left join ZSZL  with (nolock) on CGZl.ZSBH=ZSZL.ZSDH  ');
        ...
        sql.add('           group by CGZLSS.ZLBH,CGZLSS.CLBH,cgzl.zsbh ) CGZL');
        sql.add('                 on CGZL.ZLBH=ZLZLS2.ZLBH and CGZL.CLBH=ZLZLS2.CLBH');
        //20150821查詢廠商使用
        sql.add('left join zszl as cgzl_zszl on cgzl_zszl.zsdh=cgzl.zsbh ');
        sql.add('left join zszl as zlzls2_zszl on zlzls2_zszl.zsdh=zlzls2.csbh ');
          //加載入庫相關的數量及日期等數據
        ...
        sql.Add('           inner join DDZL on DDZL.DDBH=KCRKS.CGBH and DDZL.GSBH='''+main.edit2.Text+''' and DDZL.DDBH like '+''''+edit5.Text+'%'+'''');
     ...
     //edit end
        ...
        if SuppEdit.Text<>'' then
        sql.add('       and cgzl_zszl.zsjc like ''%'+SuppEdit.Text+'%'' ');
        //越南材料採購情況  edit by BillWeng 2010/3/16
        sql.add(wherestrv);
        ...
        sql.add('         ,zlzls2.csbh,cgzl.zsbh,cgzl_zszl.zsjc,zlzls2_zszl.zsywjc');
      end;
