...
var
  LYV_TLborrowCFM: TLYV_TLborrowCFM;
...
procedure TLYV_TLborrowCFM.CalculateLastStock();
...
begin
  ...
  begin
    ...
    SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO  and LTRK.CKBH in (Select CKBH from LastKCCK where LB =''05'' and GSBH='''+main.Edit2.Text+''' )  and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+''' ');
    ...
    SQL.Add('left join ( ');
    SQL.Add('   select CKBH,CLBH,SIZ,QTY as LastRem from Lastmonth where CKBH in (Select CKBH from LastKCCK where LB =''05'' and GSBH='''+main.Edit2.Text+''' ) and KCYEAR='''+ayear+''' and KCMonth='''+amonth+''' ) as LastKC on NowKC.CKBH=LastKC.CKBH and NowKC.CLBH=LastKC.CLBH and NowKC.SIZ=LastKC.SIZ');
    SQL.Add('left join ( ');
    SQL.Add('  select LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ,SUM(LTRKSS.Qty) as RKQty from LTRKSS,LTRK ');
    SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO  and LTRK.CKBH in (Select CKBH from LastKCCK where LB =''05'' and GSBH='''+main.Edit2.Text+''' ) and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+''' ');
    ...
    SQL.Add('   where LTLLSS.LLNO=LTLL.LLNO  and LTLL.CFMID<>''NO'' and LTLL.CKBH in (Select CKBH from LastKCCK where LB =''05''  and GSBH='''+main.Edit2.Text+''' ) and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+''' ');
...
procedure TLYV_TLborrowCFM.btn1Click(Sender: TObject);
var i:integer;
begin
  CalculateLastStock();
  //
  ...
  begin
    ...
    sql.add('where LTLL.SB =''T'' and LTLL.CFMID=''NO'' and isnull(LTLL.flowflag,'''')<>''X'' ');
...
procedure TLYV_TLborrowCFM.FormClose(Sender: TObject;
...
procedure TLYV_TLborrowCFM.FormDestroy(Sender: TObject);
begin
  LYV_TLborrowCFM:=nil;
end;

procedure TLYV_TLborrowCFM.FormCreate(Sender: TObject);
var year,month,day:word;
    i:integer;
begin
  decodedate(Date,Year,Month,Day);
  ...
  begin
    ...
    sql.add('where LTLLSS.LLNO=LTLL.LLNO  and LTLL.CFMID<>''NO'' and LTLL.CKBH in (Select CKBH from LastKCCK where LB =''05'' and GSBH='''+main.Edit2.Text+''' ) and LTLL.SFL=''1'' and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+'''  ');
    sql.add('Group by LTLL.DepID,LTLLSS.CLBH,LTLLSS.SIZ ');
    sql.add('Union All ');
    sql.add('select DepID,CLBH,SIZ,QTY as LastRem from Lastmonth_Out where CKBH in (Select CKBH from LastKCCK where LB =''05'' and GSBH='''+main.Edit2.Text+''' ) and  KCYEAR='''+ayear+''' and KCMonth='''+amonth+''') as tmpRKTLFL ');
    ...
    sql.add('where LTRKSS.RKNO=LTRK.RKNO  and LTRK.CKBH in (Select CKBH from LastKCCK where LB =''05'' and GSBH='''+main.Edit2.Text+''' ) and LTRK.SFL=''1'' and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+''' ');
...
procedure TLYV_TLborrowCFM.btn2Click(Sender: TObject);
begin
...
procedure TLYV_TLborrowCFM.btn3Click(Sender: TObject);
begin
...
procedure TLYV_TLborrowCFM.Qtyrevise1Click(Sender: TObject);
begin
...
procedure TLYV_TLborrowCFM.btn4Click(Sender: TObject);
