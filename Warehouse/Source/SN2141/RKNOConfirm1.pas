...
procedure TRKNOConfirm.ReadIni();
  var MyIni :Tinifile;
      AppDir:string;
begin
  //
...
procedure TRKNOConfirm.Button1Click(Sender: TObject);
begin
  ...
  begin
    ...
    if CheckBox1.Checked = true then
      SQL.Add('      and IsNull(KCRK_DK_CFM.CFMID,''NO'')<>''NO'' ')
    else
      SQL.Add('      and IsNull(KCRK_DK_CFM.CFMID,''NO'')=''NO'' ');
...
procedure TRKNOConfirm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=CaFree;
end;

procedure TRKNOConfirm.FormDestroy(Sender: TObject);
begin
  RKNOConfirm:=nil;
end;

procedure TRKNOConfirm.FormCreate(Sender: TObject);
begin
  AppDir:=ExtractFilePath(Application.ExeName);
  with TmpQry do
  begin
...
procedure TRKNOConfirm.Button3Click(Sender: TObject);
begin
      ...
      begin
        ...
        begin
           ...
           begin
             ...
             SQL.Add('Begin');
             SQL.Add('  Insert into KCRK_DK_CFM (RKNO, CFMID, CFMDATE) Values ('''+ENMas.FieldByName('RKNO').AsString+''','''+main.Edit1.Text+''',GetDate())');
             SQL.Add('End Else');
             SQL.Add('Begin');
             SQL.Add('  Update KCRK_DK_CFM Set CFMID='''+main.Edit1.Text+''' ,CFMDate=GetDate() where RKNO='''+ENMas.FieldByName('RKNO').AsString+''' ');
             SQL.Add('End');
             ExecSQL();
           end;
        end;
...
procedure TRKNOConfirm.Button4Click(Sender: TObject);
begin
  ...
  try
    ...
    begin
      ...
      begin
         ...
         SQL.Add('IF NOT EXISTS (Select RKNO from KCRK_DK_CFM where RKNO='''+ENMas.FieldByName('RKNO').AsString+''')');
         SQL.Add('Begin');
         SQL.Add('  Insert into KCRK_DK_CFM (RKNO, CFMID, CFMDATE) Values ('''+ENMas.FieldByName('RKNO').AsString+''',''NO'',GetDate())');
         SQL.Add('End Else');
         SQL.Add('Begin');
         SQL.Add('  Update KCRK_DK_CFM Set CFMID=''NO'' ,CFMDate=GetDate() where RKNO='''+ENMas.FieldByName('RKNO').AsString+''' ');
         SQL.Add('End');
         ExecSQL();
      end;
      active:=false;
...
procedure TRKNOConfirm.Button5Click(Sender: TObject);
begin
  if ENMas.Active=false then Exit;
  if ENMas.RecordCount=0 then Exit;
  if ENDet.Active=false then Exit;
...
procedure TRKNOConfirm.CGDetCalcFields(DataSet: TDataSet);
begin
