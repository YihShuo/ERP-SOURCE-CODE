...
type
  TLastSTOCK = class(TForm)
    ...
    qry6: TQuery;
    Button1: TButton;
    qry1ZWPM: TStringField;
    PDF_export: TCheckBox;
    qry1RowNum: TIntegerField;
  ...
  private
    ...
    LastMon:string; //¤W­Ó¤ë(Month)
    LastYear:string;//¤W­Ó¤ë(Year)
    STLastMonDate:string; //¤W­Ó¤ëªì
    EDLastMonDate:string; //¤W­Ó¤ë©³
    //
    AppDir:String;
    LastMon2:string;//¤W¤W¤@­Ó¤ë(Month) ¤ëµ²²Ö¿n¥[Á`­pºâ
    LastYear2:string;//¤W¤W¤@­Ó¤ë(Year)
    //
    STDate:string; //·í¤ë¤ëªì
    EDDate:string; //¤µ¤Ñ
...
begin
  ...
  STDate:=FormatDateTime('YYYY/MM/DD', EncodeDate(myYear, myMonth, 1));  //¤ëªì¤é´Á
  EDDate:=FormatDateTime('YYYY/MM/DD', DTP1.Date);//°òÂ¦¤é
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          objColumn.Title.Caption:='¤Ø½X|'+'['+FieldByName('Siz').AsString+']';
          objColumn.Width:=42;
...
begin
  ...
  decodedate(DTP1.Date,Year,Month,Day);   //§ä®w¦s¬ÛÃöªº¼Æ¾Ú
 ...
 begin
   ...
   begin
      ...
      //
      SQL.Add(';WITH T AS (');
      SQL.Add('select #CLZLKC_Last.CLBH,CLZL.YWPM,CLZL.ZWPM, SUM(Qty) as Total,');
      ...
      SQL.Add('Group by #CLZLKC_Last.CLBH,CLZL.YWPM,CLZL.ZWPM,CLZL.CQDH,CLZL.DWBH ');
      ...
      SQL.Add('select CAST(ROW_NUMBER() OVER (ORDER BY CLBH) AS INT) as RowNum, * from T ');
     ...
     begin
      ...
      try
        ...
        i := 4;
          ...
          begin
          // 1) Chèn 1 dòng t?i v? trí i (d?y ch? m?c d?nh phía du?i xu?ng)
          eclApp.ActiveSheet.Rows[i].Insert(-4121); // xlShiftDown = -4121
          // 2) Copy format c?a dòng m?u (row 4) -> row i
          eclApp.ActiveSheet.Rows[4].Copy;
          eclApp.ActiveSheet.Rows[i].PasteSpecial(-4122); // xlPasteFormats = -4122

          // 3) Ghi d? li?u
          eclApp.Cells(i,1)  := qry1.FieldByName('RowNum').AsString;
          eclApp.Cells(i,2)  := qry1.FieldByName('ZWPM').AsString;
          eclApp.Cells(i,3)  := qry1.FieldByName('YWPM').AsString;
          eclApp.Cells(i,4)  := qry1.FieldByName('CLBH').AsString;
          eclApp.Cells(i,16) := qry1.FieldByName('DWBH').AsString;
          eclApp.Cells(i,12) := qry1.FieldByName('Total').Value;
          eclApp.Cells(i,10) := 'KHO PHOM';
          eclApp.Cells(i,11) := 'SINH QUAN ¥ÍºÞ';
          Inc(i);
          qry1.Next;
          end;

        eclApp.CutCopyMode := False;
