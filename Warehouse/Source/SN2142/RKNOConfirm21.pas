...
procedure TRKNOConfirm2.ReadIni();
  var MyIni :Tinifile;
      AppDir:string;
begin
  //
...
procedure TRKNOConfirm2.Button1Click(Sender: TObject);
begin
  ...
  begin
    ...
    SQL.Add('where IsNull(KCRK_DK_CFM.CFMID,''NO'')<>''NO''  and  KCRK.RKNO like '''+Edit1.Text+'%'' ');
    ...
    if CheckBox1.Checked = true then
      SQL.Add('      and IsNull(KCRK_DK_CFM.CFMID1,''NO'')<>''NO'' ')
    else
      SQL.Add('      and IsNull(KCRK_DK_CFM.CFMID1,''NO'')=''NO'' ');
...
procedure TRKNOConfirm2.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=CaFree;
end;

procedure TRKNOConfirm2.FormDestroy(Sender: TObject);
begin
  RKNOConfirm2:=nil;
end;

procedure TRKNOConfirm2.FormCreate(Sender: TObject);
begin
  AppDir:=ExtractFilePath(Application.ExeName);
  with TmpQry do
  begin
...
procedure TRKNOConfirm2.Button3Click(Sender: TObject);
begin
      ...
      begin
        ...
        begin
           ...
           begin
             ...
             SQL.Add('Begin');
             SQL.Add('  Insert into KCRK_DK_CFM (RKNO, CFMID1, CFMDATE1) Values ('''+ENMas.FieldByName('RKNO').AsString+''','''+main.Edit1.Text+''',GetDate())');
             SQL.Add('End Else');
             SQL.Add('Begin');
             SQL.Add('  Update KCRK_DK_CFM Set CFMID1='''+main.Edit1.Text+''' ,CFMDate1=GetDate() where RKNO='''+ENMas.FieldByName('RKNO').AsString+''' ');
             SQL.Add('End');
             ExecSQL();
           end;
        end;
...
procedure TRKNOConfirm2.Button4Click(Sender: TObject);
begin
  ...
  try
    ...
    begin
      ...
      begin
         ...
         SQL.Add('IF NOT EXISTS (Select RKNO from KCRK_DK_CFM where RKNO='''+ENMas.FieldByName('RKNO').AsString+''')');
         SQL.Add('Begin');
         SQL.Add('  Insert into KCRK_DK_CFM (RKNO, CFMID1, CFMDATE1) Values ('''+ENMas.FieldByName('RKNO').AsString+''',''NO'',GetDate())');
         SQL.Add('End Else');
         SQL.Add('Begin');
         SQL.Add('  Update KCRK_DK_CFM Set CFMID1=''NO'' ,CFMDate1=GetDate() where RKNO='''+ENMas.FieldByName('RKNO').AsString+''' ');
         SQL.Add('End');
         ExecSQL();
      end;
      active:=false;
...
procedure TRKNOConfirm2.Button5Click(Sender: TObject);
begin
  if ENMas.Active=false then Exit;
  if ENMas.RecordCount=0 then Exit;
  if ENDet.Active=false then Exit;
...
procedure TRKNOConfirm2.CGDetCalcFields(DataSet: TDataSet);
begin
