...
var
  TLreturn_CL: TTLreturn_CL;

implementation

uses  LYV_TLreturn1,FunUnit;

{$R *.dfm}

procedure TTLreturn_CL.Edit4KeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
    button1Click(nil);
end;

procedure TTLreturn_CL.Button1Click(Sender: TObject);
begin
  ...
  begin
    ...
    sql.add('      and LTLL.YN=''5'' and LTLL.SB=''T'' ');
    sql.Add('      and LTLL.CKBH='''+LYV_TLreturn.DelMas.fieldbyname('CKBH').AsString+''' ');
...
procedure TTLreturn_CL.FormDestroy(Sender: TObject);
begin
  TLreturn_CL:=nil;
end;

procedure TTLreturn_CL.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=cafree;
end;

procedure TTLreturn_CL.dbgrdh1GetCellParams(Sender: TObject;
...
procedure TTLreturn_CL.dbgrdh1DblClick(Sender: TObject);
begin
...
procedure TTLreturn_CL.Button2Click(Sender: TObject);
begin
  with Query2 do
  begin
    active:=false;
...
procedure TTLreturn_CL.DBGrdh2DblClick(Sender: TObject);
begin
...
procedure TTLreturn_CL.FormCreate(Sender: TObject);
begin
  DTP1.Date:=now-1;
  DTP2.Date:=now;
end;
