...
with query1 do
  begin
    active:=false;
    sql.Clear;
    sql.add('select  max(CGZLSS.ZLBH) as ZLBH,max(CGZLSS.CGNO) as CGNO, DDZL.KHPO,XXZL.Article,XXZl.XieMing,');
    sql.add('max(DDZL.DDRQ) DDRQ,max(DDZL.Shipdate) as ShipDate,Sum(CGZLSS.CLSL) as CLSL,ZSZL.ZSYWJC,KCRKS.CLBH,CLZL.YWPM,');
    sql.add('CLZL.DWBH,CLZL.YSYWSM, max(CGZL.CGDate)as CGDate,max(CGZLSS.YQDate) as YQDate,max(CGZLSS.CFMDate) as CFMDate,KCRK.USERDate,');
    //sql.Add('KCRKS.Qty,'); 給客人報表不用這個，是假數字
    sql.add('sum(CGZLSS.Qty) as Qty ,sum(CGZLSS.Qty) as CGQty ');
    sql.add('from KCRKS ');
    sql.add('left join KCRK on KCRK.RKNO=KCRKS.RKNO');
    sql.add('left join CGZLSS on CGZLSS.ZLBH=KCRKS.CGBH and CGZLSS.CLBH=KCRKS.CLBH ');
    ...
    sql.add('group by DDZL.KHPO,XXZL.Article,XXZl.XieMing,ZSZL.ZSYWJC,KCRKS.CLBH,CLZL.YWPM,');
    sql.add('CLZL.DWBH,CLZL.YSYWSM, KCRK.USERDate,KCRKS.Qty ');
    sql.add('order by KCRK.USERDate,KCRKS.CLBH,CGNO');
    active:=true;
  end;
end;
...
procedure TDeliverTraceS.Query1CalcFields(DataSet: TDataSet);
begin
