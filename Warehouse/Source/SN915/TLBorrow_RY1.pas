...
var
  TLBorrow_RY: TTLBorrow_RY;

implementation
  uses LYV_TLborrow1,FunUnit, main1;
{$R *.dfm}

procedure TTLBorrow_RY.FormCreate(Sender: TObject);
begin
  RYQuery.Active:=true;
end;

procedure TTLBorrow_RY.RYQueryAfterPost(DataSet: TDataSet);
begin
...
procedure TTLBorrow_RY.DBGridEh1KeyPress(Sender: TObject; var Key: Char);
begin
    if ((key=#46) and (RYQuery.RecordCount>1)) then
    RYQuery.Delete;
end;

procedure TTLBorrow_RY.DBGridEh1GetCellParams(Sender: TObject;
...
procedure TTLBorrow_RY.BB4Click(Sender: TObject);
var i,count:integer;
...
procedure TTLBorrow_RY.BB7Click(Sender: TObject);
begin
  Close;
end;

procedure TTLBorrow_RY.BB5Click(Sender: TObject);
var i,j:integer;
begin
  //
  ...
  begin
    ...
    begin
      //
      LYV_TLborrow.DelDet.edit;
      LYV_TLborrow.DelDet.fieldbyname('LLNO').Value:=LYV_TLborrow.DelMas.fieldbyname('LLNO').Value;
      LYV_TLborrow.DelDet.fieldbyname('CLBH').Value:=LastQuery.fieldbyname('CLDH').Value;
      LYV_TLborrow.DelDet.fieldbyname('LLQTY').Value:='0';
      LYV_TLborrow.DelDet.fieldbyname('RKQTY').Value:='0';
      LYV_TLborrow.DelDet.FieldByName('userID').Value:=main.edit1.text;
      LYV_TLborrow.DelDet.FieldByName('userdate').Value:=Ndate;
      LYV_TLborrow.DelDet.FieldByName('YN').Value:='1';
      LYV_TLborrow.upDet.apply(ukinsert);
      //
      ...
      begin
        ...
        sql.add('where LTLLSS.LLNO='+''''+LYV_TLborrow.DelMas.fieldbyname('LLNO').AsString+'''');
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              sql.add('select '''+LYV_TLborrow.DelMas.fieldbyname('LLNo').AsString+''',CLBH,''LAST'' ,SIZ,'''+LastQuery.Fields[j].AsString+''','''+LastQuery.Fields[j].AsString+''',Qty');
              sql.add('       ,getdate() '  );
            ...
            begin
              ...
              sql.Clear;
              sql.add('INSERT into LTLLSS   ');
              sql.add('Values ('''+LYV_TLborrow.DelMas.fieldbyname('LLNo').AsString+''','''+LastQuery.fieldbyname('CLDH').AsString+''',''LAST'' ,'''+LastQuery.Fields[j].FieldName+''','''+LastQuery.Fields[j].AsString+''','''+LastQuery.Fields[j].AsString+''',0 ');
              sql.add('       ,getdate() '  );
              sql.add('       ,'''+main.edit1.text+''''  );
              sql.add('       ,''1'' '  );
              sql.add('       ,''VN'')  ');
    ...
    begin
      ...
      sql.add('                  from LTLLSS ');
      sql.add('                  WHERE 1=1');
      sql.add('                        and LTLLSS.LLNo='+''''+LYV_TLborrow.DelDet.FieldByName('LLNO').Value+''''+' and LTLLSS.CLBH='+''''+LYV_TLborrow.DelDet.FieldByName('CLBH').Value+'''' );
      sql.add('                  GROUP BY LLNo,CLBH),0) ');
      sql.add(' where LTLLS.LLNo='+''''+LYV_TLborrow.DelDet.FieldByName('LLNo').Value+''''+ ' and LTLLS.CLBH='+''''+LYV_TLborrow.DelDet.FieldByName('CLBH').Value+'''' );
      execsql;
      close;
    end;
    //
    LYV_TLborrow.DelDet.active:=false;
    LYV_TLborrow.DelDet.cachedupdates:=false;
    LYV_TLborrow.DelDet.requestlive:=false;
    LYV_TLborrow.DelDet.active:=true;
    //
    Showmessage('Success');
    Close;
  end;
...
procedure TTLBorrow_RY.LastQueryAfterOpen(DataSet: TDataSet);
begin
...
procedure TTLBorrow_RY.RYQueryAfterScroll(DataSet: TDataSet);
var i:integer;
...
procedure TTLBorrow_RY.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
   Action:=caFree;
end;

procedure TTLBorrow_RY.FormDestroy(Sender: TObject);
begin
  TLBorrow_RY:=nil;
end;

procedure TTLBorrow_RY.Excel1Click(Sender: TObject);
var
