...
type
  TAccountKeyInStock = class(TForm)
    Panel1: TPanel;
    BB2: TBitBtn;
    BB3: TBitBtn;
    BB4: TBitBtn;
    ...
    procedure Confirm2Click(Sender: TObject);
...
var
  AccountKeyInStock: TAccountKeyInStock;
...
{$R *.dfm}
procedure TAccountKeyInStock.readini();
var MyIni :Tinifile;
    AppDir:string;
begin
  //
...
procedure TAccountKeyInStock.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TAccountKeyInStock.FormDestroy(Sender: TObject);
begin
  AccountKeyInStock:=nil;
end;

procedure TAccountKeyInStock.BB1Click(Sender: TObject);
begin
  Panel3.Visible:=true;
end;

procedure TAccountKeyInStock.Button1Click(Sender: TObject);
begin
  with Query2 do
  begin
    active:=false;
...
procedure TAccountKeyInStock.FormCreate(Sender: TObject);
var
...
procedure TAccountKeyInStock.BB4Click(Sender: TObject);
begin
  with query1 do
  begin
    cachedupdates:=true;
...
procedure TAccountKeyInStock.BB2Click(Sender: TObject);
begin
  with query1 do
  begin
    cachedupdates:=true;
...
procedure TAccountKeyInStock.BB5Click(Sender: TObject);
var i:integer;
begin
  try
    query1.first;
...
procedure TAccountKeyInStock.BB6Click(Sender: TObject);
begin
  with query1 do
  begin
    active:=false;
...
procedure TAccountKeyInStock.DBGridEh1EditButtonClick(Sender: TObject);
begin
  MaterialID:=TMaterialID.create(self);
  MaterialID.Label3.Caption:='N3C';
  MaterialID.show;
end;

procedure TAccountKeyInStock.Query1AfterOpen(DataSet: TDataSet);
begin
  if ((query1.FieldByName('CFMID').AsString='') or (query1.FieldByName('CFMID').AsString='NO')) then
  begin
    BB2.Enabled:=true;
    BB4.Enabled:=true;
  ...
  if ((query1.FieldByName('CFMID').AsString<>'NO') and  (query1.FieldByName('CFMID1').AsString<>'NO') and(query1.FieldByName('CFMID').AsString<>'') and (query1.FieldByName('CFMID1').AsString<>'')   )  then
  begin
    Confirm2.Enabled:=true;
    Reset2.Enabled:=false;
  end;
  if ((query1.FieldByName('CFMID2').AsString<>'NO') and (query1.FieldByName('CFMID2').AsString<>''))  then
  begin
    Reset1.Enabled:=false;
    Confirm2.Enabled:=false;
    Reset2.Enabled:=true;
...
procedure TAccountKeyInStock.Confirm1Click(Sender: TObject);
begin
  with query2 do
  begin
    active:=false;
...
procedure TAccountKeyInStock.Reset1Click(Sender: TObject);
begin
...
procedure TAccountKeyInStock.Confirm2Click(Sender: TObject);
begin
  with query2 do
  begin
    active:=false;
    sql.Clear;
    sql.Add(' update KCCLMONTHAC') ;
    sql.Add(' set CFMID2= '''+main.Edit1.Text+''' ,CFMDate2=getdate()');
    sql.Add(' from KCCLMONTHAC');
    sql.add(' left join KCCK on KCCK.CKBH=KCCLMONTHAC.CKBH ');
    sql.add(' where 1=1 ');
    sql.add(' and KCCLMONTHAC.KCYEAR ='''+CBX1.Text+''' ');
...
procedure TAccountKeyInStock.Reset2Click(Sender: TObject);
begin
  with query2 do
  begin
    active:=false;
