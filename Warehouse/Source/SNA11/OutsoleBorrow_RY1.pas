...
var
  OutsoleBorrow_RY: TOutsoleBorrow_RY;

implementation
  uses Outsoleborrow1,FunUnit, main1;
{$R *.dfm}

procedure TOutsoleBorrow_RY.FormCreate(Sender: TObject);
begin
  RYQuery.Active:=true;
end;

procedure TOutsoleBorrow_RY.RYQueryAfterPost(DataSet: TDataSet);
begin
  ...
  begin
    ...
    begin
      Active:=false;
      SQL.Clear;
      SQL.Add('Select DDZL.DDBH,XXZL.DDMH,CLZL.CLDH from DDZL ');
      SQL.Add('left join XXZL on DDZL.XieXing=XXZL.XieXing and DDZL.SheHao=XXZL.SheHao ');
      SQL.Add('left join CLZL on CLZL.YWPM=XXZL.DDMH and CLDH like ''VF%'' ');
      SQL.Add('where DDBH='''+RYQuery.FieldByName('DDBH').AsString+''' ');
      Active:=true;
      if RecordCount>0  then
      begin
        RYQuery.Edit;
        RYQuery.FieldByName('DDMH').AsString:=TmpQry.FieldByName('DDMH').AsString;
        RYQuery.FieldByName('CLDH').AsString:=TmpQry.FieldByName('CLDH').AsString;
        RYQuery.Post;
      end else
      begin
        RYQuery.Edit;
        RYQuery.FieldByName('DDMH').AsString:='';
...
procedure TOutsoleBorrow_RY.DBGridEh1KeyPress(Sender: TObject; var Key: Char);
begin
    if ((key=#46) and (RYQuery.RecordCount>1)) then
    RYQuery.Delete;
end;

procedure TOutsoleBorrow_RY.DBGridEh1GetCellParams(Sender: TObject;
...
procedure TOutsoleBorrow_RY.BB4Click(Sender: TObject);
var i,count:integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      SQL.Add('left join XXGJS on xxgjs.XieXing=DDZL.XieXing and xxgjs.GJLB=101 and xxgjs.XXCC=SMDDS.XXCC ');
    ...
    begin
      Active:=false;
      SQL.Clear;
      SQL.Add('select  AA.CLDH,AA.Outsole as OutSoleName');
      //
      TmpQry.First;
      ...
      begin
        SQL.Add('SELECT A.SizeLast,A.Outsole,A.CLDH,A.DDBH,Max(ISNULL(A.Qty,0)');
        ...
        SQL.Add(') Qty');
        SQL.Add('FROM( ');
        SQL.Add('  Select SMDD.YSBH,SMDDS.DDBH,XXGJS.GJCC as SizeLast,CLZL.CLDH,CLZL.YWPM as Outsole,');
        ...
        SQL.Add('  GROUP BY A.SizeLast,A.Outsole,A.DDBH,A.CLDH');
        if count<>High(SubSQL) then SQL.Add('Union all');
      end;
      SQL.Add('     )AA');
      SQL.Add('GROUP BY AA.CLDH,AA.Outsole');
      Active:=true;
    end;
    TmpQry.Active:=false;
    //
...
procedure TOutsoleBorrow_RY.BB7Click(Sender: TObject);
begin
  Close;
end;

procedure TOutsoleBorrow_RY.BB5Click(Sender: TObject);
var i,j:integer;
begin
  //
  ...
  begin
    ...
    begin
      //
      Outsoleborrow.DelDet.edit;
      Outsoleborrow.DelDet.fieldbyname('LLNO').Value:=Outsoleborrow.DelMas.fieldbyname('LLNO').Value;
      Outsoleborrow.DelDet.fieldbyname('CLBH').Value:=LastQuery.fieldbyname('CLDH').Value;
      Outsoleborrow.DelDet.fieldbyname('LLQTY').Value:='0';
      Outsoleborrow.DelDet.fieldbyname('RKQTY').Value:='0';
      Outsoleborrow.DelDet.FieldByName('userID').Value:=main.edit1.text;
      Outsoleborrow.DelDet.FieldByName('userdate').Value:=Ndate;
      Outsoleborrow.DelDet.FieldByName('YN').Value:='1';
      Outsoleborrow.upDet.apply(ukinsert);
      //
      ...
      begin
        ...
        sql.add('where OSLLSS.LLNO='+''''+Outsoleborrow.DelMas.fieldbyname('LLNO').AsString+'''');
        sql.add('       and OSLLSS.CLBH='+''''+LastQuery.fieldbyname('CLDH').AsString+'''');
        execsql;
        ...
        begin
          ...
          begin
            active:=false;
            sql.Clear;
            sql.add('Select CLBH from #CLZLKC_Outsole ');
            sql.add('where #CLZLKC_Outsole.CLBH='''+LastQuery.fieldbyname('CLDH').AsString+''' and #CLZLKC_Outsole.SIZ='''+LastQuery.Fields[j].FieldName+''' ');
            active:=true;
            if RecordCount>0  then
            begin
              //
              ...
              sql.add('select '''+Outsoleborrow.DelMas.fieldbyname('LLNo').AsString+''',CLBH,''Outsole'' ,SIZ,'''+LastQuery.Fields[j].AsString+''','''+LastQuery.Fields[j].AsString+''',Qty');
              sql.add('       ,getdate() '  );
            ...
            begin
              ...
              sql.Clear;
              sql.add('INSERT into OSLLSS   ');
              sql.add('Values ('''+Outsoleborrow.DelMas.fieldbyname('LLNo').AsString+''','''+LastQuery.fieldbyname('CLDH').AsString+''',''Outsole'' ,'''+LastQuery.Fields[j].FieldName+''','''+LastQuery.Fields[j].AsString+''','''+LastQuery.Fields[j].AsString+''',0 ');
              sql.add('       ,getdate() '  );
              sql.add('       ,'''+main.edit1.text+''''  );
              sql.add('       ,''1'' '  );
              sql.add('       ,''VN'')  ');
    ...
    begin
      ...
      sql.add('                  from OSLLSS ');
      sql.add('                  WHERE 1=1');
      sql.add('                        and OSLLSS.LLNo='+''''+Outsoleborrow.DelDet.FieldByName('LLNO').Value+''''+' and OSLLSS.CLBH='+''''+Outsoleborrow.DelDet.FieldByName('CLBH').Value+'''' );
      sql.add('                  GROUP BY LLNo,CLBH),0) ');
      sql.add(' where OSLLS.LLNo='+''''+Outsoleborrow.DelDet.FieldByName('LLNo').Value+''''+ ' and OSLLS.CLBH='+''''+Outsoleborrow.DelDet.FieldByName('CLBH').Value+'''' );
      execsql;
      close;
    end;
    //
    Outsoleborrow.DelDet.active:=false;
    Outsoleborrow.DelDet.cachedupdates:=false;
    Outsoleborrow.DelDet.requestlive:=false;
    Outsoleborrow.DelDet.active:=true;
    //
    Showmessage('Success');
    Close;
  end;
...
procedure TOutsoleBorrow_RY.LastQueryAfterOpen(DataSet: TDataSet);
begin
...
procedure TOutsoleBorrow_RY.RYQueryAfterScroll(DataSet: TDataSet);
var i:integer;
  ...
  begin
    ...
    begin
      ...
      begin
        Active:=false;
        SQL.Clear;
        SQL.Add('select SMDDS.DDBH,Null as ''RY|Outsole'' ');
      ...
      SMDDSGridEh.columns[1].title.caption:='Size|Outsole';
...
procedure TOutsoleBorrow_RY.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
   Action:=caFree;
end;

procedure TOutsoleBorrow_RY.FormDestroy(Sender: TObject);
begin
  OutsoleBorrow_RY:=nil;
end;

procedure TOutsoleBorrow_RY.Excel1Click(Sender: TObject);
var
