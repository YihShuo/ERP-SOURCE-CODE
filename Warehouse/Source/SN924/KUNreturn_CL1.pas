...
var
  KUNreturn_CL: TKUNreturn_CL;

implementation

uses  LYV_KUNreturn1,FunUnit;

{$R *.dfm}

procedure TKUNreturn_CL.Edit4KeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
    button1Click(nil);
end;

procedure TKUNreturn_CL.Button1Click(Sender: TObject);
begin
  ...
  begin
    ...
    sql.Add('      and LTLL.SFL=''1'' '); 
    sql.add('      and  LTLLS.LLNO like '''+LLEdit.Text+'%'' ');
    sql.add('      and CLZL.YWPM like ''%'+LastEdit.Text+'%'+'''');
    sql.add('      and LTLL.YN=''5'' and LTLL.SB=''K'' ');
    sql.Add('      and LTLL.CKBH='''+LYV_KUNreturn.DelMas.fieldbyname('CKBH').AsString+''' ');
...
procedure TKUNreturn_CL.FormDestroy(Sender: TObject);
begin
  KUNreturn_CL:=nil;
end;

procedure TKUNreturn_CL.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=cafree;
end;

procedure TKUNreturn_CL.dbgrdh1GetCellParams(Sender: TObject;
...
procedure TKUNreturn_CL.dbgrdh1DblClick(Sender: TObject);
begin
...
procedure TKUNreturn_CL.Button2Click(Sender: TObject);
begin
  with Query2 do
  begin
    active:=false;
    ...
    sql.add('where CLDH like '''+LastNoEdit.Text+'%'+'''');
...
procedure TKUNreturn_CL.DBGrdh2DblClick(Sender: TObject);
begin
...
procedure TKUNreturn_CL.FormCreate(Sender: TObject);
begin
  DTP1.Date:=now-1;
  DTP2.Date:=now;
end;
