...
implementation
uses main1,Reality_Remain_Quantity1, untClipboard;

{$R *.dfm}

procedure TMaterialOrder.FormDestroy(Sender: TObject);
begin
   MaterialOrder:=nil;
end;

procedure TMaterialOrder.FormClose(Sender: TObject;
...
//多筆訂單編號查詢子SQL
function TMaterialOrder.GetMutiDDBHSubSQL(Str:String):String;
...
function TMaterialOrder.GetMutiBuyNoSubSQL(Str:String):String;
var tmpList:TStringlist;
   i:integer;
   TotalStr:String;
begin
...
procedure TMaterialOrder.btnQueryClick(Sender: TObject);
...
begin
  ...
  begin
    ...
    SQL.Add('select CLBH,YWPM,ZWPM,CQDH,DWBH,Sum(CLSL) as CLSL,Sum(CGQty) as CGQty,Sum(RKQty) as RKQty from (');
    SQL.Add('Select * from ( ');
    ...
    if CheckBox3.Checked=true then
    SQL.Add(' and RKZL.RKQty>0 ');
    SQL.Add(' ) MatList Group by CLBH,YWPM,ZWPM,CQDH,DWBH ) MatList ');
...
//
function TMaterialOrder.GetKCZKMemo():String;
var i:integer;
    Str:string;
begin
   Str:='';
...
//
procedure TMaterialOrder.DBGridEh1DblClick(Sender: TObject);
var Qty:double;
begin
...
procedure TMaterialOrder.btnCopyClick(Sender: TObject);
var bm:Tbookmark;
...
procedure TMaterialOrder.FormCreate(Sender: TObject);
begin
  main.FormLanguage(Pointer(self),self.Name);
  BaseDate.Date:=Date;
end;

procedure TMaterialOrder.EditPaste(EditObj:TEdit);
var
  CbBuf: PChar;
  BufSize: Integer;
  Handle: THandle;
...
procedure TMaterialOrder.edtDDBHKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TMaterialOrder.edtBUYNOKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if (ssCtrl in Shift) and (key = 86) then
  begin
...
procedure TMaterialOrder.Chk_DetClick(Sender: TObject);
begin
...
//計算可利庫
procedure TMaterialOrder.CalRemAvailQty();
var 
   year,month,day:word;
begin
  //找庫存相關的數據
...
procedure TMaterialOrder.Button1Click(Sender: TObject);
...
procedure TMaterialOrder.XLSBtnClick(Sender: TObject);
