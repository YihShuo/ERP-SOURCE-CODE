object DeliverConfirmTPM: TDeliverConfirmTPM
  Left = 684
  Top = 194
  Width = 1305
  Height = 675
  Caption = 'DeliverConfirmTPM'
  Color = clBtnFace
  ...
  object DelDet: TQuery
    ...
    SQL.Strings = (
        ...
        'SELECT KCLLS_TPM.*, CLZL.ZWPM, CLZL.YWPM, CLZL.DWBH, ISNULL(KCRK' +
        'S_TPM.Qty, 0) - ISNULL(YnKCLL.YnLLQty, 0) AS KCQty'
      'FROM KCLLS_TPM'
      'INNER JOIN CLZL ON CLZL.CLDH = KCLLS_TPM.CLBH'
      
        'LEFT JOIN (SELECT KCLLS_TPM.CLBH, Sum(KCLLS_TPM.Qty) AS YnLLQty ' +
        'FROM KCLLS_TPM, KCLL_TPM WHERE KCLLS_TPM.LLNO = KCLL_TPM.LLNO an' +
        'd KCLL_TPM.CFMID <> '#39'NO'#39' GROUP BY KCLLS_TPM.CLBH ) YnKCLL ON YnK' +
        'CLL.CLBH = KCLLS_TPM.CLBH'
      
        'LEFT JOIN (SELECT KCRKS_TPM.CLBH, SUM(KCRKS_TPM.Qty) AS Qty FROM' +
        ' KCRKS_TPM GROUP BY CLBH) KCRKS_TPM ON KCRKS_TPM.CLBH = KCLLS_TP' +
        'M.CLBH'
      'WHERE KCLLS_TPM.LLNO = :LLNO'
      'ORDER BY KCLLS_TPM.CLBH')
    UpdateObject = UPDet
    Left = 324
    Top = 340
    ParamData = <
