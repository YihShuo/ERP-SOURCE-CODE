...
var
  LYV_DAOborrowCFM: TLYV_DAOborrowCFM;
...
procedure TLYV_DAOborrowCFM.CalculateLastStock();
...
begin
  ...
  begin
    ...
    SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO  and LTRK.CKBH in (Select CKBH from LastKCCK where LB =''03'' and GSBH='''+main.Edit2.Text+''' )  and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+''' ');
    ...
    SQL.Add('left join ( ');
    SQL.Add('   select CKBH,CLBH,SIZ,QTY as LastRem from Lastmonth where CKBH in (Select CKBH from LastKCCK where LB =''03'' and GSBH='''+main.Edit2.Text+''' ) and KCYEAR='''+ayear+''' and KCMonth='''+amonth+''' ) as LastKC on NowKC.CKBH=LastKC.CKBH and NowKC.CLBH=LastKC.CLBH and NowKC.SIZ=LastKC.SIZ');
    SQL.Add('left join ( ');
    SQL.Add('  select LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ,SUM(LTRKSS.Qty) as RKQty from LTRKSS,LTRK ');
    SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO  and LTRK.CKBH in (Select CKBH from LastKCCK where LB =''03''  and GSBH='''+main.Edit2.Text+''' ) and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+''' ');
    ...
    SQL.Add('   where LTLLSS.LLNO=LTLL.LLNO  and LTLL.CFMID<>''NO'' and LTLL.CKBH in (Select CKBH from LastKCCK where LB =''03'' and GSBH='''+main.Edit2.Text+''' ) and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+''' ');
...
procedure TLYV_DAOborrowCFM.btn1Click(Sender: TObject);
var i:integer;
begin
  ...
  begin
    ...
    sql.add('where LTLL.SB =''D'' and LTLL.CFMID=''NO'' and isnull(LTLL.flowflag,'''')<>''X'' ');
...
procedure TLYV_DAOborrowCFM.FormClose(Sender: TObject;
...
procedure TLYV_DAOborrowCFM.FormDestroy(Sender: TObject);
begin
  LYV_DAOborrowCFM:=nil;
end;

procedure TLYV_DAOborrowCFM.FormCreate(Sender: TObject);
var year,month,day:word;
    i:integer;
begin
  decodedate(Date,Year,Month,Day);
  ...
  begin
    ...
    sql.add('where LTLLSS.LLNO=LTLL.LLNO  and LTLL.CFMID<>''NO'' and LTLL.CKBH in (Select CKBH from LastKCCK where LB =''03'' and GSBH='''+main.Edit2.Text+''' ) and LTLL.SFL=''1'' and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+'''  ');
    sql.add('Group by LTLL.DepID,LTLLSS.CLBH,LTLLSS.SIZ ');
    sql.add('Union All ');
    sql.add('select DepID,CLBH,SIZ,QTY as LastRem from Lastmonth_Out where CKBH in (Select CKBH from LastKCCK where LB =''03'' and GSBH='''+main.Edit2.Text+''' ) and  KCYEAR='''+ayear+''' and KCMonth='''+amonth+''') as tmpRKTLFL ');
    ...
    sql.add('where LTRKSS.RKNO=LTRK.RKNO  and LTRK.CKBH in (Select CKBH from LastKCCK where LB =''03'' and GSBH='''+main.Edit2.Text+''' ) and LTRK.SFL=''1'' and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+''' ');
...
procedure TLYV_DAOborrowCFM.btn2Click(Sender: TObject);
begin
...
procedure TLYV_DAOborrowCFM.btn3Click(Sender: TObject);
begin
...
procedure TLYV_DAOborrowCFM.Qtyrevise1Click(Sender: TObject);
begin
...
procedure TLYV_DAOborrowCFM.btn4Click(Sender: TObject);
