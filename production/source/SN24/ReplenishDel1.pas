...
begin
  ...
  begin
    ...
    begin
        ...
        begin
            edit;
            if FieldByName('CLSL_BOM').Value <> '0' then
              fieldbyname('Qty').Value:=Round((FieldByName('CLSL_BOM').AsFloat *BLMas.FieldByName('Qty').AsFloat + 1E-8) * 10) / 10
            else
              fieldbyname('Qty').Value:=trunc(fieldbyname('CLSL').Value/BLMas.fieldbyname('zlQty').Value*BLMas.fieldbyname('Qty').Value*10+0.99)/10;
            if FieldByName('Qty').Value < 0.1 then FieldByName('Qty').Value := 0.1;
            if BLMas.FieldByName('zlQty').Value = BLMas.FieldByName('Qty').Value then FieldByName('Qty').Value := FieldByName('CLSL').Value;
            post;
        end;
        next;
    end;
