            ...
            begin
              ...
              with Qtemp do
                begin
                  ...
                  sql.add('    ,SB='+''''+'3'+'''');
                  sql.add('    ,CON_NO=' + QuotedStr(Trim(Edit2.Text)));
                  sql.add('    ,BIEN_SO=' + QuotedStr(Trim(Edit5.Text)));
...
var s,rg,PlanPlateID, PlanConNo:string;
    ogw:real;
    IsPlanNull: Boolean;
begin
...
if key=#13 then
  begin
    ...
    //    update 07/07/2026 kiem tra xem bsx va con co dung vs shiping plant
    with qtemp2 do
      begin
        Active := False;
        SQL.Clear;
        SQL.Add('SELECT A.PlateID, A.Con_no, A.cartonno ');
        SQL.Add('FROM ywcp ');
        SQL.Add('LEFT JOIN ShippingPlanCarton A ON A.ry = ywcp.ddbh AND A.cartonno = ywcp.cartonno ');
        SQL.Add('WHERE ywcp.cartonbar = :CartonBar ');
        ParamByName('CartonBar').AsString := Trim(Edit1.Text);
        Active := True;
      end;
      IsPlanNull := qtemp2.FieldByName('cartonno').IsNull;
      PlanPlateID := qtemp2.FieldByName('PlateID').AsString;
      PlanConNo := qtemp2.FieldByName('Con_no').AsString;

      qtemp2.Active := False;
      if not IsPlanNull then
      begin
        if (UpperCase(PlanPlateID) <> UpperCase(Trim(Edit5.Text))) or 
           (UpperCase(PlanConNo) <> UpperCase(Trim(Edit2.Text))) then
        begin
          Windows.Beep(1500, 500);
          ShowMessage('Carton nay da co ke hoach nhung sai Bien so xe hoac so Cont! Vui long kiem tra lai.');
          Edit1.Text := '';
          Abort;
        end;
      end;
