...
procedure TJGNOConfirm3.ReadIni();
  var MyIni :Tinifile;
      AppDir:string;
begin
  //
...
procedure TJGNOConfirm3.Button1Click(Sender: TObject);
begin
  ...
  begin
    ...
    SQL.Add('where  IsNull(JGZL_DK_CFM.CFMID1,''NO'')<>''NO''  and JGZL.JGNO like '''+Edit1.Text+'%'' ');
    ...
    if CheckBox1.Checked = true then
      SQL.Add('      and IsNull(JGZL_DK_CFM.CFMID2,''NO'')<>''NO'' ')
    else
      SQL.Add('      and IsNull(JGZL_DK_CFM.CFMID2,''NO'')=''NO'' ');
...
procedure TJGNOConfirm3.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=CaFree;
end;

procedure TJGNOConfirm3.FormDestroy(Sender: TObject);
begin
  JGNOConfirm3:=nil;
end;

procedure TJGNOConfirm3.FormCreate(Sender: TObject);
begin
  AppDir:=ExtractFilePath(Application.ExeName);
  with TmpQry do
  begin
...
procedure TJGNOConfirm3.Button3Click(Sender: TObject);
begin
      ...
      begin
        ...
        begin
           ...
           begin
             ...
             SQL.Add('Begin');
             SQL.Add('  Insert into JGZL_DK_CFM (JGNO, CFMID2, CFMDATE2) Values ('''+JGMas.FieldByName('JGNO').AsString+''','''+main.Edit1.Text+''',GetDate())');
             SQL.Add('End Else');
             SQL.Add('Begin');
             SQL.Add('  Update JGZL_DK_CFM Set CFMID2='''+main.Edit1.Text+''' ,CFMDate2=GetDate() where JGNO='''+JGMas.FieldByName('JGNO').AsString+''' ');
             SQL.Add('End');
             ExecSQL();
           end;
        end;
...
procedure TJGNOConfirm3.Button4Click(Sender: TObject);
begin

  if JGMas.fieldbyname('CFMID2').Value<>'NO' then
  begin
    if (Date()-JGMas.fieldbyname('CFMDATE2').Value)>7 then
    begin
      Showmessage('Already Confirm Permit >7,can''t reset ');
      Exit;
    end;
  end;
  try
    ...
    begin
      ...
      begin
         ...
         SQL.Add('IF NOT EXISTS (Select JGNO from JGZL_DK_CFM where JGNO='''+JGMas.FieldByName('JGNO').AsString+''')');
         SQL.Add('Begin');
         SQL.Add('  Insert into JGZL_DK_CFM (JGNO, CFMID2, CFMDATE2) Values ('''+JGMas.FieldByName('JGNO').AsString+''',''NO'',GetDate())');
         SQL.Add('End Else');
         SQL.Add('Begin');
         SQL.Add('  Update JGZL_DK_CFM Set CFMID2=''NO'' ,CFMDate2=GetDate() where JGNO='''+JGMas.FieldByName('JGNO').AsString+''' ');
         SQL.Add('End');
         ExecSQL();
      end;
      active:=false;
...
procedure TJGNOConfirm3.Button5Click(Sender: TObject);
begin
  if JGMas.Active=false then Exit;
  if JGMas.RecordCount=0 then Exit;
  if JGDet.Active=false then Exit;
...
procedure TJGNOConfirm3.CGDetCalcFields(DataSet: TDataSet);
begin
