...
procedure TJGNOConfirm2.ReadIni();
  var MyIni :Tinifile;
      AppDir:string;
begin
  //
...
procedure TJGNOConfirm2.Button1Click(Sender: TObject);
begin
  ...
  begin
    ...
    SQL.Add('where  IsNull(JGZL_DK_CFM.CFMID,''NO'')<>''NO''  and JGZL.JGNO like '''+Edit1.Text+'%'' ');
    ...
    if CheckBox1.Checked = true then
      SQL.Add('      and IsNull(JGZL_DK_CFM.CFMID1,''NO'')<>''NO'' ')
    else
      SQL.Add('      and IsNull(JGZL_DK_CFM.CFMID1,''NO'')=''NO'' ');
...
procedure TJGNOConfirm2.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=CaFree;
end;

procedure TJGNOConfirm2.FormDestroy(Sender: TObject);
begin
  JGNOConfirm2:=nil;
end;

procedure TJGNOConfirm2.FormCreate(Sender: TObject);
begin
  AppDir:=ExtractFilePath(Application.ExeName);
  with TmpQry do
  begin
...
procedure TJGNOConfirm2.Button3Click(Sender: TObject);
begin
      ...
      begin
        ...
        begin
           ...
           begin
             ...
             SQL.Add('Begin');
             SQL.Add('  Insert into JGZL_DK_CFM (JGNO, CFMID1, CFMDATE1) Values ('''+JGMas.FieldByName('JGNO').AsString+''','''+main.Edit1.Text+''',GetDate())');
             SQL.Add('End Else');
             SQL.Add('Begin');
             SQL.Add('  Update JGZL_DK_CFM Set CFMID1='''+main.Edit1.Text+''' ,CFMDate1=GetDate() where JGNO='''+JGMas.FieldByName('JGNO').AsString+''' ');
             SQL.Add('End');
             ExecSQL();
           end;
        end;
...
procedure TJGNOConfirm2.Button4Click(Sender: TObject);
begin
  ...
  try
    ...
    begin
      ...
      begin
         ...
         SQL.Add('IF NOT EXISTS (Select JGNO from JGZL_DK_CFM where JGNO='''+JGMas.FieldByName('JGNO').AsString+''')');
         SQL.Add('Begin');
         SQL.Add('  Insert into JGZL_DK_CFM (JGNO, CFMID1, CFMDATE1) Values ('''+JGMas.FieldByName('JGNO').AsString+''',''NO'',GetDate())');
         SQL.Add('End Else');
         SQL.Add('Begin');
         SQL.Add('  Update JGZL_DK_CFM Set CFMID1=''NO'' ,CFMDate1=GetDate() where JGNO='''+JGMas.FieldByName('JGNO').AsString+''' ');
         SQL.Add('End');
         ExecSQL();
      end;
      active:=false;
...
procedure TJGNOConfirm2.Button5Click(Sender: TObject);
begin
  if JGMas.Active=false then Exit;
  if JGMas.RecordCount=0 then Exit;
  if JGDet.Active=false then Exit;
...
procedure TJGNOConfirm2.CGDetCalcFields(DataSet: TDataSet);
begin
