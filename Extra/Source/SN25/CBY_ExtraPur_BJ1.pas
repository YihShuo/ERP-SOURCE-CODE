...
var
  CBY_ExtraPur_BJ: TCBY_ExtraPur_BJ;

implementation

uses
  CBY_ExtraPur1, Quotation1, main1, ExchExtra1;

{$R *.dfm}

procedure TCBY_ExtraPur_BJ.Button1Click(Sender: TObject);
...
begin
  ...
  TCurrencyField(query1.FieldByName('VNPrice')).DisplayFormat := CBY_ExtraPur.VNPrice_DiplayFormat;
  TCurrencyField(query1.FieldByName('CWHL')).DisplayFormat := CBY_ExtraPur.VNPrice_DiplayFormat;
...
procedure TCBY_ExtraPur_BJ.Button2Click(Sender: TObject);
begin
...
procedure TCBY_ExtraPur_BJ.FormDestroy(Sender: TObject);
begin
  CBY_ExtraPur_BJ := Nil;
end;

procedure TCBY_ExtraPur_BJ.FormClose(Sender: TObject;
...
procedure TCBY_ExtraPur_BJ.DBGridEh1DblClick(Sender: TObject);
var
  i,CWHL: integer;
  CLBH: string;
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            fieldbyname('VNPrice').value:=ERPTrunc(query1.fieldbyname('USprice').value*CWHL*query1.FieldByName('Discount').Value,-CBY_ExtraPur.VNPrice_Decimal);
          end else
          begin
            fieldbyname('VNPrice').value:=ERPTrunc(query1.fieldbyname('USprice').value*CWHL,-CBY_ExtraPur.VNPrice_Decimal);
          end;
        end else
        begin
          if (query1.FieldByName('Discount').Value<>1)  then
          begin
            ...
            fieldbyname('VNPrice').value:=ERPTrunc_Down(query1.fieldbyname('VNPrice').value*query1.FieldByName('Discount').Value,CBY_ExtraPur.VNPrice_Decimal);
          end else
          begin
...
procedure TCBY_ExtraPur_BJ.DBGridEh1KeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
  DBGridEh1dblclick(nil);
end;

procedure TCBY_ExtraPur_BJ.ReadIni();
var MyIni :Tinifile;
    AppDir:string;
begin
  //
...
procedure TCBY_ExtraPur_BJ.FormCreate(Sender: TObject);
begin
  ReadIni();
end;

procedure TCBY_ExtraPur_BJ.FormShow(Sender: TObject);
begin
  main.FormLanguage(Pointer(self),self.Name);
end;

procedure TCBY_ExtraPur_BJ.Query1AfterOpen(DataSet: TDataSet);
var y1,m1,d1:string;
    y,m,d:word;
begin
...
procedure TCBY_ExtraPur_BJ.DBGridEh1EditButtonClick(Sender: TObject);
begin
...
procedure TCBY_ExtraPur_BJ.DBGridEh1DrawColumnCell(Sender: TObject;
